...
def validate_timestamp(created_at: str | None) -> str | None:
    """
    Validate and return datetime string.

    :param created_at: The raw string from cloud-specific tags/metadata
    :return: The validated ISO 8601 datetime string, or None if not found or invalid
    :raises CODException: If the created_at string exists but is not in valid ISO 8601 format
    """
    if not created_at:
        return None
    ...
    try:
        ...
        raise CODException(
            f"Invalid ISO date format: '{created_at}'. "
            f"Expected ISO 8601 format (e.g., 2025-10-29T17:30:43.737648Z).",
            caused_by=e
        ) from e
...
def update_file_inplace(path: str, old: str, new: str, plain_text: bool = False) -> None:
    try:
        ...
        if plain_text:
            ...
            # Use regex replacement - old is treated as a regex pattern
            modified_content = re.sub(old, new, content)

        with open(path, 'w') as f:
            f.write(modified_content)

    except Exception as e:
        log.warning(f"Failed to perform substitution in {path}: {e}")
        raise
...
def _define_dev_repository(repo: str, repo_type: str, bcm_distro: str, bcm_version: str) -> None:
    ...
    if bcm_distro.lower().startswith("ubuntu"):
        ...
        update_file_inplace(repo, major_version, bcm_version, plain_text=True)
