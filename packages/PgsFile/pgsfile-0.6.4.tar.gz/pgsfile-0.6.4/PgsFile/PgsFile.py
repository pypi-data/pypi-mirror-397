...
def get_data_text(path):
    """
    Reads a text file with automatic encoding detection and proper Unicode handling.
    ...
    -------
    text : str or None
        Decoded text content from the file, or None if it cannot be read.
    """
    ...
    with open(path, 'rb') as f:
        raw_sample = f.read(8192)  # Read first 8KB for detection
        detection = chardet.detect(raw_sample)
        detected_encoding = detection.get('encoding')

    # Build a priority list of encodings to try
    # (UTF-8 variants first, then common local encodings)
    candidate_encodings = []
    if detected_encoding:
        candidate_encodings.append(detected_encoding)
    candidate_encodings += [
        'utf-8-sig', 'utf-8', 'utf-16', 'utf-32',
        'windows-1252', 'latin1', 'gb18030', 'big5'
    ]

    # Remove duplicates while preserving order
    seen = set()
    encodings_to_try = [e for e in candidate_encodings if not (e in seen or seen.add(e))]

    # Try each encoding until one works cleanly
    for enc in encodings_to_try:
        try:
            with open(path, 'r', encoding=enc) as f:
                text = f.read()
            # Quick heuristic: if too many � replacement chars, try next
...
def get_data_lines(path, no_line_breaks=False):
    """
    Reads a text file into lines, with automatic encoding detection and support for common encodings.
    ...
    no_line_breaks : bool, optional
        If True, keeps the original line breaks in the output.
        If False (default), strips empty and whitespace-only lines.

    Returns
    -------
    lines : list[str] or None
        A list of text lines or paragraphs from the file.
        Returns None if the file cannot be read.
    """
    if not os.path.isfile(path):
        return None

    # --- Step 1: Detect encoding from a sample ---
    with open(path, 'rb') as f:
        raw_sample = f.read(8192)  # first 8KB
        detection = chardet.detect(raw_sample)
        detected_encoding = detection.get('encoding')

    # --- Step 2: Build encoding candidate list ---
    candidate_encodings = []
    if detected_encoding:
        candidate_encodings.append(detected_encoding)
    candidate_encodings += [
    ...
    # --- Step 3: Try reading with each encoding ---
    for enc in encodings_to_try:
        try:
            with open(path, 'r', encoding=enc) as f:
                lines = f.readlines()

            # Heuristic check: too many replacement characters
            if sum(line.count('\ufffd') for line in lines) > len(lines) * 0.01:
                continue
