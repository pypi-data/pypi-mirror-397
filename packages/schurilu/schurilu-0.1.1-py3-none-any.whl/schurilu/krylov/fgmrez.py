...
def fgmrez(
    ...
    """
    Flexible GMRES for solving Ax = b (complex arithmetic).

    FGMREZ allows the preconditioner M to vary at each iteration,
    which is useful for nonlinear preconditioners. Uses conjugate
    inner products appropriate for complex vectors.

    Parameters
    ----------
    A : sparse matrix or LinearOperator
        The system matrix (can be complex).
    b : ndarray
        Right-hand side vector (can be complex).
    x0 : ndarray, optional
    ...
    dtype = np.complex128 if b.dtype in [np.complex64, np.complex128] else np.complex128
    ...
    while total_iter < maxiter:
        ...
        for j in range(m):
            ...
            # Modified Gram-Schmidt orthogonalization with conjugate inner product
            for i in range(j + 1):
                H[i, j] = np.vdot(V[:, i], w)  # Conjugate inner product
                w = w - H[i, j] * V[:, i]
            ...
            for i in range(j):
                temp = np.conj(cs[i]) * H[i, j] + np.conj(sn[i]) * H[i + 1, j]
                H[i + 1, j] = -sn[i] * H[i, j] + cs[i] * H[i + 1, j]
                H[i, j] = temp

            # Compute new Givens rotation (complex version)
            denom = np.sqrt(abs(H[j, j])**2 + abs(H[j + 1, j])**2)
            cs[j] = H[j, j] / denom
            sn[j] = H[j + 1, j] / denom

            # Apply Givens rotation to H and g
            H[j, j] = np.conj(cs[j]) * H[j, j] + np.conj(sn[j]) * H[j + 1, j]
            H[j + 1, j] = 0.0

            g[j + 1] = -sn[j] * g[j]
            g[j] = np.conj(cs[j]) * g[j]
