...
# Option 1: High-level CARLoader with directory path (recommended)
loader = carlib.CARLoader("/path/to/car/files", batch_size=32, shuffle=True, modality="audio")

# Option 2: Standard PyTorch DataLoader with custom collate (most reliable)
...
def collate_car_batch(batch):
    ...
    return {
        'data': {
            # Stack tensors (audio: 'codes', image: 'encoded_latents')  
            'codes': torch.stack([item['data']['codes'] for item in batch]),
            # Keep non-tensors as lists
            'compression_ratio': [item['data']['compression_ratio'] for item in batch],
            'model_name': [item['data']['model_name'] for item in batch]
        },
        'metadata': [item['metadata'] for item in batch],
        'file_paths': [item['file_path'] for item in batch]
    }

loader = DataLoader(dataset, batch_size=32, shuffle=True, collate_fn=collate_car_batch)

for batch in loader:
    # Access encoded tokens/features for training
    audio_codes = batch['data']['codes']      # Shape: [batch, seq_len] for audio
...
# High-performance approach with custom collate function (recommended)
dataset = carlib.CARDataset(
    car_dir="/path/to/car/files",
    modality="audio",           # Filter by modality
    cache_in_memory=True,       # Smart LRU caching for performance
...
def collate_car_batch(batch):
    """Custom collate function that handles mixed data types properly"""
    # Determine data keys dynamically
    data_dict = {}
    first_item = batch[0]['data']
    
    for key in first_item.keys():
        values = [item['data'][key] for item in batch]
        
        # Stack tensors, keep non-tensors as lists
        if isinstance(values[0], torch.Tensor):
            try:
                data_dict[key] = torch.stack(values)
            except RuntimeError:
                # Different tensor shapes - keep as list
                data_dict[key] = values
        else:
            # Non-tensor data (scalars, strings, dicts)
            data_dict[key] = values
...
# Alternative: Use CARLoader if available and working
try:
    loader = carlib.CARLoader("/path/to/car/files", batch_size=32, shuffle=True, modality="audio")
except:
    # Fallback to standard DataLoader with custom collate
    loader = dataloader
...
for epoch in range(num_epochs):
    for batch in loader:
        ...
        if hasattr(loader, 'to_device'):
            # CARLoader has built-in GPU transfer
            batch = loader.to_device(batch, device, non_blocking=True)
        else:
            # Manual GPU transfer for standard DataLoader
            for key, value in batch['data'].items():
                if isinstance(value, torch.Tensor):
                    batch['data'][key] = value.to(device, non_blocking=True)
        
        # Access encoded data - different modalities have different keys:
        ...
        if 'codes' in encoded_data:              # Audio (EnCodec/DAC/SNAC)
            ...
            tokens = encoded_data[tensor_keys[0]]  # Use first tensor found
        
        print(f"Processing {tokens.shape} tokens")  # Debug: show tensor shape
        ...
        # batch['data']['model_name']         # List of model names used
```
