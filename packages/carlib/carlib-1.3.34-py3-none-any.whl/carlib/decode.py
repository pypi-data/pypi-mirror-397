...
def decode_car_file(
    ...
    target_modality = metadata.get('target_modality') or metadata.get('media_type')
    if not target_modality:
        raise ValueError("CAR file metadata missing 'target_modality' or 'media_type'")
    ...
    for car_path in car_files:
        try:
            ...
            if modality:
                ...
                if (temp_result['metadata'].get('target_modality') or temp_result['metadata'].get('media_type')) != modality:
                    continue
            ...
            temp_result = load_single_car(car_path, framework="pytorch") 
            target_modality = temp_result['metadata'].get('target_modality') or temp_result['metadata'].get('media_type')
