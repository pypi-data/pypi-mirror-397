...
class DepthwiseBufferTempConv(Layer):
    """This represents the Akida DepthwiseBufferTempConv layer.
    ...
    perform a depthwise convolution like between the buffered inputs and the layer
    kernel.
    ...
    The BufferTempConv depthwise convolution like operation can be described as follows:

        >>> fifo = fifo_op(inputs)
        >>> fifo = fifo << input_shift
        >>> output = elementwise_mult(fifo, weights)
        >>> output = reduce_sum(output, axis=-2)
    ...
    def __init__(self,
        ...
        try:
            params = LayerParams(
                LayerType.DepthwiseBufferTempConv, {
                    "fifo_size": fifo_size,
