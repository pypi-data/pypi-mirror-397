...
class InMemoryRepository(Repository[T]):
    ...
    def save(self, entity: T) -> Self:  # Returns Self for chaining
...
repo.save(User("1", "Alice")).save(User("2", "Bob"))  # Method chaining!
```

### Combining Self with Node Pattern

```python
from typing import Self

class Node:
    """Tree node with Self type."""
    ...
    def add_child(self, child: Self) -> Self:
        """Add child node and return self for chaining."""
        self.children.append(child)
        child.parent = self
        return self

    def find(self, predicate: Callable[[int], bool]) -> Self | None:
...
root = Node(1)
root.add_child(Node(2)).add_child(Node(3))
