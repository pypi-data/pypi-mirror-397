...
class VersionCheckThread(QThread):
    ...
    def run(self):
        ...
        try:
            import requests
            resp = requests.get('https://pypi.org/pypi/feedback-mcp/json', timeout=30)
...
class FeedbackUI(QMainWindow):
    ...
    def _start_version_check(self):
        ...
        try:
            ...
            # 创建并启动版本检查线程
            self.version_check_thread = VersionCheckThread(current_version, self)
            # 连接信号到槽函数
...
初始化完成后，请重新打开此界面以使用完整功能。"""
    ...
    def _handle_command_execution(self, command_content: str):
        ...
        if command_content:
            # 保存到历史记录
            if self.chat_tab and hasattr(self.chat_tab, 'save_to_history'):
                try:
                    history_content = f"💻 用户输入的指令（立即执行）：\n{command_content}"
                    self.chat_tab.save_to_history(history_content, 'sent')
                except Exception as e:
                    logger = get_debug_logger()
                    logger.log_warning(f"Failed to save command to history: {e}", "UI")
    ...
    def _execute_option_immediately(self, option_index: int):
        ...
        if 0 <= option_index < len(self.predefined_options):
            ...
            if self.chat_tab and hasattr(self.chat_tab, 'save_to_history'):
                try:
                    history_content = f"🔘 用户选择的选项（立即执行）：\n{option_text}"
                    self.chat_tab.save_to_history(history_content, 'sent')
                except Exception as e:
                    logger = get_debug_logger()
                    logger.log_warning(f"Failed to save option to history: {e}", "UI")
    ...
    def closeEvent(self, event):
        ...
        if self.version_check_thread and self.version_check_thread.isRunning():
            ...
            self.version_check_thread.wait(1000)  # 最多等待1秒
