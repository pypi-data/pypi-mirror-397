...
class ChatTab(BaseTab):
    ...
    def _submit_feedback(self):
        ...
        if selected_options:
            history_content_parts.append(f"🔘 用户选择的选项：\n{'; '.join(selected_options)}")
        if selected_command_content:
            history_content_parts.append(f"💻 用户输入的指令：\n{selected_command_content}")
        if text_content:
            history_content_parts.append(f"💬 用户输入：\n{text_content}")

        # 如果有任何内容，保存到历史记录
        if history_content_parts:
            full_content = "\n\n".join(history_content_parts)
            self.save_to_history(full_content, 'sent')
    ...
    def save_to_history(self, content: str, message_type: str = 'sent', ai_response: str = None) -> bool:
        ...
        try:
            ...
            existing_data['dialogues'].append(new_record)
    ...
    def save_input_to_history(self):
        ...
        # 如果有任何内容，保存到历史记录（标记为未提交）
        if history_content_parts:
            full_content = "\n\n".join(history_content_parts)
            # 添加未提交标记
            full_content = f"⚠️ 【未提交 - 超时/关闭时自动保存】\n\n{full_content}"
            self.save_to_history(full_content, 'draft')
