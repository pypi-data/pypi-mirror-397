...
class Speaker:
    """
    17来麦语音平台的客户端类，用于发送通知消息。
    
    该类提供了同步和异步两种方式与17来麦语音平台进行交互，支持自定义配置和参数。
    """
    def __init__(
            self,
    ...
    def notify(self, client: httpx.Client=None, message: str = "", **kwargs):
        """
        发送同步通知消息。
    ...
    async def async_notify(self, client: httpx.AsyncClient=None, message: str = "", **kwargs):
        """
        发送异步通知消息。
