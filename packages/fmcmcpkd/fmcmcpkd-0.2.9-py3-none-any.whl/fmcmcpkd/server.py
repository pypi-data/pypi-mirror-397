...
def _map_prefix_to_fmc(object_name: str) -> Optional[str]:
    ...
    if object_name.upper().startswith("ATH-"):
        return "Athens-FMC"
...
# --- Section tools ---
@mcp.tool()
def get_server_version(fmc_name: Optional[str] = None) -> str:
    """Use this tool when asked 'what is the FMC server version?'.
    Always provide an FMC name if multiple FMCs exist. If none is provided, prompt the user to specify one."""
...
def get_nat_policies(fmc_name: Optional[str] = None) -> str:
    ...
    Always provide an FMC name if multiple FMCs exist. If none is provided, prompt the user to specify one."""
    return _read_fmc_sections(fmc_name).get("NAT Policies", "Section not found")
...
def get_object_ip(object_name: str, fmc_name: Optional[str] = None) -> str:
    ...
    Always provide an FMC name if multiple FMCs exist. If none is provided, prompt the user to specify one.
    If the object name starts with 'ATH-', auto‑maps to Athens FMC if available."""
    sections = _read_fmc_sections(fmc_name or _map_prefix_to_fmc(object_name))
