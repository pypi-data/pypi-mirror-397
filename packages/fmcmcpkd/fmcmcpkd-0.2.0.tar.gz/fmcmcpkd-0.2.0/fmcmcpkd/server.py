...
def _get_fmc_file(fmc_name: str) -> str:
    ...
    # explicit aliases only when the whole name matches
...
def _read_fmc_sections(fmc_name: str) -> dict:
    ...
    # Split by section headers === Section Name === (flexible spacing)
    sections = re.split(r"===\s*(.*?)\s*===", content)
    parsed = {}
    for i in range(1, len(sections), 2):
        header = sections[i].strip().lower()   # normalize to lowercase
...
def get_fmc_info(fmc_name: str, section_name: str) -> str:
    ...
    Example: get_fmc_info("US FMC", "Access Policies")
             get_fmc_info("HQ", "Access Policies")
...
SECTION_NAMES = [
    ...
    "Prefilter Policies",
    "Access Policies",
    "NAT Polices",          # match header spelling
    "FMC Objects",          # match header spelling
    "FMC Network Groups",   # new header added
    "FMC Network Objects",  # alias tool, resolves to FMC Objects
]
