...
def get_object_ip(object_name: str, fmc_name: str = None) -> str:
    """Return the IP address for a given FMC object name by searching the FMC Objects section."""
    sections = _read_fmc_sections(fmc_name)
    objects = sections.get("FMC Objects", "")
    # Regex to find IPv4 after the object name
    match = re.search(rf"{re.escape(object_name)}\s*[:=]\s*(\d+\.\d+\.\d+\.\d+)", objects)
    return match.group(1) if match else f"Object '{object_name}' not found or has no IP"
