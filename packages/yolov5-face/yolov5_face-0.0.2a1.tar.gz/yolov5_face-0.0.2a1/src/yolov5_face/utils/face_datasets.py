        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(labels_arr):
                        assert (
                            labels_arr.shape[1] == 15
                        ), "labels require 15 columns each"
                        assert (labels_arr >= -1).all(), "negative labels"
                        assert (
                            labels_arr[:, 1:] <= 1
                        ...
                        ne += 1  # label empty
                        labels_arr = np.zeros((0, 15), dtype=np.float32)
                else:
                    nm += 1  # label missing
                    labels_arr = np.zeros((0, 15), dtype=np.float32)
...
def random_perspective(
    ...
    if n:
        ...
        ].reshape(n * 9, 2)  # x1y1, x2y2, x1y2, x2y1
        xy = xy @ M.T  # transform
        if perspective:
