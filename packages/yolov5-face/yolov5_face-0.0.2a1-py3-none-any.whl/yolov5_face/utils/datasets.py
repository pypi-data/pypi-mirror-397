...
class LoadImagesAndLabels(Dataset):  # for training/testing
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(labels_arr):
                        assert labels_arr.shape[1] == 5, "labels require 5 columns each"
                        assert (labels_arr >= 0).all(), "negative labels"
                        assert (
                            labels_arr[:, 1:] <= 1
                        ).all(), "non-normalized or out of bounds coordinate labels"
                        assert (
                            np.unique(labels_arr, axis=0).shape[0]
                            == labels_arr.shape[0]
                        ), "duplicate labels"
                    else:
                        ne += 1  # label empty
    ...
    def collate_fn(batch):
        ...
        for i, labels_i in enumerate(label):
            labels_i[:, 0] = i  # add target image index for build_targets()
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            img4.append(im)
            label4.append(labels_i)

        for i, labels_i in enumerate(label4):
            labels_i[:, 0] = i  # add target image index for build_targets()
...
def load_mosaic9(self, index):
    ...
    hp, wp = 0, 0  # height, width previous
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w) = load_image(self, index)
