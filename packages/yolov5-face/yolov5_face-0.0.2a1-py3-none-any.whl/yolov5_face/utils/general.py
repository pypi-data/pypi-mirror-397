...
def intersect(box_a, box_b):
    """Compute intersection area of two sets of (x1, y1, x2, y2) boxes.

    Args:
        box_a: Tensor[batch, A, 4]
        box_b: Tensor[batch, B, 4]

    Returns:
        Tensor[batch, A, B] of intersection areas.
    """

    a = box_a.size(1)
    b = box_b.size(1)
    max_xy = torch.min(
        box_a[:, :, 2:].unsqueeze(2).expand(-1, a, b, 2),
...
def non_max_suppression_face(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            labels_i = labels[xi]
            v = torch.zeros((len(labels_i), nc + 15), device=x.device)
            v[:, :4] = labels_i[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(labels_i)), labels_i[:, 0].long() + 15] = 1.0  # cls
            x = torch.cat((x, v), 0)
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            labels_i = labels[xi]
            v = torch.zeros((len(labels_i), nc + 5), device=x.device)
            v[:, :4] = labels_i[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(labels_i)), labels_i[:, 0].long() + 5] = 1.0  # cls
            x = torch.cat((x, v), 0)
