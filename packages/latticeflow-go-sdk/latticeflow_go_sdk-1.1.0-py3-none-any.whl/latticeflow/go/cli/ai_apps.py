...
def _create(
    ...
    for config_file in config_files:
        try:
            ...
            ai_apps_map.update_entity(new_stored_ai_app)
        except Exception as error:
            failures += 1
...
def _get_cli_ai_app_from_file(config_file: Path) -> CLICreateAIApp:
    try:
        return CLICreateAIApp.model_validate(
...
def create_or_update_single_ai_app(
    client: Client, ai_app: AIApp, stored_ai_app: StoredAIApp | None
) -> StoredAIApp:
    if stored_ai_app and is_update_payload_same_as_existing_entity(
        ai_app, stored_ai_app
    ):
        cli_print.log_no_change_info(PRETTY_ENTITY_NAME, ai_app.key)
        return stored_ai_app
    elif stored_ai_app:
        return update_single_ai_app(client, stored_ai_app, ai_app)
    else:
        return create_single_ai_app(client, ai_app)


def update_single_ai_app(
    client: Client, stored_ai_app: StoredAIApp, ai_app: AIApp
) -> StoredAIApp:
    return update_single_entity(
        PRETTY_ENTITY_NAME,
        lambda: client.ai_apps.update_ai_app(stored_ai_app.id, ai_app),
        stored_ai_app.key,
    )


def create_single_ai_app(client: Client, ai_app: AIApp) -> StoredAIApp:
    return create_single_entity(
        PRETTY_ENTITY_NAME, lambda: client.ai_apps.create_ai_app(ai_app), ai_app.key
    )
