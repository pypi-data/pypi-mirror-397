...
def _create(
    ...
    try:
        ...
        if stored_user and is_update_payload_same_as_existing_entity(user, stored_user):
            cli_print.log_no_change_info(PRETTY_ENTITY_NAME, user.email, "email")
        elif stored_user:
...
def update_single_user(
    client: Client, stored_user: StoredUser, user: User
) -> StoredUser:
    return update_single_entity(
        PRETTY_ENTITY_NAME,
        lambda: client.users.update_user(stored_user.id, user),
        stored_user.email,
        "email",
    )


def create_single_user(client: Client, user: User) -> StoredUser:
    return create_single_entity(
        PRETTY_ENTITY_NAME, lambda: client.users.create_user(user), user.email, "email"
    )
