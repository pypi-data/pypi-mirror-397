...
@tenant_app.command("create-user")
def _create_user(
    ...
    try:
        ...
        cli_print.log_create_success_info("user", stored_user.email, "email")
    except Exception as error:
...
def create_single_tenant(client: Client, tenant: Tenant) -> StoredTenant:
    return create_single_entity(
        PRETTY_ENTITY_NAME,
        lambda: client.tenants.create_tenant(tenant),
        tenant.alias,
        "alias",
    )
