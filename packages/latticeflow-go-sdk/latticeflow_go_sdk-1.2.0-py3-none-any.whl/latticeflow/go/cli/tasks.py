...
def _get_cli_task_from_file(config_file: Path) -> CLICreateTask:
    try:
        return CLICreateTask.model_validate(
...
def create_or_update_single_task(
    client: Client, task: Task, stored_task: StoredTask | None
) -> StoredTask:
    if stored_task and is_update_payload_same_as_existing_entity(task, stored_task):
        cli_print.log_no_change_info(PRETTY_ENTITY_NAME, task.key)
        return stored_task
    elif stored_task:
        return update_single_task(client, stored_task, task)
    else:
        return create_single_task(client, task)


def update_single_task(
    client: Client, stored_task: StoredTask, task: Task
) -> StoredTask:
    return update_single_entity(
        PRETTY_ENTITY_NAME,
        lambda: client.tasks.update_task(
...
def create_single_task(client: Client, task: Task) -> StoredTask:
    return create_single_entity(
        PRETTY_ENTITY_NAME, lambda: client.tasks.create_task(task), task.key
    )
