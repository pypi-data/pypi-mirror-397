...
def _add(
    ...
    for config_file in config_files:
        try:
            cli_dataset = _get_cli_dataset_from_file(config_file)
...
def _generation_preview(
    ...
    try:
        cli_dataset = _get_cli_dataset_from_file(path)
    ...
    try:
        ...
        preview_data = client.dataset_generators.preview_dataset_generation(
        ...
        cli_print.print_table(
            ...
            [
                ...
                for column_name in preview_data.column_names
            ],
        )
    except Exception as error:
        raise cli_exc.CLIGenerateDatasetPreviewFromPathError(path) from error
...
def _get_cli_dataset_from_file(path: Path) -> CLICreateDataset:
    try:
        return CLICreateDataset.model_validate(
            load_yaml_recursively(path), ignore_extra=False
        )
    except Exception as error:
...
def create_or_update_single_dataset(
    ...
    if (
        ...
        cli_print.log_no_change_info(PRETTY_ENTITY_NAME, dataset.key)
...
def _create_single_dataset(
    ...
    cli_print.log_create_attempt_info(PRETTY_ENTITY_NAME, dataset.key)
    if file_path and not dataset_generation_request_with_id:
        stored_dataset = client.datasets.create_dataset_from_path(
            dataset, _get_cwd_relative_dataset_file_path(dataset_config_path, file_path)
        )
    ...
    cli_print.log_create_success_info(PRETTY_ENTITY_NAME, stored_dataset.key)
    return stored_dataset
...
def _update_single_dataset(
    ...
    cli_print.log_update_attempt_info(PRETTY_ENTITY_NAME, stored_dataset.key)
    ...
    if file_path:
        stored_dataset = client.datasets.update_dataset_data_from_path(
            stored_dataset.id,
            _get_cwd_relative_dataset_file_path(dataset_config_path, file_path),
        )
    cli_print.log_update_success_info(PRETTY_ENTITY_NAME, stored_dataset.key)
    return stored_dataset
...
def _get_cwd_relative_dataset_file_path(
    dataset_config_path: Path, data_file_path: Path
) -> Path:
    if data_file_path.is_absolute():
        return data_file_path

    # Build a path relative to base_dir, then make it absolute or CWD-relative
    result = (dataset_config_path.parent / data_file_path).resolve(strict=False)
    return result
