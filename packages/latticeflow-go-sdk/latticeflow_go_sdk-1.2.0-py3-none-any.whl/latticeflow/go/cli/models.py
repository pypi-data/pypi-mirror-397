...
def add_model_from_provider(
    ...
    if (existing_model := models_map.get_entity_by_key(selected_model.key)) is not None:
        cli_print.log_info(
            f"Model '{selected_model.display_name}' from provider"
            f" '{cast(ModelProviderConnectionConfig, selected_model.config).provider_id.value}'"
            f" already integrated. You can reference it by the key '{selected_model.key}'."
        )
        return existing_model

    created_model = client.models.create_model(selected_model)
    cli_print.log_info(
        f"Successfully integrated model '{created_model.display_name}' from provider"
        f" '{cast(ModelProviderConnectionConfig, created_model.config).provider_id.value}'."
        f" You can reference it by the key '{selected_model.key}'."
    )
...
def _get_provider_model_from_cli_parameter(
    ...
    if not (match := _MODEL_PROVIDER_AND_KEY_REGEX.fullmatch(model_provider_and_key)):
        raise cli_exc.CLIModelIntegrationError(
            f"Provided model provider and key '{model_provider_and_key}' has invalid format."
            " Must be in the format '<provider_id>/<model_key>'."
        )
    provider, model_key = match.group("provider"), match.group("model_key")

    try:
        selected_provider_id = IntegrationModelProviderId(provider)
    except ValueError:
        raise cli_exc.CLIModelIntegrationError(
            "Provided provider ID is invalid. Must be one of: "
            + ", ".join(
                [provider_id.value for provider_id in IntegrationModelProviderId]
            )
        )

    try:
        selected_provider = client.models.get_model_provider(
            selected_provider_id.value
        ).models
        return next(
            (model for model in selected_provider if model.display_name == model_key)
        )
    except Exception as error:
        raise cli_exc.CLIModelIntegrationError(
...
def _get_provider_model_interactively(client: Client) -> Model:
    ...
    if len(models) == 0:
        raise cli_exc.CLIModelIntegrationError(
            f"No models found for model provider '{selected_provider_id.value}'. "
            f"Please run `lf integration` and ensure the integration "
            "is set up correctly."
        )
