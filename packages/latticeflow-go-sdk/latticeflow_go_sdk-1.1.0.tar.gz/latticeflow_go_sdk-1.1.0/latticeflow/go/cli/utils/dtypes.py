...
class _BaseCLIDatasetGenerator(LFBaseModel):
    ...
    definition: Union[
        ...
        ...,
        discriminator="type",
        description="Dataset generator definition. There exists two definition types"
        "discriminated by the `type` field. For user-defined generator templates, must"
        "be of type `CLIDeclarativeDatasetGeneratorDefinitionTemplate`. For built‑in "
        "or preconfigured generators must be `CLIPredefinedDatasetGeneratorDefinition`.",
    )
    tags: List[str] = Field(
...
class CLIPredefinedDatasetGeneratorDefinition(LFBaseModel):
    type: Literal["predefined"] = Field(
        "predefined", description="Refers to built-in dataset generators."
    )
...
class _BaseCLICreateRunConfig(LFBaseModel):
    ...
    models: list[CLICreateModel] = Field(
        [],
        description=(
