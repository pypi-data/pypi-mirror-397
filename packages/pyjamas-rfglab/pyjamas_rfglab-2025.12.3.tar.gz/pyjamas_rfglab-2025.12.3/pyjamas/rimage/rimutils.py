...
class rimutils:
    ...
    @classmethod
    def mip(cls, im: Optional[numpy.ndarray] = None, axis: Optional[int] = 0) -> numpy.ndarray:
        if im.ndim < 3:
            return im

        return numpy.max(im, axis=axis)

    @classmethod
    def sip(cls, im: Optional[numpy.ndarray] = None, axis: Optional[int] = 0) -> numpy.ndarray:
        if im.ndim < 3:
            return im

        return numpy.sum(im, axis=axis)
    ...
    def find_seeds(cls, im: numpy.ndarray, window_size: int = FINDSEEDS_DEFAULT_WINSIZE,
        ...
        if min_distance_edge < 0.0:
            ...
            max_dist_im = skimorph.label(skimorph.dilation(max_dist_im, skimorph.footprint_rectangle((5, 5))))
    ...
    def find_seeds_gwdt(cls, im: numpy.ndarray, window_size: int = FINDSEEDS_DEFAULT_WINSIZE,
        ...
        """if output_intermediate:
            ...
            ax[1, 1].imshow(dilation(maxima_im, skimorph.footprint_rectangle((5, 5))), cmap='jet', alpha=.5)
            plt.show()"""
        return coords_maxima, numpy.stack((thresholded_im, distance_im, seed_im))
