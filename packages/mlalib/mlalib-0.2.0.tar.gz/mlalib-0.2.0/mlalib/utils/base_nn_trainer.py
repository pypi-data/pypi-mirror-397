...
class BaseNNTrainer(ABC):
    ...
    Args:
        ...
        lrs_metric (str or None): Metric name to monitor for ReduceLROnPlateau. Defaults to None.
    ...
    def _checkpoint(
        ...
        Args:
            metric (str): name of metric to monitor. Expects one of keys in trainer history.
        ...
        """
        metr_cur_val = self._history[metric][-1]
        ...
        if (minimize_metric and metr_cur_val < metr_best_val) or (
            not minimize_metric and metr_cur_val > metr_best_val
        ):
            if verbose:
                print(f"Checkpoint at {metric} = {metr_cur_val:.4f}")
    ...
    def _train_loop(
        ...
        verbose = verbose or (epoch + 1 == epochs)
        pbar = tqdm(
            train_dataloader,
            total=num_batches,
            ...
            if (
                ...
                if self._grad_clip_val:
                    torch.nn.utils.clip_grad_norm_(
                        self._model.parameters(), self._grad_clip_val
                    )
    ...
    def _validation_loop(
        ...
        verbose = verbose or (epoch + 1 == epochs)
        pbar = tqdm(val_dataloader, total=num_batches, desc="validating", leave=verbose)
    ...
    def fit(
        ...
        Args:
            ...
            checkpoint_metric (str): Metric name to monitor while checkpointing or "val_loss" if None.
            Expects one of keys in trainer history. Defaults to None.
            minimize_metric (bool): Whether to minimize or maximize the checkpoint_metric. Defaults to True.
        ...
        for epoch in range(epochs):
            ...
            if self._checkpoint_path:
                self._checkpoint(checkpoint_metric, minimize_metric, verbose)
