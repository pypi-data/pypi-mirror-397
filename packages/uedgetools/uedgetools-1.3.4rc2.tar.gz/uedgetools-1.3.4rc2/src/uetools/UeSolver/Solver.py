...
class Solver(UeRun):
    ...
    def surfacemorph_dt(self, points, albedo, rshift, zshift, resolution, savedir, albedo_bg=1, **kwargs):
        ...
        if "savefname" in kwargs:
            print("Using progress-based savename in savedir")
            del kwargs["savefname"]

        for progress in linspace(0, 1, resolution+1)[1:]:
            rshift_use = rshift*progress
            zshift_use = zshift*progress
            p = f"{progress*100:.2f}".replace('.','p')
            fname = f"{savedir}/progress{p}"
            points_use = [(x[0]+rshift_use, x[1]+zshift_use) for x in points]
            self.surfaces.geometric_pump( 
                        points_use, 
                        albedo=albedo, 
                        albedo_bg = albedo_bg
            )
            if "commands" in kwargs:
                for command in kwargs["commands"]:
                    exec(command)
                del(kwargs["commands"])
            self.converge(savefname=fname, **kwargs)




    def gridmorph_dt(self, newgrid, resolution, savedir, gridtarget=1, store_grid=True, **kwargs):
        from os import makedirs, path
        from shutil import rmtree
        from numpy import  linspace
        if path.isdir(savedir):
        ...
        for progress in linspace(0, gridtarget, resolution+1)[1:]:
            p = f"{progress*100:.2f}".replace('.','p')
            fname = f"{savedir}/progress{p}"
            for command in [ 
                "self.setue('manualgrid',1)",
                "self.setue('newgeo',1)",
                f"self.morphed_mesh('{newgrid}', {progress}, False)",
            ]:
                exec(command)    
            if "commands" in kwargs:
                for command in kwargs["commands"]:
    ...
    def gridmorph(self, newgrid, savedir, var={}, gridtarget=1, **kwargs):
        ...
        commands_use = [ 
                "self.setue('manualgrid',1)",
                "self.setue('newgeo',1)",
                f"self.morphed_mesh('{newgrid}',self.getue('gridmorph'), False)",
        ]
        if "commands" in kwargs:
            for command in kwargs["commands"]:
                commands_use.append(command)
            del(kwargs["commands"])

        self.continuation_solve(
            var,
            savedir=savedir,
            commands=commands_use,#=[
#                f"self.setue('manualgrid',1);self.setue('newgeo',1);self.morphed_mesh('{newgrid}',"
#                + "self.getue('gridmorph'), False)",
#            ],
            newgeo=True,
            **kwargs,
        )
