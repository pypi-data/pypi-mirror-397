...
class SorenClient:
    ...
    def check_auth_status(self) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.session.get(f"{self.api_url}/auth/status")
    ...
    def login(self, api_key: str) -> Dict[str, Any]:
        ...
        response = self.session.post(
            f"{self.api_url}/auth/validate-api-key",
    ...
    def upload_output(self, run_id: str, output_content: str) -> Dict[str, Any]:
        ...
        """
        response = self.session.patch(
            f"{self.api_url}/runs/{run_id}/output",
    ...
    def get_run(self, run_id: str) -> Dict[str, Any]:
        ...
        response = self.session.get(f"{self.api_url}/runs/{run_id}")
        response.raise_for_status()
        return response.json()
    ...
    def request_upload_urls(self, run_id: str, files: list) -> Dict[str, Any]:
        ...
        response = self.session.post(
            f"{self.api_url}/runs/{run_id}/upload-urls",
            json={"files": files}
        )
        response.raise_for_status()
        return response.json()
    ...
    # =========================================================================
    # Agent Workflow API Methods
    # =========================================================================

    def store_agent_workflow_metrics(self, run_id: str, metrics: Dict[str, Any]) -> Dict[str, Any]:
        """
        Store aggregate metrics for an agent workflow experiment.
        ...
        response = self.session.post(
            f"{self.api_url}/runs/{run_id}/agent-workflow/metrics",
            json=metrics
        )
        response.raise_for_status()
        return response.json()

    def store_agent_workflow_test_cases(self, run_id: str, test_cases: list) -> Dict[str, Any]:
        """
        Store test case summaries for an agent workflow experiment.

        Args:
            run_id: Run ID
            test_cases: List of test case summary dictionaries:
                [
                    {
                        "test_case_id": "test_case_0",
                        "result": "TP",  # or TN, FP, FN, None
                        "control_summary": "First 500 chars...",
                        "expected_verdict": "Exceptions noted",
                        "actual_verdict": "Exceptions noted",
                        "document_count": 3,
                        "display_name": "SOC 2 - ClarionDoor",
                        "s3_data_key": "runs/123/test_cases/test_case_0.json"
                    },
                    ...
                ]
        ...
        response = self.session.post(
            f"{self.api_url}/runs/{run_id}/agent-workflow/test-cases",
            json={"test_cases": test_cases}
        )
        response.raise_for_status()
        return response.json()

    def notify_upload_complete(self, run_id: str, files_uploaded: int, manifest_uploaded: bool, upload_timestamp: str) -> Dict[str, Any]:
        """
        Notify backend that file uploads are complete.

        This triggers cache invalidation after files are actually in S3.

        Args:
            run_id: Run ID
            files_uploaded: Number of files uploaded
            manifest_uploaded: Whether manifest was uploaded
            upload_timestamp: Timestamp from presigned URL response
        ...
        response = self.session.post(
            f"{self.api_url}/runs/{run_id}/upload-complete",
            json={
