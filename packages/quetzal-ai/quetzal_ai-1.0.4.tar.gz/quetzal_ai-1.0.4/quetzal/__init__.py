...
class FastLanguageModel:
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        # CPU Optimization: Disable 4-bit on CPU to favor speed if CUDA is not available
        if load_in_4bit and not torch.cuda.is_available():
            print("\n⚠️  Notice: Disabling 4-bit quantization for CPU training speed.")
            print("   (Float32 is significantly faster than emulated 4-bit on CPU)")
            load_in_4bit = False
        ...
        # Pass load_in_4bit if enabled (requires bitsandbytes, usually GPU only but checks are internal)
        if load_in_4bit:
        ...
        if "device_map" not in kwargs and not load_in_4bit:
            model = model.to('cpu')
