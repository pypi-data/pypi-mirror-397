...
# # onehux_sso_client/decorators.py ( Multi-Tenant Isolated)
...
# Usage Examples:
# --------------
# from sso_client.decorators import (
#     sso_login_required, 
#     require_sso_role,
#     require_any_role,
# )
...
#     If any check fails, redirects to SSO login.
...
#     @wraps(view_func)
#     @login_required  # Django's built-in check
#     def wrapper(request, *args, **kwargs):
#         # Check if SSO token exists
#         access_token = SSOSessionManager.get_access_token(request)
        
#         if not access_token:
#             logger.warning(f"No SSO token for user {request.user.email}")
#             # Store next URL for redirect after login
#             request.session['login_next_url'] = request.path
...
#         # Check if token needs refresh
#         if not SSOSessionManager.refresh_token_if_needed(request):
#             logger.warning(f"Token refresh failed for user {request.user.email}")
...
# # ============================================================================
# # ROLE-BASED ACCESS DECORATORS (SIMPLIFIED FOR SINGLE ORG)
# # ============================================================================

# def require_sso_role(required_role):
#     """
#     Decorator to check if user has a specific role.
    
#     SIMPLIFIED: Works with single-organization model.
#     User either has the role or doesn't.
    
#     Args:
#         required_role: Required role (e.g., 'admin', 'manager', 'developer')
...
#                 logger.warning(
#                     f"User {request.user.email} lacks required role: {required_role}. "
#                     f"User role: {request.user.role}"
#                 )
                
#                 return render(request, 'errors/403.html', {
#                     'error_title': '403 Forbidden',
#                     'error_message': f"This page requires the '{required_role}' role.",
#                     'user_role': request.user.role or 'None',
#                     'user_org': request.user.organization_name or 'No Organization',
#                 }, status=403)
...
#     Usage:
#         @require_any_role('admin', 'moderator', 'support')
#         def moderation_panel(request):
#             # User has at least one of: admin, moderator, or support role
#             return render(request, 'moderation.html')
#     """
...
#                 logger.warning(
#                     f"User {request.user.email} lacks any of required roles: {roles}. "
#                     f"User role: {request.user.role}"
#                 )
                
#                 return render(request, 'errors/403.html', {
#                     'error_title': '403 Forbidden',
#                     'error_message': f"This page requires one of these roles: {', '.join(roles)}",
#                     'user_role': request.user.role or 'None',
#                     'user_org': request.user.organization_name or 'No Organization',
#                 }, status=403)
...
# def require_admin():
#     """
#     Convenience decorator to require admin or owner role.
...
#     """
#     return require_any_role('admin', 'owner')


# def require_member():
#     """
#     Convenience decorator to require any organization membership.
    
#     Usage:
#         @require_member()
#         def members_only(request):
#             # User has some role in organization
...
#                 logger.warning(
#                     f"User {request.user.email} has no role in organization"
#                 )
...
# # ============================================================================
# # API TOKEN REQUIRED DECORATOR (For API Endpoints)
# # ============================================================================

# def api_token_required(view_func):
#     """
#     Decorator for API endpoints that require Bearer token authentication.
...
#         # Validate token with Onehux IdP
#         client = OnehuxSSOClient()
#         user_info = client.get_user_info(access_token)
...
#         request.sso_user_info = user_info
