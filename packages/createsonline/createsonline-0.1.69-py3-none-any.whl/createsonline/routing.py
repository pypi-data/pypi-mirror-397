...
def register_routes(app, urlpatterns: List[CreatesonlineRoute], route_prefix: str = ""):
    """
    ...
    for route in urlpatterns:
        if isinstance(route, list):
            ...
            register_routes(app, route, route_prefix=route_prefix)
        elif isinstance(route, CreatesonlineRoute):
            # Register the route
            for method in route.methods:
                if hasattr(app, method.lower()):
                    decorator = getattr(app, method.lower())
                    full_path = f"{route_prefix}{route.path}" if route_prefix else route.path
                    decorator(full_path)(route.handler)


def discover_and_register_routes(
    app,
    search_path: Path = None,
    pattern: str = "*routes.py",
    route_prefix: str = "",
):
    """
    Discover and register route modules dynamically.

    - search_path: base directory to scan (defaults to cwd)
    - pattern: glob for route files (defaults to *routes.py)
    - route_prefix: optional prefix to add to every discovered route
    """
    base = Path(search_path or Path.cwd())
    for routes_file in base.rglob(pattern):
        try:
            module_name = f"_routes_{routes_file.stem}_{abs(hash(routes_file))}"
            ...
            if urlpatterns:
                register_routes(app, urlpatterns, route_prefix=route_prefix)
        except Exception:
            continue
