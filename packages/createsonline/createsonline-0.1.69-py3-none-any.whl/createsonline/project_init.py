...
This is your central configuration file for customizing the framework behavior.
...
APP_NAME = "My CREATESONLINE App"
APP_VERSION = "1.0.0"
APP_DESCRIPTION = "AI-Native Web Application"
...
# Session cookie security settings (for production with HTTPS)
...
# Middleware classes to load (in order of execution)
# Format: List of tuples: (middleware_class_path, kwargs_dict)
MIDDLEWARE = [
    # Example: Enable CORS for all origins (customize for production!)
    # ('createsonline.middleware.CORSMiddleware', {
...
'''
    ...
    def _ensure_alembic_config(self):
        """Create Alembic migration configuration if it doesn't exist"""
        ...
        if not migrations_dir.exists():
            ...
            # Create alembic.ini
            alembic_ini_content = '''# Alembic migration configuration for CREATESONLINE
[alembic]
...
datefmt = %%H:%%M:%%S
'''
            self._create_file(alembic_ini, alembic_ini_content, "Created alembic.ini")

            # Create env.py for migrations
            env_py_content = '''"""
Alembic Environment Configuration for CREATESONLINE
Auto-generated migration environment
"""
...
try:
    ...
    database_url = "sqlite:///createsonline.db"

# This is the Alembic Config object
config = context.config

# Override sqlalchemy.url with user config
...
'''
            self._create_file(migrations_dir / "env.py", env_py_content, "Created migrations/env.py")
...
'''
            self._create_file(migrations_dir / "script.py.mako", script_mako_content, "Created migrations/script.py.mako")

            if self.verbose:
                print(f"   ✓ Database migrations initialized!")
                print(f"   Run 'alembic revision --autogenerate -m \"Initial migration\"' to create migrations")
