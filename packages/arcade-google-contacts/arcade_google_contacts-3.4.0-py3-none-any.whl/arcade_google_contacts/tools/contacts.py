...
@tool(requires_auth=Google(scopes=["https://www.googleapis.com/auth/contacts.readonly"]))
async def search_contacts_by_phone_number(
    context: ToolContext,
    phone_number: Annotated[str, "The phone number to search for"],
    limit: Annotated[
        int | None,
        "The maximum number of contacts to return (30 is the max allowed by Google API)",
    ] = DEFAULT_SEARCH_CONTACTS_LIMIT,
    ...
    # TODO: Ideally we should warmup only if this user (or google domain?) hasn't warmed up recently
    await _warmup_cache(service)
    return {"contacts": search_contacts(service, phone_number, limit)}
...
async def create_contact(
    ...
    phone_number: Annotated[str | None, "The optional phone number of the contact"],
    ...
    if phone_number:
        contact_body["phoneNumbers"] = [{"value": phone_number, "type": "mobile"}]

    # Create the contact. The personFields parameter specifies what information
    # should be returned. Here, we return names, emailAddresses, and phoneNumbers.
    created_contact = (
        service.people()
        .createContact(body=contact_body, personFields="names,emailAddresses,phoneNumbers")
        .execute()
    )
