...
def get_dominant(underlying_symbol, start_date=None, end_date=None, rule=0, rank=1, market="cn"):
    ...
    start_date : int | str | datetime.date | datetime.datetime | pandasTimestamp, optional
        开始日期，默认为期货品种最早上市日期后一交易日
    end_date : int | str | datetime.date | datetime.datetime | pandasTimestamp, optional
        结束日期，默认为当前日期
    rule : int, optional
        主力合约选取规则。
...
def get_member_rank(obj, trading_date=None, rank_by='volume', **kwargs):
    ...
    obj : str
        可以是期货的具体合约或者品种
    trading_date : int | str | datetime.date | datetime.datetime | pandasTimestamp, optional
        查询日期，默认为当日
...
def get_current_basis(order_book_ids, market='cn'):
    ...
    pandas.DataFrame
        ...
        - datetime : pandasTimestamp - 最新一行 tick 的时间戳
        - index : str - 指数合约
...
def get_ex_factor(underlying_symbols, start_date=None, end_date=None, adjust_method='prev_close_spread', rule=0, rank=1,
    ...
    pandas.DataFrame
        包含了复权因子的日期和对应的各项数值：
        - ex_date : pandasTimestamp - 除权除息日（ 主力合约切换日）
        - underlying_symbol : str - 品种代码
        - ex_factor : float - 复权因子
        - ex_cum_factor : float - 累计复权因子
        - ex_end_date : pandasTimestamp - 复权因子所在期的截止日期
...
def get_dominant_price(
    ...
    pandas.DataFrame
        ...
        - open_interest : float - 累计持仓量
        - datetime : pandasTimeStamp - 交易所时间戳
...
def get_continuous_contracts(underlying_symbol, start_date, end_date, type='front_month', market='cn'):
    ...
    underlying_symbol : str
        期货合约品种，目前仅支持股指品种
    start_date : int | str | datetime.date | datetime.datetime | pandasTimestamp
        开始日期
    end_date : int | str | datetime.date | datetime.datetime | pandasTimestamp
        结束日期
    type : str, optional
        类型，默认为 front_month
...
def get_roll_yield(underlying_symbols, start_date=None, end_date=None, type='main_sub', rule=0, market='cn'):
    ...
    from_settlement = settlement_data.loc[pd.MultiIndex.from_frame(df[['from_contract', 'date']], names=['order_book_id', 'date'])]
    to_settlement = settlement_data.loc[pd.MultiIndex.from_frame(df[['to_contract', 'date']], names=['order_book_id', 'date'])]
