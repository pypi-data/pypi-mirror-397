...
def load(self, filename=None):
    ...
    Args:
        ...
        filename (str, optional): The path to the HDF5 file to load the study from.
                                 If just a filename (no path) is provided and study.folder
                                 is set, it will be looked for in the study folder.
    """
    ...
    if filename is None:
        ...
        if not os.path.isabs(filename) and self.folder is not None:
...
def _add_sample_noms1(
    ...
    if sample_polarity is not None:
        if study_polarity is None:
            ...
            if not getattr(self.parameters, "_adducts_explicitly_set", False):
                ...
                self.logger.info(f"Study polarity set to '{sample_polarity}' from first sample")
                self.logger.info(f"Adducts automatically set for {sample_polarity} mode: {self.parameters.adducts}")
            else:
                self.logger.info(f"Study polarity set to '{sample_polarity}' from first sample (adducts kept as user-defined)")
    ...
    # - No set_sample_color(by=None) call needed
