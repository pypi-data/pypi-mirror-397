    ...
    with h5py.File(filename, "r") as f:
        if "metadata" in f:
            ...
            if storage_version == 2 or "masster-sample-2" in str(format_str):
                self.logger.debug(f"Loading sample5 file version 2 (columnized storage)")
                return _load_sample5_v2(self, filename, map)
            else:
                self.logger.debug(f"Loading sample5 file version 1 (JSON storage)")
    ...
    with h5py.File(filename, "r") as f:
        ...
        if "lib" in f:
            ...
            if data:
                ...
                if "source_id" in self.lib_df.columns and "lib_source" not in self.lib_df.columns:
                    self.logger.info(
        ...
        if "id" in f:
            ...
            if data:
                ...
                if "matcher" in self.id_df.columns and "id_source" not in self.id_df.columns:
                    self.id_df = self.id_df.rename({"matcher": "id_source"})
                    self.logger.debug("Renamed 'matcher' column to 'id_source' for backward compatibility")
...
def _load_sample5_study(self, filename: str, map: bool = False):
    ...
    with h5py.File(filename, "r") as f:
        if "metadata" in f:
            ...
            # Check if this is version 2 (columnized storage)
            if storage_version == 2 or "masster-sample-2" in str(format_str):
                self.logger.debug(f"Loading sample5 file version 2 (columnized storage, optimized for study)")
                ...
                if hasattr(self, 'features_df') and self.features_df is not None and 'chrom' in self.features_df.columns:
                    non_null_count = self.features_df['chrom'].drop_nulls().len()
                    self.logger.debug(f"After _load_sample5_v2: {non_null_count} non-null chrom values")
                self.ms1_df = None
                self.logger.info(f"Sample loaded successfully from {filename} (optimized for study)")
                return
            else:
                self.logger.debug(f"Loading sample5 file version 1 (JSON storage, optimized for study)")
                # Continue with original version 1 loader below
...
def _save_chromatograms_columnized(chrom_group: h5py.Group, chromatograms: list, logger) -> None:
    """
    ...
    chrom_group.create_dataset('intensity_data', data=intensity_data, compression='lzf', shuffle=True)
    chrom_group.create_dataset('n_points', data=n_points, compression='lzf', shuffle=True)
    ...
    chrom_group.create_dataset('rt_start', data=rt_start_array, compression='lzf', shuffle=True)
    chrom_group.create_dataset('rt_end', data=rt_end_array, compression='lzf', shuffle=True)
    chrom_group.create_dataset('intensity', data=intensity_array, compression='lzf', shuffle=True)
    chrom_group.create_dataset('height', data=height_array, compression='lzf', shuffle=True)
    ...
    logger.debug(f"Saved {n_features} chromatograms in columnized format (max {max_points} points)")
...
def _load_chromatograms_columnized(chrom_group: h5py.Group, logger) -> list:
    ...
    max_points = chrom_group.attrs.get('max_points', 0)
    ...
    for i in range(n_features):
        ...
        chrom = Chromatogram(
            ...
            rt_start=float(rt_start_array[i]) if not np.isnan(rt_start_array[i]) else None,
        ...
        if not np.isnan(rt_array[i]):
            chrom.rt_apex = float(rt_array[i])
        if not np.isnan(height_array[i]):
            chrom.height = float(height_array[i])
        if not np.isnan(intensity_array[i]):
            chrom.intensity_sum = float(intensity_array[i])
...
def _save_ms2_specs_columnized(ms2_group: h5py.Group, ms2_specs_list: list, logger) -> None:
    """
    ...
    ms2_group.create_dataset('intensity_data', data=inty_array, compression='lzf', shuffle=True)
    ms2_group.create_dataset('spectrum_lengths', data=spectrum_lengths_array, compression='lzf', shuffle=True)
    ms2_group.create_dataset('feature_counts', data=feature_counts, compression='lzf', shuffle=True)
    ms2_group.create_dataset('feature_offsets', data=feature_offsets, compression='lzf', shuffle=True)
    ...
    logger.debug(f"Saved {total_spectra} MS2 spectra across {n_features} features in ragged array format")
...
def _load_ms2_specs_columnized(ms2_group: h5py.Group, logger) -> list:
    ...
    feature_offsets = ms2_group['feature_offsets'][:]
    ...
    logger.debug(f"Loaded {total_spectra} MS2 spectra across {n_features} features from ragged array format")
    return result
...
def _save_sample5_v2(
    ...
    self.logger.debug(f"Saving sample to {filename} with columnized chromatogram storage (v2)")
    ...
    with h5py.File(filename, "w") as f:
        ...
        metadata_group = f.create_group("metadata")
        features_group = f.create_group("features")
        chrom_group = f.create_group("chromatograms")  # New group for columnized chromatograms
        scans_group = f.create_group("scans")
        ms1_group = f.create_group("ms1")
            ...
            for col in scans_df.columns:
                ...
                if data.dtype == object:
                    try:
                        ...
                        scans_group.create_dataset(col, data=str_data, compression="gzip")
                        scans_group[col].attrs["dtype"] = "string_converted"
                    except Exception:
                        try:
                            numeric_data = np.array(
                            ...
                            if not np.isnan(numeric_data).all():
                                scans_group.create_dataset(col, data=numeric_data, compression="gzip")
                                scans_group[col].attrs["dtype"] = "numeric_converted"
                            else:
                                json_data = np.array(
                                    [json.dumps(x, default=str) for x in data],
                                ...
                                scans_group.create_dataset(col, data=json_data, compression="gzip")
                                scans_group[col].attrs["dtype"] = "json_serialized"
                        except Exception:
                            str_repr_data = np.array([str(x) for x in data], dtype="S")
                            scans_group.create_dataset(col, data=str_repr_data, compression="gzip")
                            scans_group[col].attrs["dtype"] = "string_repr"
                else:
                    ...
                    if col == "ms2_scans":
                        ...
                        for i in range(len(data)):
                            if data[i] is not None:
                                ...
                                data_as_json_strings.append("None")
                        features_group.create_dataset(col, data=data_as_json_strings, compression="gzip")
                    elif col == "ms1_spec":
                        data = features[col]
                        data_as_json_strings = []
                        for i in range(len(data)):
        ...
        if self.ms1_df is not None and include_ms1:
            for col in self.ms1_df.columns:
                ms1_group.create_dataset(col, data=self.ms1_df[col].to_numpy(), compression="gzip")
            ...
            for col in self.lib_df.columns:
                ...
                if data.dtype == object:
                    try:
                        ...
                        lib_group.create_dataset(col, data=json_data, compression="gzip")
                        lib_group[col].attrs["dtype"] = "json"
                else:
...
def _load_sample5_v2(self, filename: str, map: bool = False):
    ...
    with h5py.File(filename, "r") as f:
            ...
            if data:
                ...
                for col in self.scans_df.columns:
                    if self.scans_df[col].dtype == pl.Utf8:
                        ...
                        self.scans_df = self.scans_df.with_columns([pl.col(col).fill_nan(None).alias(col)])
                ...
                for col in self.scans_df.columns:
                    if col in schema.get("scans_df", {}).get("columns", {}):
                        try:
                            dtype_str = schema["scans_df"]["columns"][col]["dtype"]
                            if dtype_str.startswith("pl.") and "Object" not in dtype_str:
                                self.scans_df = self.scans_df.with_columns(
                                    pl.col(col).cast(eval(dtype_str))
                                )
                        except Exception as e:
                            self.logger.warning(f"Failed to cast column '{col}' in scans_df: {e}")
            else:
                self.scans_df = None
        else:
            self.scans_df = None
        ...
        if "features" in f:
            ...
            for col in schema.get("features_df", {}).get("columns", []):
                ...
                if col not in features_group:
                    self.logger.debug(f"Column '{col}' not found in sample5/features.")
                    data[col] = None
                    continue
            ...
            if "chromatograms" in f:
                ...
                data["chrom"] = [None] * len(data.get("feature_uid", []))
            ...
            if "ms2_specs" in f:
                ...
                # No MS2 spectra data
                data["ms2_specs"] = [None] * len(data.get("feature_uid", []))
            ...
            if data:
                ...
                try:
                    ...
                    object_columns = {k: v for k, v in data.items() 
                                    if k in schema.get("features_df", {}).get("columns", {})
                                    and schema["features_df"]["columns"][k]["dtype"] == "pl.Object"}
                    regular_columns = {k: v for k, v in data.items() if k not in object_columns}
                ...
                for col in self.features_df.columns:
                    if col in schema.get("features_df", {}).get("columns", {}):
                        try:
                                ...
                                if "Utf8" in dtype_str or "String" in dtype_str:
                                    self.features_df = self.features_df.with_columns(
                                        pl.col(col).map_elements(
                                            lambda x: (x.decode("utf-8") if isinstance(x, bytes) else x) if x not in (b"None", "None", b"", "") else None,
                                            return_dtype=pl.Utf8
                                        )
                                    )
                            ...
                            self.logger.warning(f"Failed to cast column '{col}' in features_df: {e}")
    ...
    if hasattr(self, 'features_df') and self.features_df is not None and 'chrom' in self.features_df.columns:
        ...
        self.logger.debug(f"_load_sample5_v2 ending: {non_null_chrom} non-null chrom values")
    
    self.logger.info(f"Sample loaded from {filename} (v2 with columnized storage)")
