...
def normalize_features_df(df: pl.DataFrame) -> pl.DataFrame:
    ...
    mz_cols = [c for c in df.columns if c.startswith("mz") and df[c].dtype in [pl.Float64, pl.Float32]]
    for col in mz_cols:
        cast_exprs.append(pl.col(col).round(5).alias(col))
    
    # rt columns - round to 3 digits
    rt_cols = [c for c in df.columns if c.startswith("rt") and df[c].dtype in [pl.Float64, pl.Float32]]
    for col in rt_cols:
    ...
    adduct_mass_cols = [c for c in df.columns if c.startswith("adduct_mass") and df[c].dtype in [pl.Float64, pl.Float32]]
    for col in adduct_mass_cols:
        cast_exprs.append(pl.col(col).round(5).alias(col))
    
    # chrom_prominence columns - round to 3 digits
    prominence_cols = [c for c in df.columns if "prominence" in c and df[c].dtype in [pl.Float64, pl.Float32]]
    for col in prominence_cols:
        cast_exprs.append(pl.col(col).round(3).alias(col))
    
    # chrom_height columns - round to 3 digits
    height_cols = [c for c in df.columns if "height" in c and df[c].dtype in [pl.Float64, pl.Float32]]
...
def _load_dataframe_from_group(
    ...
    if df_name == "samples_df":
        ...
        # No migrations needed for consensus_df in save logic
        effective_columns = hdf5_columns.copy()
    ...
    for col in missing_columns:
        ...
        if df_name == "features_df" and col in ['id_top_name', 'id_top_class', 'id_top_adduct', 'id_top_score', 'id_source']:
    ...
    if df_name == "samples_df":
        ...
        optional_columns = {'id_top_name', 'id_top_class', 'id_top_adduct', 'id_top_score', 'id_source'}
    ...
    extra_columns = [col for col in hdf5_columns if col not in (schema_columns or []) and col not in migrated_old_names and col not in ignored_columns and col not in optional_columns]
    ...
    if df_name == "features_df":
        if "chrom_start" in df.columns and "chrom_end" in df.columns and "rt_start" in df.columns and "rt_end" in df.columns:
            ...
            if chrom_start_all_null and chrom_end_all_null:
                logger.debug("Copying rt_start/rt_end to chrom_start/chrom_end for backward compatibility")
                df = df.with_columns([
...
def _load_study5_v3(self, filename):
    ...
    with h5py.File(filename, "r") as f:
        with tqdm(total=len(loading_steps), desc="Loading study (v3.0)", disable=tdqm_disable, unit="step") as pbar:
            ...
            if "samples" in f and len(f["samples"].keys()) > 0:
                ...
                self.logger.debug("No samples data found. Initializing empty samples_df.")
            ...
            if "consensus" in f and len(f["consensus"].keys()) > 0:
                ...
                if self.consensus_df is not None:
                    if "adducts" not in self.consensus_df.columns or self.consensus_df["adducts"].dtype == pl.Null:
                        ...
                        empty_adducts = [[] for _ in range(len(self.consensus_df))]
            ...
            if "lib" in f and len(f["lib"].keys()) > 0:
                self.lib_df = _load_dataframe_from_group(f["lib"], schema, "lib_df", self.logger, [])
            else:
            ...
            if "source_id" in self.lib_df.columns and "lib_source" not in self.lib_df.columns:
                self.logger.info(f"Migrating lib_df from schema v{file_schema_version}: renaming 'source_id' to 'lib_source'")
            ...
            if "id" in f and len(f["id"].keys()) > 0:
                self.id_df = _load_dataframe_from_group(f["id"], schema, "id_df", self.logger, [])
                
                # Backward compatibility: rename 'matcher' to 'id_source' if present
                if self.id_df is not None and "matcher" in self.id_df.columns and "id_source" not in self.id_df.columns:
...
def _save_study5_v1(self, filename):
    ...
    raise NotImplementedError("v1.0 (JSON) format saving not implemented. Use v2.0 or v3.0 instead.")
