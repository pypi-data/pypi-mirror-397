...
def analyze_diff(self, samples1, samples2, quant="chrom_area", test_type="welch", fdr_type="bh",
                 log2fc_min=None, pvalue_max=None, fdr_max=None, qvalue_max=None):
    """
    Perform differential analysis between two groups of samples.

    Calculates log2 fold-change of means, p-values, FDR-corrected p-values,
    and q-values for each consensus feature between two sample groups.
    ...
    Parameters:
        ...
        log2fc_min (float, optional): Minimum absolute log2 fold-change to include (default: None)
        pvalue_max (float, optional): Maximum p-value to include (default: None)
        fdr_max (float, optional): Maximum FDR to include (default: None)
        qvalue_max (float, optional): Maximum q-value to include (default: None)

    Returns:
        pl.DataFrame: DataFrame with one row per consensus feature containing:
            - consensus_uid: Unique identifier for the consensus feature
            - consensus_id: Human-readable consensus ID
            - rt: Retention time
            - mz: Mass-to-charge ratio
            - mean1: Mean abundance in samples1 (excluding missing values)
            - mean2: Mean abundance in samples2 (excluding missing values)
            - log2fc: Log2 fold-change (log2(mean2/mean1))
            - pvalue: P-value from the selected test
            - fdr: FDR-corrected p-value (Benjamini-Hochberg or Benjamini-Yekutieli)
            - qvalue: Q-value (Storey-Tibshirani 2003)
            - id_top_name, id_top_class, id_top_adduct, id_top_score, id_source: ID columns (if present)

    Example:
        >>> # Compare two sample groups with filtering
        >>> diff = study.analyze_diff(
    ...
    # Get sample columns (all columns except consensus_uid)
    sample_cols1 = [c for c in matrix1.columns if c != "consensus_uid"]
    sample_cols2 = [c for c in matrix2.columns if c != "consensus_uid"]

    self.logger.info(f"Comparing {len(sample_cols1)} samples vs {len(sample_cols2)} samples (test={test_type}, fdr={fdr_type})")

    # Align matrices by consensus_uid (inner join to get common features)
    aligned = matrix1.select(["consensus_uid"] + sample_cols1).join(
        matrix2.select(["consensus_uid"] + sample_cols2),
        on="consensus_uid",
        how="inner"
    )
    
    if aligned.is_empty():
        self.logger.error("No common consensus features found between the two sample groups")
        return pl.DataFrame()

    # Extract consensus_uids from aligned data
    consensus_uids = aligned["consensus_uid"].to_numpy()
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        _, pvalues = ttest_ind(data1, data2, axis=1, equal_var=equal_var, nan_policy='omit')
    ...
    if np.sum(valid_pval_mask) > 0:
        fdr_values[valid_pval_mask] = false_discovery_control(pvalues[valid_pval_mask], method=fdr_type)
    ...
    base_cols = ["consensus_uid", "consensus_id", "rt", "mz"]
    id_cols = ["id_top_name", "id_top_class", "id_top_adduct", "id_top_score", "id_source"]
    ...
    final_df = consensus_info.join(results_df, on="consensus_uid", how="inner")
    ...
    self.logger.success(f"Differential analysis complete: {len(final_df)} features compared")
