...
def lib_load(
    ...
    Args:
        ...
        lib_source: Can be one of the following:
            ...
            - Embedded model name (str): Special library names for built-in metabolic models:
                ...
                * 'core': Core metabolic model (intersection of iML1515, iMM904, and Recon3D)
        ...
        polarity: ionization polarity ("positive" or "negative") - used when lib_source is a CSV/JSON path.
                 If None, uses study.parameters.polarity automatically.
    ...
    if isinstance(lib_source, str):
        ...
        filtered_lf = filtered_lf.with_columns(pl.lit(lib_source_value).alias("lib_source"))
...
def _perform_identification_matching(
    consensus_to_process, study, effective_mz_tol, effective_rt_tol, adduct_prob_map, only_masster, logger
):
...
def _prepare_consensus_features(study, features, only_orphans, logger=None):
    ...
    if only_orphans and hasattr(study, "id_df") and study.id_df is not None and not study.id_df.is_empty():
        # Get consensus features that already have identifications
        matched_consensus_uids = study.id_df.select("consensus_uid").unique().to_series().to_list()
        if matched_consensus_uids:
            initial_count = len(consensus_to_process)
            consensus_to_process = consensus_to_process.filter(
                ~pl.col("consensus_uid").is_in(matched_consensus_uids)
            )
            if logger:
                logger.debug(f"Filtering orphans: {initial_count} consensus features → {len(consensus_to_process)} orphans (excluding {initial_count - len(consensus_to_process)} already matched)")
            if consensus_to_process.is_empty():
                if logger:
                    logger.info("All consensus features already have identifications - nothing to identify")
                return None, target_uids
            ...
            target_uids = consensus_to_process.select("consensus_uid").to_series().to_list()
            if logger:
                logger.debug(f"Set target_uids to {len(target_uids)} orphan consensus features to preserve existing identifications")
...
def identify(study, features=None, params=None, only_masster=True, only_orphans=True, **kwargs):
    ...
    Args:
        ...
        only_masster: If True (default), only match against lib_df rows where lib_source 
                     starts with "masster" (i.e., loaded via lib_load()). 
                     If False, match against all library entries (including those from import_tima).
        only_orphans: If True (default), only identify consensus features that are not already 
                     matched in id_df (i.e., features without any existing identifications).
                     If False, identify all specified consensus features regardless of existing matches.
    ...
    Returns:
        None. The resulting DataFrame is stored as study.id_df with columns:
            - consensus_uid: Consensus feature identifier
            - lib_uid: Library entry identifier
            - mz_delta: Absolute m/z difference
            - rt_delta: Absolute retention time difference (nullable)
            - score: Matching score (adduct probability with penalties applied, 0-100)
    
    Examples:
        >>> # Identify all orphan consensus features against masster libraries only (default)
        >>> study.identify()
        
        >>> # Identify all consensus features against all libraries (masster + TIMA)
        >>> study.identify(only_masster=False, only_orphans=False)
        
        >>> # Re-identify already matched features with a different library
        >>> study.identify(only_orphans=False)
        
        >>> # Identify specific consensus features
        >>> study.identify(features=[1, 2, 3])
    """
    ...
    consensus_to_process, target_uids = _prepare_consensus_features(study, features, only_orphans, logger)
...
def id_update(study):
    ...
    # Group by consensus_uid and select best identification (highest score)
    best_ids = (
        id_with_lib.group_by("consensus_uid")
        .agg([pl.col("score").max().alias("max_score")])
        .join(id_with_lib, on="consensus_uid")
    ...
    if logger:
        logger.debug(f"Selected best identifications for {len(best_ids)} consensus features")
    ...
    id_columns = {
        ...
        "id_source": best_ids.select("consensus_uid", "id_source"),
    }
    
    # Only add id_top_class if class column exists and has non-null values
    if "class" in best_ids.columns:
        id_columns["id_top_class"] = best_ids.select("consensus_uid", "class")
    ...
    for col_name in expected_id_columns:
        if col_name not in study.consensus_df.columns:
            if col_name == "id_top_score":
                study.consensus_df = study.consensus_df.with_columns(pl.lit(None, dtype=pl.Float64).alias(col_name))
            else:
                study.consensus_df = study.consensus_df.with_columns(pl.lit(None, dtype=pl.String).alias(col_name))
    ...
    for col_name, id_data_col in id_columns.items():
        source_column = id_data_col.columns[1]  # second column (after consensus_uid)
        ...
        # Join and update
        study.consensus_df = (
            study.consensus_df.join(update_data, on="consensus_uid", how="left", suffix="_update")
    ...
    updated_count = study.consensus_df.filter(pl.col("id_top_name").is_not_null()).height
    
    if logger:
        logger.success(f"Updated id_* columns in consensus_df. {updated_count} features have identifications.")
        ...
        if result is None:
            ...
            if action in ['filter', 'delete_others', 'delete']:
                if hasattr(study, 'id_df') and study.id_df is not None and not study.id_df.is_empty():
                    ...
                    remaining_lib_uids = study.lib_df.select('lib_uid').to_series().to_list()
                    # Filter id_df to keep only rows with lib_uids that still exist in lib_df
                    study.id_df = study.id_df.filter(pl.col('lib_uid').is_in(remaining_lib_uids))
                    study.logger.info(f"Filtered id_df to {len(study.id_df)} identifications matching remaining library entries")
...
def lib_filter(study, entries):
    ...
    if isinstance(entries, str):
        if entries == 'delete_identified':
            ...
            if hasattr(study, 'update_history'):
                study.update_history(["lib_filter"], {
            ...
            if not ms2_conditions:
                if logger:
                    logger.warning("'id_source' column not found in id_df. Cannot filter by MS2.")
                return
            ...
            ms2_lib_uids = study.id_df.filter(ms2_filter).select('lib_uid').unique().to_series().to_list()
            
            # Keep only lib_uids that have MS2 identifications
            study.lib_df = study.lib_df.filter(pl.col("lib_uid").is_in(ms2_lib_uids))
            
            if logger:
                deleted_count = original_count - len(study.lib_df)
                logger.info(f"Deleted {deleted_count} MS1-only library entries. Remaining: {len(study.lib_df)}")
            ...
            if hasattr(study, "id_df") and study.id_df is not None and not study.id_df.is_empty():
                ...
                remaining_lib_uids = study.lib_df.select('lib_uid').unique().to_series().to_list()
                study.id_df = study.id_df.filter(pl.col('lib_uid').is_in(remaining_lib_uids))
                if logger:
            ...
            if logger:
                deleted_count = original_count - len(study.lib_df)
                logger.info(f"Deleted {deleted_count} non-CHNOPS library entries. Remaining: {len(study.lib_df)}")
    ...
    if hasattr(study, "id_df") and study.id_df is not None and not study.id_df.is_empty():
        study.id_df = study.id_df.filter(pl.col('lib_uid').is_in(lib_uids_to_keep))
        if logger:
