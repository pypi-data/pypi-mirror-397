...
def _process_kd_chunk_parallel(chunk_data):
    ...
    for sample_data in chunk_samples_data:
        ...
        for feature_dict in sample_features:
            ...
            # Set unique ID using feature_uid (integer) for mapping back
            feature.setUniqueId(int(feature_dict["feature_uid"]))
    ...
    **KDTree** - *High Sample Feature Preservation*
    ...
    try:
        ...
        if not cached_adducts_df.is_empty():
            ...
            study.logger.warning(f"No valid adducts found for polarity '{study.parameters.polarity}'")
    ...
    if params.method == "kd":
        ...
        study.logger.debug(f"Returned from _merge_kd_chunked - consensus_df has {len(study.consensus_df)} features")
        ...
        study.logger.debug(f"Returned from _merge_qt_chunked - consensus_df has {len(study.consensus_df)} features")
    ...
    study.logger.debug(f"Starting post-merge processing (adduct grouping, finalization)")
...
def _generate_feature_maps_on_demand(study):
    ...
    if use_groupby_optimization:
        ...
        for row in features_by_sample.iter_rows(named=True):
            ...
            sample_feature_dict[sample_uid] = {
                "feature_uid": np.array(row["feature_uid"]),
                "mz": np.array(row["mz"]),
                "rt": np.array(row["rt"]),
    ...
    for sample_index, row_dict in enumerate(study.samples_df.iter_rows(named=True)):
        ...
        if use_groupby_optimization:
            ...
            n_sample_features = len(sample_data["feature_uid"])
            ...
            for i in range(n_sample_features):
                try:
                    feature = oms.Feature()
                    feature.setUniqueId(int(sample_data["feature_uid"][i]))
...
def _merge_kd_chunked(
    ...
    try:
        ...
        study.logger.debug(f"Completed _dechunk_results - consensus_df has {len(study.consensus_df)} features")
    except Exception as e:
...
def _dechunk_results(
    ...
    study.logger.debug(f"_dechunk_results called with {len(chunk_consensus_maps)} chunks")
    ...
    if len(chunk_consensus_maps) == 1:
        ...
        study.logger.debug(f"Single chunk extraction completed - {len(study.consensus_df)} features extracted")
        return

    study.logger.debug(f"Multiple chunks detected - proceeding with cross-chunk consensus building")
    
    # Build feature_uid to feature_data lookup for fast access
    # Note: feature_uid is now used directly (no longer need to map from feature_id)
    ...
    for chunk_idx, (chunk_start_idx, chunk_data) in enumerate(chunk_consensus_maps):
        ...
        if isinstance(chunk_data, list):
            ...
            for consensus_feature in chunk_consensus_map:
                ...
                for feature_handle in consensus_feature.getFeatureList():
                    # fuid is now directly the feature_uid since we use setUniqueId(feature_uid)
                    feature_uid = int(feature_handle.getUniqueId())
        ...
        for consensus_feature_data in consensus_features_data:
            ...
            if isinstance(chunk_data, list):
                ...
                for handle_data in consensus_feature_data["features"]:
                    # unique_id is now directly the feature_uid since we use setUniqueId(feature_uid)
                    feature_uid = int(handle_data["unique_id"])
                    feature_data = features_lookup.get(feature_uid)
                    if feature_data:
                        feature_uids.append(feature_uid)
                        feature_data_list.append(feature_data)
    ...
    study.logger.info(f"Extracted {len(study.consensus_df)} consensus features from {len(chunk_consensus_maps)} chunks.")
...
def _extract_consensus_features(study, consensus_map, min_samples, cached_adducts_df=None, cached_valid_adducts=None):
    ...
    # create a set of valid feature_uids for quick lookup
    valid_feature_uids = set(study.features_df["feature_uid"].to_list())
    ...
    for i, feature in enumerate(
        ...
        for _j, f in enumerate(features_list):
            ...
            if fuid not in valid_feature_uids:
        ...
        metadata_list.append(
            {
                ...
                "rt": round(float(np.median(rt_values)), 4) if len(rt_values) > 0 else 0.0,
                "mz": round(float(np.median(mz_values)), 4) if len(mz_values) > 0 else 0.0,
    ...
    study.logger.debug(f"Created {len(consensus_mapping)} consensus mapping entries from {len(metadata_list)} consensus features")
    consensus_mapping_df = pl.DataFrame(consensus_mapping)
    study.logger.debug(f"consensus_mapping_df shape: {consensus_mapping_df.shape}, columns: {consensus_mapping_df.columns}")
    ...
    if (not study.features_df.is_empty() and "feature_uid" in study.features_df.columns 
        and not consensus_mapping_df.is_empty() and "feature_uid" in consensus_mapping_df.columns):
...
def _perform_adduct_grouping(study, rt_tol, mz_tol):
    ...
    study.logger.debug(f"Starting adduct grouping on {len(study.consensus_df)} consensus features")
    import polars as pl

    # Add adduct grouping and adduct_of assignment
    if len(study.consensus_df) == 0:
        study.logger.warning("⚠️ consensus_df is empty! Skipping adduct grouping. Check why no consensus features were extracted.")
        return
...
def _merge_partial_consensus_features(study, rt_tol, mz_tol):
    ...
    """
    study.logger.debug(f"Starting merge of partial consensus features for chunked method")
    if len(study.consensus_df) == 0:
        study.logger.warning("⚠️ consensus_df is empty! Skipping partial feature merging. Check why no consensus features were extracted.")
        return
...
def __identify_adduct_by_mass_shift(study, rt_tol, cached_adducts_df=None):
    ...
    if adduct_updates:
        ...
        study.logger.debug(f"Adduct information updated for {updated_count} consensus features.")
    else:
...
def __finalize_merge(study, link_ms2, extract_ms1, min_samples):
    ...
    study.logger.debug(f"Starting finalize_merge with link_ms2={link_ms2}, extract_ms1={extract_ms1}, min_samples={min_samples}")
    import polars as pl
    ...
    if len(study.consensus_df) == 0 or "number_samples" not in study.consensus_df.columns:
        study.logger.warning("⚠️ No consensus features found or consensus_df is empty/invalid. Skipping finalize merge. Check why no features were extracted from chunks.")
        return
...
def _get_features_matrix(study, consensus_data, quant_col="inty"):
    ...
    study.logger.debug(
        f"Building optimized features matrix: {len(consensus_uids)} features x {len(study.samples_df)} samples"
    )
    ...
    try:
        ...
        intensity_data = (
            mapping_filtered
            .join(
                study.features_df.select(["feature_uid", "sample_uid", quant_col]),
                on=["feature_uid", "sample_uid"],
                how="left"
            )
            .join(
                study.samples_df.select(["sample_uid", "sample_name"]),
                on="sample_uid",
                how="left"
            )
            .select(["consensus_uid", "sample_name", quant_col])
        )
        
        # Pivot to create matrix (consensus_uid × sample_name)
        ...
        matrix_pd = matrix_pl.to_pandas().set_index("consensus_uid")
        
        study.logger.debug(f"Optimized matrix built successfully with shape {matrix_pd.shape}")
...
def _fast_correlation(vec1, vec2):
    ...
    OPTIMIZED: Uses direct formula without computing full covariance matrix.
    """
    ...
    if n_valid < 2:  # Need at least 2 valid points
        return 0.0

    v1 = vec1[mask]
    v2 = vec2[mask]

    # Direct correlation formula (faster than np.corrcoef)
    try:
        v1_mean = v1.mean()
        v2_mean = v2.mean()
        ...
        denominator = np.sqrt(np.dot(v1_centered, v1_centered) * np.dot(v2_centered, v2_centered))
                ...
                if candidate["uid"] not in uid_to_boss or uid_to_boss[candidate["uid"]] == candidate["uid"]:
                    ...
                    if mass_shift_key in mass_shift_map:
                        ...
                        if candidate_intensity > 0 and boss_intensity > 0:
                            intensity_ratio = max(boss_intensity, candidate_intensity) / min(boss_intensity, candidate_intensity)
