...
def plot_2d(
    ...
    Parameters:
        ...
        title (str, optional):
            Title of the plot.
        raster_cmap (str, default 'iridescent'):
            Colormap to use for the background rasterized MS1 data. Defaults to "iridescent".
        marker_cmap (str, default 'plasma_r'):
            Colormap to use for continuous feature coloring when colorby is specified with a numeric column.
            Only applies when colorby is not None and contains numeric data. Common options include
            'viridis', 'plasma', 'plasma_r', 'inferno', 'magma', 'cividis', etc.
        ...
        tooltip (str, optional):
            Controls the feature hover tooltip content:
            - None or 'id' (default): Shows rt, m/z, feature_uid, inty, and any id_* columns
            - 'chrom': Shows full chromatogram details including adduct, chrom_coherence, and chrom_prominence_scaled
            - 'full': Shows both chromatogram and identification information
    Behavior:
    ...
    if self.features_df is not None and show_features:
        ...
        if show_only_features_with_id:
            if "id_top_name" in feats.columns:
                id_mask = feats["id_top_name"].notna() & (feats["id_top_name"].astype(str).str.strip() != "")
                feats = feats[id_mask]
            else:
                self.logger.warning("show_only_features_with_id=True requested but 'id_top_name' column is missing")
        ...
        if (not handled_colorby) and colorby is not None and colorby not in feats.columns:
            self.logger.warning(
        ...
        if (not handled_colorby) and colorby is not None and colorby in feats.columns:
            ...
            is_numeric = feats[colorby].dtype in ["int64", "float64", "int32", "float32"] or (
                len(colorby_values) > 0 and isinstance(colorby_values.iloc[0], (int, float, np.integer, np.floating))
            )
            ...
            if is_categorical:
                ...
                try:
                    ...
                    colormap_func = get_cmap(marker_cmap if marker_cmap not in ["iridescent", "iridescent_r"] else "tab20")
                    feature_colors = {}
                ...
                if colorby in ["inty", "chrom_area", "chrom_height_scaled"]:
                    ...
                    if len(positive_values) > 0:
                        ...
                        feats[f"{colorby}_log"] = feats[colorby].apply(lambda x: np.log10(x) if x > 0 else continuous_min)
        ...
        if use_categorical_coloring and colorby is not None:
            ...
            for i, group in enumerate(categorical_groups):
                ...
                if len(group_with_ms2) > 0:
                    ...
                    vdims_available = [col for col in vdims_with_ms2_cat if col in group_with_ms2.columns]
                ...
                if len(group_without_ms2) > 0:
                    ...
                    # Filter vdims to only include columns that exist
                    vdims_available = [col for col in vdims_without_ms2_cat if col in group_without_ms2.columns]
        ...
        if use_continuous_coloring and colorby is not None:
            ...
            if len(features_with_ms2) > 0:
                ...
                vdims_available = [col for col in vdims_with_ms2_cont if col in features_with_ms2.columns]
                feature_points_1 = hv.Points(
                    features_with_ms2,
                    kdims=["rt", "mz"],
                    vdims=vdims_available,
            ...
            if len(features_without_ms2) > 0:
                ...
                vdims_available = [col for col in vdims_without_ms2_cont if col in features_without_ms2.columns]
                feature_points_2 = hv.Points(
        ...
        if not handled_colorby and not use_categorical_coloring and not use_continuous_coloring:
            ...
            if len(features_df) > 0:
                ...
                vdims_available = [col for col in vdims_with_ms2 if col in features_df.columns]
                feature_points_1 = hv.Points(
                    features_df,
                    kdims=["rt", "mz"],
                    vdims=vdims_available,
            ...
            if len(features_df) > 0:
                # Filter vdims to only include columns that exist in features_df
                vdims_available = [col for col in vdims_without_ms2 if col in features_df.columns]
                feature_points_2 = hv.Points(
                    features_df,
                    kdims=["rt", "mz"],
                    vdims=vdims_available,
...
def plot_comparison(
    ...
    self.logger.debug(f"Found {len(unique_self)} unique to sample, {len(common)} common, {len(unique_ref)} unique to reference")
    
    # Apply log10 transformation to intensity
    feats_self["inty"] = np.where(feats_self["inty"] <= 0, np.nan, np.log10(feats_self["inty"]))
    feats_ref["inty"] = np.where(feats_ref["inty"] <= 0, np.nan, np.log10(feats_ref["inty"]))
    
    # Apply log10 transformation to quality
    feats_self["quality"] = np.where(feats_self["quality"] <= 0, np.nan, np.log10(feats_self["quality"]))
    feats_ref["quality"] = np.where(feats_ref["quality"] <= 0, np.nan, np.log10(feats_ref["quality"]))
    ...
    for col in cols_to_plot:
        ...
        data_unique_self = unique_self[col].dropna().values if len(unique_self) > 0 else np.array([])
        data_common = common_self[col].dropna().values if len(common_self) > 0 else np.array([])
        data_unique_ref = unique_ref[col].dropna().values if len(unique_ref) > 0 else np.array([])
        ...
        if len(data_unique_self) > 0:
            dist_unique_self = hv.Distribution(data_unique_self, label=f"Unique to sample (n={len(data_unique_self)})").opts(
                fill_color="blue",
                fill_alpha=0.1,
                line_color="blue",
                line_width=2,
        ...
        if len(data_common) > 0:
            dist_common = hv.Distribution(data_common, label=f"Common (n={len(data_common)})").opts(
                fill_color="green",
                fill_alpha=0.1,
                line_color="green",
                line_width=2,
        ...
        if len(data_unique_ref) > 0:
            dist_unique_ref = hv.Distribution(data_unique_ref, label=f"Unique to reference (n={len(data_unique_ref)})").opts(
                fill_color="red",
                fill_alpha=0.1,
                line_color="red",
                line_width=2,
...
def plot_ms2(
    ...
    Parameters:
        features: Feature selection using same format as features_select():
            - None: all features with MS2 spectra
            - int: single feature UID
            - list: list of feature UIDs
            - tuple: range of feature UIDs (min, max)
            - DataFrame: with feature_uid or feature_id column
    ...
    # Get feature UIDs using the helper method
    feature_uids = self._get_feature_uids(features=features, verbose=False)
    
    if not feature_uids:
        self.logger.warning("No features selected.")
        return None
    
    # Filter features_df for selected UIDs with non-null ms2_specs
    if self.features_df is None:
        self.logger.warning("No features_df found.")
        return None
    
    feats = self.features_df.filter(
        pl.col("feature_uid").is_in(feature_uids) & 
    ...
    for idx, row in enumerate(feats_list):
        ...
        self.logger.debug(f"Processing feature {idx+1}/{len(feats_list)}: UID={feature_uid}, ID={feature_id}")
        
        # Get MS2 spectrum
        ms2_specs = row["ms2_specs"]
        if ms2_specs is None or (isinstance(ms2_specs, list) and len(ms2_specs) == 0):
            self.logger.warning(f"Feature {feature_uid} has null or empty ms2_specs, skipping.")
            continue
        ...
        # Build title
        title = f"MS2 spectrum for m/z {mz:.4f}, rt {rt:.2f}, e {energy}, uid {feature_uid}"
        
        # Extract m/z and intensity
        if not (hasattr(spectrum, "mz") and hasattr(spectrum, "inty")):
            self.logger.warning(f"Feature {feature_uid} spectrum missing mz or inty attributes, skipping.")
            continue
        ...
        if len(spec_mz) == 0:
            self.logger.info(f"Feature {feature_uid} has empty spectrum, skipping.")
            continue
        
        self.logger.debug(f"Feature {feature_uid}: spectrum has {len(spec_mz)} peaks")
        ...
        self.logger.debug(f"Created plot for feature {feature_uid}")
        plots.append(stems)
