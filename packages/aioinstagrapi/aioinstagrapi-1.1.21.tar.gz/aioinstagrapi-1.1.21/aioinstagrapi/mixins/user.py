...
from aioinstagrapi.utils import json_value, PersistentDict
...
class UserMixin:
    ...
    _usernames_cache = PersistentDict('usernames_cache.json')  # username -> user_pk
    ...
    async def user_following_gql(self, user_id: str, amount: int = 0) -> List[UserShort]:
        ...
        while True:
            ...
            edges = json_value(data, "user", "edge_follow",
                               "edges", default=[])
            for edge in edges:
    ...
    async def user_following_chunk_v1_new(self, user_id: str, max_id: str = None):
        unique_set = set()
        user_chunk = []
        DATA_KEY = "1$xdt_api__v1__friendships__following(_request_data:$request_data,enable_groups:$enable_groups,include_friendship_status:$include_friendship_status,max_id:$max_id,order:$order,query:$query,user_id:$user_id)"
        headers = {
            'X-Fb-Rmd': 'state=URL_ELIGIBLE',
            'X-Graphql-Client-Library': 'pando',
            'X-Fb-Friendly-Name': 'FollowingList',
            'X-Root-Field-Name': 'xdt_api__v1__friendships__following',
        ...
        variables = {"enable_groups": True,
                     "request_data": {"includes_hashtags": True,
                                      "rank_token": str(uuid4()), "search_surface": "follow_list_page"},
                     "is_pando": True, "include_friendship_status": False, "user_id": user_id, "query": ""}
        if max_id:
        ...
        data = {
            ...
            'locale': 'user',
            'fb_api_req_friendly_name': 'FollowingList',
            'client_doc_id': '161046392810505602396855880514',
        ...
        response = await self.private_request(domain='https://i.instagram.com/graphql/query', headers=headers, data=data,with_signature=False)
        #input(response)
    ...
    async def user_following_v1_chunk(
        ...
        """
        response = (await self.user_following_chunk_v1_new(user_id,max_id))
        if response:
            return response
    ...
    async def user_following_v1(self, user_id: str, amount: int = 0, sleep: float = 3.5) -> List[UserShort]:
        """
        ...
        max_id = ''
        while True:
            users, max_id = await self.user_following_v1_chunk(str(user_id), amount, max_id)
            for user in users:
                if user not in total and (amount and len(total) < amount):
                    total.append(user)
            if len(total) >= amount or max_id is None:
                break
            await asyncio.sleep(random.uniform(sleep, sleep+1))
        if amount:
            total = total[:amount]
    ...
    async def user_followers_gql_chunk(
        ...
        while True:
            ...
            edges = json_value(
                data, "user", "edge_followed_by", "edges", default=[])
            for edge in edges:
                users.append(extract_user_short(edge["node"]))
            end_cursor = page_info.get("end_cursor")
            if not page_info.get("has_next_page") or not end_cursor:
    ...
    async def user_followers_chunk_v1_new(self, user_id: str, max_id: str = None):
        unique_set = set()
        user_chunk = []
        DATA_KEY = "1$xdt_api__v1__friendships__followers(_request_data:$request_data,include_friendship_status:$include_friendship_status,include_global_blacklist_sample:false,max_id:$max_id,query:$query,search_surface:$search_surface,user_id:$user_id)"
        headers = {
            'X-Fb-Rmd': 'state=URL_ELIGIBLE',
            'X-Fb-Request-Analytics-Tags': '{"network_tags":{"product":"567067343352427","purpose":"none","request_category":"graphql","retry_attempt":"0"},"application_tags":"pando"}',
            'X-Graphql-Client-Library': 'pando',
        ...
        variables = {"include_friendship_status":False,
        "query":"",
        "is_pando":True,
        "request_data":{
            "enableGroups":True,
            "rank_token":str(uuid4())},
            "search_surface":"follow_list_page","user_id":user_id}
        if max_id:
            variables['max_id'] = max_id
        ...
        data = {
            ...
            'client_doc_id': '284797047911869366832842874808',
            'enable_canonical_naming': 'true',
            'enable_canonical_variable_overrides': 'true',
            'enable_canonical_naming_ambiguous_type_prefixing': 'true',
    ...
    async def user_followers_v1_chunk(
        ...
        Tuple[List[UserShort], str]
            Tuple of List of users and max_id
        """
        response = (await self.user_followers_chunk_v1_new(user_id, max_id))
        if response:
    ...
    async def user_followers_v1(self, user_id: str, amount: int = 0, sleep: float = 3.5) -> List[UserShort]:
        """
        ...
        max_id = ''
        while True:
            users, max_id = await self.user_followers_v1_chunk(str(user_id), amount, max_id)
            #print(max_id)
            for user in users:
                if user not in total and (amount and len(total) < amount):
                    total.append(user)
    ...
    async def enable_posts_notifications(self, user_id: str, disable: bool = False) -> bool:
        ...
        data = self.with_action_data(
            {"user_id": user_id, "_uid": self.user_id})
