...
from aioinstagrapi.utils import generate_mutation_token
from aioinstagrapi.exceptions import ClientNotFoundError, DirectThreadNotFound
...
class DirectMixin:
    ...
    async def direct_threads_chunk(
        ...
        if cursor:
            params.update(
                {"cursor": cursor, "direction": "older", "fetch_reason": "page_scroll","igd_request_log_tracking_id":str(uuid.uuid4())}
            )
        if seq_id:
            params.update({"seq_id":seq_id})
    ...
    async def direct_send_video(
        ...
        DirectMessage
            An object of DirectMessage
        """
        return await self.direct_send_file(path, user_ids, thread_ids, content_type="video")
