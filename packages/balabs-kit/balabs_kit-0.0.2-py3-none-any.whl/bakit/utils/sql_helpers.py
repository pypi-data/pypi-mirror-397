def generate_kpi_card_changes(
    column_name, usd_change=False, percentage_change=False, first=False
):
    """
    ...
    - USD changes (if usd_change is True), also generates the same calculations for
      USD values.
    ...
    template = f"""
        {f"c.{column_name}" if first else f", c.{column_name}"}
    ...
    if usd_change:
        ...
        if percentage_change:
            template += f"""
                , (
                    (c.{column_name}_usd - COALESCE(p.{column_name}_usd, 0))
                    / COALESCE(NULLIF(p.{column_name}_usd, 0),
                        NULLIF(c.{column_name}_usd, 0))
