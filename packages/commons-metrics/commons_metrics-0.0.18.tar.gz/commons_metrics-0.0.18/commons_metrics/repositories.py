...
class ComponentRepository:
    ...
    def save_adoption(self, version_id: int, component_id: int, aplication_code: str, count: int, date_id: int = None):
        """
        ...
        try:
            cursor = self.db.connection.cursor()
            insert_query = """
                INSERT INTO schmetrc.general_adoption(version_id, component_id, aplication_code, count, date_id) 
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id;
            """
            
            cursor.execute(insert_query, (version_id, component_id, aplication_code, count, date_id))
