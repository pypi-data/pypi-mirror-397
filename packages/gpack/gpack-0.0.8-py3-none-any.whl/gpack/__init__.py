            ...
            class Transformer(ast.NodeTransformer):
                def visit_Constant(self, node):
                    if isinstance(node.value, str):
                        ...
                        # Возвращаем ссылки на обернутые константы TRUE или FALSE
                        if node.value:
...
def _gpack_wrapper(obj):
    if isinstance(obj, bytes):
        ...
        # Используем наши обернутые константы вместо создания новых
        if obj:
            return _GPACK_TRUE
        else:
            return _GPACK_FALSE
    # Если это уже обернутый boolean, возвращаем его как есть
...
class BoolWrapper:
    def __init__(self, value):
        self._value = bool(value)  # Приводим к bool
    ...
    # Для совместимости с операторами and/or/not
...
# Создаем обернутые версии boolean значений как синглтоны
...
# Экспортируем необходимые функции и константы
...
globals()['_GPACK_TRUE'] = _GPACK_TRUE
globals()['_GPACK_FALSE'] = _GPACK_FALSE

# Добавляем в builtins функции обертки
...
# Создаем удобные алиасы для использования в коде
TRUE = _GPACK_TRUE
FALSE = _GPACK_FALSE
globals()['TRUE'] = TRUE
globals()['FALSE'] = FALSE
