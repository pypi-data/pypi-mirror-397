        ...
        try:
            ...
            class Transformer(ast.NodeTransformer):
                ...
                def visit_Compare(self, node):
                    # Mark that we're in a comparison
                ...
                def visit_Assign(self, node):
                    # Mark that we're in an assignment
                ...
                def visit_AugAssign(self, node):
                    # Mark that we're in an augmented assignment
                    old_in_assignment = self.in_assignment
                    self.in_assignment = True
                    node = self.generic_visit(node)
                    self.in_assignment = old_in_assignment
                ...
                def visit_Call(self, node):
                    # Only wrap gpack objects for method calls
                    if isinstance(node.func, ast.Attribute):
                        # Check if this is a gpack object
                        node.func.value = self.visit(node.func.value)
                        
                        # Check if the value is a gpack function call
                ...
                def visit_Subscript(self, node):
                    # Only wrap gpack object subscripts
                    node = self.generic_visit(node)
                    
                    if (isinstance(node.value, ast.Call) and
                        isinstance(node.value.func, ast.Name) and
                        node.value.func.id.startswith('gpack_func_')):
                        # Wrap gpack object subscripts
                        return ast.Call(
                            func=ast.Name(id='_gpack_wrapper', ctx=ast.Load()),
                            args=[node],
                            keywords=[]
            ...
            self.transforming = False
            # Don't print errors in normal operation
            return self.original_compile(source, filename, mode, flags, dont_inherit, optimize)

# Simplified wrapper that preserves comparison behavior
def _gpack_wrapper(obj):
    # If obj is a gpack function call, execute it
    if isinstance(obj, ast.AST):
        # This is an AST node, we need to compile and execute it
        return obj
    
    # For actual gpack instances in runtime
    if hasattr(obj, '_is_gpack') and obj._is_gpack:
        return obj
    
    # For bool values in runtime comparisons
    if isinstance(obj, bool):
        return gpack_func_bool(obj) if not getattr(obj, '_is_gpack', False) else obj
    
    return obj

# Fixed BoolWrapper that properly handles comparisons
class BoolWrapper:
    def __init__(self, value):
        self._value = bool(value)
        self._gpack_instance = None
        self._is_gpack = True  # Mark as gpack object
    ...
    def __eq__(self, other):
        # Handle comparison with regular booleans and other BoolWrappers
    ...
    # Add rich comparison methods for completeness
...
# Create wrapped boolean values (these will be used in comparisons)
...
if (len(sys.argv) > 0 and 
    ...
    try:
        ...
        # Create execution namespace with proper __name__
        exec_ns = {
            '__name__': '__main__', 
            '__file__': sys.argv[0],
