...
async def get_user(
    user_id: UUID,
    async_session = Depends(api_db_deps.get_read_asql)
):
...
async def create_user(
    ...
    async_session = Depends(api_db_deps.get_write_asql)
):
...
async def get_products_async(async_sql_session = Depends(api_db_deps.get_read_asql)):
...
| `get_write_asql()` | `SQLModelAsyncSession \| None` | Asynchronous SQL session for write operations |
| `get_read_asql()` | `SQLModelAsyncSession \| None` | Asynchronous SQL session for read operations |
...
| `get_write_sql()` | `get_write_asql()` | SQL write client |
| `get_read_sql()` | `get_read_asql()` | SQL read client |
