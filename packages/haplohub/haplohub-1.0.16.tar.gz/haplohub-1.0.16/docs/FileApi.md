...
with haplohub.ApiClient(configuration) as api_client:
    ...
    try:
        # List files
        api_response = api_instance.list_files(cohort_id, recursive=recursive, sample_id=sample_id, member_id=member_id, sample_external_id=sample_external_id, member_external_id=member_external_id, path=path)
