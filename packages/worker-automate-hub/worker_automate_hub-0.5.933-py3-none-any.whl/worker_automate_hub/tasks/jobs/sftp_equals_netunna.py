...
# CAMINHOS FIXOS
# =========================================================
EQUALS_OPENSSH_KEY_FIXED = r"C:\Users\automatehub\Documents\GitHub\worker-assets\ssh_key\redesim-07473735000262.com.br.openssh.key"
...
async def load_netunna_ftp_cfg(get_config_by_name, log) -> NetunnaFtpConfig:
    ...
    pasta_destino = pick(c, "pasta_destino", "remote_dir", "dir", "pasta", default="") or "/ADQUIRENTES"
    ...
    log(f"NETUNNA FTP -> host={host} | porta={porta} | usuario={usuario} | pasta_destino={pasta_destino}")
...
async def load_equals_ftp_cfg(get_config_by_name, log) -> EqualsSftpConfig:
    ...
    pasta_remota = pick(c, "pasta_remota", "remote_dir", "dir", "pasta", default="") or "/download"
    ...
    log(f"EQUALS SFTP -> host={host} | porta={porta} | usuario={usuario} | pasta_remota={pasta_remota} | key={key_path}")
        ...
        for attempt in range(1, retry_per_file + 2):
            try:
                ...
                if "550" in msg and dest == original:
                    ...
                    if alt != original:
                        log(f"[{idx}/{total}] FTP recusou nome original (550). Tentando nome alternativo: {alt}")
                        dest = alt
                        continue
...
async def sftp_equals_netunna(task):
    ...
    try:
        ...
        if not files:
            log("Nenhum arquivo D-1 encontrado")
            return
