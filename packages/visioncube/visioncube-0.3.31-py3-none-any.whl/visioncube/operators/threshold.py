...
class DateCodeMask(AbstractTransform):
    def __init__(
        ...
        """
        super().__init__(use_gpu=False)
        self.date_code_list = [f"{week:02d}{date_code_year}" for week in range(53)]
        self.date_code_list.extend([f"{week:02d}{date_code_year+1}" for week in range(53)])
    ...
    def _apply(self, sample):
        ...
        filtered_infos = [
            info for info in res.ocr if info["text"].replace(')', '') in self.date_code_list
        ]
        if filtered_infos:
