...
**First, get your marching orders:**
1. **Use `ask_human` immediately** to ask what they want you to work on (with 30-min timeout)
2. **Wait for their response** - they'll tell you what to do before stepping away
3. **Acknowledge with `send_message`** - let them know you got it, then start working

**While they're away:**
4. **Keep working autonomously** on the task they gave you
5. **Route ALL questions through `ask_human`** - set `timeout_seconds=1800` (30 min)
6. **Send progress updates** - use `send_message` to keep them in the loop
7. **Batch non-urgent questions** - group smaller questions together if possible
