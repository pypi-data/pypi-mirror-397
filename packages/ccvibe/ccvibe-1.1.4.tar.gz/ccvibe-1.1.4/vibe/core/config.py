...
class ProviderConfig(BaseModel):
    ...
    def get_api_base(self) -> str:
        ...
        Priority:
        1. Config file's api_base (if user explicitly customized it)
        2. Environment variable (as fallback)
        3. Default value
        """
        default_base = _DEFAULT_API_BASES.get(self.name, "")

        # If config has a custom value (different from default), use it
        if self.api_base and self.api_base.rstrip("/") != default_base.rstrip("/"):
            return self.api_base.rstrip("/")

        # Otherwise, check env var
        if self.api_base_env_var and (env_base := os.getenv(self.api_base_env_var)):
            return env_base.rstrip("/")

        # Fall back to the config/default value
        return self.api_base.rstrip("/")
...
DEFAULT_PROVIDERS = [
    ...
    ProviderConfig(
        ...
        api_key_env_var="",
        api_base_env_var="LLAMACPP_API_BASE",
        api_style="openai",
        backend=Backend.GENERIC,
    ),
]
