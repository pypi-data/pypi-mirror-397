...
class AgentConfig(BaseModel):
    ...
    def load(
        ...
        try:
            ...
            cfg._mcp_servers_aliases = asyncio.run(_build_mcp_aliases_map(
                connections=cfg._mcp_server_connections,
            ))
            cfg._remote_agents_aliases = asyncio.run(_build_agents_aliases_map(
                mcp_connections=cfg._mcp_agent_connections,
                a2a_connections=cfg._a2a_agent_connections,
            ))
    ...
    async def list_tools(
        ...
        for name in mcp_server_names:
            if name in connections:
                ...
                _logger.warning(f"MCP Server {name} not fonud in configuration.")
        return tools
