...
def _extract_shortcode(shortcode_or_url: str) -> str:
    ...
    """
    # Match URLs like https://slp.sh/aBcDeF or https://staging.slp.sh/aBcDeF
...
class SleapShareClient:
        ...
        try:
            ...
            with open(path, "rb") as f:
                if progress_callback:
                    ...
                    upload_content: BinaryIO | _ProgressReader = _ProgressReader(
    ...
    def get_metadata(self, shortcode_or_url: str) -> Metadata:
        ...
        """
        shortcode = _extract_shortcode(shortcode_or_url)
        metadata_url = f"{self.config.url}/{shortcode}/metadata.json"
...
class _ProgressReader:
    """File wrapper that calls a progress callback while reading."""
    ...
    def __next__(self) -> bytes:
        chunk: bytes = self._file.read(DOWNLOAD_CHUNK_SIZE)
        if not chunk:
            raise StopIteration
