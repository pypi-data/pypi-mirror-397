...
.PHONY: docker-build
docker-build:
	docker build -t typus:latest -f dockerfile .

.PHONY: clean
clean:
	rm -rf dist
	rm -rf typus_dsl.egg-info
	rm -rf *.db*
...
release: format-check
	...
	@echo Replace version in typus/__init__.py
	@sed -i.bak "s/__version__ = \"$(CURRENT_VERSION)\"/__version__ = \"$(NEW_VERSION)\"/" typus/__init__.py

	@echo Remove backup files
	@rm pyproject.toml.bak typus/__init__.py.bak

	@uv sync --all-extras

	@echo "Committing version bump..."
	@git add pyproject.toml typus/__init__.py uv.lock
