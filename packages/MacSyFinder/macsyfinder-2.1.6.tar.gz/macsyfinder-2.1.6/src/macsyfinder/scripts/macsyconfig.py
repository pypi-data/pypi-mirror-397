...
from macsyfinder import __version__ as msf_vers
...
def _validator(cast_func: typing.Callable,
    ...
    if raw == '':
        if default is None:
            raise MacsylibError('Please enter some value')
        else:
            raw = default
    elif sequence:
    ...
    try:
        ...
        raise MacsylibError(f'Invalid value: {err}') from err
    return value
...
def prolog() -> str:
    ...
    rep = fr"""
      ...
      | '_ ` _ \/ __| |_     / __/ _ \| '_ \| |_| |/ _` |
      | | | | | \__ \  _|   | (_| (_) | | | |  _| | (_| |
      |_| |_| |_|___/_|______\___\___/|_| |_|_| |_|\__, |
                     |________|                    |___/

{theme.EMPHASIZE}Welcome to the MacSyFinder {msf_vers} configuration utility.{theme.RESET}
...
def parse_args(args: list[str], color:bool=True) -> argparse.Namespace:
    """
    parse command line
    ...
    if sys.version_info.minor >= 14:
        parser = argparse.ArgumentParser(color=color)
    else:
...
'macsyconfig' has been renamed.  Use 'msf_config' instead.
macsyconfig will be removed in future versions.
