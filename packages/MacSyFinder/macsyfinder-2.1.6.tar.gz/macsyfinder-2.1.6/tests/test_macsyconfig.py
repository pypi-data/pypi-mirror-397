...
import macsyfinder.scripts.macsyconfig as msf_cfg
...
class TestMacsyconfig(MacsyTest):
    ...
    def test_check_file(self):
        test_file = __file__
        sibling = os.path.join(os.path.dirname(test_file), 'test_macsyfinder.py')
    ...
    def test_parse_args(self):
        ...
        parsed_args = msf_cfg.parse_args(args.split()[1:], color=False)
        ...
        try:
            with self.catch_io(err=True):
                with self.assertRaises(TypeError) as ctx:
                    msf_cfg.parse_args(args.split()[1:], color=False)
...
{bin_name}: error: argument --dark-bg: not allowed with argument --white-bg"""
    ...
    def test_functional_dark_theme(self):
        ...
        try:
            with self.catch_io(out=True):
                macsyconfig_args = "msf_config --defaults"
...
Welcome to the MacSyFinder {msf_vers} configuration utility.
...
"""
    ...
    def test_functional_file_already_exists(self):
        ...
        conf_template = self.find_data("macsyfinder_default.conf")
