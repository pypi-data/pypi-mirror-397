...
from robert_test_24 import RobertTest24, AsyncRobertTest24, DefaultAioHttpClient
from robert_test_24._utils import is_dict
...
logging.getLogger("robert_test_24").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[RobertTest24]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with RobertTest24(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncRobertTest24]:
