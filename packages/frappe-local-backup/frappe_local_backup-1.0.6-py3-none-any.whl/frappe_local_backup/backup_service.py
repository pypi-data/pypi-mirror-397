...
class BackupService:
    ...
    def download_backup(
        ...
        if backup_info.database_url:
            ...
            # Offsite backups (S3) use pre-signed URLs, onsite backups need authentication
            use_auth = not backup_info.offsite
            if self._should_download_file(
                client, backup_info.database_url, db_file, site.name, use_auth, force
            ):
