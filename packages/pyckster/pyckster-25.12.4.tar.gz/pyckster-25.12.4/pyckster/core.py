...
class MainWindow(QMainWindow):
    ...
    def _clear_layout(self, layout):
        try:
            while layout.count():
                ...
                w = item.widget()
                if w is not None:
                    w.hide()  # Hide before removing parent to prevent cascading windows
                    w.setParent(None)
                    # On Windows, ensure widget doesn't show as top-level before deletion
                    w.setAttribute(QtCore.Qt.WA_DeleteOnClose, True)
                    w.close()  # Close the widget explicitly
                    w.deleteLater()
    ...
    def setCheckboxes(self):
        ...
        try:
            if hasattr(self, 'seismoShowCrosshairCheck'):
                ...
                self.seismoShowCrosshairCheck.setChecked(self.seismo_show_crosshair)
    ...
    def stackShots(self):
        ...
        for group_key, shot_indices in stackable_groups.items():
            ...
            preview_summary.append(
                f"  • Shots {shot_indices_str} → FFID {preview_next_ffid} "
    ...
    def updateTitle(self):
        ...
        if self.topPlotType == 'seismogram':
            ...
            self.bottomPlotWidget.getPlotItem().setTitle(title, size='12pt', color=self.col, justify='left')
            self.plotWidget.getPlotItem().setTitle("")
        else:
    ...
    def _add_source_marker(self, plot_widget):
        ...
        try:
            ...
            star_html = '<div style="color:#ff0000; font-size:16pt; font-weight:bold; line-height:16pt;">★</div>'  # ★
    ...
    def exportMplPlot(self, type):
        ...
        if fname:
            ...
            # IMPORTANT: Close the figure to prevent cascading windows when switching between shots
            # This prevents memory leaks and avoids cluttering the screen with invisible windows
            plt.close('all')
