...
function initializePopup() {
    ...
    // Check if current tab is on sims.plato.so
    async function checkDomain() {
        try {
            const tab = await getCurrentTab();
            const url = tab?.url || '';

            // Check if URL matches sims.plato.so (including dev/staging variants)
            const isSimsPlato = url.includes('sims.plato.so') ||
                url.includes('dev.sims.plato.so') ||
                url.includes('staging.sims.plato.so');

            if (!isSimsPlato) {
                domainWarning.style.display = 'block';
                // Disable recording buttons if not on sims.plato.so
