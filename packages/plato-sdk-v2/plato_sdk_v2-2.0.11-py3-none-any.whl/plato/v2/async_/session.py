...
from plato._generated.api.v1.simulator import get_env_flows
    ...
    async def login(
        ...
        for env in self.envs:
            ...
            if not env.artifact_id:
                await context.close()
                raise RuntimeError(f"No artifact ID found for env {env.alias}")

            env_flows_response = await get_env_flows.asyncio(
                client=self._http,
                artifact_id=env.artifact_id,
                x_api_key=self._api_key,
            )
            response_data = env_flows_response.model_extra or {}
            data = response_data.get("data", {}).get("flows") if isinstance(response_data.get("data"), dict) else None
            if not data:
                await context.close()
                raise RuntimeError(f"No flows found for env {env.alias}")
            scripts = yaml.safe_load(data)
