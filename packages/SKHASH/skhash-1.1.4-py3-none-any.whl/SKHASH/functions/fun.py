...
def create_takeoff_table(vmodel_depthvp,deptab,delttab,nump,nx0,nd0,takeoff_az_precision):
	...
	for i in range(npts-1,0,-1):
		for idep in range(ndep-1,-1,-1):
			if (z[i-1]<=(deptab[idep]-0.00001)) & (z[i]>=(deptab[idep]+.00001)):
...
def mech_probability(norm1in,norm2in,cangle,prob_max,iterative_avg=False):
	...
	if nf==1: # If there's only one mech, return that mech
		...
		prob[0]=1
		mech_df=pd.DataFrame(columns=['str_avg','dip_avg','rak_avg','prob','rms_diff','rms_diff_aux'],index=[0])
		mech_df['str_avg']=str_avg[0]
		mech_df['dip_avg']=dip_avg[0]
		mech_df['rak_avg']=rak_avg[0]
		mech_df['prob']=prob[0]
		mech_df['rms_diff']=rms_diff[0][0]
		mech_df['rms_diff_aux']=rms_diff[1][0]
		return mech_df
...
def mech_quality(mech_df,qual_criteria_dict):
	...
	# If the following conditions are not met, the lowest quality letter is given.
	mech_df.loc[((mech_df['azimuthal_gap']>qual_criteria_dict['azimuthal_gap']) |
			 	(mech_df['takeoff_gap']>qual_criteria_dict['takeoff_gap']) |
				(mech_df['num_p_pol']<qual_criteria_dict['num_p_pol'])
				),'qual']=qual_criteria_dict['qual_letter'][-1]
...
def calculate_misfit(p_azi_mc,p_the_mc,p_pol,sp_amp,str_avg,dip_avg,rak_avg):
	...
	strike_r=np.deg2rad(str_avg)
	dip_r=np.deg2rad(dip_avg)
	rake_r=np.deg2rad(rak_avg)

	M=np.zeros((3,3))
	M[0,0]=-np.sin(dip_r)*np.cos(rake_r)*np.sin(2*strike_r)-np.sin(2*dip_r)*np.sin(rake_r)*np.sin(strike_r)*np.sin(strike_r)
	M[1,1]=np.sin(dip_r)*np.cos(rake_r)*np.sin(2*strike_r)-np.sin(2*dip_r)*np.sin(rake_r)*np.cos(strike_r)*np.cos(strike_r)
	M[2,2]=np.sin(2*dip_r)*np.sin(rake_r)
	M[0,1]=np.sin(dip_r)*np.cos(rake_r)*np.cos(2*strike_r)+0.5*np.sin(2*dip_r)*np.sin(rake_r)*np.sin(2*strike_r)
	M[1,0]=M[0,1]
	M[0,2]=-np.cos(dip_r)*np.cos(rake_r)*np.cos(strike_r)-np.cos(2*dip_r)*np.sin(rake_r)*np.sin(strike_r)
	M[2,0]=M[0,2]
	M[1,2]=-np.cos(dip_r)*np.cos(rake_r)*np.sin(strike_r)+np.cos(2*dip_r)*np.sin(rake_r)*np.cos(strike_r)
	M[2,1]=M[1,2]

	bb3,bb1=vector_from_sdr(strike_r,dip_r,rake_r)
	bb2=np.cross(bb1,bb3)*-1
	...
	stdr=np.sum(wt[pol_ind]*np.abs(p_pol[pol_ind]))
	...
	if acount>0:
		...
		#Difference between S/P ratio and expected given the solution
		sp_diff=sp_amp[sp_ind]-sp_rat
...
def create_lookup_table(p_dict):
	...
	for vmodel_ind,vmodel_path in enumerate(p_dict['vmodel_paths']):
		...
		vmodel_depthvp[:,0]=np.round(vmodel_depthvp[:,0],5) # Rounds velocity model depths
		if len(vmodel_depthvp)==1:
			raise ValueError('***Velocity model ({}) has only a single velocity. There must be at least two points.***'.format(vmodel_path))
		else:
			if not(all(np.diff(vmodel_depthvp[:,0])>=0)):
				print('*WARNING: The velocity model ({}) is expected to be ordered in terms of increasing depth.'.format(vmodel_path))
				vm_sort_ind=np.argsort(vmodel_depthvp[:,0])
				vmodel_depthvp=vmodel_depthvp[vm_sort_ind,:]
			if np.any(np.diff(vmodel_depthvp[:,0])==0):
				print('*WARNING: The velocity model ({}) has duplicate velocities for a given depth.'.format(vmodel_path))
				vmodel_depthvp=np.delete(vmodel_depthvp,np.where(np.diff(vmodel_depthvp[:,0])==0)[0],axis=0)
...
def circstd(data, high=360, low=0, axis=1):
	'''
	Compute the circular standard deviation.

	Inputs:
		data: your 2d array of azimuths.
		high: High boundary for circular range (default 360 deg).
		low: Low boundary for circular range (default 0 deg).
		axis: Axis along which the standard deviations are computed (default 1)
	'''
	data = np.asanyarray(data)

	# Maps data from [low, high] to [0, 2*pi] radians
	ang_range = high - low
	radians = (data - low) * 2 * np.pi / ang_range

	# Mean resultant vector length
	mean_vector = np.mean(np.exp(1j * radians), axis=axis)
	r_val = np.abs(mean_vector)

	# Circular standard deviation
	r_val = np.clip(r_val, 0, 1)
	res = np.sqrt(-2 * np.log(r_val))

	# Scale back to the original range
	return res * (ang_range / (2 * np.pi))