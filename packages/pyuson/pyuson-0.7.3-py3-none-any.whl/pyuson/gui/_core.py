...
PROGRAM_NAME = "pyuson"  # to get the relevant logger
...
class MainWindow(BaseMainWindow):
    ...
    `self.wplots` : the widget holding all the graphs
    ...
    def init_demodulation(self):
        ...
        self.wplots.add_reference_in_frame_tab()
    ...
    def init_plots(self):
        ...
        # Connect
        self.wplots.sig_roi_changed.connect(self.roi_changed)
    ...
    def init_log(self):
        ...
        """
        super().init_log(PROGRAM_NAME, log_level=LOG_LEVEL)
    ...
    def align_field_finished(self):
        ...
        self.wplots.field.setLabel("top", "frame (#)")
    ...
    def show_frames(self):
        ...
        self.wplots.amp_frame.clearPlots()
        self.wplots.phase_frame.clearPlots()
        if self.worker.proc.is_digital:
            self.wplots.reference_frame.clearPlots()
        ...
        for ind in frame_indices:
            ...
            if self.worker.proc.is_digital:
                ...
                self.wplots.reference_frame.plot(
                    self.worker.proc.get_data_raw("time_meas"),
                ...
                self.wplots.phase_frame.plot(
                    self.worker.proc.get_data_raw("time_meas"),
                    self.worker.proc.get_data_raw(iname)[:, idx],
                    pen=self.wplots.pen_in_phase,
    ...
    def show_frames_demodulated(self):
        ...
        # Change labels
        self.wplots.phase_frame.setLabel("left", "phase (rad)")
        ...
        for ind in range(len(frames_ind)):
            ...
            self.wplots.amp_frame.plot(
                xt,
                amplitude[:, idx],
                pen=self.wplots.pen_amp_demod,
            )
            self.wplots.phase_frame.plot(
                xt,
                phase[:, idx],
                pen=self.wplots.pen_phase_demod,
            )
    ...
    def average_frame_finished(self, status: bool):
        ...
        self.plot_var_field(self.wplots.amp_field, "attenuation", mult=1e-2)
        # Versus time (dB/cm)
        self.plot_var_time(self.wplots.amp_time, "attenuation", mult=1e-2)

        # Phase-shift
        # Versus magnetic field
        self.plot_var_field(self.wplots.phase_field, "phaseshift")
        # Versus time
        self.plot_var_time(self.wplots.phase_time, "phaseshift")
    ...
    def plot_field(self):
        ...
        if self.check_data_loaded():
            ...
            if not hasattr(self.wplots, "field_vline"):
                self.wplots.init_field_crosshair()
