...
class GraphsWidget(QtWidgets.QWidget):
    ...
    def field_crosshair_moved(self, evt):
        ...
        if self.field.sceneBoundingRect().contains(pos):
            ...
            index = int(mousePoint.x() * self.time2frame_scale)
            self.field.setLabel("top", f"frame (#), current : {index}")
            self.field_vline.setPos(mousePoint.x())
    ...
    def clear_all_plots(self):
        ...
        self.amp_frame.clearPlots()
        self.phase_frame.clearPlots()
        self.amp_field.clearPlots()
        self.amp_time.clearPlots()
        self.phase_field.clearPlots()
        self.phase_time.clearPlots()
    ...
    def switch_mouse_pan_mode(self, state: bool):
        ...
        self.amp_frame.getPlotItem().getViewBox().setMouseMode(mode)
        self.phase_frame.getPlotItem().getViewBox().setMouseMode(mode)
        if hasattr(self, "reference_frame"):
            self.reference_frame.getPlotItem().getViewBox().setMouseMode(mode)
        self.amp_field.getPlotItem().getViewBox().setMouseMode(mode)
        self.amp_time.getPlotItem().getViewBox().setMouseMode(mode)
        self.phase_field.getPlotItem().getViewBox().setMouseMode(mode)
        self.phase_time.getPlotItem().getViewBox().setMouseMode(mode)
