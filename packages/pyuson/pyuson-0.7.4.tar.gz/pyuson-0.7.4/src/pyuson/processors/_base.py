...
from .. import sp
from ..data import DataProcessed, DataRaw
from ._types import BaseMetadataProcessor, tBaseProcessor

RESULTS_NAME = "results"
SERIE_NAME = "serie"
PROGRAM_NAME = "pyuson"
NEXUS_FORMAT = ("nx5", "nxs", "h5", "hdf5")

logger = logging.getLogger(__name__)


class BaseProcessor(tBaseProcessor):
    """Basic class to load, analyze and store data during pulsed magnetic field."""

    def __init__(self, file_path: Path | str | None = None, **kwargs):
        """
        Basic class to load, analyze and store data during pulsed magnetic field.

        It provides methods that act as getter and setter methods for the two Data
        objects, `data_raw` and `data_processed`. The `save()` method dumps the Data
        objects to a NeXus-compliant HDF5 file, with the required metadata to restore
        the processing state.

        Common methods to load binary data and integrate a pickup coil voltage are also
        included.

        This class is mostly meant to be subclassed to build more specialized Processor
        objects.
        ...
        """
        # Prepare internals in case it was not set before
    ...
    def idx_serie(self) -> int:
        """
        Used to track the current analyzed serie.

        This is set as a property to sync this index with an item in another place, such
        as the Config object. To do implement this, it is required to a setter for this
        property with the @idx_serie.setter decorator.
        """
        return -1
    ...
    def analysis_window(self) -> Sequence[float]:
        """
        Used to track a 2-element list.

        This is set as a property to sync this index with an item in another place, such
        as the Config object. To do implement this, it is required to a setter for this
        property with the @analysis_window.setter decorator.
        """
        return [0.0]
    ...
    def load_file(self, filepath: str | Path | None, **kwargs):
        """
        Load a configuration or NeXus file with `pyuson.Config()`.
        ...
        **kwargs : passed to `pyuson.Config()`.
        """
        if filepath is not None and str(filepath).endswith(NEXUS_FORMAT):
    ...
    def _init(self):
        ...
        It should happen after loading the configuration file or before loading a NeXus
        file.

        The Data objects from `pyuson.data` modules and the NeXus structure are
        initialized.
        """
        ...
        self.metadata = BaseMetadataProcessor()
    ...
    def load_bin(
        ...
        **kwargs,
        ...
        """
        Read binary file with given precision and endian.
    ...
    def _load_pickup(
        ...
        """
        Load pickup data from binary file.
    ...
    def _set_data(self, where: str, name: str, value: Any):
        """
    ...
    def _remove_data(self, where: str, name: str):
        """
    ...
    def set_data_raw(self, name: str, value: Any):
        """
