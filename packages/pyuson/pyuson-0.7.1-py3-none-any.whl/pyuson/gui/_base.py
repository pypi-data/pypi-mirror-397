...
class BaseMainWindow(QtWidgets.QMainWindow):
    ...
    - _type_wplots : a graph widget that holds all the plots,
    ...
    `_type_wplots` is intantiated as `wplots`.
    ...
    wbatch: protocols.BatchProcessingProtocol
    wbuttons: protocols.MainButtonsProtocol
    wconfiguration: protocols.ConfigurationProtocol
    wfiles: protocols.FileBrowserProtocol
    wplots: protocols.GraphsProtocol
    worker: protocols.DataWorkerProtocol
    ...
    def init_files(self):
        ...
        self.wfiles.sig_autoload_changed.connect(self.update_autoload_from_files)
    ...
    def init_plots(self):
        ...
        self.wplots = self._type_wplots()

    def init_log(self, program_name: str, log_level: str = "info"):
        """
    ...
    def init_layout(self):
        ...
        splitter2.addWidget(self.wplots)
    ...
    def load_file(self):
        ...
        self.wconfiguration.file_parameters["expid"] = self.worker.proc.expid
        self.flag_do_reload_expid = True
        ...
        # Enable buttons
        self.wconfiguration.button_reload.setOpts(enabled=True)
        self.wconfiguration.button_save_cfg.setOpts(enabled=True)

        # Load data directly if autoload is on
        if self.wconfiguration.file_parameters["autoload"]:
            self.load_data()
    ...
    def create_worker(self):
        ...
        # Create worker
        self.worker = self._type_worker(self.wconfiguration.file_parameters["file"])
    ...
    def reload_expid(self):
        ...
        if not self.flag_do_reload_expid:
            ...
            return
        # Infer experiment ID
        expid = self.infer_expid(self.wconfiguration.file_parameters["expid"])
        # Set it, without reloading since that's we're doing
        self.flag_do_reload_expid = False
        self.wconfiguration.file_parameters["expid"] = expid
        self.flag_do_reload_expid = True
    ...
    def save_config(self):
        ...
        cfg_file = Path(self.wconfiguration.file_parameters["file"])
    ...
    def select_file_in_browser(self, is_toml: bool, filepath: str):
        ...
        """
        if is_toml:
            self.wconfiguration.file_parameters["file"] = filepath
        else:
            self.wconfiguration.file_parameters["expid"] = filepath
    ...
    def batch_process_finished(self):
        ...
        self.flag_do_reload_expid = False
        self.wconfiguration.file_parameters["expid"] = self.batch_expids[-1]
        self.flag_do_reload_expid = True
        self.batch_files = []
        self.batch_expids = []
    ...
    def dropEvent(self, e):
        ...
        for url in e.mimeData().urls():
            ...
            if file_path.endswith(ALLOWED_FORMAT):
                self.wconfiguration.file_parameters["file"] = file_path
                self.logger.debug("Dropped file")
