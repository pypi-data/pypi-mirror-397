...
@dataclass_json
@dataclass
class V4AclCreateRequest(CtyunOpenAPIRequest):
    ...
    applyToPublicLb: Optional[bool] = None  # 是否启用acl管控lb流量，不传默认不管控
...
@dataclass_json
@dataclass
class V4AclCreateResponse(CtyunOpenAPIResponse):
    statusCode: Optional[int] = None  # 返回状态码（800为成功，900为失败）
    errorCode: Optional[str] = None  # 错误码，为product.module.code三段式码
    error: Optional[str] = None  # 错误码，为product.module.code三段式码
    message: Optional[str] = None  # 失败时的错误描述，一般为英文描述
    description: Optional[str] = None  # 失败时的错误描述，一般为中文描述
    returnObj: Optional['V4AclCreateReturnObj'] = None  # 成功时返回的数据
