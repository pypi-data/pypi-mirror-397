...
class LimaSavingParameters(BeaconObject):
    ...
    def __init__(self, config, proxy, name):
        ...
        # Check if HW compression is supported in this version
        self._has_hw_comp = "saving_use_hw_comp" in proxy
    ...
    _use_hw_comp = BeaconObject.property_setting("_use_hw_comp", default=False)

    _statistics_log_enable = BeaconObject.property_setting(
        "_statistics_log_enable", default=False
    )

    @_statistics_log_enable.setter
    def _statistics_log_enable(self, value):
    ...
    def to_dict(self):
        ...
        pars = {
            ...
            "saving_statistics_log_enable": self.settings["_statistics_log_enable"],
        }
        if self._has_hw_comp:
            pars["saving_use_hw_comp"] = self.settings["_use_hw_comp"]
        return pars
...
{mode_prefix}{av_modes}
            ...
            use HW comp(ression):      {tmp.get('saving_use_hw_comp', False)}
            statistics log enable:     {self._statistics_log_enable}
            """
        )
