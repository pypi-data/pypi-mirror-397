...
class _Signal:
    ...
    def _missing_edge_of_gaussian_left(npoints, frac_missing):
        ...
        return numpy.concatenate(
            (
                signal.windows.gaussian(p, 0.1 * npoints)[p2:],
                numpy.zeros(p2),
    ...
    SIGNALS = {
        ...
        "bimodal": lambda npoints: numpy.concatenate(
            (
                signal.windows.gaussian(npoints - npoints // 2, 0.15 * npoints) * 1.5,
                signal.windows.gaussian(npoints // 2, 0.15 * npoints),
            )
        ),
        ...
        - signal.windows.gaussian(npoints, 0.2 * npoints),
        "expo_gaussian": lambda npoints: numpy.exp(
            signal.windows.gaussian(npoints, 0.1 * npoints) * 30
        ),
...
class AutoFilterDetMon:
    ...
    def __init__(self, name, config):
        self._axis = SoftAxis(name + "_axis", self)
    ...
    def init_signal(self):
        ...
        self._data = numpy.exp(signal.windows.gaussian(n, stdev) * 10)
