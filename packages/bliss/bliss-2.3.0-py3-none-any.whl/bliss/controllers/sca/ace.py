    ...
    def reading(self):
        ...
        with gevent.Timeout(2.0):
            while not self._stop_flag:
                ...
                if status == "D":
                    self._emit_new_data([[numpy.float64(counts)]])
                    break
                else:
                    gevent.sleep(0.001)
...
class Ace:
    ...
    def _get_value(self, param):
        ...
        if param == "sca_win":
            if "WIN" in ans:
                return numpy.float64(ans.split()[2])
            else:
                return "No window value available with sca_mode = INT"
        elif param == "sca_low":
            return numpy.float64(ans.split()[1])

        elif param == "sca_mode":
            return ans.split()[0]

        elif param == "sca_hhv":
            return numpy.float64(ans.split()[0])
        # ---------------
    ...
    def putget(self, command):
        ...
        """
        log_debug(self, f"putget '{command}' ")
    ...
    def sca_low(self):
        ...
        return numpy.float64(self._get_value("sca_low"))
