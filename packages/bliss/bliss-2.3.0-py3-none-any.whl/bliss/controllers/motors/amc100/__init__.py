...
class AMC100(Controller):
    ...
    def activate_closed_loop(self, axis, onoff=True):
        ...
        self._dev.control.setControlMove(axis.channel, onoff)
        self._closed_loop[axis.name] = onoff

    def _do_get_closed_loop_state(self, axis):
        state = self._closed_loop.get(axis.name, False)
        return ClosedLoopState.ON if state else ClosedLoopState.OFF
    ...
    def read_position(self, axis):
        if self._closed_loop[axis.name]:
