...
class FlexDC(Controller):
    ...
    def __info__(self):
        """
        Return FlexDC Specific information.
        """
        info_str = "FlexDC CONTROLLER:\n"
        info_str += f"     ID = {self.get_id()}\n"

        return info_str

    def get_axis_info(self, axis):
        """
        Return FlexDC specific information about <axis>
        """
        info_str = f"FlexDC parameters for {axis.name} Axis:\n"
        status_byte = self._flexdc_query("%sMS" % axis.channel)
        info_str += f"     status (MS): {status_byte}\n"
