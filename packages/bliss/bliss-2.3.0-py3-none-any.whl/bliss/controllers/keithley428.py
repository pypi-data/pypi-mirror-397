...
class keithley428(BeaconObject, HasMetadataForScan):
    def __init__(self, name, config_tree):
        ...
        self._refgain_log10 = None
        self._refoffset = None
    ...
    def __info__(self):
        ...
        try:
            ...
            info_str += f"filter_rise_time: {self.filter_rise_time_str}\n"
    ...
    def scan_metadata(self):
        ...
        metadata["gain_log10"] = self.gain_log10
        metadata["filter_rise_time"] = self.filter_rise_time_str
        metadata["refgain_log10"] = self._refgain_log10
        metadata["refoffset"] = self._refoffset
        return metadata
    ...
    @property
    def filter_rise_time_str(self):
        return self._FilterRiseTimes[self.filter_rise_time]
    ...
    def gain(self) -> int:
        ...
        gain = pow(10, gain_log10)  ## * ur.V / ur.A
        return gain
    ...
    def gain(self, value: int):
        ...
        if value not in self._gain_values:
            raise ValueError(
                f"gain value {value} not a power of ten in range [1e3; 1e10]"
            )
    ...
    @gain_log10.setter
    def gain_log10(self, gain_log: int):
        ...
        # adjust the suppress current offset
        if self._refgain_log10 is not None:
            gainval = self._refgain_log10 - gain_log
            newoff = self._refoffset * math.pow(10, gainval)
            self._offset = newoff
            self.put(f"S{newoff:g},0X")
    ...
    def offset(self, refgain_log10, refoffset):
        """
        This is the offset current to apply, according to that gain.
        For the other gain, a multiple factor will be applied (factor of 10
        if gain+1, factor of 100 if gain+2 ... divide by 10 if gain-1 ...)
        """
        if refgain_log10 < 3 or refgain_log10 > 10:
            print("gain must be an integer between 3 and 10")
            return
        self._refgain_log10 = refgain_log10
        self._refoffset = refoffset
        curr_gain = self.gain_log10
        val = refgain_log10 - curr_gain
        newoff = refoffset * math.pow(10, val)
        self._offset = newoff
        self.put(f"S{newoff:g},0X")
