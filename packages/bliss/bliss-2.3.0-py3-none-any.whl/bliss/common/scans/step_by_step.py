...
def _get_default_acquisition_chain(default_chain=DefaultAcquisitionChain()):
    try:
        return current_session.default_acquisition_chain
    except AttributeError:
        # no session... still we can return a default chain,
        # this is useful for tests only
        return default_chain


DEFAULT_CHAIN = Proxy(_get_default_acquisition_chain)
...
def ascan(
    ...
    *counter_args: _countables,
    ...
    Keyword Args:
        ...
        stab_time (float): stabilisation time after each move [default: None]
...
def lookupscan(
    ...
    *counter_args: _countables,
    ...
    """Scan multiple motors with their own position table.
...
def anscan(
    ...
    *counter_args: _countables,
    ...
    """
    Absolute scan with of multiple motors.
    ...
    10 intervals scan at 0.1 second integration on motor **m1** from
    ...
    *stop_m2_pos* and with one counter.
    ...
    if not scan_info.has_default_curve_plot():
        ...
        scan_info.add_curve_plot(x=mot.fullname)
    ...
    for motor, start, stop in motor_tuple_list:
        ...
        scan_info.set_channel_meta(
            motor.fullname, start=start + d, stop=stop + d, points=npoints
        )
...
def dnscan(
    ...
    *counter_args: _countables,
    ...
    """
    Relative scan on multiple motors.
