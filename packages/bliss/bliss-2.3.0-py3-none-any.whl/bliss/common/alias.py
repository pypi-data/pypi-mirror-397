...
class CounterAlias(ProxyWithoutCall):
    def __init__(self, alias_name, cnt):
        if isinstance(cnt, ProxyWithoutCall):
            raise RuntimeError(
...
class ObjectAlias(ProxyWithoutCall):
    def __init__(self, alias_name, obj):
        if isinstance(obj, ProxyWithoutCall):
            raise RuntimeError(
    ...
    @property
    def fullname(self):
        original_fullname = self.__wrapped__.fullname
        prefix, _, _ = original_fullname.rpartition(":")
        return f"{prefix}:{self.__alias_name__}"
    ...
    def original_fullname(self):
        # TODO :
        # check why it is not trying to use the fullname
        # of the original object instead of its short name ?
        # try:
        #     return self.__wrapped__.fullname
        # except AttributeError:
        return self.__obj_name__
...
class Aliases:
    ...
    def __create_alias(self, alias_name: str, obj_or_name: str):
    ...
    def add(self, alias_name, obj_or_name):
        ...
        return self.__create_alias(alias_name, obj_or_name)
    ...
    def set(self, alias_name, obj_or_name):
        alias = self.get(alias_name)
        if alias:
            try:
                self.remove(alias_name)
                return self.add(alias_name, obj_or_name)
            except Exception:
...
class MapWithAliases(Map):
    ...
    def get_positioners_iter(self) -> Iterator[Axis]:
        """Iter all the axes which are positioners"""
        for axis in self.get_axes_iter():
            if axis._positioner:
                yield axis

    def get_axes_names_iter(self) -> Iterator[str]:
        """Iter all the axis names which are positioners

        NOTE: This function should be removed, better to use `get_positioners_iter`
        """
        for axis in self.get_positioners_iter():
            yield axis.name

    def get_axes_positions_iter(self, on_error=None):
        """
        NOTE: It contains very specific data fetching and formatting, ideally
              this should be part of another module
        """
