...
from flint.client import plots as flint_plots
from flint.client import proxy as flint_proxy

from flint.client.proxy import FLINT_LOGGER
from flint.client.proxy import FLINT_OUTPUT_LOGGER  # noqa: F401
...
def get_axis_and_counter_fullnames():
    result = []
    result.extend([p.fullname for p in global_map.get_positioners_iter()])
    result.extend([c.fullname for c in global_map.get_counters_iter()])
    return result
...
def get_channel_names(*objs) -> list[str]:
    ...
    for obj in objs:
        ...
        if isinstance(obj, str):
            if is_pattern(obj):
                if not all_objects:
                    all_objects = get_axis_and_counter_fullnames()
                ...
                if alias is not None:
                    ...
                    if not all_objects:
                        all_objects = get_axis_and_counter_fullnames()
                    channel_names = get_matching_names([obj], all_objects, True)[
                        obj
                    ] or [obj]
                else:
                    channel_names = [obj]
        elif hasattr(obj, "fullname"):
            # Assume it's a counter or axis
...
def get_channel_name(channel_item):
    ...
    if hasattr(channel_item, "fullname"):
        # Assume it's a counter or an axis
