...
def wait_tango_device(
    ...
    admin_device_fqdn: Optional[str] = None,
    secondary_devices_fqdns: Optional[list[str]] = None,
    state=DevState.ON,
    secondary_state=DevState.ON,
    wait_db: bool = True,
    timeout: float = 10,
    ...
    :param str secondary_devices_fqdns: secondary tango device URL for ping and state checking
    ...
    exception = None
    err_msg = None
    secondary_devices_fqdns = secondary_devices_fqdns or []
    try:
        with gevent.Timeout(timeout):
            ...
            for device_fqdn in secondary_devices_fqdns:
                wait_tango_device(
                    device_fqdn=device_fqdn,
                    wait_db=False,
                    state=secondary_state,
                    timeout=timeout,
                    timeout_msg=f"Secondary device {device_fqdn} is not running",
                )
