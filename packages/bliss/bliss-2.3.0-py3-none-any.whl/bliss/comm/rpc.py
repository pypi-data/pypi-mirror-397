...
def _discover_object(obj):
    if isinstance(obj, proxy.ProxyWithoutCall):
        obj = obj.__wrapped__
...
class RpcConnection:
    ...
    def connect(self):
        ...
        if self.host:
            ...
            # No host means connection with an UNIX socket
            self._socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            try:
                self._socket.connect(self.port)
            except BaseException:
                # Clean up, as we are about to connect
                self._socket = None
                raise
    ...
    def _call__(self, code, args, kwargs, retry_on_disconnect=True):
        ...
        if self._socket is None:
            try:
                ...
                raise RpcConnectionError(f"Disconnected from {self._address}")
