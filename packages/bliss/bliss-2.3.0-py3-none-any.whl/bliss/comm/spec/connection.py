...
from bliss.common.logtools import log_error, log_exception, log_debug
    ...
    while True:
        ...
        try:
            ...
            with gevent.Timeout(1, SpecClientNotConnectedError):
                ...
                conn.socket = s
                conn.send_msg_hello()
                m = message.message(version=None)
    ...
    while True:
        ...
        while offset < len(s):
            if received_message is None:
                received_message = message.message(version=serverVersion)

            consumedBytes = received_message.readFromStream(s[offset:])
            ...
            if received_message.isComplete():
                try:
                    try:
                        # dispatch incoming message
                        if received_message.cmd == message.REPLY:
                            replyID = received_message.sn
                            if replyID > 0:
                                try:
                                    ...
                                    reply.update(
                                        ...
                                        received_message.type == message.ERROR,
                                        received_message.err,
                                    )
                        elif received_message.cmd == message.EVENT:
                            try:
                                ...
                                channel.update(
                                    received_message.data,
                                    received_message.flags == message.DELETED,
                                )
                    except Exception:
