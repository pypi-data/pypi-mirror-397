...
class Modbus_RTU:
    ...
    def _cmd(self, address, func_code, nb, data_write=None):
        ...
        nb = int(
            nb
        )  # in python2 is was always int in python3 it occurred to be float, so we make sure it is always int
...
class ModbusTcp:
    ...
    @try_connect_modbustcp
    def read_discrete_inputs(self, address, nb_inputs, timeout=None):
        log_debug_data(self, "read_coils", {"address": address, "num": nb_inputs})
        timeout_errmsg = "timeout on read_coils tcp (%s, %d)" % (self._host, self._port)
        nb_bytes = ((nb_inputs + 7) & ~7) // 8
        struct_format = "%dB" % nb_bytes
        result = self._read(
            0x02, address, nb_inputs, struct_format, timeout_errmsg, timeout
        )
        if isinstance(result, tuple):
            result = [int("{0:08b}".format(x)[::-1], 2) for x in result]
        else:
            result = int("{0:08b}".format(result)[::-1], 2)
        a = numpy.array(result, dtype=numpy.uint8)
        return numpy.unpackbits(a)[:nb_inputs]
