...
from bliss.common.axis.axis import DEFAULT_POLLING_TIME
from bliss.common.axis.trajectory import CyclicTrajectory
...
class UndershootMixin(object):
    """
    Included in MotorMaster and MeshTrajectoryMaster.
    This mixin provides calculation of acceleration distance required by a movement.
    Can consider additionnal margins if specified.
    * [<start_margin>] : in motor units
    * [<end_margin>] : in motor units
    """
    ...
    def undershoot(self):
        """
        Return acceleration distance required by a movement.
        """
    ...
    def _calculate_undershoot(self, pos, end=False):
        """
        Return absolute position corresponding to <pos> corrected by:
        * undershoot
        * margin if specified at object initialization.
        """
        direction = 1 if self.end_pos >= self.start_pos else -1
        direction *= -1 if end else 1
        pos -= direction * self.undershoot

        if end:
            margin = direction * self._undershoot_end_margin
        else:
            margin = direction * self._undershoot_start_margin
        return pos - margin
...
class HKLTrajectoryMaster(AcquisitionMaster):
    def __init__(
        ...
        **keys,
        ...
        AcquisitionMaster.__init__(
            self, calc_axis, name=calc_axis.name, trigger_type=trigger_type, **keys
        )
