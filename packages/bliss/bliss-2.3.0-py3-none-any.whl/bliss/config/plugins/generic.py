...
def create_objects_from_config_node(cfg_obj, cfg_node):
    ...
    if cfg_node.plugin == "bliss":  # act as the older Bliss plugin
        ...
        klass_name = node.get("name")
        if klass_name != item_name:
            if klass_name is not None:
                raise RuntimeError(
                    f"subitem '{item_name}' cannot be imported directly and should be accessed via its controller '{klass_name}'"
                )
        ...
        else:  # prevent instantiation of an item coming from a top object that is not a ConfigItemContainer
            raise TypeError(
