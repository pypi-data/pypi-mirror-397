...
class ConfigNode(MutableMapping):
    ...
    @staticmethod
    def goto_path(d: dict, path_as_list: list[str], key_error_exception=True):
...
class Config(metaclass=Singleton):
    ...
    def reload(self, base_path=None, timeout=3):
        ...
        """
        self._do_reload(base_path=base_path, timeout=timeout)
        if is_bliss_shell():
            self.parsing_report()
    ...
    def _do_reload(self, base_path=None, timeout=3):
        ...
        for path, file_content in path2file:
            ...
            if isinstance(fs_node, MutableSequence):
                # This have to be transformed into a map
                # and move the content into __children__
                key = [k for k, v in fs_node._parent.items() if v is fs_node][0]
                new_node = ConfigNode(fs_node._parent)
                fs_node._parent[key] = new_node
                new_node["__children__"] = fs_node
                fs_node = new_node
