...
from bliss.common.proxy import deref_proxy


_default_connection: connection.Connection | None = None
...
def lock(*devices, priority=50, timeout=10, connection: Connection | None = None):
    """Lock a set of BLISS objects"""
    connection = connection or get_default_connection()
    devices_name = [deref_proxy(d).name for d in devices]
    connection.lock(*devices_name, timeout=timeout, priority=priority)


def unlock(*devices, priority=50, timeout=10, connection: Connection | None = None):
    """Unlock a set of BLISS objects"""
    connection = connection or get_default_connection()
    devices_name = [deref_proxy(d).name for d in devices]
    connection.unlock(*devices_name, timeout=timeout, priority=priority)


def get_cache_address(connection: Connection | None = None):
    connection = connection or get_default_connection()
...
def get_redis_proxy(
    db=0, connection: Connection | None = None, caching=False, shared=True
):
...
def close_all_redis_connections(connection: Connection | None = None):
    connection = connection or get_default_connection()
...
def get_config_file(file_path, connection: Connection | None = None):
    connection = connection or get_default_connection()
...
def get_config_db_files(
    base_path="", timeout=3.0, connection: Connection | None = None
):
    """
...
def get_config_db_tree(base_path="", timeout=3.0, connection: Connection | None = None):
    connection = connection or get_default_connection()
    return connection.get_config_db_tree(base_path, timeout=timeout)


def set_config_db_file(
    filepath, content, timeout=3.0, connection: Connection | None = None
):
    connection = connection or get_default_connection()
    connection.set_config_db_file(filepath, content, timeout=timeout)


def remove_config_file(file_path, connection: Connection | None = None):
    connection = connection or get_default_connection()
    return connection.remove_config_file(file_path)


def move_config_path(src_path, dst_path, connection: Connection | None = None):
    connection = connection or get_default_connection()
    return connection.move_config_path(src_path, dst_path)


def get_python_modules(base_path="", timeout=3.0, connection: Connection | None = None):
    connection = connection or get_default_connection()
    return connection.get_python_modules(base_path, timeout)


def get_log_server_address(connection: Connection | None = None):
    connection = connection or get_default_connection()
