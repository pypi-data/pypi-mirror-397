...
class AbstractMaterial(MaterialBackend):
    ...
    def cross_section(self, energies, kind=None, fix_cs=None):
        """
        :param sequence or num energies: keV
        :param str kind: total (default), coherent, incoherent, ...
        :param dict fix_cs: fix cross-section for optimization
        ...
        if fix_cs is None:
            ...
            fix_cs = {el: cs for el, cs in zip(elements, arr)}
        cs = fix_cs.get(self)
        if cs is None:
            # Calculate and cash the cross-section
            cs = 0
            for item, wfrac in self.mass_fractions.items():
                cs += wfrac * item.cross_section(energies, kind=kind, fix_cs=fix_cs)
            fix_cs[self] = cs
        return cs
...
class Element(AbstractMaterial):
    ...
    def cross_section(self, energies, kind=None, fix_cs=None):
        ...
        :param sequence or num energies: keV
        :param str kind: total (default), coherent, incoherent, ...
        :param dict fix_cs: fix the cross-section for optimization
        :returns numpy.ndarray:
        """
        if kind is None:
            kind = "total"
        ...
        cs = fix_cs.get(self)
        if cs is None:
            cs = self.get_backend().cross_section(self._Z, energies, kind)[0]
            fix_cs[self] = cs
        return cs
