...
class HklGeometry(object):
    ...
    HC_OVER_E = 12.39842  # Angstrom (with E in keV)
    ...
    def _init_settings(self):
        ...
        self._extended_limits = self._settings.get(
            "geo_limits_extended", {name: (-360, 360) for name in self._axis_names}
        )
    ...
    def set_calc_controller(self, hklmot):
        self._calc_controller = hklmot

    @property
    def extended_limits(self):
        return self._extended_limits

    @extended_limits.setter
    def extended_limits(self, dict_values):
        self._extended_limits.update(dict_values)
        self._settings["geo_limits_extended"] = self._extended_limits
    ...
    def set_axis_limits(self, axis_limits):
        for axis_name, (axis_min, axis_max) in axis_limits.items():
            axis = self.__get_axis(axis_name)
            _min = min(axis_min, axis_max)
            _max = max(axis_min, axis_max)
            axis.min_max_set(_min, _max, self.UNIT)
            self._geometry.axis_set(axis_name, axis)
            self._axis_limits_init[axis_name] = True
            self._settings["{0}_limits".format(axis_name)] = (_min, _max)
    ...
    def get_axis_names_involved(self, *pseudo_names, return_read_axes=False):
        engine_name = None
        ...
        if return_read_axes:
            return engine.get_axis_read_names()
        else:
    ...
    def __alias_to_pseudo(self, alias):
        ...
        if len(pars) >= 2:
            ...
            raise ValueError(f"Invalid pseudo name [{alias}]")
        if engname not in list(self._engines.keys()):
            raise ValueError(f"Invalid engine name in alias [{alias}]")
        if name not in self._engines[engname].get_pseudo_names():
            raise ValueError(f"Invalid pseudo name in alias [{alias}]")
        return (engname, name)
    ...
    def set_pseudo_pos(self, pos_dict, all_solutions=None):
        ...
        solutions = []
        curr_pos = self.get_axis_pos()

        # === try to find solutions from current positions
        try:
            solutions.extend(engine.set_pseudo_pos(engine_pos))
        except Exception as e:
            log_debug(self, "set_pseudo_pos %s from %s: %s", pos_dict, curr_pos, str(e))

        # === try to find extra solutions from random positions
        frozen_angles = {
            ax: self._calc_controller.frozen_angles.get(ax, curr_pos[ax])
            for ax in engine.get_axis_constant_names()
        }
        try:
            all_rand_pos = self._generate_random_position(frozen_angles)
            for rand_pos in all_rand_pos:
                self.set_axis_pos(rand_pos, update=False)
                try:
                    solutions.extend(engine.set_pseudo_pos(engine_pos))
                except Exception as e:
                    log_debug(
                        self,
                        "set_pseudo_pos %s from %s: %s",
                        pos_dict,
                        rand_pos,
                        str(e),
                    )
            ...
            # restore current axes positions
            self._calc_controller.update()

        # === filter duplicated solutions
        solutions = self._filter_duplicated(solutions)

        # === raise if no solution was found
        if not solutions:
            err_str = f"Cannot find a solution for {engine_pos}"
            if engine_name == "hkl":
                tth = self._sample.calc_tth(*engine_pos.values())
                err_str += f" with 2_theta = {tth:.3f}"
            raise RuntimeError(err_str)

        # === sort solutions by distance to current positions
        solutions.sort(key=self.calc_dist)

        # === select and set preferred solution
        axis_pos = self.__select_solution(solutions)
        self.set_axis_pos(axis_pos, update=True)
    ...
    def _generate_random_position(self, frozen_angles):
        """Generate random positions within geometry limits (keeping frozen angles)"""

        geo_limits = self.get_axis_limits()
        num_of_axes = len(geo_limits)
        rpos = []
        for rvect in rand(self._rand_num, num_of_axes):
            rpos.append(
                {
                    ax: frozen_angles.get(ax, ll + rand_factor * (hl - ll))
                    for (ax, (ll, hl)), rand_factor in zip(geo_limits.items(), rvect)
                }
            )

        return rpos

    def _filter_duplicated(self, solutions, keep_original=True):
        seen = set()
        out = []
        for sol in solutions:
            key = tuple(
                sorted((k, round(v, ndigits=self._ndigits)) for k, v in sol.items())
            )
            if key not in seen:
                seen.add(key)
                out.append(sol if keep_original else dict(key))
        return out

    def calc_dist(self, solution):
        curr_pos_dict = self.get_axis_pos()
        dist = sum(
            [abs(solution[ax] - curr_pos) for ax, curr_pos in curr_pos_dict.items()]
        )
        return dist
    ...
    def info(self, mapnames=None, ordered_names=None):
        if mapnames is None:
            mapnames = {}

        if ordered_names:
            axis_names = ordered_names
        else:
