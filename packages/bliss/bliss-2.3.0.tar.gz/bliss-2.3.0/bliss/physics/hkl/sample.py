...
from .common import numpy_to_matrix
...
class HklSample(object):
    ...
    def get_B(self):
        _, _, a3, alpha1, _, _ = self.get_lattice()
        b1, b2, b3, _, beta2, beta3 = self.get_reciprocal_lattice()
        B = numpy.array(
            [
                [
                    b1,
                    b2 * numpy.cos(numpy.radians(beta3)),
                    b3 * numpy.cos(numpy.radians(beta2)),
                ],
                [
                    0,
                    b2 * numpy.sin(numpy.radians(beta3)),
                    -b3
                    * numpy.sin(numpy.radians(beta2))
                    * numpy.cos(numpy.radians(alpha1)),
                ],
                [0, 0, 2 * numpy.pi / a3],
            ]
        )
    ...
    def calc_tth(self, h, k, l):  # noqa: E741
        q = self.get_B() @ [h, k, l]
        wl = self._geometry.wavelength_get(self.UNIT)
        tth = 2 * numpy.arcsin(wl * numpy.sqrt(q @ q) / (4 * numpy.pi))
        return numpy.degrees(tth)
    ...
    def info(self, mapnames=None, ordered_names=None):
        ...
        if not nref:
            ...
            axstr = " ".join(
                ["{0:>10}".format(mapnames.get(name, name)) for name in axis_names]
            )
