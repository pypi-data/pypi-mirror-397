...
class LinearTrajectory(object):
    ...
    Arguments:
        pi: Initial position
        pf: Final position
        velocity: Velocity (relative to the second)
        acceleration: Acceleration (relative to the second)
        ti: Initial time (in second), if None (default) this value is set with
            `time.time()`.
    """
    ...
    def position(self, instant: float | None = None) -> float:
        """Position at a given instant in time.

        Argument:
            instant: An instant in second. If unset, it uses the actual instant
                     with `time.time()`.
        """
        if instant is None:
            instant = time.time()
        ...
        if instant < self.tb:
            ...
            decel_time = self.tf - instant
            decel_dp = 0.5 * self.acceleration * decel_time**2
            # print("SLOWING DOWN", self.pf - f * decel_dp)
            return self.pf - f * decel_dp

    def instant(self, position: float) -> float:
        """Instant (in second) when the trajectory passes at the given position.

        If the `ti` was not set in the constructor, the instant is relative to
        ...
        # Durig deceleration
        f = 1 if self.positive else -1
        decel_dp = f * (self.pf - position)
        if decel_dp < 0:
            decel_dp = 0
        dt += (self.tf - self.tb) - math.sqrt(2 * decel_dp / self.acceleration)
        return dt

    def __repr__(self):
        return f"{type(self)}({self.pi}, {self.pf}, {self.velocity}, {self.acceleration}, {self.ti})"
