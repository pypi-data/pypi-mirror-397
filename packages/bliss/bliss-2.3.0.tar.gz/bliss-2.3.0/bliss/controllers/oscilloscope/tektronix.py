...
class TektronixOscCtrl(OscilloscopeHardwareController):
    ...
    def opc(self):
        stat = self.write_readline("*OPC?")  # OPeration Complete
        if stat == "0":
            return False
        elif stat == "1":
            return True
    ...
    def header_to_dict(self, header_string):
        ...
        for key, value in res.items():
            ...
            # Returned string may not contain a closing double-quote.
            if value.count('"') % 2 == 1:
    ...
    def acq_read_channel(self, name, acq_time):
        # Wait for trigger...
        ...
        # print(RED(f"--------     record_length: {length}   --------"))
        ...
        # print(RED(f"--------     data len: {len(datastring)}   --------"))

        raw_data = numpy.frombuffer(datastring[-2 * length - 2 : -2], dtype=numpy.int16)
        # raw_data = numpy.frombuffer(datastring[-2*length-2:-2], dtype=numpy.int16)
        data = (raw_data.astype(numpy.float64) - header["YOFF"]) * header[
            "YMULT"
        ] + header["YZERO"]
