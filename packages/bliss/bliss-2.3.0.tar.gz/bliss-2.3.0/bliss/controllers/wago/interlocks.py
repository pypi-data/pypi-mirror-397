...
ParsedChannelLine = namedtuple(
    "ParsedChannelLine",
    "logical_device logical_device_channel type low_limit high_limit hysteresis flags dac dac_scale dac_offset",
)
...
def interlock_parse_channel_line(line):
    ...
    regex_control_ch_line = (
        ...
        + r"|".join(TYPES.keys())
        + r")( +(?P<min>\-?[0-9\.]+))?( +(?P<max>\-?[0-9\.]+))?( +(?P<hysteresis>[0-9\.]+))?( +(?P<chflags>(inverted|inv|INV|INVERTED|sticky|STICKY|monitor|MONITOR| )*))?"
    )
    ...
    if m:
        ...
        line = ParsedChannelLine(
            ...
            min_,
            max_,
            float(m["hysteresis"]) if m["hysteresis"] is not None else None,
...
def specfile_to_yml(iterable):
    ...
    for line in remove_comments(splitlines(iterable)):
        if interlock_parse_relay_line(line) is not None:
            ...
            if c.hysteresis is not None:
                c_y["hysteresis"] = pretty_float(c.hysteresis)
...
def interlock_to_yml(interlock_list):
    ...
    for intrlck in interlock_list:
        ...
        for ch in intrlck["channels"]:
            ...
            if ch["hysteresis"] is not None:
                c_d["hysteresis"] = float(
                    round(decimal.Decimal(to_signed(int(ch["hysteresis"]))) / 10, 2)
                )
...
def _interlock_channel_info_from_plc(
    ...
    main_module = modules_config.modules[0]
    ...
    if len(get_conf_output) > 3:
        ...
        if main_module == "750-8001":
            info["hysteresis"] = to_unsigned(get_conf_output[6], bits=16)
            if info["hysteresis"] == 0.0:
                info["hysteresis"] = None

    if len(get_conf_output) > 7:
        info["type"]["type"] = word_to_2ch(get_conf_output[4])
        info["dac"] = get_conf_output[7]
        info["dac_scale"] = two_short_to_float(get_conf_output[8], get_conf_output[9])
        info["dac_offset"] = two_short_to_float(
            get_conf_output[10], get_conf_output[11]
        )
...
def _interlock_channel_info_from_parsed(num, parsed, modules_config: ModulesConfig):
    ...
    if info["type"]["type"] in ("TC", "IV", "OV"):
        ...
        if parsed.hysteresis is not None:
            info["hysteresis"] = to_unsigned(int(parsed.hysteresis * scale))
    elif info["type"]["type"] in ("OW", "IW"):
        if module_type == "750-474" or module_type == "750-492":
            info["low_limit"] = round((parsed.low_limit - 4) * (2**15 - 1) / 16)
            info["high_limit"] = round((parsed.high_limit - 4) * (2**15 - 1) / 16)
            if parsed.hysteresis is not None:
                info["hysteresis"] = round(parsed.hysteresis * (2**15 - 1) / 16)
        else:
            ...
            if parsed.hysteresis is not None:
                info["hysteresis"] = to_unsigned(int(parsed.hysteresis))
    ...
    for int_1, int_2 in zip_longest(int_list_1, int_list_2):
        ...
        for ch1, ch2 in zip_longest(int_1["channels"], int_2["channels"]):
            ...
            if ch1 is None or ch2 is None:
                ...
                continue
            ch_keys = "num logical_device logical_device_key logical_device_channel type configuration low_limit high_limit hysteresis dac dac_scale dac_offset".split()
            for ck in ch_keys:
...
def interlock_upload(
    wago: Union[TangoWago, WagoController],
    interlock_list: list,
    ...
    for interlock in interlock_list:
        ...
        for channel in interlock["channels"]:
            ...
            if not channel["type"]["digital"]:
                ...
                if main_module == "750-8001" and channel["hysteresis"] is not None:
                    params += (to_unsigned(channel["hysteresis"]),)
...
def interlock_show(wc_name: str, interlock_info: list, modules_config: ModulesConfig):
    ...
    for info in sorted(interlock_info, key=lambda i: i["num"]):
        ...
        for ch in channels:
            ...
            channel_module_type = modules_config.read_table[logical_device][0][
                "module_reference"
            ]
            type_ = ch["type"]["type"]
            scale = ch["type"]["scale"]
            if type_ in ("IW", "OW"):
                if channel_module_type == "750-474" or channel_module_type == "750-492":
                    ch_value = (
                        round(ch["value"] / (2**15 - 1) * 16 + 4, 1)
                        if ch["value"]
                        else None
                    )
            else:
            ...
            if ch["type"]["digital"]:
                ...
                if ch["hysteresis"] is not None:
                    hysteresis = to_signed(ch["hysteresis"]) / scale

                if type_ in ("IW", "OW"):
                    if (
                        channel_module_type == "750-474"
                        or channel_module_type == "750-492"
                    ):
                        low_limit = low_limit / (2**15 - 1) * 16 + 4
                        high_limit = high_limit / (2**15 - 1) * 16 + 4
                        if ch["hysteresis"] is not None:
                            hysteresis = hysteresis / (2**15 - 1) * 16
                    if ch["hysteresis"] is not None:
                        chline += f"Low:{low_limit:.2f} High:{high_limit:.2f} Hyst:{hysteresis:.2f}"
                    else:
                        chline += f"Low:{low_limit:.2f} High:{high_limit:.2f}"
                elif ch["hysteresis"] is not None:
                    chline += f"Low:{low_limit:.2f} High:{high_limit:.2f} Hyst:{hysteresis:.2f}"
                else:
                    chline += f"Low:{low_limit:.2f} High:{high_limit:.2f}"
            ...
            if isinstance(ch_value, float):
                out += chline.ljust(79) + f"[{ch_value:.2f}]\n"
            else:
