...
def remove_comments(iterable):
    ...
    for line in iterable:
        if "#" in line:
            ...
            if line.strip():  # if is not only whitespace
                yield line
        else:
            line = line.rstrip()
            if line:
...
def float_to_2ushort(float_value):
    shortTuple = struct.unpack("HH", struct.pack("f", float_value))
    return shortTuple


def two_ushort_to_float(short0, short1):
    tuple = struct.unpack("f", struct.pack("HH", short0, short1))
    return float("{:.3f}".format(tuple[0]))


###                 unused from now               ############
def float_to_2short(float_value):
    shortTuple = struct.unpack("hh", struct.pack("f", float_value))
    return shortTuple


def two_short_to_float(short0, short1):
    tuple = struct.unpack("f", struct.pack("hh", short0, short1))
    return float("{:.3f}".format(tuple[0]))
