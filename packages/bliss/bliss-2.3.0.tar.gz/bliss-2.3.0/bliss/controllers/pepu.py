...
def idint_to_float(value, integer=40, decimal=8):
    ...
    if type(value) is numpy.ndarray:
        return value.astype(float) / float(1 << decimal)
    else:
...
class Stream(object):
    ...
    def read_state(self):
        return self.pepu.raw_write_read(self._cmd("STATUS", query=True))
...
class PEPU(CounterController):
    ...
    IN_CHANNELS = list(range(1, 9))  # 7 and 8 are development only
    ...
    def __init__(self, name, config, master_controller=None):
        ...
        cntnames = config.get("counter_names", None)
        if cntnames is None:
            ...
            for channel in itertools.chain(
                ...
                all_cnts.append(channel.name)

            cnts = [cnt for cnt in cntnames if cnt in all_cnts]
            if len(cnts) != len(cntnames):
                wrong_cnts = [cnt for cnt in cntnames if cnt not in all_cnts]
                raise ValueError(f"Invalid pepu counter names : {wrong_cnts}")
            for cnt in cnts:
                self.create_counter(PepuCounter, cnt)
        ...
        global_map.register(
            self, children_list=[self.comm], parents_list=["controllers"]
        )
    ...
    def raw_write(self, message, data=None):
        return _command(self.comm, message, data=data)

    def raw_write_read(self, message, data=None):
        return _ackcommand(self.comm, message, data=data)
...
class PepuCalcController(CalcCounterController):
    def __init__(self, name, config, register_counters=True):
        super().__init__(name, config, register_counters)

        for cnt_conf in config.get("outputs"):
            tag = cnt_conf.get("tags")
            if tag is None:
                cnt_name = cnt_conf["name"]
                raise ValueError(f"Missing tags in output counter {cnt_name}")

            cnt = self.get_output_counter(cnt_conf["tags"])
            cnt.scaling = cnt_conf.get("scaling", 1.0)
            cnt.offset = cnt_conf.get("offset", 0.0)
            cnt.raw_offset = cnt_conf.get("raw_offset", 0.0)

    def __info__(self):
        txt = f"Output counters of {self.name}:\n"
        for cnt in self._output_counters:
            tag = self._tags[cnt.name]
            cin = self.get_input_counter(tag)
            txt += f"{cnt.name}:\n"
            txt += f"    input counter = {cin.fullname}\n"
            txt += f"    scaling = {cnt.scaling}   raw_offset = {cnt.raw_offset}   offset = {cnt.offset}\n"
        return txt

    def get_output_counter(self, tag):
        for cnt in self._output_counters:
            if self._tags[cnt.name] == tag:
                return cnt
        return None

    def get_input_counter(self, tag):
    ...
    def calc_function(self, input_dict):
        output_dict = dict()
        for tag, data in input_dict.items():
            cnt = self.get_output_counter(tag)
            if cnt is not None:
                output_dict[tag] = (data + cnt.raw_offset) / cnt.scaling + cnt.offset
        return output_dict
