...
class Multiplexer:
    ...
    def _get_capability(self, capability: type[Capability]) -> Capability:
        if capability == MenuCapability:
            from .multiplexer_menu import MultiplexerMenuCapability

            return MultiplexerMenuCapability()
        return None
    ...
    def configure(self):
        """Display a shell dialog to configure this multiplexer."""
...
class Switch(BaseSwitch):
    ...
    def _set(self, state):
        self._mux_ctrl.switch(self._mux_name, state, synchronous=True)
