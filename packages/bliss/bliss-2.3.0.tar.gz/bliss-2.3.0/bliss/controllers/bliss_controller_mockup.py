...
from bliss.controllers.counter import IntegratingCounterController  # noqa: F401
...
class HardwareController:
    ...
    def send_cmd(self, cmd, *values):
        now = perf_counter()
        log_debug(self, f"@{now:.3f} send_cmd {cmd} {values}")
    ...
    def _init_com(self):
        log_debug(self, f"_init_com {self.config}")
...
class BCMockup(BlissController):
    ...
    def _load_config(self):
        ...
        self._counter_controllers["scc"] = BCSCC(self.name, self)
        self._counter_controllers["icc"] = BCICC(self.name, self)
...
# ==== BlissController with sampling counters ==========
...
class SamplingBlissController(BlissController):
