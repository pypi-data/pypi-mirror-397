...
class Communication:
    def __init__(self, gcs_version=2):
        self.sock = None
        self.gcs_version = gcs_version
    ...
    def com_initialize(self):
        ...
        NB:
          ...
          - get_pi_comm(,TCP) return a bliss.comm.tcp.Tcp -> Socket or Command
        """
    ...
    def get_error(self):
        ...
        try:
            ...
            raise RuntimeError(
                "PI controller timeout while querying error. Restart the controller?"
            )
    ...
    def command(self, cmd: str, nb_line: int = 1):
        ...
        Note:
            ...
            self.sock.lock is a gevent.lock.RLock
        ...
        cmd = cmd.encode()
        if need_reply:
            # Write and read command and answer.
            with self.sock.lock:
                if nb_line > 1:
            ...
            if nb_line > 1:
                ...
                if len(commands) == nb_line:
                    ...
                    for command, rep in zip(commands, reply):
                        ...
                        if space_pos > -1:
                            ...
                            parsed_reply.append(self._parse_reply(rep, args, command))
                        else:
                ...
                reply = parsed_reply
                # print("   parsed_reply=", reply)
            else:
                # Single line answer (cmd = "VEL? 1")
                space_pos = cmd.find(b" ")
            ...
            # no reply expected -> just check error.
            with self.sock.lock:
                self.sock.write(cmd + b"\n")
                if self.gcs_version == 1:
                    gevent.sleep(0.01)
        ...
        * SPA? 1 0X07000000   ->  1 0x7000000=-3.00000000e+1  # NB: PI 727 replies with:
        ...
        if self.gcs_version == 2:
            ...
            if u_reply[:args_pos] != u_args:  # weird
                ...
                print(f"@ command={cmd}")
                print(
                    f"@   reply={reply} args={args} reply[:args_pos]={reply[:args_pos]}"
                )
                print("@ ---------------------------------------------------------")
