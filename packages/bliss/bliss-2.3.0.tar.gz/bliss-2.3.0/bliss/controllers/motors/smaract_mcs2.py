...
7. power_mode: initialization mode of the positioner power supply (optional)
...
For open-loop usage (ex: SR-1908 on id16b), velocity and acceleration are not
used but mandatory. User has to tune the movement via open_loop_step_amplitude
and open_loop_step_frequency properties.

ex:
 DEMO [11]: minirot.positioner.open_loop_step_amplitude=60000
 DEMO [12]: minirot.positioner.open_loop_step_frequency=4000
...
class PositionerType(enum.IntEnum):
    ...
    SL_S1LC1 = 348  # Linear stage SLC1720lc-17 (previous lc-16)
...
class Positioner:
    ...
    # Removed: dangerous in case of error of positioner type.
    # To be discussed if it has to be re-introduced.
    # @positioner_type.setter
    # def positioner_type(self, ptype):
    #    """Accept PositionerType, int or string"""
    ...
    def closed_loop_speed(self):
        """Return closed loop speed in nano-degree/s for rotary sensors or
    ...
    def closed_loop_acceleration(self):
        """Return closed loop acceleration in nano-degree/s/s for rotary
    ...
    def open_loop_step_frequency(self, value):
        ...
        if value < 1 or value > 20000:
            raise RuntimeError("Step Frequency must be between 1 and 20 000 Hz")

        self.set_property(":STEP:FREQ", int(value))
    ...
    def open_loop_step_amplitude(self, value):
        ...
        self.set_property(":STEP:AMPL", int(value))
    ...
    def initialize_hardware_axis(self, axis):
        ...
        if "positioner_type" in axis.config.config_dict:
            ...
            if new_positioner_type != curr_positioner_type:
                _msg = (
                    f"Positioner type in config ({new_positioner_type}) does not match "
                    f"the type defined in hardware controller ({curr_positioner_type})"
                )
