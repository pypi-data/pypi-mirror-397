...
class Aerotech(Controller):
    ...
    def read_status(self, axis):
        with self._lock:
            try:
                ...
                log_info(self, "Retry on read_status SocketTimeout on %s", axis.name)
                self._comm.flush()
                return self.__try_read_status(axis)
    ...
    def get_axis_info(self, axis):
        ...
        """
        info_str = f"Aerotech parameters for {axis.name} Axis:\n"

        idstr = self.get_id(axis)
        (fault, status) = self.read_status(axis)
        info_str += (
            f"     {idstr}\n     Status : {status.get():08x}\n     {str(status)}\n\n"
            f"Axis Fault : 0x{fault.get():08x}\n     {str(fault)}\n"
        )
    ...
    def read_position(self, axis):
        with self._lock:
            ...
            try:
                ...
                log_info(self, "Retry on read_position SocketTimeout on %s", axis.name)
                self._comm.flush()
                reply = self.raw_write_read(cmd)
    ...
    def read_encoder(self, encoder):
        ...
        try:
            ...
            log_info(self, "Retry on read_encoder SocketTimeout on %s", encoder.name)
            self._comm.flush()
            reply = self.raw_write_read(cmd)
        return float(reply)
