...
class Elmo(Controller):
    ...
    def initialize_hardware(self):
        ...
        tout = 1.0 if is_serial else 200e-3
        try:
    ...
    def _query(self, msg, in_error_code=False, **keys):
        ...
        while retry < self.max_retries:
            ...
            except (SerialTimeout, UnicodeDecodeError, RuntimeError) as e:
                retry = retry + 1
                # print exception and number of tries done
                log_warning(self, "Error during query: %s: %s", msg, sys.exc_info())
    ...
    def start_one(self, motion):
        ...
        max_steps = 2**31
        if abs(motion.target_pos) > max_steps:
            raise RuntimeError("Cannot move to a position out of 31 bits steps")
        if abs(motion.delta) > max_steps:
            raise RuntimeError("Requested move exceeds 31 bits steps")
