...
class Musst(BlissController):
    class Channel(object):
        ...
        def __init__(
            ...
            self.encoder_conversion_param = encoder_conversion_param
        ...
        def mode(self):
            if self._mode_number is None:
                self._mode_number = self.chan_config
        ...
        def _cnt_cmd(self, cmd):
            if self.mode == self.COUNTER or self.mode == self.ENCODER:
                musst = self._musst()
        ...
        def _convert(self, string_value):
            ...
            mode = self.mode
            if mode == self.SWITCH:
                mode = self.chan_config
            if mode == self.COUNTER:
                return int(string_value)
            elif mode == self.ADC10:
        ...
        for channel_config in channels_list:
            ...
            if channel_number is None:
                ...
                if channel_type in ("cnt", "encoder", "ssi", "adc5", "adc10"):
                    ...
                    encoder_conversion_param = None
                    if channel_type in ("encoder", "ssi"):
                        for param in ("steps_per_unit", "offset", "sign", "unit"):
                            value = channel_config.get(param, None)
                            if value is not None:
                                if encoder_conversion_param is None:
                                    encoder_conversion_param = {param: value}
                                else:
                                    encoder_conversion_param[param] = value
                    ...
                    channel = self.get_channel(
                        channel_number,
                        type=channel_type,
                        encoder_conversion_param=encoder_conversion_param,
                    )
            ...
            if cnt_name is not None:
                if channel is None:  # Must be TIMER
                    ...
                    if channel_type == "switch":
                        ...
                        cnt_dtype = type(convert("1"))

                    else:
                        convert = channel._convert

                        # see casting in 'Channel._convert'
                        if channel_type in ("adc5", "adc10"):
                            cnt_dtype = float
                        else:
                            cnt_dtype = int
        ...
        with self._cnx._lock:
            ...
            if msg.startswith("?") or ack:
                ...
                if answer == b"$":
                    ...
                    if answer != b"OK":
                        raise RuntimeError(
                            "%s: invalid answer: %r" % (self.name, answer)
                        )
                    return True
                else:
