...
class MySensorBindToAxis(ExternalInput):
    ...
    def __init__(self, name, config):
        ...
        self.device.get_heating_rate = lambda: numpy.exp(self.axis.position)
...
class RegulMockup(Controller):
    ...
    def _pid_task(self, tloop):
        ...
        while not self._stop_pid_events[tloop.name].is_set():
            ...
            pid_output = self.pids[tloop.name](input_value)
            output_value = tloop.output._scale_pid_output(tloop.pid_range, pid_output)
