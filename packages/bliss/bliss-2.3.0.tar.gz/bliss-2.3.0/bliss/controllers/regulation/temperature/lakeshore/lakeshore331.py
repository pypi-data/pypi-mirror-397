...
"""
LakeShore331 is the base class used by other BLISS Lakeshore controllers (340, 335, 332).

Lakeshore 331 can communicate only via Serial line (RS232), other models have at
least a GPIB interface.
...
class LakeShore331(Controller):
    ...
    def __init__(self, config):
        ...
        max_freq = self.config.get("comm_max_frequency", 20)
    ...
    def set_setpoint(self, tloop, sp, **kwargs):
        ...
        # ???
        input_status = self.read_status(tloop.input)
        if input_status[0] == 0 or input_status[0] == 2:
    ...
    def start_ramp(self, tloop, sp, **kwargs):
        ...
        if rate == 0.0:
            self.send_cmd("RAMP", 0, 0, channel=tloop.channel)
        else:
    ...
    def send_cmd(self, command, *args, channel=None):
        ...
        Returns:
            Answer from the controller if there is a '?' in the command.
        """
    ...
    def eol(self):
        """
        Return eol as a str (not bytes).
        """
        if isinstance(self._comm._eol, bytes):
            return self._comm._eol.decode()
    ...
    def show(self, name=None):
        ...
        if sensor is not None:
            ...
            input_status = self.read_status(input)
            if input_status[0] != 0 and input_status[0] != 2:
                repr_list.append(input_status[1])
    ...
    def read_status(self, tinput):
        """Shows input reading status.

        Args:
            tinput:  Input class type object
        Returns:
            integer: represents the sum of the bit weightting of the input status flag bits.
        """
        log_debug(self, "reading_status")
        asw = int(self.send_cmd("RDGST?", channel=tinput.channel))

        if asw == 0:
            message = f"New reading on input {tinput.channel}"
        if asw & 1:
            log_warning(self, "Invalid reading on input %s" % tinput.channel)
            message = "fInvalid reading on input {tinput.channel}"
        if asw & 2:
            log_warning(self, "Old value read on input %s" % tinput.channel)
            message = f"Old value read on input {tinput.channel}"
        if asw & 16:
            log_warning(self, "Temperature UnderRange on input %s" % tinput.channel)
            message = f"Temperature value on input {tinput.channel} is invalid"
        if asw & 32:
            log_warning(self, "Temperature OverRange on input %s" % tinput.channel)
            message = f"Temperature value on input {tinput.channel} is invalid"
        if asw & 64:
            log_warning(
                self, "Temperature in Sensor_unit = 0 on input %s" % tinput.channel
            )
            message = f"Temperature in Sensor_unit = 0 on input {tinput.channel}"
        if asw & 128:
            log_warning(
                self,
                "Temperature OverRange in Sensor_unit on input %s" % tinput.channel,
            )
            message = f"Temperature OverRange in Sensor_unit on input {tinput.channel}"
    ...
    def get_heater_range(self, toutput):
        ...
        if toutput.channel == 2:
            ana = self.send_cmd("ANALOG?")
            if ana.startswith("0,3"):
                r = 1
            else:
                r = 0
        else:
            r = int(self.send_cmd("RANGE?", channel=toutput.channel))
    ...
    def set_heater_range(self, toutput, value):
        ...
        # Lakeshore331/2 loop2 is drived via the analog output
        if toutput.channel == 2:
            if value == 0:
                self.send_cmd("ANALOG 0,0")
            else:
                self.send_cmd("ANALOG 0,3")
        else:
    ...
    def read_curve(self, crvn, crvfile):
        ...
        if os.path.isfile(crvfile):
            raise ValueError("A file %s exits, should not overwrite" % crvfile)

        print(f"Readings from actual curve {crvn} in LakeShore {self.model} :")
        ...
        print("Downloading now the curve points, be patient ....")
        curvepts = {}
        for index in range(1, 201):
            pair = self.send_cmd(f"CRVPT? {crvn} {index}").split(",")
            if float(pair[0]) == 0.0:
                curve_breakpoints = index - 1
                break
            curvepts[index] = (float(pair[0]), float(pair[1]))

        with open(crvfile, "a") as f:
            f.write(f"Sensor Model: {curve_name}\n")
            f.write(f"Serial Number: {curve_sn}\n")
            f.write(f"Data Format: {header[2]}    ({curve_format})\n")
            f.write(f"SetPoint Limit: {curve_temp_limit}    (Kelvin)\n")
            f.write(f"Temperature coefficient: {header[3]}    ({curve_temp_coeff})\n")
            f.write(f"Number of Breakpoints: {curve_breakpoints}\n\n")
            f.write("No.      Units           Temperature (K)\n\n")
            for index in range(1, curve_breakpoints + 1):
                f.write(
                    f"{index:<8d} {curvepts[index][0]:<15f} {curvepts[index][1]:>f}\n"
                )

        print(f"\t{curve_breakpoints} points downloaded successfully !")
    ...
    def write_curve(self, crvn, crvfile):
        ...
        if breakpts == int(curve_nb_breakpts):
            print(
                f"\nCurve {crvn} has been written into the LakeShore model {self.model} temperature controller."
            )
