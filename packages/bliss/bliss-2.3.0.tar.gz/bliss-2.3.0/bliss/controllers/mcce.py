...
class Mcce(BeaconObject, HasMetadataForScan):
    """Commmands"""

    def __init__(self, name, config):
        # share_hardware default value is True, but force it there.
        super().__init__(config, share_hardware=True)
    ...
    def scan_metadata(self):
        ...
        if self.mcce_type not in [1, 2, 3, 6]:
            metadata["gain"] = self.gain
        metadata["range"] = self.range
        if self.mcce_type not in [4, 5]:
            metadata["frequency"] = self.frequency
        metadata["polarity"] = self.polarity
        return metadata
    ...
    @property
    def range_str(self):
        return self.mcce_range_str[self.range]
    ...
    def frequency(self):
        ...
        """
        if self.mcce_type in (4, 5):
            log_warning(self, "No frequency for photocondictive electrometer")
    ...
    def frequency(self, value):
        ...
        """
        if self.mcce_type in (4, 5):
            log_error(self, "No frequency for photocondictive electrometer")
            return
    ...
    def gain(self):
        ...
        if self.mcce_type in (1, 2, 3, 6):
            log_warning(self, "No gain for photovoltaic electrometer")
            return None
    ...
    def gain(self, value):
        ...
        """
        if self.mcce_type in (1, 2, 3, 6):
            log_error(self, "No gain for photovoltaic electrometer")
            return
        _gain = log10(value)
        self._set_on(False)
