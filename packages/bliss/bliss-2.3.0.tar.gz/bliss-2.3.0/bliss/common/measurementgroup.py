...
from bliss import current_session, _get_current_session
...
def _mg_map_parent_name():
    try:
        session_name = current_session.name
    except AttributeError:
        # no current session, the proxy evaluates to None
        # so current_session.name => None.name => AttributeError("name")
        session_name = "None"
    return f"{session_name}:measurement groups"
...
def get_all():
    ...
    """
    try:
        return list(global_map.instance_iter(_mg_map_parent_name()))
    except KeyError:
...
def get_active_name() -> str:
    ...
    * !! this is only the name, the MG object may not exist.
    ...
    active_mg_name = settings.SimpleSetting(
        f"{session.name}:active_measurementgroup",
...
def set_active_name(name: str):
    ...
    event.send(session, "active_mg", name)
...
class MeasurementGroup:
    def __init__(self, name: str, config_tree):
        ...
        global_map.register(self, parents_list=[_mg_map_parent_name()])
    ...
    def _get_capability(self, capability: type[Capability]) -> Capability:
        if capability == MenuCapability:
            from .measurementgroup_menu import MeasurementGroupMenuCapability

            return MeasurementGroupMenuCapability()
        return None
    ...
    def _get_counters_names(self, counters: Iterable[Counter]) -> list[str]:
    ...
    @property
    def _available_counters(self) -> list[Counter]:
        counters, _ = _get_counters_from_names(
            itertools.chain(self._config_counters, self._extra_counters)
        )
        return counters

    def _get_disabled_counters_names(
        self, available_counters: list[Counter]
    ) -> list[str]:
        """Return list of disabled counters

        Remove counters from redis that are not in config, if any
        """
        available_counter_names = self._get_counters_names(available_counters)

        s = set(self._disabled_setting.get())
        disabled_counters = [name for name in available_counter_names if name in s]

        not_present_counters = []
        for cnt_fullname in disabled_counters:
            if (
                cnt_fullname not in available_counter_names
    ...
    @property
    def _disabled_setting(self) -> settings.QueueSetting:
    ...
    @_check_counter_name
    def disable(self, *counter_patterns: str):
        if len(counter_patterns) == 0:
            return
        ...
        event.send(self, "disabled", disabled)

    def _get_enabled_counters_names(
        self, available_counters: list[Counter]
    ) -> list[str]:
        return _list_diff(
    ...
    @_check_counter_name
    def enable(self, *counter_patterns: str):
        if len(counter_patterns) == 0:
            return
        available_counters = self._available_counters
        counter_names = self._find_counter_names(counter_patterns, available_counters)
        ...
        event.send(self, "disabled", new_disabled)
    ...
    def switch_state(self, state_name: str):
    ...
    def remove_states(self, *state_names: str):
        """
    ...
    def add(self, *counters: Counter):
        ...
        available_names = self._get_counters_names(self._available_counters)
        event.send(self, "available", available_names)
...
class ActiveMeasurementGroupProxy(ProxyWithoutCall):
    def __init__(self):
