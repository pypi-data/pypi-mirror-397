...
def anmesh(
    ...
    *counter_args: _countables,
    ...
    scan_info.add_scatter_plot(x=motor_list[0].fullname, y=motor_list[1].fullname)
    ...
    for _top_master_name, chain_description in scan_info["acquisition_chain"].items():
        for device_key in chain_description["devices"]:
            device_info = scan_info["devices"][device_key]
