...
class Encoder(SamplingCounter):
    ...
    def __info__(self):
        info_str = "ENCODER:\n"
        info_str += (
            f"          steps per unit: {self.steps_per_unit}      unit: {self.unit}\n"
        )
        info_str += f"      axis read_position: {self.axis._read_position_mode}\n"
        info_str += f"      axis check_encoder: {self.axis._check_encoder}\n"
        info_str += f"  dial_measured_position: {self.raw_read:10.5f}\n"
        info_str += f"       encoder tolerance: {self.tolerance}   (position check at end of a move)\n"
        return info_str
