...
# Patch main greenlet with ._children attribute to keep track of
# the spawning tree, even if its not a BlissGreenlet.
_main_greenlet = gevent.getcurrent()
_main_greenlet._children = set()
_main_greenlet.name = "Main greenlet"
...
class BlissGreenlet(_GeventGreenlet):
    ...
    def _flush_spawn_tree(self, greenlet=None):
        if self or self._children:
            return
        self._parent._children.discard(self)
        try:
            self._parent._flush_spawn_tree()
        except AttributeError:
            pass

    def start(self):
        self._parent = gevent.getcurrent()
        try:
            self._parent._children.add(self)
            self.rawlink(self._flush_spawn_tree)
        except AttributeError:
            pass  # parent is not a BlissGreenlet, it doesn't implement parentality
        return super().start()
    ...
    def child_dict(self) -> dict:
        return {
            "greenlet": self,
            "dead": self.dead,
            "children": [child.child_dict() for child in self._children],
        }
