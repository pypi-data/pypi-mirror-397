...
class ProxyWithoutCall(with_metaclass(_ProxyMetaType)):
    """
...
def is_proxy(obj: Any) -> bool:
    """
    Return True if <obj> is a Proxy.
    """
    return issubclass(type(obj), ProxyWithoutCall)


def deref_proxy(obj: Any) -> Any:
    """
    Return the wrapped object from this `obj` proxy, else return this `obj`.

    - If the proxy is an object, the wrapped object is returned
        - If the wrapped object is `None`, `None` is returned
    - If the object is not a proxy, the input `obj` is returned
    """
    if not is_proxy(obj):
        return obj
