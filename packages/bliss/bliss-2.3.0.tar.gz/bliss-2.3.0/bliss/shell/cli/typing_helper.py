...
from bliss.common.proxy import is_proxy
...
def get_obj_from_text(text, namespace):
    """
    Return object from <namespace> corresponding to <text>.
    text should not contain ( ) ...
    """
    text_split = text.split(".")
    if text_split[0] in namespace:
        obj = namespace[text_split[0]]
        for subtext in text_split[1:]:

            if is_proxy(obj):
                # safe_getattr() does not work with a proxy. Better dereference it before.
                obj = obj.__wrapped__
...
class TypingHelper(object):
    ...
    def add_helper_key_binding(self, repl):
        ...
        def _(event):
            text = repl.default_buffer.text
            if self.is_active and not is_multiline(text):
                if self._check_callable(repl, event):
                    self._insert_parenthesis_if_noargs(repl, event)
                elif not is_property(text, repl):
                    self._check_terminating_bracket(repl, event)
