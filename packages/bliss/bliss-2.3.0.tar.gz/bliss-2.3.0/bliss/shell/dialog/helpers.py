...
from bliss.shell.cli.user_dialog import Widget, Container, UserSelect
from .core import show_dialog
...
def normalize_dialog(user_dialog: list[list[Widget]] | list[Widget] | Widget) -> Widget:
    """Normalize list syntax into a typed structure"""

    if isinstance(user_dialog, list):
        dlgs = []
        for sub_user_dialog in user_dialog:
            if isinstance(sub_user_dialog, list):
                sub_user_dialog = Container(sub_user_dialog, splitting="v")
            dlgs.append(sub_user_dialog)
        return Container(dlgs, splitting="h")
    return user_dialog
    ...
    def display_dialog(dialog_type, *args, **kwargs):
        if dialog_type in dialog_classes:
            ...
            if len(dialog_classes) == 1:
                ...
                desc = [(v, str(v).capitalize()) for v in dialog_classes]
                submenu_class = show_dialog(
