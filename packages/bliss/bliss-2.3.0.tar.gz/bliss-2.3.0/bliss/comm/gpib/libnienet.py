...
class EnetSocket:
    ...
    if "dummy_io" in debug:
        ...
        def _open(self):  # noqa: F811
            pass

        def _send(self, s):  # noqa: F811
            return sys.stderr.write("DBG: > %s" % (repr(s)))

        def _recv(self, len):  # noqa: F811
            return input("DBG: < #%s:" % (len))[:len]

        def _close(self):  # noqa: F811
            pass
...
if __name__ == "__main__":
    ...
    obj = EnetLib(nienet_host)
    ud = obj.ibdev(pad=13)
    print("ibrsp", obj.ibrsp(ud))
    print("iblines", obj.iblines(ud))
    print("ibtrg", obj.ibtrg(ud))
    print("ibask", obj.ibask(ud, 1))
    print("ibln 13", obj.ibln(ud, 13))
    print("ibln 11", obj.ibln(ud, 11))
    print("ibwrt", obj.ibwrt(ud, "ID?;"))
    print("ibrd", obj.ibrd(ud, 10))
    print("ibwrt", obj.ibwrt(ud, "SET?;"))
    print("ibrd", repr(obj.ibrd(ud, 640)))
    print("ibwrt", obj.ibwrt(ud, "DSTB;"))
    print("ibrd", repr(obj.ibrd(ud, 4096)))
    print("ibbna", obj.ibbna(ud))
