...
class ControlMatcher:
    ...
    def _get_control_id_variations(self, control_id: str) -> set:
        ...
        if framework == "NIST":
            ...
            # SOC2 variations: just case variations
            variations = {parsed, parsed.upper(), parsed.lower()}
            # Also try without dots: CC1.1 -> CC11
            nodot_version = parsed.replace(".", "")
            variations.update({nodot_version, nodot_version.upper(), nodot_version.lower()})
            return variations

        elif framework == "CIS":
            # CIS controls are numeric - limited variations
            return {parsed}

        elif framework == "ISO":
            # ISO variations: case variations
            variations = {parsed, parsed.upper(), parsed.lower()}
            return variations

        else:
            # Generic/unknown framework: return as-is with case variations
    ...
    def _detect_framework(self, control_id: str) -> str:
        ...
        :return: Framework type (NIST, SOC2, CIS, ISO, or GENERIC)
        :rtype: str
        """
