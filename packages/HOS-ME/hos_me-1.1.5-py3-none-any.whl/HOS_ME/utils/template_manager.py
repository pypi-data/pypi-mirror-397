...
*本周报遵循结构化模板生成，由HOS办公平台提供技术支持*"""
    ...
    def _save_template(self, template_id, template_name, content, structure=None, format="xml"):
        """保存模板文件，支持XML格式，默认格式为XML"""
        if format == "xml":
            # 保存为XML格式
            template_path = os.path.join(self.templates_dir, f"{template_id}.xml")
            self._save_template_as_xml(template_path, template_id, template_name, content, structure)
        else:
            # 保存为旧版TXT格式
            template_path = os.path.join(self.templates_dir, f"{template_id}.txt")
            with open(template_path, "w", encoding="utf-8") as f:
                f.write(content)
            # 同时保存XML格式，确保所有模板都有XML版本
            xml_template_path = os.path.join(self.templates_dir, f"{template_id}.xml")
            self._save_template_as_xml(xml_template_path, template_id, template_name, content, structure)
    
    def _save_template_as_xml(self, template_path, template_id, template_name, content, structure=None):
        """将模板内容保存为XML格式"""
        # 创建根元素
        template = ET.Element("template", version="1.0", id=template_id)
        
        # 创建元数据
        metadata = ET.SubElement(template, "metadata")
        ET.SubElement(metadata, "name").text = template_name
        ET.SubElement(metadata, "description").text = ""
        ET.SubElement(metadata, "type").text = "custom"
        ET.SubElement(metadata, "created_at").text = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
        ET.SubElement(metadata, "updated_at").text = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
        ET.SubElement(metadata, "is_default").text = "false"
        
        # 创建内容元素
        content_elem = ET.SubElement(template, "content")
        
        # 将内容转换为XML结构
        # 直接保存内容文本
        content_p = ET.SubElement(content_elem, "paragraph")
        content_p.text = content
        
        # 创建配置元素
        config = ET.SubElement(template, "config")
        
        # 创建输出格式配置
        output_format = ET.SubElement(config, "output_format")
        output_format.text = "txt"
        output_format.set("default", "txt")
        
        # 创建占位符元素（如果有）
        if structure and "placeholders" in structure:
            placeholders = ET.SubElement(template, "placeholders")
            for placeholder in structure["placeholders"]:
                ph = ET.SubElement(placeholders, "placeholder")
                ET.SubElement(ph, "name").text = placeholder
                ET.SubElement(ph, "type").text = "text"
                ET.SubElement(ph, "required").text = "true"
        
        # 创建ElementTree并写入文件
        tree = ET.ElementTree(template)
        
        # 创建声明
        declaration = ET.ProcessingInstruction("xml", "version='1.0' encoding='UTF-8'")
    ...
    def _read_template(self, template_id):
        ...
        if os.path.exists(template_path):
            return self._read_xml_template(template_path)
    ...
    def _read_xml_template(self, template_path):
        """读取XML格式模板"""
        try:
            # 解析XML文件
            ...
            # 获取结构信息
            structure = {}
            placeholders = root.find("placeholders")
            if placeholders is not None:
                structure["placeholders"] = []
                for placeholder in placeholders.findall("placeholder"):
                    name = placeholder.find("name")
                    if name is not None and name.text:
                        structure["placeholders"].append(name.text)
            
            return content, structure
        except Exception as e:
            print(f"读取XML模板失败: {str(e)}")
            return None, {}
    ...
    def create_template(self, name, content, description="", template_type="weekly_report", output_format="txt", prompt="", structure=None, format="xml"):
        """创建新模板，默认使用XML格式，支持多种输出格式"""
        template_id = f"template_{int(time.time())}"
        config = self._load_config()
        
        # 保存模板文件 - 始终使用XML格式
        self.save_template(template_id, name, content, structure, format="xml")
    ...
    def update_template(self, template_id, name=None, content=None, description=None, template_type=None, output_format=None, prompt=None, format_settings=None, batch_settings=None, structure=None, format=None):
        """更新模板，强制使用XML格式"""
        config = self._load_config()
        template = next((t for t in config["templates"] if t["id"] == template_id), None)
        ...
        if content is not None:
            self.save_template(template_id, name or template["name"], content, structure, format=template_format)
    ...
    def import_template(self, name, content, description="", template_type="weekly_report", output_format="txt", prompt=""):
        """导入模板，自动检测格式并转换为XML格式"""
        # 自动识别原始格式
        original_format = self._detect_template_format(content)
        
        # 调用create_template，它会自动转换为XML格式
    ...
    def _detect_template_format(self, content):
        """自动检测模板格式"""
        # 检查是否为XML格式
        if content.strip().startswith('<?xml') or content.strip().startswith('<template'):
            return 'xml'
        # 检查是否为JSON格式
    ...
    def export_template(self, template_id, format="json"):
        """导出模板，支持多种格式"""
        ...
        if format == "json":
            ...
            if os.path.exists(template_path):
                ...
                # 如果不存在，将当前模板转换为XML格式导出
                content = template["content"]
                structure = template.get("structure", {})
                # 创建临时文件路径
                temp_path = os.path.join(tempfile.gettempdir(), f"{template_id}_export.xml")
                # 保存为XML格式
                self._save_template_as_xml(temp_path, template_id, template["name"], content, structure)
                # 读取并返回内容
    ...
    def save_template(self, template_id, template_name, content, structure=None, format="xml"):
        """
        保存模板文件，支持保存模板结构和XML格式
        ...
        Args:
            ...
            content: 模板内容
            structure: 模板结构信息
            format: 模板保存格式，支持"xml"和"txt"
        """
        # 保存模板内容
        self._save_template(template_id, template_name, content, structure, format)
        
        # 保存结构信息（如果提供且使用txt格式）
        if structure and format == "txt":
