...
class KnowledgeBase:
    ...
    def add_document_to_rag_library(self, library_id, document_id):
        ...
        if library and document_id not in library["documents"]:
            # 确保文档有总结
            document = self.get_document(document_id)
            if document and not document.get("summary"):
                # 生成文件总结
                summary = self.generate_file_summary(document.get("file_name", "无文件名"), document.get("content", ""))
                # 更新文档
                self.update_document(document_id, {"summary": summary})
    ...
    def update_all_document_summaries(self):
        """更新所有文档的总结"""
        """
        更新所有文档的总结，确保每个文档都有基于文件名和内容的总结
        ...
        for document in self.documents:
            if not document.get("summary"):
                ...
                summary = self.generate_file_summary(document.get("file_name", "无文件名"), document.get("content", ""))
                # 更新文档
                self.update_document(document["id"], {"summary": summary})
                updated_count += 1
        
        return updated_count
    
    def update_rag_library_summaries(self, library_id):
        """更新RAG库中所有文档的总结"""
        """
        更新RAG库中所有文档的总结，确保每个文档都有基于文件名和内容的总结
        
        Args:
            library_id (str): RAG库ID
