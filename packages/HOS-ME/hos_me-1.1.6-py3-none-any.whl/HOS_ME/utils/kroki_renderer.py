    ...
    def detect_chart_blocks(self, text_content):
        ...
        try:
            ...
            for elem in root.iter():
                tag = elem.tag.lower()
                if tag in self.supported_diagrams or 'chart' in tag or 'diagram' in tag:
                    # 提取图表内容
                    chart_code = ET.tostring(elem, encoding='unicode', method='text').strip()
                    if chart_code:
                        # 确定图表类型
                        diagram_type = tag
                        if diagram_type not in self.supported_diagrams:
