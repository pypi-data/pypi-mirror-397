...
def setup_logging():
    """设置日志系统"""
    # 统一日志目录
    log_dir = os.path.join(os.path.expanduser('~'), '.hos-me', 'logs')
    ...
    text_handler = RotatingFileHandler(
        os.path.join(log_dir, 'hos-me.log'),
...
try:
    ...
    logger.warning("OCRProcessor模块导入失败，OCR功能将不可用")
    OCRProcessor = None
...
def upload_multiple_files():
    ...
    logger.info(f"多文件上传成功: {len(uploaded_files)} 个文件")
...
# 上传文件下载端点
@app.route('/api/download/upload/<filename>', methods=['GET'])
...
@click.group(cls=FlaskGroup, create_app=create_app, help='HOS ME 命令行工具')
...
def cli(config, debug, host, port):
    """HOS ME 命令行工具"""
    if config:
        # 加载配置文件
        try:
            ...
            click.echo(f"加载配置文件失败: {str(e)}", err=True)
    ...
    # 初始化命令注册表
    try:
        from HOS_ME.cli import command_registry
        # 自动发现命令
        command_registry.discover_commands()
        # 注册所有命令
        command_registry.register_all(cli)
        click.echo(f"已注册 {len(command_registry.get_commands())} 个命令")
    except ImportError as e:
        click.echo(f"命令模块导入失败: {str(e)}", err=True)
    except Exception as e:
        click.echo(f"命令注册失败: {str(e)}", err=True)

@cli.command('run')
def run_server():
    """运行HOS ME服务器"""
    logger.info(f"启动HOS ME服务器，监听 {app.config['HOST']}:{app.config['PORT']}")
    app.run(
...
def initialize_app():
    ...
    if not os.path.exists(config_dir):
        ...
        click.echo(f"创建配置目录: {config_dir}")
    ...
    if not os.path.exists(config_path):
        ...
        click.echo(f"创建配置文件: {config_path}")
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        # 默认运行服务器
        logger.info("启动HOS ME服务器，监听 0.0.0.0:5000")
...
# 新增文件管理器
from HOS_ME.utils.file_manager import FileManager
file_manager = FileManager(config)

# Ollama管理器
ollama_manager = OllamaManager(config.system_settings.get('install_settings', {}).get('ollama', {}))
...
# 获取文件信息
@app.route('/api/files/<file_key>', methods=['GET'])
def get_file_info(file_key):
    """获取文件信息"""
    try:
        file_info = file_manager.get_file_by_key(file_key)
        if file_info:
...
def upload_file_to_manager():
    ...
    try:
        ...
        # 获取文件类型
        file_type = request.form.get('type', 'uploads')
        
        # 保存文件
        file_content = file.read()
        file_key = file_manager.save_file(file_type, file.filename, file_content)
    ...
    try:
        ...
        if not file_info:
            ...
            if '/' in file_key:
                ...
                if file_manager.exists(file_type, filename):
...
def delete_file(file_key):
    ...
    try:
        if '/' in file_key:
            file_type, filename = file_key.split('/', 1)
            success = file_manager.delete_file(file_type, filename)
            if success:
                return jsonify({'success': True, 'message': '文件删除成功'})
            else:
                return jsonify({'success': False, 'message': '文件不存在'})
        return jsonify({'success': False, 'message': '无效的文件键'})
    except Exception as e:
...
def download_report(filename):
    try:
        ...
        if os.path.exists(filepath):
            logger.info(f"报告文件下载: {filename}")
            return send_file(filepath, as_attachment=True)
        else:
            return jsonify({'success': False, 'message': '文件不存在'})
    except Exception as e:
        logger.error(f"报告文件下载失败: {filename}, 错误: {str(e)}")
