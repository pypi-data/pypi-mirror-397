    ...
    def convert(self, input_content, from_format, to_format, options=None):
        ...
        Args:
            ...
            options: 额外的转换选项，支持以下特殊选项：
                ...
                - template_mode: 是否为模板模式（默认False，模板模式下会处理占位符）
        ...
        try:
                ...
                try:
                    ...
                    if content_elem is not None:
                        ...
                        for elem in content_elem.iter():
                            if elem.text:
                                template_content += elem.text
                            if elem.tail:
                                template_content += elem.tail
                        
                        # 提取图表元素
                        chart_elements = []
                        for elem in root.iter():
                            tag = elem.tag.lower()
                            if tag in ['chart', 'diagram', 'mermaid', 'plantuml', 'graphviz']:
                                chart_content = ET.tostring(elem, encoding='unicode', method='text').strip()
                                if chart_content:
                        ...
                        # 将XML模板的内容作为Markdown处理
                        processed_content = template_content
                        from_format = 'markdown'  # 转换为Markdown格式进行处理
                except Exception as e:
                    print(f"解析XML模板失败: {str(e)}")
                    # 如果解析失败，尝试直接转换
                    pass
            ...
            if to_format == 'docx':
                # 优化DOCX输出
                cmd.extend(['--reference-doc', 'default-reference.docx'])
            elif to_format == 'pdf':
                # 优化PDF输出
