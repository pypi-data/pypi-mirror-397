...
from intugle.adapters.types.duckdb.models import DuckdbConfig, DuckdbS3Config
...
class DuckdbAdapter(Adapter):
    ...
    def __init__(self):
        ...
        try:
            duckdb.load_extension('delta')
        except duckdb.Error:
            duckdb.install_extension('delta')
            duckdb.load_extension('delta')

        self._configure_s3()

    def _configure_s3(self):
        s3_settings = settings.PROFILES.get("duckdb", {}).get("s3")
        if s3_settings:
            config = DuckdbS3Config.model_validate(s3_settings)
            
            query_parts = ["TYPE S3"]
            if config.s3_access_key_id:
                query_parts.append(f"KEY_ID '{config.s3_access_key_id}'")
            if config.s3_secret_access_key:
                query_parts.append(f"SECRET '{config.s3_secret_access_key}'")
            if config.s3_region:
                query_parts.append(f"REGION '{config.s3_region}'")
            if config.s3_endpoint:
                query_parts.append(f"ENDPOINT '{config.s3_endpoint}'")
            if config.s3_session_token:
                query_parts.append(f"SESSION_TOKEN '{config.s3_session_token}'")
            if config.s3_url_style:
                query_parts.append(f"URL_STYLE '{config.s3_url_style}'")
            
            secret_options = ", ".join(query_parts)
            query = f"CREATE OR REPLACE SECRET s3_secret ({secret_options})"
    ...
    def intersect_count(self, table1: "DataSet", column1_name: str, table2: "DataSet", column2_name: str) -> int:
        ...
        query = f"""
            SELECT COUNT(DISTINCT L.{column1_safe}) AS intersect_count
            FROM {table1_name_safe}  AS L
            JOIN {table2_name_safe} AS R
            ON L.{column1_safe} = R.{column2_safe}
            WHERE L.{column1_safe} IS NOT NULL
            AND R.{column2_safe} IS NOT NULL
            """
