"""MCP server exposing Kytchen prep station tools.

Too many cooks in the kitchen? Let us handle the prep.

This server exposes a small set of tools:
- load_context: Load ingredients into the prep
- peek_context: View character/line ranges of the prep
- search_context: Regex search the prep
- exec_python: Execute code in the sandbox (prep work)
- sub_query: Ask sub-questions (sous chef)
...
class KytchenMCPServer:
    """MCP server wrapping Kytchen-style REPL sessions (prep station)."""
    ...
    async def run(self, transport: str = "stdio") -> None:
        if transport != "stdio":
            raise ValueError("Only stdio transport is supported in Kytchen v1")

        await self.server.run_stdio_async()


def main() -> None:
    """CLI entry point: `python -m kytchen.mcp.server`"""

    import argparse

    parser = argparse.ArgumentParser(description="Run Kytchen as an MCP server (stdio transport)")
    parser.add_argument("--provider", default="anthropic", choices=["anthropic", "openai"], help="LLM provider (the chef)")
    ...
    server = KytchenMCPServer(provider=args.provider, model=model)
