"""Sandboxed Python execution environment (Kytchen Prep Station).

Kytchen stores the full context (the "prep") in a REPL namespace (default variable: `ctx`).
The root LLM (the "chef") can write Python code to inspect and process the prep
...
Do not expose Kytchen code-execution mode to untrusted users without stronger
...
from ..types import ContextType, ExecutionResult, SubKytchenFn, SubQueryFn
...
def _compile_with_last_expr(source: str) -> tuple[CodeType, CodeType | None]:
    ...
    if tree.body:
        ...
        if isinstance(last_stmt, ast.Expr):
            ...
            exec_code = compile(tree, filename="<kytchen_prep>", mode="exec")
            eval_code = compile(expr, filename="<kytchen_prep_expr>", mode="eval")
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Attribute):
            ...
            if node.attr in ("format", "format_map"):
                raise SecurityError(
                    f"Use of '{node.attr}' method is not allowed. Use f-strings or %-formatting instead."
                )
...
class REPLEnvironment:
    ...
    def __init__(
        ...
        self._namespace.update(
            {
                ...
                "semantic_search": lambda query, chunk_chars=1000, overlap=100, max_results=10: _helpers.semantic_search(
                    ctx_getter(), query=query, chunk_chars=chunk_chars, overlap=overlap, max_results=max_results
                ),
                ...
                "convert_pdf": lambda pages=None: __import__("kytchen.converters.pdf", fromlist=["convert_pdf_bytes"]).convert_pdf_bytes(
                    ctx_getter() if isinstance(ctx_getter(), (bytes, bytearray)) else str(ctx_getter()).encode("utf-8", errors="replace"),
                    pages=pages,
                ),
                "convert_docx": lambda: __import__("kytchen.converters.docx", fromlist=["convert_docx_bytes"]).convert_docx_bytes(
                    ctx_getter() if isinstance(ctx_getter(), (bytes, bytearray)) else str(ctx_getter()).encode("utf-8", errors="replace"),
                ),
                "convert_xlsx": lambda formulas="evaluated": __import__("kytchen.converters.xlsx", fromlist=["convert_xlsx_bytes"]).convert_xlsx_bytes(
                    ctx_getter() if isinstance(ctx_getter(), (bytes, bytearray)) else str(ctx_getter()).encode("utf-8", errors="replace"),
                    formulas=formulas,
                ),
        ...
        self._sub_kytchen_fn: SubKytchenFn | None = None
    ...
    def inject_sub_query(self, fn: SubQueryFn) -> None:
        ...
        Internally it may schedule an async coroutine on the Kytchen event loop.
        """

        self._sub_query_fn = fn
        self._namespace["sub_query"] = self._sync_bridge(fn)

    def inject_sub_kytchen(self, fn: SubKytchenFn) -> None:
        """Inject sub_kytchen(query, context=None) into the REPL namespace (open a sub-kitchen)."""

        self._sub_kytchen_fn = fn
        bridged = self._sync_bridge(fn)
        self._namespace["sub_kytchen"] = bridged
    ...
    def _sync_bridge(self, fn: Callable[..., object | Awaitable[object]]) -> Callable[..., object]:
        ...
        def _wrapped(*args: object, **kwargs: object) -> object:
            ...
            if threading.current_thread() is threading.main_thread() and self._loop.is_running():
                ...
                raise RuntimeError(
                    "sub_query/sub_kytchen called from the event loop thread. "
                    "Kytchen runs REPL code in a worker thread; if you are calling execute() "
    ...
    def execute(self, code: str) -> ExecutionResult:
        ...
        still allowing sub_query/sub_kytchen, run it in a worker thread (Kytchen
