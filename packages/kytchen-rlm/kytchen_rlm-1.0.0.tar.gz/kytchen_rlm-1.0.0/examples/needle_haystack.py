...
- Otherwise it falls back to a tiny MockProvider that emits a couple of REPL
  actions to demonstrate the full Kytchen loop.

Run:
    python examples/needle_haystack.py

Optional:
    KYTCHEN_PROVIDER=openai OPENAI_API_KEY=... python examples/needle_haystack.py
"""
...
async def main() -> None:
    ...
    from kytchen import Kytchen, Budget

    provider_name = os.getenv("KYTCHEN_PROVIDER", os.getenv("ELIPH_PROVIDER", "anthropic")).lower()
    ...
    resp = await kytchen.complete(query=query, context=haystack)
