"""Comprehensive tests for the Kytchen MCP Local Server.
...
from kytchen.mcp.local_server import KytchenMCPServerLocal, _detect_format, _analyze_text_context
from kytchen.repl.sandbox import SandboxConfig
from kytchen.types import ContentFormat
...
def mcp_server(sandbox_config):
    ...
    return KytchenMCPServerLocal(sandbox_config=sandbox_config)
...
async def _call_tool(server, tool_name: str, **kwargs):
    ...
    if tool_name == "load_context":
        ...
        from kytchen.repl.sandbox import REPLEnvironment
        ...
        from kytchen.mcp.local_server import _Session
    ...
    if tool_name == "search_context":
        from kytchen.mcp.local_server import _Evidence
...
class TestServerInit:
    ...
    def test_server_creates_fastmcp_instance(self, mcp_server):
        ...
        assert mcp_server.server.name == "kytchen-local"
    ...
    def test_custom_sandbox_config_applied(self, sandbox_config):
        server = KytchenMCPServerLocal(sandbox_config=sandbox_config)
...
class TestPerformanceLimits:
    ...
    async def test_output_truncation(self):
        ...
        server = KytchenMCPServerLocal(sandbox_config=config)
