...
from kytchen.core import Kytchen


class TestMessageTrimming:
    """Tests that message trimming preserves system prompts."""

    def _make_mock_kytchen(self, context_limit: int = 1000, output_limit: int = 500) -> Kytchen:
        """Create a mock Kytchen instance with configurable limits."""
        kytchen = Kytchen.__new__(Kytchen)
        kytchen.provider = MagicMock()
        kytchen.provider.get_context_limit.return_value = context_limit
        kytchen.provider.get_output_limit.return_value = output_limit
        # Simple token counting: ~4 chars per token
        kytchen.provider.count_tokens.side_effect = lambda text, model: len(text) // 4
        return kytchen

    def test_trim_keeps_system(self) -> None:
        """System message should always be preserved."""
        kytchen = self._make_mock_kytchen(context_limit=100, output_limit=50)
        ...
        kytchen._trim_messages(messages, model="test")
    ...
    def test_trim_removes_middle_messages(self) -> None:
        ...
        kytchen = self._make_mock_kytchen(context_limit=500, output_limit=100)
    ...
    def test_no_trim_when_under_limit(self) -> None:
        ...
        kytchen = self._make_mock_kytchen(context_limit=10000, output_limit=500)
    ...
    def test_trim_keeps_recent_messages(self) -> None:
        ...
        kytchen = self._make_mock_kytchen(context_limit=300, output_limit=50)
    ...
    def test_trim_minimum_messages(self) -> None:
        ...
        kytchen = self._make_mock_kytchen(context_limit=10, output_limit=5)
