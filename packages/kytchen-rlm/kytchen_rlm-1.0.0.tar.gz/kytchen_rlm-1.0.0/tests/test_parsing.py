...
from kytchen.core import _FINAL_RE, _FINAL_VAR_RE, _CODE_BLOCK_RE, Kytchen
...
Done."""
    ...
    def test_final_extraction(self) -> None:
        ...
        assert kytchen._extract_final("FINAL(the answer)") == "the answer"
        assert kytchen._extract_final("Prefix FINAL(result) suffix") == "result"
...
class TestFinalVarParsing:
    ...
    def test_final_var_extraction(self) -> None:
        kytchen = Kytchen.__new__(Kytchen)
        assert kytchen._extract_final_var('FINAL_VAR("result")') == "result"
        assert kytchen._extract_final_var("FINAL_VAR('result')") == "result"
        assert kytchen._extract_final_var("FINAL_VAR(result)") == "result"
        assert kytchen._extract_final_var("FINAL_VAR(  spaced  )") == "spaced"
