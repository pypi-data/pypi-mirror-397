# Python Client For Nutanix Networking APIs

The Python client for Nutanix Networking APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage networking configuration on Nutanix clusters, including AHV and advanced networking.
## Features
...
api_client = ntnx_networking_py_client.ApiClient(allow_version_negotiation=False)
```
...
## Configuration
The python client for Nutanix Networking APIs can be configured with the following parameters
...
```python
# Initialize the API
aws_subnets_api_instance = AwsSubnetsApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get NC2A subnets.
try:
    api_response = aws_subnets_api_instance.list_aws_subnets(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=networking&version=v4.2&language=python). This documentation is auto-generated, and the location may change.
