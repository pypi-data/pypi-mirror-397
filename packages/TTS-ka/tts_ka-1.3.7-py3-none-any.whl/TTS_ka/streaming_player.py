    ...
    def _playback_worker_windows(self) -> None:
        ...
        while True:
            ...
            if not first_played:
                try:
                    if use_vlc:
                        # Use VLC for better control. Start instance with --one-instance
                        # so we can enqueue remaining files later without restarting VLC.
                        # Copy the chunk to a temp file to avoid playback interruption if the
                        # original file is later replaced/merged by the generator.
                        try:
                        ...
                        if self.show_gui:
                            vlc_cmd = [vlc_player, '--one-instance', play_target]
                            print("🔊 Playing with VLC GUI while generating remaining chunks...")
                        else:
                            vlc_cmd = [vlc_player, '--intf', 'dummy', '--one-instance', play_target]
                            print("🔊 Playing with VLC while generating remaining chunks...")
                        # Start VLC instance for first-chunk playback and keep reference
                        try:
                            ...
                            # Fallback to starting without capturing output
                            self.process = subprocess.Popen(vlc_cmd)
                    else:
                        # Fallback to Windows default player; copy to temp first
                        try:
                            tmpf = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3")
                            tmpf.close()
                            shutil.copy2(chunk, tmpf.name)
        ...
        # Exclude the chunk already being previewed (output_file) to avoid duplicate playback
        if use_vlc and len(chunks_to_play) > 1 and self.finished_generating:
            try:
                playlist_chunks = [c for c in chunks_to_play if c != output_file]
                if playlist_chunks:
                ...
                if playlist:
                    ...
                    # Enqueue remaining playlist into the existing VLC instance
                    if self.show_gui:
                        enqueue_cmd = [vlc_player, '--one-instance', '--playlist-enqueue', playlist]
                        print(f"🎵 Enqueuing {len(playlist_chunks)} chunks into VLC GUI")
                    else:
                        enqueue_cmd = [vlc_player, '--intf', 'dummy', '--one-instance', '--playlist-enqueue', playlist]
                    ...
                    try:
                        ...
                        # As a fallback, try spawning without redirecting output
                ...
                try:
                    ...
                    if len(chunks) > 1:
                        ...
                        if playlist:
                            ...
                            import time
                            time.sleep(1)
                            # Enqueue remaining chunks into running VLC instance to continue playback
                            if self.show_gui:
                                enqueue_cmd = [player, '--one-instance', '--playlist-enqueue'] + playlist_chunks
                                print(f"🎵 Enqueuing {len(playlist_chunks)} chunks into VLC GUI")
                            else:
                                enqueue_cmd = [player, '--intf', 'dummy', '--one-instance', '--playlist-enqueue'] + playlist_chunks
                                print(f"🎵 Enqueuing {len(playlist_chunks)} chunks into VLC (headless)")
                            try:
                                subprocess.Popen(
                                    enqueue_cmd,
