...
class StreamingAudioPlayer:
    ...
    def __init__(self, show_gui: bool = True):
        ...
        self._temp_play_files: List[str] = []
    ...
    def _playback_worker_windows(self) -> None:
        """Windows streaming playback - plays first chunk immediately, then full playlist when ready."""
        chunks_to_play = []
        first_played = False
        output_file = None
        ...
        while True:
            ...
            if not first_played and use_vlc:
                try:
                    # Create temp copy of first chunk
                    tmpf = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3")
                    tmpf.close()
                    shutil.copy2(chunk, tmpf.name)
                    self._temp_play_files.append(tmpf.name)
                    
                    if self.show_gui:
                        vlc_cmd = [vlc_player, tmpf.name]
                        print("🔊 Playing first chunk in VLC GUI while generating...")
                    else:
                        vlc_cmd = [vlc_player, '--intf', 'dummy', tmpf.name]
                        print("🔊 Playing first chunk in VLC while generating...")
                    ...
                    try:
                        ...
                        self.process = subprocess.Popen(vlc_cmd)
        ...
        # Once all chunks collected, create temp copies and build complete playlist
        if use_vlc and len(chunks_to_play) > 1:
            try:
                # Terminate first preview playback
                if self.process:
                    try:
                ...
                # Create temp copies of ALL chunks
                playlist_items = []
                for chunk in chunks_to_play:
                    try:
                        ...
                        shutil.copy2(chunk, tmpf.name)
                        self._temp_play_files.append(tmpf.name)
                        playlist_items.append(tmpf.name)
                    except Exception:
                        playlist_items.append(chunk)
                
                # Build complete ordered playlist
                playlist = self._create_vlc_playlist(playlist_items)
                
                if playlist:
                    if self.show_gui:
                        vlc_cmd = [vlc_player, '--playlist-autostart', '--no-random', '--no-loop', '--no-repeat', playlist]
                        print(f"🎵 Playing complete playlist ({len(playlist_items)} chunks) in VLC GUI")
                    else:
                        vlc_cmd = [vlc_player, '--intf', 'dummy', '--playlist-autostart', '--no-random', '--no-loop', '--no-repeat', playlist]
                        print(f"🎵 Playing complete playlist ({len(playlist_items)} chunks) in VLC")
        ...
        elif not use_vlc and chunks_to_play:
            # Fallback to Windows default player
            try:
                tmpf = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3")
                tmpf.close()
                shutil.copy2(chunks_to_play[0], tmpf.name)
                self._temp_play_files.append(tmpf.name)
                os.startfile(os.path.abspath(tmpf.name))
                print("🔊 Playing audio...")
            except Exception as e:
        ...
        # Wait for player process to finish (so it has read temp files), then cleanup
        try:
            if self.process:
                try:
                    # Wait up to 300s for playback to finish
                    self.process.wait(timeout=300)
                except Exception:
                    # If it doesn't exit, attempt to terminate
                    try:
                        self.process.terminate()
                    except Exception:
                        pass
        ...
        # Cleanup any temp play files we created
        for tf in getattr(self, '_temp_play_files', []) or []:
            try:
                os.remove(tf)
            except Exception:
                pass
    ...
    def _playback_worker_unix(self) -> None:
        ...
        if 'vlc' in player:
            ...
            if chunks:
                # Play first chunk immediately for instant feedback
                try:
                    # Start VLC instance for first chunk with --one-instance so we can enqueue later
                    # Copy first chunk to temp file to avoid interruptions during merges
                    try:
                        tmpf = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3")
                        tmpf.close()
                        shutil.copy2(chunks[0], tmpf.name)
                        play_target = tmpf.name
                        self._temp_play_files.append(tmpf.name)
                    except Exception:
                        play_target = chunks[0]

                    if self.show_gui:
                        first_cmd = [player, '--one-instance', play_target]
                        print("🔊 Starting VLC GUI playback...")
                    else:
                        first_cmd = [player, '--intf', 'dummy', '--one-instance', play_target]
                    ...
                    try:
                        ...
                        self.process = subprocess.Popen(first_cmd)
                    
                    # If multiple chunks, create playlist for full audio
                    if len(chunks) > 1:
                        # Exclude the first chunk (already playing) to avoid replaying it
                        playlist_chunks = chunks[1:]
                        if playlist_chunks:
                            playlist = self._create_vlc_playlist(playlist_chunks)
                        else:
                            playlist = None
                        if playlist:
                            # Wait briefly, then enqueue the playlist file so VLC plays chunks in order
                            import time
                            time.sleep(1)
                            if self.show_gui:
                                enqueue_cmd = [player, '--one-instance', '--playlist-enqueue', playlist]
                                print(f"🎵 Enqueuing {len(playlist_chunks)} chunks into VLC GUI")
                            else:
                                enqueue_cmd = [player, '--intf', 'dummy', '--one-instance', '--playlist-enqueue', playlist]
                                print(f"🎵 Enqueuing {len(playlist_chunks)} chunks into VLC (headless)")
                            try:
                                subprocess.Popen(
                                    enqueue_cmd,
                                    stdout=subprocess.DEVNULL,
