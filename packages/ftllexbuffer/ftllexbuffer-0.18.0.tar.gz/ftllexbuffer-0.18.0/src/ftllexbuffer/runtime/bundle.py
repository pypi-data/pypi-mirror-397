...
from ftllexbuffer.analysis.graph import detect_cycles
...
from ftllexbuffer.introspection import extract_references, extract_variables, introspect_message
...
class FluentBundle:
    ...
    def __init__(
        ...
        *,
        ...
        """
        # Validate locale format (consistent with create() factory method)
    ...
    def create(
        ...
        *,
        ...
        """Factory method to create a FluentBundle.

        Equivalent to __init__ but provides a more explicit factory pattern.
        Both methods perform the same locale validation.

        Args:
            locale: Locale code (e.g., "en_US", "lv_LV", "de_DE")
        ...
        Returns:
            Configured FluentBundle instance

        Raises:
            ValueError: If locale code is empty or invalid format

        Example:
            >>> bundle = FluentBundle.create("en_US", enable_cache=True)
            >>> bundle.locale
            'en_US'
        """
        # Validation is now done in __init__ (unified validation)
        return cls(
            locale,
    ...
    def _detect_circular_references(
        ...
        Uses the analysis.graph module for cycle detection, delegating the
        graph algorithm to a dedicated, testable component.
        ...
        """
        # Build dependency graph for messages using extract_references
        ...
        # Build dependency graph for terms using extract_references
        ...
        # Detect message cycles using analysis.graph module
        ...
        # Detect term cycles using analysis.graph module
        ...
        try:
            ...
            for entry in resource.entries:
                if isinstance(entry, Junk):
                    ...
                    if entry.span:
                        # Compute line/column from span using Cursor
            ...
            # Second pass: check for undefined references using extract_references
            for msg_name, message in messages_dict.items():
            ...
            # Check term references using extract_references
            for term_name, term in terms_dict.items():
