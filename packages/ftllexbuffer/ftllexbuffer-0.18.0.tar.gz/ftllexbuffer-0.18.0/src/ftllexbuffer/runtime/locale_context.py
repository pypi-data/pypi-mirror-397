    ...
    def format_currency(
        ...
        *,
        ...
        try:
            ...
            # Babel's format_currency() with currency_digits=True automatically
            # uses CLDR currency-specific decimals (0 for JPY, 3 for BHD, 2 for most)
            formatted = str(
            ...
            # For "code" display, replace the currency symbol with the ISO code
            # This preserves the correct CLDR decimal places (0 for JPY, 3 for BHD, etc.)
            # while changing the display from symbol (€) to code (EUR)
            if currency_display == "code":
                symbol = babel_numbers.get_currency_symbol(
                    currency, locale=self.babel_locale
                )
