...
Architecture Note - Circular Imports:
    This module has a mutual dependency with patterns.py:
...
# Thread-local storage for nesting depth tracking (DoS prevention)
_parse_thread_local = thread_local()

# Default maximum nesting depth (configurable via set_max_nesting_depth)
DEFAULT_MAX_NESTING_DEPTH = 100


def _get_nesting_depth() -> int:
    """Get current nesting depth for this thread."""
    if not hasattr(_parse_thread_local, "nesting_depth"):
        _parse_thread_local.nesting_depth = 0
    depth: int = _parse_thread_local.nesting_depth
    return depth


def _set_nesting_depth(depth: int) -> None:
    """Set nesting depth for this thread."""
    _parse_thread_local.nesting_depth = depth


def _get_max_nesting_depth() -> int:
    """Get maximum nesting depth for this thread."""
    if not hasattr(_parse_thread_local, "max_nesting_depth"):
        _parse_thread_local.max_nesting_depth = DEFAULT_MAX_NESTING_DEPTH
    max_depth: int = _parse_thread_local.max_nesting_depth
    return max_depth


def set_max_nesting_depth(max_depth: int) -> None:
    """Configure maximum nesting depth for placeable parsing.

    This is a security feature to prevent DoS attacks via deeply nested
    placeables like { { { { ... } } } }.
    ...
    _parse_thread_local.max_nesting_depth = max_depth
...
def parse_placeable(cursor: Cursor) -> ParseResult[Placeable] | None:
    ...
    Security:
        ...
        nested placeables. Configure via set_max_nesting_depth().
    ...
    current_depth = _get_nesting_depth()
    ...
    # Increment depth for nested parsing
    _set_nesting_depth(current_depth + 1)
    ...
    try:
        ...
        # Always restore depth after parsing (even on failure/exception)
        _set_nesting_depth(current_depth)
