...
class PathResourceLoader:
    ...
    def load(self, locale: LocaleCode, resource_id: ResourceId) -> FTLSource:
        ...
        Raises:
            ValueError: If resource_id contains path traversal sequences (security)
            FileNotFoundError: If file doesn't exist
            OSError: If file cannot be read

        Security:
            Validates resource_id to prevent directory traversal attacks.
        ...
        # This catches edge cases that simple string checking might miss
        if not self._is_safe_path(base_dir, full_path):
            msg = (
                f"Path traversal detected: resource_id '{resource_id}' "
