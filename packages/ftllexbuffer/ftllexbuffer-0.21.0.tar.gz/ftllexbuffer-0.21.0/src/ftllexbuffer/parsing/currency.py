...
# Number of locales sampled during module initialization for currency discovery.
# 150 locales covers all major CLDR currencies without scanning 600+ locales.
_CURRENCY_DISCOVERY_SAMPLE_SIZE: int = 150
...
def _build_currency_maps_from_cldr() -> tuple[dict[str, str], set[str], dict[str, str]]:
    ...
    representative_sample = all_locale_ids[:_CURRENCY_DISCOVERY_SAMPLE_SIZE]
...
def _build_currency_pattern() -> re.Pattern[str]:
    """Build dynamic regex pattern for currency symbol/code detection.

    Constructs pattern from CLDR-derived symbol maps at module initialization,
    eliminating the need for hardcoded symbol lists that miss locale-specific
    symbols (zl, Kc, Ft, RM, Rp, R, etc.).

    Returns:
        Compiled regex pattern matching:
        - ISO 4217 3-letter currency codes (e.g., EUR, USD, JPY) - matched first
        - All symbols from _CURRENCY_SYMBOL_MAP (unambiguous)
        - All symbols from _AMBIGUOUS_SYMBOLS
    ...
    all_symbols: set[str] = set(_CURRENCY_SYMBOL_MAP.keys()) | _AMBIGUOUS_SYMBOLS
...
# Pre-compile currency detection pattern at module initialization
_CURRENCY_PATTERN: re.Pattern[str] = _build_currency_pattern()
...
def parse_currency(
    ...
    *,
    ...
    # Pattern includes all symbols from _CURRENCY_SYMBOL_MAP, _AMBIGUOUS_SYMBOLS,
    # and 3-letter ISO codes. Sorted by length to match longer symbols first.
