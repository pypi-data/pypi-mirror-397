...
parsing of FTL source files into AST structures defined in :mod:`ftllexbuffer.syntax.ast`.

Architecture:
    The parser uses an immutable cursor pattern (:class:`~ftllexbuffer.syntax.cursor.Cursor`)
    to traverse source text. Each sub-parser (in :mod:`~ftllexbuffer.syntax.parser.entries`,
    :mod:`~ftllexbuffer.syntax.parser.patterns`, etc.) returns either a
    :class:`~ftllexbuffer.syntax.cursor.ParseResult` containing the parsed AST node
    and updated cursor position, or None on parse failure.

AST Types:
    The parser produces a :class:`~ftllexbuffer.syntax.ast.Resource` containing entries:

    - :class:`~ftllexbuffer.syntax.ast.Message` - User-visible messages with optional attributes
    - :class:`~ftllexbuffer.syntax.ast.Term` - Reusable terms (referenced with ``-term`` syntax)
    - :class:`~ftllexbuffer.syntax.ast.Comment` - Single-line, group, or resource comments
    - :class:`~ftllexbuffer.syntax.ast.Junk` - Unparseable content (robustness principle)
...
See Also:
    - :mod:`ftllexbuffer.syntax.ast` - All AST node type definitions
    - :mod:`ftllexbuffer.syntax.cursor` - Cursor and ParseResult types
    - :mod:`ftllexbuffer.syntax.parser.entries` - Entry parsers (message, term, comment)
    - :mod:`ftllexbuffer.syntax.parser.patterns` - Pattern and expression parsers
"""

from ftllexbuffer.diagnostics import DiagnosticCode
...
DEFAULT_MAX_SOURCE_SIZE = 10 * 1024 * 1024
...
class FluentParserV1:
    ...
    def parse(self, source: str) -> Resource:
        ...
        Returns:
            :class:`~ftllexbuffer.syntax.ast.Resource` with tuple of entries, where
            each entry is one of:

            - :class:`~ftllexbuffer.syntax.ast.Message` - Parsed message with id, value, attributes
            - :class:`~ftllexbuffer.syntax.ast.Term` - Parsed term (id starts with ``-``)
            - :class:`~ftllexbuffer.syntax.ast.Comment` - Standalone comment
            - :class:`~ftllexbuffer.syntax.ast.Junk` - Unparseable content with annotations
        ...
        See Also:
            - :func:`~ftllexbuffer.syntax.parser.entries.parse_message` - Message parsing
            - :func:`~ftllexbuffer.syntax.parser.entries.parse_term` - Term parsing
            - :func:`~ftllexbuffer.syntax.parser.entries.parse_comment` - Comment parsing
        """
