...
- Import: `FunctionRegistry` from `ftllexbuffer.runtime.function_bridge`.
...
### Constraints
- Return: Configured FluentBundle instance.
- Raises: `ValueError` on invalid locale or cache config.
- State: Creates internal registries with early validation.
- Thread: Safe.
...
- Thread: Safe.
- Security: Validates resource_id against directory traversal attacks (rejects "..", absolute paths).
