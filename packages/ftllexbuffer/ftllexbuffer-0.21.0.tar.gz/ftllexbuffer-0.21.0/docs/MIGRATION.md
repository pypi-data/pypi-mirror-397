...
**Target Audience**: Developers currently using `fluent.runtime` (Mozilla's Python implementation) who want to migrate to FTLLexBuffer for Python 3.13+ features, better type safety, and improved performance.

**[IMPORTANT] REQUIREMENT: Python 3.13+**
FTLLexBuffer requires Python 3.13 or later. If your project uses Python 3.12 or earlier, you must upgrade your Python version before migrating.

---

## Why Migrate?

### FTLLexBuffer Advantages
...
- Your project requires Python 3.6-3.12 (FTLLexBuffer requires 3.13+)
...
- [ ] Update type annotations to use FTLLexBuffer type aliases
...
from ftllexbuffer import FluentError, FluentReferenceError, FluentResolutionError
```

**Changes**:
- ✅ Top-level imports (no `ftllexbuffer.runtime` submodule needed)
...
# FTLLexBuffer - extract first locale from list
locales = ['en-US']
...
# FTLLexBuffer - simpler (no get_message step)
result, errors = bundle.format_pattern('hello', {})
```
...
- FTLLexBuffer: 13 lines (32% reduction)
...
| fluent.runtime | FTLLexBuffer | Notes |
|----------------|--------------|-------|
...
| fluent.runtime | FTLLexBuffer | Changes |
|----------------|--------------|---------|
...
**Benefit**: Similar API, FTLLexBuffer returns errors for better handling
...
# ✅ FTLLexBuffer - direct message ID
result, errors = bundle.format_pattern('hello', {})
```
...
# ✅ FTLLexBuffer - specific error types
...
- [ ] Install FTLLexBuffer: `pip install ftllexbuffer`
- [ ] Update imports: `fluent.runtime` → `ftllexbuffer`
...
**Migration Guide Last Updated**: December 14, 2025
**FTLLexBuffer Version**: 0.13.0
