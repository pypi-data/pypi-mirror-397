...
**Factory Methods** (v0.13.0+):
```python
# Early validation (raises ValueError on invalid locale/config)
...
with FluentBundle.create("en", enable_cache=True) as bundle:
...
```python
from ftllexbuffer.parsing import parse_decimal, parse_date, parse_currency
from ftllexbuffer.parsing import is_valid_decimal
...
**Quick Reference Last Updated**: December 16, 2025
**FTLLexBuffer Version**: 0.21.0
