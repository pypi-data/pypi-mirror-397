...
FTLLexBuffer is built with modern Python 3.13+ features and provides full `mypy --strict` type safety. This guide shows how to leverage type hints for better code quality and IDE support.
...
## Python 3.13+ Features in FTLLexBuffer

### 1. PEP 695: Type Parameter Syntax (`type` keyword)

FTLLexBuffer uses the new `type` keyword for type aliases:

```python
# FTLLexBuffer source code (Python 3.13+)
type MessageId = str
type LocaleCode = str
type ResourceId = str
type FTLSource = str
...
FTLLexBuffer uses `TypeIs` for runtime type narrowing:
...
Python 3.10+ pattern matching + FTLLexBuffer types:

```python
from ftllexbuffer import parse_ftl, Message, Term, Comment, Junk
...
from ftllexbuffer import FluentError, FluentReferenceError
...
For maximum type safety with FTLLexBuffer:
