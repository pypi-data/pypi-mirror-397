...
class FluentResolverStateMachine(RuleBasedStateMachine):
    ...
    def test_unknown_expression_type(self):
        """Test unknown expression type handling.

        Target: Line 151 (unknown expression type error - line 275 mentioned in gaps).
        Note: This is hard to trigger in practice since all expression types are covered.
        We would need to mock an unknown expression type.
        """
        # This error path is likely unreachable in normal operation
        # since all AST node types are handled in the match statement.
        # We document this as architectural correctness rather than runtime coverage.
    ...
    @invariant()
    def resolution_stack_empty_between_calls(self):
        """Invariant: Resolution stack is cleared after each top-level resolve."""
        # Stack should only be non-empty during active resolution
        # After any rule completes, stack should be empty
        # Access thread-local stack via module function
        from ftllexbuffer.runtime.resolver import (  # noqa: PLC0415
            _get_resolution_stack,
        )

        assert len(_get_resolution_stack()) == 0
