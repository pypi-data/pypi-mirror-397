...
from ftllexbuffer.introspection import IntrospectionVisitor, ReferenceExtractor
...
class TestFluentBundleFunctionsParameter:
    ...
    def test_create_factory_accepts_functions(self) -> None:
        """FluentBundle.create() accepts functions parameter."""
        registry = create_default_registry()

        def custom_dt(_value: str) -> str:
            return "CUSTOM-DT"

        registry.register(custom_dt, ftl_name="DATETIME")

        bundle = FluentBundle.create("en", functions=registry, use_isolating=False)
        bundle.add_resource("test = { DATETIME($d) }")
        result, errors = bundle.format_pattern("test", {"d": "2025-01-01"})

        assert errors == ()
        assert result == "CUSTOM-DT"
...
class TestFunctionRegistryRemoval:
    ...
    def test_function_registry_not_in_module(self) -> None:
        ...
        from ftllexbuffer.runtime import functions  # noqa: PLC0415
    ...
    def test_function_registry_import_raises(self) -> None:
        ...
        module = importlib.import_module("ftllexbuffer.runtime.functions")
