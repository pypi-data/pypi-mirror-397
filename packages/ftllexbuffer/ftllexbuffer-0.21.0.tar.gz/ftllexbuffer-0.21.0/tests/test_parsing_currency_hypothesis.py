...
class TestParseCurrencyHypothesis:
    ...
    def test_parse_currency_symbol_in_regex_but_not_in_map(
        ...
        monkeypatch.setattr(
            "ftllexbuffer.parsing.currency._CURRENCY_SYMBOL_MAP", modified_map
        )
    ...
    @given(
        value=st.text(
            ...
            lambda x: (
                ...
                and not any(
                    symbol in x for symbol in _CURRENCY_SYMBOL_MAP
                )
            )
        ),
    )
