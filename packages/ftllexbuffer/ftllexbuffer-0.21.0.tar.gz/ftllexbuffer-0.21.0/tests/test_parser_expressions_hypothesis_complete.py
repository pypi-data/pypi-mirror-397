...
class TestNestingDepthControl:
    ...
    def test_set_max_nesting_depth_positive(self) -> None:
        """Verify set_max_nesting_depth accepts valid values."""
        set_max_nesting_depth(50)
        set_max_nesting_depth(200)
        # Reset to default
        set_max_nesting_depth(DEFAULT_MAX_NESTING_DEPTH)

    def test_set_max_nesting_depth_rejects_zero(self) -> None:
        """Verify set_max_nesting_depth raises ValueError for 0."""
        with pytest.raises(ValueError, match="at least 1"):
            set_max_nesting_depth(0)

    def test_set_max_nesting_depth_rejects_negative(self) -> None:
        """Verify set_max_nesting_depth raises ValueError for negative."""
        with pytest.raises(ValueError, match="at least 1"):
            set_max_nesting_depth(-1)

    @given(st.integers(min_value=1, max_value=1000))
    def test_set_max_nesting_depth_property(self, depth: int) -> None:
        """PROPERTY: All positive depths are accepted."""
        set_max_nesting_depth(depth)
        # Reset to default
        set_max_nesting_depth(DEFAULT_MAX_NESTING_DEPTH)
    ...
    def test_nesting_depth_exceeded_in_parse_placeable(self) -> None:
        ...
        # Set very low limit
        set_max_nesting_depth(3)
        try:
            bundle = FluentBundle("en_US")
            # Create deeply nested placeables
            nested = "{ " * 10 + "text" + " }" * 10
            bundle.add_resource(f"msg = {nested}")
