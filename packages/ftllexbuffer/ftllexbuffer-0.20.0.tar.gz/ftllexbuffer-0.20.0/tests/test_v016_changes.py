...
class TestFluentBundleLocaleValidation:
    ...
    def test_create_with_empty_locale_raises(self) -> None:
        """create() also raises for empty locale (uses same validation)."""
        with pytest.raises(ValueError, match="Locale code cannot be empty"):
            FluentBundle.create("")

    def test_create_with_invalid_locale_format_raises(self) -> None:
        """create() also raises for invalid locale format."""
        with pytest.raises(ValueError, match="Invalid locale code format"):
            FluentBundle.create("en US")
