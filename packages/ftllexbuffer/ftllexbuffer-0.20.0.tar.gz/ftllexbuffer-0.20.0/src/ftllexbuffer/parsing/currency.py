...
def _build_currency_pattern() -> re.Pattern[str]:
    ...
    if escaped_symbols:
        ...
        pattern = rf"([A-Z]{{3}}|{symbols_pattern})"
    else:
...
def parse_currency(
    ...
    *,
    ...
    # Determine if this is an ISO code (3 uppercase letters) or a symbol
    # ISO codes are always unambiguous; symbols need lookup in CLDR maps
    is_iso_code = len(currency_str) == 3 and currency_str.isupper() and currency_str.isalpha()
