...
# Forward reference for caching function - will be defined after LocaleContext
_locale_context_cache: dict[str, "LocaleContext"] = {}
...
class LocaleContext:
    ...
    def create(cls, locale_code: str) -> "LocaleContext":
        ...
        try:
            ...
            ctx = cls(locale_code=locale_code, _babel_locale=fallback_locale)
        ...
        if len(_locale_context_cache) < 128:
        ...
        *,
        ...
        try:
            ...
            if currency_display == "code":
                ...
                # This avoids brittle string replacement that could fail if symbol
                # appears elsewhere in the formatted output
                ...
                if standard_pattern and hasattr(standard_pattern, "pattern"):
                    # Access the raw pattern string, not the NumberPattern repr
                    # Convert single currency sign to double for ISO code display
                    # Single ¤ = symbol, Double ¤¤ = ISO code per CLDR spec
                    code_pattern = standard_pattern.pattern.replace("\xa4", "\xa4\xa4")
                    return str(
                        babel_numbers.format_currency(
                            value,
                            currency,
                ...
                # (shouldn't happen with valid Babel locales)
