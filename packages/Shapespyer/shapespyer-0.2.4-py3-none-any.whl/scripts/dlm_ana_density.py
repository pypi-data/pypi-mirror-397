#!/usr/bin/env python3
"""
The script determines mass density or scattering length density (SLD) profiles from
DL_MESO_DPD results - either equilibration or production runs - by finding
radial distribution functions of species from centre-of-mass of molecular
structure using provided configurations/trajectory frames.

Usage:
    dlm-ana-density [--in <input>] [--fieldin <fieldin>] [--out <output>] 
                       (--lscale <lscale> | --water <water>)
                       [--averaged] [--frame <frame>] [--dr <dr>] 
                       [--rmax <rmax>] [--masses <masses>] [--csl <csl>]
                       [--plot <plot>]
...
Options:
    ...
    --out <output>          Name of output file to write mass density or 
                            scattering length density profiles
                            [default: massdensity.dat]
    ...
    --frame <frame>         Use frame number <frame> from HISTORY file for 
                            mass density profile (default value of 0 sets it 
                            to final frame) [default: 0]
    --dr <dr>               Histogram bin size (distance spacing) in nm to 
                            use for density profile [default: 0.1]
    ...
    --masses <masses>       Override masses for each bead species with values
                            ...
                            comma-separated list in same order as original
                            simulation's FIELD file 
    --csl <csl>             Coherent scattering lengths for each bead species
                            given in fm as a comma-separated list in same 
                            order as original simulation's FIELD file: 
                            calculates scattering length density (SLD) 
                            profiles instead of mass density profiles
    --plot <plot>           Plot resulting mass density or SLD profiles to 
                            PDF file <plot>.pdf (only use if name specified)

michael.seaton@stfc.ac.uk, 21/06/24
...
def read_config(filename):
    ...
    # (and check against number in line 2 if available)
        ...
        if nbeads>0 and nbeads!=nsyst:
            logger.warning("WARNING: Mismatch in reported number of particles - {0:d} != {1:d}".format(nbeads, nsyst))
...
def main():
    ...
    logger.info(f"DL_MESO_DPD Mass/Scattering Length Density Analysis{NL_INDENT}"
                f"==================================================={NL_INDENT}"
                "Calculates mass/scattering length density profiles from DL_MESO_DPD "
                "simulation results")
    ...
    # see if user has provided coherent scattering lengths for 
    # all bead species: if so, set option to calculate scattering
    # length density profiles instead of mass density profiles

    sld = False
    if args["--csl"] != None:
        # parse list of coherent scattering lengths in femtometres
        csl = [float(x) for x in args["--csl"].split(',')]
        sld = (len(csl)>=len(speciesprop))
    ...
    if sld:
        ...
        for spec in range(numspecies):
            print("{0:8s} {1:f} fm".format(species[spec], csl[spec]))
    
    print("\nGrid spacing for density profile = {0:f} nm".format(dr))
    print("Maximum possible distance available = {0:f} nm".format(rmax))
    ...
    if sld:
        if dlmhistory:
            ...
            if averaged:
                ...
                print("Using all available frames for scattering length density profile (over {0:f} ns)\n".format(0.001*time*tscale))
            elif framenum==numframe:
                time, _, _, _, _ = read_history_frame(input, ri, rd, numframe-1, framesize, headerpos, keytrj)
                print("Using final available frame for scattering length density profile (after {0:f} ns)\n".format(0.001*time*tscale))
            else:
                time, _, _, _, _ = read_history_frame(input, ri, rd, framenum-1, framesize, headerpos, keytrj)
                print("Using frame {0:d} for scattering length density profile (after {1:f} ns)\n".format(framenum, 0.001*time*tscale))
        elif dlmexport:
            time, _, _, _, _ = read_export_configuration(input, bo, ri, rd, intsize, realsize)
            print("\nSimulation restart state available (at {0:f} ns) for scattering length density profile\n".format(0.001*time*tscale))
        elif dlmconfig:
            print("\nSimulation initial configuration available for scattering length density profile\n")
    else:
        if dlmhistory:
            print("\nNumber of trajectory frames available = {0:d}".format(numframe))
            if averaged:
                time, _, _, _, _ = read_history_frame(input, ri, rd, numframe-1, framesize, headerpos, keytrj)
                print("Using all available frames for mass density profile (over {0:f} ns)\n".format(0.001*time*tscale))
            elif framenum==numframe:
                time, _, _, _, _ = read_history_frame(input, ri, rd, numframe-1, framesize, headerpos, keytrj)
                print("Using final available frame for mass density profile (after {0:f} ns)\n".format(0.001*time*tscale))
            else:
                time, _, _, _, _ = read_history_frame(input, ri, rd, framenum-1, framesize, headerpos, keytrj)
                print("Using frame {0:d} for mass density profile (after {1:f} ns)\n".format(framenum, 0.001*time*tscale))
        elif dlmexport:
            time, _, _, _, _ = read_export_configuration(input, bo, ri, rd, intsize, realsize)
            print("\nSimulation restart state available (at {0:f} ns) for mass density profile\n".format(0.001*time*tscale))
        elif dlmconfig:
    ...
    rdf = np.zeros((numspecies, numbins+1))
    ...
    if dlmhistory:
        ...
        for frame in tqdm(range(first, last)):
            ...
            # now run through all particles in the system, identify its
            # species, find its distance from the centre-of-mass
            # (taking periodic boundaries into account) and assign to
            # the required histogram bin
            ...
            for part in range(nsyst):
                ...
                bin_i = min(int(dr_i/dr), numbins) # use additional bin for all distances above rmax
                rdf[spec][bin_i] += 1.0
    ...
    # convert numbers of beads in each histogram bin to densities,
    # taking masses of beads and volumes of spherical shells into account 

    binlims = np.linspace(0.0, rmax, numbins+1)
    r = binlims[:-1] + 0.5 * dr
    ...
    if sld:
        for spec in range(numspecies):
            rdf[spec][0:numbins] = 1.0e-8 * csl[spec] * rdf[spec][0:numbins] / (4.0 * np.pi * dr * (r * r + dr * dr / 12.0))
            rdfall[0:numbins] += rdf[spec][0:numbins]
    else:
        for spec in range(numspecies):
            rdf[spec][0:numbins] = 0.01 * speciesmass[spec] * rdf[spec][0:numbins] / (4.0 * np.pi * dr * (r * r + dr * dr / 12.0) * 6.02214076)
            rdfall[0:numbins] += rdf[spec][0:numbins]

    # output resulting density profiles to tab-delimited file

    so = "#     position"
    for spec in range(numspecies):
        so += "      {:>8}".format(species[spec])
    so += "   all species\n"
    if sld:
        for i in range(numbins):
            so += "{0:14.7f}".format(r[i])
            for spec in range(numspecies):
                so +="{0:14.6e}".format(rdf[spec][i])
            so += "{0:14.6e}\n".format(rdfall[i])
    else:
        for i in range(numbins):
            so += "{0:14.7f}".format(r[i])
            for spec in range(numspecies):
                so +="{0:14.7f}".format(rdf[spec][i])
            so += "{0:14.7f}\n".format(rdfall[i])
    open(output,"w").write(so)
    logger.info("Density profiles written to {0:s}".format(output))
    ...
    if args["--plot"] != None:
        ...
        if (plotname[-4:]!='.pdf'):
            plotname += ".pdf"
        for spec in range(numspecies):
            plt.plot(r[0:numbins], rdf[spec][0:numbins], '.-', label='{0:s}'.format(species[spec]))
        plt.plot(r[0:numbins], rdfall[0:numbins], '.-', label='all species')
        ...
        plt.close()
        logger.info("Density profiles plotted in {0:s}".format(plotname))
