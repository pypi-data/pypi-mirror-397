...
class VisualizationTester:
    ...
    async def test_1_filter_buttons_graph_redraw(self):
        ...
        if code_nodes["visible"] > 1000:
            result["passed"] = False
            result["details"].append(f"❌ FAIL: Too many nodes expanded ({code_nodes['visible']}). Tree should stay collapsed.")
            print(f"  ❌ FAIL: Massive expansion detected ({code_nodes['visible']} nodes)")
        else:
            result["details"].append(f"✓ Tree stayed collapsed ({code_nodes['visible']} nodes)")
        ...
        if doc_nodes > 0:
            result["passed"] = False
            result["details"].append(f"❌ FAIL: {doc_nodes} doc nodes still visible with Code filter")
        ...
        if all_nodes != initial_nodes:
            result["passed"] = False
            result["details"].append(f"❌ FAIL: Node count mismatch ({all_nodes} vs {initial_nodes})")
    ...
    async def test_2_section_nav_hidden_for_reports(self):
        ...
        if section_nav_visible:
            result["passed"] = False
            result["details"].append("❌ FAIL: Section nav visible for Complexity report")
        ...
        if section_nav_visible:
            result["passed"] = False
            result["details"].append("❌ FAIL: Section nav visible for Code Smells report")
            print("  ❌ FAIL: Section nav should be hidden")
        else:
        ...
        if section_nav_visible:
            result["passed"] = False
            result["details"].append("❌ FAIL: Section nav visible for Dependencies report")
            print("  ❌ FAIL: Section nav should be hidden")
        else:
        ...
        if not section_nav_visible:
            result["passed"] = False
            result["details"].append("❌ FAIL: Section nav hidden for code chunk viewing")
    ...
    async def test_3_no_javascript_errors(self):
        ...
        if errors:
            result["passed"] = False
            result["details"].append(f"❌ FAIL: {len(errors)} JavaScript errors detected")
            print("\n  JavaScript Errors:")
