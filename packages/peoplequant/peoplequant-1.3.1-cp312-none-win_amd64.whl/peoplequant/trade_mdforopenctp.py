...
class PeopleQuantApi():
    ...
    def get_position(self,InstrumentID:str="",wait_return=False,_ctp = None) -> Union[Dict[str,Position] ,Position]:
        ...
        if InstrumentID in self.positions:
            ...
            if InstrumentID : self.get_quote(InstrumentID)
    ...
    def open_close(self,symbol:str,kaiping:str='',lot:int=0,price=None,block=True,n_price_tick=1,che_time=0,order_info='无',signal_price=float('nan'),order_close_chan=True,advanced=None,HedgeFlag:str="1",WaitReturn=False,OrderMemo="",_print=True,**kw):
        '''
        ...
        if isinstance(ping_zuo,Order): #报单成功发向交易所
            ...
            while True:
                ...
                if ping_zuo["OrderStatus"] not in finished or (ping_zuo["VolumeTraded"] if not_Combination else ping_zuo["VolumeTraded"]*2) != sum( trade["Volume"] for trade in trades_list): #平昨单是否完成
                    ...
                    and (last_price != quote.LastPrice == quote.LastPrice and not_Combination or not not_Combination and not isinstance(price,(int,float))) 
    ...
    def check_order(self,order,block=True,n_price_tick=1,che_time=0,order_info='无',signal_price=float('nan'),order_close_chan=True,HedgeFlag:str="1",WaitReturn=False,OrderMemo="",_print=True,**kw):
        '''
        检查委托单,并获取委托单成交结果
                ...
                with self.data_lock:
                    ...
                    if "instruments" in i:
                        ...
                        for order_id in i["order"]:
                            ...
                            trades_list = self.get_trade_of_order(order_id,_print=False)
                            self.orders[order_id]["trades_list"] = trades_list
                            if self.orders[order_id]["OrderStatus"] in ['0','4' ,'5' ] and 0 < self.orders[order_id]["VolumeTraded"] == sum( trade["Volume"] for trade in trades_list):
                                self.orders[order_id]["trade_price"] = sum( trade["Volume"]*trade["Price"] for trade in trades_list) / self.orders[order_id]["VolumeTraded"]
