...
class IranIPMiddleware:
    """
    این میانی‌افزار آی‌پی واقعی کاربر را شناسایی کرده و به شیء request اضافه می‌کند.
    استفاده: request.client_ip
    """
    ...
    def __call__(self, request):
        # شناسایی آی‌پی و اتچ کردن آن به request
        client_ip = self.resolver.get_client_ip(request)

        # اضافه کردن IP به request
        ...
        # اضافه کردن هدر IP به response (اختیاری)
...
class IranIPLoggingMiddleware:
    """
    میانی‌افزار برای لاگ کردن تمام IPها
    """
...
class IranIPRateLimitMiddleware:
    """
    میانی‌افزار برای محدودیت نرخ درخواست بر اساس IP
    """
    ...
    def __init__(self, get_response):
        ...
        # تنظیمات rate limit از settings
