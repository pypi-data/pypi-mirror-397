...
class HeaderStrategy(BaseIPStrategy):
    """استخراج IP از هدرهای HTTP (مناسب برای سرورهای پشت CDN)"""
    HEADERS = [
    ...
    def get_ip(self, request=None) -> Optional[str]:
        ...
        for header in self.HEADERS:
            ...
            if value:
                # در X-Forwarded-For، اولین IP معتبر است
...
class ServiceStrategy(BaseIPStrategy):
    """استخراج IP از طریق فراخوانی APIهای خارجی"""
    ...
    def __init__(self, services: List[str] = None, timeout: float = 3.0, use_iran_services: bool = True):
        """
        Args:
            services: لیست URLهای سرویس‌ها
            timeout: تایم‌اوت برای هر درخواست
            use_iran_services: آیا سرویس‌های ایرانی هم استفاده شوند؟
        """
...
class SocketStrategy(BaseIPStrategy):
    """
    استخراج IP از طریق ایجاد سوکت به سرورهای DNS عمومی
    این روش بدون ارسال داده واقعی، فقط IP محلی سیستم را پیدا می‌کند
    """

    def __init__(self, test_host: str = "8.8.8.8", test_port: int = 80):
        """
        Args:
            test_host: هاست برای تست اتصال (مثلاً DNS گوگل)
            test_port: پورت برای تست
        """
    ...
    def get_ip(self, request=None) -> Optional[str]:
        """
        این متد IP محلی سیستم را برمی‌گرداند نه Public IP
        فقط در صورتی مفید است که سرور مستقیماً به اینترنت متصل باشد
        """
        try:
...
class CachedServiceStrategy(BaseIPStrategy):
    """
    استراتژی کش‌شده برای جلوگیری از فراخوانی مکرر APIها
    ...
    def __init__(self, base_strategy: BaseIPStrategy = None, cache_duration: int = 3600):
        """
        Args:
            base_strategy: استراتژی پایه برای دریافت IP
            cache_duration: مدت زمان کش به ثانیه (پیش‌فرض: 1 ساعت)
        """
    ...
    def get_ip(self, request=None) -> Optional[str]:
        ...
        # در غیر این صورت، IP جدید دریافت کن
...
class FallbackStrategy(BaseIPStrategy):
    """
    استراتژی ترکیبی با fallback
    اگر استراتژی اول کار نکرد، به بعدی می‌رود
    """
