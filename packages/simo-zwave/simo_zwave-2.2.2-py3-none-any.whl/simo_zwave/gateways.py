...
class ZwaveGatewayHandler(BaseObjectCommandsGatewayHandler):
    ...
    def _heal_node_routes(self, node_id: int) -> bool:
        ...
        if not (self._client and self._client.connected and self._loop):
            return False
        schema_version = getattr(self._client, 'schema_version', None)
        if isinstance(schema_version, int):
            if schema_version >= 32:
                commands = ['controller.rebuild_node_routes', 'controller.heal_node']
            else:
                commands = ['controller.heal_node', 'controller.rebuild_node_routes']
        else:
            commands = ['controller.rebuild_node_routes', 'controller.heal_node']
        ...
        for cmd in commands:
            try:
                resp = self._async_call(
                    self._client.async_send_command({'command': cmd, 'nodeId': int(node_id)}),
                    timeout=60,
                )
    ...
    def heal_dead_nodes(self):
        ...
        """
        try:
            if not (self._client and getattr(self._client, 'driver', None) and self._client.connected and self._loop):
                return
            ...
            try:
                ...
                for nid in sorted(candidate_ids):
                    try:
                        ...
                        self._heal_node_routes(int(nid))
                        break
                    except Exception as e:
                        if 'node_not_found' in str(e).lower():
                            continue
