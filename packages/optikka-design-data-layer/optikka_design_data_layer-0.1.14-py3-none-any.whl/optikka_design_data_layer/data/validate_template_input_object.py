...
class TemplateInputValidator:  # pylint: disable=too-many-instance-attributes
        ...
        if (
            ...
            if group_parsing_label:
                for item in group.items:
                    ...
                    # Note: Text items are not supported in groups - if a text item is found, it will be ignored
    ...
    def _validate_groups(self):
        ...
        if len(registry_groups) > 0 and input_group is None:
            self.errors.append(
                InputObjectValidationError(
                    f"Template registry defines groups {list(registry_groups.keys())} but template input has no group"
                )
            )
            return
