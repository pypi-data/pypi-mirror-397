...
def _format_cmd_option_legacy(k: str, v: Any) -> str:
    return '-Des.{0}={1}'.format(k, v)


def _format_cmd_option(k: str, v: Any) -> str:
...
class CrateNode(contextlib.ExitStack):
    ...
    def __init__(self,
        ...
        self.settings = settings = _get_settings(settings)
...
def _extract_tarball(tarball: Path) -> str:
    with tarfile.open(tarball) as t:
        ...
        t.extractall(tarball.parent, filter=tarfile.data_filter)
...
def create_node(version: str,
    ...
    return CrateNode(
        crate_dir=crate_dir,
        env=(env is not None and _parse_options(env) or None),
        settings=settings,
        keep_data=keep_data,
