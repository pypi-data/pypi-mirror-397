...
- Added support for Python 3.14. Because of the asyncio event loop changes in
  Python 3.14 this includes breaking changes. Some of the cr8 functions like
  ``do_run_spec`` now must be called within an ``async`` function with ``await``.
