...
class DensityFlowMO(nn.Module):
    ...
    >>> from DensityFlowMO.perturb import LabelMatrix
    >>> import scanpy as sc
    ...
    >>> ln = lb.labels_
    >>> model = DensityFlowMO(input_size = xs.shape[1],
    ...
    def __init__(self,
                 ...
                 atac_loss_func: Literal['negbinomial','poisson','multinomial','bernoulli'] = 'bernoulli',
        ...
        print(f"🧬 DensityFlowMO Initialized:")
    ...
    def setup_networks(self):
        ...
        if self.perturb_size>0:
            ...
            self.decoder_rna_effect_from_atac = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, 
            ...
            self.decoder_atac_effect_from_rna = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, 
                                                               transforms=self.transforms, hidden_features=self.flow_hidden_layers)
        
        self.decoder_atac2rna = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, 
                                               transforms=self.transforms, hidden_features=self.flow_hidden_layers)
        self.decoder_rna2atac = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, 
                                               transforms=self.transforms, hidden_features=self.flow_hidden_layers)
    ...
    def model(self, xs_rna, xs_atac, us=None, cs=None):
        pyro.module('DensityFlowMO', self)
        ...
        with pyro.plate('data'):
            ...
            ###############################################################
            # atac2rna
            rna_zs = pyro.sample('rna_zns', ZukoToPyro(self.decoder_atac2rna(atac_zns)), obs=rna_zns)
            if self.perturb_size>0:
                rna_zus2 = pyro.sample('rna_zus', ZukoToPyro(self.decoder_rna_effect_from_atac(atac_zus)), obs=rna_zus)
                rna_zs = rna_zs+rna_zus2
            ...
            if self.rna_loss_func == 'negbinomial':
                if self.use_zeroinflate:
                    pyro.sample("rna_x2", MyZINB(mu=rna_mu, theta=rna_dispersion, zi_logits=rna_gate_logits).to_event(1), obs=xs_rna)
                else:
                    pyro.sample("rna_x2", MyNB(mu=rna_mu, theta=rna_dispersion).to_event(1), obs=xs_rna)
            elif self.rna_loss_func == 'poisson':
                if self.use_zeroinflate:
                    pyro.sample('rna_x2', dist.ZeroInflatedDistribution(dist.Poisson(rate=rna_rate),gate_logits=rna_gate_logits).to_event(1), obs=xs_rna.round())
                else:
                    pyro.sample('rna_x2', dist.Poisson(rate=rna_rate).to_event(1), obs=xs_rna.round())
            elif self.rna_loss_func == 'multinomial':
                pyro.sample('rna_x2', dist.Multinomial(total_count=int(1e8), probs=rna_theta), obs=xs_rna)
            elif self.rna_loss_func == 'bernoulli':
                if self.use_zeroinflate:
                    pyro.sample('rna_x2', dist.ZeroInflatedDistribution(dist.Bernoulli(logits=rna_log_theta),gate_logits=rna_gate_logits).to_event(1), obs=xs_rna)
                else:
                    pyro.sample('rna_x2', dist.Bernoulli(logits=rna_log_theta).to_event(1), obs=xs_rna) 
                    
            ###############################################################
            # rna2atac
            atac_zs = pyro.sample('atac_zns', ZukoToPyro(self.decoder_rna2atac(rna_zns)), obs=atac_zns)
            if self.perturb_size>0:
                atac_zus2 = pyro.sample('atac_zus', ZukoToPyro(self.decoder_atac_effect_from_rna(rna_zus)), obs=atac_zus)
                atac_zs = atac_zs+atac_zus2
            ...
            if self.atac_loss_func == 'negbinomial':
                if self.use_zeroinflate:
                    pyro.sample("atac_x2", MyZINB(mu=atac_mu, theta=atac_dispersion, zi_logits=atac_gate_logits).to_event(1), obs=xs_atac)
                else:
                    pyro.sample("atac_x2", MyNB(mu=atac_mu, theta=atac_dispersion).to_event(1), obs=xs_atac)
            elif self.atac_loss_func == 'poisson':
                if self.use_zeroinflate:
                    pyro.sample('atac_x2', dist.ZeroInflatedDistribution(dist.Poisson(rate=atac_rate),gate_logits=atac_gate_logits).to_event(1), obs=xs_atac.round())
                else:
                    pyro.sample('atac_x2', dist.Poisson(rate=atac_rate).to_event(1), obs=xs_atac.round())
            elif self.atac_loss_func == 'multinomial':
                pyro.sample('atac_x2', dist.Multinomial(total_count=int(1e8), probs=atac_theta), obs=xs_atac)
            elif self.atac_loss_func == 'bernoulli':
                if self.use_zeroinflate:
                    pyro.sample('atac_x2', dist.ZeroInflatedDistribution(dist.Bernoulli(logits=atac_log_theta),gate_logits=atac_gate_logits).to_event(1), obs=xs_atac)
                else:
                    pyro.sample('atac_x2', dist.Bernoulli(logits=atac_log_theta).to_event(1), obs=xs_atac)

    def guide(self, xs_rna, xs_atac, us=None, cs=None):
        with pyro.plate('data'):
            # RNA
    ...
    def _rna_codebook_map(self, xs_rna, soft_assign):
        if soft_assign:
            ns = self._soft_assignments(xs_rna)
        else:
            ns = self._hard_assignments(xs_rna)
        cb = self._get_rna_codebook()
    ...
    def _atac_codebook_map(self, xs_atac, soft_assign):
        if soft_assign:
            ns = self._soft_assignments(xs_atac)
        else:
            ns = self._hard_assignments(xs_atac)
        cb = self._get_rna_codebook()
        zs = torch.matmul(ns, cb)
        return zs
    ...
    def rna_predict(self, xs_rna, perturb, library_sizes=None, show_progress=True):
        ...
        counts = self.get_rna_counts(zs, library_sizes=library_sizes, show_progress=show_progress)
    ...
    def atac_predict(self, xs_atac, perturb, library_sizes=None, show_progress=True):
        ...
        counts = self.get_atac_counts(zs, library_sizes=library_sizes, show_progress=show_progress)
    ...
    def get_atac_cell_shift(self, 
        ...
        #xs = self.preprocess(xs)
        if use_codebook:
            zs = self.rna_codebook_map(xs_atac, soft_assign=soft_assign, show_progress=show_progress)
        else:
    ...
    def fit(self, 
        ...
        use_jax: bool
            ...
            the Python script or Jupyter notebook. It is OK if it is used when runing DensityFlowMO in the shell command.
        show_progress: bool
            verbose on or off
        """
            ...
            for epoch in range(num_epochs):
                ...
                for batch_x_rna, idx in dataloader:
                    ...
                    for loss_id in range(num_losses):
                        if us is None:
                            if cs is None:
                                new_loss = losses[loss_id].step(batch_x_rna, batch_x_atac)
                            else:
                                new_loss = losses[loss_id].step(batch_x_rna, batch_x_atac, cs=batch_c)
                        else:
                            if cs is None:
                                new_loss = losses[loss_id].step(batch_x_rna, batch_x_atac, us=batch_u)
                            else:
                                new_loss = losses[loss_id].step(batch_x_rna, batch_x_atac, batch_u, batch_c)
    ...
    def load_model(cls, file_path):
        ...
        print(f"🧬 DensityFlowMOMO Initialized:")
