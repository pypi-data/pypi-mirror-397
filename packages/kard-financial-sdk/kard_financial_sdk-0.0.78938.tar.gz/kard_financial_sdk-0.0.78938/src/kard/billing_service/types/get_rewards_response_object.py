# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ...billing_commons.types.reward import Reward
from ...commons.types.links import Links
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel


class GetRewardsResponseObject(UniversalBaseModel):
    """
    Examples
    --------
    import datetime
    import uuid

    from kard.billing_commons import Reward, RewardEntry
    from kard.billing_service import GetRewardsResponseObject
    from kard.commons import Links

    GetRewardsResponseObject(
        data=[
            Reward(
                reward_id=uuid.UUID(
                    "abd6e578-d130-44e6-bf4b-8dddb31038b0",
                ),
                transaction_ids=["5e27318c9b346f00087fbb32"],
                issuer_id="6e21318c9b346f12387ffe73",
                merchant_id="6e21318c9b346f12387ffe73",
                offer_id="6e21318c9b346f12387ffe73",
                user_id="6e21318c9b346f12387ffe73",
                transaction_amount=3000,
                transaction_timestamp=datetime.datetime.fromisoformat(
                    "2024-10-01 01:23:45.670000+00:00",
                ),
                issuer_amount=100,
                card_holder_amount=400,
                kard_fees=50,
                created_date=datetime.datetime.fromisoformat(
                    "2024-10-01 00:00:00+00:00",
                ),
                last_modified=datetime.datetime.fromisoformat(
                    "2024-10-02 00:00:00+00:00",
                ),
                entries=[
                    RewardEntry(
                        issuer_amount=150,
                        card_holder_amount=500,
                        kard_fees=50,
                        description="Reward payout for transaction",
                        type="MATCHED",
                        created_at=datetime.datetime.fromisoformat(
                            "2024-10-01 00:00:00+00:00",
                        ),
                    ),
                    RewardEntry(
                        issuer_amount=-50,
                        card_holder_amount=-100,
                        kard_fees=0,
                        description="Value adjusted upon request of issuer",
                        type="ADJUSTMENT",
                        created_at=datetime.datetime.fromisoformat(
                            "2024-10-02 00:00:00+00:00",
                        ),
                    ),
                ],
            )
        ],
        links=Links(
            self_="/billing/rewards?page[size]=50",
            next="/billing/rewards?page[after]=NDMyNzQyODI3OTQw&page[size]=50",
        ),
    )
    """

    data: typing.List[Reward]
    links: Links

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
