...
class Platform:
    ...
    def x_error(self, pauli_index:int, p:float|int):
        if np.random.rand() < p:
            self.x(pauli_index)

    ##  SECTION：----执行pauli_index上的Y-error----
    def y_error(self, pauli_index:int, p:float|int):
        if np.random.rand() < p:
            self.y(pauli_index)

    ##  SECTION：----执行pauli_index上的Z-error----
    def z_error(self, pauli_index:int, p:float|int):
        if np.random.rand() < p:
            self.z(pauli_index)

    ##  SECTION：----执行majorana_index上的U-error----
    def u_error(self, majorana_index:int, p:float|int):
        if np.random.rand() < p:
            self.u(majorana_index)

    ##  SECTION：----执行majorana_index上的V-error----
    def v_error(self, majorana_index:int, p:float|int):
        if np.random.rand() < p:
            self.v(majorana_index)

    ##  SECTION：----执行majorana_index上的N-error----
    def n_error(self, majorana_index:int, p:float|int):
        if np.random.rand() < p:
