...
##  USER：---返回两个线性空间的差集（matrix1 \ cap(matrix1,matrix2)）---
...
def distance(stabilizer_matrix,gauge_matrix=None,method:str='random',search_number:int=200)->int:
    """""
    input.stabilizer_matrix：GF(2)矩阵，稳定子或校验矩阵
    input.gauge_matrix：GF(2)矩阵，无影响
    input.method：'mip' 或 'random'
    input.search_number：随机法搜索次数
    output：int，最小码距估计
    """""

    if method == 'mip':
        logic_op=minus(stabilizer_matrix.null_space(), stabilizer_matrix)
        logic_op=minus(logic_op, gauge_matrix)
        if gauge_matrix is not None:
            return mip_distance_caculator(direct_sum(stabilizer_matrix, gauge_matrix), logic_op)
        else:
            return mip_distance_caculator(stabilizer_matrix, logic_op)
    elif method == 'random':
        return _random_distance_caculator(stabilizer_matrix, gauge_matrix,search_number)
    else:
...
#%%  SECTION：===计算随机距离===
def _random_distance_caculator(stabilizers_matrix, gauge_matrix, number)->int:
    """""
    input.gx：GF(2)矩阵
    input.gz：GF(2)矩阵
    input.num：搜索迭代次数
