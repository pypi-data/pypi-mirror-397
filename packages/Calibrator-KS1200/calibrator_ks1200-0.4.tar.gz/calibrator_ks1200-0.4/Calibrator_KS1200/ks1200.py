...
#   -----------------------------------------------------------------
    ...
    def itm_exchange_worker(self):
        while True:
            ...
            if self.itm_command:
                ...
                self.itm_meas_result = exchange_data_itm(self, 'R,readMeas', self.portname_itm)
                print(f'worker result: {self.itm_meas_result}')
                if self.itm_meas_result.find('Err')>-1:
    ...
    def start_stop(self):
        ...
        exec(f'button = {button_name}.setEnabled(False)')
        ...
        exec(f'button = {button_name}.setEnabled(True)')

    def start_measuring(self):
        ustavka_row_count = eval(f'self.tableWidget_ustavka_{self.measuring_mode}.rowCount()-1')     #т.к. последняя строка с "+"
        if self.measuring_mode == 'auto':
            if ustavka_row_count <= 0:
        ...
        for i in range(8):
            ...
            self.measured_data_counter_channel.append(0)    # добавляем пустой счётчик на канал i
        ...
        while self.db_query(f"SELECT COUNT(*) FROM measurements WHERE id={self.counter_of_measurements}")[0][0]>0: self.counter_of_measurements+=1  # убеждаемся, что номер измерения уникален
        ...
        if ustavka_row_count > 0:                                            # если в таблице уставки есть хоть одна уставка, то обрабатываем её.
            self.ustavka_step = 0
            total_time = 0
            exec(f'self.textEdit_log_{self.measuring_mode}.append("Количество уставок: {str(ustavka_row_count)}")')
            exec(f'self.tableWidget_progress_bar_{self.measuring_mode}.setColumnCount(ustavka_row_count)')
            match self.measuring_mode:
                case 'auto':
                    table_width = 971
                    # time = QtCore.QTime.fromString("00:00:56", "hh:mm:ss").msecsSinceStartOfDay()
                    time = 8
                    total_time = ustavka_row_count * time
                case 'manual':
                    table_width = 921
                    for row in range(ustavka_row_count): total_time += QtCore.QTime.fromString(self.tableWidget_ustavka_manual.item(row,1).text(), "hh:mm").msecsSinceStartOfDay()
            for col in range(ustavka_row_count):
    ...
    def do_measuring_step_body(self):
        ...
        if self.itm_meas_result:
            ...
            for chan in self.itm_request_channels:
                ...
                temp_data[chan-1].append(t)
                r_or_e = extract_val_param(itm_meas_result, "dopch"+str(chan))
                temp_data[chan-1].append(r_or_e)
                ...
                if self.measuring_mode == 'manual':                             # если ручной режим, то рисуем графики
                    ...
                    exec(f"self.label_temp_of_chan_{str(chan)}.setText(str(val))")  # отображаем температуру у кнопок
            ...
            if eval('self.tableWidget_ustavka_' + self.measuring_mode + '.rowCount()')-1 == 0:   # если уставки не заданы, то в ручном режиме сохраняем данные
                if self.measuring_mode == 'manual': self.save_measured_data_to_list(temp_data)
            else:                                                                                # иначе проверяем стабильность
                if self.measuring_mode == 'auto':                               # определяем, какую температуру будем использвать в качестве эталонной
                    current_temp = temp_data[self.etalon_channel][0]
                elif self.itm_request_channels[0]:
                    current_temp = temp_data[self.itm_request_channels[0]-1][0]   # если есть хоть один датчик, то берём температуру с него, иначе оставляем температуру с БУ7
                if self.stab_prev_temp:
                    exec('self.textEdit_log_'+ self.measuring_mode +'.append("Дрейф:'+str(abs(float(current_temp) - float(self.stab_prev_temp)))+'")')
                if abs(float(current_temp) - float(self.stab_prev_temp)) >= 1:  # 0.1 должно быть в релизе          # если мы вышли из зоны стабильности
                    self.stab_time = None
                    self.auto_measuring_step = 0
                    self.timer_ustavka.stop()
                ...
                if self.stab_time == None:
                    ...
                    return
                if abs(QtCore.QTime.currentTime().secsTo(self.stab_time)) < 5: return   # выходим если меньше, 1800 должно быть в релизе, т.е. 30 мин.
                ...
                self.save_measured_data_to_list(temp_data)
                if self.measuring_mode == 'auto':
                    self.itm_send_command("W,stopmeas=1", "W,stopmeas-OK", "Останов измерений в ИТМ", "Невозможно остановить измерения в ИТМ")
                    ...
                    self.itm_send_command("W,startmeas=1", "W,startmeas-OK", "Запуск измерений в ИТМ", "Невозможно запустить измерения в ИТМ")
                    print('Шаг автоматического измерения---------------', self.auto_measuring_step)
                    self.auto_measuring_step += 1
                    if self.auto_measuring_step == 8:
                        self.do_ustavka_step()

    def save_measured_data_to_list(self, temp_data):
        for chan in self.itm_request_channels:
            t = temp_data[chan-1][0]
    ...
    def add_sensor_to_measuring_tab(self, selected_row, selected_col):
        ...
        row_val = eval(table_name + '.item(selected_row,0).text()')
        if row_val == "Перемычка": return
        items = []
        ...
        if row_val != "+":
            cell_val = eval(table_name+'.item(selected_row, selected_col).text()')
            ...
            if selected_col in columns_add_data:
                match input_dialog_mode[selected_col]:
                    case 'getItem':
                        if cell_val: index = input_dialog_items.index(cell_val)
                        else: index = 0
                        cell_val, ok = QInputDialog.getItem(table, dialog_name[selected_col], 'Выберите значение:', input_dialog_items, index, False)
                    case 'getText':
                        if not cell_val: cell_val = default_text[selected_col]
                        cell_val, ok = QInputDialog.getText(table, dialog_name[selected_col], 'Введите новое значение:', QLineEdit.EchoMode.Normal, cell_val)
                    case 'getDouble':
                        if not cell_val: cell_val = default_text[selected_col]
                        cell_val, ok = QInputDialog.getDouble(table, dialog_name[selected_col], 'Введите новое значение:', float(cell_val), 0, 1000, 1)
                if ok:
                    exec(table_name+'.item(selected_row, selected_col).setText(str(cell_val))')
                    match dialog_name[selected_col]:                # обновляем данные в ИТМ
                        case 'Компенсация ХК':
                            self.itm_send_command(f'W,hk{str(selected_row+1)}={str(cell_val)}', f'W,hk{str(selected_row+1)}-{str(cell_val)}', f"Компенсация ХК для канала {selected_row+1} включена", f"Невозможно включить компенсацию ХК для канала {selected_row+1}")
                        case 'R выводов':
                            self.itm_send_command(f'W,Rp{str(selected_row+1)}={str(cell_val)}', f'W,Rp{str(selected_row+1)}-{str(cell_val)}', f"Сопротивление выводов для канала {selected_row+1} установлено в {str(cell_val)}", f"Невозможно установить сопротивление выводов для канала {selected_row+1}")
                return
        ...
        if ok:
            if row_val != "+":
                self.delete_sensor_from_measuring_tab(selected_row, table_name, mode, sh_set_nsh_tp)
            if item == "Удалить": return
            sensor_sn = item.split(", №")[1]
            sensor = self.db_query(f'SELECT * FROM sensors WHERE sn="{sensor_sn}"')
            match table.objectName():
                case "tableWidget_manual_channels_settings":
                    exec("self.pushButton_"+str(selected_row+1)+".setEnabled(True)")        # Разблокируем кнопки для графиков
                    exec("self.pushButton_"+str(selected_row+1)+".setChecked(True)")
                    self.graphWidget.addItem(self.sensor_graph[selected_row])
                    if sh_set_nsh_tp.find(sensor[0][4])>=0:       # Если тип датчика ТП и на первом канале нет перемычки, то ставим её и говорим об этом ИТМ.
                        default_text = {6:'OFF'}
                        if eval(table_name + '.item(0,0).text()') != 'Перемычка':
                            exec(table_name + '.item(0,0).setText("Перемычка")')
                            exec(table_name + '.item(0,0).setFlags(Qt.ItemFlag.NoItemFlags)')
                    elif sh_set_nsh_tr.find(sensor[0][4])>=0:       # если тип датчика ТС, то в шестом столбце будет сопротивление выводов.
                        default_text = {6:'0'}
            ...
            if ish_set[0][0].find(sensor[0][4])>=0:                             # если выбранный датчик является эталонным
                if self.etalon_channel >= 0:                                    # и если эталонный датчик уже существует, то удаляем его.
                    self.delete_sensor_from_measuring_tab(self.etalon_channel, table_name, mode, sh_set_nsh_tp)
                ...
                for coef_num in range(len(coef_set)):
                    coef_data = self.db_query(f'SELECT {coef_set[coef_num]} FROM ish_data WHERE sensor_id="{sensor[0][0]}"')[0][0]
                    if coef_data:
                        exec(f'self.textEdit_log_{mode}.append("{coef_data}")')
                        print(f"W,{coef_set_itm[coef_num]}={str(coef_data)}", self.portname_itm)
                        self.itm_send_command(f"W,{coef_set_itm[coef_num]}={str(coef_data)}", f"W,{coef_set_itm[coef_num]}-OK", f'Коэффициент {coef_set_itm[coef_num]} установлен в значение {str(coef_data)}', f'Невозможно установить коэффициент {coef_set_itm[coef_num]} установлен в значение {str(coef_data)}')
            else:
            ...
            if sh_set_nsh_tr.find(sensor[0][4])>=0:                   # если тип датчика ТС, то пишем в итм Rвыв, иначе XK
                self.itm_send_command(f'W,Rp{str(selected_row+1)}=0.0', f'W,Rp{str(selected_row+1)}-0.0', f"Сопротивление выводов для канала {selected_row+1} установлено в 0", f"Невозможно установить сопротивление выводов для канала 0")
            else:
                self.itm_send_command(f'W,hk{str(selected_row+1)}=OFF', f'W,hk{str(selected_row+1)}-OFF', f'Для канала {selected_row+1} выключена компенсация ХК', f'Невозможно выключить компенсацию ХК для канала {selected_row+1}')
            ...
            if mode == 'manual': self.tableWidget_manual_measuring_result.item(3, selected_row).setText(str(sensor[0][4])) # Пишем тип датчика в таблицу ручного измерения

    def delete_sensor_from_measuring_tab(self, deleting_row, table_name, mode, sh_set_nsh_tp):
        sensor_deleting_sh = eval(table_name + '.item(deleting_row, 3).text()')
        for i in range(eval(table_name+'.columnCount()')): exec(table_name + '.item(deleting_row, i).setText("")')           # сначала очищаем всю строку датчика
        if mode == 'manual':                                            # если измерение ручное, то очищаем таблицу резултатов ручного измерения
            for row in range(6): self.tableWidget_manual_measuring_result.item(row+1, deleting_row).setText('')
        exec(table_name + '.item(deleting_row, 0).setText("+")')
        if self.etalon_channel == deleting_row: self.etalon_channel = -1
        if mode == 'manual':
            exec(f"self.label_temp_of_chan_{deleting_row+1}.setText('-.-')")
            exec("self.pushButton_"+str(deleting_row+1)+".setEnabled(False)")
            exec("self.pushButton_"+str(deleting_row+1)+".setChecked(False)")
            self.graphWidget.removeItem(self.sensor_graph[deleting_row])
            if sh_set_nsh_tp.find(sensor_deleting_sh)>=0: #если удаляемый датчик является ТП, то проверяем последний-ли он и удаляем Перемычку
                is_it_last_TP = True
                for row in range(int(eval(table_name + '.rowCount()'))):
                    sh = eval(table_name+'.item(row, 3).text()')
                    if sh:
                        if sh_set_nsh_tp.find(sh) >= 0 and deleting_row != row: is_it_last_TP = False
                if is_it_last_TP:
                    exec(table_name + '.item(0,0).setText("+")')
                    exec(table_name + '.item(0,0).setFlags(Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEnabled)')
        self.itm_send_command(f"W,sen{str(deleting_row+1)}=non", f"W,sen{str(deleting_row+1)}-OK", f'Для канала {deleting_row+1} установлен тип датчика non', f'Невозможно установить тип датчика non  для канала {deleting_row+1}')
    ...
    def clear_tabs_and_itm(self):
        ...
        self.itm_send_command("W,clearChans=1", "W,clearChans-OK", "Все канали ИТМ очищены", "Ошибка очистки всех каналов ИТМ")
        self.itm_send_command("W,stopmeas=1", "W,stopmeas-OK", "Останов измерений в ИТМ", "Невозможно остановить измерения в ИТМ")
    ...
    def accept_data(self, selected_row, sensor_id):
        ...
        if selected_row >= 0:
            ...
            if self.comboBox_sensor_type.currentText() == "ЭТС" or self.comboBox_sensor_type.currentText() == "ППО" or self.comboBox_sensor_type.currentText() == "ПРО":
                ...
                MainWindow.db_query("INSERT INTO ish_data (id, sensor_id, A, B, C, D, W, Rttb, M, tZn, tAl, tCu, eZn, eAl, eCu, tPd, tPt, ePd, ePt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", "write", data_tuple_ish)
