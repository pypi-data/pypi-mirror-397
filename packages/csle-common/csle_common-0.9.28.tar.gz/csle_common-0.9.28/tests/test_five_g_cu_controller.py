...
class TestFiveGCUControllerSuite:
    """
    Test five_g_cu_controller
    """
    ...
    def test_start_five_g_cus(self, mock_start_five_g_cu, example_emulation_env_config: EmulationEnvConfig) -> None:
        ...
        :return: None
        """
        constants.CONTAINER_IMAGES.FIVE_G_CU_IMAGES = \
            [example_emulation_env_config.containers_config.containers[0].name]
        FiveGCUController.start_five_g_cus(
            ...
            logger=self.logger)
        mock_start_five_g_cu.assert_called_once_with(
            emulation_env_config=example_emulation_env_config,
    ...
    def test_stop_five_g_cus(self, mock_stop_five_g_cu, example_emulation_env_config: EmulationEnvConfig) -> None:
        ...
        constants.CONTAINER_IMAGES.FIVE_G_CU_IMAGES = \
            [example_emulation_env_config.containers_config.containers[0].name]
        FiveGCUController.stop_five_g_cus(
            emulation_env_config=example_emulation_env_config,
            physical_server_ip=example_emulation_env_config.containers_config.containers[0].physical_host_ip,
            logger=self.logger)
        mock_stop_five_g_cu.assert_called_once_with(
            emulation_env_config=example_emulation_env_config,
            ip=example_emulation_env_config.containers_config.containers[0].docker_gw_bridge_ip,
            logger=self.logger)

    @patch("csle_collector.five_g_cu_manager.query_five_g_cu_manager.start_five_g_cu")
    ...
    def test_start_five_g_cu(self, mock_insecure_channel, mock_start_five_g_cu,
        ...
        FiveGCUController.start_five_g_cu(
            emulation_env_config=example_emulation_env_config,
            ip=example_emulation_env_config.containers_config.containers[0].docker_gw_bridge_ip,
            logger=self.logger)
        ...
        mock_start_five_g_cu.assert_called()

    @patch("csle_collector.five_g_cu_manager.query_five_g_cu_manager.stop_five_g_cu")
    @patch("grpc.insecure_channel")
    def test_stop_five_g_cu(self, mock_insecure_channel, mock_stop_five_g_cu,
        ...
        mock_stub = MagicMock()
        mock_channel.__enter__.return_value = mock_stub
        FiveGCUController.stop_five_g_cu(
            emulation_env_config=example_emulation_env_config,
            ip=example_emulation_env_config.containers_config.containers[0].docker_gw_bridge_ip,
            logger=self.logger)
        self.logger.info.assert_called_once_with(
            ...
            f"of emulation: {example_emulation_env_config.name}")
        mock_insecure_channel.assert_called()
        mock_stop_five_g_cu.assert_called()

    @patch("csle_common.controllers.five_g_cu_controller.FiveGCUController.start_five_g_cu_manager")
    def test_start_five_g_cu_managers(self, start_five_g_cu_manager,
        ...
        :return: None
        """
        FiveGCUController.start_five_g_cu_managers(
            emulation_env_config=example_emulation_env_config,
            physical_server_ip=example_emulation_env_config.containers_config.containers[0].physical_host_ip,
            logger=self.logger)
        start_five_g_cu_manager.assert_called()
    ...
    def test_start_five_g_cu_manager(self, mock_sleep, mock_execute_ssh_cmd, mock_connect_admin,
        ...
        mock_execute_ssh_cmd.side_effect = [
            ...
            (b"", b"", 0),  # Output for starting the five_g_cu_manager
        ]
        FiveGCUController.start_five_g_cu_manager(
            emulation_env_config=example_emulation_env_config,
    ...
    def test_stop_five_g_cu_managers(self, stop_five_g_cu_manager,
        ...
        :return: None
        """
        FiveGCUController.stop_five_g_cu_managers(
            emulation_env_config=example_emulation_env_config,
            physical_server_ip=example_emulation_env_config.containers_config.containers[0].physical_host_ip,
            logger=self.logger)
        stop_five_g_cu_manager.assert_called()
    ...
    def test_stop_five_g_cu_manager(self, mock_sleep, mock_execute_ssh_cmd, mock_connect_admin,
        ...
        FiveGCUController.stop_five_g_cu_manager(
            emulation_env_config=example_emulation_env_config,
            ip=example_emulation_env_config.containers_config.containers[0].docker_gw_bridge_ip, logger=self.logger)
        mock_connect_admin.assert_called_once_with(
            emulation_env_config=example_emulation_env_config,
        ...
        mock_execute_ssh_cmd.assert_called()
        self.logger.info.assert_any_call(
            f"Stopping 5G CU manager on node "
            f"{example_emulation_env_config.containers_config.containers[0].docker_gw_bridge_ip}")
    ...
    def test_get_five_g_cu_managers_ips(self, example_emulation_env_config: EmulationEnvConfig) -> None:
        ...
        :return: None
        """
        ips = FiveGCUController.get_five_g_cu_managers_ips(emulation_env_config=example_emulation_env_config)
    ...
    def test_get_five_g_cu_managers_ports(self, example_emulation_env_config: EmulationEnvConfig) -> None:
        ...
        :return: None
        """
        ports = FiveGCUController.get_five_g_cu_managers_ports(emulation_env_config=example_emulation_env_config)
        expected_ports = [50053]
        assert ports == expected_ports

    @patch("csle_common.controllers.five_g_cu_controller.FiveGCUController.get_five_g_cu_managers_ips")
    @patch("csle_common.controllers.five_g_cu_controller.FiveGCUController.get_five_g_cu_managers_ports")
    @patch("csle_common.controllers.five_g_cu_controller.FiveGCUController."
           "get_five_g_cu_status_by_ip_and_port")
    @patch("csle_common.util.emulation_util.EmulationUtil.physical_ip_match")
    def test_get_five_g_cu_managers_info(self, mock_physical_ip_match, mock_get_statuses, mock_get_ports,
                                         mock_get_ips, example_emulation_env_config: EmulationEnvConfig) -> None:
        """
        Test the method that extracts the information of the 5G CU managers for a given emulation
        ...
        mock_get_ports.return_value = [example_emulation_env_config.five_g_config.five_g_cu_manager_port]
        ...
        physical_server_ip = example_emulation_env_config.containers_config.containers[0].physical_host_ip
        FiveGCUController.get_five_g_cu_managers_info(
