...
class TrackioLogger(BaseLogger):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        try:
            ...
            raise ModuleNotFoundError(
                "This contrib module requires trackio to be installed. "
                "You man install trackio with the command:\n pip install trackio\n"
            ) from err
        if kwargs.get("init", True):
            kwargs.pop("init", None)
            self._trackio.init(*args, **kwargs)

    def __getattr__(self, attr: Any) -> Any:
        return getattr(self._trackio, attr)
...
class OutputHandler(BaseOutputHandler):
    ...
    def __call__(self, engine: Engine, logger: TrackioLogger, event_name: str | Events) -> None:
        if not isinstance(logger, TrackioLogger):
            raise TypeError(f"Handler '{self.__class__.__name__}' works only with TrackioLogger.")
...
class OptimizerParamsHandler(BaseOptimizerParamsHandler):
    ...
    def __call__(self, engine: Engine, logger: TrackioLogger, event_name: str | Events) -> None:
        if not isinstance(logger, TrackioLogger):
            raise TypeError("Handler OptimizerParamsHandler works only with TrackioLogger")
