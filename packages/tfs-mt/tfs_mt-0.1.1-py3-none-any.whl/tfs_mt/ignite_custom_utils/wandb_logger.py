...
__all__ = ["OptimizerParamsHandler", "OutputHandler", "WandBLogger", "global_step_from_engine"]
...
class OutputHandler(BaseOutputHandler):
    ...
    def __call__(self, engine: Engine, logger: WandBLogger, event_name: str | Events) -> None:
        if not isinstance(logger, WandBLogger):
            raise TypeError(f"Handler '{self.__class__.__name__}' works only with WandBLogger.")
...
class OptimizerParamsHandler(BaseOptimizerParamsHandler):
    ...
    def __call__(self, engine: Engine, logger: WandBLogger, event_name: str | Events) -> None:
        if not isinstance(logger, WandBLogger):
            raise TypeError("Handler OptimizerParamsHandler works only with WandBLogger")
