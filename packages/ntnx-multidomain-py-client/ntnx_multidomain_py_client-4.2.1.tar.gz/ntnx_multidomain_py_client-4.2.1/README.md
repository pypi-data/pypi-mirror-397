# Python Client For Nutanix Multidomain Versioned APIs

The Python client for Nutanix Multidomain Versioned APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage all Nutanix services running on the Nutanix Cloud Platform (NCP) deployed in on-prem data centers ,Nutanix Cloud Clusters (NC2) or edge sites.
## Features
...
api_client = ntnx_multidomain_py_client.ApiClient(allow_version_negotiation=False)
```
...
```sh
<my-env>/bin/pip install ntnx-multidomain-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-multidomain-py-client
```
...
## Configuration
The python client for Nutanix Multidomain Versioned APIs can be configured with the following parameters
...
```python
# Initialize the API
external_repositories_api_instance = ExternalRepositoriesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get an External repository by id.
try:
    api_response = external_repositories_api_instance.get_external_repository_by_id(extId)
...
try:
    ...
    api_response = external_repositories_api_instance.update_external_repository_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
try:
    api_response = external_repositories_api_instance.list_external_repositories(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=multidomain&version=v4.2&language=python). This documentation is auto-generated, and the location may change.
