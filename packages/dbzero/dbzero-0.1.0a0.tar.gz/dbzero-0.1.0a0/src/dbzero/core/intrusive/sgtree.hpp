...
struct alpha_0_75_by_max_size_t
{
   ...
   uint32_t operator()(uint32_t max_tree_size) const
   {
      const uint32_t max_tree_size_limit = (uint32_t) ((~std::size_t(0))/std::size_t(3));
...
struct h_alpha_t
{
   ...
   std::size_t operator()(std::size_t n) const
   {
      ...
      // floor(m_log(n)/m_log(1/alpha)) ->
      // floor(m_log(n)/(-m_log(alpha)))
      //return static_cast<std::size_t>(std::m_log(float(n))*inv_minus_logalpha_);
   ...
   //calls to m_log and division.
   //This will store 1/(-std::m_log(alpha_))
...
struct alpha_by_max_size_t
{
   ...
   float operator()(uint32_t max_tree_size) const
...
struct alpha_holder
{
   typedef intrusive::detail::h_alpha_t h_alpha_t;
   typedef intrusive::detail::alpha_by_max_size_t multiply_by_alpha_t;
...
struct alpha_holder<false>
{
   ...
   typedef intrusive::detail::h_alpha_sqrt2_t           h_alpha_t;
   typedef intrusive::detail::alpha_0_75_by_max_size_t  multiply_by_alpha_t;
...
class sgtree_impl
{
   ...
   static_assert(((int)real_value_traits::link_mode != (int)auto_unlink));
   ...
   struct Initializer : public sgtree_impl::value_traits
   {
      typedef typename sgtree_impl::value_traits value_traits;
      Initializer(const value_compare & comp, const value_traits &val_traits)
   ...
   iterator insert_equal(reference value)
   {
      ...
      if(safemode_or_autounlink)
         BOOST_VSO_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::unique(to_insert));
   ...
   iterator erase(iterator i)
   {
      ...
      if(safemode_or_autounlink)
         BOOST_VSO_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(!node_algorithms::unique(to_erase));
   ...
   {
      assert((new_alpha > 0.5f && new_alpha < 1.0f));
...
/*
   ...
   static void remove_node(T& value)
   {
      ...
      //time lists. 
      //static_assert((!(safemode_or_autounlink && constant_time_size)));
      static_assert((!constant_time_size));
      static_assert((boost::is_convertible<T, value_type>::value));
...
*/
   ...
   static sgtree_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
   {
      ...
      Initializer *d = detail::parent_from_member<Initializer, node_plus_pred_t>(n, &Initializer::node_plus_pred_);
      sgtree_impl *scapegoat  = detail::parent_from_member<sgtree_impl, Initializer>(d, &sgtree_impl::data_);
      return *scapegoat;
   }
};
...
#ifndef BOOST_VSO_INTRUSIVE_DOXYGEN_INVOKED
   ...
   :  public make_sgtree<T, O1, O2, O3, O4>::type
{
   ...
   static_assert((detail::is_same<typename real_value_traits::value_type, T>::value));
