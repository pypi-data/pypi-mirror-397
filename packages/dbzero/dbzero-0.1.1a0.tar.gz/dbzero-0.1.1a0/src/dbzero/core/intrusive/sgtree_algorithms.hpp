...
class sgtree_algorithms
{
   ...
   typedef typename tree_algorithms::link_data	link_data;
   ...
   {
	   return tree_algorithms::get_header(node);
   }
   ...
   //! 
   //! <b>Throws</b>: Nothing.
   static std::size_t count(const node_ptr &node)
      ...
      if (tree_size != 1 && depth > h_alpha(tree_size))
	   {
		 ...
		 {
			// scapegoat node reached ( need rebalance )
			if ((i!=1) && (i > h_alpha(size)))
			{
				rebalance_subtree(s);
				break;
			}
   ...
   static node_ptr upper_equal_bound
      (const node_ptr &header, const KeyType &key, KeyNodePtrCompare comp)
   {  return tree_algorithms::upper_equal_bound(header, key, comp); }

   template<class KeyType, class KeyNodePtrCompare>
   static node_ptr lower_equal_bound
      (const node_ptr &header, const KeyType &key, KeyNodePtrCompare comp)
   {  return tree_algorithms::lower_equal_bound(header, key, comp);  }
   ...
   static node_ptr insert_equal
      (const node_ptr &header, const node_ptr &hint, node_ptr &new_node, NodePtrCompare comp
   ...
   template<class KeyType, class KeyNodePtrCompare>
   static void link_equal
      (const node_ptr &header, const node_ptr &hint, const KeyType &key, KeyNodePtrCompare comp,link_data &ld, size_t &depth)
   {      
	  tree_algorithms::link_equal (header, hint, key, comp, ld, &depth);
   }
   ...
   static std::pair<node_ptr, bool> insert_unique_check
      (const node_ptr &header,  const KeyType &key,KeyNodePtrCompare comp, insert_commit_data &commit_data)
   {
      std::size_t depth;
   ...
   template<class KeyType, class KeyNodePtrCompare>
   static std::pair<node_ptr, bool> insert_unique_check
      (const node_ptr &header,  const node_ptr &hint, const KeyType &key,KeyNodePtrCompare comp, 
