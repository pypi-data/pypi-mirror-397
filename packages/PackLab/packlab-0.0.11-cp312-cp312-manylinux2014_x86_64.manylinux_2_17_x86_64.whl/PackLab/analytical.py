...
class PercusYevickSolver:
    ...
    def get_Cpy(self, epsilons: List[Any]) -> Any:
        """Compute the full Cpy tensor with shape (N, N, P) without Python loops."""
        densities = self.densities            # shape (N,)
        radii = self.radii                    # shape (N,)
        p = self.p                            # shape (P,)

        e_0, e_1, e_2, e_3 = epsilons
        denominator = 1 - e_3

        number_of_species = radii.size
        number_of_p_samples = p.size

        # Species dependent diameter like quantity, shape (N,)
        R_species = 2 * radii

        # Build X for all species, shape (N, P)
        X = (R_species[:, None] * p[None, :] / 2).to("dimensionless")

        sin_over_X = self._safe_sin_over_x(X)  # array with your returned units logic
        sin_over_X3_minus_cos_over_X2 = self._safe_sin_over_x3_minus_cos_over_x2(X)

        # Shape (N, P)
        N = (R_species[:, None] ** 2) * sin_over_X
        M = 3 * (R_species[:, None] ** 3) * sin_over_X3_minus_cos_over_X2

        # Broadcast to pair space, shapes (N, N, P) except term0 which is (N, N, 1)
        X_i = X[:, None, :]                    # (N, 1, P)
        X_j = X[None, :, :]                    # (1, N, P)
        ...
        R_i = R_species[:, None, None]         # (N, 1, 1)
        R_j = R_species[None, :, None]         # (1, N, 1)

        n_i = densities[:, None, None]         # (N, 1, 1)
        n_j = densities[None, :, None]         # (1, N, 1)

        p_grid = p[None, None, :]              # (1, 1, P)

        # Trig terms. X is already dimensionless, so numpy trig is fine on Pint quantities.
        cos_X_i = cos(X_i)
        sin_X_i = sin(X_i)
        ...
        term0 = -(pi / 6) * np.sqrt(n_i * n_j) / denominator              # (N, N, 1)

        bracket_i = (
            cos_X_i
            + X_i * sin_X_i
            + 3 * e_2 * R_i * cos_X_i / denominator
            + 3 * e_1 * N_i / denominator
            + 9 * e_2 ** 2 * N_i / denominator ** 2
        )
        ...
        bracket_j = (
            cos_X_j
            + X_j * sin_X_j
            + 3 * e_2 * R_j * cos_X_j / denominator
            + 3 * e_1 * N_j / denominator
            + 9 * e_2 ** 2 * N_j / denominator ** 2
        )

        term2 = M_j * bracket_i                                           # (N, N, P)
        term3 = M_i * bracket_j                                           # (N, N, P)

        bracket_4 = (
            e_0 / denominator
            + p_grid ** 2 * e_2 / (4 * denominator)
            + 6 * e_1 * e_2 / denominator ** 2
            + 9 * e_2 ** 3 / denominator ** 3
        )

        term4 = (M_i * M_j) * bracket_4                                    # (N, N, P)

        # Keep your exact term5 and term6 forms
        term5 = 3 * N_i * R_j * cos_X_j                                    # (N, N, P)
        term6 = 3 * N_j * R_i * cos_X_i                                    # (N, N, P)

        term7 = 9 * e_2 * N_i * N_j / denominator                          # (N, N, P)

        Cpy = term0 * (term2 + term3 + term4 + term5 + term6 + term7)      # (N, N, P)

        return Cpy.to("dimensionless")

    def get_Cpy_(self, index_i: int, index_j: int, epsilons: List[Any]) -> Any:
        """Compute one curve Cpy[index_i, index_j, :] by slicing the vectorized tensor."""
        Cpy = self.get_Cpy(epsilons=epsilons)
        return Cpy[index_i, index_j, :]