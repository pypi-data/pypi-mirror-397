...
from uniception.models.libs.perception_encoder.vision_encoder.config import (
    PE_TEXT_CONFIG,
    PE_VISION_CONFIG,
    PEConfig,
    PETextConfig,
...
from uniception.models.libs.perception_encoder.vision_encoder.rope import Rope2D
...
class Transformer(nn.Module):
    ...
    def forward_intermediate(
        ...
        x: torch.Tensor,
        attn_mask: Optional[torch.Tensor] = None,
        intermediate_layers: Union[int, List[int]] = 1,  # return the last n layers
    ):
        """
        ...
        Args:
            ...
            intermediate_layers (Union[int, List[int]]): Number of layers to return intermediate outputs from.
                If an integer is provided, it returns the last n layers. If a list is provided, it returns those specific layers.
                Note: the index in the list format counts from the first layer, not the last.
        """
        intermediate_outputs = []

        if isinstance(intermediate_layers, int):
            intermediate_layers = [len(self.resblocks) - intermediate_layers]
...
class VisionTransformer(nn.Module):
    ...
    def load_ckpt(self, ckpt_path: str):
        ...
        m, u = self.load_state_dict(_sd, strict=True)
    ...
    @classmethod
    def from_config(cls, name: str, pretrained: bool = False, checkpoint_path: Optional[str] = None, **kwdargs):
    ...
    def get_intermediate_layers(
        self,
        x: torch.Tensor,
        n: Union[int, List[int]] = 1,  # return the last n layers or specific layers
        norm: bool = True,
        ...
        x = self.ln_pre(x)
        intermediate_x = self.transformer.forward_intermediate(x, intermediate_layers=n)

        if norm:
            intermediate_x = [self.ln_post(x) for x in intermediate_x]

        if strip_cls_token and self.use_cls_token:
            intermediate_x = [x[:, 1:, :] for x in intermediate_x]
...
class CLIP(TextTransformer):
    def __init__(self, vision_cfg: PEConfig, text_cfg: PETextConfig, init_logit_scale: float = np.log(1 / 0.07)):
    ...
    @classmethod
    def from_config(
        cls, name: str, pretrained: bool = False, checkpoint_path: Optional[str] = None  # To load your own
    ):
