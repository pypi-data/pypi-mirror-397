...
def main(
    ...
    typer.echo(f"\n💡 Next steps:")
    typer.echo(f"   1. import {route_name}_routes and use the routes in main.py (example : all_routes = list(itertools.chain(routes, api_routes,product_routes)))")
    typer.echo(f"   2. Execute python -m cli.migrations --refresh or python3 -m cli.migrations --refresh")
    typer.echo(f"   3. Review and customize app/routes/{route_name}.py")
    typer.echo(f"   4. Uncomment Pydantic validation if needed")
    typer.echo(f"   5. Customize the HTML template if needed")
    typer.echo(f"   6. Restart your server to see the changes")
