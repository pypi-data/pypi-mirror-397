...
class LatexScanner:
    ...
    def extract_formulas(self, content: str) -> list[Formula]:
        """Extract all LaTeX formulas from document content."""
        formulas = []

        # Track line numbers for formulas
    ...
    def scan(self, content: str) -> list[FormulaIssue]:
        ...
        unclosed_issues = self._check_unclosed_formulas(content)
        issues.extend(unclosed_issues)

        # Check for nested delimiters in raw content
        nested_issues = self._check_nested_delimiters(content)
        issues.extend(nested_issues)

        # Check for double pipe errors in raw content
        pipe_issues = self._check_double_pipe_errors(content)
    ...
    def _check_unclosed_formulas(self, content: str) -> list[FormulaIssue]:
        ...
        if block_count % 2 != 0:
            ...
            if positions:
                ...
                line_num = content[:last_pos].count("\n") + 1
        ...
        if inline_count % 2 != 0:
            ...
            if in_formula and formula_start >= 0:
                line_num = content_no_block[:formula_start].count("\n") + 1
                issues.append(FormulaIssue(
                    formula=Formula(
                        content="",
                        line_number=line_num,
    ...
    def _check_nested_delimiters(self, content: str) -> list[FormulaIssue]:
    ...
    def _check_double_pipe_errors(self, content: str) -> list[FormulaIssue]:
