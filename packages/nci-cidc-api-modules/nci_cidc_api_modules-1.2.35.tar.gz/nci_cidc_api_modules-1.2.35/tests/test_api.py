...
def test_resource_and_item_get(resource, config, cidc_api, clean_db, monkeypatch):
    ...
    if "GET" in config["allowed_methods"]:
        ...
        # With the admin user, there's a chance that the first user is not the user we want.
        # This line ensures we get the sample user and not the admin user
        if resource == "users":
            item = next((user for user in response.json["_items"] if user["email"] == "test-admin@example.com"), None)
