...
class IRISGraphOperators:
    ...
    def kg_KNN_VEC(self, query_vector: str, k: int = 50, label_filter: Optional[str] = None) -> List[Tuple[str, float]]:
        """
        K-Nearest Neighbors vector search using native IRIS vector functions

        Now uses optimized VECTOR table with HNSW index for 1790x performance improvement.
    ...
    def _kg_KNN_VEC_python_optimized(self, query_vector: str, k: int = 50, label_filter: Optional[str] = None) -> List[Tuple[str, float]]:
        """
        Optimized Python implementation using the fast CSV parsing approach
        ...
        try:
            ...
            logger.error(f"Optimized Python kg_KNN_VEC failed: {e}")
            raise
        finally:
            cursor.close()

    def _kg_KNN_VEC_fallback(self, query_vector: str, k: int = 50, label_filter: Optional[str] = None) -> List[Tuple[str, float]]:
        """
        Fallback Python implementation for debugging when native IRIS functions fail
        """
        cursor = self.conn.cursor()
        try:
            # Parse query vector from JSON string
            ...
            # Get embeddings with optional label filter
            if label_filter is None:
                sql = """
                    SELECT n.id, n.emb
                    FROM kg_NodeEmbeddings n
            ...
            similarities = []
            for entity_id, emb_csv in cursor.fetchall():
                try:
                    ...
                    logger.warning(f"Failed to process embedding for {entity_id}: {emb_error}")
                    continue
            ...
            logger.error(f"Fallback kg_KNN_VEC failed: {e}")
            return []
        finally:
            cursor.close()
    ...
    def kg_RRF_FUSE(self, k: int = 50, k1: int = 200, k2: int = 200, c: int = 60,
                    query_vector: str = None, query_text: str = None) -> List[Tuple[str, float, float, float]]:
        """
