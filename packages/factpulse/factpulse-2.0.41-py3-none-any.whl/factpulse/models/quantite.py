...
QUANTITE_ANY_OF_SCHEMAS = ["float", "str"]

class Quantite(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Quantite.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Quantite with anyOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Quantite with anyOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
