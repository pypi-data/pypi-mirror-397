...
class LinearLabelManager:
    ...
    async def setup_issue_labels(self, team_id: str) -> tuple[int, int]:
        ...
        # Check team-specific labels first, then workspace-wide
        claude_info = existing.get("Claude")
        if not claude_info:
            workspace_labels = await self._get_workspace_issue_labels()
            claude_info = workspace_labels.get("Claude")
        ...
        if not claude_info:
            ...
            if was_created and label:
                ...
                if claude_info:
                    ...
                    raise RuntimeError("Failed to create or find 'Claude' label")
        else:
            claude_group_id = claude_info["id"]
        ...
        for label_name in ISSUE_LABELS_CLAUDE_GROUP:
            ...
            _, was_created = await self._try_create_issue_label(
                team_id, label_name, color, claude_group_id
            )
            if was_created:
    ...
    async def _try_create_project_label(
