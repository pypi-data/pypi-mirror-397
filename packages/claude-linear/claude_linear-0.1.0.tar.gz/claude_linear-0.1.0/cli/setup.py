...
async def select_team(manager: LinearLabelManager) -> dict[str, Any]:
    """Prompt user to select a Linear team.

    Args:
        manager: Initialized LinearLabelManager

    Returns:
        Selected team dict with 'id', 'key', 'name'
    """
    ...
    options = [(t["id"], f"{t['key']}: {t['name']}") for t in teams]
    team_id = prompt_choice("Select Linear team for labels:", options)
...
async def run_setup(
    ...
    async with LinearLabelManager(api_key) as manager:
        team = await select_team(manager)
        config["DEFAULT_LINEAR_TEAM_ID"] = team["id"]

        if not skip_labels:
            print_info("Creating issue labels...")
            ...
            total_created = created_issue + created_project
            total_existing = existing_issue + existing_project
