...
def _row_to_event_trigger(row: EventTriggerRow) -> EventTrigger:
    ...
    return EventTrigger(
        id=int(et_id),
        eventtype_id=int(eventtype_id),
        should_log=bool(should_log) if should_log is not None else None,
...
def fetch_event_trigger(event_trigger_id: int) -> EventTrigger:
    """
...
def fetch_event_triggers_for_eventtype(eventtype_id: int) -> List[EventTrigger]:
    """
