...
This module provides functions for generating and managing standardized USGS gauge data catalogs
for HEC-RAS projects. Creates a "USGS Gauge Data" folder structure similar to the precipitation
module's storm catalog pattern.
...
def load_gauge_data(
    ...
    Example
    -------
    >>> flow_data = load_gauge_data('01547200', parameter='flow')
...
def get_gauge_folder(
    ...
    Example
    -------
    >>> folder = get_gauge_folder('01547200')
...
def update_gauge_catalog(
    ...
    Example
    -------
    >>> summary = update_gauge_catalog()
    >>> print(f"Updated {summary['gauges_updated']} gauges")
    """
    start_time = datetime.now()

    # Load existing catalog
    catalog_df = load_gauge_catalog(ras_object=ras_object, catalog_folder=catalog_folder)
