...
class MotorsManager:
        ...
        for motor_name in motor_names:
            ...
            if motor.can_interface is None:
                for can_interface in get_all_physical_can_interfaces():
                    ...
                    raise MotorNotFoundError(f"Warning: Could not find motor {motor_name} on any physical CAN interface", motor_name_or_id=motor_name)
    ...
    async def read_current_state_async(self, motor_name_or_id: Union[str, int]):
        ...
        Note:
            ...
            But if we start using active reporting, we need to revisit this.
        """
        motor = self.get_motor(motor_name_or_id)
    ...
    def get_range(
        ...
        lower_limit = self._find_limit(
            ...
            step_time=step_time,
            max_torque=max_torque,
            threshold_target_distance=expected_range * 2,
