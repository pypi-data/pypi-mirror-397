...
PITCH_OFFSET = -2 * math.pi / 180  # 2 degrees offset for pitch motor
...
class RobstrideNeckPlatform(NeckPlatform):
    ...
    def __init__(self, motors_manager: MotorsManager, config: dict):
        ...
        if (
            ...
            print("Neck recovering from power cycle")
    ...
    def sleep_and_disable(self):
        ...
        try:
            ...
            if self.healthy_state and not self.recalibrating and self.enabled and in_range:
                ...
                for motor_name in self.motors.keys():
                    print(f"Setting movement profile for {motor_name} to {self.motor_vel_max[motor_name]} and {self.motor_acc_set[motor_name]}")
    ...
    def recalibrate(self, verbose: bool = False):
        ...
        try:
            ...
            for motor_name in [self.PITCH_MOTOR_NAME, self.YAW_MOTOR_NAME]:
                # recalibrate with default vel_max and acc_set
                self.motors_manager.write_param(motor_name, "vel_max", self.motor_vel_max[motor_name])
                self.motors_manager.write_param(motor_name, "acc_set", self.motor_acc_set[motor_name])
