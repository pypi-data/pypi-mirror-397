...
class FeatherRobot(Robot):
    def __init__(self, robot_name: str, config: dict = {}):
        ...
        # TODO: Why do we allow no enable can? Isn't it better to raise an error if the CAN interface is not enabled?
        # Remove the comment after discussing with the team and updating the code if needed.
        ...
        try:
            if "navigation_platform" in self.cfg:
                ...
                if "torso" in self.cfg:
                    self.torso = EZMotionTorsoPlatform(self.motors_manager, base_can_iface, self.cfg["torso"])
                else:
                    self.torso = None
            else:
        ...
        if "neck" in self.cfg:
            self.neck = RobstrideNeckPlatform(self.motors_manager, self.cfg["neck"])
        else:
            self.neck = None
