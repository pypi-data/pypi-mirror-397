...
_LIB_COMPILE_TIMEOUT_SECONDS = 3.0
...
def __compile_load_lib() -> None:
    ...
    try:
        with FileLock(_LIB_LOCK, timeout=_LIB_COMPILE_TIMEOUT_SECONDS):
            ...
            if not os.path.exists(_LIB_SO):
                cmd = ["gcc", "-fvisibility=hidden", "-shared", "-O3", "-march=native", "-flto", "-fPIC", "-o", _LIB_SO, _LIB_SRC]
        ...
        while time.time() - start_time < _LIB_COMPILE_TIMEOUT_SECONDS:
            ...
            raise Exception(f"Library {_LIB_SO} was not compiled within {max_wait} seconds")
