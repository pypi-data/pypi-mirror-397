...
from .system import is_can_interface, enable_can_interface
...
class CommsManager:
    ...
    def start(self, endpoints: list[str], allow_no_enable_can: bool = False) -> None:
        ...
        Args:
            ...
            allow_no_enable_can: If True, will not raise an error if the CAN interface is not enabled.
        """
        with self._lock:
            if self.is_running():
                ...
                if is_can_interface(ep):
                    try:
                        ...
                        if not allow_no_enable_can:
                            ...
                            # TODO: Shouldn't we return here after the print so that we don't end up starting the
                            # thread below and thus moving the comms manager to a running state?
                            print(f"Warning: Could not enable CAN interface {ep}: {e}")
    ...
    def close(self) -> None:
        ...
        with self._lock:
            ...
            _stop_polling()
            time.sleep((_POLL_TIMEOUT_MS / 1000.0) * 1.1)  # Give it a bit of extra time to stop polling
