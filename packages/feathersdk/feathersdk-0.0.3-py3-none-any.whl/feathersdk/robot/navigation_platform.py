...
class VelocityNavigationPlanner(NavigationPlanner):
    ...
    follow _handle_command() pattern in existing methods to ensure safety and error handling.
    """
    ...
    def __init__(
        ...
        self._delta_acceleration_hz = self._max_jerk / self.operating_frequency
        self.current_velocity = 0
        self.current_angle = {"BwC": 0, "BwR": 0, "BwL": 0}
        self.current_spd_ref = {"BpC": 0, "BpR": 0, "BpL": 0}
        self.current_acceleration = 0
        self._last_drive_key = time.time()
        self._last_turn_key = time.time()
        self._is_rotating = False
        self._last_rotate_key = time.time()
    ...
    def _handle_command(self, command):
        """
        Execute a command and catch safety or motor-related exceptions, reraising them with meaningful error messages.
        
        Args:
            command: A callable that executes the navigation command
        ...
        if not self._enabled:
            raise Exception("Error: Navigation platform is not enabled. Try calling enable_motors method on NavigationPlanner again or try a power cycle on the Robot.")
        ...
        if state.value != CalibrationState.HEALTHY.value:
            if state.value == CalibrationState.UNCALIBRATED.value:
                raise Exception("Error: Turning motors are not calibrated. Please calibrate with recalibrate() method.")
            elif state.value == CalibrationState.OUT_OF_RANGE.value:
                raise Exception("Error: Turning motors are out of valid range. Please recalibrate with recalibrate() method.")
            elif state.value == CalibrationState.CALIBRATION_FAILED.value:
                raise Exception("Error: Previous calibration attempt failed. Please recalibrate with recalibrate() method.")
            else:
                raise Exception(f"Error: Navigation platform is not ready (state: {state.value}). Please recalibrate with recalibrate() method.")
        ...
        except MotorModeInconsistentError as e:
            raise NavigationMotorError(
    ...
    def _set_run_mode_and_enable(self):
        try:
            for motor_name in self._nav_platform.TURNING_MOTORS:
                ...
                self._motor_manager.write_param(motor_name, "limit_spd", math.pi / 2) # for safety right now.
        ...
        try:
            ...
            for motor_name in self._nav_platform.ALL_MOTORS:
                if self._motor_manager.motors[motor_name].mode[0] != MotorMode.Run:
                    ...
                    print (f"Motor {motor_name} is not in run mode after waiting for 0.1 seconds, navigation platform will be disabled, try calling enable_motors method on NavigationPlanner again or try a power cycle on the Robot")
    ...
    def orient_rotate(self):
        def _execute_orient_rotate():
            ...
            for motor_name, angle in self._rotation_target_angles.items():
                self._motor_manager.set_target_position(motor_name, self._motor_manager.motors[motor_name].middle_pos + angle)
                self.current_angle[motor_name] = angle
        
        self._handle_command(_execute_orient_rotate)
    ...
    def rotate_in_place(self, speed):
        ...
        def _execute_rotate():
            ...
            for motor_name in self._nav_platform.SPINNING_MOTORS:
                self._motor_manager.set_target_velocity(motor_name, target)
                self.current_velocity = speed
        
        self._handle_command(_execute_rotate)
    ...
    def _orient_angle(self, degrees):
        ...
        for motor_name in self._nav_platform.TURNING_MOTORS:
            ...
            self._motor_manager.set_target_position(motor_name, motor.middle_pos + target_angle)
            self.current_angle[motor_name] = target_angle

    def _set_speed(self, speed):
        # Calculate degrees per second: speed (m/s) / circumference (m) * 360 degrees
        degrees = speed / self._WHEEL_CIRCUMFERENCE_M * 360
        target = self.degrees_to_radians(degrees)
        self.current_velocity = speed
        for motor_name in self._nav_platform.SPINNING_MOTORS:
            self._motor_manager.set_target_velocity(motor_name, target * self._motor_manager.motors[motor_name].direction)
    ...
    def go_to(self, degrees, speed, motors_is_on=False):
        ...
        self._handle_command(_execute_go_to)
    ...
    def orient_angle(self, degrees):
        ...
        self._handle_command(_execute_orient)
    ...
    def set_speed(self, speed):
        ...
        self._handle_command(_execute_set_speed)
...
class ThreeWheelServeDrivePlatform(SteppableSystem):
    ...
    def __init__(self, motors_manager: MotorsManager, config: dict):
        ...
        self._validate_calibration_state()
        self.nav_planner._set_run_mode_and_enable()

    def on_abort(self):
        self.nav_planner._disable()
    ...
    def _load_calibration_data(self, calibrate_if_not_found=False):
        ...
        for motor_name in self.TURNING_MOTORS:
            ...
            if not motor.load_calibration_state():
                if calibrate_if_not_found:
                    self._recalibrate_private(motor_name)
                else:
                    # If any motor fails to load calibration, mark as uncalibrated
                    self.calibration_state = CalibrationState.UNCALIBRATED
    
    def _validate_calibration_state(self):
        """
        Validate the calibration state by checking if motors have calibration data and are in range.
        This should be called after _load_calibration_data() to ensure state is accurate.
        """
        try:
            # Check if motors have calibration data and are in range
            if all(
                self.motors_map[motor_name].calibration_time is not None
                and self.motors_map[motor_name].can_interface is not None
                for motor_name in self.TURNING_MOTORS
            ):
                for motor_name in self.TURNING_MOTORS:
                    self.motors_manager.recover_from_power_cycle(motor_name)
                # Check if motors are in valid range
                if self.motors_manager.check_motors_in_range(
                    self.TURNING_MOTORS, raise_error=False
                ):
            ...
            print(f"Warning: Could not check turning motors health: {e}")
            self.calibration_state = CalibrationState.UNCALIBRATED
    ...
    def _recalibrate_private(self, motor_name=None, verbose=False):
        ...
        Else recalibrate all the motors.
        """
