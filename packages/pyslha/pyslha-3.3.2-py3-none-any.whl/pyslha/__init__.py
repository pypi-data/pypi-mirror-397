...
__author__ = "Andy Buckley <andy.buckley@glasgow.ac.uk>"
__version__ = "3.3.2"

## Python version: require >= 3.6 (cf. TOML config)
import sys
if sys.version_info < (3,6):
    raise Exception("pyslha requires Python >= 3.6")
...
def _autotuple(a):
    ...
    if type(a) is str or not hasattr(a, "__iter__"):
        return a
    if len(a) == 1:
        return a[0]
    return tuple(a)
...
class Block(object):
    ...
    def set_value(self, *args):
        ...
        If there is only one arg, or longer list that does not start
        with an int, this will be treated as setting the value of a
        single-valued (None-indexed) block. In this case the block
        must already contain at most one None-indexed value (i.e. be
        empty or single-valued) otherwise an AccessError exception
        will be raised.
        """
        if not args:
            raise AccessError("set_value() called without arguments")

        ## If only one argument, treat it as a None-indexed unique-value block if possible.
        ## Otherwise split into an integer-tuple key and scalar/tuple value: the index
        ## tuple is the longest possible set of consecutive ints other than the final arg;
        ## if there are no leading ints, a None-index will be attempted.
        if len(args) == 1:
            idx = None
            val = args[0]
        else:
            ...
            for i, x in enumerate(args):
                if type(x) is not int: #< TODO: actually check for general int-likeness?
                    i_first_nonint = i
                    break
            ## Get the index tuple/scalar/None and value tuple/scalar
            idx = _autotuple(args[:i_first_nonint]) if i_first_nonint != 0 else None
            val = _autotuple(args[i_first_nonint:])

        ## Set the entry if possible
        if idx is None and len(self.entries) > 0 and not self.is_single_valued():
            raise AccessError("Tried to set a unique value (no index) on multi-value block " + self.name)
...
class XSec(object):
    ...
    TODO:
     - Link to a definition of the XSEC block
     - Extend/systematise the QCD and EW order codes, to cover higher fixed _and_ resummation orders
    """
    ...
    def qcd_order_str(self):
        return { 0 : "Born", 1 : "NLO", 2 : "NLO+LL"}.get(self.qcd_order, str(self.qcd_order)) \
            if self.qcd_order is not None else ""

    @property
    def ew_order_str(self):
        return { 0 : "Born", 1 : "NLO", 2 : "NLO+LL"}.get(self.ew_order, str(self.ew_order)) \
            if self.ew_order is not None else ""
...
def writeSLHADecays(decays, ignorenobr=False, precision=8):
    ...
    for pid, particle in decays.items():
        width = particle.totalwidth if particle.totalwidth is not None else -1
        blockstr = ("DECAY %d " % particle.pid) + _autostr(width, precision) + "\n"
        decaystrs = []
