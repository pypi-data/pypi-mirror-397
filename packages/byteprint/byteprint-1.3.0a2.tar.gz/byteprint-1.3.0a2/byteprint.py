...
from typing import Any as _Any, List as _List, Union as _Union, Callable as _Callable, Tuple as _Tuple, Dict as _Dict, cast as _cast
from warnings import warn as _warn
from types import FunctionType as _Function, ModuleType as _Lib
from traceback import print_exc as _perr
from sys import version as _ver, platform as _plat, version_info as _vin, executable as _ee
...
def upgrade(lib: str = "byteprint") -> None:
    ...
    """
    _system(f"{_ee} -m pip install {lib.lower()} --upgrade --pre")
...
def versions() -> None:
    ...
    try:
        ...
        _system(f"{_ee} -m pip install pip --upgrade")
        _system(f"{_ee} -m pip index versions byteprint --pre")

def execute(inp: _Union[str, bool, None] = None) -> None:
    """
    ...
    :note: >
        可交互内容如下： / The interactive content is as follows:
            ...
            'exit' (交互式 / interactive) -> '交互存在 / interactive existence'
        ------
        其中，m函数用法特殊，只重点讲这个
        他本质就是一个字符串
        用法：
    ...
    def _form(a: str) -> str:
        ...
        for d in a.split(";"):
            ...
            b += cs.strip() + ";"
        return b[:-1]
    sl: _List[str] = ["math", "cmath", "time", "datetime", "random", "tqdm", "sympy", "scipy", "numpy", "pandas"]
    def _run(c: str, use: _Dict[str, _Any]) -> None:
        r: bool = True
        ...
        use = {**use, "m": m, "clear": lambda: print(_clear)}
    ...
    glo["repr"] = lambda x: print(repr(x))
    glo["globals"] = lambda: print(var)
    glo["rm"] = lambda v: var.pop(v, None)
    if inp is None or isinstance(inp, bool):
        alls.extend(["exit", "info"])
        print(f"BytePrint {__version__} [Python {_ver} on {_plat}]")
        infs: _Callable[[], None] = lambda: print(f"""
        ...
        现在，“；” <{hex(ord('；'))}> 可以替换成“;”了！ / Now, '；'<{hex(ord('；'))}> can be replaced with ';'
        ...
        if inp is None:
            try:
                ...
                print("跳过（确定为n） / Skip (determine as n)")
        elif inp:
            infs()
        else:
            print("跳过（确认为n） / Skip (determine as n)")
        while True:
            try:
                c: bool = True
                su = ""
...
__init__ = (bd, helps, execute, versions, upgrade, install)
