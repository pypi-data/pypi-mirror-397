...
from .schemas import get_schema_for_endpoint
...
def stripe_source(
    ...
    def stripe_resource(
        ...
        data_yielded = False
        for page in pagination(endpoint, start_date, end_date):
            if page:  # Non-empty page
                data_yielded = True
            yield page

        # If no data yielded, create empty table with proper schema
        # Schema definitions from Stripe API docs ensure correct column types
        if not data_yielded:
            # Yield single empty dict to create table structure
            # Schema hints below define all expected columns
            yield [{}]

    # Configure resource with proper Stripe API schema
    def make_resource(endpoint: str):
        schema = get_schema_for_endpoint(endpoint)
        return dlt.resource(
            stripe_resource,
            name=endpoint,
            write_disposition="replace",
            primary_key="id",  # All Stripe objects have an 'id' field
            columns=schema
        )(endpoint)

    for endpoint in endpoints:
        yield make_resource(endpoint)
