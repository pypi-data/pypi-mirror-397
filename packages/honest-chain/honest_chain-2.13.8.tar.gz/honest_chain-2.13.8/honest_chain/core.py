...
class HonestChain:
    ...
    def __init__(
        ...
        # HIGH FIX: Validate storage_path to prevent path traversal
        default_base = Path.home() / ".honest_chain"
        if storage_path is None:
            self.storage_path = default_base / agent_id
        else:
            # Check for path traversal attempts FIRST
            if ".." in str(storage_path):
                raise ValueError("Invalid storage_path: path traversal (..) not allowed")
            # Validate custom storage_path
            resolved = Path(storage_path).resolve()
            # Must be within home directory, current working directory, or /tmp (for tests)
            home = Path.home().resolve()
            cwd = Path.cwd().resolve()
            tmp = Path("/tmp").resolve()
            if not (str(resolved).startswith(str(home)) or
                    str(resolved).startswith(str(cwd)) or
                    str(resolved).startswith(str(tmp))):
                raise ValueError(
                    f"Invalid storage_path: must be within home directory, cwd, or /tmp. "
                    f"Got: {resolved}"
                )
