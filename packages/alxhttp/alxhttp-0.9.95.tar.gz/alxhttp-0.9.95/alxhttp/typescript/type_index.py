...
from alxhttp.typescript.syntax_tree import (
  ObjectInit,
  ObjectInitField,
  ObjectType,
  ObjectTypeField,
...
def recurse_model_types(t: type | TypeAliasType, seen: set[type | TypeAliasType] | None = None) -> Generator[type | TypeAliasType, None, None]:
  ...
  if is_alias(t):
    if is_union_of_models_recursive(t.__value__):  # pyright: ignore[reportUnknownAttribute, reportAttributeAccessIssue]  # ty:ignore[unresolved-attribute]
      yield t
...
def _first_real_field(
  subtype: type | TypeAliasType,
) -> tuple[str, type | TypeAliasType]:
...
def _discrimination_expr(src_name: str, type_args: tuple[type | TypeAliasType, ...]) -> str:
  discrimination_expr = ''
...
class TypeIndex:
  ...
  def recurse_model(self, mt: type, init_from_wire: bool = True, init_to_wire: bool = True) -> None:
    for m in recurse_model_types(mt):
      if should_skip_field('', m):
        continue
      elif is_alias(m) and is_union_of_models_recursive(m.__value__):
        type_name = str(m)
        union_type = m.__value__
        union_args = tuple(
          union_arg
          for union_arg in get_union_args_recursive(union_type)
          if isinstance(union_arg, type)  # this is just to convince the type checker that all is well, we expect all the union args to be types
        )
        self.py_to_ts_union[union_type] = UnionType(
          type_name,
          members=[extract_class(c) for c in union_args],
          export=True,
        )
        if init_from_wire:
          self.init_discriminated_union_from_wire(union_type, union_args)
      else:
  ...
  def _gen_init_field_assignment(
    self,
    field_type: type | TypeAliasType,
    src_name: str = 'root',
    depth: int = 0,
    self_type: type | TypeAliasType | None = None,
  ) -> str | None:
    depth += 1
    ...
    if is_annotated(field_type):
      ...
      return src_name
    elif field_type == datetime or field_type == AwareDatetime:  # pyright: ignore[reportUnnecessaryComparison]
      return f'new Date({src_name} * 1000)'
    elif field_type == Any:
      return src_name
    elif field_type == NoneType:
      return src_name
    elif is_class_var(field_type):
      return None
    elif is_alias(field_type):
      return self._gen_init_field_assignment(field_type.__value__, src_name, depth, self_type)
    elif is_list(field_type) or is_tuple(field_type):
    ...
    elif is_union_of_safe_primitive_types_or_none(field_type):
      return src_name
    ...
    elif is_union_of_models_recursive(field_type):
      union_args = tuple(
        union_arg
        for union_arg in get_union_args_recursive(field_type)
        if isinstance(union_arg, type)  # this is just to convince the type checker that all is well, we expect all the union args to be types
      )
      return _discrimination_expr(src_name, union_args)
      ...
      return f'get{pytype_to_tstype(field_type)}FromWire({src_name})'
    elif field_type == typing.Self:
      assert self_type is not None
      return self._gen_init_field_assignment(self_type, src_name, depth, self_type)
    else:
      raise ValueError

  def _gen_uninit_field_assignment(
    self,
    field_type: type | TypeAliasType,
    src_name: str = 'root',
    depth: int = 0,
    ...
    if is_annotated(field_type):
      ...
      return src_name
    elif should_skip_field('', field_type):
      return None
    elif field_type == typing.Self:
      assert self_type is not None
      return self._gen_uninit_field_assignment(self_type, src_name, depth, self_type)
    elif is_alias(field_type):
      return self._gen_uninit_field_assignment(field_type.__value__, src_name, depth, self_type)
    elif is_list(field_type) or is_tuple(field_type):
      ...
      first_first_name = None
      finished = False
      for n, subtype in enumerate(get_union_args_recursive(field_type)):
      ...
      return f'convert{pytype_to_tstype(field_type, self_type=self_type)}ToWire({src_name})'
    else:
      raise ValueError

  def init_discriminated_union_from_wire(
    self,
    py_union_type: type,
    union_args: tuple[type | TypeAliasType, ...],
    wire_arg: str = 'root',
  ) -> str:
    ts_type = self.py_to_ts_union[py_union_type]
    de = _discrimination_expr(wire_arg, union_args)

    wire_func_name = f'get{ts_type.name}FromWire'
    wire_func = f'export function {wire_func_name}(root: any): {ts_type.name} {{ return {de} }};\n'

    self.py_to_wire_func_name[py_union_type] = wire_func_name
    self.py_to_wire_func[py_union_type] = wire_func
  ...
  def init_from_wire(self, py_type: type | TypeAliasType, wire_arg: str = 'root') -> str:
    ...
    for tsfield in ts_type.fields:
      ...
      field_type = self._gen_init_field_assignment(
        tsfield.decl.decl,
        f'{wire_arg}.{tsfield.name}',
        depth=0,
        self_type=py_type,
      )
  ...
  def init_to_wire(self, py_type: type | TypeAliasType, wire_arg: str = 'root') -> str:
    ...
    for tsfield in ts_type.fields:
      ...
      field_assignments.append(
        ObjectInitField(
          tsfield.name,
          self._gen_uninit_field_assignment(
            tsfield.decl.decl,
            f'{wire_arg}.{tsfield.name}',
            self_type=py_type,
          ),
