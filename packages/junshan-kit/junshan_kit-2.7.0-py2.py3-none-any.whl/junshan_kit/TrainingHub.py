...
from junshan_kit import DataHub, TrainingHub, Evaluate_Metrics, DataProcessor, Print_Info
...
def get_dataloader(data_name, train_dataset, test_dataset, Paras):
    ParametersHub_.set_seed(Paras["seed"])
    g = torch.Generator()
...
def train(train_loader, optimizer_name, optimizer, model, loss_fn, Paras):
    ...
    metrics = ParametersHub_.metrics()
...
def Save_Results(Paras, model_name, data_name, optimizer_name):
    ...
    filename = (
        ...
        f'Results_{ParametersHub_.model_abbr(model_name)}_'
        f'{data_name}_'
