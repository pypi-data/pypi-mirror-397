...
"""
import math
from . import ParametersHub_
import matplotlib.pyplot as plt
import numpy as np
import matplotlib as mpl
from collections import defaultdict
from junshan_kit import kit
...
def Search_Paras(Paras, args, model_name, data_name, optimizer_name, metric_key = "training_loss"):
    ...
    for idx, (param_str, info) in enumerate(param_dict.items()):
        ...
        ax.set_ylabel(ParametersHub_.fig_ylabel(metric_key))
        ax.grid(True)
    ...
    filename = f'{Paras["Results_folder"]}/{metric_key}_{ParametersHub_.model_abbr(model_name)}_{data_name}_{optimizer_name}.pdf'
...
def Read_Results_from_pkl(info_dict, Exp_name, model_name):
    ...
    for data_name, info in info_dict.items():
        for optimizer_name, info_opt in info["optimizer"].items():
            ...
            pkl_path = f'{Exp_name}/seed_{info["seed"]}/{model_name}/{data_name}/{optimizer_name}/train_{info["train_test"][0]}_test_{info["train_test"][1]}/Batch_size_{info["batch_size"]}/epoch_{info["epochs"]}/{info_opt["ID"]}/Results_{ParametersHub_.model_abbr(model_name)}_{data_name}_{optimizer_name}.pkl'

            data_ = kit.read_pkl_data(pkl_path)

            param_str = ParametersHub_.opt_paras_str(info["optimizer"][optimizer_name])
...
def Mul_Plot(model_name, info_dict, Exp_name = "SPBM", cols = 3, svae_path = None, fig_show = False):
    # matplotlib settings
    ...
    for idx, (data_name, info) in enumerate(draw_data.items()):
        ...
        ax.set_ylabel(ParametersHub_.fig_ylabel(info_dict[data_name]["metric_key"]), fontsize=12) 
    ...
    plt.tight_layout()
    if svae_path is None:
        svae_path = f'{model_name}.pdf'

    else:
        svae_path = f'{svae_path}.pdf'

    plt.savefig(svae_path, bbox_inches="tight")
    if fig_show:
        plt.show()
...
def Opt_Paras_Plot(model_name, info_dict, Exp_name = "SPBM", svae_path = None, fig_show = False):
    ...
    plt.ylabel(f'{ParametersHub_.fig_ylabel(info_dict[data_name]["metric_key"])}')  
    if svae_path is None:
        svae_path = f'{model_name}.pdf'
    else:
        svae_path = f'{svae_path}.pdf'
