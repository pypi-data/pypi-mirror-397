...
from gwaslab_agent.g_sys_prompt import system_prompt_utility_runner
...
class UtilityRunner():
    def __init__(self, log_object=None, tools=None, llm=None, history=None, archive=None, verbose=True):
        ...
        self.log.write("Initiating GWASLab Agent UtilityRunner...", verbose=verbose)
        ...
        self.tools = self._filter_utility_tools(tools or [])
    ...
    def _init_agent(self):
        return create_agent(
            model=self.llm,
            tools=self.tools,
            system_prompt=system_prompt_utility_runner,
            middleware=[
                handle_tool_errors,
    ...
    def _filter_utility_tools(self, tools):
        out = []
        for t in tools:
            name = getattr(t, "name", "")
            if not isinstance(name, str):
                continue
            if name in UTILITY_SET or name == "run_on_filtered" or name == "get_data_from_registry":
                out.append(t)
        try:
            _v = getattr(self, "_init_verbose", True)
        except Exception:
            _v = True
        self.log.write(f" -Registered {len(out)} tools for UtilityRunner.", verbose=_v)
        return out
    ...
    def run(self, message: str, verbose=True, verbose_return=False, return_message=False, if_print=False):
        self.history.append({"role": "user", "gwaslab_agent": "Worker_UtilityRunner", "content": self._compose_log_message(message)})
        self.archive.append({"role": "user", "gwaslab_agent": "Worker_UtilityRunner", "content": self._compose_log_message(message)})
        ...
        for chunk in self.agent.stream({"messages": self.history}, stream_mode="updates"):
            for step, data in chunk.items():
                ...
                if not messages:
                    continue
                msg = messages[-1]
                out_msg = print_message(self, console, msg, step, True, verbose, verbose_return, if_print=False, title="UTILITY", role="UtilityRunner")
                ...
                if getattr(msg, "content", None):
                    self.history.append({"role": "assistant", "gwaslab_agent": "Worker_UtilityRunner", "content": msg.content})
                    self.archive.append({"role": "assistant", "gwaslab_agent": "Worker_UtilityRunner", "content": msg.content})
