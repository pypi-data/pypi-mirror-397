...
from arclet.cithun import PermissionEngine, ResourceNode, Role, User

Attach: TypeAlias = "Callable[[User, dict | None, int, Callable[[User | Role, dict | None], int]], int]"
TAttach = TypeVar("TAttach", bound=Attach)
Attach1: TypeAlias = "Callable[[User, str, dict | None, int, Callable[[User | Role, dict | None], int]], int]"
...
class Attacher:
    ...
    def _run_attachs(
        ...
        context: dict | None,
        current_mask: int,
        permission_lookup: Callable[[User | Role, dict | None], int],
    ) -> int:
        results = []
