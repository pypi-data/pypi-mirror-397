...
def get_tailadmin_color(category: str, shade: str = "500") -> str:
    ...
    Returns:
        Hex color value
    """
    return TAILADMIN_COLORS.get(category, {}).get(
        shade, TAILADMIN_COLORS["gray"]["500"]
    )
...
def create_tailadmin_metric_card(
    ...
    if change:
        ...
        change_bg = (
            TAILADMIN_COLORS["success"]["100"]
            if change_type == "positive"
            else TAILADMIN_COLORS["error"]["100"]
        )
...
def create_tailadmin_progress_bar(
    ...
    percentage_text = (
        f"<span style='font-weight: 600;'>{percentage}%</span>"
        if show_percentage
        else ""
    )
...
def create_tailadmin_stat_widget(
    title: str, stats: List[Dict], chart_data: Optional[str] = None
) -> str:
    """
