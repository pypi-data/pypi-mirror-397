...
def set_page_config():
    ...
    st.set_page_config(
        page_title="VP Retail Operations Dashboard", page_icon="🏪", layout="wide", initial_sidebar_state="collapsed"
    )
...
def create_tailadmin_header():
    """Create TailAdmin-style header with avatar using hybrid approach."""
    ...
    st.markdown(
        """
    <div style="background: white; border-bottom: 1px solid #e5e7eb; padding: 1rem 2rem; margin: -1rem -1rem 1rem -1rem; position: sticky; top: 0; z-index: 999;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div style="display: flex; align-items: center; gap: 2rem;">
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <h1 style="font-size: 1.25rem; font-weight: 600; color: #1f2937; margin: 0;">VP Retail Operations</h1>
                </div>
            </div>
    ...
    # User controls using Streamlit components
    col1, col2, col3, col4, col5 = st.columns([2, 3, 1, 1, 1])

    with col2:
        st.text_input(
            "search",
            placeholder="🔍 Search stores, reports, or type command...",
            label_visibility="collapsed",
            key="header_search",
        )

    with col3:
        if st.button("☀️", help="Toggle dark mode", key="dark_mode"):
            st.toast("Dark mode toggle clicked!")

    with col4:
        if st.button("🔔", help="Notifications", key="notifications"):
            st.toast("3 new notifications")

    with col5:
        # User dropdown using Streamlit selectbox
        user_action = st.selectbox(
            "user_menu",
            ["Profile", "Edit Profile", "Account Settings", "Support", "Sign Out"],
            index=0,
            label_visibility="collapsed",
            key="user_dropdown",
        )
        ...
        if user_action != "Profile":
            st.toast(f"{user_action} clicked!")
            # Reset to default
            st.session_state.user_dropdown = "Profile"


def create_tailadmin_header_v2():
    """Alternative: Pure Streamlit columns approach."""

    # Header with columns
    st.markdown(
        """
    <div style="background: white; border-bottom: 1px solid #e5e7eb; padding: 0.5rem 0; margin-bottom: 1rem;">
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    with col2:
        st.markdown("**VP Retail Operations**")
        st.caption("Executive Dashboard")

    with col3:
        st.text_input(
            "search",
            placeholder="🔍 Search stores, reports, or command...",
            label_visibility="collapsed",
            key="search_v2",
        )

    with col4:
        if st.button("☀️", key="dark_v2"):
            st.toast("Dark mode!")

    with col5:
        if st.button("🔔", key="notif_v2"):
        ...
        with st.container():
            ...
            with user_col1:
                ...
                <div style="text-align: right; padding-top: 0.5rem;">
                    <div style="font-weight: 600; font-size: 0.875rem;">Sarah Chen</div>
                    <div style="color: #6b7280; font-size: 0.75rem;">VP Operations</div>
                </div>
                """,
                    unsafe_allow_html=True,
                )
            ...
            with user_col2:
                if st.button("SC", key="avatar_v2", help="User menu"):
                    st.toast("User menu clicked!")
...
def create_tailadmin_header_v3():
    ...
    with st.container():
        ...
        with col1:
            st.text_input("", placeholder="🔍 Search stores, reports, or commands...", key="search_v3")
...
def main():
    ...
    # create_tailadmin_header()        # Hybrid approach with selectbox
    # create_tailadmin_header_v2()     # Pure Streamlit columns
    create_tailadmin_header_v3()  # Sidebar user menu
