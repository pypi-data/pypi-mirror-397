...
class ClusterDelete(ClusterCommand):
    ...
    def delete_storage_account_plink_privdns_zones(self, resource_group_name: str) -> None:
        ...
        @retry(
            retry=retry_if_exception_type(ResourceExistsError),
            stop=stop_after_delay(180),  # 3 minutes
            wait=wait_exponential(multiplier=1, min=4, max=60),
            reraise=True,
        )
        def _delete_privdns_zone_with_retry(rg_name: str, zone_name: str) -> None:
            """Delete private DNS zone with retry logic to handle race conditions with nested resources."""
        ...
        for entity in entities:
            ...
            _delete_privdns_zone_with_retry(resource_group_name, entity.name)
