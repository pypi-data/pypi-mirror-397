...
def _apply_corporate_actions_to_frame(
    ...
    # Apply dividend adjustments to OHLC prices for backtesting accuracy.
    # This matches Yahoo Finance's Adj Close calculation.
    # For each dividend, all prices BEFORE the ex-date are adjusted by:
    #   factor = 1 - (dividend_amount / close_price_on_day_before_ex_date)
    # We work from newest to oldest dividend, accumulating the adjustment factor.
    if not dividends.empty and "close" in frame.columns:
        price_columns = ["open", "high", "low", "close"]
        available_price_cols = [col for col in price_columns if col in frame.columns]

        if available_price_cols:
            # Sort dividends by date (newest first for reverse iteration)
            sorted_divs = dividends.sort_values("event_date", ascending=False)

            # Filter out future dividends (dividends after data's end date)
            data_end_date = max(index_dates)
            applicable_divs = sorted_divs[sorted_divs["event_date"].dt.date <= data_end_date]

            if not applicable_divs.empty:
                # Create cumulative dividend adjustment factor series
                div_cumulative_factor = pd.Series(1.0, index=frame.index)

                # Work through dividends from newest to oldest
                for _, div_row in applicable_divs.iterrows():
                    ex_date = div_row["event_date"].date()
                    div_amount = div_row["cash_amount"]

                    if div_amount <= 0:
                        continue

                    # Find the close price on the day BEFORE the ex-date
                    # This is used as the reference for calculating the adjustment factor
                    dates_before_ex = [d for d in index_dates if d < ex_date]
                    if not dates_before_ex:
                        continue

                    day_before_ex = max(dates_before_ex)
                    # Get close price for that day (already split-adjusted at this point)
                    close_mask = pd.Series(index_dates) == day_before_ex
                    if not close_mask.any():
                        continue

                    close_on_day_before = frame.loc[close_mask.values, "close"].iloc[0]
                    if close_on_day_before <= 0:
                        continue

                    # The dividend amount from ThetaData is already split-adjusted
                    # (we adjusted it in the split section above)
                    # So we can use it directly with the split-adjusted close price
                    # Get the dividend value for this day (already split-adjusted)
                    div_for_calc = div_amount
                    # Check if we have the adjusted dividend in the frame
                    if "dividend" in frame.columns:
                        div_mask = pd.Series(index_dates) == ex_date
                        if div_mask.any():
                            div_for_calc = frame.loc[div_mask.values, "dividend"].iloc[0]
                            if div_for_calc <= 0:
                                div_for_calc = div_amount

                    # Calculate adjustment factor: 1 - (dividend / close_price)
                    # If dividend is $1 and close is $100, factor = 0.99
                    # All prior prices should be multiplied by this
                    factor = 1.0 - (div_for_calc / close_on_day_before)

                    # Sanity check: factor should be between 0.5 and 1.0
                    # A factor < 0.5 would mean dividend > 50% of stock price (very unusual)
                    if factor <= 0.5 or factor > 1.0:
                        logger.warning(
                            "[THETA][DIV_ADJUST] Skipping unusual dividend factor %.4f for %s "
                            "on %s (div=%.4f, close=%.2f)",
                            factor, asset.symbol, ex_date, div_for_calc, close_on_day_before
                        )
                        continue

                    # Apply factor to all dates BEFORE the ex-date
                    mask = pd.Series(index_dates) < ex_date
                    div_cumulative_factor.loc[mask.values] *= factor

                # Apply the dividend adjustment to price columns
                total_div_adjustment = 1.0 - div_cumulative_factor.min()
                if total_div_adjustment > 0.001:  # More than 0.1% adjustment
                    for col in available_price_cols:
                        if col in frame.columns:
                            frame[col] = frame[col] * div_cumulative_factor

                    logger.debug(
                        "[THETA][DIV_ADJUST] asset=%s total_div_factor_min=%.4f dividends=%d",
                        asset.symbol, div_cumulative_factor.min(), len(applicable_divs)
                    )
...
def get_request(url: str, headers: dict, querystring: dict):
