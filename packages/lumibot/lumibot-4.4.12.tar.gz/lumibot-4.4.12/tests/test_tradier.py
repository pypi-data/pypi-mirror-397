...
class TestTradierBrokerAPI:
    ...
    def test_submit_order_fractional_not_supported(self, tradier):
        """
        Test that Tradier API does NOT support fractional share orders via quantity.

        VERIFIED Dec 12, 2025: Tradier API explicitly rejects decimal quantities with:
        "Invalid parameter, quantity: decimal places are not allowed."

        Key findings from testing:
        1. Tradier's WEB PLATFORM supports fractional shares ($5 minimum via dollar amounts)
        2. Tradier's API requires quantities "in whole numbers" - no notional/dollar parameter
        3. Direct API calls with decimal quantities are rejected by Tradier
        4. The lumiwealth_tradier library silently truncates decimals to integers
        5. Quantities < 1.0 (like 0.5) truncate to 0, which fails API validation
        6. Quantities >= 1.0 (like 1.5) truncate to 1 - user gets fewer shares than requested!

        Unlike Alpaca (which has explicit `qty` and `notional` parameters), Tradier's API
        does not support fractional share trading. Alpaca is recommended for fractional trading.
        """
        asset = Asset("AAPL")
        # Try to submit an order for 0.5 shares (fractional < 1)
        order = Order('strat_unittest', asset, 0.5, 'buy', order_type='market')
        submitted_order = tradier._submit_order(order)

        # Tradier rejects fractional orders < 1 - the order will NOT be submitted
        # The status remains 'unprocessed' because 0.5 truncates to 0, which fails
        assert submitted_order.status != "submitted", \
            "Tradier unexpectedly accepted fractional shares - API behavior may have changed!"
...
class TestTradierBroker:
    ...
    def test_fractional_quantity_warning(self, mocker, caplog):
        """
        Test that Tradier broker emits a warning when fractional quantities are used.

        VERIFIED Dec 12, 2025: Tradier's web platform supports fractional shares via
        dollar amounts, but their API requires quantities "in whole numbers" with no
        notional/dollar parameter. The lumiwealth_tradier library truncates decimals
        to integers, so we added a warning to alert users of this behavior.
        """
        import logging
        broker = Tradier(account_number="1234", access_token="a1b2c3", paper=True, connect_stream=False)
        ...
        # Mock the tradier.orders.order method to prevent actual API calls
        mock_order = mocker.patch.object(broker.tradier.orders, "order", return_value={"id": 12345})

        stock_asset = Asset("SPY")

        # Test with whole number - no warning should be emitted
        with caplog.at_level(logging.WARNING):
            caplog.clear()
            order = Order("my_strat", stock_asset, 10, Order.OrderSide.BUY, order_type="market")
            broker._submit_order(order)
            assert "fractional shares" not in caplog.text

        # Test with fractional number - warning should be emitted
        with caplog.at_level(logging.WARNING):
            caplog.clear()
            order = Order("my_strat", stock_asset, 1.5, Order.OrderSide.BUY, order_type="market")
            broker._submit_order(order)
            assert "fractional shares" in caplog.text.lower()
            assert "1.5" in caplog.text
            assert "1" in caplog.text  # truncated value
