...
class Tradier(Broker):
    ...
    ***Note: Tradier does not support Trailing StopLoss orders.
    ...
    def _submit_order(self, order: Order):
        ...
        # Check for fractional quantities - Tradier API does NOT support fractional shares via quantity
        # Note: Tradier's web platform supports fractional trading ($5 minimum), but their API
        # requires quantities "in whole numbers". There's no documented notional/dollar-amount
        # parameter like Alpaca has. The lumiwealth_tradier library truncates decimals to integers.
        if order.quantity != int(order.quantity):
            logger.warning(
                f"Tradier API does not support fractional share quantities. Order quantity "
                f"{order.quantity} will be truncated to {int(order.quantity)} shares. "
                f"Note: Tradier's web platform supports fractional shares via dollar amounts, "
                f"but this is not available through their API. Consider using Alpaca for "
                f"API-based fractional share trading."
            )
