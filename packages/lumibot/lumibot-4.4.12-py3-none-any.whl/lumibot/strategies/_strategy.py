...
class _Strategy:
            ...
            for position in positions:
                ...
                if price is not None:
                    ...
                    if numeric_price is not None and numeric_price <= 0:
                        should_store_price = False
                        if (
                            getattr(base_asset, "asset_type", None) == "option"
                            and getattr(base_asset, "expiration", None) is not None
                        ):
                            try:
                                if self.broker.datetime.date() >= base_asset.expiration:
                                    should_store_price = True
                                    numeric_price = 0.0
                            except Exception:
                                should_store_price = False

                    if should_store_price:
                        self._last_known_prices[asset] = numeric_price if numeric_price is not None else price
    ...
    def _pick_snapshot_price(self, asset, snapshot):
        ...
        base_asset = asset[0] if isinstance(asset, tuple) else asset
        asset_type = getattr(base_asset, "asset_type", None)
        ...
        close_positive = _positive(close_price)
        bid_positive = _positive(bid_price)
        ask_positive = _positive(ask_price)

        # Backtesting MTM: historical quote timestamps are often missing or not aligned with the
        # strategy clock (especially for EOD option NBBO). Prefer NBBO mid whenever no trade
        # price is available to avoid marking positions at $0.00 between trades.
        if self.is_backtesting and asset_type == "option":
            if close_positive is not None:
                return close_positive
            if bid_positive is not None and ask_positive is not None:
                return (bid_positive + ask_positive) / 2.0
            if bid_positive is not None:
                return bid_positive
            if ask_positive is not None:
                return ask_positive
            expiry = getattr(base_asset, "expiration", None)
        ...
        close_price = close_positive
        bid_price = bid_positive
        ask_price = ask_positive
