...
class TestApplyCorporateActionsToFrame:
    ...
    @patch("lumibot.tools.thetadata_helper._get_theta_dividends")
    @patch("lumibot.tools.thetadata_helper._get_theta_splits")
    def test_options_skip_split_adjustment(self, mock_splits, mock_dividends):
        """
        Test: Option assets should NOT have price split adjustment applied

        Option prices are already adjusted by the exchange - we only adjust
        the strike for API queries, not the option price itself.
        """
        mock_dividends.return_value = pd.DataFrame()
        mock_splits.return_value = pd.DataFrame({
            "event_date": pd.to_datetime(["2022-07-15"], utc=True),
        ...
        # Option prices should NOT be adjusted
        assert enriched["close"].iloc[0] == pytest.approx(52.0, rel=0.01)
...
class TestSplitEdgeCases:
    ...
    def test_reverse_split(self, mock_get_splits):
        ...
        # Reverse splits: ratio < 1 means no adjustment in current logic
        # (cumulative_split_factor > 1.0 check)
        assert original_strike == 50.0  # No adjustment for reverse split
