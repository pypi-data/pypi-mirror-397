...
class MLOpsManager:
    ...
    def __init__(
        ...
        # Fallback to API host if not set, but warn the user
    ...
    def add_model(self, source_type, model_name=None, version=None, source_path=None, code_path=None, category=None, tags=None,
        ...
        print(f"[SDK_INFO] Adding model: {model_name} (Source: {source_type})")
        ...
        if source_type == "s3":
            ...
            if not provided_creds:
                if self.verbose:
                    print("[SDK_INFO] Auto-fetching S3 credentials...")
                try:
        ...
        try:
            ...
            print(f"[SDK_SUCCESS] Model '{model_name}' (Ver: {version}) added successfully.")
            return model_id
    ...
    def get_model(self, model_name, local_dest, version=None):
        ...
        try:
            ...
            print(f"[SDK_SUCCESS] Downloaded to {local_dest}")
    ...
    def list_models(self, verbose=False):
        ...
        try:
            ...
            print(f"{'Model Name':<25} | {'ID':<25} | {'Latest':<10} | {'Size (MB)':<15} | {'Ver Count':<10} | {'Versions'}")
            print("-" * 130)
            ...
            for m in models:
                ...
                print(f"{name:<25} | {mid:<25} | {latest:<10} | {str(total_size):<15} | {str(version_count):<10} | {display_versions}")
