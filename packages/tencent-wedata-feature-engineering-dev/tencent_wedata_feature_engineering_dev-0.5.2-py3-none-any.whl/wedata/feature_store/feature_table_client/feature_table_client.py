...
class FeatureTableClient(AbstractBaseTableClient):
    ...
    def create_table(
        ...
        Args:
            ...
            timestamp_key: 时间戳键（用于时态特征，可选）。如果指定，则该表将被视为时序特征表，
                          在 create_training_set 时需要在 FeatureLookup 中提供 timestamp_lookup_key。
                          如果不需要时序语义，请保持为 None（默认）。
        ...
        # 过滤掉保留属性
        filtered_tags = {k: v for k, v in (tags or {}).items() if k.lower() not in RESERVED_TABLE_PROPERTIES}
...
--     STORED AS PARQUET
    ...
    def _read_online_table(self,
        ...
        # 已注释：不使用 Feast 读取在线表
        raise NotImplementedError("Online table reading is disabled. Feast integration has been commented out.")
    ...
    def drop_online_table(self, table_name: str, online_config: RedisStoreConfig, database_name: Optional[str] = None):
        # 已注释：不使用 Feast 删除在线表
        raise NotImplementedError("Online table operations are disabled. Feast integration has been commented out.")
