...
class InvalidInformation(BaseModel):
    ...
    information: StrictStr = Field(
        description="Inference result not conforming to the template.       This is the raw response from the model."
    )
    error: StrictStr = Field(
        description="Error message explaining why the inference result is invalid."
    )
    type: StrictStr
    __properties: ClassVar[List[str]] = ["information", "error", "type"]
