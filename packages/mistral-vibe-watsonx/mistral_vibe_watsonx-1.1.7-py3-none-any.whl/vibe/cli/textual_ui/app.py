...
from vibe.cli.analyze import execute_analyze
from vibe.cli.analyze.executor import AnalyzeProgress, format_analyze_summary
...
"""
    ...
    async def _run_analyze(self) -> None:
        """Run /analyze command to generate VIBE-ANALYSIS.md with codebase analysis."""
        # Ensure agent is initialized so we can use its backend for LLM calls
        init_task = self._ensure_agent_init_task()
        if init_task and not init_task.done():
            await self._mount_and_scroll(
                UserCommandMessage("Initializing agent for /analyze command...")
            )
        ...
        if not self.agent:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Agent not initialized. Cannot run /analyze.",
                    collapsed=self._tools_collapsed,
                )
            )
            return
        ...
        await self._mount_and_scroll(
            UserCommandMessage("Starting /analyze - analyzing codebase...")
        )
        ...
        # Create an async generator wrapper for the agent's LLM
        async def llm_complete(prompt: str):
            """Wrapper to call the agent's backend for LLM completion."""
            if not self.agent:
                return

            # Use proper LLMMessage objects, not dicts
            ...
            try:
                async with self.agent.backend as backend:
                    ...
                    # LLMChunk has .message.content, not .content directly
                    if response.message and response.message.content:
                        logger.debug(f"/analyze LLM response: {response.message.content[:200]}...")
                        yield response.message.content
                    else:
                        logger.warning("/analyze LLM response had no content")
                        yield ""
            except Exception as e:
                logger.error(f"/analyze LLM call failed: {e}", exc_info=True)
                yield f"Error: {e}"

        # Progress callback
        last_phase = ""

        def on_progress(progress: AnalyzeProgress) -> None:
            nonlocal last_phase
            if progress.phase != last_phase:
                ...
                if self._loading_widget:
                    self._loading_widget.set_status(
                        f"{progress.phase.title()}: {progress.message[:30]}"
                    )
        ...
        try:
            ...
            # Show result
            summary = format_analyze_summary(result)
