...
class Agent:
    ...
    def _setup_middleware(self, max_turns: int | None, max_price: float | None) -> None:
        ...
        if self.config.context_injection:
            self.middleware_pipeline.add(
                ContextInjectionMiddleware(
                    workspace=self.config.effective_workdir,
                    llm_complete=self._create_llm_complete_for_middleware(),
                    enabled=True,
                )
            )

    def _create_llm_complete_for_middleware(self) -> Callable[[str], AsyncGenerator[str, None]]:
        """Create an async generator function for middleware LLM calls.

        Returns:
            Async generator function that yields text chunks from LLM
        """
        async def llm_complete(prompt: str) -> AsyncGenerator[str, None]:
            """Complete a prompt using the agent's LLM backend."""
            active_model = self.config.get_active_model()
            provider = self.config.get_provider_for_model(active_model)
            ...
            try:
                async with self.backend as backend:
                    result = await backend.complete(
                        ...
                        max_tokens=2000,  # Limit for middleware operations
                    )
                    if result.message.content:
                        yield result.message.content
            except Exception:
                # Silently fail for middleware - don't break the main conversation
                pass
    ...
    async def _conversation_loop(self, user_msg: str) -> AsyncGenerator[BaseEvent]:
        ...
        try:
            ...
            while not should_break_loop:
                ...
                # Buffer events - we may need to discard them if enhancement is requested
                ...
                try:
                    ...
                    # On interruption, yield buffered events before re-raising
                ...
                if user_cancelled:
                    # Yield buffered events before returning on cancellation
                    for event in buffered_events:
                        yield event
                    self._flush_new_messages()
                    await self.interaction_logger.save_interaction(
                ...
                # Run after-turn middleware to check for enhancement
                after_result = await self.middleware_pipeline.run_after_turn(
                    self._get_context()
                )

                # Handle ENHANCE_RESPONSE: discard buffered events, inject context, continue loop
                if after_result.action == MiddlewareAction.ENHANCE_RESPONSE:
                    if after_result.message:
                        logger.info(
                            "Agent: ENHANCE_RESPONSE received - discarding initial response, "
                            "injecting context, regenerating"
                        )
                        # Remove the assistant message we just generated
                        if self.messages and self.messages[-1].role == Role.assistant:
                            self.messages.pop()

                        # Add enhancement context as a user message for the LLM to process
                        self.messages.append(
                            LLMMessage(role=Role.user, content=after_result.message)
                        )

                        # Continue the loop - don't yield buffered events, don't break
                        should_break_loop = False
                        continue

                # No enhancement - yield buffered events
                for event in buffered_events:
                    yield event
