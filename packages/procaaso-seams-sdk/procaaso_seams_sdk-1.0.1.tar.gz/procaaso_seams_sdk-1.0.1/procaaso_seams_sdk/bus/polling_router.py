...
class PollingBusRouter:
    ...
    async def _process_command(
        ...
        try:
            # Check idempotency
            cached_reply = await self.idempotency_store.get(request.command_id)
            if cached_reply:
                if self.log:
                    self.log.info(
                        f"Command {request.command_id} already processed (idempotent), "
                        f"returning cached reply"
                    )
                await self.bus.publish_reply(cached_reply)
                return
            ...
            await self.idempotency_store.record(request, reply)
            ...
            if self.log:
                ...
                self.log.info(
                    f"Processed command {request.command_id} for {component_id}: {status}"
                )

        except Exception as e:
            # Handler raised exception - create error reply
            if self.log:
                self.log.error(
            ...
            # Publish error reply
            try:
                await self.bus.publish_reply(error_reply)
            except Exception as publish_error:
                if self.log:
                    self.log.error(f"Failed to publish error reply: {publish_error}")
