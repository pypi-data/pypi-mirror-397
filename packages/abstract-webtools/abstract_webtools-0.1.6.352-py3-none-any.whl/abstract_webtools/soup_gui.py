...
import requests
from abstract_webtools import UserAgentManager,UrlManager,SafeRequest,SoupManager,LinkManager,big_user_agent_list

# Function to send an HTTP GET request and return the response object
url_manager = UrlManager(url="www.example.com")
...
layout = [
    ...
    [sg.Text('User-Agent:', size=(8, 1)), sg.Combo(big_user_agent_list, default_value=big_user_agent_list[0], key='-USERAGENT-', disabled=False)],
    [get_cypher_checks()],
...
while True:
    ...
    if event == 'Grab URL':
        url = values['-URL-']
        update_urls(url)
        window['-SOUP_OUTPUT-'].print(soup_manager.soup, end='', text_color='black')
    ...
    # Handle All Soup button click event
    if event == 'All Soup':
        pass
        # Replace with your BeautifulSoup logic to find and display all elements
        # Example: soup.find_all(tag)
        # Update the -FIND_ALL_OUTPUT- element with the result
