...
class infoRegistry(metaclass=SingletonMeta):
    ...
    def _upsert(self, video_id, data: dict):
        ...
        data.update(safe_load_from_json(info_path))
        safe_dump_to_file(data=data,file_path=info_path)
...
def get_video_id(url=None, video_id=None, video_path=None,video_root=None, documents_root=None):
    registry_mgr = infoRegistry(video_root=video_root, documents_root=documents_root)
    video_info = registry_mgr.get_video_info(url=url, video_id=video_id, video_path=video_path)
    if video_info:
