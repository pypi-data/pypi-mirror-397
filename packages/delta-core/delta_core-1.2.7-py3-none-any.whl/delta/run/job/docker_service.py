...
class DockerJobService(JobService):
    ...
    def check_jobs(self) -> None:
        ...
        for event in self._cli.events(
            ...
            # check event
            docker_28_below = "status" in event and event["status"] == "die"
            docker_29_above = "Action" in event and event["Action"] == "die"
            if docker_28_below or docker_29_above:
                container = self._cli.containers.get(attributes["name"])
