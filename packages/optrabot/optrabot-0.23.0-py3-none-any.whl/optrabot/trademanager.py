...
class TradeManager(metaclass=SingletonMeta):
	...
	async def openTrade(self, entryOrder: Order, template: Template):
		...
		if template.meets_maximum_premium(entryOrder.price) == False:
			logger.error(f'Entry order for account {template.account} exceeds maximum premium requirement. Unable to place entry order')
			return
	...
	def _calculate_trade_fees(self, managed_trade: ManagedTrade) -> float:
		...
		# Count transactions by type (entry vs exit based on timestamp or order association)
		entry_quantity = managed_trade.entryOrder.quantity if managed_trade.entryOrder else 0
		num_legs = len(managed_trade.entryOrder.legs) if managed_trade.entryOrder and managed_trade.entryOrder.legs else 4
		expected_exit_transactions = entry_quantity * num_legs
		...
		for transaction in managed_trade.transactions:
			...
			# Count entry vs exit transactions (entry transactions come first)
			if len([t for t in managed_trade.transactions if t.timestamp <= transaction.timestamp]) <= entry_quantity * num_legs:
				entry_fees += fee
			else:
				exit_fees += fee
