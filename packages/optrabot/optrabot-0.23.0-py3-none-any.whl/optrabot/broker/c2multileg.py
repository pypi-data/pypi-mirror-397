                ...
                for result in results:
                    if result.get('SignalId') == leg_order.signal_id:
                        ...
                        if order_status == '2':  # Filled
                            ...
                            logger.debug(f'Leg {leg.strike} {leg.right} found FILLED by SignalId {leg_order.signal_id}')
                            return True, float(fill_price)
                        elif order_status == '4':  # Cancelled
                            # Order cancelled - but check if there's another order for the same leg that was filled
                            logger.debug(f'Leg {leg.strike} {leg.right} SignalId {leg_order.signal_id} is cancelled, checking for replacement order...')
                            break  # Continue to search by leg characteristics
