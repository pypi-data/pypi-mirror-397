...
class MultilegOrderHandler:
    ...
    async def _check_leg_fill_status(
        ...
        # Note: This uses the same API as _track_orders but for a specific signal
        import pytz
        ...
        try:
            ...
            for result in results:
                if result.get('SignalId') == leg_order.signal_id:
                    order_status = result.get('OrderStatus')
            ...
            try:
                ...
                if response.status_code != status.HTTP_200_OK:
                    ...
                    if 'Unable to cancel' in response.text or 'xrpeplace' in response.text.lower():
                        ...
                        # Wait a moment for C2 to update status, then check multiple times
                        for retry in range(3):
                            await asyncio.sleep(self._min_delay_between_requests)  # Respect rate limits
                            filled, fill_price = await self._check_leg_fill_status(leg_order, managed_trade)
                            if filled:
                            ...
                            if retry < 2:
                                logger.debug(f'Retry {retry + 1}/3: Leg {leg.strike} {leg.right} not yet showing as filled, waiting...')
    ...
    def _calculate_rollback_limit_price(
        ...
        # Try to get current market data for the leg
        try:
            from optrabot.broker.optionpricedata import OptionStrikePriceData
            price_data = OptionStrikePriceData.get_price_data(
                symbol, leg.expiration, leg.strike, leg.right
            )
            
            if price_data:
                if closing_action in [OrderAction.SELL, OrderAction.SELL_TO_CLOSE]:
                    # Selling: use bid or minimum price
                    if price_data.bid and price_data.bid > 0:
                        return max(price_data.bid, min_option_price)
                    else:
                        # No bid available - use minimum price to get filled
                        return min_option_price
                else:
                    # Buying: use ask with small buffer
                    if price_data.ask and price_data.ask > 0:
                        return price_data.ask + 0.05
                    elif price_data.mid and price_data.mid > 0:
                        return price_data.mid * 1.1  # 10% above mid
                    else:
