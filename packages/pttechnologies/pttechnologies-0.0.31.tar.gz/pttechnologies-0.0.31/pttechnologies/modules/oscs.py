        ...
        if r1 is not None and r2 is not None:
            ...
            if r1.status_code == r2.status_code and ct1 == ct2:
                ...
                if product:
                    ...
                    storage.add_to_storage(technology=technology_name, technology_type=category_name, vulnerability="PTV-WEB-INFO-OSSEN", vendor=vendor, product_id=6)
                ...
                if product:
                    ...
                    category_name = product_manager.get_category_name(product.get('category_id'))
                    vendor = product.get('vendor')
                    storage.add_to_storage(technology=technology_name, technology_type=category_name, vulnerability="PTV-WEB-INFO-OSSEN", vendor=vendor, product_id=167)
                    ptprint(f"Identified OS: {display_name}", "VULN", not self.args.json, indent = 4, end=" ")
                    ptprint(f"({probability}%)", "ADDITIONS", not self.args.json, colortext=True)
        else:
            ptprint("Connection error occurred", "INFO", not self.args.json, indent=4)
