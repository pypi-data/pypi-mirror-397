...
from .permission_helpers import PermissionChecker
...
class PermissionManager:
    ...
    def check(self, operation: str, info: Info) -> bool:
        ...
        """
        if info is None or not hasattr(info, "context") or info.context is None:
            logger.debug(
                f"Permission check failed: no context for operation '{operation}'"
            )
            return False
        ...
        try:
            ...
            logger.error(
                f"Error in permission checker for operation '{operation}': {e}",
                exc_info=True,
            )
...
# Permission helper functions are now in permission_helpers.py
# Import them here for backward compatibility
from .permission_helpers import (  # noqa: F401
    PermissionChecker,
    has_all_roles,
    has_any_role,
    has_role,
