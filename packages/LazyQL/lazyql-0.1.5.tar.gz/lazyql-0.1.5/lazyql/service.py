...
class LazyQL(Generic[T]):
    ...
    async def get_all(
        ...
        try:
            ...
            if sort_by:
                cursor = cursor.sort(sort_by, sort_order)  # noqa: E501

            docs = []
            async for doc in cursor:
                try:
                    docs.append(self.model_cls(**doc))
                except ValidationError as e:
                    logger.warning(
                        f"Validation error for document {doc.get('_id')} "
    ...
    async def _update_implementation(
        ...
        """
        Internal implementation of document update.
        ...
        try:
            ...
            if not update_data:
                logger.warning(
                    f"No fields to update for document {id} "
                    f"in '{self.collection_name}'"
                )
                # Return existing document if no updates
            ...
            logger.error(
                f"Error updating document {id} "
                f"in '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    async def _hard_delete_implementation(
        ...
        try:
            ...
            logger.error(
                f"Error hard deleting document {id} "
                f"in '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
