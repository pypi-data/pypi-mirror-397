...
class Instinct8Agent:
    ...
    def execute(self, prompt: str, show_reasoning: bool = False) -> str:
        ...
        if any(keyword in prompt_lower for keyword in ["review", "look at", "read", "show me", "list files", "what files", "take a look", "deep dive", "check", "examine", "see what", "what's in"]):
            if show_reasoning:
    ...
    def _read_files_from_prompt(self, prompt: str) -> str:
        ...
        # First, always list directory structure to understand the project layout
        try:
            structure = []
            structure.append(f"=== Directory Structure: {cwd} ===\n")
            ...
            for item in sorted(cwd.iterdir()):
                if item.is_dir():
                    ...
                    if item.name in ["src", "my-react-app", "react-app", "app", "frontend", "client"]:
                        structure.append(f"   (React app directory)")
                elif item.is_file():
                    structure.append(f"📄 {item.name}")
            
            structure_text = "\n".join(structure)
            file_contexts.append(structure_text + "\n")
        except Exception:
            pass
        
        # If asking to review "all files" or "the website" or "current folder"
        if any(phrase in prompt_lower for phrase in ["all files", "all the files", "review all", "look at all", "review the website", "review website", "current folder", "this folder", "my files", "the files", "take a look", "deep dive"]):
            # Detect React app directory
            react_app_dir = None
            for possible_dir in ["my-react-app", "react-app", "app", "frontend", "client", "src"]:
                if (cwd / possible_dir).exists() and (cwd / possible_dir).is_dir():
                    react_app_dir = cwd / possible_dir
                    file_contexts.append(f"\n=== Found React app directory: {possible_dir} ===\n")
                    break
            
            # If React app found, prioritize reading from there
            search_dir = react_app_dir if react_app_dir else cwd
            ...
            try:
                ...
                for ext in ["*.py", "*.js", "*.ts", "*.jsx", "*.tsx", "*.html", "*.css", "*.json", "*.md", "*.txt", "*.yaml", "*.yml", "*.toml"]:
                    ...
                    all_files.extend(search_dir.glob(ext))
                    # Search recursively in subdirectories
                    all_files.extend(search_dir.glob(f"**/{ext}"))
                
                # Also get common config files from both root and React app
                for name in ["package.json", "pyproject.toml", "requirements.txt", "README.md", "index.html", "app.js", "main.py", "App.js", "App.jsx"]:
                    # Check root
                    if (cwd / name).exists():
                        all_files.append(cwd / name)
                    # Check React app directory
                    if react_app_dir and (react_app_dir / name).exists():
                        all_files.append(react_app_dir / name)
                
                # Remove duplicates and sort
                all_files = sorted(set(all_files), key=lambda p: (p.is_dir(), str(p)))
                
                # Read files (prioritize React app files if found)
                files_to_read = []
                if react_app_dir:
                    # Read React app files first
                    react_files = [f for f in all_files if react_app_dir in f.parents or f.parent == react_app_dir]
                    files_to_read.extend(react_files[:5])  # First 5 React files
                    # Then add root files
                    root_files = [f for f in all_files if f not in react_files]
                    files_to_read.extend(root_files[:5])  # First 5 root files
                else:
                    files_to_read = all_files[:10]  # First 10 files
