...
from codecheck.main import CodeCheckConfig
...
def main(
    repo_path: str, parent_branch: Optional[str], module: str, log_level: int, all_files: bool
) -> int:
    """Run mypy with --disallow-untyped-defs option on changed files."""
    logging.basicConfig(level=log_level or logging.WARNING)
    parent_branch = parent_branch or CodeCheckConfig.load_from_toml(repo_path).git_parent_branch
    ...
    if all_files:
        ...
        files = gitcov.get_changed_files(
            repo_path, parent_branch=parent_branch, file_extensions=["py"]
        )
