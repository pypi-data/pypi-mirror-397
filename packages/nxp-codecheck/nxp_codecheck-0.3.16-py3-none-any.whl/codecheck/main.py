...
class CodeCheckConfig:
    ...
    @classmethod
    def load_from_toml(cls, repo_path: Optional[str] = None) -> Self:
...
class LogPathFormatter(ABC):
    """Base class for log path formatters."""

    @classmethod
    @abstractmethod
    def can_handle(cls) -> bool:
        """Determine if this formatter can handle the current environment."""

    @abstractmethod
    def format_path(self, log_path: str) -> str:
        """Format the log path for the specific environment."""


class DefaultLogPathFormatter(LogPathFormatter):
    """Default formatter that returns the original path."""

    @classmethod
    def can_handle(cls) -> bool:
        """Determine if this formatter can handle the current environment."""
        return True  # Always can handle as a fallback
...
class JenkinsLogPathFormatter(LogPathFormatter):
    """Format log paths for Jenkins environment."""
    ...
    def format_path(self, log_path: str) -> str:
        ...
        if not (job_name and build_number and jenkins_url):
            return log_path
        try:
            project, job = job_name.split("/", 1)
            # URL encode project and job names to handle spaces and special characters
            project_enc = urllib.parse.quote(project, safe="")
            job_enc = urllib.parse.quote(job, safe="")
            jenkins_artifact_url = (
                f"{jenkins_url}job/{project_enc}/job/{job_enc}/{build_number}/artifact"
            )
        except ValueError:
            # If job_name doesn't contain a slash, use it as is
            project_encoded = urllib.parse.quote(job_name, safe="")
            jenkins_artifact_url = f"{jenkins_url}job/{project_encoded}/{build_number}/artifact"
        ...
        return f"{jenkins_artifact_url}/{rel_path}"


class LogPathFormatterRegistry:
    """Registry for log path formatters."""

    _formatters: Dict[str, Type[LogPathFormatter]] = {}

    @classmethod
    def register(cls, formatter_name: str, formatter_class: Type[LogPathFormatter]) -> None:
        """Register a formatter."""
        cls._formatters[formatter_name] = formatter_class

    @classmethod
    def get_formatter(cls) -> LogPathFormatter:
        """Get the appropriate formatter for the current environment."""
        for formatter_class in cls._formatters.values():
            if formatter_class.can_handle():
                return formatter_class()
        # Fallback to default
        return DefaultLogPathFormatter()

    @classmethod
    def register_defaults(cls) -> None:
        """Register default formatters."""
        cls.register("jenkins", JenkinsLogPathFormatter)
        cls.register("default", DefaultLogPathFormatter)


LogPathFormatterRegistry.register_defaults()
...
def print_results(tasks: List[TaskInfo]) -> None:
    ...
    formatter = LogPathFormatterRegistry.get_formatter()
    for i, task in enumerate(tasks, start=1):
        ...
        log_display = formatter.format_path(log_path)
        table.add_row(
            [
