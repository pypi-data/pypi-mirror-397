...
def connection_create_s3(
    ...
    if env == "local" and not client.check_aws_credentials():
        return {
            "name": None,
            "error": "No AWS credentials found. Please run `tb local restart --use-aws-creds` to pass your credentials. "
    ...
    # Track which secrets were successfully created
    ...
    if cloud_client:
        try:
            ...
            click.echo(
                FeedbackManager.warning(message=f"Failed to create secret in cloud: {e}. Secret created in local only.")
            )
