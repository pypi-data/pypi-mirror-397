...
class UpgradeJob(object):
    ...
    """__Request body for Config Upgrade__  It contains one field: - spec  The spec can be one of two types based on $dryrun boolean value passed in query parameter.  A sample request body with the $dryrun set to true in query parameter would look like this: ``` { \"spec\": {   \"policyNames\" : [],   \"shouldIncludeSecureSubnetsInfo\" : false   } } ```  A sample request body with the $dryrun not set as part of query parameter would look like this: ``` { \"spec\": {   \"shouldSetMonitorMode\" : true,   \"shouldUpgradeSecuredSubnetsOnly\" : false   } } ``` 

    :param spec: (:attr:`spec`) Specifies the type of upgradeJobSpec (UpgradeSpec or DryRunSpec).
    :type spec: required
