# Python Client For Nutanix Flow Management APIs

The Python client for Nutanix Flow Management APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Network Security Policy configuration of Nutanix clusters.
## Features
...
api_client = ntnx_microseg_py_client.ApiClient(allow_version_negotiation=False)
```
...
```sh
<my-env>/bin/pip install ntnx-microseg-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-microseg-py-client
```
...
## Configuration
The python client for Nutanix Flow Management APIs can be configured with the following parameters
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=microseg&version=v4.2&language=python). This documentation is auto-generated, and the location may change.
