...
from mielto.knowledge.extract.base import ExtractConfig
...
class Reader:
    ...
    def chunk_document(self, document: Document) -> List[Document]:
        print(self.chunking_strategy, "CHUNKING STRATEGY")
    ...
    def extract_content(self, documents: List[Document]) -> List[Document]:
        if self.extract is None:
            return documents
        from mielto.knowledge.extract.base import ExtractStrategyFactory
        extract_strategy = ExtractStrategyFactory.create_strategy(self.extract)
        return extract_strategy.extract(documents)