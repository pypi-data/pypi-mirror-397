...
QtObject {
    ...
    // 背景色
    readonly property color backgroundColor: isDark ? "#1A202C" : "#F7FAFC"
    ...
    readonly property var _schemeColors: ({
        "gray": grayColor, "red": redColor, "green": greenColor, "blue": blueColor,
        "teal": tealColor, "pink": pinkColor, "purple": purpleColor, "cyan": cyanColor,
        "orange": orangeColor, "yellow": yellowColor, "primary": primaryColor,
        "secondary": secondaryColor, "success": successColor, "warning": warningColor, "error": errorColor
    })

    readonly property var _schemeHovers: ({
        "gray": grayHover, "red": redHover, "green": greenHover, "blue": blueHover,
        "teal": tealHover, "pink": pinkHover, "purple": purpleHover, "cyan": cyanHover,
        "orange": orangeHover, "yellow": yellowHover, "primary": primaryHover,
        "secondary": secondaryHover, "success": successColor, "warning": warningColor, "error": errorColor
    })

    readonly property var _schemeLights: ({
        "gray": grayLight, "red": redLight, "green": greenLight, "blue": blueLight,
        "teal": tealLight, "pink": pinkLight, "purple": purpleLight, "cyan": cyanLight,
        "orange": orangeLight, "yellow": yellowLight, "primary": infoLight,
        "secondary": grayLight, "success": successLight, "warning": warningLight, "error": errorLight
    })

    readonly property var _buttonHeights: ({ "sm": buttonHeightSm, "md": buttonHeightMd, "lg": buttonHeightLg })
    readonly property var _inputHeights: ({ "sm": inputHeightSm, "md": inputHeightMd, "lg": inputHeightLg })
    readonly property var _fontSizes: ({ "xs": fontSizeXs, "sm": fontSizeSm, "md": fontSizeMd, "lg": fontSizeLg, "xl": fontSizeXl })
    readonly property var _boxSizes: ({ "sm": 16, "md": 20, "lg": 24 })
    readonly property var _switchTrackWidths: ({ "sm": 32, "md": 44, "lg": 52 })
    readonly property var _switchTrackHeights: ({ "sm": 18, "md": 24, "lg": 28 })
    readonly property var _progressHeights: ({ "xs": 4, "sm": 8, "md": 12, "lg": 16 })
    readonly property var _tagHeights: ({ "sm": 20, "md": 26, "lg": 32 })
    readonly property var _badgeFontSizes: ({ "sm": fontSizeXs, "md": fontSizeSm, "lg": fontSizeMd })
    readonly property var _paddingHs: ({ "sm": spacing1, "md": spacing2, "lg": spacing3 })
    readonly property var _spinnerSizes: ({ "xs": 12, "sm": 16, "md": 24, "lg": 32, "xl": 48 })
    readonly property var _spinnerThicknesses: ({ "xs": 2, "sm": 2, "md": 3, "lg": 4, "xl": 4 })
    readonly property var _segmentHeights: ({ "xs": 24, "sm": 28, "md": 32, "lg": 40 })
    readonly property var _cardPaddings: ({ "sm": spacing3, "md": spacing4, "lg": spacing6 })
    readonly property var _menuItemHeights: ({ "sm": 32, "md": 38, "lg": 44 })
    readonly property var _containerMaxWidths: ({ "sm": 640, "md": 768, "lg": 1024, "xl": 1280 })
    readonly property var _dialogWidths: ({ "xs": 320, "sm": 400, "md": 512, "lg": 640, "xl": 768 })

    // ========== 辅助函数 (使用缓存映射表) ==========
    function getSchemeColor(scheme) { return _schemeColors[scheme] || blueColor; }
    function getSchemeHover(scheme) { return _schemeHovers[scheme] || blueHover; }
    function getSchemeLight(scheme) { return _schemeLights[scheme] || blueLight; }
    function getButtonHeight(size) { return _buttonHeights[size] || buttonHeightMd; }
    function getInputHeight(size) { return _inputHeights[size] || inputHeightMd; }
    function getFontSize(size) { return _fontSizes[size] || fontSizeMd; }
    function getBoxSize(size) { return _boxSizes[size] || 20; }
    function getSwitchTrackWidth(size) { return _switchTrackWidths[size] || 44; }
    function getSwitchTrackHeight(size) { return _switchTrackHeights[size] || 24; }
    function getProgressHeight(size) { return _progressHeights[size] || 12; }
    function getTagHeight(size) { return _tagHeights[size] || 26; }
    function getBadgeFontSize(size) { return _badgeFontSizes[size] || fontSizeSm; }
    function getPaddingH(size) { return _paddingHs[size] || spacing2; }
    function getSpinnerSize(size) { return _spinnerSizes[size] || 24; }
    function getSpinnerThickness(size) { return _spinnerThicknesses[size] || 3; }
    function getSegmentHeight(size) { return _segmentHeights[size] || 32; }
    function getCardPadding(size) { return _cardPaddings[size] || spacing4; }
    function getMenuItemHeight(size) { return _menuItemHeights[size] || 38; }
    function getContainerMaxWidth(size) { return _containerMaxWidths[size] || 1024; }
    function getDialogWidth(size) { return _dialogWidths[size] || 512; }
}
