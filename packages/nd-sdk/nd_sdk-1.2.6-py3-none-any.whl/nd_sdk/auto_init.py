...
class AutoInitializer:
    ...
    def initialize_all(
        ...
        Environment Variables:
            ...
            # Storage
            STORAGE_PROVIDER: Storage provider (awss3/azureblob/minio) - required if STORAGE_ENABLED=true
    ...
    def _initialize_service_config(self):
        ...
        config = get_config.init(provider="service", config_data=get_config.init(config_data={}, provider="yaml"))
        print(f"Service config initialized with provider: service, environment: yaml", flush=True)
        return config
    ...
    def _initialize_storage(self):
        ...
        # FIXED: Pass provider parameter to get_storage.init()
        provider = os.getenv('STORAGE_PROVIDER')

        if not provider:
            error_msg = "STORAGE_ENABLED=true but STORAGE_PROVIDER is not set. Please set STORAGE_PROVIDER to one of: awss3, azureblob, minio"
            if 'logger' in self.components:
                self.components['logger'].error(error_msg)
            else:
                print(f"ERROR: {error_msg}", flush=True)
            raise ValueError(error_msg)

        storage = get_storage.init(provider=provider)

        if 'logger' in self.components:
            self.components['logger'].info(f"Storage initialized with provider: {provider}")
