...
import { createSpinner, createToast, gettext, vars } from "../util.js";
...
class PlayerLyrics {
    ...
    async #searchLyrics() {
        ...
        if (await queue.currentTrack.searchLyrics()) {
            createToast("sync", gettext("Found new lyrics"));
        } else {
            createToast("alert-circle", gettext("Could not find any lyrics"));
        }
    ...
    #updateLyrics() {
        ...
        if (!vars.offlineMode) {
            const searchLyricsButton = document.createElement('a');
            searchLyricsButton.classList.add('secondary', 'search-lyrics-button')
            searchLyricsButton.textContent = gettext("Search for new lyrics");
            searchLyricsButton.addEventListener('click', () => {
                searchLyricsButton.replaceWith(createSpinner());
                this.#searchLyrics();
            });
            newContent.push(searchLyricsButton);
        }
