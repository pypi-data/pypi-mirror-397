#!/usr/bin/env python3
"""
Async LLM Demo - demonstrates concurrent LLM operations using send_message()
...
Note: send_message() is the primary working async API in Ceylon.
"""
...
async def demo_concurrent_queries():
    ...
    agent = ceylonai_next.LlmAgent("concurrent_agent", "ollama::gemma3:latest")
    ...
    tasks = [agent.send_message(q) for q in questions]
...
async def demo_streaming_responses():
    ...
    agent = ceylonai_next.LlmAgent("streaming_agent", "ollama::gemma3:latest")
    ...
    for label, question in questions:
        task = asyncio.create_task(agent.send_message(question))
    ...
    while pending:
        ...
        for task in done:
            try:
                ...
                print(
                    f"[{completed_count}/{len(questions)}] {task.label} query completed ({elapsed:.2f}s)"
                )
...
async def demo_batch_processing():
    ...
    # Create LLM agent
    print("\n1. Creating LLM agent...")
    agent = ceylonai_next.LlmAgent("batch_agent", "ollama::gemma3:latest")
    ...
    for i in range(0, len(queries), batch_size):
        ...
        tasks = [agent.send_message(q) for q in batch]
...
async def demo_error_handling():
    ...
    # Create LLM agent
    print("\n1. Creating LLM agent...")
    agent = ceylonai_next.LlmAgent("error_agent", "ollama::gemma3:latest")
...
async def main():
    ...
    print("- Concurrent LLM queries using send_message()")
