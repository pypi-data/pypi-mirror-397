...
def ensure_ollama_running():
    ...
    try:
        ...
        if "qwen2.5:0.5b" not in result.stdout:
            ...
            subprocess.run(["ollama", "pull", "qwen2.5:0.5b"], check=True, timeout=600)
...
async def interactive_conversation(
    messages: list[str], personality: str = "friendly"
) -> dict:
    """
    ...
    for i, msg in enumerate(messages, 1):
        print(f"\n[Turn {i}] User: {msg}")
        response = await chatbot.send_message(msg)
        print(f"[Turn {i}] Bot: {response}")

        conversation.append({"turn": i, "user": msg, "bot": response})
