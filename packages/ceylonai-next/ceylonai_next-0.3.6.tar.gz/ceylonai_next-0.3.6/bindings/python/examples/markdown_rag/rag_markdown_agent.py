...
class MarkdownRAGAgent:
    ...
    def __init__(
        ...
        if llm_config is None:
            ...
            llm_config = LlmConfig.anthropic(
                api_key=api_key, model="claude-3-5-sonnet-20241022"
            )

        # Create mesh and agent
        self.mesh = LocalMesh()
        self.agent = LlmAgent(
            agent_id="markdown-rag-agent", mesh=self.mesh, llm_config=llm_config
        )
    ...
    def retrieve_context(
        self, query: str, filter_file: Optional[str] = None
        ...
        for i, (chunk, score) in enumerate(results, 1):
            ...
            heading_str = (
                " > ".join(chunk.heading_path) if chunk.heading_path else "Content"
            )
    ...
    def generate_answer(
        self, question: str, context: str, include_citations: bool = True
    ) -> str:
        """
...
Please answer the question based on the context provided above. Provide a comprehensive answer with proper citations."""
        ...
        full_prompt = f"{system_prompt}\n\n{user_prompt}"
        response = await self.agent.send_message(full_prompt)
    ...
    async def answer_question_async(
        ...
        if not sources:
            ...
            answer = await self.generate_answer_async(
                question, context, include_citations=True
            )
            print("✓ Answer generated")
    ...
    def save_qa_to_markdown(
        self, qa_record: Dict, file_name: Optional[str] = None
    ) -> Path:
        """
        ...
        if qa_record["sources"]:
            ...
            lines.append("")
            lines.append(
                "The following sources from the knowledge base were used to generate this answer:"
            )
            lines.append("")
    ...
    async def answer_multiple_questions_async(
        self, questions: List[str], save_to_file: bool = True
    ) -> List[Dict]:
        """
        ...
        tasks = [
            self.answer_question_async(q, save_to_file=save_to_file) for q in questions
        ]
        ...
        for i, qa in enumerate(self.qa_history, 1):
            ...
            if qa["sources"]:
                ...
                for j, source in enumerate(qa["sources"], 1):
                    lines.append(
                        f"{j}. `{Path(source['file']).name}` - {source['section']} (Relevance: {source['relevance']:.2f})"
                    )
                lines.append("")
