    ...
    def on_message(self, message, context=None):
        ...
        if message.startswith("route:"):
            ...
            if len(parts) >= 3:
                ...
                self.mesh.send_to_sync(target, actual_msg)
...
def main():
    ...
    mesh.send_to_sync("ping_agent", "Hello Ping!")
    time.sleep(0.1)

    mesh.send_to_sync("echo_agent", "Hello Echo!")
    time.sleep(0.1)

    mesh.send_to_sync("ping_agent", "Another ping")
    time.sleep(0.1)
    ...
    mesh.send_to_sync("router_agent", "route:ping_agent:Routed ping message")
    time.sleep(0.1)

    mesh.send_to_sync("router_agent", "route:echo_agent:Routed echo message")
    time.sleep(0.1)
