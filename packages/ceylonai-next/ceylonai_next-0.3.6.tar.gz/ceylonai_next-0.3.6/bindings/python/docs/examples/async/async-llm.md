...
- **`send_message(q)`**: Async version of `send_message()`
...
This demo showcases Ceylon's async functionality:
- Concurrent LLM queries using send_message()
...
```python
tasks = [asyncio.create_task(agent.send_message(q)) for q in questions]
...
async def my_function():
    result = await agent.send_message(question)
```
...
1. Verify you're using `send_message()` not `send_message()`
...
async def limited_concurrent_queries(agent, questions, max_concurrent=3):
    ...
    async def limited_query(question):
        async with semaphore:
            return await agent.send_message(question)
...
async def queries_with_progress(agent, questions):
    ...
    tasks = {
        asyncio.create_task(agent.send_message(q)): q
        for q in questions
    }
...
- ✅ Using `send_message()` for concurrent operations
