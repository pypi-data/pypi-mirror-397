...
class Pipeline:
    ...
    async def process(self, data: str) -> dict:
        ...
        # Stage 1: Validate
        validation = await self.validator.send_message(
            f"Validate: {data}"
        )

        # Stage 2: Process
        processing = await self.processor.send_message(
            f"Process: {data}"
        )

        # Stage 3: Analyze
        analysis = await self.analyzer.send_message(
...
class FallbackAgent:
    ...
    async def query(self, message: str) -> str:
        ...
        try:
            ...
            try:
                return await self.secondary.send_message(message)
...
async def test_with_mock():
    ...
    response = await mock.send_message("What is AI?")
...
async def bad():
    ...
    agent.send_message("Hello")  # Not awaited!
