...
### `send_message(message: str) -> str` (async)

Send an asynchronous message to the agent. This is the primary async method - use `await`.
...
async def main():
    ...
    response = await agent.send_message("How are you?")
    print(response)

    # Concurrent messages
    responses = await asyncio.gather(
        agent.send_message("Question 1"),
        agent.send_message("Question 2"),
        agent.send_message("Question 3"),
    )
...
### `query(message: str) -> str` (async)

Alias for `send_message()`. Provides alternative naming convention.
...
async def process_documents():
    ...
    summaries = await asyncio.gather(
        *[agent.send_message(doc) for doc in documents]
    )
