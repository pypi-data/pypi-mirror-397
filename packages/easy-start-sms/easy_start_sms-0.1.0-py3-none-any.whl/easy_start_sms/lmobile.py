...
class Sender:
    ...
    def client(self, **kwargs):
        ...
        """
        kwargs = kwargs if isinstance(kwargs, httpx.Client) else dict()
        # 设置默认基础URL
    ...
    def async_client(self, **kwargs):
        ...
        """
        kwargs = kwargs if isinstance(kwargs, httpx.AsyncClient) else dict()
        # 设置默认基础URL
        kwargs.setdefault("base_url", self.base_url)
        # 设置默认超时时间为120秒
        kwargs.setdefault("timeout", 120)
    ...
    def send_sms(self, client: httpx.Client = None, phone_nos: str = "", content: str = "", **kwargs):
        """发送短信（同步方式）
        ...
        # 返回发送结果（根据Result字段判断）、响应JSON和原始响应对象
        return str(response_json.get("Result", "fail")).lower() == "succ", response_json, response

    async def async_send_sms(self, client: httpx.AsyncClient = None, phone_nos: str = "", content: str = "", **kwargs):
        """发送短信（异步方式）
        ...
        if not isinstance(client, httpx.AsyncClient):
            async with self.async_client(**kwargs) as client:
