...
class OperationSchemaChangeImpact(object):
    ...
    Allowed enum values:
        ...
        - _UNKNOWN
        - _REDACTED
        - IMPACTED: This indicates that the given role contains only the old operation(s) and is missing the newer equivalent operation(s).
        - NOT_IMPACTED: This indicates that the given role contains the newer equivalent operation(s) of the old operation(s).
        - UNDETERMINED: This indicates that the given role could not be successfully scanned for analyzing the impact due to the current set of operation(s).
    """
    _UNKNOWN = "$UNKNOWN"
    _REDACTED = "$REDACTED"
    IMPACTED = "IMPACTED"
    ...
    def _initialize_object_type(self):
        return 'iam.v4.authz.OperationSchemaChangeImpact'
        ...
        for attr in vars(self):
            ...
            if isinstance(value, list):
                ...
                if sanitize:
                    ...
                    result[attr] = value
        if issubclass(OperationSchemaChangeImpact, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, OperationSchemaChangeImpact):
            return False
