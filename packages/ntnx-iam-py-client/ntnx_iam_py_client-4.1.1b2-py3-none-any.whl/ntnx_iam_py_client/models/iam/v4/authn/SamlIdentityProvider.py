...
class SamlIdentityProvider(ExternalizableAbstractModel):
    ...
    :param redirect_url: (:attr:`redirect_url`) The URL to redirect the user after successful authentication. Use this optional input when the user needs to be redirected to a different URL other than the default, such as when the PC is behind a reverse proxy and the user needs to be redirected to the reverse proxy URL instead of the PC URL.
    :type redirect_url: 
     min=3 chars, max=2000 chars
    ...
    def idp_metadata_url(self, idp_metadata_url):
        ...
        if idp_metadata_url is not None and not re.search('^((http[s]?|nfs):\/)?\/?(([a-zA-Z0-9\\-\\.]+)|(\\d{1,3}(\\.\\d{1,3}){3})|(\\[[0-9a-fA-F:]+\\]))(:\\d+)?(\/[^\\s?#]*)?(\\?[^#\\s]*)?(#[^\\s]*)?$', idp_metadata_url):  # noqa: E501
            raise ValueError(r"Invalid value for `idp_metadata_url`, must be a follow pattern or equal to `/^((http[s]?|nfs):\/)?\/?(([a-zA-Z0-9\\-\\.]+)|(\\d{1,3}(\\.\\d{1,3}){3})|(\\[[0-9a-fA-F:]+\\]))(:\\d+)?(\/[^\\s?#]*)?(\\?[^#\\s]*)?(#[^\\s]*)?$/`")  # noqa: E501
    ...
    def redirect_url(self, redirect_url):
        if redirect_url is not None and len(redirect_url) > 2000:
            raise ValueError("Invalid value for `redirect_url`, length must be less than or equal to `2000`")  # noqa: E501
        if redirect_url is not None and len(redirect_url) < 3:
            raise ValueError("Invalid value for `redirect_url`, length must be greater than or equal to `3`")  # noqa: E501
        if redirect_url is not None and not re.search('^((http[s]?):\/)?\/?(\\[[a-fA-F0-9:]+\\]|[^:\/\\s]+)(:[0-9]+)?$', redirect_url):  # noqa: E501
            raise ValueError(r"Invalid value for `redirect_url`, must be a follow pattern or equal to `/^((http[s]?):\/)?\/?(\\[[a-fA-F0-9:]+\\]|[^:\/\\s]+)(:[0-9]+)?$/`")  # noqa: E501
