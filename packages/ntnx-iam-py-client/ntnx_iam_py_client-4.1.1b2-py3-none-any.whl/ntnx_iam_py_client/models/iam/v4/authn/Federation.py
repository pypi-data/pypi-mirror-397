...
class Federation(ExternalizableAbstractModel):
    ...
    def well_known_config(self, well_known_config):
        ...
        if well_known_config is not None and not re.search('^((http[s]?|nfs):\/)?\/?(([a-zA-Z0-9\\-\\.]+)|(\\d{1,3}(\\.\\d{1,3}){3})|(\\[[0-9a-fA-F:]+\\]))(:\\d+)?(\/[^\\s?#]*)?(\\?[^#\\s]*)?(#[^\\s]*)?$', well_known_config):  # noqa: E501
            raise ValueError(r"Invalid value for `well_known_config`, must be a follow pattern or equal to `/^((http[s]?|nfs):\/)?\/?(([a-zA-Z0-9\\-\\.]+)|(\\d{1,3}(\\.\\d{1,3}){3})|(\\[[0-9a-fA-F:]+\\]))(:\\d+)?(\/[^\\s?#]*)?(\\?[^#\\s]*)?(#[^\\s]*)?$/`")  # noqa: E501
