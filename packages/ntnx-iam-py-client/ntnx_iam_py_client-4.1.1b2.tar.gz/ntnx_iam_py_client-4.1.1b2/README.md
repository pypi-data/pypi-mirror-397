# Python Client For Nutanix Identity and Access Management APIs

The Python client for Nutanix Identity and Access Management APIs is designed for Python client application developers offering them simple and flexible access to APIs that nutanix APIs for managing users, user-groups, directory services, identity providers, roles and authorization policies.
## Features
...
api_client = ntnx_iam_py_client.ApiClient(allow_version_negotiation=False)
```
...
## Configuration
The python client for Nutanix Identity and Access Management APIs can be configured with the following parameters
...
```python
# Initialize the API
directory_services_api_instance = DirectoryServicesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# List directory services
try:
    api_response = directory_services_api_instance.list_directory_services(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=iam&version=v4.1.b2&language=python). This documentation is auto-generated, and the location may change.
