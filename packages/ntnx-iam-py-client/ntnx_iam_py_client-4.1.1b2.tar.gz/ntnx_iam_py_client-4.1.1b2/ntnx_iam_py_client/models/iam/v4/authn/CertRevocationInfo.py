...
class CertRevocationInfo(object):
    ...
    :param is_ocsp_enabled: (:attr:`is_ocsp_enabled`) Flag to enable/disable OCSP revocation check. For the POST/PUT APIs, you must add this attribute directly to the payload, not under the certRevocationInfo object.
    :type is_ocsp_enabled: 
    
    
    :param ocsp_responder: (:attr:`ocsp_responder`) URL of the OCSP responder used to override the URL from AIA extension. For the POST/PUT APIs, you must add this attribute directly to the payload, not under the certRevocationInfo object.
    :type ocsp_responder: 
    
    
    :param is_crl_enabled: (:attr:`is_crl_enabled`) Flag to enable/disable CRL revocation check. For the POST/PUT APIs, you must add this attribute directly to the payload, not under the certRevocationInfo object.
    :type is_crl_enabled: 
    
    
    :param crl_dps: (:attr:`crl_dps`) List of the CRL distribution points which will be used to fetch the CRLs. For the POST/PUT APIs, you must add this attribute directly to the payload, not under the certRevocationInfo object.
    :type crl_dps: 
    
    
    :param global_crl_refresh_interval: (:attr:`global_crl_refresh_interval`) Interval in seconds at which the CRL should be fetched from the CRLDP. The default is 86400 seconds(1 day). For the POST/PUT APIs, you must add this attribute directly to the payload, not under the certRevocationInfo object.
    ...
    def ocsp_responder(self, ocsp_responder):
        if ocsp_responder is not None and not re.search('^((http[s]?|nfs):\/)?\/?(([a-zA-Z0-9\\-\\.]+)|(\\d{1,3}(\\.\\d{1,3}){3})|(\\[[0-9a-fA-F:]+\\]))(:\\d+)?(\/[^\\s?#]*)?(\\?[^#\\s]*)?(#[^\\s]*)?$', ocsp_responder):  # noqa: E501
            raise ValueError(r"Invalid value for `ocsp_responder`, must be a follow pattern or equal to `/^((http[s]?|nfs):\/)?\/?(([a-zA-Z0-9\\-\\.]+)|(\\d{1,3}(\\.\\d{1,3}){3})|(\\[[0-9a-fA-F:]+\\]))(:\\d+)?(\/[^\\s?#]*)?(\\?[^#\\s]*)?(#[^\\s]*)?$/`")  # noqa: E501
