...
class ProductCategoryTechnologySupplier(SCTestCase):
    def setUp(self, *args, **kwargs):
        ...
        self.orange_fiber_contract_service_info = self.env[
        ...
        self.contract_fiber_asociatel_args = {
            "name": "contract w/service technology to fiber",
            "service_technology_id": self.ref("somconnexio.service_technology_fiber"),
        ...
        self.product_broadband_adsl_oneshot = (
            product_adsl_oneshot_tmpl.product_variant_id
        )  # noqa
        ...
        self.product_mobile = self.browse_ref("somconnexio.TrucadesIllimitades20GB")
        self.product_wo_fix_fiber = self.browse_ref("somconnexio.SenseFixFibra100Mb")
        self.product_mobile_oneshot = self.browse_ref("somconnexio.DadesAddicionals1GB")
        self.product_mobile_additional = self.browse_ref(
            "somconnexio.Internacional100Min"
        )
        self.product_broadband_additional = self.browse_ref("somconnexio.IPv4Fixa")
        self.product_router_4G = self.browse_ref("somconnexio.Router4G")

    def test_contract_adsl_wrong_product(self):
        contract_adsl_wrong_product_args = self.contract_adsl_args.copy()
        contract_adsl_wrong_product_args["contract_line_ids"] = [
            (0, False, {"name": "Fiber", "product_id": self.product_broadband_fiber.id})
        ]
        self.assertRaises(
            ValidationError,
    ...
    def test_contract_adsl_wrong_technology(self):
        ...
        contract_adsl_wrong_tech_args["service_technology_id"] = self.ref(
            "somconnexio.service_technology_fiber"
        )
        contract_adsl_wrong_tech_args["contract_line_ids"] = [
            (0, False, {"name": "ADSL", "product_id": self.product_broadband_adsl.id}),
            (
                0,
                False,
                {
    ...
    def test_contract_adsl_wrong_supplier(self):
        ...
        contract_adsl_wrong_supplier_args["service_supplier_id"] = self.ref(
            "somconnexio.service_supplier_vodafone"
        )
        contract_adsl_wrong_supplier_args["contract_line_ids"] = [
            (0, False, {"name": "ADSL", "product_id": self.product_broadband_adsl.id}),
            (
                0,
                False,
    ...
    def test_contract_adsl_allowed_product(self):
        ...
        contract_adsl_allowed_product_args["contract_line_ids"] = [
            (0, False, {"name": "ADSL", "product_id": self.product_broadband_adsl.id}),
            (
                0,
                False,
    ...
    def test_contract_mobile_wrong_product(self):
        ...
        contract_mobile_wrong_product_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "ADSL 20Mb", "product_id": self.product_broadband_adsl.id},
            )
        ]
        self.assertRaises(
            ValidationError,
    ...
    def test_contract_mobile_allowed_product(self):
        ...
        contract_mobile_allowed_product_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "Mobile Sense Minuts", "product_id": self.product_mobile.id},
            ),
            (
                0,
                False,
        ...
        self.assertTrue(
            self.env["contract.contract"].create(contract_mobile_allowed_product_args)
        )

    def test_contract_mobile_wrong_technology(self):
        contract_mobile_wrong_tech_args = self.contract_mobile_args.copy()
        contract_mobile_wrong_tech_args["service_technology_id"] = self.ref(
            "somconnexio.service_technology_adsl"
        )
        contract_mobile_wrong_tech_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "Mobile Sense Minuts", "product_id": self.product_mobile.id},
    ...
    def test_contract_mobile_wrong_supplier(self):
        ...
        contract_mobile_wrong_supplier_args["service_supplier_id"] = self.ref(
            "somconnexio.service_supplier_jazztel"
        )
        contract_mobile_wrong_supplier_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "Mobile Sense Minuts", "product_id": self.product_mobile.id},
    ...
    def test_contract_fiber_mm_wrong_tech_product(self):
        contract_fiber_wrong_tech_args = self.contract_fiber_vdf_args.copy()
        ...
        contract_fiber_wrong_tech_args["contract_line_ids"] = [
            (0, False, {"name": "Fiber", "product_id": self.product_xoln_fiber.id}),
            (
                0,
                False,
                {
    ...
    def test_contract_fiber_xoln_wrong_tech_product(self):
        ...
        contract_fiber_wrong_tech_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "Fiber", "product_id": self.product_broadband_fiber.id},
            ),
            (
                0,
                False,
    ...
    def test_contract_fiber_mm_allowed_product(self):
        contract_fiber_allowed_product_args = self.contract_fiber_vdf_args.copy()
        ...
        contract_fiber_allowed_product_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "Fiber", "product_id": self.product_broadband_fiber.id},
    ...
    def test_contract_fiber_asociatel_allowed_product(self):
        contract_fiber_allowed_product_args = self.contract_fiber_asociatel_args.copy()
        contract_fiber_allowed_product_args["service_supplier_id"] = self.ref(
            "somconnexio.service_supplier_asociatel_vdf"
        )
        contract_fiber_allowed_product_args["contract_line_ids"] = [
    ...
    def test_contract_fiber_mm_wrong_product(self):
        contract_fiber_wrong_product_args = self.contract_fiber_vdf_args.copy()
        contract_fiber_wrong_product_args["service_supplier_id"] = self.ref(
            "somconnexio.service_supplier_masmovil"
        )
        contract_fiber_wrong_product_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "ADSL 20Mb", "product_id": self.product_broadband_adsl.id},
    ...
    def test_contract_fiber_wrong_supplier(self):
        contract_fiber_wrong_supplier_args = self.contract_fiber_vdf_args.copy()
        contract_fiber_wrong_supplier_args["service_supplier_id"] = self.ref(
            "somconnexio.service_supplier_masmovil"
        )
        contract_fiber_wrong_supplier_args["contract_line_ids"] = [
            (
                0,
                False,
                {"name": "Fiber", "product_id": self.product_broadband_fiber.id},
    ...
    def test_contract_fiber_orange_allowed_product(self):
        contract_fiber_allowed_product_args = self.contract_fiber_orange_args.copy()
        contract_fiber_allowed_product_args["contract_line_ids"] = [
            (0, False, {"name": "Fiber", "product_id": self.product_wo_fix_fiber.id}),
            (
                0,
                False,
                {
    ...
    def test_contract_fiber_orange_wrong_product(self):
        contract_fiber_wrong_product_args = self.contract_fiber_orange_args.copy()
        contract_fiber_wrong_product_args["contract_line_ids"] = [
            (
                0,
                False,
    ...
    def test_contract_router_4G_with_fix_ip(self):
        contract_router_4G_allowed_product_args = self.contract_router_4G_args.copy()
        contract_router_4G_allowed_product_args.update(
            {
                "service_supplier_id": self.ref(
