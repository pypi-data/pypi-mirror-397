...
class InvoiceServiceRestCase(BaseRestCaseAdmin):
    ...
    def test_route_right_create(self):
        ...
        self.assertEqual(invoice.date_invoice, self.date_invoice.date())
        self.assertEqual(invoice.name, self.name)
        self.assertEqual(
            invoice.journal_id,
    ...
    def test_route_right_create_extra_fields(self):
        url = "/api/invoice"
        data = {
            "auditable": {"created": 1607590848146, "creator": "opencell.superadmin"},
            "billingAccountCode": str(self.partner.ref) + "_0",
            "invoiceDate": int(self.date_invoice.timestamp() * 1000),
            "invoiceNumber": self.name,
            "amountWithoutTax": 1.0000000000,
    ...
    def test_create_right_regular_invoice(self):
        ...
        invoice_line = self.env["account.move.line"].create(invoice_line_params)
        invoice_params = {
        ...
        self.assertEqual(self.account, invoice.invoice_line_ids[0].account_id)
        self.assertEqual(invoice.amount_untaxed, self.quantity * self.price_unit)
        self.assertEqual(invoice.amount_total, (self.quantity * self.price_unit) * 1.21)
    ...
    def test_route_right_create_invoice_lines(self):
            ...
            "categoryInvoiceAgregates": [
                {
                    ...
                    "listSubCategoryInvoiceAgregateDto": [
                        ...
                        },
                        {
                            ...
                            "invoiceSubCategoryCode": ("ISCAT_SC_CONSUMPTION_SMS_NAC"),
                            "userAccountCode": "1808_0",
                            "ratedTransaction": None,
                        },
                        {
                        ...
                        },
                        {
                            ...
                            "amountTax": 0e-12,
                            "amountWithTax": 0e-12,
                            "invoiceSubCategoryCode": ("ISCAT_SC_CONSUMPTION_VOICE_SC"),
                            "userAccountCode": "1808_0",
                            "ratedTransaction": None,
                        },
                    ],
        ...
        self.assertEqual(0.08, invoice.invoice_line_ids[0].price_subtotal)
        self.assertEqual(0.08, invoice.invoice_line_ids[0].price_unit)
        self.assertEqual(0.100000000000, invoice.invoice_line_ids[0].price_total)
        self.assertEqual(len(invoice.invoice_line_ids[0].invoice_line_tax_ids), 1)
        self.assertEqual(invoice.invoice_line_ids[0].invoice_line_tax_ids[0], self.tax)
        self.assertEqual(invoice.tax_line_ids[0].tax_id, self.tax)
        self.assertEqual(invoice.tax_line_ids[0].amount, 22.79)
        self.assertEqual(
    ...
    def test_route_right_force_totals(self):
        ...
        self.assertIn("id", content)
        invoice = self.env["account.invoice"].browse(content["id"])
        self.assertEqual(invoice.amount_tax, 0.22)
        self.assertEqual(invoice.amount_untaxed, 1.01)
        self.assertEqual(invoice.amount_total, 1.23)
        self.assertEqual(0.22, invoice.invoice_line_ids[0].price_tax)
        self.assertEqual(
            invoice.journal_id,
            self.browse_ref("somconnexio.consumption_invoices_journal"),
        )
