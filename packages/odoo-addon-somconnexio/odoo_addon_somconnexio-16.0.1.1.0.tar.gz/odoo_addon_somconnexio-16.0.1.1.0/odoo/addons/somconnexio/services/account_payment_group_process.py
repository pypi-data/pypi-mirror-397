...
class AccountPaymentGroupProcess:
    ...
    def process_payment_group(self, group_reference, raw_payment_group_lines):
        ...
        payment_group_vals = {
            ...
            "company_partner_bank_id": self.env.ref("base.main_company")
        ...
        for row in raw_payment_group_lines:
            ...
            self.env["account.payment.line"].create(payment_line_vals)
    ...
    def _get_partner(self, ref):
        ...
        if len(partner) > 1:
            raise UserError(_("More than one partner with reference {}.").format(ref))
        return partner[0]

    def _get_partner_bank(self, partner, iban):
        partner_bank = self.env["res.partner.bank"].search(
            [("partner_id", "=", partner.id), ("acc_number", "=", iban)], limit=1
        )
        if not partner_bank:
            raise UserError(
                _("Bank account with IBAN {} and Partner Ref {} not found.").format(
                    iban,
                    partner.ref,
                )
            )
    ...
    def _get_partner_bank_mandate(self, partner, partner_bank):
        ...
        if not mandate:
            raise UserError(
                _("SEPA mandate for IBAN {} and Partner Ref {} not found.").format(
