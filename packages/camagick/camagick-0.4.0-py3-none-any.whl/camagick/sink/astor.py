...
class Processor(SinkBase):
    ...
    def handle_shape_error(self, err, tag=None):
        if self._strict or not isinstance(err, RefuseShape):
            raise err
        logger.error(err)
        logger.error(f'msg="Ignoring shape error as instructed" tag={tag}')
    ...
    async def __call__(self, data=None, context=None):
        ...
        except (RefuseIndex, RefuseShape) as e:
            if isinstance(e, RefuseIndex):
                self.handle_index_error(e)
            elif isinstance(e, RefuseShape):
                self.handle_shape_error(e)
