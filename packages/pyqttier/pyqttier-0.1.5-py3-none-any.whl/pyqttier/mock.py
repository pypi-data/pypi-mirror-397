...
class MockConnection(IBrokerConnection):
    ...
    def simulate_message(self, message: Message) -> None:
        ...
        """
        self._logger.debug(f"Simulating incoming message on topic: {message.topic}")
        with self._lock:
    ...
    def publish(self, message: Message) -> Future:
        ...
        """
        self._logger.debug(f"Publishing message to topic: {message.topic}")
        with self._lock:
    ...
    def subscribe(self, topic: str, callback: Optional[MessageCallback] = None) -> int:
        ...
        """
        self._logger.debug(f"Subscribe to topic: {topic}")
        with self._lock:
