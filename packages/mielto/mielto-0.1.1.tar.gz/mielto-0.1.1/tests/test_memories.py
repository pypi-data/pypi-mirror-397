...
class TestMemories:
    ...
    def test_delete_memory(self, memories, mock_client):
        ...
        mock_client.delete.return_value = {"memory_id": "mem_123", "message": "Memory deleted successfully"}
    ...
    def test_from_messages(self, memories, mock_client):
        """Test generating memories from messages."""
        mock_client.post.return_value = {
            "memory": {
                "memory_id": "mem_123",
                "user_id": "user_123",
        ...
        result = memories.from_messages(messages=messages, user_id="user_123", topics=["test"])

        assert len(result) > 0
        # Should be called for each non-system message
