...
# Run tests without API keys (simulates GitHub Actions environment)
test-nokeys:
  env -u PERPLEXITY_API_KEY -u OPENAI_API_KEY -u CONSENSUS_API_KEY uv run pytest -m "not integration and not llm"
...
gh-add-topics:
  gh repo edit --add-topic "deep-research-client,monarchinitiative,linkml"
...
gh-invite-the-dragon:
  gh api repos/ai4curation/deep-research-client/collaborators/dragon-ai-agent -X PUT -f permission=push
