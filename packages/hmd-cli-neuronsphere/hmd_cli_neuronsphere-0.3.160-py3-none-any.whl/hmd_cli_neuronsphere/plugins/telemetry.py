...
def enabled(config_overrides: Dict[str, bool] = {}):
    val = os.environ.get("HMD_LOCAL_NEURONSPHERE_ENABLE_TELEMETRY")
    if val is not None:
        return val == "true"
    return config_overrides.get("telemetry", True)
...
def render_compose_yaml(
    ...
    compose_dict = {}
    with open(_services_dir / "docker-compose.telemetry.yml", "r") as dc:
        compose_dict = yaml.safe_load(dc)

    compose_path = cache_dir / "docker-compose.telemetry.yml"
