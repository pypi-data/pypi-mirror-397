...
def enabled(config_overrides: Dict[str, bool] = {}):
    val = os.environ.get("HMD_LOCAL_NEURONSPHERE_ENABLE_GRAPH")
    if val is not None:
        return val == "true"
    return config_overrides.get("graph", True)
...
def prepare_hmd_home(hmd_home: str, configs: Dict[str, bool] = {}):
    ...
    required_dirs = [Path("graph_db"), Path("graph_db", "logs")]
...
def render_compose_yaml(
    ...
    compose_dict = {}
    with open(_services_dir / "docker-compose.graph.yml", "r") as dc:
        compose_dict = yaml.safe_load(dc)

    compose_path = cache_dir / "docker-compose.graph.yml"
