...
def get_json_for_whisper_reviewer(instance, csv, output):
    ...
    with open(csv, 'r') as my_csv:
        ...
        for row in tqdm(reader, total=total_rows):
            ...
            if 'files' not in media_object_json:
                print(f"Missing media object: {row.get('media_object_id')}")
                all_missing.append(row.get('media_object_id'))
            for file in media_object_json.get('files', []):
                for filename in file['files']:
                    if 'medium' in filename['hls_url']:
                        ...
                        item = {
                            ...
                            "url": f"https://tamulib-dc-labs.github.io/edge-grant-json-and-vtts/json{row.get('path_to_json', '')}/{final}.json",
                            "vtt": f"https://tamulib-dc-labs.github.io/edge-grant-json-and-vtts/vtts{row.get('path_to_vtts', '')}/{final}.vtt",
                            "name": final,
                        }
                        all_items.append(item)

        with open('missing.txt', 'w') as my_file:
