...
class BatchClient:
    ...
    def download_results(
        ...
        Args:
            batch_id: 批量任务ID
            local_dir: 本地下载目录
            download_error_file: 是否同时下载错误文件（默认True）
        ...
        # 获取输出数据集路径
        output_dataset = detail.output_dataset
        if not output_dataset:
            raise ValueError(f"Batch task {batch_id} has no output_dataset configured")
        
        logger.info(f"Downloading results from {output_dataset} to {local_dir}")
        
        # 准备要下载的文件列表
        files_to_download = []
        
        # 添加输出文件（如果有）
        if detail.output_file_path:
            output_filename = Path(detail.output_file_path).name
            files_to_download.append(output_filename)
        
        # 添加错误文件（如果需要且存在）
        if download_error_file and detail.error_file_path:
            error_filename = Path(detail.error_file_path).name
            files_to_download.append(error_filename)
        ...
        try:
            if files_to_download:
                ...
                logger.info(f"Downloaded {len(files_to_download)} files to {local_dir}")
            else:
                # 如果没有指定文件，下载整个数据集
                self.asset.download_from_dataset(
                    dataset_path=output_dataset,
                ...
                logger.info(f"Downloaded all files from {output_dataset} to {local_dir}")
