# Python Client For Nutanix Monitoring APIs

The Python client for Nutanix Monitoring APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Alerts, Alert policies, Events and Audits.
## Features
...
api_client = ntnx_monitoring_py_client.ApiClient(allow_version_negotiation=False)
```
...
```sh
<my-env>/bin/pip install ntnx-monitoring-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-monitoring-py-client
```
...
## Configuration
The python client for Nutanix Monitoring APIs can be configured with the following parameters
...
try:
    api_response = system_defined_policies_api_instance.get_cluster_config_by_id(extId)
...
try:
    ...
    api_response = system_defined_policies_api_instance.update_cluster_config_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
# List alerts
try:
    api_response = alerts_api_instance.list_alerts(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=monitoring&version=v4.2&language=python). This documentation is auto-generated, and the location may change.
