...
class NavigationDrawer(Control):
    ...
    def main(page: mx.Page):
        ...
        def handle_dismissal(e):
            page.add(mx.Text("Drawer dismissed"))

        def handle_change(e):
            page.add(mx.Text(f"Selected Index changed: {e.selected_index}"))
            # page.close(drawer)

        drawer = mx.NavigationDrawer(
            on_dismiss=handle_dismissal,
            on_change=handle_change,
            controls=[
                mx.Container(height=12),
                mx.NavigationDrawerDestination(
                    label="Item 1",
                    icon=mx.icons.DOOR_BACK_DOOR_OUTLINED,
                    selected_icon_content=mx.Icon(mx.icons.DOOR_BACK_DOOR),
                ),
                mx.Divider(thickness=2),
                mx.NavigationDrawerDestination(
                    icon_content=mx.Icon(mx.icons.MAIL_OUTLINED),
                    label="Item 2",
                    selected_icon=mx.icons.MAIL,
                ),
                mx.NavigationDrawerDestination(
                    icon_content=mx.Icon(mx.icons.PHONE_OUTLINED),
                    label="Item 3",
                    selected_icon=mx.icons.PHONE,
                ),
            ],
        )

        page.add(mx.ElevatedButton("Show drawer", on_click=lambda e: page.open(drawer)))
    ...
    Online docs: https://mantarix.dev/docs/controls/navigationdrawer
    """
