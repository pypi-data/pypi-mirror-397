...
from mantarix.core.textfield import KeyboardType
...
class DatePicker(Control):
    ...
    def main(page):
        ...
        def handle_date_change(e: mx.ControlEvent):
            page.add(mx.Text(f"Date changed: {e.control.value.strftime('%Y-%m-%d %H:%M %p')}"))

        cupertino_date_picker = mx.CupertinoDatePicker(
            date_picker_mode=mx.CupertinoDatePickerMode.DATE_AND_TIME,
        ...
        page.add(
            mx.CupertinoFilledButton(
                ...
                on_click=lambda e: page.open(
                    mx.CupertinoBottomSheet(
                        cupertino_date_picker,
                        height=216,
                        padding=mx.padding.only(top=6),
                    )
                ),
            )
        )
    ...
    Online docs: https://mantarix.dev/docs/controls/datepicker
    """
