...
from mantarix.core.buttons import OutlinedBorder
...
class AlertDialog(AdaptiveControl):
    ...
    def main(page: mx.Page):
        ...
        dlg = mx.AlertDialog(
            title=mx.Text("Hi, this is a non-modal dialog!"),
            on_dismiss=lambda e: page.add(mx.Text("Non-modal dialog dismissed")),
        )

        def handle_close(e):
            page.close(dlg_modal)
            page.add(mx.Text(f"Modal dialog closed with action: {e.control.text}"))

        dlg_modal = mx.AlertDialog(
            modal=True,
            title=mx.Text("Please confirm"),
            content=mx.Text("Do you really want to delete all those files?"),
            actions=[
                mx.TextButton("Yes", on_click=handle_close),
                mx.TextButton("No", on_click=handle_close),
            ],
            actions_alignment=mx.MainAxisAlignment.END,
            on_dismiss=lambda e: page.add(
                mx.Text("Modal dialog dismissed"),
            ),
        )

        page.add(
            mx.ElevatedButton("Open dialog", on_click=lambda e: page.open(dlg)),
            mx.ElevatedButton("Open modal dialog", on_click=lambda e: page.open(dlg_modal)),
        )
    ...
    Online docs: https://mantarix.dev/docs/controls/alertdialog
    """
