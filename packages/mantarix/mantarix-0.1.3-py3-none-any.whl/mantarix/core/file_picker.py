...
class FilePicker(Control):
    ...
    def main(page: mx.Page):
        def pick_files_result(e: mx.FilePickerResultEvent):
            selected_files.value = (
                ", ".join(map(lambda f: f.name, e.files)) if e.files else "Cancelled!"
            )
            selected_files.update()

        pick_files_dialog = mx.FilePicker(on_result=pick_files_result)
        selected_files = mx.Text()
    ...
    Online docs: https://mantarix.dev/docs/controls/filepicker
    """
