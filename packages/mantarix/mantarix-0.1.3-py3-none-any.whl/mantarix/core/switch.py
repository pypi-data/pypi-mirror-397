...
class Switch(ConstrainedControl, AdaptiveControl):
    ...
    def main(page: mx.Page):
        def theme_changed(e):
            page.theme_mode = (
                mx.ThemeMode.DARK
                if page.theme_mode == mx.ThemeMode.LIGHT
                else mx.ThemeMode.LIGHT
            )
            c.label = (
                "Light theme" if page.theme_mode == mx.ThemeMode.LIGHT else "Dark theme"
            )
            page.update()

        page.theme_mode = mx.ThemeMode.LIGHT
        c = mx.Switch(label="Light theme", on_change=theme_changed)
        page.add(c)
    ...
    Online docs: https://mantarix.dev/docs/controls/switch
    """
