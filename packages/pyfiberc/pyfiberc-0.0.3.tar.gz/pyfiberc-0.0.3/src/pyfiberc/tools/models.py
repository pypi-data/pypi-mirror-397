from typing_extensions import Self, Any, List, Union
from pydantic import BaseModel, Field, field_validator, ConfigDict, model_validator
from typing import Optional, List
from pycivil.EXAStructural.materials import Concrete, ConcreteModel
...
class FiberConcrete(BaseModel):
    ...
    yc: float = Field(default=1.5, description="Coefficiente parziale calcestruzzo in compressione")
    ...
    # Tipo di Analisi
    tipo_analisi: Optional[List[TpAnalysis]] = Field(default=[TpAnalysis.TP_SECTIONAL, TpAnalysis.TP_FEM], description="Tipo di analisi (sezionale o FEM)")
    ...
    valutazione_precisa: Optional[TpValutation] = Field(default=None, description="Tipo di valutazione per Model Code")
...
def fromModel(model: FiberConcrete) -> FiberConcrete:
    """
    ...
    if model.ductility_class is not None:
