...
import ginsapy.giutility.buffer.postprocessbuffer_manager as postprocessbuffer_manager
import numpy as np
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("-i", "--SourceID", type=str, default="0a929f60-9536-11f0-b59d-2c58b9196ae7", help="Source ID (UUID)")
    parser.add_argument("-n", "--SourceName", type=str, default="python_udbf_buffer", help="Source name")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    result = conn.create_udbf_buffer(
        args.FilePath,
        args.SourceID,
        args.SourceName,
    ...
    sleep_ms = int(conn.frameBufferLength / float(args.SampleRateHz) * 1000)
    ns_per_sample = int(1_000_000_000 / float(args.SampleRateHz))
    nanos = np.uint64(timestamp_start_ns)

    try:
        while True:
            for frameindex in range(int(conn.frameBufferLength)):
                conn.write_timestamps(int(frameindex), int(nanos))
                nanos = np.uint64(nanos + ns_per_sample)
                value = float(np.random.random_sample() * 100.0)
                conn.write_to_framebuffer(int(frameindex), 0, value)
            conn.append_to_framebuffer()
            logging.info("wrote %d frames", int(conn.frameBufferLength))
            conn.buffer_sleep(sleep_ms)
