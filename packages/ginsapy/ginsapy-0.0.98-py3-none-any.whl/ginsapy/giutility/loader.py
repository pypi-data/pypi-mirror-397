...
def load_giutility() -> ctypes.CDLL:
    ...
    Load GiUtility with the following precedence (no relative paths):
      1) OS default absolute path
         ...
         - Linux:   /usr/lib/x86_64-linux-gnu/libGInsUtility.so
    ...
    if default_path:
        raise FileNotFoundError(
            ...
            f"  Linux (bash):          export GINS_GIUTILITY_PATH='/opt/gins/lib/libGInsUtility.so'\n"
        )
    ...
    raise FileNotFoundError(
        ...
        f"  Linux:            export GINS_GIUTILITY_PATH='/opt/gins/lib/libGInsUtility.so'\n"
