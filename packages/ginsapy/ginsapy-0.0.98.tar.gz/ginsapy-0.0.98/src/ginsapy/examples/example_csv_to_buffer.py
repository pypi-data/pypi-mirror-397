...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    sleep_ms = int(conn_stream.frameBufferLength / args.StreamSampleRate * 1000)
    ...
    try:
        while data_index < data_len:
            for frameindex in range(int(conn_stream.frameBufferLength)):
                ...
                conn_stream.write_timestamps(int(frameindex), int(nanos))
                if report_n and data_index and (data_index % report_n == 0):
                    logging.info("Progress: %d/%d", data_index, data_len)
                conn_stream.write_to_framebuffer(int(frameindex), 0, value)

            conn_stream.append_to_framebuffer()
            conn_stream.buffer_sleep(sleep_ms)
