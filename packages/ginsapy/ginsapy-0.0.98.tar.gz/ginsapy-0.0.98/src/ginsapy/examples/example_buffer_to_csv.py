...
import ginsapy.giutility.buffer.postprocessbuffer_client as postprocessbuffer_client
...
def list_available_buffers() -> None:
    ...
    try:
        conn_get_stream = postprocessbuffer_client.PostProcessBufferClient()
        ...
        for idx in range(count_buffer):
            name, buf_id = conn_get_stream.get_buffer_info(int(idx))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with out_path.open("w", encoding="utf-8", newline="") as f:
            generator = conn.yield_buffer_st()
            try:
                while True:
