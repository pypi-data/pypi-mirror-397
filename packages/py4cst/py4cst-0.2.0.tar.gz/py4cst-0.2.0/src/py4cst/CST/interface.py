...
class Interface(IQuietModeController):
    class StartMode(Enum):
        ...
        def to_cst(value):
            ...
            return getattr(DesignEnvironment.StartMode, value.value)

        @staticmethod
        def from_cst(value):
            return Interface.StartMode(value.name)
    ...
    def __init_cst_interface(self, start_mode: StartMode) -> None:
        from cst.interface import DesignEnvironment # type: ignore
        self.design_env = DesignEnvironment(mode=Interface.StartMode.to_cst(start_mode))
    ...
    def get_version() -> str:
        from cst.interface import DesignEnvironment # type: ignore
        return DesignEnvironment.version()
