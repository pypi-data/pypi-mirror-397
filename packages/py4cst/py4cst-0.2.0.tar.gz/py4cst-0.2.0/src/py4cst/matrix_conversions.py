...
def convert_s_to_s(s_mat, z_ref: complex, z_ref_new: complex) -> np.ndarray:
...
def convert_s_to_z(s_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
    E = np.eye(s_mat.shape[0])
    sqrt_z_ref_mat = E*np.sqrt(z_ref)
    return sqrt_z_ref_mat@np.linalg.solve(E-s_mat, E+s_mat)@sqrt_z_ref_mat

def convert_z_to_s(z_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
    E = np.eye(z_mat.shape[0])
    z_ref_mat = z_ref*E
    return np.linalg.solve(z_mat + z_ref_mat, z_mat - z_ref_mat)

def convert_s_to_y(s_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
    E = np.eye(s_mat.shape[0])
    return (E - s_mat) @ np.linalg.inv(z_ref * (E + s_mat))

def convert_y_to_s(y_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
    E = np.eye(y_mat.shape[0])
    return np.linalg.solve(E + z_ref*y_mat, E - z_ref*y_mat)

def convert_s_to_abcd(s_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
...
def convert_abcd_to_s(abcd_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
...
def convert_s_to_h(s_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
    s11, s12, s21, s22 = s_mat[0][0], s_mat[0][1], s_mat[1][0], s_mat[1][1]
...
def convert_h_to_s(h_mat, z_ref: complex = DEFAULT_Z_REF) -> np.ndarray:
