...
class Byneuron:
    ...
    def recent(self, round: str = 'h', offset: int = None):
        ...
        if not isinstance(round, str):
            return n
        d, p = (int(round[:-1]), round[-1]) if len(round) > 1 else (1, round)
        if p == 'm':
            ...
            if offset:
                n -= datetime.timedelta(hour=offset)
        elif p == 'd': # day of the week
            n = n.replace(hour=0, minute=0)
            # process as isoweekday if d and offset are both not None
            if offset is not None and bool(round in ['1d', '2d', '3d', '4d', '5d', '6d', '7d']):
                while n.isoweekday() != d:
                    n -= datetime.timedelta(day=1)
                if offset:
                    n -= datetime.timedelta(day=offset*7)
            else:
                n -= datetime.timedelta(day= d-1 if offset is None else offset)
    ...
    def create_from_definition(self, definition, inputs):
        ...
        def runDefinition(self, entityAPItype, id, inputs={}):
            '''
    ...
    def create_gateway(self, **kwargs):
        ...
        e = Entity().create('Gateway', self.indexset_active, **{k: v for k, v in device_config.items() if v is not None})
        return self.create_entity(e)
    ...
    def create_defition(self, type, external_id, data):
        ...
        :param label: externalId for entity; e.g. MY_DEFINITION_01;  name will be auto created as 'My Definition 01'
    ...
    def create_mastercode(self, codetype, code):
        ...
        :return:
        """
        e = Entity().create('Code', indexset=None, key=f'{codetype}#{code}', masterCodeType=codetype, masterCode=code)
        return self.create_entity(e)
    ...
    def create_code(self, codetype, code):
        ...
        :param parent_codetype: e.g. DEFAULT
        :return:
        """
