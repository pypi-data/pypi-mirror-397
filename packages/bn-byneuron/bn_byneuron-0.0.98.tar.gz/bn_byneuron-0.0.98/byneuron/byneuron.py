...
class Byneuron:
    ...
    def recent(self, round):
        n = self.now.replace(second=0, microsecond=0)
        if isinstance(round, str) and len(round) > 1:
            d, p = int(round[:-1]), round[-1]
            if p == 'm':
                ...
                for i in range(d - 1):
                    if n.month == 1:
                        n = n.replace(year=n.year-1, month=12)
                    else:
                        n = n.replace(month=n.month-1)
            elif p == 'Y':  # 1Y > round to start of year, 2Y > moves one more year
                n = n.replace(year=n.year - (d - 1), month=1, day=1, hour=0, minute=0)
        return n
        ...
        try:
            ...
            if name in ['name', 'externalId', 'publicId']:
                ...
                values = {"lat": float(value[0]),"lon": float(value[1])} \
                    if isinstance(value, list) and len(list) == 2 else {}
                attr = _attribute(name, values, 'ExtensionKeyword', 'geoPointValue')
