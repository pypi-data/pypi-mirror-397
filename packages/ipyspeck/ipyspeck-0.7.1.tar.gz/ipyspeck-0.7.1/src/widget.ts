...
export class SpeckView extends DOMWidgetView {
  ...
  switchColorSchema() {
    ...
    for (const color in speckColors) {
      if (first_color === undefined) first_color = color;
      if (update_color) {
    ...
    if (first_color) this.setColorSchema(first_color);
  }
  ...
  reflow() {
    ...
    if (ww === 0) ww = this.view.resolution.x;
    if (wh === 0) wh = this.view.resolution.y;
    if (this.view.resolution.x === ww && this.view.resolution.y === wh) return;
