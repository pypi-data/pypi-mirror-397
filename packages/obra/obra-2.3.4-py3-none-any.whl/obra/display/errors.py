...
ERROR_CODE_MAP: Dict[str, ErrorDisplay] = {
    "SESSION_NOT_FOUND": ErrorDisplay(
        ...
        recovery="Run 'obra derive' to start a new session.",
    ),
    ...
    "SERVER_ERROR": ErrorDisplay(
        ...
        recovery="Run 'obra derive --resume {session_id}' to continue.",
    ),
    ...
    "TERMS_NOT_ACCEPTED": ErrorDisplay(
        ...
        recovery="Run 'obra login' to accept terms.",
    ),
    ...
    "EXECUTION_TIMEOUT": ErrorDisplay(
        ...
        recovery="Try breaking the task into smaller steps.",
    ),
...
def display_error(exc: Exception, console: Optional[Console] = None) -> None:
...
def display_obra_error(error: ObraError, console: Optional[Console] = None) -> None:
