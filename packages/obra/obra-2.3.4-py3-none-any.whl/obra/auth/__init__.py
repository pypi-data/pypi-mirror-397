...
<body>
    ...
    def wait_for_callback(self, timeout: float = 300) -> Optional[dict]:
...
def get_current_auth() -> Optional[AuthResult]:
...
def ensure_valid_token() -> str:
    ...
    # In a production app, we'd check token expiration
    # For now, just return it - Firebase will reject if expired

    return auth.id_token


def verify_beta_access(id_token: str, api_base_url: Optional[str] = None) -> dict:
    ...
    try:
        ...
        if response.status_code == 403:
            ...
            elif error_code == "access_revoked":
                raise AuthenticationError(
