...
class HybridOrchestrator:
    ...
    def __init__(
        ...
        on_escalation: Optional[Callable[[EscalationNotice], UserDecisionChoice]] = None,
        ...
        self._llm_config: Optional[Dict[str, str]] = None
    ...
    def _action_to_phase(self, action: ActionType) -> Optional[SessionPhase]:
        ...
        if action not in self._handlers:
            ...
            if action == ActionType.DERIVE:
                ...
                self._handlers[action] = DeriveHandler(
                    self._working_dir, on_stream=self._on_stream, llm_config=self._llm_config
                )
                ...
                self._handlers[action] = ExecuteHandler(
                    self._working_dir, llm_config=self._llm_config
                )
                ...
                self._handlers[action] = FixHandler(self._working_dir)
            else:
    ...
    def _handle_action(self, server_action: ServerAction) -> Dict[str, Any]:
        ...
        if action == ActionType.DERIVE:
            ...
            return handler.handle(derive_req)
        elif action == ActionType.EXAMINE:
            ...
            review_req = ReviewRequest.from_payload(payload)
    ...
    def _handle_escalation(self, notice: EscalationNotice) -> Dict[str, Any]:
        ...
        console.print(f"  Blocking issues: {len(notice.blocking_issues)}")
        for issue in notice.blocking_issues[:5]:  # Show first 5
                        ...
                        "fix_details": [
                            {
                                ...
                                "summary": (fr.get("verification") or {}).get("summary", ""),
                            }
    ...
    def derive(
        ...
        Args:
            ...
            max_iterations: Maximum orchestration loop iterations (safety limit)
    ...
    def resume(self, session_id: str) -> CompletionNotice:
        ...
        # Get session state
        try:
            response = self._client._request("GET", f"session/{session_id}")
    ...
    def get_status(self, session_id: Optional[str] = None) -> ResumeContext:
        """Get session status.
