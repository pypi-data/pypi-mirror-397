...
class DatabricksPathKind(str, Enum):
    ...
    @classmethod
    def parse(cls, path: str) -> Tuple["DatabricksPathKind", Optional["Workspace"], str]:
        if path.startswith("/Workspace") or path.startswith("/Users") or path.startswith("/Shared"):
            return cls.WORKSPACE, None, path
        if path.startswith("/Volumes"):
            return cls.VOLUME, None, path
        ...
        if path.startswith("dbfs://"):
            ...
            workspace = Workspace(host=parsed.hostname)

            return kind, workspace, inner_path

        return cls.DBFS, None, path
