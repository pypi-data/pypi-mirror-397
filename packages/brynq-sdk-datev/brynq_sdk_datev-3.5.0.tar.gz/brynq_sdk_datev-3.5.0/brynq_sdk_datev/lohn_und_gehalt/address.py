...
from .schemas.address import AddressSchema, AddressUpdateSchema
from brynq_sdk_functions import Functions

class Address(BaseResource):
    """
    Address resource for DATEV Lohn und Gehalt
    """
    ...
    def get(self, employee_id: Optional[str] = None, reference_date: Optional[datetime] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get address data for employees

        Args:
            employee_id: Optional ID of a specific employee. If not provided, address data for all employees is returned.
        ...
        if employee_id:
            if self.debug:
                print(f"Fetching address data for employee {employee_id}")
            endpoint = f"clients/{client_id}/employees/{employee_id}/address"
        else:
            if self.debug:
                print("Fetching address data for all employees")
            endpoint = f"clients/{client_id}/address"
        ...
        valid_data, invalid_data = Functions.validate_data(df, AddressSchema, debug=self.debug)

        return valid_data, invalid_data

    def update(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Update address data for an employee

        Args:
            data: Updated address data, including employee_id and reference_date
        ...
        validated_data = self._validate_request_data(
            data=data,
            schema_class=AddressUpdateSchema
        )
        ...
        if self.debug:
            print(f"Updating address data for employee {employee_id}")

        # PUT request returns 204 No Content on success
        return self._make_request(f"clients/{client_id}/employees/{employee_id}/address",
                                method="PUT",
