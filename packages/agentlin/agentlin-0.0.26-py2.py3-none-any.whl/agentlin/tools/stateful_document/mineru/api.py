...
class MinerUClient:
    ...
    def __init__(
        ...
        """
        self.api_base = api_base or os.getenv("MINERU_API_BASE", "https://mineru.net")
        self.api_key = api_key or os.getenv("MINERU_API_KEY", "")
        self.LOG_DIR = log_dir or os.getenv("LOG_DIR", "output/logs")
        self.logger = logger or create_logger(log_dir, "mineru")
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        self.logger.debug(f"请求参数: {log_kwargs}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request(method, url, **kwargs) as response:
                ...
                self.logger.debug(f"API响应: {response_json}")
    ...
    async def submit_file_url_task(
        ...
        self.logger.info(f"开始处理 {len(urls_config)} 个文件URL")
        self.logger.debug(f"批量URL任务提交成功，批次ID: {batch_id}")
    ...
    async def submit_file_task(
        ...
        self.logger.info(f"开始上传 {len(file_urls)} 个本地文件")
        self.logger.debug(f"获取上传URL成功，批次ID: {batch_id}")
        ...
        for i, (file_config, upload_url) in enumerate(zip(files_config, file_urls)):
            ...
            try:
                with open(file_path, "rb") as f:
                    ...
                    self.logger.debug(f"文件 {file_path.name} 上传成功")
                    uploaded_files.append(file_path.name)
            except Exception as e:
                raise ValueError(f"文件 {file_path.name} 上传失败: {str(e)}")

        self.logger.info(f"文件上传完成，共 {len(uploaded_files)} 个文件")
    ...
    async def process_file_to_markdown(
        ...
        try:
            ...
            self.logger.debug(f"批量任务提交成功。Batch ID: {batch_id}")
            ...
            for i in range(max_retries):
                ...
                self.logger.debug(f"轮训结果：{status_info}")

                if (
                    "data" not in status_info
                    or "extract_result" not in status_info["data"]
                ):
                    self.logger.error(f"获取批量任务状态失败: {status_info}")
                ...
                for result in status_info["data"]["extract_result"]:
                    ...
                    if state == "done":
                        ...
                        self.logger.warning(f"文件 {file_name} 处理失败: {err_msg}")
                        # 不抛出异常，继续处理其他文件
                    else:
                        all_done = False
                        # 显示进度信息
                ...
                # 如果没有进度信息，只显示简单的等待消息
                if not has_progress:
                    self.logger.info(f"等待文件处理完成... ({i+1}/{max_retries})")
            ...
            for file_name, download_url in files_download_urls.items():
                try:
                    self.logger.debug(f"下载文件处理结果: {file_name}")
                    ...
                    self.logger.error(f"文件 {file_name} {error_msg}")
                    results.append(
                        {
                            "filename": file_name,
                            "status": "error",
            ...
            self.logger.info(f"处理失败: {fail_count}")

            if failed_files:
                self.logger.info("\n失败文件详情:")
                for file_name, error_msg in failed_files.items():
                    self.logger.info(f"  - {file_name}: {error_msg}")
            ...
            self.logger.error(f"处理 File 到 Markdown 失败: {str(e)}")
            raise
