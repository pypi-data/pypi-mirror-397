...
# 转换后文件的默认输出目录
DEFAULT_OUTPUT_DIR = os.getenv("OUTPUT_DIR", "./output/document")
...
async def convert_file_path(
    ...
    elif isinstance(file_path, list) and len(file_path) > 0 and isinstance(file_path[0], dict):
        # 文件配置字典列表
        files_to_process = file_path
...
async def local_parse_file(
    ...
    try:
        ...
        if USE_LOCAL_API:
            logger.debug(f"使用本地API: {LOCAL_MINERU_API_BASE}")
...
async def read_converted_file(
    ...
    try:
        ...
        if not target_file.exists():
            ...
            # 递归搜索所有子目录下的同后缀文件
            similar_files_paths = [str(f) for f in parent_dir.rglob(f"*{suffix}") if f.is_file()]
...
async def find_and_read_markdown_content(result_path: str) -> Dict[str, Any]:
    ...
    logger.debug(f"找到 {len(possible_files)} 个可能的文件")
    ...
    for file_path in possible_files:
        if file_path.exists():
            ...
            if result["status"] == "success":
                logger.debug(f"成功读取文件内容: {file_path}")
                found_contents.append({"file_path": str(file_path), "content": result["content"]})

    # 如果找到了文件内容
    if found_contents:
        logger.debug(f"在结果目录中找到了 {len(found_contents)} 个可读取的文件")
...
async def _process_conversion_result(result: Dict[str, Any], source: str, is_url: bool = False) -> Dict[str, Any]:
    """
    处理转换结果，统一格式化输出。
    ...
    if result["status"] == "success":
        ...
        # 记录调试信息
        logger.debug(f"处理结果 result_path 类型: {type(result_path)}")

        if result_path:
            # 情况1: result_path是字典且包含results字段（批量处理结果）
            if isinstance(result_path, dict) and "results" in result_path:
                logger.debug("检测到批量处理结果格式")

                # 查找与当前源文件匹配的结果
                for item in result_path.get("results", []):
                    if item.get("filename") == filename or (not is_url and Path(source).name == item.get("filename")):
                ...
                if "extract_dir" in result_path:
                    logger.debug(f"尝试从extract_dir读取: {result_path['extract_dir']}")
                    try:
                        content_result = await find_and_read_markdown_content(result_path["extract_dir"])
                        ...
                        logger.error(f"从extract_dir读取内容时出错: {str(e)}")
            ...
            # 情况2: result_path是字符串（传统格式）
                ...
                logger.debug(f"处理传统格式结果路径: {result_path}")
            ...
            # 情况3: result_path是其他类型的字典（尝试处理）
                ...
                logger.debug(f"处理其他字典格式: {result_path}")
                # 尝试从字典中提取可能的路径
                extract_path = result_path.get("extract_dir") or result_path.get("path") or result_path.get("dir")
                if extract_path and isinstance(extract_path, str):
                    try:
                        ...
                        logger.error(f"从extract_path读取内容时出错: {str(e)}")

                # 如果没有找到有效路径，返回错误
                base_result.update({"status": "error", "error_message": "转换结果格式无法识别"})
            else:
            ...
            base_result.update({"status": "error", "error_message": "转换成功但未返回结果路径"})
    else:
        base_result.update({"status": "error", "error_message": result.get("error", "未知错误")})
...
async def parse_documents(
    ...
    output_dir: Annotated[
        str,
        Field(
            description=f"""转换后文件的输出目录，默认"{DEFAULT_OUTPUT_DIR}"。如果目录不存在会自动创建。""",
        ),
    ] = DEFAULT_OUTPUT_DIR,
    # 通用参数
    enable_ocr: Annotated[bool, Field(description="启用OCR识别,默认False")] = False,
    language: Annotated[str, Field(description='文档语言，默认"ch"中文，可选"en"英文等')] = "ch",
    # 远程API参数
    page_ranges: Annotated[
        str | None,
        Field(description='指定页码范围，格式为逗号分隔的字符串。例如："2,4-6"：表示选取第2页、第4页至第6页；"2--2"：表示从第2页一直选取到倒数第二页。（远程API）,默认None'),
    ] = None,
) -> Dict[str, Any]:
    """
    ...
    logger.debug(f"去重后的文件路径: {sources}")

    # 记录去重信息
    original_count = len(parse_list_input(file_sources))
    unique_count = len(sources)
    if original_count > unique_count:
        logger.debug(f"检测到重复路径，已自动去重: {original_count} -> {unique_count}")
    ...
    if USE_LOCAL_API:
        ...
        logger.info(f"使用本地API处理 {len(file_paths)} 个文件")
        ...
        for path in file_paths:
            try:
                ...
                result = await local_parse_file(
                    file_path=path,
                    parse_method=("ocr" if enable_ocr else "txt"),  # 如果启用OCR，使用ocr，否则使用txt
                )
                ...
                logger.error(f"处理文件 {path} 时出现错误: {str(e)}")
                results.append(
                    {
                        "filename": Path(path).name,
                        "source_path": path,
        ...
        if url_paths:
            logger.info(f"使用远程API处理 {len(url_paths)} 个文件URL")
            ...
            try:
                ...
                if url_result["status"] == "success":
                    ...
                    for url in url_paths:
                        result_item = await _process_conversion_result(url_result, url, is_url=True)
                ...
                logger.error(f"处理URL时出现错误: {str(e)}")
                for url in url_paths:
                    results.append(
                        {
                            "filename": url.split("/")[-1].split("?")[0],
        ...
        if file_paths:
            logger.info(f"使用远程API处理 {len(file_paths)} 个本地文件")
                ...
                try:
                    ...
                    if file_result["status"] == "success":
                        ...
                        for file_path in existing_files:
                            result_item = await _process_conversion_result(file_result, file_path, is_url=False)
                            results.append(result_item)
                    else:
                        # 转换失败，为所有文件添加错误结果
                        for file_path in existing_files:
                    ...
                    logger.error(f"处理本地文件时出现错误: {str(e)}")
                    for file_path in existing_files:
                        results.append(
                            {
                                "filename": Path(file_path).name,
...
async def _parse_file_local(
    ...
    """
    # API URL路径
    api_url = f"{LOCAL_MINERU_API_BASE}/file_parse"
    ...
    form_data.add_field("file", file_data, filename=file_path_obj.name, content_type=file_type)
    form_data.add_field("parse_method", parse_method)

    logger.debug(f"发送本地API请求到: {api_url}")
    logger.debug(f"上传文件: {file_path_obj.name} (大小: {len(file_data)} 字节)")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(api_url, data=form_data) as response:
                if response.status != 200:
                    ...
                    logger.error(f"API返回错误状态码: {response.status}, 错误信息: {error_text}")
                    raise RuntimeError(f"API返回错误: {response.status}, {error_text}")

                result = await response.json()

                logger.debug(f"本地API响应: {result}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", "-o", type=str, help="保存转换后文件的目录 (默认: ./downloads)")
    parser.add_argument("--port", "-p", type=int, default=8001, help="服务器端口 (默认: 8001, 仅在使用HTTP协议时有效)")
    parser.add_argument("--host", type=str, default="0.0.0.0", help="服务器主机地址 (默认: 0.0.0.0, 仅在使用HTTP协议时有效)")
