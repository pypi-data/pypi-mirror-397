...
def show_status():
    ...
    try:
        ...
        if active_profile and active_profile.mcp_profile:
            ...
            current_mcp = _detect_current_mcp_profile(config)
...
def _detect_current_mcp_profile(config) -> Optional[str]:
    """
    Detect which MCP profile is currently active.

    This is a helper function to determine the current MCP configuration.
    Returns the profile name that best matches the current MCP servers.

    Args:
        config: OrchestratorConfig instance.

    Returns:
        Name of detected MCP profile, or None.
    """
    settings = config.settings
