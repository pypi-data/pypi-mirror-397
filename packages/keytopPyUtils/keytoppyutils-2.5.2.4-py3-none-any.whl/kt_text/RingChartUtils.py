...
class RingChartUtils:
    def __init__(self, param):
        ...
        if isinstance(self.values, str):
            # 尝试解析字符串格式的数组，如 '[100,200,345,120]'
            try:
                # 移除方括号并按逗号分割
                values_str = self.values.strip('[]')
                self.values = [float(val.strip()) for val in values_str.split(',')]
            except (ValueError, AttributeError):
                raise Exception(f"饼图数值格式错误：无法将字符串 '{self.values}' 转换为数值数组")
        elif not isinstance(self.values, (list, tuple)):
            raise Exception(f"饼图数值格式错误：values必须是数组或字符串格式的数组，当前类型为 {type(self.values)}")
        ...
        if isinstance(self.colors, str):
            ...
            try:
                # 移除方括号并按逗号分割
                colors_str = self.colors.strip('[]')
                self.colors_str = [val.strip() for val in colors_str.split(',')]
            except (ValueError, AttributeError):
                raise Exception(f"饼图颜色格式错误：无法将字符串 '{self.colors}' 转换为颜色数组")
        elif not isinstance(self.colors, (list, tuple)):
            raise Exception(f"饼图颜色格式错误：colors必须是数组或字符串格式的数组，当前类型为 {type(self.colors)}")
