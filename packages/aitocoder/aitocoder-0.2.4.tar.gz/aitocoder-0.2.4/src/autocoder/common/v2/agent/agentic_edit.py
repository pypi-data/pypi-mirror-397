...
from autocoder.common.threaded_llm_streamer import ThreadedLLMStreamer
# New add
...
class AgenticEdit:
    ...
    def analyze(self, request: AgenticEditRequest) -> Generator[
        ...
        while True:
            ...
            # Wrap the LLM generator in ThreadedLLMStreamer for responsive cancellation.
            # This allows checking for Ctrl+C every 100ms instead of waiting for network I/O.
            llm_streamer = ThreadedLLMStreamer(
                cancel_token=self.cancel_token,
                queue_timeout=0.1  # Check cancellation every 100ms
            )
            llm_streamer.start(llm_response_gen)
            ...
            parsed_events = self.stream_and_parse_llm_response(
                llm_streamer.iter_chunks(), agentic_context
            )
            ...
            # Ensure the LLM streamer thread is cleaned up after the for loop
            llm_streamer.stop()
