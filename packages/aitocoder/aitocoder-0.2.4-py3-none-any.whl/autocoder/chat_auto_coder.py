...
from autocoder.common.interrupt_handler import (
    InterruptHandler,
    set_interrupt_handler,
    trigger_cancellation_from_keybinding,
)
# NEW addition
...
async def run_app():
    ...
    # 创建并安装中断处理器
    interrupt_handler = InterruptHandler(task_event=task_event)
    ...
    async def consumer_loop():
        ...
        while True:
            ...
            try:
                ...
                task_event.set_state("completed")
                # Reset all active tokens to allow new tasks
                global_cancel.reset_active_tokens()
                continue
            except CancelRequestedException:
                print("\n\033[93m⚠️ Operation cancelled\033[0m")
                task_event.set_state("completed")
                # Reset all active tokens to allow new tasks
                global_cancel.reset_active_tokens()
                continue
    ...
    def _(event):
        ...
        elif task_event.is_running():
            # Task is running - trigger cancellation
            trigger_cancellation_from_keybinding()
        else:
            # No task running - clear current input line
            event.app.current_buffer.reset()
    ...
    def get_bottom_toolbar():
        ...
        # 获取当前模型
        current_model = memory_manager.get_config("model", "N/A")
        ...
        # Return FormattedText for transparent background styling
        return FormattedText([
            ("class:toolbar-label", "│ Dir: "),
            ("class:toolbar-value", pwd),
            ("class:toolbar-label", async_tasks_info),
            ("", "\n"),
            ("class:toolbar-label", "│ Model: "),
            ("class:toolbar-model", current_model),
        ])
    ...
    style = Style.from_dict(
        {
            ...
            "busy": "#ff6600 italic",
            # Transparent bottom toolbar styles (override default reverse video)
            "bottom-toolbar": "noreverse",
            "bottom-toolbar.text": "#888888",
            "toolbar-label": "#666666",
            "toolbar-value": "#00aaaa",
            "toolbar-model": "#aa8800",
            "toolbar-async": "#00aa00",
        }
    )
...
def main():
    ...
    # Initialize global plug-in directories
