...
def run_fastq_quality_control(
        ...
        trim_tail_1: int = 0,
        trim_tail_2: int = 0
    ) -> run_fastq_quality_control_outputs:
    return run_fastq_quality_control_outputs(
...
def run_genome_mapping(
        ...
        sample_name: str,
        genome: str = "human"
    ) -> run_genome_mapping_outputs:
    return run_genome_mapping_outputs(
...
def run_calculate_expression(
        ...
        genome: str = "human",
        method: str = "m1"
    ) -> run_calculate_expression_outputs:
    return run_calculate_expression_outputs(
        tpm_file = f"/output/result/expression/TPM.txt",
        count_file = f"/output/result/expression/Count.txt"
    )
...
def run_deg(
    ...
    return run_deg_outputs(
        deg_file = f"/output/result/deg/deg_result.txt"
    )
...
def run_chromosome_diagnosis(
        ...
        genome: str = "human",
        all_chr: bool = True
    ) -> run_chromosome_diagnosis_outputs:
    return run_chromosome_diagnosis_outputs(
...
def run_tumor_diagnosis(
        ...
        genome: str = "human",
        cutoff_tpm: float = 1
    ) -> run_tumor_diagnosis_outputs:
    return run_tumor_diagnosis_outputs(
...
def run_donor_diagnosis(
    ...
    ) -> run_donor_diagnosis_outputs:
    return run_donor_diagnosis_outputs(
...
def run_cell_tracing(
        ...
        genome: str = "human",
        method: str = "m1"
    ) -> run_cell_tracing_outputs:
    return run_cell_tracing_outputs(
        cell_tracing_file = f"/output/result/cell_tracing/cell_tracing_result.txt"
    )
...
def run_find_biomarker(
    ...
    ) -> run_find_biomarker_outputs:
    return run_find_biomarker_outputs(
        biomarker_file = f"/output/result/biomarker/biomarker_result.txt"
    )
...
async def mcp_server() -> None:
    ...
    async def list_tools() -> list[Tool]:
        return [
            ...
            Tool(
                name=scrna_mcp_tools.run_chromosome_diagnosis,
                description=""" 染色体诊断：检查样品是否患有染色体异常疾病 """,
                inputSchema=run_chromosome_diagnosis_inputs.model_json_schema(),
            ),
            Tool(
                name=scrna_mcp_tools.run_tumor_diagnosis,
                description=""" 肿瘤诊断：检查样品是否包含肿瘤片段 """,
                inputSchema=run_tumor_diagnosis_inputs.model_json_schema(),
            ),
            Tool(
                name=scrna_mcp_tools.run_donor_diagnosis,
                description=""" 供体诊断：检查器官移植是否发生排斥反应及其反应程度 """,
                inputSchema=run_donor_diagnosis_inputs.model_json_schema(),
            ),
            Tool(
                name=scrna_mcp_tools.run_cell_tracing,
                description=""" 细胞溯源分析：分析样品中不同类型的细胞比例 """,
                inputSchema=run_cell_tracing_inputs.model_json_schema(),
            ),
            Tool(
                name=scrna_mcp_tools.run_find_biomarker,
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        match name:
            case scrna_mcp_tools.run_fastq_quality_control:
            ...
            case scrna_mcp_tools.run_genome_mapping:
            ...
            case scrna_mcp_tools.run_calculate_expression:
            ...
            case scrna_mcp_tools.run_chromosome_diagnosis:
                return run_chromosome_diagnosis(
                    tpm_file = arguments["tpm_file"]
                )

            case scrna_mcp_tools.run_tumor_diagnosis:
                return run_tumor_diagnosis(
                    tpm_file = arguments["tpm_file"]
                )

            case scrna_mcp_tools.run_donor_diagnosis:
                return run_donor_diagnosis(
                    tpm_file = arguments["tpm_file"]
                )

            case scrna_mcp_tools.run_cell_tracing:
                return run_cell_tracing(
                    tpm_file = arguments["tpm_file"]
                )

            case scrna_mcp_tools.run_find_biomarker:
