...
陌香快应用接口说明：https://doc.weixin.qq.com/doc/w3_AIgARAadAMMHTa6HFymQJ6sCH4SF6?scode=AFoA2wfSABEb32pl87AGgAigZTABE
快应用独立包接口说明：https://doc.weixin.qq.com/doc/w3_AIgARAadAMMGRrJi1MKTxaRLF4yMf
微信分销接口说明：https://doc.weixin.qq.com/doc/w3_AIgARAadAMMV2g61jwjSba2zEekCJ?scode=AFoA2wfSABEfFC3J8oAGgAigZTABE
...
class Basics:
    ...
    def get_app_list(
            self,
            coop_type: str,
            start_time: int = 1546358400,  # 2019-01-02 00:00:00
            end_time: int = None,
            page: int = 1,
        ...
        """
        获取产品列表
        :param coop_type: 业务类型，wechat/quickapp/moxiang
        ...
        url = f'https://{self.host}/{coop_type}/GetAppList'
        data = {
        ...
        response = lazyrequests.lazy_requests(
            url=url,
            method='GET',
            params=self.make_sign(data=data),
            return_json=True
        )
        return response
    ...
    def query_consume_log(
        ...
        if int(self.coop_type) == 1:
            url = f'https://{self.host}/cpapi/WxConsume/QueryConsumeLog'
        elif int(self.coop_type) == 9:
            url = f'https://{self.host}/cpapi/WxConsume/QuickAppQueryConsumeLog'
    ...
    def query_user_info(
        ...
        if self.coop_type == 1:
            url = f'https://{self.host}/cpapi/WxUserInfo/QueryUserInfo'
        elif self.coop_type == 9:
            url = f'https://{self.host}/cpapi/WxUserInfo/QuickAppQueryUserInfo'
        elif self.coop_type == 11:
            url = f'https://{self.host}/cpapi/WxUserInfo/QuickAppFbQueryUserInfo'
        else:
            return
        data = {
            'start_time': start_time,
    ...
    def get_novel_info(
        ...
        :param app_flag: 【必填】产品标识（可从后台>>合作产品管理 >> 业务管理获取），每次请求仅能传一个。
        :param book_id: 【必填】作品唯一标识
        ...
        """
        url = f'https://{self.host}/wechat/Book/GetNovelInfo'
        data = {
            'book_id': book_id,
            'appflag': app_flag
        }
    ...
    def get_free_chapter_list(
        ...
        """
        url = f'https://{self.host}/wechat/Book/GetFreeChapterList'
        data = {
            'book_id': book_id,
            'appflag': app_flag
        }
    ...
    def add_h5_spread(
            ...
            cost: float = None,
            wx_work_force_style: int = None,
            wx_work_force_chapter: str = None
    ):
        """
        4.7、创建作品推广链接
        注：
        ...
        :param app_flag: 【必填】产品标识（可从后台>>合作产品管理 >> 业务管理获取），每次请求仅能传一个。
        :param book_id: 【必填】作品唯一标识
        :param chapter_id: 【必填】章节 id
        :param name: 【必填】渠道名称
        :param channel_type: 推广类型 1 外部 2 内部
        :param force_style: 强关设置 1 不设置强关 2 主动关注 3 强制关注
        :param bottom_qr: 底部关注：1是，0否
        :param force_chapter: 强关章节序号
        :param cost: 推广成本(保留小数点后两位)
        :param wx_work_force_style: 企微强关设置：1不设置强关，2 主动关注，3 强制关注
        :param wx_work_force_chapter: 企微强关章节序号（企微强关设置传值时，此值必传）
        ...
        返回值：
            ...
            chapter_title String 章节名
        """
        url = f'https://{self.host}/wechat/Book/AddH5Spread'
        data = {
            'appflag': app_flag,
            'book_id': book_id,
        ...
        if cost is not None:
            data['cost'] = cost
        if wx_work_force_style is not None:
            data['wxwork_force_style'] = wx_work_force_style
        if wx_work_force_chapter is not None:
            data['wxwork_force_chapter'] = wx_work_force_chapter
        if force_chapter is not None:
            data['force_chapter'] = force_chapter

        response = lazyrequests.lazy_requests(
            url=url,
            method='POST',
            data=self.make_sign(data=data),
            return_json=True
        )
        return response
    ...
    def add_page_spread(
        ...
        """
        url = f'https://{self.host}/cpapi/wxNovel/addPageSpread'
        data = {
            'appflags': app_flags,
            'name': name,
            'channel_type': channel_type,
    ...
    def add_quick_spread(
        ...
        返回值：
            ...
            url_id Int 链接 id
        """
        url = f'https://{self.host}/cpapi/wxNovel/addQuickSpread'
        data = {
            'appflags': app_flags,
            'cbid': cbid,
            'ccid': ccid,
    ...
    def add_recharge_template(
        ...
        返回值：
            ...
            channel_url String 页面推广链接
        """
        url = f'https://{self.host}/cpapi/wxActivity/addRechargeTemplate'
        data = {
            'appflags': app_flags,
    ...
    def query_charge_log(
        ...
        url = f'https://{self.host}/{coop_type}/Recharge/GetRechargeLog'
