...
class Yuewen:
    ...
    def get_user_coops(
        ...
        获取所有和合作类型
        """
        url = 'https://open.yuewen.com/api/account/getUserCoops'
        headers = copy.deepcopy(self.headers)
        return lazyrequests.lazy_requests(
            method="GET",
            url=url,
    ...
    def get_user_apps(
        ...
        获取所有app
        """
        url = 'https://open.yuewen.com/api/account/getUserApps'
        headers = copy.deepcopy(self.headers)
        return lazyrequests.lazy_requests(
            method="GET",
            url=url,
    ...
    def wechat_spread__page_spread(
            self,
            referer: str = "https://open.yuewen.com/new/promotion/pagePromotion"
    ):
        """
        获取微信分销H5新建推广链接时的所有页面链接
        【推广运营】/【页面推广】/【新增推广链接】
        """
        url = 'https://open.yuewen.com/api/wechatspread/pageSpread'
        headers = copy.deepcopy(self.headers)
        if referer:
    ...
    def get_active_info(
            self,
            referer: str = "https://open.yuewen.com/new/dashboard"
    ):
        """
        获取 当前app的id信息
        """
        url = 'https://open.yuewen.com/api/Account/getActiveInfo'
        headers = copy.deepcopy(self.headers)
        if referer:
            headers['Referer'] = referer
        return lazyrequests.lazy_requests(
    ...
    def promotion_activity_template(
            self,
            page: int = 1,
            business: int = 1,
            referer: str = "https://open.yuewen.com/new/promotion/promotionActivity/templateActivity"
    ):
        """
        获取链接
        【推广运营】/【促销活动】/【模版充值活动】
        """
        url = 'https://open.yuewen.com/api/WechatActivity/templateActivity'
        headers = copy.deepcopy(self.headers)
        if referer:
            headers['Referer'] = referer
        params = {
    ...
    def add_page_spread(
        ...
        """
        获取链接
        【推广运营】/【促销活动】/【模版充值活动】/【推广】创建链接
        :param sub_id: 活动id
        :param page_name: 推广页面入口（活动名称）
        :param name: 渠道名称
        :param channel_type: 内外推类别，1:微信外推广，2:微信内推广
        :param referer:
        """
        url = 'https://open.yuewen.com/api/wechatspread/addPageSpread'
        headers = copy.deepcopy(self.headers)
        if referer:
            headers['Referer'] = referer

        if not name:
            name = lazytime.get_datetime()

        data = {
            "page": "actRecharge",
            "name": name,  # 渠道名称
            "cost": 0,
            "channel_type": channel_type,  # 内外推类别，1:微信外推广，2:微信内推广
            "page_name": page_name,  # 推广页面入口（活动名称）
            "param": f"subid={sub_id}",  # 活动id
            "ranktype": "",
            "type": 5
        }
        return lazyrequests.lazy_requests(
