...
if TYPE_CHECKING:
    from qblox_instruments.qcodes_drivers.io_channel_qsm import QSMIOChannelConfig
...
class Cluster(ClusterScpi):
    ...
    # ------------------------------------------------------------------------
    set_io_channel_config = _set_io_channel_config  # Make the method "public"
    ...
    # ------------------------------------------------------------------------
    get_io_channel_config = _get_io_channel_config  # Make the method "public"
    ...
    # -------------------------------------------------------------------------
    def set_qsm_config(self, slot: int, configs: list["QSMIOChannelConfig"]) -> None:
        """
        Sets the configuration for the QSM for all IO channels.
        ...
        Exception
            RuntimeError
        """
        check_is_valid_type(self._is_qsm_type(slot=slot))
        # Check config against one of the IO channels
        # because parameters are only declared at class instantiation.
        for config in configs:
            self.modules[slot - 1].io_channels[0]._validate_config(config)
        self._set_qsm_output_config(slot=slot, config=configs)
    ...
    # -------------------------------------------------------------------------
    set_qsm_channel_to_zero = _set_qsm_channel_to_zero  # Make the method "public"
