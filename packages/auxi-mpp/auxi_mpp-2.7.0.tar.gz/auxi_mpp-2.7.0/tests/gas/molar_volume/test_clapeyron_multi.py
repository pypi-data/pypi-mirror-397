...
from auxi.mpp.gas.Vm import ClapeyronBinary, ClapeyronMulti
...
def test_clapeyron_multi(temperature: float, pressure: float, composition: dict[str, float]):
    """Test temperature and composition limits."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        model = ClapeyronMulti()
...
def test_clapeyron_multi_errors(temperature: float, pressure: float, composition: dict[str, float]):
...
def test_mv_binary_vs_multi(temperature: float, pressure: float, composition: dict[str, float]):
    """Test if the binary and multi model agrees."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        binary_model = ClapeyronBinary()
        multi_model = ClapeyronMulti()
...
def test_mv_multi3_vs_multi7(temperature: float, pressure: float, composition: dict[str, float]):
    """Test if the multi model agrees when three and seven components are specified."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        multi3_model = ClapeyronMulti()
        multi7_model = ClapeyronMulti()
...
def test_clapeyron_multi_warning():
    """Test if a warning is raised for calling the class."""
