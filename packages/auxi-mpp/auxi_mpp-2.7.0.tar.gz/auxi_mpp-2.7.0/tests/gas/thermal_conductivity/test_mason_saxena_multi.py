...
from auxi.mpp.gas.κ import MasonSaxenaBinary, MasonSaxenaMulti
...
def test_mason_saxena_multi(temperature: float, pressure: float, composition: dict[str, float]):
    """Test temperature and composition limits."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        model = MasonSaxenaMulti()
...
def test_mason_saxena_multi_errors(temperature: float, pressure: float, composition: dict[str, float]):
...
def test_tc_binary_vs_multi(temperature: float, pressure: float, composition: dict[str, float]):
    """Test if the binary and multi model agrees."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")

        binary_model = MasonSaxenaBinary()
        multi_model = MasonSaxenaMulti()
...
def test_tc_multi3_vs_multi7(temperature: float, pressure: float, composition: dict[str, float]):
    """Test if the multi model agrees when three and seven components are specified."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")

        multi3_model = MasonSaxenaMulti()
        multi7_model = MasonSaxenaMulti()
...
def test_mason_saxena_multi_warning():
    """Test if a warning is raised for calling the class."""
    expected_msg = r"MasonSaxenaMulti model logic is validated for binary systems only. Application to multi-component systems is untested and may yield inaccurate results\."
