...
class PersonalFilter(BaseModel):
    """
    A user-specific filter. The source dataset must contain user and item identifiers.
    """ # noqa: E501
    user_id_column: Optional[StrictStr] = Field(default='user_id', description="Column name for user identifiers.")
    item_id_column: Optional[StrictStr] = Field(default='item_id', description="Column name for item identifiers.")
    index_type: Optional[FilterIndexType] = Field(default=None, description="Index type for fast filter lookups.")
    type: Optional[StrictStr] = Field(default='personal', description="Filter type discriminator.")
    __properties: ClassVar[List[str]] = ["user_id_column", "item_id_column", "index_type", "type"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['personal']):
            raise ValueError("must be one of enum values ('personal')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PersonalFilter from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PersonalFilter from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "user_id_column": obj.get("user_id_column") if obj.get("user_id_column") is not None else 'user_id',
            "item_id_column": obj.get("item_id_column") if obj.get("item_id_column") is not None else 'item_id',
