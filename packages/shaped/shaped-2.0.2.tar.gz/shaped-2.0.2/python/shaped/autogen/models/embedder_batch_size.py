...
EMBEDDERBATCHSIZE_ANY_OF_SCHEMAS = ["TunableInt", "int"]

class EmbedderBatchSize(BaseModel):
    """
    Batch size for the embedder model (e.g., BERT).
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = EmbedderBatchSize.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in EmbedderBatchSize with anyOf schemas: TunableInt, int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into EmbedderBatchSize with anyOf schemas: TunableInt, int. Details: " + ", ".join(error_messages))
        else:
            return instance
