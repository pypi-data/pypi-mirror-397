...
FACTORS1_ANY_OF_SCHEMAS = ["TunableInt", "int"]

class Factors1(BaseModel):
    """
    Number of latent factors in the matrix factorization.
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Factors1.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Factors1 with anyOf schemas: TunableInt, int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Factors1 with anyOf schemas: TunableInt, int. Details: " + ", ".join(error_messages))
        else:
            return instance
