...
class BoostedReorderStep(BaseModel):
    """
    BoostedReorderStep
    """ # noqa: E501
    retriever: Retriever
    strength: Optional[Union[StrictFloat, StrictInt]] = Field(default=0.5, description="Boost strength (0.0-1.0). Closer to 1.0 pulls more items from boosted retriever (items with boost=1).")
    name: Optional[StrictStr] = None
    type: Optional[StrictStr] = Field(default='boosted', description="Reorder step type discriminator.")
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['boosted']):
            raise ValueError("must be one of enum values ('boosted')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BoostedReorderStep from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BoostedReorderStep from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "retriever": Retriever.from_dict(obj["retriever"]) if obj.get("retriever") is not None else None,
            "strength": obj.get("strength") if obj.get("strength") is not None else 0.5,
            "name": obj.get("name"),
            "type": obj.get("type") if obj.get("type") is not None else 'boosted'
        })
