...
class ReferenceTableConfig(BaseModel):
    """
    Table config that references a table by name with column selection and optional where clause. Limited to single table queries (no joins).
    """ # noqa: E501
    type: Optional[StrictStr] = Field(default='table', description="Type discriminator for table reference config.")
    name: StrictStr = Field(description="Name of the table to use.")
    columns: Optional[List[StrictStr]] = None
    where: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["type", "name", "columns", "where"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ReferenceTableConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ReferenceTableConfig from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "type": obj.get("type") if obj.get("type") is not None else 'table',
            "name": obj.get("name"),
