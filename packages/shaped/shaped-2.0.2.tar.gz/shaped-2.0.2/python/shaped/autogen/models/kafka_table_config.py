...
class KafkaTableConfig(BaseModel):
    """
    KafkaTableConfig
    """ # noqa: E501
    schema_type: Optional[StrictStr] = Field(default='KAFKA', description="Schema type discriminator for Kafka datasets.")
    name: StrictStr = Field(description="Unique identifier for the Kafka dataset.")
    description: Optional[StrictStr] = None
    topic: StrictStr = Field(description="Kafka topic name to consume from.")
    bootstrap_server: StrictStr = Field(description="Kafka bootstrap server address (host:port).")
    username: StrictStr = Field(description="Kafka username for authentication.")
    password: StrictStr = Field(description="Kafka password for authentication.")
    unique_keys: Optional[List[StrictStr]] = None
    column_schema: Optional[Dict[str, ValueType]] = None
    __properties: ClassVar[List[str]] = ["schema_type", "name", "description", "topic", "bootstrap_server", "username", "password", "unique_keys", "column_schema"]
    ...
    def schema_type_validate_enum(cls, value):
        ...
        if value not in set(['KAFKA']):
            raise ValueError("must be one of enum values ('KAFKA')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of KafkaTableConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of KafkaTableConfig from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "schema_type": obj.get("schema_type") if obj.get("schema_type") is not None else 'KAFKA',
            "name": obj.get("name"),
            "description": obj.get("description"),
            "topic": obj.get("topic"),
            "bootstrap_server": obj.get("bootstrap_server"),
            "username": obj.get("username"),
            "password": obj.get("password"),
            "unique_keys": obj.get("unique_keys"),
            "column_schema": dict((_k, _v) for _k, _v in obj.get("column_schema").items()) if obj.get("column_schema") is not None else None
        })
        return _obj
