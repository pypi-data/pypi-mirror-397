...
class OnlineStoreConfig(BaseModel):
    """
    Configuration for the online store (Redis) used in the fast_tier. These settings control data retention for scalability and performance.
    """ # noqa: E501
    interaction_max_per_user: Optional[StrictInt] = Field(default=30, description="Maximum number of recent interactions per user in Redis.")
    interaction_expiration_days: Optional[StrictInt] = Field(default=90, description="Time-to-live in days for user interaction history in Redis.")
    __properties: ClassVar[List[str]] = ["interaction_max_per_user", "interaction_expiration_days"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OnlineStoreConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OnlineStoreConfig from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "interaction_max_per_user": obj.get("interaction_max_per_user") if obj.get("interaction_max_per_user") is not None else 30,
            "interaction_expiration_days": obj.get("interaction_expiration_days") if obj.get("interaction_expiration_days") is not None else 90
        })
        return _obj
