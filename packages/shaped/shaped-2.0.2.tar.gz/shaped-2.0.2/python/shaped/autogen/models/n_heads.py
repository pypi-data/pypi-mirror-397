...
NHEADS_ANY_OF_SCHEMAS = ["TunableInt", "int"]

class NHeads(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = NHeads.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in NHeads with anyOf schemas: TunableInt, int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into NHeads with anyOf schemas: TunableInt, int. Details: " + ", ".join(error_messages))
        else:
            return instance
