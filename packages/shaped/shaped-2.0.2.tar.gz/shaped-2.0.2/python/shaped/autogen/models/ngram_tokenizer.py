...
class NgramTokenizer(BaseModel):
    """
    A tokenizer that breaks text into n-grams of a specified size.
    """ # noqa: E501
    min_length: Optional[StrictInt] = Field(default=3, description="Minimum n-gram length.")
    max_length: Optional[StrictInt] = Field(default=3, description="Maximum n-gram length.")
    prefix_only: Optional[StrictBool] = Field(default=False, description="Whether to generate only prefix n-grams.")
    type: Optional[StrictStr] = Field(default='ngram', description="Tokenizer type discriminator.")
    __properties: ClassVar[List[str]] = ["min_length", "max_length", "prefix_only", "type"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of NgramTokenizer from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of NgramTokenizer from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "min_length": obj.get("min_length") if obj.get("min_length") is not None else 3,
            "max_length": obj.get("max_length") if obj.get("max_length") is not None else 3,
            "prefix_only": obj.get("prefix_only") if obj.get("prefix_only") is not None else False,
            "type": obj.get("type") if obj.get("type") is not None else 'ngram'
        })
        return _obj
