...
LAPLACESMOOTHING_ANY_OF_SCHEMAS = ["TunableFloat", "float"]

class LaplaceSmoothing(BaseModel):
    """
    Laplace smoothing parameter to handle unseen n-grams.
    """

    # data type: float
    anyof_schema_1_validator: Optional[Union[StrictFloat, StrictInt]] = None
    # data type: TunableFloat
    anyof_schema_2_validator: Optional[TunableFloat] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[TunableFloat, float]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "TunableFloat", "float" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = LaplaceSmoothing.model_construct()
        error_messages = []
        ...
        # validate data type: TunableFloat
        if not isinstance(v, TunableFloat):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TunableFloat`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in LaplaceSmoothing with anyOf schemas: TunableFloat, float. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[TunableFloat] = None
        try:
            instance.actual_instance = TunableFloat.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into LaplaceSmoothing with anyOf schemas: TunableFloat, float. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], TunableFloat, float]]:
