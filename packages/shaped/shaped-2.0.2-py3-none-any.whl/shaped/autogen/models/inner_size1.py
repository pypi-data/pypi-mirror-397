...
INNERSIZE1_ANY_OF_SCHEMAS = ["TunableIntCategorical", "int"]

class InnerSize1(BaseModel):
    """
    Size of the feed-forward network inner layer.
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = InnerSize1.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in InnerSize1 with anyOf schemas: TunableIntCategorical, int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InnerSize1 with anyOf schemas: TunableIntCategorical, int. Details: " + ", ".join(error_messages))
        else:
            return instance
