...
OBJECTIVE1_ANY_OF_SCHEMAS = ["Objective", "TunableString"]
...
class Objective1(BaseModel):
    ...
    # data type: Objective
    anyof_schema_1_validator: Optional[Objective] = None
    # data type: TunableString
    anyof_schema_2_validator: Optional[TunableString] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Objective, TunableString]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Objective", "TunableString" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Objective1.model_construct()
        error_messages = []
        # validate data type: Objective
        if not isinstance(v, Objective):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Objective`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Objective1 with anyOf schemas: Objective, TunableString. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[Objective] = None
        try:
            instance.actual_instance = Objective.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Objective1 with anyOf schemas: Objective, TunableString. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Objective, TunableString]]:
