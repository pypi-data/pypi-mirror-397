...
class PrecomputedUserEmbedding(BaseModel):
    """
    Looks up a pre-computed (cached) embedding for a given user ID.
    """ # noqa: E501
    input_user_id: StrictStr = Field(description="User ID parameter or value to lookup precomputed embedding.")
    type: Optional[StrictStr] = Field(default='precomputed_user', description="Encoder type discriminator.")
    __properties: ClassVar[List[str]] = ["input_user_id", "type"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['precomputed_user']):
            raise ValueError("must be one of enum values ('precomputed_user')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PrecomputedUserEmbedding from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PrecomputedUserEmbedding from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type") if obj.get("type") is not None else 'precomputed_user'
        })
        return _obj
