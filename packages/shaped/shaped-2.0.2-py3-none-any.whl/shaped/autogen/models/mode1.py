...
MODE1_ANY_OF_SCHEMAS = ["TrendingMode", "TunableString"]
...
class Mode1(BaseModel):
    ...
    # data type: TrendingMode
    anyof_schema_1_validator: Optional[TrendingMode] = None
    # data type: TunableString
    anyof_schema_2_validator: Optional[TunableString] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[TrendingMode, TunableString]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "TrendingMode", "TunableString" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Mode1.model_construct()
        error_messages = []
        # validate data type: TrendingMode
        if not isinstance(v, TrendingMode):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TrendingMode`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Mode1 with anyOf schemas: TrendingMode, TunableString. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[TrendingMode] = None
        try:
            instance.actual_instance = TrendingMode.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Mode1 with anyOf schemas: TrendingMode, TunableString. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], TrendingMode, TunableString]]:
