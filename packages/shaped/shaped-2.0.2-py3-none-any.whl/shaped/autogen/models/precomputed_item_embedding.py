...
class PrecomputedItemEmbedding(BaseModel):
    """
    Looks up a pre-computed (cached) embedding for a given item ID.
    """ # noqa: E501
    input_item_id: StrictStr = Field(description="Item ID to lookup precomputed embedding.")
    type: Optional[StrictStr] = Field(default='precomputed_item', description="Encoder type discriminator.")
    __properties: ClassVar[List[str]] = ["input_item_id", "type"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['precomputed_item']):
            raise ValueError("must be one of enum values ('precomputed_item')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PrecomputedItemEmbedding from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PrecomputedItemEmbedding from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type") if obj.get("type") is not None else 'precomputed_item'
        })
        return _obj
