...
class AutoPolicy:
    ...
    def fit(self, df):
        ...
        progress = Progress(
            ...
            MofNCompleteColumn(),
            TimeElapsedColumn(),
            TextColumn("[blue]best[/blue]=[grey37]{task.fields[best_cost]:.4f}[/grey37]"),
            TextColumn("[blue]last[/blue]=[grey37]{task.fields[last_cost]:.4f}[/grey37]"),
        ...
        policy = Policy(self.p,self.label,self.bin_col,**best).fit(df)
        progress.console.print(f"[green]autopolicy[/green]", " ".join([
            f"[blue]{k}[/blue]=[grey37]{v:0.3f}[/grey37]" if type(v) == float else 
            f"[blue]{k}[/blue]=[grey37]{v}[/grey37]" for k,v in best.items()
        ]))
