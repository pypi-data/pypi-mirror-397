...
System Role:
    ...
    targets register the console script defined in :mod:`uid_check_austria.__init__conf__`.
"""
...
def cli_check(
    ...
    Examples:
      ...
      uid-check-austria check FR12345678901 --format json
      uid-check-austria check IT12345678901 --no-email
      uid-check-austria check ES12345678A --recipient admin@example.com
...
def _show_config_help(error_message: str) -> None:
    ...
    click.echo(f"  UID_CHECK_AUSTRIA___FINANZONLINE__TID=... ({_('8-12 alphanumeric')})", err=True)
    click.echo(f"  UID_CHECK_AUSTRIA___FINANZONLINE__BENID=... ({_('5-12 chars')})", err=True)
    click.echo(f"  UID_CHECK_AUSTRIA___FINANZONLINE__PIN=... ({_('5-128 chars')})", err=True)
    click.echo("  UID_CHECK_AUSTRIA___FINANZONLINE__UID_TN=ATU...", err=True)
    click.echo(f"  UID_CHECK_AUSTRIA___FINANZONLINE__HERSTELLERID=... ({_('10-24 alphanumeric')})", err=True)
