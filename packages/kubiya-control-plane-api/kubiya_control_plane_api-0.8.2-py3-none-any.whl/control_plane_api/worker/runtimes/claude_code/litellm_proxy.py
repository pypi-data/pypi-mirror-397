...
def build_langfuse_metadata(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Format: EMAIL-ORG_NAME (same as agno)
...
class LiteLLMProxyApp:
            ...
            if not execution_id:
                ...
                if not execution_id:
                    logger.warning(
                        ...
                        path=path,
                        note="Cannot inject metadata without execution_id (no header and no current execution)"
                    )
    ...
    async def _proxy_streaming_request(
        ...
        """
        Proxy a streaming request to LiteLLM.
        ...
        async def stream_generator():
            """Generator that yields chunks from LiteLLM."""
