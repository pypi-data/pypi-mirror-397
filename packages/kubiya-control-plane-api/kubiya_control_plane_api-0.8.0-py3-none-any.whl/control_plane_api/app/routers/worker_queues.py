...
# Stale worker threshold: Reduced from 120s to 30s for faster ephemeral queue cleanup
# Workers that haven't sent heartbeat in 30s are considered inactive
STALE_WORKER_THRESHOLD_SECONDS = 30
...
async def list_queue_workers(
    ...
    try:
        ...
        for worker_id, heartbeat_data in active_workers.items():
            ...
            if not system_info_data and db_data.get("worker_metadata"):
                system_info_data = db_data.get("worker_metadata")
