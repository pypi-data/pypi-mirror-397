...
class EventPublisherConfig:
    ...
    def from_env(cls) -> "EventPublisherConfig":
        ...
        return cls(
            ...
            websocket_enabled=os.getenv("EVENT_WEBSOCKET_ENABLED", "true").lower() == "true",
    ...
    async def _publish_immediate(self, event_type: str, data: Dict[str, Any]) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # CRITICAL: Always await if coroutine or awaitable
                # Use inspect.isawaitable() and inspect.iscoroutine() for comprehensive detection
                if inspect.iscoroutine(result) or inspect.isawaitable(result):
                    await result
                elif asyncio.isfuture(result):
                    # If it's a Future, we must await it
                    await result
                ...
                # If result was a coroutine/task that wasn't awaited, try to clean it up
                if result is not None:
                    if inspect.iscoroutine(result):
                        # Close unawaited coroutine to prevent warnings
                        result.close()
                    elif asyncio.isfuture(result) and not result.done():
                        # Cancel pending future/task
                        result.cancel()
                        try:
                            await result
