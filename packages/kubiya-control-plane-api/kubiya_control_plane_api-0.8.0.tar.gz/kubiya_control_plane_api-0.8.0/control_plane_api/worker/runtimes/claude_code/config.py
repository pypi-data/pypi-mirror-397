...
async def build_claude_options(
    ...
    # START LOCAL PROXY: Get local proxy URL (starts proxy if not running)
    # This proxy will inject Langfuse metadata before forwarding to real LiteLLM
    try:
        ...
        logger.error(
            ...
            fallback="Using direct LiteLLM connection (no metadata injection)",
        )
    ...
    # Extract metadata from context for Langfuse tracking
    execution_context = {}
    ...
    # Create per-execution prompt builder to support dynamic skill context
