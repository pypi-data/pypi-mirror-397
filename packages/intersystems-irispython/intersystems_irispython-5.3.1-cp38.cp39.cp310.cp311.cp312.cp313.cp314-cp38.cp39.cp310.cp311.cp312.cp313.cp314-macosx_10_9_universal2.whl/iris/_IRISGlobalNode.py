...
'''
    ...
    def keys(self):
        '''
Returns an IRISGlobalNodeView object of the current IRISGlobalNode which can be iterated over to yield all subscripts.
'''
...
'''
    ...
    def values(self):
        '''
Returns an IRISGlobalNodeView object of the current IRISGlobalNode which can be iterated over to yield all values.
'''
        return iris.IRISGlobalNodeView(self, iris.IRISIterator.VIEW_VALUES)

    def nodes(self):
        '''
Returns an IRISGlobalNodeView object of the current IRISGlobalNode which can be iterated over to yield all subnodes.
'''
        return iris.IRISGlobalNodeView(self, iris.IRISIterator.VIEW_NODES)

    def valuetuples(self):
        '''
Returns an IRISGlobalNodeView object of the current IRISGlobalNode which can be iterated over to yield all subscript-value tuples.
'''
        return iris.IRISGlobalNodeView(self, iris.IRISIterator.VIEW_VALUE_TUPLES)

    def nodetuples(self):
        '''
Returns an IRISGlobalNodeView object of the current IRISGlobalNode which can be iterated over to yield all subscript-subnode tuples.
'''
        return iris.IRISGlobalNodeView(self, iris.IRISIterator.VIEW_NODE_TUPLES)

    def valuenodetuples(self):
        '''
Returns an IRISGlobalNodeView object of the current IRISGlobalNode which can be iterated over to yield all subscript-value-subnode tuples.
'''
        return iris.IRISGlobalNodeView(self, iris.IRISIterator.VIEW_VALUE_NODE_TUPLES)

    def items(self):
        '''This method is deprecated. Please use valuetuples() instead.'''
        return self.valuetuples()

    def nodesitems(self):
        '''This method is deprecated. Please use nodetuples() instead.'''
        return self.nodetuples()
    ...
    def _min_stop(self, X, Y, reversed):
        sort_order = self._sort_order2(self._irisnative, self._global_name, X, Y)
        if sort_order > 0:
    ...
    @staticmethod
    def _sort_order2(irisnative, global_name, X, Y):
        ...
        if isinstance(X._key, BEFOREFIRST):
            ...
            irisnative._sortOrder2(global_name, reference_Y, None)
        ...
        if isinstance(Y._key, BEFOREFIRST):
            ...
            irisnative._sortOrder2(global_name, reference_X, None)
        ...
        reference_X = iris.IRISReference(X._key, str)
        reference_Y = iris.IRISReference(Y._key, str)
        sort_order = irisnative._sortOrder2(global_name, reference_X, reference_Y)
        X._key = reference_X.get_value()
        Y._key = reference_Y.get_value()
        return sort_order

    @classmethod
    def _sort_order_by_key2(cls, irisnative, global_name, keyX, keyY):
        return cls._sort_order2(irisnative, global_name, _point(keyX, None), _point(keyY, None))
