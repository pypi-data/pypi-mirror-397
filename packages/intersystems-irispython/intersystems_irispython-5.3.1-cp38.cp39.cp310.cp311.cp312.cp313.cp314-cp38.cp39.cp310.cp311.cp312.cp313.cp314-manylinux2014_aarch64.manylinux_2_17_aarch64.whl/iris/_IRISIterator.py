...
'''
    ...
    def __next__(self):
        ...
        if self._view_type == iris.IRISIterator.VIEW_SUBSCRIPTS:
            ...
            return self._irisnative.node(self._global_name, *self._subscripts, key_value[0])
        elif self._view_type == iris.IRISIterator.VIEW_VALUE_TUPLES:
            return key_value
        elif self._view_type == iris.IRISIterator.VIEW_NODE_TUPLES:
            return (key_value[0], self._irisnative.node(self._global_name, *self._subscripts, key_value[0]))
        elif self._view_type == iris.IRISIterator.VIEW_VALUE_NODE_TUPLES:
            return (key_value[0], key_value[1], self._irisnative.node(self._global_name, *self._subscripts, key_value[0]))
        else:
    ...
    def _nextNode(self):
        ...
        SUPPORT_STOP_ON = self._irisnative._connection._protocol_version() >= 63
        if SUPPORT_STOP_ON:
    ...
    def _has_passed_stop(self, current_value):
        ...
        if self._reversed:
            if self._stop._inclusive:
                if iris.IRISGlobalNode._sort_order_by_key2(self._irisnative, self._global_name, self._stop._key, current_value) < 0:
                    return True
            else:
                if iris.IRISGlobalNode._sort_order_by_key2(self._irisnative, self._global_name, self._stop._key, current_value) <= 0:
                    return True
