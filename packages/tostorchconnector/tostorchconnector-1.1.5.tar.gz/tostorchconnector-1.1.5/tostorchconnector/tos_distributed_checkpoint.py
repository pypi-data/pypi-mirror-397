...
class TOSFileSystem(FileSystemBase):
    ...
    def __init__(self, region: str, endpoint: str, 
        ...
        """
        self.cli_wrapper = TosCloudClient(bucket='dataset-test-why',access_key=access_key,secret_key=secret_key,endpoint=endpoint,region=region)
    ...
    def create_stream(self, path: Union[str, os.PathLike], mode: str) -> Generator[io.IOBase, None, None]:
        ...
        if mode == 'rb':
            ...
            with TosCloudObjectReader(bucket=bucket, key=key,stream=TosCloudSyncGetStream(self.cli_wrapper.client,key)) as stream:
                yield stream 
        elif mode == 'wb':
            with TosCloudObjectWriter(bucket=bucket, key=key,put_stream=TosCloudSyncPutSteam(self.cli_wrapper.client,key)) as stream:
                yield stream
...
class TosDistributedCkptWriter(FileSystemWriter):
    def __init__(self,  path: str, region: str="", endpoint: str="",access_key: str = "",secret_key: str = "",prefix_strategy: Optional[PrefixStrategyBase] = None,**kwargs):
        super().__init__(
            path=path,
            sync_files=False,
            **kwargs
        ...
        self.fs = TOSFileSystem(region=self.region,endpoint=self.endpoint,access_key=access_key,secret_key=secret_key)
...
class TosDistributedCkptReader(FileSystemReader):
    def __init__(self, path: str, region: str="", endpoint: str="",access_key: str = "",secret_key: str = "",**kwargs):
        self.path = path
        self.region = region
        self.endpoint = endpoint
        self.access_key = access_key
        self.secret_key = secret_key
        self.fs = TOSFileSystem(region=self.region,endpoint=self.endpoint,access_key=self.access_key,secret_key=self.secret_key)
