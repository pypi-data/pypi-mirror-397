...
class Config:
    ...
    # API Configuration
    api_url: str = field(default_factory=lambda: os.getenv("AIGIE_API_URL", "http://localhost:8000/api"))
    api_key: str = field(default_factory=lambda: os.getenv("AIGIE_API_KEY", "test-api-key"))
    ...
    # License key for self-hosted installations (kytte_lic_xxx format)
    license_key: Optional[str] = field(
        default_factory=lambda: os.getenv("KYTTE_LICENSE_KEY")
    )

    # License server URL (defaults to Kytte's licensing server)
    license_server_url: str = field(
        default_factory=lambda: os.getenv("KYTTE_LICENSE_SERVER", "https://license.kytte.io")
    )

    # Unique installation ID (auto-generated if not provided)
    installation_id: Optional[str] = field(
        default_factory=lambda: os.getenv("KYTTE_INSTALLATION_ID")
    )

    # Enable usage telemetry reporting to license server
    enable_usage_telemetry: bool = field(
        default_factory=lambda: os.getenv("KYTTE_USAGE_TELEMETRY", "true").lower() == "true"
    )

    # Skip license validation (for development/testing only)
    skip_license_validation: bool = field(
        default_factory=lambda: os.getenv("KYTTE_SKIP_LICENSE_VALIDATION", "false").lower() == "true"
    )
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            api_url=os.getenv("AIGIE_API_URL", "http://localhost:8000/api"),
            api_key=os.getenv("AIGIE_API_KEY", "test-api-key"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # License fields (mask the key)
            "has_license_key": self.license_key is not None,
