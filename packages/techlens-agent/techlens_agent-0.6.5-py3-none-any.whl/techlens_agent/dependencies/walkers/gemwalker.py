...
class GemWalker(Walker):
    def parse(self, file: str, expand=False):
        command = (
            f"gem install -r --explain --no-user-install --install-dir gems -g {file}"
        )
        try:
            ...
            for line in self.lines:
                ...
                if len(line) > 0:
                    ...
                    if line.startswith("source"):
                        ...
                        sources.append({"source": source, "layer": do_nest})
                        first_source = True
                            ...
                            for column in line2:
                                ...
                                for column in column_list:
                                    ...
                                    for (
                                        criteria,
                                        criteria_regex,
                                    ) in GemfileParser.gemfile_regexes.items():
                                        ...
                                        if match:
                                            if criteria == "requirement":
                                                ...
                                                setattr(
                                                    dep, criteria, match.group(criteria)
                                                )
                                            break
    ...
    def get_license(self, name: str) -> str:
        ...
        try:
            resp = self.getUrl(
                f"https://rubygems.org/api/v2/rubygems/{name}/versions/{version}.json"
            )
            resp = json.loads(resp)
            license = " ".join(resp["licenses"]) if resp["licenses"] is not None else ""
        except Exception as e2:
            self.log(f"License not found for: {name}@{version}", display=False)
