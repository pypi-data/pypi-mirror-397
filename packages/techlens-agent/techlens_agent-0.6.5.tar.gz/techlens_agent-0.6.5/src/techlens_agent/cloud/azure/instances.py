...
from techlens_agent.cloud.cloud_dataclass import (
...
def get_metrics_for_instance(
    ...
    try:
        ...
        for dp in o.metrics[0].timeseries[0].data:
            n = Datapoint(Minimum=dp.minimum, Maximum=dp.maximum, Average=dp.average)
            datum = Datum(Timestamp=dp.timestamp.timestamp(), cpu=n)
            data.append(datum)
...
def get_instances(
    cloud_name: str, sub_id: str, path_to_output: str = "./", dry=False, log=None
):
    ...
    if not dry:
        ...
        client = ComputeManagementClient(
            ...
            base_url=f"https://{management_endpoint}",
            credential_scopes=cred_scopes,
        )
        ...
        networkClient = NetworkManagementClient(
            credential=credential, subscription_id=sub_id, cloud_setting=cloud_setting
        )
        ...
        for vm in res:
            try:
                ...
                iv = client.virtual_machines.instance_view(
                    vm_name=name, resource_group_name=tgt
                )
                ...
                nics = networkClient.network_interface_ip_configurations.list(
                    resource_group_name=tgt, network_interface_name=nic_name
                )
                subnet = ""
                public_ip = "n/a"
                for nic in nics:
                ...
                if log:
                    log(tag="Azure Get Instance Error", msg=str(e))
                continue
