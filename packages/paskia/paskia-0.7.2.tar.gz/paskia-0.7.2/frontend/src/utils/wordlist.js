...
/**
 * Find words matching the given prefix.
...
export function findMatches(prefix) {
  ...
  return prefixMap.get(normalized) || [];
}

/**
 * Get unique autocomplete suggestion if prefix matches exactly one word.
 * @param {string} prefix - The prefix to match (case-insensitive)
 * @returns {string|null} The matching word if unique, null otherwise
 */
export function getUniqueMatch(prefix) {
  const matches = findMatches(prefix);
  return matches.length === 1 ? matches[0] : null;
}

/**
 * Check if a word is valid (exists in the wordlist).
...
/**
 * Check if a string is a valid prefix (could be the start of a valid word).
 * @param {string} prefix - The prefix to check (case-insensitive)
 * @returns {boolean} True if the prefix matches at least one word
 */
export function isValidPrefix(prefix) {
  if (!prefix) return true;  // Empty is always valid
  const normalized = prefix.toLowerCase();
