INSERT INTO customer_metrics_table
...
customer_segments AS (
  SELECT
    ...
    registration_date,
    CASE
      WHEN TIMESTAMPDIFF(DAY, registration_date, CURRENT_DATE) <= 30 THEN 'new'
      WHEN TIMESTAMPDIFF(DAY, registration_date, CURRENT_DATE) <= 365 THEN 'regular'
      ELSE 'veteran'
...
customer_metrics AS (
  SELECT
    ...
    TIMESTAMPDIFF(DAY, ph.last_purchase_date, CURRENT_DATE) as days_since_last_purchase
