#!/usr/bin/env python3
"""Test SoftDTWSequenceMetric compute_matrix method."""
...
class TestSoftDTWComputeMatrix:
    """
    Test SoftDTWSequenceMetric compute_matrix method.
    """
    ...
    @pytest.mark.parametrize("pool_type", ["event", "state", "interval"])
    def test_compute_matrix_with_gamma(
        self, sequence_pools, pool_type, metric_snapshot
    ):
        """
        Test compute_matrix with custom gamma value.
        """
        pool = sequence_pools[pool_type]
        metric = SoftDTWSequenceMetric(
