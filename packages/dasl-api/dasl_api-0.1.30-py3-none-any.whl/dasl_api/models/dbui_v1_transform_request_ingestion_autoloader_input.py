...
class DbuiV1TransformRequestIngestionAutoloaderInput(BaseModel):
    """
    Configures ingestion from an external Datasource using Databricks Auto Loader. This option is mutually exclusive with `input` and enables structured streaming to process data incrementally from cloud storage or other external sources. 
    """ # noqa: E501
    ...
    format_options: Optional[Dict[str, StrictStr]] = Field(default=None, description="Optional format-specific options passed to Databricks read_files(). Currently only supported for CSV format. For available options, see https://docs.databricks.com/sql/language-manual/functions/read_files#csv-options ", alias="formatOptions")
