...
config_ns.add_enumeration_parameter(
    "sort",
    default=["oci_region", "image_created"],
...
class ClusterList(ClusterCommand):
    ...
    def _build_table_row_from_cluster(self, cluster: Cluster):
        ...
        if cluster.created_at:
            cluster_creation_date = format_to_local_date_time(cluster.created_at)
            cluster_age = get_time_ago_from_iso(cluster.created_at)
