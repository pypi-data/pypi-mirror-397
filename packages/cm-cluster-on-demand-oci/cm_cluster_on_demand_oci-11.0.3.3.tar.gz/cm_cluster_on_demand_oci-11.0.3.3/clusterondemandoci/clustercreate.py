...
config_ns.override_imported_parameter("version", default="trunk")
...
class ClusterCreate(ClusterCommand):
    ...
    def _create_cluster(self, compute_cid: str, networking_cid: str):
        ...
        private_ips = self.client.network.list_private_ips(vnic_id=primary_vnic_attachment.vnic_id)
        primary_private_ip = next(ip for ip in private_ips if ip.is_primary)

        log.info("Creating public IP")
        public_ip = self.client.network.create_public_ip(
        ...
        wait_for_cluster(config, public_ip.ip_address)
