...
from clusterondemand.utils import validate_timestamp
from clusterondemandconfig import config
from clusterondemandoci.const import BCM_CREATED_TAG_KEY, BCM_HA_TAG, CLUSTER_NAME_TAG
...
class Cluster:
    ...
    def __init__(self, cluster_name: str, head_nodes: list[Instance] = None, vcn: Vcn = None, oci_region: str = None):
        ...
        self.created_at: str | None = self._get_created_at()
    ...
    def _get_created_at(self) -> str | None:
        """Retrieve and validate the created_at timestamp from OCI resources."""
        created_at: str | None = None
        if self.cluster_vcn and self.cluster_vcn.freeform_tags:
            created_at = self.cluster_vcn.freeform_tags.get(BCM_CREATED_TAG_KEY)
        if not created_at and self.primary_head_node and self.primary_head_node.freeform_tags:
            created_at = self.primary_head_node.freeform_tags.get(BCM_CREATED_TAG_KEY)

        try:
            return validate_timestamp(created_at)
