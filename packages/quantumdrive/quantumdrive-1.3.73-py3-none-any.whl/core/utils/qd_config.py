...
class QDConfig(BaseModel):
    ...
    def from_toml(cls, toml_path: Optional[str] = None) -> "QDConfig":
        ...
        Args:
            toml_path: Path to TOML file. If None, searches for:
                1. ~/.config/quantumdrive/quantumdrive.toml
                2. quantumdrive/resources/default_quantumdrive.toml
        ...
        if toml_path is None:
            ...
            if user_config.exists():
                ...
                logger.info(f"Loading config from user file: {toml_path}")
            else:
