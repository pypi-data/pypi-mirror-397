...
inline FrameMsgInfo ubx_frame_validate_packet(const uint8_t* buffer, size_t length) {
    if (length < UBX_FRAME_OVERHEAD) return FrameMsgInfo();

    if (buffer[0] != UBX_FRAME_START_BYTE1) return FrameMsgInfo();
    if (buffer[1] != UBX_FRAME_START_BYTE2) return FrameMsgInfo();
