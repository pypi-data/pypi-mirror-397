...
MODEL_NAME = _CONSTANTS.get("DEFAULT_MODEL", "groq/llama-3.1-8b-instant")
        ...
        try:
            ...
            if not cloud_mode:
                ...
                if not prompt_template:
                     prompt_template = (
                        "You are Vorp, a proficient terminal-based AI assistant running directly on the user's machine.\n"
                        "You have access to the local file system and shell. You can Create, Read, Update, and Delete files, and execute shell commands.\n"
                        "GUIDELINES:\n"
                        "1. ANALYZE: When asked to perform an action, determine if it is a CREATE, UPDATE, or DELETE operation.\n"
                        "2. CREATE: Use 'write_file'. Ensure the content is complete.\n"
                        "3. UPDATE: Use 'replace_string' for targeted edits. Use 'write_file' ONLY if you are rewriting the entire file.\n"
                        "   - For 'replace_string': 'old_string' must be UNIQUE and EXACT (including whitespace). Use 'read_file' first to verify.\n"
                        "   - For 'write_file': You must provide the FULL NEW CONTENT of the file, not just a diff.\n"
                        "4. DELETE: Use 'delete_file'. The system will ask the user for permission.\n"
                        "5. SHELL: Use 'run_shell_command' for system tasks. The system will ask for permission.\n"
                        "6. CONTEXT: If a file isn't found, try listing the directory to find the correct path.\n"
                        "7. FORMATTING: When calling tools, ensure arguments are valid JSON. Do not invent new tools.\n"
                        "Your operating environment is: {os_name} ({cwd})"
                     )
                    ...
                    with Live(console=console, refresh_per_second=20) as live:
                        ...
                        try:
                            ...
                            if len(llm_messages) > 0 and llm_messages[-1].get("role") == "tool":
                                console.print(f"[dim yellow]Warning: Model failed to generate post-action response ({str(e)}).[/dim yellow]")
                                console.print("[bold green]✓ Action completed successfully.[/bold green]")
                                
                                # Mock a final response to keep history valid and exit the tool loop
                                messages.append({"role": "assistant", "content": "I have completed the requested action."})
                                break 
                            else:
                                # ROBUSTNESS: Handle API/Provider errors without crashing
                                error_str = str(e)
                                console.print(f"\n[bold red]Provider Error:[/bold red] {error_str}")
                                
                                if "Failed to call a function" in error_str:
                                    console.print("[dim yellow]Hint: The model tried to use a tool but formatted it incorrectly. Try rephrasing your request.[/dim yellow]")
                                elif "rate_limit" in error_str.lower():
                                    console.print("[dim yellow]Hint: You are being rate limited by the provider. Wait a moment.[/dim yellow]")

                                # Break the inner tool loop to return to user input (don't crash app)
                                break
