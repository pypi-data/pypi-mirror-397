...
def chat(
    ...
    console.print(f"\n[bold cyan]Vorp[/bold cyan] [dim]|[/dim] [magenta]{model_display_name}[/magenta]")
    
    if cloud_mode:
        usage = config.get("cloud_usage", 0)
        remaining = max(0, TRIAL_LIMIT - usage)
        status_color = "yellow" if remaining < 3 else "green"
        console.print(f"[dim]Cloud Trial: [{status_color}]{remaining}/{TRIAL_LIMIT}[/{status_color}] messages remaining.[/dim]")
    else:
        console.print("[dim]Local Mode Active[/dim]")
    ...
    while True:
        try:
            ...
            if cloud_mode:
                ...
                # Increment and save
                config["cloud_usage"] = current_usage + 1
                save_config(config)
            ...
            if not cloud_mode:
                ...
                if not prompt_template:
                     prompt_template = (
                        ...
                        "3. UPDATE: Use 'write_file'. THIS TOOL OVERWRITES THE ENTIRE FILE. You must provide the FULL NEW CONTENT of the file, not just a diff.\n"
                        "   - ALWAYS use 'read_file' FIRST to see the current content before updating.\n"
                        "4. DELETE: Use 'delete_file'. The system will ask the user for permission.\n"
                        "5. SHELL: Use 'run_shell_command' for system tasks (installing packages, git commands, listing dirs). The system will ask for permission.\n"
                        "6. CONTEXT: If a file isn't found, try listing the directory to find the correct path.\n"
                        "7. SAFETY: Do not assume permissions. Respect the user's decision if an action is denied.\n"
                        "8. MEMORY: The user may refer to the 'last file' or 'it'. The last file you interacted with was: '{active_file}'.\n"
