...
## 🔄 v2.1.16 更新内容
### Linux串口连接修复和依赖修复版本
- 🔧 **Linux串口连接修复**: 修复Debian Linux下串口设备识别但连接不上的权限问题
- 🔧 **psutil依赖修复**: 修复pyproject.toml中缺失的psutil依赖，确保Linux安装后正常运行
- 🔧 **游戏模块修复**: 修复games模块缺少__init__.py文件导致的导入错误
- 🔧 **环境管理器改进**: 优化Linux串口权限检查逻辑，提供更详细的错误诊断和解决方案
- 🔧 **串口管理器增强**: 添加Linux平台检测、设备路径检查和权限错误处理
- 🛠️ **Linux脚本优化**: 更新启动脚本，提供更好的用户交互和权限检查
- 📝 **新增诊断工具**: 添加`test_linux_serial.py`脚本用于Linux串口环境诊断
- 📖 **故障排除指南**: 创建详细的`LINUX_TROUBLESHOOTING.md`文档
- 🔄 **版本号统一**: 更新所有模块和文档版本号至v2.1.16
