...
class EnvironmentChecker:
    ...
    def check_linux_serial_permissions(self) -> Tuple[bool, str, List[str]]:
        ...
        try:
            ...
            # 检查用户是否在dialout组（串口设备组）
            in_dialout = "dialout" in current_groups
                ...
                for device in glob.glob(pattern):
                    try:
                        ...
                        if not (user_read and user_write):
                            permission_issues.append(f"{device}: 用户缺少读写权限")
            ...
            suggestions = []
            if not in_dialout and serial_devices:
                suggestions.append("将用户添加到dialout组: sudo usermod -a -G dialout $USER")
                suggestions.append("然后重新登录或重启")
            
            if permission_issues:
                suggestions.append("临时解决方案: sudo chmod 666 /dev/ttyUSB*")
                suggestions.append("或使用sudo运行: sudo colorbridge")
            
            if not in_dialout or permission_issues:
                return False, f"Linux串口权限问题: 用户不在dialout组" if not in_dialout else "串口设备权限不足", suggestions
            else:
                return True, "Linux串口权限检查通过", []
