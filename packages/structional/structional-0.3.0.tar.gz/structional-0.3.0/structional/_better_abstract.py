"""Introduces `AbstractVar` and `AbstractClassVar` type annotations.

They can be used to mark abstract instance attributes and abstract class attributes.

Provides enhanced versions of `abc.ABCMeta` and `dataclasses.dataclass` that respect
this type annotation. (In practice the sole consumer of these is `Struct`, which
is the public API. But the two pieces aren't directly related under-the-hood.)
"""

#
# This file is basically copy-pasted from `equinox/_better_abstract.py`.
# The only changes are to:
# - docstrings, to use `Struct` instead of `eqx.Module`.
# - rename `dataclass` -> `better_dataclass`.
# - rename `ABCMeta` -> `BetterABCMeta`.
# - add `__abstractvars__` and `__abstractclassvars__` annotations.
#
...
if TYPE_CHECKING:
    ...
    class AbstractVar(Generic[_T]):
        """Used to mark an abstract instance attribute, along with its type.
...
def _process_annotation(annotation):
    if isinstance(annotation, str):
        if annotation.startswith(("AbstractVar[", "AbstractClassVar[")):
        ...
        if annotation in (AbstractVar, AbstractClassVar):  # noqa: PLR6201
            raise TypeError(
...
class BetterABCMeta(abc.ABCMeta):
    ...
    def register(cls, subclass):
        del subclass
        raise ValueError(
            "Dynamic registration of Struct subclasses is not allowed. This is not "
            "compatible with static type checking."
        )

    def __new__(mcs, cls_name, bases, namespace, /, **kwargs):  # pyright: ignore[reportArgumentType]
