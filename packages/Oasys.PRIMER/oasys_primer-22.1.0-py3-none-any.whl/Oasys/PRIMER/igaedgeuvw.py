...
class IGAEdgeUVWType(type):
    _consts = {'BASIS_TRANSFORM', 'NONE'}

    def __getattr__(cls, name):
        if name in IGAEdgeUVWType._consts:
            return Oasys.PRIMER._connection.classGetter(cls.__name__, name)

        raise AttributeError("IGAEdgeUVW class attribute '{}' does not exist".format(name))


    def __setattr__(cls, name, value):
# If one of the constants we define then error
        if name in IGAEdgeUVWType._consts:
            raise AttributeError("Cannot set IGAEdgeUVW class constant '{}'".format(name))

# Set the property locally
        cls.__dict__[name] = value


class IGAEdgeUVW(Oasys.gRPC.OasysItem, metaclass=IGAEdgeUVWType):
    _props = {'edgeid', 'eid', 'elid', 'exyzid', 'include', 'label', 'option', 'patchid', 'rend', 'rstart', 'sense'}
...
# If one of the properties we define then get it
        if name in IGAEdgeUVW._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGAEdgeUVW._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGAEdgeUVW instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGAEdgeUVW._rprops:
            raise AttributeError("Cannot set read-only IGAEdgeUVW instance attribute '{}'".format(name))
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free IGA Edge UVW label in the model.
        Also see IGAEdgeUVW.LastFreeLabel(),
        IGAEdgeUVW.NextFreeLabel() and
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGAEdgeUVW objects or properties for all of the IGA Edge UVWs in a model in PRIMER.
        If the optional property argument is not given then a list of IGAEdgeUVW objects is returned.
        If the property argument is given, that property value for each IGA Edge UVW is returned in the list
        instead of a IGAEdgeUVW object
        ...
        property : string
            Optional. Name for property to get for all IGA Edge UVWs in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGAEdgeUVW objects for all of the flagged IGA Edge UVWs in a model in PRIMER
        If the optional property argument is not given then a list of IGAEdgeUVW objects is returned.
        If the property argument is given, then that property value for each IGA Edge UVW is returned in the list
        instead of a IGAEdgeUVW object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA Edge UVWs in the model
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free IGA Edge UVW label in the model.
        Also see IGAEdgeUVW.FirstFreeLabel(),
        IGAEdgeUVW.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) IGA Edge UVW label in the model.
        Also see IGAEdgeUVW.FirstFreeLabel(),
        IGAEdgeUVW.LastFreeLabel() and
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Edge UVWs from that model can be selected.
            If the argument is a Flag then only IGA Edge UVWs that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA Edge UVWs can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Edge UVWs are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA Edge UVWs several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA Edge UVWs should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Edge UVWs are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGAEdgeUVW property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGAEdgeUVW.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA edge uvw (\*IGA_EDGE_UVW).
        Note that a carriage return is not added.
        See also IGAEdgeUVW.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA edge uvw.
        Note that a carriage return is not added.
        See also IGAEdgeUVW.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Edge UVW is sketched.
            If omitted redraw is true. If you want to sketch several IGA Edge UVWs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Edge UVW is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA Edge UVWs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
