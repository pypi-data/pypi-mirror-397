...
class IGA2DNurbsUVWType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGA2DNurbsUVW class attribute '{}' does not exist".format(name))


class IGA2DNurbsUVW(Oasys.gRPC.OasysItem, metaclass=IGA2DNurbsUVWType):
...
# If one of the properties we define then get it
        if name in IGA2DNurbsUVW._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGA2DNurbsUVW._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGA2DNurbsUVW instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGA2DNurbsUVW._rprops:
            raise AttributeError("Cannot set read-only IGA2DNurbsUVW instance attribute '{}'".format(name))
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free IGA 2D Nurbs UVW label in the model.
        Also see IGA2DNurbsUVW.LastFreeLabel(),
        IGA2DNurbsUVW.NextFreeLabel() and
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGA2DNurbsUVW objects or properties for all of the IGA 2D Nurbs UVWs in a model in PRIMER.
        If the optional property argument is not given then a list of IGA2DNurbsUVW objects is returned.
        If the property argument is given, that property value for each IGA 2D Nurbs UVW is returned in the list
        instead of a IGA2DNurbsUVW object
        ...
        property : string
            Optional. Name for property to get for all IGA 2D Nurbs UVWs in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGA2DNurbsUVW objects for all of the flagged IGA 2D Nurbs UVWs in a model in PRIMER
        If the optional property argument is not given then a list of IGA2DNurbsUVW objects is returned.
        If the property argument is given, then that property value for each IGA 2D Nurbs UVW is returned in the list
        instead of a IGA2DNurbsUVW object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA 2D Nurbs UVWs in the model
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free IGA 2D Nurbs UVW label in the model.
        Also see IGA2DNurbsUVW.FirstFreeLabel(),
        IGA2DNurbsUVW.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) IGA 2D Nurbs UVW label in the model.
        Also see IGA2DNurbsUVW.FirstFreeLabel(),
        IGA2DNurbsUVW.LastFreeLabel() and
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA 2D Nurbs UVWs from that model can be selected.
            If the argument is a Flag then only IGA 2D Nurbs UVWs that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA 2D Nurbs UVWs can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        IGA2DNurbsUVW.Unsketch(),
        IGA2DNurbsUVW.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 2D Nurbs UVWs are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA 2D Nurbs UVWs several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA 2D Nurbs UVWs should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 2D Nurbs UVWs are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGA2DNurbsUVW property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGA2DNurbsUVW.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA 2d nurbs uvw (\*IGA_2D_NURBS_UVW).
        Note that a carriage return is not added.
        See also IGA2DNurbsUVW.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA 2d nurbs uvw.
        Note that a carriage return is not added.
        See also IGA2DNurbsUVW.Keyword()
    ...
    def SetControlPoint(self, index, data):
        ...
        control poin U, V, W and control weight WGT. There are nr x ns control points
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 2D Nurbs UVW is sketched.
            If omitted redraw is true. If you want to sketch several IGA 2D Nurbs UVWs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 2D Nurbs UVW is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA 2D Nurbs UVWs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
