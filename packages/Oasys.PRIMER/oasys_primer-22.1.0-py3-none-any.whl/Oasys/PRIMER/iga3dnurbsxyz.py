...
class IGA3DNurbsXYZType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGA3DNurbsXYZ class attribute '{}' does not exist".format(name))


class IGA3DNurbsXYZ(Oasys.gRPC.OasysItem, metaclass=IGA3DNurbsXYZType):
    _props = {'include', 'label', 'nr', 'ns', 'nt', 'patchid', 'pr', 'ps', 'pt', 'rfirst', 'rlast', 'sfirst', 'slast', 'tfirst', 'tlast', 'unir', 'unis', 'unit'}
...
# If one of the properties we define then get it
        if name in IGA3DNurbsXYZ._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGA3DNurbsXYZ._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGA3DNurbsXYZ instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGA3DNurbsXYZ._rprops:
            raise AttributeError("Cannot set read-only IGA3DNurbsXYZ instance attribute '{}'".format(name))
...
# Constructor
    def __init__(self, model, details):
        ...
        """
        Create a new IGA3DNurbsXYZ object
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free IGA 3D Nurbs XYZ label in the model.
        Also see IGA3DNurbsXYZ.LastFreeLabel(),
        IGA3DNurbsXYZ.NextFreeLabel() and
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGA3DNurbsXYZ objects is returned.
        If the property argument is given, that property value for each IGA 3D Nurbs XYZ is returned in the list
        instead of a IGA3DNurbsXYZ object
        ...
        property : string
            Optional. Name for property to get for all IGA 3D Nurbs XYZs in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGA3DNurbsXYZ objects is returned.
        If the property argument is given, then that property value for each IGA 3D Nurbs XYZ is returned in the list
        instead of a IGA3DNurbsXYZ object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA 3D Nurbs XYZs in the model
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free IGA 3D Nurbs XYZ label in the model.
        Also see IGA3DNurbsXYZ.FirstFreeLabel(),
        IGA3DNurbsXYZ.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) IGA 3D Nurbs XYZ label in the model.
        Also see IGA3DNurbsXYZ.FirstFreeLabel(),
        IGA3DNurbsXYZ.LastFreeLabel() and
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA 3D Nurbs XYZs from that model can be selected.
            If the argument is a Flag then only IGA 3D Nurbs XYZs that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA 3D Nurbs XYZs can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        IGA3DNurbsXYZ.Unsketch(),
        IGA3DNurbsXYZ.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 3D Nurbs XYZs are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA 3D Nurbs XYZs several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA 3D Nurbs XYZs should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 3D Nurbs XYZs are unsketched.
...
# Instance methods
    ...
    def Copy(self, range=Oasys.gRPC.defaultArg):
        """
        Copies the IGA 3D Nurbs XYZ. The target include of the copied IGA 3D Nurbs XYZ can be set using Options.copy_target_include
    ...
    def GetControlPoint(self, index):
        """
        Returns the data for a specific control point an a list. There are nr x ns x nt control points
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGA3DNurbsXYZ property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGA3DNurbsXYZ.ViewParameters()
    ...
    def GetTKnotVector(self, index):
        ...
        """
        return Oasys.PRIMER._connection.instanceMethod(self.__class__.__name__, self._handle, "GetTKnotVector", index)

    def Keyword(self):
        """
        Returns the keyword for this IGA 3d nurbs xyz (\*IGA_3D_NURBS_XYZ).
        Note that a carriage return is not added.
        See also IGA3DNurbsXYZ.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA 3d nurbs xyz.
        Note that a carriage return is not added.
        See also IGA3DNurbsXYZ.Keyword()
    ...
    def SetControlPoint(self, index, data):
        ...
        control point X, Y, Z and control weight WGT. There are nr x ns x nt control points
    ...
    def SetRKnotVector(self, index, r):
        """
        Sets knot value, r in \*IGA_3D_NURBS_XYZ. There are [nr + pr + 1] knot values
    ...
    def SetSKnotVector(self, index, r):
        """
        Sets knot value, s in \*IGA_3D_NURBS_XYZ. There are [ns + ps + 1] knot values
    ...
    def SetTKnotVector(self, index, r):
        """
        Sets knot value, t in \*IGA_3D_NURBS_XYZ. There are [nt + pt + 1] knot values
        ...
        None
            No return value
        """
        return Oasys.PRIMER._connection.instanceMethod(self.__class__.__name__, self._handle, "SetTKnotVector", index, r)

    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches the IGA 3D Nurbs XYZ. The IGA 3D Nurbs XYZ will be sketched until you either call
        IGA3DNurbsXYZ.Unsketch(),
        IGA3DNurbsXYZ.UnsketchAll(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 3D Nurbs XYZ is sketched.
            If omitted redraw is true. If you want to sketch several IGA 3D Nurbs XYZs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 3D Nurbs XYZ is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA 3D Nurbs XYZs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
