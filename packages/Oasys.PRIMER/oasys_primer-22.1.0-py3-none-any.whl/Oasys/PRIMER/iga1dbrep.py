...
class IGA1DBrepType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGA1DBrep class attribute '{}' does not exist".format(name))


class IGA1DBrep(Oasys.gRPC.OasysItem, metaclass=IGA1DBrepType):
...
# If one of the properties we define then get it
        if name in IGA1DBrep._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGA1DBrep._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGA1DBrep instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGA1DBrep._rprops:
            raise AttributeError("Cannot set read-only IGA1DBrep instance attribute '{}'".format(name))
...
# Static methods
    def Create(model, modal=Oasys.gRPC.defaultArg):
        ...
        -------
        IGA1DBrep
            IGA1DBrep object (or None if not made)
        """
    ...
    def First(model):
        ...
        -------
        IGA1DBrep
            IGA1DBrep object (or None if there are no IGA 1D Breps in the model)
        """
        return Oasys.PRIMER._connection.classMethod(__class__.__name__, "First", model)

    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free IGA 1D Brep label in the model.
        Also see IGA1DBrep.LastFreeLabel(),
        IGA1DBrep.NextFreeLabel() and
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGA1DBrep objects or properties for all of the IGA 1D Breps in a model in PRIMER.
        If the optional property argument is not given then a list of IGA1DBrep objects is returned.
        If the property argument is given, that property value for each IGA 1D Brep is returned in the list
        instead of a IGA1DBrep object
        ...
        property : string
            Optional. Name for property to get for all IGA 1D Breps in the model

        Returns
        -------
        list
            List of IGA1DBrep objects or properties
        """
        return Oasys.PRIMER._connection.classMethodStream(__class__.__name__, "GetAll", model, property)

    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGA1DBrep objects for all of the flagged IGA 1D Breps in a model in PRIMER
        If the optional property argument is not given then a list of IGA1DBrep objects is returned.
        If the property argument is given, then that property value for each IGA 1D Brep is returned in the list
        instead of a IGA1DBrep object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA 1D Breps in the model
    ...
    def GetFromID(model, number):
        ...
        -------
        IGA1DBrep
            IGA1DBrep object (or None if IGA 1D Brep does not exist)
        """
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free IGA 1D Brep label in the model.
        Also see IGA1DBrep.FirstFreeLabel(),
        IGA1DBrep.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) IGA 1D Brep label in the model.
        Also see IGA1DBrep.FirstFreeLabel(),
        IGA1DBrep.LastFreeLabel() and
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA 1D Breps from that model can be selected.
            If the argument is a Flag then only IGA 1D Breps that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA 1D Breps can be selected.
            from any model
        modal : boolean
        ...
        -------
        int
            Number of IGA 1D Breps selected or None if menu cancelled
        """
        return Oasys.PRIMER._connection.classMethod(__class__.__name__, "Select", flag, prompt, limit, modal)

    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches all of the flagged IGA 1D Breps in the model. The IGA 1D Breps will be sketched until you either call
        IGA1DBrep.Unsketch(),
        IGA1DBrep.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Breps are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA 1D Breps several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA 1D Breps should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Breps are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGA1DBrep property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGA1DBrep.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA 1d brep (\*IGA_1D_BREP).
        Note that a carriage return is not added.
        See also IGA1DBrep.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA 1d brep.
        Note that a carriage return is not added.
        See also IGA1DBrep.Keyword()
    ...
    def Next(self):
        ...
        -------
        IGA1DBrep
            IGA1DBrep object (or None if there are no more IGA 1D Breps in the model)
        """
    ...
    def SetData(self, index, eid):
        ...
        None
            No return value
        """
        return Oasys.PRIMER._connection.instanceMethod(self.__class__.__name__, self._handle, "SetData", index, eid)
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches the IGA 1D Brep. The IGA 1D Brep will be sketched until you either call
        IGA1DBrep.Unsketch(),
        IGA1DBrep.UnsketchAll(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Brep is sketched.
            If omitted redraw is true. If you want to sketch several IGA 1D Breps and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Brep is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA 1D Breps and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
