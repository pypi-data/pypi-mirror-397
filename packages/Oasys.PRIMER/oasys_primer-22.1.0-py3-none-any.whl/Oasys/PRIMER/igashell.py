...
class IGAShellType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGAShell class attribute '{}' does not exist".format(name))


class IGAShell(Oasys.gRPC.OasysItem, metaclass=IGAShellType):
    _props = {'idfne', 'include', 'nisr', 'niss', 'pid', 'sid'}
...
# If one of the properties we define then get it
        if name in IGAShell._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGAShell._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGAShell instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGAShell._rprops:
            raise AttributeError("Cannot set read-only IGAShell instance attribute '{}'".format(name))
...
# Static methods
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGAShell objects is returned.
        If the property argument is given, that property value for each IGA Shell is returned in the list
        instead of a IGAShell object
        ...
        property : string
            Optional. Name for property to get for all IGA Shells in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGAShell objects is returned.
        If the property argument is given, then that property value for each IGA Shell is returned in the list
        instead of a IGAShell object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA Shells in the model
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Shells from that model can be selected.
            If the argument is a Flag then only IGA Shells that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA Shells can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches all of the flagged IGA Shells in the model. The IGA Shells will be sketched until you either call
        IGAShell.Unsketch(),
        IGAShell.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Shells are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA Shells several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA Shells should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Shells are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGAShell property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGAShell.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA shell (\*IGA_SHELL).
        Note that a carriage return is not added.
        See also IGAShell.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA shell.
        Note that a carriage return is not added.
        See also IGAShell.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches the IGA Shell. The IGA Shell will be sketched until you either call
        IGAShell.Unsketch(),
        IGAShell.UnsketchAll(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Shell is sketched.
            If omitted redraw is true. If you want to sketch several IGA Shells and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Shell is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA Shells and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
