...
class IGA1DNurbsXYZType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGA1DNurbsXYZ class attribute '{}' does not exist".format(name))


class IGA1DNurbsXYZ(Oasys.gRPC.OasysItem, metaclass=IGA1DNurbsXYZType):
...
# If one of the properties we define then get it
        if name in IGA1DNurbsXYZ._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGA1DNurbsXYZ._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGA1DNurbsXYZ instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGA1DNurbsXYZ._rprops:
            raise AttributeError("Cannot set read-only IGA1DNurbsXYZ instance attribute '{}'".format(name))
...
# Constructor
    def __init__(self, model, details):
        ...
        ----------
        model : Model
            Model that IGA 1d nurbs xyz will be created in
        details : dict
            Details for creating the IGA1DNurbsXYZ
...
# Static methods
    def Create(model, modal=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that the IGA 1D Nurbs XYZ will be created in
        modal : boolean
    ...
    def First(model):
        ...
        ----------
        model : Model
            Model to get first IGA 1D Nurbs XYZ in
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free IGA 1D Nurbs XYZ label in the model.
        Also see IGA1DNurbsXYZ.LastFreeLabel(),
        IGA1DNurbsXYZ.NextFreeLabel() and
        Model.FirstFreeItemLabel()

        Parameters
        ----------
        model : Model
            Model to get first free IGA 1D Nurbs XYZ label in
    ...
    def FlagAll(model, flag):
        ...
        ----------
        model : Model
            Model that all IGA 1D Nurbs XYZs will be flagged in
        flag : Flag
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGA1DNurbsXYZ objects is returned.
        If the property argument is given, that property value for each IGA 1D Nurbs XYZ is returned in the list
        instead of a IGA1DNurbsXYZ object

        Parameters
        ----------
        model : Model
            Model to get IGA 1D Nurbs XYZs from
        property : string
            Optional. Name for property to get for all IGA 1D Nurbs XYZs in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGA1DNurbsXYZ objects is returned.
        If the property argument is given, then that property value for each IGA 1D Nurbs XYZ is returned in the list
        instead of a IGA1DNurbsXYZ object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA 1D Nurbs XYZs in the model
    ...
    def GetFromID(model, number):
        ...
        ----------
        model : Model
            Model to find the IGA 1D Nurbs XYZ in
        number : integer
    ...
    def Last(model):
        ...
        ----------
        model : Model
            Model to get last IGA 1D Nurbs XYZ in
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free IGA 1D Nurbs XYZ label in the model.
        Also see IGA1DNurbsXYZ.FirstFreeLabel(),
        IGA1DNurbsXYZ.NextFreeLabel() and
        ...
        ----------
        model : Model
            Model to get last free IGA 1D Nurbs XYZ label in
        layer : Include number
            Optional. Include file (0 for the main file) to search for labels in (Equivalent to
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) IGA 1D Nurbs XYZ label in the model.
        Also see IGA1DNurbsXYZ.FirstFreeLabel(),
        IGA1DNurbsXYZ.LastFreeLabel() and
        Model.NextFreeItemLabel()

        Parameters
        ----------
        model : Model
            Model to get next free IGA 1D Nurbs XYZ label in
        layer : Include number
            Optional. Include file (0 for the main file) to search for labels in (Equivalent to
    ...
    def RenumberAll(model, start):
        ...
        ----------
        model : Model
            Model that all IGA 1D Nurbs XYZs will be renumbered in
        start : integer
    ...
    def RenumberFlagged(model, flag, start):
        ...
        ----------
        model : Model
            Model that all the flagged IGA 1D Nurbs XYZs will be renumbered in
        flag : Flag
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA 1D Nurbs XYZs from that model can be selected.
            If the argument is a Flag then only IGA 1D Nurbs XYZs that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA 1D Nurbs XYZs can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        IGA1DNurbsXYZ.Unsketch(),
        IGA1DNurbsXYZ.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        model : Model
            Model that all the flagged IGA 1D Nurbs XYZs will be sketched in
        flag : Flag
            Flag set on the IGA 1D Nurbs XYZs that you want to sketch
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Nurbs XYZs are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA 1D Nurbs XYZs several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA 1D Nurbs XYZs should be counted. If false or omitted
    ...
    def UnflagAll(model, flag):
        ...
        ----------
        model : Model
            Model that the defined flag for all IGA 1D Nurbs XYZs will be unset in
        flag : Flag
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all IGA 1D Nurbs XYZs will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Nurbs XYZs are unsketched.
    ...
    def UnsketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        model : Model
            Model that all IGA 1D Nurbs XYZs will be unsketched in
        flag : Flag
...
# Instance methods
    def AssociateComment(self, comment):
        ...
        ----------
        comment : Comment
            Comment that will be attached to the IGA 1D Nurbs XYZ
    ...
    def DetachComment(self, comment):
        ...
        ----------
        comment : Comment
            Comment that will be detached from the IGA 1D Nurbs XYZ
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGA1DNurbsXYZ property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGA1DNurbsXYZ.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA 1d nurbs xyz (\*IGA_1D_NURBS_XYZ).
        Note that a carriage return is not added.
        See also IGA1DNurbsXYZ.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA 1d nurbs xyz.
        Note that a carriage return is not added.
        See also IGA1DNurbsXYZ.Keyword()
    ...
    def SetControlPoint(self, index, data):
        ...
        control point X, Y, Z and control weight WGT. There are nr control points
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Nurbs XYZ is sketched.
            If omitted redraw is true. If you want to sketch several IGA 1D Nurbs XYZs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA 1D Nurbs XYZ is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA 1D Nurbs XYZs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
