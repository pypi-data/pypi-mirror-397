...
class IGASolidType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGASolid class attribute '{}' does not exist".format(name))


class IGASolid(Oasys.gRPC.OasysItem, metaclass=IGASolidType):
    _props = {'include', 'nisr', 'niss', 'nist', 'pid', 'rid', 'sid'}
...
# If one of the properties we define then get it
        if name in IGASolid._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGASolid._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGASolid instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGASolid._rprops:
            raise AttributeError("Cannot set read-only IGASolid instance attribute '{}'".format(name))
...
# Static methods
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGASolid objects is returned.
        If the property argument is given, that property value for each IGA Solid is returned in the list
        instead of a IGASolid object
        ...
        property : string
            Optional. Name for property to get for all IGA Solids in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGASolid objects is returned.
        If the property argument is given, then that property value for each IGA Solid is returned in the list
        instead of a IGASolid object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA Solids in the model
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Solids from that model can be picked.
            If the argument is a Flag then only IGA Solids that
            are flagged with limit can be selected.
            If omitted, or None, any IGA Solids from any model can be selected.
            from any model
        modal : boolean
    ...
    def RedrawAll(model):
        ...
        None
            No return value
        """
        return Oasys.PRIMER._connection.classMethod(__class__.__name__, "RedrawAll", model)
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Solids from that model can be selected.
            If the argument is a Flag then only IGA Solids that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA Solids can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches all of the flagged IGA Solids in the model. The IGA Solids will be sketched until you either call
        IGASolid.Unsketch(),
        IGASolid.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Solids are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA Solids several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA Solids should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all IGA Solids will be unblanked in
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Solids are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGASolid property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGASolid.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA solid (\*IGA_SOLID).
        Note that a carriage return is not added.
        See also IGASolid.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA solid.
        Note that a carriage return is not added.
        See also IGASolid.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        """
        Sketches the IGA Solid. The IGA Solid will be sketched until you either call
        IGASolid.Unsketch(),
        IGASolid.UnsketchAll(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Solid is sketched.
            If omitted redraw is true. If you want to sketch several IGA Solids and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Solid is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA Solids and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
