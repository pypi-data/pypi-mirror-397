...
class IGAVolumeXYZType(type):
    _consts = {'BASIS_TRANSFORM', 'NONE'}

    def __getattr__(cls, name):
        if name in IGAVolumeXYZType._consts:
            return Oasys.PRIMER._connection.classGetter(cls.__name__, name)

        raise AttributeError("IGAVolumeXYZ class attribute '{}' does not exist".format(name))


    def __setattr__(cls, name, value):
# If one of the constants we define then error
        if name in IGAVolumeXYZType._consts:
            raise AttributeError("Cannot set IGAVolumeXYZ class constant '{}'".format(name))

# Set the property locally
        cls.__dict__[name] = value


class IGAVolumeXYZ(Oasys.gRPC.OasysItem, metaclass=IGAVolumeXYZType):
    _props = {'esid', 'fsid', 'include', 'label', 'option', 'patchid', 'psid', 'vid'}
...
# If one of the properties we define then get it
        if name in IGAVolumeXYZ._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGAVolumeXYZ._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGAVolumeXYZ instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGAVolumeXYZ._rprops:
            raise AttributeError("Cannot set read-only IGAVolumeXYZ instance attribute '{}'".format(name))
...
# Static methods
    ...
    def FirstFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the first free IGA Volume XYZ label in the model.
        Also see IGAVolumeXYZ.LastFreeLabel(),
        IGAVolumeXYZ.NextFreeLabel() and
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGAVolumeXYZ objects is returned.
        If the property argument is given, that property value for each IGA Volume XYZ is returned in the list
        instead of a IGAVolumeXYZ object
        ...
        property : string
            Optional. Name for property to get for all IGA Volume XYZs in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        ...
        If the optional property argument is not given then a list of IGAVolumeXYZ objects is returned.
        If the property argument is given, then that property value for each IGA Volume XYZ is returned in the list
        instead of a IGAVolumeXYZ object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA Volume XYZs in the model
    ...
    def LastFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the last free IGA Volume XYZ label in the model.
        Also see IGAVolumeXYZ.FirstFreeLabel(),
        IGAVolumeXYZ.NextFreeLabel() and
    ...
    def NextFreeLabel(model, layer=Oasys.gRPC.defaultArg):
        """
        Returns the next free (highest+1) IGA Volume XYZ label in the model.
        Also see IGAVolumeXYZ.FirstFreeLabel(),
        IGAVolumeXYZ.LastFreeLabel() and
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Volume XYZs from that model can be selected.
            If the argument is a Flag then only IGA Volume XYZs that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA Volume XYZs can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        IGAVolumeXYZ.Unsketch(),
        IGAVolumeXYZ.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Volume XYZs are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA Volume XYZs several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA Volume XYZs should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Volume XYZs are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGAVolumeXYZ property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGAVolumeXYZ.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA volume xyz (\*IGA_VOLUME_XYZ).
        Note that a carriage return is not added.
        See also IGAVolumeXYZ.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA volume xyz.
        Note that a carriage return is not added.
        See also IGAVolumeXYZ.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Volume XYZ is sketched.
            If omitted redraw is true. If you want to sketch several IGA Volume XYZs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Volume XYZ is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA Volume XYZs and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
