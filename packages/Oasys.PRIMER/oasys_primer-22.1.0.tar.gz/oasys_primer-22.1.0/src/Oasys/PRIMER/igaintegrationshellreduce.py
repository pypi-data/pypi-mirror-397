...
class IGAIntegrationShellReduceType(type):

    def __getattr__(cls, name):

        raise AttributeError("IGAIntegrationShellReduce class attribute '{}' does not exist".format(name))


class IGAIntegrationShellReduce(Oasys.gRPC.OasysItem, metaclass=IGAIntegrationShellReduceType):
    _props = {'include', 'nrdr', 'nrds', 'patchid'}
...
# If one of the properties we define then get it
        if name in IGAIntegrationShellReduce._props:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

# If one of the read only properties we define then get it
        if name in IGAIntegrationShellReduce._rprops:
            return Oasys.PRIMER._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("IGAIntegrationShellReduce instance attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then error
        if name in IGAIntegrationShellReduce._rprops:
            raise AttributeError("Cannot set read-only IGAIntegrationShellReduce instance attribute '{}'".format(name))
...
# Static methods
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGAIntegrationShellReduce objects or properties for all of the IGA Integration Shell Reduces in a model in PRIMER.
        If the optional property argument is not given then a list of IGAIntegrationShellReduce objects is returned.
        If the property argument is given, that property value for each IGA Integration Shell Reduce is returned in the list
        instead of a IGAIntegrationShellReduce object
        ...
        property : string
            Optional. Name for property to get for all IGA Integration Shell Reduces in the model
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of IGAIntegrationShellReduce objects for all of the flagged IGA Integration Shell Reduces in a model in PRIMER
        If the optional property argument is not given then a list of IGAIntegrationShellReduce objects is returned.
        If the property argument is given, then that property value for each IGA Integration Shell Reduce is returned in the list
        instead of a IGAIntegrationShellReduce object
        ...
        property : string
            Optional. Name for property to get for all flagged IGA Integration Shell Reduces in the model
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Integration Shell Reduces from that model can be picked.
            If the argument is a Flag then only IGA Integration Shell Reduces that
            are flagged with limit can be selected.
            If omitted, or None, any IGA Integration Shell Reduces from any model can be selected.
            from any model
        modal : boolean
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            Optional. If the argument is a Model then only IGA Integration Shell Reduces from that model can be selected.
            If the argument is a Flag then only IGA Integration Shell Reduces that
            are flagged with limit can be selected (limit should be different to flag).
            If omitted, or None, any IGA Integration Shell Reduces can be selected.
            from any model
        modal : boolean
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        IGAIntegrationShellReduce.Unsketch(),
        IGAIntegrationShellReduce.UnsketchFlagged(),
        Model.UnsketchAll(),
        or delete the model
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Integration Shell Reduces are sketched.
            If omitted redraw is true. If you want to sketch flagged IGA Integration Shell Reduces several times and only
    ...
    def Total(model, exists=Oasys.gRPC.defaultArg):
        ...
        exists : boolean
            Optional. true if only existing IGA Integration Shell Reduces should be counted. If false or omitted
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Integration Shell Reduces are unsketched.
...
# Instance methods
    ...
    def GetParameter(self, prop):
        """
        Checks if a IGAIntegrationShellReduce property is a parameter or not.
        Note that object properties that are parameters are normally returned as the integer or
        float parameter values as that is virtually always what the user would want. For this function to
        work the JavaScript interpreter must use the parameter name instead of the value. This can be done by setting
        the Options.property_parameter_names option to true
        before calling the function and then resetting it to false afterwards..
        This behaviour can also temporarily be switched by using the IGAIntegrationShellReduce.ViewParameters()
    ...
    def Keyword(self):
        """
        Returns the keyword for this IGA integration shell reduce (\*IGA_SHELL).
        Note that a carriage return is not added.
        See also IGAIntegrationShellReduce.KeywordCards()
    ...
    def KeywordCards(self):
        """
        Returns the keyword cards for the IGA integration shell reduce.
        Note that a carriage return is not added.
        See also IGAIntegrationShellReduce.Keyword()
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        IGAIntegrationShellReduce.Unsketch(),
        IGAIntegrationShellReduce.UnsketchAll(),
        Model.UnsketchAll(),
        or delete the model
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Integration Shell Reduce is sketched.
            If omitted redraw is true. If you want to sketch several IGA Integration Shell Reduces and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            Optional. If model should be redrawn or not after the IGA Integration Shell Reduce is unsketched.
            If omitted redraw is true. If you want to unsketch several IGA Integration Shell Reduces and only
            redraw after the last one then use false for redraw and call
            View.Redraw()
