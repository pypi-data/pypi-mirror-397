...
from .utils.custom_mlp import MLP, Exp, ZeroBiasMLP, ZeroBiasMLP2
...
class DensityFlow(nn.Module):
    ...
    def setup_networks(self):
        ...
        if self.perturb_size>0:
            self.perturb_effect = nn.ModuleList()
            for _ in np.arange(self.perturb_size):
                if self.specific_mode=='none':
                    self.perturb_effect.append(ZeroBiasMLP(
                        [1+self.perturb_classes] + self.decoder_hidden_layers + [self.latent_dim],
                        activation=activate_fct,
                        output_activation=None,
                        post_layer_fct=post_layer_fct,
                        post_act_fct=post_act_fct,
                    ...
                    self.perturb_effect.append(ZeroBiasMLP(
                        [1+self.perturb_classes+self.latent_dim] + self.decoder_hidden_layers + [self.latent_dim],
                        activation=activate_fct,
                        output_activation=None,
                        post_layer_fct=post_layer_fct,
                        post_act_fct=post_act_fct,
    ...
    def _total_effects(self, zs, us, ts):
        zus = None
        for i in np.arange(self.perturb_size):
            if i==0:
                zus = self._cell_shift(zs, i, us[:,i].reshape(-1,1), ts)
            else:
                zus = zus + self._cell_shift(zs, i, us[:,i].reshape(-1,1), ts)
        return zus
    ...
    def predict(self, xs, perturbs_predict:list, perturbs_reference:list, perturb_type:list, library_sizes=None, show_progress=True):
        ...
        dzs_total = np.zeros_like(zs)
        for i,pert in enumerate(perturbs_predict):
            if pert in perturbs_reference:
                pert_idx = int(np.where(perturbs_reference==pert)[0])
            
                # perturbation effect 
                ps = np.ones([zs.shape[0],1])
                ts = np.ones([zs.shape[0],1]) * perturb_type[i]
                dzs = self.get_cell_shift(xs, perturb_idx=pert_idx, perturb_us=ps, perturb_type=ts, show_progress=show_progress)          
                dzs_total = dzs_total + dzs
    ...
    def _cell_shift(self, zs, perturb_idx, perturb, perturb_type):
        #zns,_ = self.encoder_zn(xs)    
        #zns,_ = self._get_basal_embedding(xs)   
        if perturb.ndim==2:
            if self.specific_mode=='none':
                ms = self.perturb_effect[perturb_idx]([perturb, perturb_type])
            else:
                ms = self.perturb_effect[perturb_idx]([perturb, perturb_type, zs])
        else:
            if self.specific_mode=='none':
                ms = self.perturb_effect[perturb_idx]([perturb.reshape(-1,1), perturb_type])
            else:
                ms = self.perturb_effect[perturb_idx]([perturb.reshape(-1,1), perturb_type, zs])
    ...
    def get_cell_shift(self, 
        ...
        with tqdm(total=len(dataloader), disable=not show_progress, desc='', unit='batch') as pbar:
            for Z_batch, idx in dataloader:
                ...
                T_batch = ts[idx].to(self.get_device())
                zns = self._cell_shift(Z_batch, perturb_idx, P_batch, T_batch)
                Z.append(tensor_to_numpy(zns))
                pbar.update(1)
