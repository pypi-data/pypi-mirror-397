...
def generate_client_methods_metadata(
    ...
    for route_path, route_methods in sorted(routes.items()):
        for http_method, route_infos in route_methods.items():
            ...
            # Filter out injected dependencies
            user_params = _filter_injected_params(list(route_info.signature.parameters.values()), registry)
            ...
            for param in user_params:
                ...
                if param.annotation != inspect.Parameter.empty:
                    param_info["type"] = _format_type_annotation(param.annotation)
...
def _filter_injected_params(
    ...
    return [p for p in params if not registry.is_dependency(p.name, p)]
