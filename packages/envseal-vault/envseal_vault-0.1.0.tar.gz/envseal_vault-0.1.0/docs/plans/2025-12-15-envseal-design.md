...
│   ├── scanner.py        # Scan repos and .env*
│   ├── vault.py          # Vault path mapping
│   └── sops.py           # SOPS encryption/decryption
└── tests/

secrets-vault/              # Private encrypted repository
├── .sops.yaml             # SOPS encryption rules
├── secrets/               # Encrypted .env files
│   ├── repo1/
│   │   ├── dev.env       # SOPS encrypted
│   │   └── prod.env
│   └── repo2/
│       └── local.env
└── templates/             # .env.example templates (optional)
```
...
envseal init
  → Auto-generate age key (if not exists)
  → Scan current directory for Git repos
  → Interactive selection of repos to manage
  → Generate ~/.config/envseal/config.yaml
  → Create .sops.yaml in secrets-vault
  → Auto-execute first push
...
- `env_mapping` supports customization (e.g., `.env.test` → `test`)
...
```
1. Scan Phase
   ├─ Read config.yaml to get repos list
   ├─ Iterate each repo, find .env* files
   ├─ Apply env_mapping to determine environment type
   └─ Generate metadata: {repo, env, path, mtime, hash}

2. Normalization Phase
   ├─ Parse using python-dotenv into key-value dict
   ├─ Sort keys alphabetically
   ├─ Remove empty lines and comments (keep clean)
   └─ Normalize quote format (only quote if spaces/special chars)

3. Vault Write Phase
   ├─ Map path: secrets/<repo>/<env>.env
   ├─ Write to temp file first (plaintext)
   └─ Detect changes (compare with existing decrypted content)

4. Encryption Phase
   ├─ Call: sops -e --input-type dotenv temp.env
   ├─ Write to: secrets/<repo>/<env>.env (encrypted)
   └─ Delete temp file

5. Git Prompt Phase
   └─ Prompt user to cd to vault and git commit/push
```
...
3. Based on parameters:
   ├─ Default: Write to /tmp/envseal-XXXXX/<env>.env (show path)
   ├─ --replace: Write to original repo's .env.<env> file
...
1. Age Key Storage
   ...
   ├─ Permissions: Auto-set to 600 (owner read/write only)
   └─ Backup reminder: init prompts user to backup key (for multi-device sync)

2. Plaintext Temp Files
   ├─ Only briefly exist in memory or /tmp
   ├─ Use tempfile.mkdtemp() for random directory
   └─ Auto-cleanup via atexit on process exit

3. Environment Variable Injection (optional feature)
   └─ envseal run repo1 --env prod -- command
       Decrypt to memory → Set env vars → Execute command → Cleanup
```
...
1. Prevent Plaintext Push
   ├─ Vault repo .gitignore includes *.env.tmp
   ├─ push command checks: ensure only SOPS-encrypted files written
   └─ Recommend enabling GitHub Secret Scanning on vault repo

2. Prevent Local File Overwrite
   ├─ pull --replace backs up original file to .env.backup first
   └─ Display clear warning message

3. Vault Integrity Checks
   ├─ Check .sops.yaml exists before push
   ├─ Verify age key configured correctly
   └─ Test encrypt/decrypt on sample file to validate config
```
...
1. Friendly Error Messages
   ├─ Age key missing → Prompt to run envseal init
   ├─ sops not installed → Show install command
   └─ Vault path doesn't exist → Prompt to configure or create

2. Partial Failure Handling
   ├─ When pushing multiple repos, one failure doesn't affect others
   └─ Show summary of success/failure list at end

3. Rollback Mechanism
   └─ Rely on Git: vault is Git repo, any issue can git revert
```
...
│   ├── scanner.py            # Scan repos and .env*
│   ├── dotenvio.py          # Parse/normalize/write .env
│   ├── vault.py             # Vault path mapping and management
│   ├── sops.py              # SOPS CLI wrapper
│   ├── crypto.py            # Age key generation and management
│   ├── diffing.py           # Key-only diff/status
...
✅ Age key created: ~/Library/Application Support/sops/age/keys.txt

⚠️  IMPORTANT: Back up this key! You'll need it on other devices.
...
✅ Created .sops.yaml in secrets-vault

🔄 Pushing initial secrets to vault...
  ✓ project1: dev.env, prod.env
  ✓ my-api: local.env, prod.env
  ✓ client-app: dev.env, staging.env, prod.env
...
1. Unit Tests (pytest)
   ├─ test_scanner.py
   │   ├─ Test Git repo identification
   │   ├─ Test .env* file discovery
   │   └─ Test ignore_dirs filtering
   │
   ├─ test_dotenvio.py
   │   ├─ Test .env parsing (various formats)
   │   ├─ Test normalization output
   │   └─ Test quote handling edge cases
   │
   └─ test_diffing.py
       ├─ Test key-only diff logic
       └─ Test status generation

2. Integration Tests
   ├─ test_integration.py
   │   ├─ Full init → push → diff → pull flow
   │   ├─ Use real SOPS encrypt/decrypt
   │   └─ Use temporary Git repos and vault
   │
   └─ Fixtures (conftest.py)
       ├─ Create temp repos (with .env files)
       ├─ Create temp vault
       └─ Generate test age keys

3. CLI Tests
   └─ Use Typer's CliRunner to test command-line interactions
```
...
1. **Critical path**: init → scan → push → status (must have integration tests)
