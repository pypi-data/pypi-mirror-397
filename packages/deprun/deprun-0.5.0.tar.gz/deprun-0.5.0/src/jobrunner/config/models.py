...
class ArgumentSpec(BaseModel):
    """Argument specification for a job."""
    name: str = Field(..., description="Argument name (environment variable)")
...
class Job(BaseModel):
    ...
    needs: List[ArgumentSpec] = Field(default_factory=list, description="Required arguments for the job")
...
class Template(BaseModel):
    ...
    needs: list[ArgumentSpec] | None = None
...
class Config(BaseModel):
    ...
    versions_file: Optional[str] = Field(None, alias="versions-file")
    version_refs: Dict[str, str] = Field(default_factory=dict, alias="version-refs")
