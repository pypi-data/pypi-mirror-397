"""
Stronghold Crusader Extreme DirectPlay protocol implementation for game server discovery.
"""
...
class StrongholdCEProtocol(ProtocolBase):
    """Stronghold Crusader Extreme DirectPlay protocol handler for broadcast discovery"""
    
    def __init__(self, timeout: float = 5.0):
        # Stronghold CE uses DirectPlay UDP port - get from opengsq-python constants
        from opengsq.protocols.directplay import DirectPlay
        from opengsq.protocols.stronghold_ce import StrongholdCE as OpenGSQStrongholdCE
        ...
        self.protocol_config = {
            ...
            'tcp_port': DirectPlay.DIRECTPLAY_TCP_PORT,  # 2300 - Standard DirectPlay TCP port!
            'query_data': OpenGSQStrongholdCE.STRONGHOLD_CE_UDP_PAYLOAD  # Use payload from opengsq-python
        }
    
    def get_discord_fields(self, server_info: dict) -> list:
        """
        Get additional Discord embed fields for Stronghold Crusader Extreme servers.
        ...
        # Add DirectPlay specific info with standard TCP port
        tcp_port = self.protocol_config.get('tcp_port', 2300)
        fields.append({
            'name': '🔧 Protokoll',
    ...
    async def scan_servers(self, scan_ranges: List[str]) -> List[ServerResponse]:
        """
        Scan for Stronghold Crusader Extreme servers using DirectPlay protocol.
        
        DirectPlay works by:
        1. Opening a TCP server on port 2300 (standard DirectPlay port) to receive responses
        ...
        Returns:
            List of ServerResponse objects for Stronghold Crusader Extreme servers
        """
        servers = []
        udp_port = self.protocol_config['port']  # 47624
        tcp_port = self.protocol_config['tcp_port']  # 2300
        
        self.logger.info("🏰 Starting Stronghold Crusader Extreme DirectPlay server discovery...")
        ...
        for network_range in scan_ranges:
            try:
                ...
                self.logger.info(f"📡 DirectPlay discovery for Stronghold CE: {broadcast_addr}:{udp_port}")
                ...
                self.logger.info(f"🔍 DirectPlay found {len(discovered_servers)} potential Stronghold CE servers")
                ...
                for server_ip, server_port in discovered_servers:
                    try:
                        ...
                        server_info_dict = await self._query_stronghold_ce_server_via_opengsq(
                        ...
                        if server_info_dict:
                            ...
                            self.logger.info(f"✅ Stronghold CE server discovered: {server_ip}:{server_port}")
                            ...
                            self.logger.debug(f"❌ Could not get details for Stronghold CE server {server_ip}:{server_port}")
                        
                    except Exception as e:
                        self.logger.debug(f"❌ Failed to query Stronghold CE server {server_ip}:{server_port}: {e}")
        ...
        self.logger.info(f"🎯 Stronghold CE DirectPlay discovery completed: {len(servers)} servers found")
        return servers
    ...
    async def _directplay_discovery(self, broadcast_addr: str, udp_port: int, tcp_port: int, query_data: bytes) -> List[Tuple[str, int]]:
        ...
        Args:
            ...
            tcp_port: TCP port to listen on (2300 for Stronghold CE)
        ...
        try:
            ...
            # Start TCP server on port 2300 for Stronghold CE (or next available port)
            tcp_server = None
    ...
    async def _query_stronghold_ce_server_via_opengsq(self, host: str, port: int) -> Optional[Dict[str, Any]]:
        """
        Query a Stronghold Crusader Extreme server using opengsq-python library to get complete server information.
        ...
        """
        try:
            self.logger.debug(f"Querying Stronghold CE server {host}:{port} via opengsq-python...")
            
            # Create Stronghold CE protocol instance
            stronghold_client = StrongholdCE(host, port, self.timeout)
            ...
            info_dict = {
                'name': getattr(server_info, 'name', f'Stronghold CE Server {host}:{port}'),
                ...
                'game_version': getattr(server_info, 'game_version', '1.4.1'),
                'game_type': getattr(server_info, 'game_type', 'Stronghold Crusader Extreme'),
                'raw': getattr(server_info, 'raw', {})
            }
            
            self.logger.debug(f"✅ opengsq-python returned Stronghold CE server info for {host}:{port}")
            ...
            self.logger.debug(f"❌ Error querying Stronghold CE server {host}:{port} via opengsq-python: {e}")
            return None
