...
class VmGcProfileComputerName(object):

    """Computer name of the VM.

    :param value: (:attr:`value`) Computer name value.
    :type value: required
    ...
    def _initialize_object_type(self):
        return 'vmm.v4.ahv.config.VmGcProfileComputerName'
    ...
    def value(self, value):
        ...
        if value is not None and not re.search('^[^\\s{}~[\\]\\\\^\':;<=>?@!\"#$%`()+\/.,*&]{1,15}$', value):  # noqa: E501
            raise ValueError(r"Invalid value for `value`, must be a follow pattern or equal to `/^[^\\s{}~[\\]\\\\^':;<=>?@!\"#$%`()+\/.,*&]{1,15}$/`")  # noqa: E501
        ...
        for attr, attr_type in six.iteritems(self.swagger_types):
            ...
            if isinstance(value, list):
                ...
                if sanitize:
                    ...
                    result[attr] = value
        if issubclass(VmGcProfileComputerName, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, VmGcProfileComputerName):
            return False
