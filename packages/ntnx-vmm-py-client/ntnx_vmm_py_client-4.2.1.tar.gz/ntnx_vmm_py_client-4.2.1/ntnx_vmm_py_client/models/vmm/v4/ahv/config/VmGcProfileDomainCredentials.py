...
class VmGcProfileDomainCredentials(object):

    """Credentials of the domain account to use to join the domain.

    :param domain_name: (:attr:`domain_name`) The name of the domain to use for authentication of the account before the computer can be joined to a domain. A domain name can be the fully qualified DNS name or the NetBIOS name of the domain.
    :type domain_name: required
     min=1 chars, max=255 chars
    
    :param username: (:attr:`username`) Name of the domain user account with permission to add the computer to a domain.
    :type username: required
     min=1 chars, max=256 chars
    
    :param password: (:attr:`password`) The password of the domain user account to use for authenticating an account to the domain before the computer can be joined to a domain.
    ...
    def _initialize_object_type(self):
        return 'vmm.v4.ahv.config.VmGcProfileDomainCredentials'
        ...
        for attr, attr_type in six.iteritems(self.swagger_types):
            ...
            if isinstance(value, list):
                ...
                if sanitize:
                    ...
                    result[attr] = value
        if issubclass(VmGcProfileDomainCredentials, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, VmGcProfileDomainCredentials):
            return False
