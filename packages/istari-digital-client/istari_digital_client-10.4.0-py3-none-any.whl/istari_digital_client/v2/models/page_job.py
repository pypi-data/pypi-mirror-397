...
class PageJob(Pageable[Job], ClientHaving):
    """
    A paginated response object for listing jobs.

    This class represents a single page of :class:`Job` instances returned from the API.
    It includes pagination metadata such as page number and page size, along with the
    list of jobs retrieved in the current page.
    """
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "items": [Job.from_dict(_item) for _item in obj["items"]]
