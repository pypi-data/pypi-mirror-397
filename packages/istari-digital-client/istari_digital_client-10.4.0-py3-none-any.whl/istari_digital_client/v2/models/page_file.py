...
class PageFile(Pageable[File], ClientHaving):
    """
    A paginated response object for listing files.

    This class represents a single page of :class:`File` results returned from the API.
    It contains pagination metadata such as page size, page number, and total result count,
    along with the list of files retrieved in the current page.
    """
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "items": [File.from_dict(_item) for _item in obj["items"]]
