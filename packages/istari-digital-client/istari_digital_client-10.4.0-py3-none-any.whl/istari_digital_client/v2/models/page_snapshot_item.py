...
class PageSnapshotItem(Pageable[SnapshotItem]):
    """
    A paginated response object for listing snapshot items.

    This class represents a single page of :class:`SnapshotItem` instances returned from the API.
    It includes pagination metadata such as page number and size, and a list of snapshots that
    represent stored versions of resources at a specific point in time.
    """
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "items": [SnapshotItem.from_dict(_item) for _item in obj["items"]]
