...
class ScoutFunctionDecorator:
    ...
    def update_progress(self, progress: FunctionProgress) -> None:
        signed_upload_dictionary = scout.context.get(
            "SCOUT_FUNCTION_PROGRESS_UPLOAD_URL"
        )
        if not signed_upload_dictionary:
            print(
                "Unable to update progress. Ignore if you are in dev mode, if not, perhaps you forgot to add supports_progress flag to @scout.function?"
            )
            return

        progress_json = json.dumps(progress.model_dump())
