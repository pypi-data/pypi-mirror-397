...
class SkillsSyncModule:
    ...
    def _load_skill_ids(self) -> None:
        ...
        if file.exists():
            with open(file, "r") as f:
                try:
                    self._deployed_ids = DeployedSkillIds.model_validate(json.load(f))
                except Exception as e:
    ...
    def _find_skill_id(self, skill: ConfigSkill) -> str | None:
        ...
        for url_ids in self._deployed_ids.ids_by_url:
            if url_ids.url == self._api_url:
                # Find the matching skill ID
                for skill_id in url_ids.ids:
    ...
    def _update_deployed_skill_id(self, skill_id: str, deployed_id: str) -> None:
        ...
        if not url_entry:
            url_entry = DeployedSkillUrlIds(url=self._api_url, ids=[])
            self._deployed_ids.ids_by_url.append(url_entry)

        # Check if an entry for this skill ID already exists
        existing_entry = None
        for skill_id_entry in url_entry.ids:
            if skill_id_entry.id == skill_id:
                existing_entry = skill_id_entry
                break

        if existing_entry:
            # Update the existing entry with the deployed skill ID
            existing_entry.skill_id = deployed_id
        else:
            # Add a new entry
            url_entry.ids.append(SkillIds(id=skill_id, skill_id=deployed_id))
