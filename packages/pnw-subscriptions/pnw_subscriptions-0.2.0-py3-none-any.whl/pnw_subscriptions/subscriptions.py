...
def ensure_node() -> str:
    """
    Ensure Node.js is downloaded and executable. Returns path to Node binary.
    ...
    if archive_path.suffixes[-2:] == ['.tar', '.xz'] or archive_path.suffix == ".gz":
        with tarfile.open(archive_path) as tar:
            tar.extractall(path=node_dir)
        node_folder = next(node_dir.glob(f"node-v{NODE_VERSION}-{system}-x64*"))
