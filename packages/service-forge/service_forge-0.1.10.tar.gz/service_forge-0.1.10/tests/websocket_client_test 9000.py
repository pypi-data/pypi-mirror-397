...
def trigger_workflow(client_id):
    ...
    try:
        ...
        # 发送HTTP请求触发工作流
        url = f"http://localhost:9000/api/v1/tags?client_id={client_id}"
...
async def connect_without_client_id():
    ...
    uri = f"ws://localhost:9000/ws/connect"
    print("使用新客户端ID连接")
    print(f"正在连接到 {uri}...")
...
async def reconnect_with_client_id(client_id):
    ...
    uri = f"ws://localhost:9000/ws/connect?client_id={client_id}"
...
if __name__ == "__main__":
    ...
    print("请确保服务已启动并运行在 http://localhost:9000")
    print()
