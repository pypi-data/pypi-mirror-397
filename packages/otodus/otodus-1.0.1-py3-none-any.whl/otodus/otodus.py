...
CRAWLERX_PATH = Path.home() / "myenv" / "bin" / "crawlerx"
...
class CrawlerXRunner:
    """Runs CrawlerX on target"""
   
    def __init__(self, crawlerx_path: Path):
        self.crawlerx_path = crawlerx_path
   
    def check_crawlerx(self) -> bool:
        """Check if CrawlerX is available"""
        if not self.crawlerx_path.exists():
            log(f"❌ CrawlerX not found at {self.crawlerx_path}", "error")
            log("Install: pip install crawlerx", "info")
            return False
        return True
   
    def run_crawler(self, target_url: str, output_dir: Path) -> Optional[Path]:
        """Run CrawlerX on target URL"""
        if not self.check_crawlerx():
            return None
       
        reasoning_log(f"🕷️ Running CrawlerX on {target_url}", "recon")
       
        cmd = [
            str(self.crawlerx_path),
            "-u", target_url,
            "-o", str(output_dir)
        ]
