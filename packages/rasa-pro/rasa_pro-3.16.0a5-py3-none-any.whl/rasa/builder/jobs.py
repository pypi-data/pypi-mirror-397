...
async def run_train_job(
    ...
    """Run a training job on the current project files.

    This validates and trains the bot model using the current files on disk,
    without any file modifications. Useful when files have been edited externally.
    ...
    try:
        ...
        error_message = str(exc)
        structlogger.debug(
            "train_job.operation_in_progress",
            job_id=job.id,
            error=error_message,
        )
        await push_job_status_event(job, JobStatus.error, message=error_message)
        ...
        # Get current bot files for copilot analysis
        bot_files = project_generator.get_bot_files()
