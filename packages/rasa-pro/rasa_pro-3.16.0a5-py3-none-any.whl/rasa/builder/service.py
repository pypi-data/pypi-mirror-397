...
def get_model_trained_at(request: Request) -> Optional[str]:
    ...
    Returns:
        ISO 8601 formatted timestamp string, or None if no model is loaded.
    """
    agent: Optional[Agent] = request.app.ctx.agent
    if agent is None or not agent.is_ready():
        return None

    if not agent.processor or not agent.processor.model_metadata:
        return None

    trained_at: datetime = agent.processor.model_metadata.trained_at
    # Ensure trained_at is timezone-aware (UTC)
    if trained_at.tzinfo is None:
        trained_at = trained_at.replace(tzinfo=timezone.utc)

    return trained_at.isoformat()


def has_files_changed_since_training(request: Request) -> bool:
    """Check if any bot files have been modified since the last training.

    Args:
        request: The incoming request.

    Returns:
        True if files have changed since last training, False otherwise.
        Also returns True if no agent is loaded (since there's no trained model).
    """
    from rasa.builder.project_generator.project_utils import bot_file_paths

    agent: Optional[Agent] = request.app.ctx.agent
    if agent is None or not agent.is_ready():
        # No trained model, so consider files as "changed" (needs training)
        return True

    # Get the training timestamp from model metadata
    if not agent.processor or not agent.processor.model_metadata:
        return True
    ...
    # Get project folder and check file modification times
    project_generator = get_project_generator(request)
    project_folder = project_generator.project_folder

    for file_path in bot_file_paths(project_folder, exclude_models_directory=True):
        try:
            mtime = file_path.stat().st_mtime
            file_modified_at = datetime.fromtimestamp(mtime, tz=timezone.utc)
            if file_modified_at > trained_at:
                return True
        except OSError:
...
@openapi.response(
    200,
    {
        "application/json": {
            ...
            "auth_required": bool,
            "agent_status": str,
            "files_changed_since_training": bool,
    ...
    return response.json(
        {
            ...
            "files_changed_since_training": has_files_changed_since_training(request),
            "model_trained_at": get_model_trained_at(request),
        }
    )
...
@openapi.summary("Train the bot model")
@openapi.description(
    "Triggers a training run on the current project files. Use this when files "
    "have been modified externally (e.g., in an editor) and you want to retrain "
    "the model without providing the files via the API. "
    ...
    "4. `training` - Training the bot model\n"
    "5. `train_success` - Model training completed\n"
    "6. `done` - Training completed\n\n"
    "**Error Events (can occur at any time):**\n"
    "- `validation_error` - Bot configuration files are invalid\n"
    "- `train_error` - Training failed\n"
    "- `copilot_analysis_start` - Copilot analysis started (includes `copilot_job_id` "
    "in payload)\n"
    "- `error` - Unexpected error occurred\n\n"
    "**Copilot Analysis Events:**\n"
    ...
    "- `copilot_analysis_error` - Copilot analysis failed\n\n"
    "**Usage:**\n"
    "1. Send POST request (no body required)\n"
    "2. The response will be a JSON object `{job_id: ...}`\n"
    "3. Connect to `/job-events/<job_id>` for a server-sent event stream of progress."
)
@openapi.tag("training")
@openapi.response(
    200,
    {"application/json": model_to_schema(JobCreateResponse)},
    description=(
        "Job created. Poll or subscribe to /job-events/<job_id> "
        "for progress and SSE updates on training."
    ),
)
@openapi.response(
    500,
...
@protected()
async def trigger_training(request: Request) -> HTTPResponse:
    """Trigger a training run on the current project files."""
    try:
        job = job_manager.create_job()
        request.app.add_task(run_train_job(request.app, job))
        return response.json(JobCreateResponse(job_id=job.id).model_dump(), status=200)
    except Exception as exc:
        capture_exception_with_context(
            exc,
            "bot_builder_service.trigger_training.unexpected_error",
            tags={"endpoint": "/api/train", "method": "POST"},
        )
        return response.json(
            ApiErrorResponse(
                error="Failed to start training job", details={"error": str(exc)}
            ).model_dump(),
            status=HTTPStatus.INTERNAL_SERVER_ERROR,
        )
...
# Path to the frontend dist directory (relative to this file's location)
FRONTEND_DIST_PATH = Path(__file__).parent / "frontend"


def setup_frontend_static_routes(app: Sanic) -> None:
    """Setup static file serving for the frontend SPA.

    This configures the Sanic app to serve the frontend build from
    ../rasa-spark-genius/frontend/dist relative to the rasa repository root.
    ...
    if not frontend_path.exists():
        structlogger.warning(
            "bot_builder_service.frontend.not_found",
            path=str(frontend_path),
            message="Frontend dist directory not found, skipping static file setup",
        )
        return

    structlogger.info(
        "bot_builder_service.frontend.setup",
        path=str(frontend_path),
    )

    # Serve static assets (js, css, images, etc.) from the assets directory
    assets_path = frontend_path / "assets"
    if assets_path.exists():
        app.static("/assets", str(assets_path), name="frontend_assets")


def setup_frontend_404_handler(app: Sanic) -> None:
    """Setup a 404 handler that serves the frontend SPA for unmatched GET requests.

    This approach ensures that API, webhooks, socket.io, etc. routes are tried first,
    and only truly unmatched routes fall through to serve the frontend.
    ...
    async def handle_404_with_frontend(
        request: Request, exception: NotFound
    ) -> HTTPResponse:
        """Serve frontend for unmatched GET requests, 404 for others."""
        ...
        # Check if the path points to an actual file in dist (e.g., favicon.ico)
        if path:
            file_path = FRONTEND_DIST_PATH / path
            # Security: ensure we don't serve files outside dist directory
            try:
                file_path.resolve().relative_to(FRONTEND_DIST_PATH.resolve())
        ...
        return await response.file(str(index_path), mime_type="text/html")
