...
from rasa.builder.service import (
    bp,
    setup_frontend_404_handler,
    setup_frontend_static_routes,
...
def create_app(project_folder: str) -> Sanic:
    ...
    # Can be disabled via RASA_BUILDER_DISABLE_AUTH=true environment variable
    app.config.USE_AUTHENTICATION = (
        os.environ.get("RASA_BUILDER_DISABLE_AUTH", "false").lower() != "true"
    )
    ...
    # Setup frontend static file serving (404 handler is set up later in main()
    # after register_custom_sanic_error_handler to avoid being overwritten)
    setup_frontend_static_routes(app)
...
def main(project_folder: Optional[str] = None) -> None:
    ...
    try:
        ...
        # Setup frontend 404 handler AFTER custom error handler registration
        # (otherwise the custom error handler replaces our 404 handler)
        setup_frontend_404_handler(app)
