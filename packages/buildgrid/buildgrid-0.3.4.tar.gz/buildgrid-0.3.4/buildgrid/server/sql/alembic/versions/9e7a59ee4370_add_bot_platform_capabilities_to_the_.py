...
"""Add bot platform capabilities and job scheduling delay

Revision ID: 9e7a59ee4370
Revises: 1f959c3834d3
Create Date: 2025-05-30 14:24:15.278812
...
revision = "9e7a59ee4370"
down_revision = "1f959c3834d3"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("bot_name", "platform"),
    )

    # Use a covering index in PostgreSQL to enable a cheaper index-only scan
    # for this table when matching bots to jobs.
    op.create_index(
        op.f("ix_bot_platforms_platform"), "bot_platforms", ["platform"], unique=False, postgresql_include=["bot_name"]
    )
    op.create_index(op.f("ix_bots_instance_name"), "bots", ["instance_name"], unique=False)
    ...
    table = sa.Table(
        ...
        sa.Column("worker_name", sa.String, index=True, nullable=True),
    ...
    with op.batch_alter_table("jobs", copy_from=table) as batch_op:
        batch_op.add_column(sa.Column("schedule_after", sa.DateTime, nullable=False, server_default=sa.func.now()))


def downgrade() -> None:
    op.drop_index(op.f("ix_bot_platforms_platform"), table_name="bot_platforms")
    op.drop_index(op.f("ix_bots_instance_name"), table_name="bots")
    op.drop_table("bot_platforms")
    ...
    table = sa.Table(
        ...
        sa.Column("stderr_stream_name", sa.String, nullable=True),
        sa.Column("stderr_stream_write_name", sa.String, nullable=True),
        sa.Column("schedule_after", sa.DateTime, nullable=False, server_default=sa.func.now()),
    )

    with op.batch_alter_table("jobs", copy_from=table) as batch_op:
        batch_op.drop_column("schedule_after")
