...
def test_wildcard_bot_lease_assignment(isolated_controllers):
    ...
    with instance_context("instance-b"):
        ...
        leases = scheduler_b.synchronize_bot_leases(bot_name, bot_id, BotStatus.OK.value, leases)
        assert len(leases) == 0
...
def test_proactive_fetch_to_capacity_enabled(controller_postgres):
    ...
    new_leases = scheduler.synchronize_bot_leases(
        bot_name, bot_id, BotStatus.OK.value, completed_leases + remaining_leases
    )
