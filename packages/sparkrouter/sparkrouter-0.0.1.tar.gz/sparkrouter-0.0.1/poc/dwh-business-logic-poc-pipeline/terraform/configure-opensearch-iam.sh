...
LAMBDA_ROLE_ARN=$(terraform output -raw lambda_job_events_role_arn 2>/dev/null || echo "")

if [ -z "$OPENSEARCH_ENDPOINT" ] || [ -z "$LAMBDA_ROLE_ARN" ]; then
  echo "ERROR: Could not get terraform outputs. Make sure terraform has been applied."
  echo "  OPENSEARCH_ENDPOINT: ${OPENSEARCH_ENDPOINT:-<not set>}"
  echo "  LAMBDA_ROLE_ARN: ${LAMBDA_ROLE_ARN:-<not set>}"
  exit 1
fi
