...
class SQLExampleJobFactory(AbstractJobFactory):
    """
    Factory for creating SQLExampleJob instances.
    """
    ...
    def create_job(self, **kwargs) -> SQLExampleJob:
        """Create a SQLExampleJob with the specified configuration."""
        config = self.parse_job_config(job_name='sql_example_job', **kwargs)
        print("Configuration for SQLExampleJob:", config)
...
def main(**kwargs):
    """
    Entrypoint for SQLExample job.
    """
    operator = SQLExampleJobFactory(**kwargs)
