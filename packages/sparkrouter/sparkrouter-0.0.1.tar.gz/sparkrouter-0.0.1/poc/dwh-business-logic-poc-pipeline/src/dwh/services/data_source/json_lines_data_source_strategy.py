...
class JsonLinesDataSourceStrategy(DataSourceStrategy):
    """
    Strategy for reading from JSONL files with strict schema enforcement.
    ...
    def __init__(self, spark: SparkSession, schema_service: SchemaService, path: str, recursive: bool = False,
    ...
    def get_source_df(self, schema_ref=None, table_name=None) -> DataFrame:
        ...
        df = self._read_json_lines_data(required_schema, table_name)
    ...
    def _read_json_lines_data(self, required_schema: StructType, table_name: str = None) -> DataFrame:
        """Isolated backend operation for reading json lines """
        ...
        if table_name and not self.base_path.endswith('.txt') and not self.base_path.endswith('.jsonl'):
            read_path = f"{self.base_path}/{table_name}"
        else:
            read_path = self.base_path

        print(f"JSONL SOURCE: base_path={self.base_path}, table_name={table_name}, read_path={read_path}")

        # TODO: Read JSON Lines data with required schema
        reader = self.spark.read.schema(required_schema)
        if self.recursive:
            return reader.option("recursiveFileLookup", "true").json(read_path)
        else:
            return reader.json(read_path)
    ...
    def _register_table(self, df: DataFrame, read_path: str) -> None:
        ...
        view_name = view_name.replace('/', '_').replace('.', '_') or "json_lines_source"
