"""
Unit tests for SQLExampleJob parameter validation
"""
import pytest
...
class TestSQLExampleJobParameterValidation:
    """Unit tests for parameter validation in SQLExampleJob"""

    @pytest.fixture
    def job_factory(self):
        """Create job factory with Noop dependencies"""
        return SQLExampleJobFactory(
    ...
    def test_missing_sql_example_job_config(self, job_factory):
        """Test that missing sql_example_job config raises ValueError"""
        with pytest.raises(ValueError, match="sql_example_job value must be a dict"):
            job_factory.create_job(
                distribution_list="test@example.com",
                from_addr="noreply@example.com"
            )
