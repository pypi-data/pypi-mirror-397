...
async def _search_flight(
        ...
        timeout: float = 10.0, price_increase_threshold: float = 20.0, price_reduction_threshold: float = 10.0
) -> None:
    try:
        # 1. 判断是否存在出行提醒
    ...
    # 5. 判断价格是否在上浮，下降阈值区间内
    if amount > price_sell + price_increase_threshold:
        raise RuntimeError(
            f"单航程航班查询页面，航班<{flight_no}>官网价：{amount} 高于：订单销售价[{price_sell}] + 上浮阈值[{price_increase_threshold}]，损失过高")
    if amount < price_sell - price_reduction_threshold:
        raise RuntimeError(
            f"单航程航班查询页面，航班<{flight_no}>官网价：{amount} 低于：订单销售价[{price_sell}] - 下降阈值[{price_reduction_threshold}]，收益过高")
...
async def booking_flight_callback(
        *, page: Page, logger: Logger, dep_city: str, arr_city: str, flight_no: str, cabin_class: str,
        ...
        order_cabin_code: str, dep_date: str, protocol: str, domain: str, timeout: float = 10.0,
    ...
    await _search_flight(
        ...
        prodcut_name=prodcut_name, price_increase_threshold=price_increase_threshold,
        price_reduction_threshold=price_reduction_threshold, timeout=timeout,
    )
