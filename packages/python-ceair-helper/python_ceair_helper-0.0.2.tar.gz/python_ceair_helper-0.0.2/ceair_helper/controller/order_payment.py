...
async def _select_world_pay_type(payment_po: PaymentPage, logger: Logger, timeout: float = 10.0) -> None:
    # 1. 获取待支付的订单号
    pre_order_number = await payment_po.get_pre_order_number(timeout=timeout)
    logger.info(f'支付页面，订单号：{pre_order_number}获取成功')
...
async def _fill_cvv_payment(
    ...
    logger.info(f'支付详情页面，订单号：{pre_order_number}获取成功')
    ...
    card_number: str = card_info.get("number")
    ...
    logger.info(f"支付详情页面，卡片信息区域，有效期年份<{card_number}>选择完成")
    ...
    logger.info(f"支付详情页面，卡片信息区域，有效期月份<{card_number}>选择完成")
    ...
    logger.info(f"支付详情页面，个人信息区域，城市<{payment_info.get("city")}>输入完成")

    # 14. 填写街道名称
    street_input = await cvv_po.get_form_street_input(timeout=timeout)
    await street_input.fill(value=payment_info.get("street"))
    logger.info(f"支付详情页面，个人信息区域，街道名称<{payment_info.get("street")}>输入完成")
    ...
    logger.info(f"支付详情页面，个人信息区域，门牌号吗<{payment_info.get("house_number")}>输入完成")
    ...
    logger.info(f"支付详情页面，个人信息区域，邮政编码<{payment_info.get("postal_code")}>输入完成")
    ...
    logger.info(
        f"劲旅订单<{order_id}>，航班<{flight_no}>，东航订单<{pre_order_number}>，采用空中云汇卡<{card_number}>支付完成"
    )
...
async def order_payment_callback(
    ...
    await _select_world_pay_type(payment_po=payment_page, logger=logger, timeout=timeout)
    ...
    desk_web_pay_details_po = CVVPaymentPage(page)
    ...
    pre_order_number: str = await _fill_cvv_payment(
