...
async def web_ticket_checkout_flow(
        *, page: Page, logger: Logger, dep_city: str, arr_city: str, flight_no: str, cabin_class: str,
        ...
        order_cabin_code: str, dep_date: str, protocol: str, main_domain: str, timeout: float = 10.0,
    ...
    await booking_flight_callback(
        ...
        price_sell=price_sell, prodcut_name=prodcut_name, passengers=passengers, email=email, mobile=mobile,
        order_cabin_code=order_cabin_code, dep_date=dep_date, protocol=protocol, domain=main_domain, timeout=timeout,
        price_increase_threshold=price_increase_threshold, price_reduction_threshold=price_reduction_threshold, **kwargs
    )
    pre_order_number: str = await order_payment_callback(
