...
def set_agent(agent: Agent, conversations_dir: Optional[str] = None, max_tool_iterations: int = 10):
    """
    ...
    Args:
        ...
        max_tool_iterations: Maximum number of tool execution rounds per turn (default: 10)
    """
    state.agent = agent
    ...
    state.agent_handler = WebSocketAgentHandler(agent, width=100, non_streaming_models=state.non_streaming_models, show_model_names_ref=show_model_names_ref, streaming_enabled_ref=streaming_enabled_ref, max_tool_iterations=max_tool_iterations)
...
def run_server(agent: Agent, host: str = "127.0.0.1", port: int = 8000, open_browser: bool = True, max_tool_iterations: int = 100):
    """
    ...
    set_agent(agent, max_tool_iterations=max_tool_iterations)
    ...
    # Run server
    print(f"\n🌐 Orchestral App (with conversation persistence)")
