...
class WebSocketAgentHandler:
    ...
    def __init__(self, agent: Agent, width: int = 80, non_streaming_models: Optional[set] = None, show_model_names_ref: Optional[list] = None, 
                 streaming_enabled_ref: Optional[list] = None, cache_enabled_ref: Optional[list] = None, max_tool_iterations: int = 10):
        """
        Initialize handler.
        ...
        Args:
            ...
            max_tool_iterations: Maximum number of tool execution rounds (default: 10)
        """
        self.agent = agent
        self.width = width
    ...
    async def handle_message(self, websocket: WebSocket, user_message: str):
        ...
        await self._handle_tool_loop(websocket, max_iterations=self.max_tool_iterations)
    ...
    async def _handle_tool_loop(self, websocket: WebSocket, max_iterations: int = 10):
        ...
        while iteration < max_iterations:
            if self.should_stop.is_set():
                print(f"[Agent Handler] Tool loop interrupted at iteration {iteration}, should_stop is set", flush=True)
                # Check if there are pending tool calls that won't be executed
                if self.agent.context.messages and isinstance(self.agent.context.messages[-1], Response):
                    last_resp = self.agent.context.messages[-1]
                    if last_resp.message.tool_calls:
                        print(f"[Agent Handler] WARNING: Skipping {len(last_resp.message.tool_calls)} tool call(s) due to interrupt", flush=True)
                break
            ...
            if not isinstance(last_response, Response) or not last_response.message.tool_calls:
                # No more tool calls
                print(f"[Agent Handler] Tool loop exiting: iteration={iteration}, "
                      f"is_Response={isinstance(last_response, Response)}, "
                      f"has_tool_calls={hasattr(last_response, 'message') and bool(last_response.message.tool_calls) if isinstance(last_response, Response) else False}",
                      flush=True)
                if isinstance(last_response, Response) and last_response.message.text:
                    print(f"[Agent Handler] Final LLM response text (first 200 chars): {last_response.message.text[:200]}", flush=True)
                break
        ...
        if iteration >= max_iterations:
            print(f"[Agent Handler] Reached maximum tool iterations ({max_iterations})", flush=True)
            # Check if there are pending tool calls that won't be executed
            if self.agent.context.messages and isinstance(self.agent.context.messages[-1], Response):
                last_resp = self.agent.context.messages[-1]
                if last_resp.message.tool_calls:
                    print(f"[Agent Handler] WARNING: {len(last_resp.message.tool_calls)} tool call(s) remain unexecuted", flush=True)
            await websocket.send_json({
                "type": "warning",
