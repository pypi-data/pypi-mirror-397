...
from orchestral.ui.colors import AGENT_COLOR, CODE_THEME

# LATEX CONFIGURATION: Set to True to enable $...$ and $$...$$ syntax (see LATEX_FORMATTING.md)
ENABLE_DOLLAR_LATEX = False
...
class AgentPanel:
    def __init__(self, response_text=None, tool_uses=None, content_items=None, width=80, subtitle=None):
        """
        ...
        Args:
            ...
            subtitle (str, optional): Subtitle to display below title (e.g., model name)
        """
    ...
    def _has_latex(self, text):
        ...
        if not text:
            return False
        # Check for \[...\] or \(...\) patterns (and optionally $...$, $$...$$ if enabled)
    ...
    def _protect_latex_blocks(self, text):
        ...
        Skips LaTeX inside code blocks (triple backticks).

        Uses short placeholder format ℒNℒ (script L Unicode character) to avoid
        truncation in narrow table columns. Previous format LATEX_BLOCK_N_PLACEHOLDER
        would get truncated to LATEX_BLOCK_N_PLACEHO… breaking restoration.
        ...
        # First, find all code blocks and temporarily replace them with markers
        code_blocks = []
        code_block_pattern = r'```[\s\S]*?```'

        def save_code_block(match):
            marker = f"___CODE_BLOCK_{len(code_blocks)}___"
            code_blocks.append(match.group(0))
            return marker

        # Save code blocks
        text_without_code = re.sub(code_block_pattern, save_code_block, text)

        # Now protect LaTeX in the text without code blocks
        def replacer(match):
            # Use short placeholder to avoid truncation in narrow table columns
            placeholder = f"ℒ{counter[0]}ℒ"
            latex_map[placeholder] = match.group(0)
            counter[0] += 1
            return placeholder

        # Match \[...\] and \(...\) patterns (and optionally $...$, $$...$$ if enabled)
        # Use non-greedy matching to handle multiple equations
        if ENABLE_DOLLAR_LATEX:
            pattern = r'\$\$[^$]+\$\$|\$[^$]+\$|\\\[[\s\S]+?\\\]|\\\([\s\S]+?\\\)'
        else:
            pattern = r'\\\[[\s\S]+?\\\]|\\\([\s\S]+?\\\)'
        protected_text = re.sub(pattern, replacer, text_without_code)

        # Restore code blocks
        for i, code_block in enumerate(code_blocks):
            marker = f"___CODE_BLOCK_{i}___"
            protected_text = protected_text.replace(marker, code_block)
    ...
    def _render_text_as_markdown(self, text_content):
        ...
        # Render as markdown with configured theme for code blocks
        md = Markdown(text_content, code_theme=CODE_THEME)
    ...
    def display(self):
        ...
        if self.legacy_mode:
            ...
            for item in self.content_items:
                if item.get('type') == 'text':
                    ...
                    is_failed = item.get('is_failed', False)
        ...
        # Create the main panel with optional subtitle
        panel = Panel(
            group,
            title="Agent",
            subtitle=self.subtitle,
            title_align="left",
            subtitle_align="right" if self.subtitle else None,
            width=self.width,
