...
from .. import _PKG_NAME, DEFAULT_REC, VERSION, ArrayDouble, MPFloat  # noqa: TID252
...
def hhi_delta_boundary(
    ...
    bdry = np.asarray(np.vstack((half_bdry[::-1], half_bdry[1:, ::-1])), float)
...
def hhi_pre_contrib_boundary(
    ...
    return GuidelinesBoundary(
        np.asarray(np.vstack((half_bdry[::-1], half_bdry[1:, ::-1])), float),
...
def diversion_share_boundary_wtd_avg(
    ...
    *,
    ...
    bdry_array = np.asarray(bdry, float)
...
def diversion_share_boundary_xact_avg(
    ...
    *,
    ...
    return GuidelinesBoundary(
        np.asarray(bdry, float), round(float(bdry_area_simpson), dps)
    )
...
def diversion_share_boundary_min(
    ...
    *,
    ...
    return GuidelinesBoundary(
        np.asarray(list(zip(s_1, s_1[::-1])), float), round(float(bdry_area), dps)
    )
