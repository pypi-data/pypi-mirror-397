...
DEFAULT_DIST_PARMS: ArrayFloat = np.array([0.0, 1.0]).view(ArrayFloat)
DEFAULT_BETA_DIST_PARMS: ArrayFloat = np.array([1.0, 1.0]).view(ArrayFloat)
...
def _dist_parms_conv(_v: ArrayLike | None, _i: MultithreadedRNG) -> ArrayFloat:
    if _v is None or not len(_v) or not _v.any():  # type: ignore
        return {
            ...
            "Dirichlet": np.ones(_i.values.shape[-1]).view(ArrayFloat),
