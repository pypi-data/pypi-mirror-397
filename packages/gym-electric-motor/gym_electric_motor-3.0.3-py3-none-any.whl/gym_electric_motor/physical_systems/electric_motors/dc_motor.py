...
class DcMotor(ElectricMotor):
    ...
    def _update_model(self):
        ...
        self._model_constants = np.array([[-mp["r_a"], 0, -mp["l_e_prime"], 1, 0], [0, -mp["r_e"], 0, 0, 1]])
    ...
    def get_state_space(self, input_currents, input_voltages):
        ...
        e_converter = 1
        low = {
            "omega": -1 if input_voltages.low[a_converter] == -1 or input_voltages.low[e_converter] == -1 else 0,
            "torque": -1 if input_currents.low[a_converter] == -1 or input_currents.low[e_converter] == -1 else 0,
        ...
        high = {"omega": 1, "torque": 1, "i_a": 1, "i_e": 1, "u_a": 1, "u_e": 1}
        return low, high
