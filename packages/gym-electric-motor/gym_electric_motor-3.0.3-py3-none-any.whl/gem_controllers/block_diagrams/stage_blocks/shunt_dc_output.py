...
from control_block_diagram.predefined_components import DcConverter, DcShuntMotor, Limit
...
def shunt_dc_output(emf_feedforward):
    ...
    def _shunt_dc_output(start, control_task):
        ...
        conv_motor = Connection.connect(
            converter.output_bottom, dc_shunt.input_top, text=["", r"$i_{\mathrm{a}}$"], text_align="right", arrow=False
        )

        # Connection between previous connection and current output
        con_i = Connection.connect_to_line(
            conv_motor[0], pwm.position.sub_y(1.5), text=r"$i_{\mathrm{a}}$", arrow=False, fill=False, radius=0.1
        )
        ...
        if emf_feedforward or control_task in ["SC"]:
            ...
            con_omega = Connection.connect(
                dc_shunt.output_left[0].sub_x(2), dc_shunt.output_left[0], text=r"$\omega_{\mathrm{me}}$", arrow=False
            )
