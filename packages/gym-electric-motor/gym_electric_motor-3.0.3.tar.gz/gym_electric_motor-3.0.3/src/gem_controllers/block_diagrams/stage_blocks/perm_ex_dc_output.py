...
from control_block_diagram.predefined_components import DcConverter, DcPermExMotor, Limit
...
def perm_ex_dc_output(emf_feedforward):
    ...
    def _perm_ex_dc_output(start, control_task):
        ...
        conv_motor = Connection.connect(
            converter.output_bottom, dc_perm_ex.input_top, text=["", r"$i$"], text_align="right", arrow=False
        )
        ...
        if emf_feedforward or control_task in ["SC"]:
            ...
            con_omega = Connection.connect(
                dc_perm_ex.output_left[0].sub_x(2),
                dc_perm_ex.output_left[0],
