...
def pmsm_cc(emf_feedforward):
    ...
    def cc_pmsm(start, control_task):
        ...
        Connection.connect(
            multiply_u_sd.output_top,
            add_u_sd.input_bottom,
            text=r"-",
            text_position="end",
            text_align="right",
            move_text=(-0.2, -0.2),
        ...
        Connection.connect(
            add_psi_p.input_left[0].sub_x(0.3),
            add_psi_p.input_left[0],
            text=r"$\Psi_{\mathrm{p}}$",
            text_position="start",
            text_align="left",
            distance_x=0.25,
        )
        ...
        box_ls_1 = Box(
            add_psi_p.position.sub_y(distance),
            size=(0.6, 0.6),
        ...
        box_ls_2 = Box(
            Point.merge(multiply_u_sd.position, box_ls_1.position),
            size=(0.6, 0.6),
            inputs=dict(bottom=1),
            outputs=dict(top=1),
            text=r"$L_{\mathrm{q}}$",
        ...
        box_d_dt = Box(
            Point.merge(alpha_beta_to_dq.position, start.sub_y(6.57020066645696)).sub_x(2),
            size=(1, 0.8),
        ...
        con_omega = Connection.connect(
            box_d_dt.output_left,
            multiply_u_sq.input_left,
            space_y=1,
        ...
        # Inputs of the stage
        inputs = dict(
            i_d_ref=[add_i_sd.input_left[0], dict(text=r"$i^{*}_{\mathrm{sd}}$", distance_y=0.25)],
