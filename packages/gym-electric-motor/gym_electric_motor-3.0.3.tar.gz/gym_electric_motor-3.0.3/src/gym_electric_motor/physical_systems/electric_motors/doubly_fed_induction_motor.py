...
class DoublyFedInductionMotor(InductionMotor):
    ...
    def _update_limits(self, limit_values={}, nominal_values={}):
        ...
        nominal_agenda = {}
        for u, i in zip(
            self.IO_VOLTAGES + self.ROTOR_VOLTAGES,
            self.IO_CURRENTS + self.ROTOR_CURRENTS,
        ):
            limits_agenda[u] = voltage_limit
            nominal_agenda[u] = voltage_nominal
            limits_agenda[i] = self._limits.get("i", None) or self._limits[u] / self._motor_parameter["r_r"]
            nominal_agenda[i] = (
                self._nominal_values.get("i", None) or self._nominal_values[u] / self._motor_parameter["r_r"]
            )
    ...
    def _update_initial_limits(self, nominal_new={}, omega=None):
        ...
        flux_alphabeta_limits = self._flux_limit(
            omega=omega,
            eps_mag=eps_mag,
            u_q_max=self._nominal_values["u_sq"],
            u_rq_max=self._nominal_values["u_rq"],
        )
        flux_nominal_limits = {state: value for state, value in zip(self.FLUXES, flux_alphabeta_limits)}
