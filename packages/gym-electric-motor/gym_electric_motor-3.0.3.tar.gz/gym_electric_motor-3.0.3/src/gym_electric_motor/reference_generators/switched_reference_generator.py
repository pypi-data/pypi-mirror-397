...
class SwitchedReferenceGenerator(ReferenceGenerator, RandomComponent):
    ...
    def set_modules(self, physical_system):
        ...
        ref_space_low = np.min(
            [sub_generator.reference_space.low for sub_generator in self._sub_generators],
            axis=0,
        )
        ref_space_high = np.max(
            [sub_generator.reference_space.high for sub_generator in self._sub_generators],
            axis=0,
        )
