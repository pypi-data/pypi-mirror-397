...
class CascadedFieldOrientedController:
    ...
    def __init__(
        ...
        **controller_kwargs,
        ...
        if self.torque_control:
            ...
            self.torque_controller = TorqueToCurrentConversion(
                environment,
                plot_torque,
                plot_modulation,
                update_interval,
        ...
        # Initialize discrete controller stages
            ...
            if self.omega_control:
                ...
                self.overlaid_controller = [
                    _controllers[stages[3][i]["controller_type"]][1].make(
                        environment, stages[3][i], cascaded=True, **controller_kwargs
                    )
                    for i in range(len(stages[3]))
                ]
                self.overlaid_type = [
                    _controllers[stages[3][i]["controller_type"]][1]
                    == ContinuousController
                    for i in range(len(stages[3]))
                ]
            else:
    ...
    def control(self, state, reference):
            ...
            for i in range(len(self.overlaid_controller) + 1, 1, -1):
                ...
                if (
                    ...
                    self.ref[i] = np.clip(
                        ...
                        / self.limit[self.ref_state_idx[i]]
        ...
        if self.has_cont_action_space:
            ...
            if self.torque_control:
                u_sd = (
                    self.d_controller.control(state[self.i_sd_idx], self.ref[1])
                    + self.u_sd_0
                )
            else:
                u_sd = (
                    self.d_controller.control(
                        state[self.i_sd_idx], reference[self.ref_d_idx]
                    )
                    + self.u_sd_0
                )
            ...
            u_sq = (
                self.q_controller.control(state[self.i_sq_idx], self.ref[0])
                + self.u_sq_0
            )
    ...
    def get_plot_data(self):
        ...
        return dict(
            ref_state=self.ref_state_idx[:-1], ref_value=self.ref[:-1], external=[]
        )
