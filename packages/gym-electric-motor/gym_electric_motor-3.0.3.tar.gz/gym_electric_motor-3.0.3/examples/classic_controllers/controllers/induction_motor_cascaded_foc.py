...
from .induction_motor_torque_to_current_conversion import (
    InductionMotorTorqueToCurrentConversion,
)
...
class InductionMotorCascadedFieldOrientedController:
    ...
    def __init__(
        ...
        **controller_kwargs,
        ...
        if self.torque_control:
            ...
            self.torque_controller = InductionMotorTorqueToCurrentConversion(
                environment, stages
            )
        ...
        labels = [
            {
                "y_label": r"|$\Psi_{r}$|/Vs",
                "state_label": r"|$\hat{\Psi}_{r}$|/Vs",
                "ref_label": r"|$\Psi_{r}$|$^*$/Vs",
            },
            {
        ...
        if self.has_cont_action_space:
            ...
            if self.omega_control:
                self.overlaid_controller = [
                    _controllers[stages[1][i]["controller_type"]][1].make(
                        environment,
                        stages[1][i],
                        _controllers,
                        cascaded=True,
                ...
                self.overlaid_type = [
                    _controllers[stages[1][i]["controller_type"]][1]
                    == ContinuousController
                    for i in range(0, len(stages[1]))
                ]

        self.ref = np.zeros(
            len(self.ref_state_idx)
        )  # Define array for reference values
    ...
    def control(self, state, reference):
        ...
        if self.omega_control:
            for i in range(len(self.overlaid_controller) + 1, 1, -1):
                # Calculate reference
                self.ref[i] = self.overlaid_controller[i - 2].control(
                    state[self.ref_state_idx[i + 1]], self.ref[i + 1]
                )

                # Check limit and integrate
                if (
                    0.85 * self.state_space.low[self.ref_state_idx[i]]
                    <= self.ref[i]
                    <= 0.85 * self.state_space.high[self.ref_state_idx[i]]
                ) and self.overlaid_type[i - 2]:
                    self.overlaid_controller[i - 2].integrate(
                        state[self.ref_state_idx[i + 1]], self.ref[i + 1]
                    )
                else:
                    self.ref[i] = np.clip(
                        self.ref[i],
                        self.nominal_values[self.ref_state_idx[i]]
                        / self.limits[self.ref_state_idx[i]]
                        * self.state_space.low[self.ref_state_idx[i]],
                        self.nominal_values[self.ref_state_idx[i]]
                        / self.limits[self.ref_state_idx[i]]
                        * self.state_space.high[self.ref_state_idx[i]],
                    )

        # Calculate reference values for i_d and i_q
        if self.torque_control:
            torque = self.ref[2] * self.limits[self.torque_idx]
            self.ref[0], self.ref[1], self.psi_opt = self.torque_controller.control(
                state, torque, self.psi_abs
            )
        ...
        if self.has_cont_action_space:
            ...
            u_sd_delta = self.d_controller.control(
                state[self.i_sd_idx], self.ref[1] * self.limits[self.i_sd_idx]
            )
            u_sq_delta = self.q_controller.control(
                state[self.i_sq_idx], self.ref[0] * self.limits[self.i_sq_idx]
            )
        ...
        plot(
            ...
            external_plot=self.external_plot,
    ...
    def get_plot_data(self):
        ...
        return dict(
            ref_state=self.ref_state_idx[:-1],
            ref_value=self.ref[:-1],
            external=[[self.psi_abs, self.psi_opt], [self.psi_angle]],
        )
