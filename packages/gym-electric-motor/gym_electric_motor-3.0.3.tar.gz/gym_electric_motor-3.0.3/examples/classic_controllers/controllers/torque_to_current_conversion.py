...
class TorqueToCurrentConversion:
    ...
    def __init__(
        ...
        self.psi_high = 0.2 * np.sqrt(
            (self.psi_p + self.l_d * self.nominal_values[self.i_sd_idx]) ** 2
        ...
        def mtpc():
            def i_q_(i_d, torque):
                return (
                    torque / (i_d * (self.l_d - self.l_q) + self.psi_p) / (1.5 * self.p)
                )
            ...
            self.max_torque = max(
                1.5
                * self.p
                * (self.psi_p + (self.l_d - self.l_q) * (-self.limit[self.i_sd_idx]))
            ...
            for t in torque:
                if self.psi_p != 0:
                    if self.l_d == self.l_q:
                        ...
                        i_d = np.linspace(
                            -2.5 * self.limit[self.i_sd_idx], 0, self.i_count
                        )
                    i_q = i_q_(i_d, t)
                else:
                    i_q = np.linspace(
                        -2.5 * self.limit[self.i_sq_idx],
                        2.5 * self.limit[self.i_sq_idx],
                        self.i_count,
                    )
                    if self.l_d == self.l_q:
                ...
                psi = np.sqrt(
                    (self.psi_p + self.l_d * i_d_ret) ** 2 + (self.l_q * i_q_ret) ** 2
                )
                ...
                if psi_ == 0:
                    ...
                    if self.psi_p == 0:
                        ...
                        t = (
                            1.5
                            * self.p
                            * (self.psi_p + (self.l_d - self.l_q) * i_d_best)
                            * i_q_best
                        )
                    else:
                        i_d_idx = np.where(
                            psi_**2 - np.power(self.psi_p + self.l_d * i_d, 2) >= 0
                        )
                        i_d_ = i_d[i_d_idx]
                        ...
                        i_idx = np.where(
                            np.sqrt(
                                np.power(i_q / self.nominal_values[self.i_sq_idx], 2)
                                + np.power(i_d_ / self.nominal_values[self.i_sd_idx], 2)
                            )
                            <= 1
                        )
                        i_d_ = i_d_[i_idx]
                        ...
                        torque = (
                            1.5
                            * self.p
                            * (self.psi_p + (self.l_d - self.l_q) * i_d_)
                            * i_q
                        )
        ...
        self.psi_t = np.sqrt(
            np.power(self.psi_p + self.l_d * self.mtpc[:, 1], 2)
            + np.power(self.l_q * self.mtpc[:, 2], 2)
        )
        ...
        self.i_d_max = -np.sqrt(
            self.nominal_values[self.i_sq_idx] ** 2 - np.power(self.i_q_max, 2)
        )
        ...
        if self.l_d != self.l_q:
            idx = np.where(
                np.sign(self.psi_p + i_d * self.l_d)
                * np.power(self.psi_p + i_d * self.l_d, 2)
        ...
        psi = np.sqrt(
            np.power(self.l_d * i_d + self.psi_p, 2) + np.power(self.l_q * i_q, 2)
        )
        ...
        if torque_control == "analytical":
            ...
            self.i_d_inter = griddata(
                (t, psi), i_d, (self.t_grid, self.psi_grid), method="linear"
            )
    ...
    def intitialize_torque_plot(self):
        if self.plot_torque:
            ...
            mtpc_i_idx = np.where(
                np.sqrt(np.power(self.mtpc[:, 1], 2) + np.power(self.mtpc[:, 2], 2))
                <= self.nominal_values[self.i_sd_idx]
            )
            ...
            self.i_d_q_characteristic_.plot(
                self.mtpf[:, 2], self.mtpf[:, 3], label=r"MTPF", c="tab:green"
            )
            self.i_d_q_characteristic_.plot(
                self.i_d_max, self.i_q_max, label=r"$i_\mathrm{max}$", c="tab:red"
            )
            self.i_d_q_characteristic_.plot(
                [], [], label=r"$i_\mathrm{d,q}$", c="tab:blue"
            )
            ...
            self.torque_plot.plot(
                self.mtpf[:, 0], torque, label=r"$T_\mathrm{max}(\Psi)$", c="tab:orange"
            )
            ...
            if self.torque_control in ["interpolate", "analytical"]:
                self.i_q_plot.plot_surface(
                    self.t_grid,
                    self.psi_grid,
                    self.i_q_inter_plot,
                    cmap=cm.jet,
                    linewidth=0,
                    vmin=np.nanmin(self.i_q_inter_plot),
                ...
                self.i_d_plot.plot_surface(
                    self.t_grid,
                    self.psi_grid,
                    self.i_d_inter_plot,
                    cmap=cm.jet,
                    linewidth=0,
                    vmin=np.nanmin(self.i_d_inter_plot),
    ...
    def solve_analytical(self, torque, psi):
        ...
        i_q = (
            2 * torque / (3 * self.p * (self.psi_p + (self.l_d - self.l_q) * i_d))
        )  # Calculate the corresponding i_q
        return i_d, i_q
    ...
    def get_t_idx(self, torque):
        ...
        return int(
            round(
                (torque - self.t_min) / (self.t_max - self.t_min) * (self.t_count - 1)
            )
        )
    ...
    def get_psi_idx_mtpf(self, psi):
        return np.clip(
            int(
                (self.psi_count - 1)
                - round(psi / self.psi_max_mtpf * (self.psi_count - 1))
            ),
            0,
            self.psi_count,
        )
    ...
    def control(self, state, torque):
        ...
        if self.plot_torque:
            ...
            if self.k % self.update_interval == 0:
                self.psi_plot.scatter(
                    self.torque_list, self.psi_list, c="tab:blue", s=3
                )
                self.torque_plot.scatter(
                    self.psi_list, self.torque_list, c="tab:blue", s=3
                )
                self.i_d_q_characteristic_.scatter(
                    self.i_d_list, self.i_q_list, c="tab:blue", s=3
                )
