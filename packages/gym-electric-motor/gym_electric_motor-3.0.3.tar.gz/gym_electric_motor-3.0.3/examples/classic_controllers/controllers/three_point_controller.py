...
class ThreePointController(DiscreteController):
    ...
    def __init__(
        ...
        environment,
        action_space,
        switch_to_positive_level=0.02,
        switch_to_negative_level=0.02,
        switch_to_neutral_from_positive=0.01,
        switch_to_neutral_from_negative=0.01,
        param_dict={},
        cascaded=False,
        control_e=False,
        **controller_kwargs,
        ...
        self.neutral_from_pos = param_dict.get(
            "switch_to_neutral_from_positive", switch_to_neutral_from_positive
        )
        self.neutral_from_neg = param_dict.get(
            "switch_to_neutral_from_negative", switch_to_neutral_from_negative
        )
    ...
    def control(self, state, reference):
        if reference - state > self.pos or (
            (self.neutral_from_pos < reference - state) and self.recent_action == 1
        ):
