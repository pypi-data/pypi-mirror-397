...
class ExternalPlot(TimePlot):
    ...
    def reset_data(self):
        ...
        if self.added:
            for i in range(self.add_lines):
                self._additional_data[i] = np.full(
                    shape=self._x_data.shape, fill_value=np.nan
                )
    ...
    def initialize(self, axis):
        ...
        if self.added:
            for i in range(self.add_lines):
                (self._additional_lines[i],) = self._axis.plot(
                    self._x_data,
                    self._additional_data[i],
                    **self._add_line_config,
                    zorder=self.add_lines - i,
                )
        ...
        if self._referenced:
            if self.added:
                ...
                self._axis.legend(
                    ([self._state_line, self._reference_line]),
                    ([self.state_label, self.ref_label]),
                    loc="upper left",
                    numpoints=20,
                )
        else:
            self._axis.legend(
                (self._state_line,), (self.state_label,), loc="upper left", numpoints=20
            )
