...
pmsm_motor_parameter = {
    ...
    "reward_weights": dict(
        omega=1, torque=0, i_a=0, i_b=0, i_c=0, u_a=0, u_b=0, u_c=0, epsilon=0, u_sup=0
    ),
}
pmsm_state_positions = {
    "omega": 0,
    "torque": 1,
    "i_a": 2,
    "i_b": 3,
...
test_motor_parameter = {
    "DcSeries": series_motor_parameter,
...
test_motor_initializer = {
    "DcSeries": series_initializer,
...
def jacobian(t, state, u):
    ...
    y = state[1]
    result = np.array(
        [[3 - 2 * y + 6 * x, 5 - 2 * x - y], [-0.6 - 6 * x * y, 1.8 * y - 3 * x**2]]
    )
    return result
