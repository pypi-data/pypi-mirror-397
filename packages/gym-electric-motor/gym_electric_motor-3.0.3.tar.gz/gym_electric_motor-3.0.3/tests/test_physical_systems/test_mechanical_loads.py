...
def concretePolynomialLoad():
    ...
    return PolynomialStaticLoad(
        load_parameter=test_load_params, load_initializer=test_initializer
    )
...
def test_MechanicalLoad_reset(concreteMechanicalLoad):
    ...
    resetVal = concreteMechanicalLoad.reset(
        a=7,
        b=9,
...
def test_MechanicalLoad_get_state_space(concreteMechanicalLoad):
    ...
    retVal2 = concreteMechanicalLoad.get_state_space(
        test_omega_range_2
...
@pytest.mark.parametrize(
    "omega, expected_result", [(-3, 23400), (0, 20000), (5, 11400)]
)  # to verify all 3 branches
def test_PolynomialStaticLoad_MechanicalOde(
    concretePolynomialLoad, omega, expected_result
):
    """
...
class TestConstSpeedLoad(TestMechanicalLoad):
    ...
    def test_reset(self, const_speed_load):
        ...
        # gymnasium.Box state space with random size
        test_space = Box(low=-1.0, high=1.0, shape=(3,))
        reset_val = const_speed_load.reset(
            state_positions=test_positions,
            nominal_state=test_nominal,
            state_space=test_space,
        )
    ...
    @pytest.mark.parametrize(
        "omega, omega_fixed, expected",
        [(-0.5, 1000, (0, 0)), (0, 0, (0, 0)), (2, -523, (0, 0)), (2, 0.2, (0, 0))],
    )
...
class TestExtSpeedLoad(TestMechanicalLoad):
    ...
    def test_initialization(self):
        ...
        assert load.omega == speed_profile_(
            t=0, amp=test_amp, bias=test_bias, freq=test_freq
        )
    ...
    # to verify all 3 branches
    @pytest.mark.parametrize(
        "omega, expected_result", [(-3, -69840.0), (0, -99840.0), (5, -149840.0)]
    )
    ...
    def test_reset(self, ext_speed_load):
        ...
        # gymnasium.Box state space with random size
        test_space = Box(low=-1.0, high=1.0, shape=(3,))
        reset_var = ext_speed_load.reset(
            state_positions=test_positions,
            nominal_state=test_nominal,
            state_space=test_space,
        )
...
class TestPolyStaticLoad(TestMechanicalLoad):
    ...
    @pytest.mark.parametrize(
        "omega, load_parameter, expected",
        [
            (
                -0.5,
                dict(a=12, b=1, c=0, j_load=1),
                (np.array([[-1.0]]), np.array([[1.0]])),
            ),
            (0, dict(j_load=0.5), (np.array([[-1000.0]]), np.array([[2.0]]))),
