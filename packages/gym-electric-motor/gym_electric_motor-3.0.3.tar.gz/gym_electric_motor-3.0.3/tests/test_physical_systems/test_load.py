from gym_electric_motor.physical_systems.mechanical_loads import (
...
def test_polynomial_load():
    ...
    assert load_init.load_parameter == load_parameter["parameter"], (
        "Wrong Parameter "
        + str(load_init.load_parameter)
    ...
    for load in loads:
        ...
        for omega in [-10, 0, 10]:
            ...
            # test load ode
            for torque in [-3, 0, 5]:
                assert load.mechanical_ode(0, mechanical_state, torque) == np.array(
                    [(torque - electrical_torque) / j_total]
                )
