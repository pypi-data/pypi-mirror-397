...
from gym_electric_motor.physical_systems import (
    ...
    converters as cv,
    electric_motors as em,
    mechanical_loads as ml,
    voltage_supplies as vs,
    solvers as sv,
)
...
class TestSCMLSystem:
    ...
    def mock_build_state(self, motor_state, torque, u_in, u_sup):
        ...
        return np.concatenate(
            (
                self.motor_state[: len(DummyLoad.state_names)],
                [torque],
                self.motor_state[len(DummyLoad.state_names) :],
                [u_sup],
            )
    ...
    def scml_system(self, monkeypatch):
        ...
        monkeypatch.setattr(
            self.class_to_test,
            "_build_state_names",
            lambda _: DummyLoad.state_names
            + ["torque"]
            + DummyElectricMotor.CURRENTS
            + DummyElectricMotor.VOLTAGES
            + ["u_sup"],
        )
        monkeypatch.setattr(
            self.class_to_test,
    ...
    def test_reset(self, scml_system):
        ...
        assert (
            scml_system._k == 0
        ), "Episode step of the system was not set to zero after reset"
        assert (
            scml_system.converter.reset_counter
            == scml_system.electrical_motor.reset_counter
            == scml_system.mechanical_load.reset_counter
            == scml_system.supply.reset_counter
        ...
        assert all(
            scml_system._ode_solver.y
            == np.zeros_like(
                scml_system.mechanical_load.state_names
                + scml_system.electrical_motor.CURRENTS,
                dtype=float,
            )
    ...
    def test_system_equation(self, scml_system):
        ...
        assert all(
            derivative
            == np.array([torque, -torque, currents[0] - u_in[0], currents[1] - u_in[1]])
        ), "The system equation return differs from the expected"
        assert (
            scml_system.mechanical_load.t == t
        ), "The time t was not passed through to the mech. load equation"
        assert np.all(
            scml_system.mechanical_load.mechanical_state == state[:2]
    ...
    def test_simulate(self, scml_system):
        ...
        assert all(
            desired_next_state == next_state
        ), "The calculated next state differs from the expected one"
        assert (
            scml_system.converter.action == action
        ), "The action was not passed correctly to the converter"
        assert (
            scml_system.converter.action_set_time == 0
        ), "The action start time was passed incorrect to the converter"
        assert scml_system.converter.last_i_out == scml_system.electrical_motor.i_in(
            scml_system._ode_solver.last_y[2:]
        )
    ...
    def test_system_jacobian(self, scml_system):
        ...
        scml_system.electrical_motor.electrical_jac_return = (
            el_jac,
            el_over_omega,
            torque_over_el,
        )
        ...
        assert np.all(
            sys_jac[2:, 0] == el_over_omega
        ), "the derivative of the el.state over omega is false"
