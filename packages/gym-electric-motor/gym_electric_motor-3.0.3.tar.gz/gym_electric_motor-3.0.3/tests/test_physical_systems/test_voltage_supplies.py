...
class TestRCVoltageSupply(TestVoltageSupply):
    ...
    def test_initialization(
        self, u_nominal=450.0, supply_parameter={"R": 3, "C": 6e-2}
    ):
    ...
    def test_system_equation(self):
        ...
        # time invariance
        assert system_equation(0, [30], 50, 3, 3, 3) == system_equation(
            5, [30], 50, 3, 3, 3
        )
...
class TestAC3PhaseSupply(TestVoltageSupply):
    ...
    def test_default_initialization(self):
        ...
        assert voltage_supply.supply_range == [
            -400.0 / np.sqrt(3) * np.sqrt(2),
            400.0 / np.sqrt(3) * np.sqrt(2),
        ]
        assert voltage_supply._fixed_phi == False
        assert voltage_supply._f == 50
    ...
    def test_reset(self, u_nominal=400.0):
        ...
        _ = voltage_supply.reset()
        assert (
            voltage_supply._phi != first_phi
        ), "Test this again and if this error doesn't appear next time you should consider playing lotto"
    ...
    def test_get_voltage(self):
        ...
        for time in times:
            assert np.allclose(
                supply.get_voltage(time)[0], 400.0 / np.sqrt(3) * np.sqrt(2)
            )
        ...
        supply = vs.AC3PhaseSupply(supply_parameter=supply_parameter)
        assert np.allclose(
            supply.get_voltage(1 / (2 * np.pi)), [89.536111, 227.238311, -316.774422]
        )
        assert np.allclose(
            supply.get_voltage(2 / (2 * np.pi)), [-138.223662, -187.151049, 325.374712]
        )
