...
class TestLimitConstraint:
    @pytest.mark.parametrize(
        ["ps", "observed_state_names", "expected_state_names"],
        [
            [
    ...
    def test_initialization(self, ps, observed_state_names, expected_state_names):
        ...
        assert np.all(
            lc._observed_states
            == np.array([state in expected_state_names for state in ps.state_names])
        )

    @pytest.mark.parametrize(
        ["ps", "observed_state_names", "state", "expected_violation"],
        [
