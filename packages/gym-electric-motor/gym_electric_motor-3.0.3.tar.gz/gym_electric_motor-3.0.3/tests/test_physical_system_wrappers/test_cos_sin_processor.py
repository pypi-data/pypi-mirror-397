...
class TestCosSinProcessor(TestPhysicalSystemWrapper):
    ...
    def processor(self, physical_system):
        return gem.physical_system_wrappers.CosSinProcessor(
            angle="dummy_state_0", physical_system=physical_system
        )

    def test_limits(self, processor, physical_system):
        assert all(
            processor.limits == np.concatenate((physical_system.limits, [1.0, 1.0]))
        )

    def test_nominal_state(self, processor, physical_system):
        assert all(
            processor.nominal_state
            == np.concatenate((physical_system.nominal_state, [1.0, 1.0]))
        )
    ...
    def test_reset(self, processor, physical_system):
        assert all(
            processor.reset() == np.concatenate((physical_system.state, [1.0, 0.0]))
        )
