...
from gym_electric_motor.reference_generators.sawtooth_reference_generator import (
    SawtoothReferenceGenerator,
)
from gym_electric_motor.reference_generators.sinusoidal_reference_generator import (
    SinusoidalReferenceGenerator,
)
from gym_electric_motor.reference_generators.step_reference_generator import (
    StepReferenceGenerator,
)
from gym_electric_motor.reference_generators.subepisoded_reference_generator import (
    SubepisodedReferenceGenerator,
)
from gym_electric_motor.reference_generators.switched_reference_generator import (
    SwitchedReferenceGenerator,
)
from gym_electric_motor.reference_generators.triangle_reference_generator import (
    TriangularReferenceGenerator,
)
from gym_electric_motor.reference_generators.wiener_process_reference_generator import (
    WienerProcessReferenceGenerator,
)
from gym_electric_motor.reference_generators.zero_reference_generator import (
    ZeroReferenceGenerator,
)
...
class TestSwitchedReferenceGenerator:
    ...
    def monkey_instantiate(self, superclass, instance, **kwargs):
        ...
        assert (
            instance == self._sub_generator[self._monkey_instantiate_counter]
    ...
    def test_reset(self, monkeypatch, setup, initial_state, initial_reference):
        ...
        assert (
            sum(sum(abs(res_2 - self._trajectory))) < 1e-6
    ...
    @pytest.mark.parametrize(
        "k, current_episode_length, k_new, reset_counter",
        [(10, 15, 11, 0), (10, 10, 11, 1)],
    )
    def test_get_reference_observation(self, monkeypatch, k, current_episode_length, k_new, reset_counter):
        ...
        monkeypatch.setattr(
            test_object._current_ref_generator,
    ...
    def test_reset_reference(self, monkeypatch):
        ...
        test_object = SwitchedReferenceGenerator(
            sub_generators=sub_reference_generators,
        ...
        assert (
            test_object._super_episode_length[0]
...
class TestWienerProcessReferenceGenerator:
    ...
    def test_reset_reference(self, monkeypatch):
        ...
        monkeypatch.setattr(
            SubepisodedReferenceGenerator,
            "_get_current_value",
            self.monkey_get_current_value,
        )
        test_object = WienerProcessReferenceGenerator(
...
class TestFurtherReferenceGenerator:
    ...
    @pytest.mark.parametrize(
        "amplitude_range, expected_amplitude",
        [((0.1, 0.8), (0.1, 0.45)), ((-0.5, 0.35), (0.0, 0.35))],
    )
    @pytest.mark.parametrize(
        "offset_range, expected_offset",
        [((-0.8, 0.5), (0.0, 0.5)), ((0.1, 0.96), (0.1, 0.9))],
    )
    ...
    @pytest.mark.parametrize(
        "reference_class, expected_reference, frequency_range",
        [
            (
                SawtoothReferenceGenerator,
                np.array([-0.2, -0.1, 0.0, 0.1, 0.2, 0.3, -0.4, -0.3, -0.2, -0.1]),
                1 / 8,
            ),
            (
                SinusoidalReferenceGenerator,
...
class TestSubepisodedReferenceGenerator:
    ...
    def test_init(self, monkeypatch, limit_margin):
        ...
        test_object = SubepisodedReferenceGenerator(
            ...
            episode_lengths=self._episode_length,
            limit_margin=limit_margin,
        )
        # verify the expected results
    ...
    @pytest.mark.parametrize(
        "limit_margin, expected_low, expected_high",
    ...
    @pytest.mark.parametrize(
        ...
        [(np.array([0.2, 0.4, 0.7]), 0.4), (None, 0.0)],
    )
    ...
    @pytest.mark.parametrize(
        "k, expected_reference, expected_parameter",
        [(8, 0.6, (10, 0)), (10, 0.4, (35, 1))],
    )
    # setup test scenario
    def test_get_reference_observation(self, monkeypatch, k, expected_reference, expected_parameter):
        ...
        assert (
            self._monkey_reset_reference_counter == expected_parameter[1]
