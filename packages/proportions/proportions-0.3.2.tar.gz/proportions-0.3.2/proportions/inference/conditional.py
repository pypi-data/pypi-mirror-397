...
def conditional_inference_k_out_of_k(
    ...
    """There are two tasks A and B. In many cases A = B but they dont need to be so. .
    Goal is to estimate posterior distribution of E[θ_B | k/k successes on task A].
    where  θ_A, θ_B are the success rates in both tasks

    This addresses the winner's curse / selection bias problem: scenarios
    that show k successes out of k trials may be truly good OR just lucky.
    ...
    **IMPORTANT:** This function expects SUCCESS RATE posteriors. If your data
    ...
    - Weight for scenario i: w_i = θ_A_i^k (prob of k/k successes)
    - Posterior mean: E[θ_B | filter] = Σ w_i θ_B_i / Σ w_i
    ...
    Example:
        ...
        >>>
        >>> # Self-conditional: success rate given k/k successes on same task
    ...
    for j in range(n_samples):
        # Weights: w_i = θ_A_i^k (probability of k/k successes)
        weights = theta_a_samples[j, :] ** k

        # Weighted average: μ = Σ w_i θ_B_i / Σ w_i
        mu_samples[j] = np.sum(weights * theta_b_samples[j, :]) / np.sum(weights)
