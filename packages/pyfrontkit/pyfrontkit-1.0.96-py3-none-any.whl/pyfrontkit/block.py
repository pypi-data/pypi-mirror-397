...
class Block:
    ...
    def _get_main_selector(self):
        ...
        # 1. Prioridad: ID Propio (o el auto-generado)
        if self.attrs.get("id"):
            return f"#{self.attrs['id']}", "id"

        # 2. Prioridad: Clase (Fallback de seguridad)
        elif self.attrs.get("class"):
            return f".{self.attrs['class']}", "class"
        
        # 3. Prioridad: Etiqueta (Fallback de seguridad)
        else:
            return self.tag, "tag"
        

    def align(self, orientation=None, gap=None, padding=None,pad_left=None,pad_right=None, grid_column=None, fsize=None):
        """
        Controls the layout of this block and optionally sets font-size.
        """
        ...
        pad_left_text = f"padding_left: {pad_left};" if pad_left else ""
        pad_right_text=f"padding_right: {pad_right};" if pad_right else ""
        font_size_text = f"font-size: {fsize};" if fsize else ""
        
        # Build final CSS text
...
# Assuming this method belongs to a class like BlockStyle or similar
    ...
    def position(self, mode="absolute", top=None, left=None, right=None, bottom=None, z_index=None):
        selector, _ = self._get_main_selector()

        css_lines = []
        css_lines.append(f"position: {mode};")
