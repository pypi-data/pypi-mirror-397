...
class Liner:
    ...
    def getLinerResizedByThickness(self, thickness):
        """return a liner that has a resized geometry by given thickness. Positive values increase the liner"""
    ...
    def getContour(self, nodeNumber=250):
        ...
        pointsLiner = np.array(
            [
                np.linspace(0.0, self.lcyl / linerLenFac, nodeNumber, False) + self.dome.domeLength,
                [self.rCyl] * nodeNumber,
            ]
        )[:, 1:]
