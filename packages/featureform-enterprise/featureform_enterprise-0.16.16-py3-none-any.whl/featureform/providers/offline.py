...
class OfflineSparkProvider(OfflineProvider):
    ...
    def streaming_sql_transformation(
        ...
        **Examples**:
        ``` py
        @spark.streaming_sql_transformation(variant="quickstart")
        def average_user_transaction():
            return "SELECT CustomerID as user_id, avg(TransactionAmount) as avg_transaction_amt from {{transactions.v1}} GROUP BY user_id"
        ```
        ...
        Args:
            ...
            spark_params (Dict[str, str]): Additional Spark parameters
            write_options (Dict[str, str]): Write options for the output
            table_properties (Dict[str, str]): Table properties for the output
