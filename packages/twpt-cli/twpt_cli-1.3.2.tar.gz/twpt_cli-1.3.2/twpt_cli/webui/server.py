...
# ===========================================
# Phase 1.3: Concurrent Stream Limiting (Enhanced)
# ===========================================
from concurrent.futures import ThreadPoolExecutor
import atexit

MAX_CONCURRENT_STREAMS = 20
STREAM_MAX_TTL = 1800.0  # 30 minutes max lifetime for any stream
STREAM_REAPER_INTERVAL = 60.0  # Check for zombie streams every 60 seconds

_stream_semaphore = threading.Semaphore(MAX_CONCURRENT_STREAMS)
_active_streams = 0
_streams_lock = threading.Lock()

# Thread pool for stream handlers (bounded to prevent unlimited thread creation)
_stream_thread_pool: Optional[ThreadPoolExecutor] = None

# Stream registry for tracking and cleanup
_stream_registry: Dict[str, Dict[str, Any]] = {}
_stream_reaper_thread: Optional[threading.Thread] = None
_stream_reaper_stop = threading.Event()


def _get_stream_thread_pool() -> ThreadPoolExecutor:
    """Get or create the stream thread pool."""
    global _stream_thread_pool
    if _stream_thread_pool is None:
        # Use bounded pool - max 2x concurrent streams to allow some headroom
        _stream_thread_pool = ThreadPoolExecutor(
            max_workers=MAX_CONCURRENT_STREAMS * 2,
            thread_name_prefix="webui_stream_"
        )
    return _stream_thread_pool


def _register_stream(stream_id: str, stop_event: threading.Event, loop_ref: list) -> None:
    """Register an active stream for tracking and cleanup.

    Args:
        stream_id: Unique identifier for this stream
        stop_event: Threading event to signal stream termination
        loop_ref: Reference to the event loop (list containing loop or None)
    """
    with _streams_lock:
...
def _force_stop_stream(stream_id: str, reason: str = "TTL exceeded") -> bool:
    """Force-stop a stream by setting its stop event and stopping its loop.
    ...
    Returns:
        True if stream was found and stop was signaled
    """
    with _streams_lock:
        stream_info = _stream_registry.get(stream_id)
        if not stream_info:
            return False

        stop_event = stream_info.get('stop_event')
        loop_ref = stream_info.get('loop_ref', [None])

    logging.warning(f"Force-stopping stream {stream_id}: {reason}")

    # Signal the stop event
    if stop_event:
        stop_event.set()

    # Force-stop the event loop if it exists
    if loop_ref and loop_ref[0]:
        try:
            loop = loop_ref[0]
            if loop.is_running():
                loop.call_soon_threadsafe(loop.stop)
        except Exception as e:
            logging.debug(f"Error stopping loop for stream {stream_id}: {e}")

    return True


def _reap_zombie_streams() -> int:
    """Reap streams that have exceeded their TTL.
    ...
    with _streams_lock:
        for stream_id, info in _stream_registry.items():
            age = now - info.get('start_time', now)
            if age > STREAM_MAX_TTL:
                to_reap.append((stream_id, age))

    reaped = 0
    for stream_id, age in to_reap:
        if _force_stop_stream(stream_id, f"TTL exceeded ({age:.0f}s > {STREAM_MAX_TTL}s)"):
            reaped += 1

    return reaped


def _stream_reaper_loop():
    """Background thread that periodically reaps zombie streams."""
    logging.info("Stream reaper started")
    while not _stream_reaper_stop.wait(STREAM_REAPER_INTERVAL):
        try:
            reaped = _reap_zombie_streams()
            if reaped > 0:
                logging.info(f"Stream reaper cleaned up {reaped} zombie stream(s)")

            # Also log current stream count for monitoring
            stats = _get_stream_stats()
            if stats['active_streams'] > MAX_CONCURRENT_STREAMS * 0.8:
                logging.warning(
                    f"High stream usage: {stats['active_streams']}/{stats['max_streams']} "
                    f"({len(_stream_registry)} registered)"
                )
        except Exception as e:
            logging.error(f"Stream reaper error: {e}")

    logging.info("Stream reaper stopped")


def _start_stream_reaper():
    """Start the background stream reaper thread."""
    global _stream_reaper_thread
    if _stream_reaper_thread is None or not _stream_reaper_thread.is_alive():
        _stream_reaper_stop.clear()
        _stream_reaper_thread = threading.Thread(
            target=_stream_reaper_loop,
            daemon=True,
            name="webui_stream_reaper"
        )
        _stream_reaper_thread.start()


def _stop_stream_reaper():
    """Stop the stream reaper thread."""
    global _stream_reaper_thread
    _stream_reaper_stop.set()
    if _stream_reaper_thread:
        _stream_reaper_thread.join(timeout=5)
        _stream_reaper_thread = None


def _shutdown_stream_pool():
    """Shutdown the stream thread pool and reaper."""
    global _stream_thread_pool
    ...
    with _streams_lock:
        stream_ids = list(_stream_registry.keys())

    for stream_id in stream_ids:
        _force_stop_stream(stream_id, "Server shutdown")

    # Shutdown thread pool
    if _stream_thread_pool:
        _stream_thread_pool.shutdown(wait=False, cancel_futures=True)
        _stream_thread_pool = None

    logging.info("Stream pool shutdown complete")


# Register cleanup on exit
atexit.register(_shutdown_stream_pool)
...
def _get_stream_stats() -> Dict[str, int]:
    ...
    with _streams_lock:
        return {
            ...
            'registered_streams': len(_stream_registry),
        }
    ...
    def api_stream_pentest():
        ...
        try:
            ...
            def generate_events() -> Generator[str, None, None]:
                ...
                # Generate unique stream ID and register for TTL tracking
                stream_id = f"schedule_{uuid.uuid4().hex[:8]}"
                _register_stream(stream_id, stop_event, loop_ref)
                ...
                def run_stream():
                    ...
                    try:
                        ...
                        loop_ref[0] = None  # Clear reference

                # Start streaming in background thread (using pool)
                try:
                    future = _get_stream_thread_pool().submit(run_stream)
                except RuntimeError:
                    # Pool is shutdown, fall back to daemon thread
                ...
                try:
                    ...
                    stop_event.set()
                    # Force-stop the event loop if still running
                    if loop_ref[0] and loop_ref[0].is_running():
                        try:
                            loop_ref[0].call_soon_threadsafe(loop_ref[0].stop)
                        except Exception:
                            pass
                    # Wait briefly for thread/future to finish
                    if future:
                        try:
                            future.result(timeout=STREAM_THREAD_TIMEOUT)
                        except Exception:
                            pass
                    # Unregister stream and release slot
    ...
    def api_watch_pentest(pentest_id: str):
        ...
        try:
            ...
            def generate_events() -> Generator[str, None, None]:
                ...
                # Generate unique stream ID and register for TTL tracking
                stream_id = f"watch_{pentest_id}_{uuid.uuid4().hex[:8]}"
                _register_stream(stream_id, stop_event, loop_ref)
    ...
    def api_attach_pentest(pentest_id: str):
        ...
        try:
            ...
            def generate_events() -> Generator[str, None, None]:
                ...
                # Generate unique stream ID and register for TTL tracking
                stream_id = f"attach_{pentest_id}_{uuid.uuid4().hex[:8]}"
                _register_stream(stream_id, stop_event, loop_ref)
    ...
    def api_submit_custom_task():
        ...
        try:
            ...
            def generate_events() -> Generator[str, None, None]:
                ...
                # Generate unique stream ID and register for TTL tracking
                stream_id = f"task_submit_{uuid.uuid4().hex[:8]}"
                _register_stream(stream_id, stop_event, loop_ref)
    ...
    def api_attach_custom_task(task_id: str):
        ...
        try:
            ...
            def generate_events() -> Generator[str, None, None]:
                ...
                # Generate unique stream ID and register for TTL tracking
                stream_id = f"task_attach_{task_id}_{uuid.uuid4().hex[:8]}"
                _register_stream(stream_id, stop_event, loop_ref)
...
def run_server(
    ...
    # Start stream reaper for TTL-based cleanup of zombie streams
    _start_stream_reaper()
