...
class LoggingCapture(BufferingHandler):
    ...
    .. __: behave_bo.html#command-line-arguments
    """
    ...
    def getvalue(self):
        logs_to_exclude = self.config.log_capture_exclude_template.split(',')
        return '\n'.join(self.formatter.format(r) for r in self.buffer if r.name not in logs_to_exclude)
...
def capture(*args, **kw):
    ...
    It configures the logging capture using the *behave_bo* context - the first
