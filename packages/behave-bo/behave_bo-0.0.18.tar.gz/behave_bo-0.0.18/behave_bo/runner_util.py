...
from behave_bo.textutil import (
    ensure_stream_with_encoder,
)


# LAZY: from behave_bo.step_registry import setup_step_decorators
...
class FeatureScenarioLocationCollector:
    ...
    USE CASE:
        behave_bo feature/foo.feature:10
        behave_bo @selected_features.txt
        behave_bo @rerun_failed_scenarios.txt
...
def collect_feature_locations(paths, strict=True):
    ...
    for path in paths:
        if os.path.isdir(path):
            ...
            # -- USE: behave_bo @list_of_features.txt
...
def timer(func):
    # TODO временная функция-секундомер, удалить после
    @wraps(func)
    def wrapper(*args, **kwargs):
        ts = time.time()
        result = func(*args, **kwargs)
        te = time.time()
        print('{!r}  {:2.2f} ms'.format(func.__name__, (te - ts) * 1000))
        return result
    return wrapper
...
def print_undefined_step_snippets(undefined_steps, stream=None, colored=True):
    ...
    if colored:
        ...
        from behave_bo.formatter.ansi_escapes import (
            escapes,
        )
