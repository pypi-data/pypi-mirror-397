...
"""
This module provides the model element class that represent a behave_bo model:
...
class Feature(TagAndStatusStatement, Replayable):
    ...
    .. attribute:: background
       ...
       The :class:`~behave_bo.model.Background` for this feature, if any.

    .. attribute:: scenarios

       A list of :class:`~behave_bo.model.Scenario` making up this feature.

    .. attribute:: tags

       A list of @tags (as :class:`~behave_bo.model.Tag` which are basically
    ...
    def __init__(self, filename, line, keyword, name, tags=None,
                 ...
                 language=None):
        tags = tags or []
        super().__init__(filename, line, keyword, name, tags)
        self.description = description or []
        self.scenarios = []
        self.steps_definitions_dirs = defaultdict(set)
    ...
    def skip(self, reason=None, require_not_executed=False):
        ...
        """
        if reason:
            logger = logging.getLogger("behave_bo")
    ...
    def run(self, runner):
        """Вместо итерации по списку сценариев через for используется оператор while.
        В случае запуска в параллельном режиме, у runner будет доступен атрибут parallel_features_scenarios
        являющийся DictProxy содержащий ключи hash(feature) всех выполняемых feature-файлов и значения ListProxy
        со списком идентификаторов сценариев которые должны быть выполнены.
        Во время выполнения feature-файла каждый процесс будет забирать идентификатор сценария из ListProxy
        и выполнять соответствующий сценарий, пока список идентификаторов не будет пустым.

        В случае запуска в обычном режиме у runner не будет атрибута parallel_features_scenarios, и в качестве списка
        будет использоваться список сценариев этого feature.
        """
        ...
        if not skip_feature_untested:
            if hasattr(runner, 'parallel_features_scenarios'):
                scenarios = runner.parallel_features_scenarios[hash(self)]
                tests_logger.info(f'({os.getpid()}) - {self.location.basename()} | left_scenarios {len(scenarios)}')
            else:
            ...
            while scenarios:
                if hasattr(runner, 'parallel_features_scenarios'):
                    try:
                        scenario = self.scenarios_map[scenarios.pop()]
                    except IndexError as e:
                        tests_logger.info(f'({os.getpid()}) - {self.location.basename()} | {e}')
                        break
                else:
...
class Background(BasicStatement, Replayable):
    ...
    .. attribute:: steps
       ...
       A list of :class:`~behave_bo.model.Step` making up this background.
    ...
    def __init__(self, filename, line, keyword, name, steps=None):
        super().__init__(filename, line, keyword, name)
        self.steps = steps or []
...
class Scenario(TagAndStatusStatement, Replayable):
    ...
    .. attribute:: feature
       ...
       The :class:`~behave_bo.model.Feature` this scenario belongs to.

    .. attribute:: steps

       A list of :class:`~behave_bo.model.Step` making up this scenario.
    ...
    @property
    def duration_pt(self):
        # -- ORIG: for step in self.steps:  Background steps were excluded.
        scenario_duration = 0
        for step in self.all_steps:
            scenario_duration += step.duration_pt
        return scenario_duration
    ...
    def run(self, runner):
        ...
        if not runner.config.dry_run and run_scenario:
            ...
            if self.hook_failed:
                # -- SKIP: Scenario steps and behave_bo like dry_run_scenario
                failed = True
            ...
            for step in self.all_steps:
                if run_steps:
                    ...
                    if not found_step_match:
                        ...
                        self.feature.steps_definitions_dirs[f'{self.name}{self.location}'].add(
                            found_step_match.location.filename
                        )
    ...
    def main_tag(self):
        ...
        for tag in self.tags:
            if all(not tag.startswith(skip_tag) for skip_tag in self.base_skip_tags):
                main_tag = tag
                break

        if main_tag is None:
            raise MainTagExc(f'Scenario {self.name} does not have a main tag.')
    ...
    def name_w_tag(self):
        try:
            name = f'{self.main_tag} {self.name}'
        except MainTagExc:
            name = self.name
...
class ScenarioOutline(Scenario):
    ...
    A scenario outline extends the existing :class:`~behave_bo.model.Scenario`
    class with the addition of the :class:`~behave_bo.model.Examples` tables of
    ...
    .. attribute:: feature
       ...
       The :class:`~behave_bo.model.Feature` this scenario outline belongs to.

    .. attribute:: steps

       A list of :class:`~behave_bo.model.Step` making up this scenario outline.

    .. attribute:: examples

       A list of :class:`~behave_bo.model.Examples` used by this scenario outline.
    ...
    def duration_pt(self):
        outline_duration = 0
        for scenario in self._scenarios:    # -- AVOID: BUILD-SCENARIOS
            outline_duration += scenario.duration_pt
        return outline_duration
...
class Examples(TagStatement, Replayable):
    ...
    .. attribute:: table
       ...
       An instance  of :class:`~behave_bo.model.Table` that came with the example
       in the *feature file*.
...
class Step(BasicStatement, Replayable):
    ...
    .. attribute:: text
       ...
       An instance of :class:`~behave_bo.model.Text` that came with the step
       in the *feature file*.

    .. attribute:: table

       An instance of :class:`~behave_bo.model.Table` that came with the step
       in the *feature file*.
    ...
    def run(self, runner, quiet=False, capture=True):
        ...
        self.duration_pt = time.process_time() - start_pt
...
class Table(Replayable):
    ...
    **iteration**
      Iterating over the Table will yield the :class:`~behave_bo.model.Row`
    ...
    **indexed access**
    ...
    .. attribute:: rows
       ...
       An list of instances of :class:`~behave_bo.model.Row` that make up the body
...
class Text(str):
    ...
    def replace(self, old, new, count=-1):
        return Text(super().replace(old, new, count), self.content_type,
                    self.line)
