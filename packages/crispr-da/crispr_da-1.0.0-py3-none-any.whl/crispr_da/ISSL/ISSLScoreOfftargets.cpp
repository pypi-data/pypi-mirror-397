...
int main(int argc, char** argv)
{
    ...
    */
    ...
    vector<uint64_t> queryUniqueSites(queryCount);
    vector<uint64_t> queryTotalSites(queryCount);
    ...
    #pragma omp parallel
    {
        ...
        for (int searchIdx = 0; searchIdx < querySignatures.size(); searchIdx++) {
            ...
            queryTotalSites[searchIdx] = totalSites;
            queryUniqueSites[searchIdx] = uniqueSites;
    ...
    for (size_t searchIdx = 0; searchIdx < querySignatures.size(); searchIdx++) {
        auto querySequence = decode2bit(querySignatures[searchIdx], 20);
        ...
        if (calcCfd)
            printf("%f\t", querySignatureCfdScores[searchIdx]);
        else
            printf("-1\t");

        printf("%ld\t", queryUniqueSites[searchIdx]);
        printf("%ld\n", queryTotalSites[searchIdx]);
