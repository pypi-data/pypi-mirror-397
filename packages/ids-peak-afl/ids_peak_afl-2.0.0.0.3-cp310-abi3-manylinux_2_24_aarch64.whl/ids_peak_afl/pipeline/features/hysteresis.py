from ids_peak_common import Range
from ids_peak_common.serialization import IArchive

from ids_peak_afl.ids_peak_afl import Controller

from ids_peak_afl.pipeline.features.ifeature import IRangeFeature
from ids_peak_afl.pipeline._internal.utils import (
    get_and_reraise as _get_and_reraise,
)


class Hysteresis(IRangeFeature[int]):
    """
    Hysteresis prevents oscillation in automatic control systems by
    introducing a dead zone around the target value where no adjustments are
    made.

    Hysteresis is important in automatic focus systems to prevent continuous
    minor adjustments that can cause instability. The hysteresis value
    specifies how far the measured value must deviate from the target before
    the controller makes an adjustment.

    .. versionadded:: 2.0
    """

    def __init__(self, controller: Controller) -> None:
        """
        Creates a Hysteresis feature with the specified controller

        :param controller: The auto controller that will be used to manage the
                           hysteresis parameter.

        .. versionadded:: 2.0
        """
        self._controller = controller

    @property
    def value(self) -> int:
        """
        Returns the hysteresis value

        .. versionadded:: 2.0
        """
        return self._controller.GetHysteresis()

    @value.setter
    def value(self, value: int) -> None:
        """
        Sets the hysteresis value

        :param value: The hysteresis value to set. Use ``range`` to get the
                      valid range.

        .. versionadded:: 2.0
        """
        self._controller.SetHysteresis(value)

    @property
    def range(self) -> Range:
        """
        Returns the valid range for auto percentile values

        .. versionadded:: 2.0
        """
        value_range = self._controller.GetHysteresisRange()
        return Range(value_range.min, value_range.max, value_range.inc)

    def serialize(self, archive: IArchive) -> None:
        """
        Stores the current state of this feature to an archive.

        :param archive: The archive in which this feature's state will
                        be stored.

        .. versionadded:: 2.0
        """
        archive.set("Hysteresis", self.value)

    def deserialize(self, archive: IArchive) -> None:
        """
        Restores this feature's state from an archive.

        :param archive: The archive from which the state will be restored.
        :raises ids_peak_afl.exceptions.CorruptedDataException:
                if deserialization fails because of a corrupted file

        .. versionadded:: 2.0
        """
        self.value = _get_and_reraise(archive, "Hysteresis", int)
