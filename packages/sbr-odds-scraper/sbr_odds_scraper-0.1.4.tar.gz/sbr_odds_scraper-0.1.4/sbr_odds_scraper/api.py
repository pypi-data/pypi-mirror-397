...
def scrape(
    ...
    if len(df) > 0:
        ...
        if null_count > 0:
            # Count unique games (not rows) without game_id
            unmatched_games = df[df['game_id'].isna()][['date', 'away_team', 'home_team', 'start_time']].drop_duplicates()
            print(f"Warning: {len(unmatched_games)} game(s) could not be matched to MLB schedule ({null_count} rows)")
