...
from rich.console import Group  # ✅ FIX

from .progress import load_state, save_state

ACHIEVEMENTS_DIR = Path(__file__).resolve().parent / "achievements"


# -------------------------------------------------
# Terminal bell (sound cue)
# -------------------------------------------------
def _bell():
...
def _rarity(ach):
    ...
    if "difficulty_completed_gte" in cond:
        ...
        if diff in {"expert", "architect", "principal", "staff", "distinguished", "fellow"}:
            return "Legendary"
        return "Rare"
...
def show_badges():
    ...
    for ach in achievements:
        ...
        bar = Text("—", style="dim")
        if tgt > 0:
            p = Progress(TextColumn("{task.percentage:>3.0f}%"), BarColumn(18))
            p.add_task("", total=tgt, completed=min(cur, tgt))
            bar = p

        rarity = _rarity(ach)
        rarity_style = {"Common": "dim", "Rare": "cyan", "Legendary": "bold yellow"}[rarity]
        ...
        table.add_row(
            ...
            bar,
            "🎉 NEW!" if ach in newly_unlocked else ("✅" if earned else "❌"),
        )
