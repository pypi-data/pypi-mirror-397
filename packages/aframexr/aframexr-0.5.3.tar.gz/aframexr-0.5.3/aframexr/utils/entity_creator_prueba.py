...
MAX_BAR_WIDTH_QUANTITATIVE_X_DATA = 10  # TODO --> Borrar, meter en cosntants
...
def _translate_encoding_type(series_dtype: pl.DataType) -> str:
    """Translates the series dtype into encoding type."""

    if series_dtype in pl.NUMERIC_DTYPES:
        return 'quantitative'
    return 'nominal'
...
class ChartCreator:
    ...
    @staticmethod
    def _set_coordinates_for_quantitative(data_series: Series, max_size: float) -> Series:
        """Returns a Series of the coordinates for the axis represented by the quantitative type data series."""

        max_value = data_series.max()  # Proportional dimensions of the data
        coordinates = (data_series / max_value) * max_size  # Series coordinates
        return coordinates / 2

    @staticmethod
    def _set_coordinates_for_nominal(data_series: Series, step: float) -> Series:
        """Returns a Series of the coordinates for the axis represented by the nominal type data series."""

        category_names = sorted(data_series.unique().to_list())  # Sorted for consistency
        z_coordinates_map = Series(
            (step / 2) + (  # Correct the position (because of element creation)
            pl.int_range(
                start=0,
                end=len(data_series),
                step=1,
        ...
        coordinates = data_series.replace(mapping_dict)
        return coordinates
...
class BarChartCreator(ChartCreator):
    ...
    def _set_bars_heights(self) -> Series:
        ...
        y_encoding = _translate_encoding_type(self._y_data.dtype) if self._y_data is not None else None
        if self._y_data is None or y_encoding == 'nominal':
            return pl.repeat(
                value=DEFAULT_BAR_HEIGHT_WHEN_NO_Y_AXIS,
                n=len(self._raw_data),
                eager=True  # Returns a Series
            ).alias('height')
        elif y_encoding == 'quantitative':
            max_value = self._y_data.max()
            return ((self._y_data / max_value) * self._max_height).alias('height')
        else:
            raise ValueError(f'Encoding {y_encoding} is not supported.')

    def _set_bars_widths(self) -> Series:
        """Returns a Series of the width for each bar composing the bar chart."""

        x_encoding = _translate_encoding_type(self._x_data.dtype) if self._x_data is not None else None
        if self._x_data is None or x_encoding == 'nominal':  # There is no x-axis data or the type is nominal
            return pl.repeat(
                value=self._bar_width,
                n=len(self._raw_data),
                eager=True  # Returns a Series
            ).alias('width')
        elif x_encoding == 'quantitative':
            max_value = self._x_data.max()
            return ((self._x_data / max_value) * MAX_BAR_WIDTH_QUANTITATIVE_X_DATA).alias('width')
        else:
            raise ValueError(f'Encoding {x_encoding} is not supported.')
    ...
    def _set_x_coordinates(self) -> Series:
        ...
        x_encoding = _translate_encoding_type(self._x_data.dtype)
        if x_encoding == 'quantitative':
            return self._set_coordinates_for_quantitative(
                data_series=self._x_data,
                max_size=MAX_BAR_WIDTH_QUANTITATIVE_X_DATA
            ).alias('x_coordinates')
        elif x_encoding == 'nominal':
            return self._set_coordinates_for_nominal(
                data_series=self._x_data,
                step=self._bar_width
            ).alias('x_coordinates')
        else:
            raise ValueError(f'Encoding {x_encoding} is not supported.')

    def _set_y_coordinates(self) -> Series:
        """Returns a Series of the y coordinates for each bar composing the bar chart."""

        if self._y_data is None:  # No field for y-axis
            return pl.repeat(
                value=DEFAULT_BAR_HEIGHT_WHEN_NO_Y_AXIS / 2,
                n=len(self._raw_data),
                eager=True  # Returns a Series
            ).alias('y_coordinates')

        y_encoding = _translate_encoding_type(self._y_data.dtype)
        if y_encoding == 'quantitative':
            return self._set_coordinates_for_quantitative(
                data_series=self._y_data,
                max_size=self._max_height
            ).alias('y_coordinates')
        elif y_encoding == 'nominal':
            return self._set_coordinates_for_nominal(
                data_series=self._y_data,
                step=DEFAULT_BAR_HEIGHT_WHEN_NO_Y_AXIS
            ).alias('y_coordinates')
        else:
            raise ValueError(f'Encoding {y_encoding} is not supported.')
    ...
    def _set_z_coordinates(self) -> Series:
        ...
        z_encoding = _translate_encoding_type(self._z_data.dtype)
        if z_encoding == 'quantitative':
            return self._set_coordinates_for_quantitative(
                data_series=self._z_data,
                max_size=DEFAULT_MAX_DEPTH
            ).alias('z_coordinates')
        elif z_encoding == 'nominal':
            return self._set_coordinates_for_nominal(  # Negative because of chart creation
                data_series=self._z_data,
                step=DEFAULT_BAR_DEPTH
            )
        else:
            raise ValueError(f'Encoding {z_encoding} is not supported.')
    ...
    def get_elements_specs(self) -> list[dict]:
        ...
        bar_widths = self._set_bars_widths()  # Series of the width for each bar
        ...
        y_coordinates = self._set_y_coordinates()  # Y-axis coordinate for each bar
