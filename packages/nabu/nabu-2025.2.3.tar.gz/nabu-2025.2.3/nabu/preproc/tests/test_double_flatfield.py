...
if __has_cupy__:
    from nabu.preproc.double_flatfield_cuda import CudaDoubleFlatField
...
class TestDoubleFlatField:
    ...
    @pytest.mark.skipif(not (__has_cupy__), reason="Need cupy for double flatfield with cuda backend")
    def test_dff_cuda(self):
        dff = CudaDoubleFlatField(
            self.radios.shape, result_url=self.ff_cuda_dump_url
        )  # , cuda_options={"ctx": self.ctx})
        d_radios = dff.cuda_processing.to_device("d_radios", self.radios)
