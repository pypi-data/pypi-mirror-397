"""
   ...
   Copyright [2021] [Nghia T. Vo, Diamond Light Source Ltd]
...
"""
This function was taken from "algotom" (3b9a75a5f7b4d407ce142964ca683c067be8c246),
with the following minor differences:
  - That the center of rotation is kept where it is, so that the sinogram is
    not shifted to have the CoR in the middle.
  - When CoR is on the left, use 2*cor instead of 2*cor+2 for overlapping region
"""


def extend_sinogram(sino_360, cor, apply_log=True, shift_sinogram=True):
    """
    ...
    if side == 1:
        if sub_pixel > 0.0 and shift_sinogram:
