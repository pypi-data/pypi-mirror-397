...
class AgentBill:
    ...
    def _track_usage(self, model: str, provider: str, input_tokens: int, output_tokens: int, latency_ms: float, cost: float, event_name: str = "ai_request", trace_id: str = None, span_id: str = None, parent_span_id: str = None, request_id: str = None, prompt_metadata: Dict = None):
        ...
        # Add OTEL correlation if available (v6.8.9: include parent_span_id)
        if trace_id:
    ...
    def _cache_response(self, model: str, prompt_hash: str, response_content: str, tokens_used: int, cost: float, prompt_content: str = None):
        """Cache AI response for semantic caching (v7.6.6)"""
        if not response_content:
            return
            
        url = f"{self.config.get('base_url', 'https://bgwyprqxtdreuutzpbgw.supabase.co')}/functions/v1/cache-ai-response"
        
        # v7.6.6 FIX: Use correct field names expected by edge function
        payload = {
            "api_key": self.config["api_key"],
            "prompt_hash": prompt_hash,
            "response_content": response_content,
            "model": model,
            "tokens_used": tokens_used,  # Fixed: was tokens_saved
            "cost": cost,                # Fixed: was cost_saved
            "prompt_content": prompt_content,  # Added: for prompt_normalized field
            "customer_id": self.config.get("customer_id"),
            "cacheable": True,
            "ttl_hours": 24
        }
    ...
    def wrap_openai(self, client: Any) -> Any:
        ...
        def tracked_chat_create(*args, **kwargs):
            ...
            # v7.6.7: Check for cached response from semantic cache
            ...
            if validation.get("cached") and validation.get("response_data"):
                ...
                # v7.6.7 FIX: Extract token usage from cache_info or response_data.usage
                # This ensures cached spans have valid token data for OTEL validation
                cache_info = validation.get("cache_info", {})
                cached_response = validation.get("response_data", {})
                
                # Try to get tokens from cache_info first, then from response_data.usage
                tokens_saved = cache_info.get("tokens_saved", 0)
                ...
                # If we have tokens_saved but no usage breakdown, estimate 20% input / 80% output
                if tokens_saved > 0 and total_tokens == 0:
                    total_tokens = tokens_saved
                if total_tokens > 0 and input_tokens == 0 and output_tokens == 0:
                    input_tokens = int(total_tokens * 0.2)
                ...
                span.set_attributes({
                    ...
                    "agentbill.cost_saved": cache_info.get("cost_saved", 0)
                })
                span.set_status(0)
                span.end()
            ...
            try:
                ...
                try:
                    ...
                    # v7.6.6: Pass prompt_content for prompt_normalized field
