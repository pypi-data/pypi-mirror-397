...
__all__ = ["Sandbox", "run", "check_output", "patch_subprocess", "unpatch_subprocess", "SandboxError", "SandboxedPopen"]
...
class Sandbox:
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        if check and code != 0:
            raise subprocess.CalledProcessError(code, command_str, output=result.stdout, stderr=result.stderr)
...
def _should_sandbox(args: Union[str, Sequence[str]], shell: bool, env: Optional[Mapping[str, str]] = None) -> bool:
        ...
        if _should_sandbox(args, shell, env):
            ...
            try:
                ...
                self._sandboxed_result = subprocess.CompletedProcess(
                    args=[command_str], returncode=self.returncode, stdout=b"", stderr=b"Sandboxed Popen failed to execute.\n"
                )
            
            # Mimic stdout/stderr pipes
            if stdout == subprocess.PIPE:
                self.stdout = io.BytesIO(self._sandboxed_result.stdout.encode() if isinstance(self._sandboxed_result.stdout, str) else self._sandboxed_result.stdout)
            else:
                self.stdout = None

            if stderr == subprocess.PIPE:
                self.stderr = io.BytesIO(self._sandboxed_result.stderr.encode() if isinstance(self._sandboxed_result.stderr, str) else self._sandboxed_result.stderr)
            else:
                self.stderr = None
...
def _create_sandboxed_run():
        ...
        def sandboxed_run(
            ...
            *,
            ...
            if _should_sandbox(args, shell, env):
                ...
                try:
                    ...
                    if check:
                        ...
                        raise subprocess.CalledProcessError(1, command_str, stderr=str(e).encode())
            ...
            return original_run(args, bufsize=bufsize, executable=executable, stdin=stdin, stdout=stdout, stderr=stderr, preexec_fn=preexec_fn, close_fds=close_fds, shell=shell, cwd=cwd, env=env, universal_newlines=universal_newlines, startupinfo=startupinfo, creationflags=creationflags, restore_signals=restore_signals, start_new_session=start_new_session, pass_fds=pass_fds, capture_output=capture_output, timeout=timeout, check=check, text=text, encoding=encoding, errors=errors)
        ...
        def _create_sandboxed_call():
            ...
            def sandboxed_call(
                ...
                *,
                ...
                return original_call(args, bufsize=bufsize, executable=executable, stdin=stdin, stdout=stdout, stderr=stderr, preexec_fn=preexec_fn, close_fds=close_fds, shell=shell, cwd=cwd, env=env, universal_newlines=universal_newlines, startupinfo=startupinfo, creationflags=creationflags, restore_signals=restore_signals, start_new_session=start_new_session, pass_fds=pass_fds, timeout=timeout)
            ...
            def _create_sandboxed_check_call():
                ...
                def sandboxed_check_call(
                    ...
                    *,
                    ...
                    return original_check_call(args, bufsize=bufsize, executable=executable, stdin=stdin, stdout=stdout, stderr=stderr, preexec_fn=preexec_fn, close_fds=close_fds, shell=shell, cwd=cwd, env=env, universal_newlines=universal_newlines, startupinfo=startupinfo, creationflags=creationflags, restore_signals=restore_signals, start_new_session=start_new_session, pass_fds=pass_fds, timeout=timeout)
                ...
                def _create_sandboxed_check_output():
                    ...
                    def sandboxed_check_output(
                        ...
                        *,
                        ...
                        return original_check_output(args, bufsize=bufsize, executable=executable, stdin=stdin, stderr=stderr, preexec_fn=preexec_fn, close_fds=close_fds, shell=shell, cwd=cwd, env=env, universal_newlines=universal_newlines, startupinfo=startupinfo, creationflags=creationflags, restore_signals=restore_signals, start_new_session=start_new_session, pass_fds=pass_fds, timeout=timeout, text=text, encoding=encoding, errors=errors)
                    ...
                    def _create_sandboxed_getoutput():
                        ...
                        def sandboxed_getoutput(
                            ...
                            if _should_sandbox(cmd, shell=True, env=None): # getoutput always uses shell=True
                                ...
                                result = sb.run(cmd, capture_output=True, check=True, text=True)
                        ...
                        def _create_sandboxed_getstatusoutput():
                            ...
                            def sandboxed_getstatusoutput(
                                ...
                                if _should_sandbox(cmd, shell=True, env=None): # getstatusoutput always uses shell=True
                                ...
                                return original_getstatusoutput(cmd, globals=globals, locals=locals)
