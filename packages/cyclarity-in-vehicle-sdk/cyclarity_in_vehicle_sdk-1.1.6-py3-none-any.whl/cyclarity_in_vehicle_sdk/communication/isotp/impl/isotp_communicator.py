...
class IsoTpCommunicator(IsoTpCommunicatorBase):
    ...
    def validate_addressing(self):
        if not self.extended_addressing and (self.rxid > CAN_ID_MAX_NORMAL_11_BITS or self.txid > CAN_ID_MAX_NORMAL_11_BITS):
            raise ValueError("Non-extended addressing is not supported for CAN ids above 0x7FF")
        return self

    def model_post_init(self, *args, **kwargs):
        super().model_post_init(*args, **kwargs)
        mode = AddressingMode.Normal_29bits if self.extended_addressing else AddressingMode.Normal_11bits
