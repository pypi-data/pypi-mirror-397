...
class SurfaceGeometry(ProjectElementGeometry):
    ...
    triangles = properties.Instance("Vertex indices of surface triangles", Int3Array)
...
class SurfaceGridGeometry(ProjectElementGeometry):
    ...
    tensor_u = properties.Array(
        "Grid cell widths, u-direction", shape=("*",), dtype=float
    )
    tensor_v = properties.Array(
        "Grid cell widths, v-direction", shape=("*",), dtype=float
    )
...
class SurfaceElement(ProjectElement):
    ...
    geometry = properties.Union(
        "Structure of the surface element", props=(SurfaceGeometry, SurfaceGridGeometry)
    )
    ...
    subtype = properties.StringChoice(
        "Category of Surface", choices=("surface",), default="surface"
    )
