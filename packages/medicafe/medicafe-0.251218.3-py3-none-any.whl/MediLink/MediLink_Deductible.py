                    ...
                    try:
                        ...
                        if use_cache:
                            if cache_reason == 'zero':
                                ...
                                cache_hits_recent += 1
                            result = _build_cached_result(patient_info, cache_payload, dob, member_id, cache_reason=cache_reason)
                            # Apply CSV backfill for consistency with API results (service_date_display, etc.)
                            try:
                                result = backfill_enhanced_result(result, patient_info)
                            except Exception:
                                pass  # Preserve original result if backfill fails
                            results_by_patient[(dob, member_id)] = result
                            ...
                            if cache_reason == 'recent':
                                MediLink_ConfigLoader.log(
                                    "Cache fresh (<24h) for patient {} (DOB {}, Member ID {}) - skipping API refresh".format(
                                        patient_id or 'UNKNOWN', dob, member_id),
                                    level="DEBUG"
                                )
                            else:
                                MediLink_ConfigLoader.log(
                                    "Cache satisfied patient {} (DOB {}, Member ID {}) -> remaining {} (skipping API)".format(
