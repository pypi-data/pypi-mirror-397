...
def move_csvs_from_storage(local_storage_path, source_folder, silent=False):
    ...
    if not silent:
        print("Checking for new CSV files in local storage...")
    _log("Moving {} CSV file(s) from local storage to source folder".format(len(csv_files)), level="INFO")
    ...
    for csv_file in csv_files:
        try:
            ...
            if not silent:
                print("Moving {} to {}...".format(csv_file, source_folder))
            
            shutil.move(csv_file, dest_path)
            moved_count += 1
            _log("Moved CSV from local storage: {}".format(filename), level="INFO")
        except Exception as e:
...
def process_csv(source_folder, target_folder, config_file, local_storage_path=None, python_script=None, silent=False):
    ...
    _log("Starting CSV processing", level="INFO")
    _log("Source folder: {}".format(source_folder), level="INFO")
    _log("Target folder: {}".format(target_folder), level="INFO")
    _log("Config file: {}".format(config_file), level="INFO")
    ...
    if local_storage_path:
        ...
        if moved_count > 0 and not silent:
            print("Moved {} CSV file(s) from local storage.".format(moved_count))
    ...
    if not latest_csv_name:
        ...
        _log("No CSV files found in source folder - skipping CSV processing", level="INFO")
    ...
    _log("Moved CSV to target folder: {} -> {}".format(latest_csv_name, os.path.basename(new_csv_path)), level="INFO")
    
    # Update config with new CSV path
    if HAS_UPDATE_JSON:
        try:
            update_csv_path(config_file, new_csv_path)
            _log("Updated config with new CSV path: {}".format(new_csv_path), level="INFO")
        except Exception as e:
    ...
    if HAS_ADD_DOWNLOADED_EMAIL:
        ...
        if os.path.exists(new_csv_path):
            try:
                ...
                if success:
                    _log("Updated downloaded_emails.txt with: {}".format(original_csv_filename), level="INFO")
                else:
                    if not silent:
    ...
    _log("CSV processing completed successfully", level="INFO")
    return 0
