...
from thingsboard.common.utils import timestamp_helpers as timestamp_helpers
...
class JsonMessageFormat(MessageFormat):
    ...
    def __init__(self):
        super(JsonMessageFormat, self).__init__()
    ...
    def serialize_attribute(self, attribute):
        data = {
            'type': attribute.get_type_as_string(),
            'constraint': attribute.get_constraint().to_string()
        }
    ...
    def deserialize_attribute(self, data, attribute):
        ...
        # Replace key (topic) with 'value'. (The following code needs to have 'timestamp' and 'value' in dictionary)
        if not 'value' in data_dict:
            if len(data_dict) == 1:
                # Replace current key (topic) with 'value'
                data_dict['value'] = data_dict.pop(next(iter(data_dict)))

        # In case there is no timestamp present, create one using the current time
        ...
        if isinstance(data_dict, dict) and \
            ...
            if timestamp != 0 and value is not None:
                attr_value = attribute.get_type()

                if attr_value == AttributeType.Invalid:
                    pass
                elif attr_value == AttributeType.Boolean:
                    bool_value = False
                ...
                elif attr_value == AttributeType.Integer:
                    attribute.set_value_from_cloud(int(value), timestamp)
                elif attr_value == AttributeType.Number:
                    attribute.set_value_from_cloud(float(value), timestamp)
                elif attr_value == AttributeType.String:
