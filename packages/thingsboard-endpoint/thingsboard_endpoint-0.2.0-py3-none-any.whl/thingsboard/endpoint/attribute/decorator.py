...
#
class endpoint_attribute(object):
    """Decorator class adding the possibility to auto-update a model attribute to the cloud whenever it changes.
    ...
    def __set__(self, obj, value):
        ...
        try:
            # Update value on the cloud by calling method '_update_endpoint_attribute'
            # which must be provided by the instance having the endpoint_attribute
            # decorated attribute.
            #
            # Give as second parameter the value using the fget.__get__ property and
            # not the value parameter. It may be different.
            #        obj._update_endpoint_attribute(self._fget.__name__, self._fget.__get__(obj)())
            obj._update_endpoint_attribute(self.__name__, self.__get__(obj))
        except (AttributeError, TypeError):
            traceback.print_exc()
            callback_name = '_update_endpoint_attribute'
