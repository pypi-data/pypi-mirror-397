...
from thingsboard.common.utils import timestamp_helpers
from thingsboard.endpoint.attribute.constraint import AttributeConstraint as AttributeConstraint
from thingsboard.endpoint.attribute.type import AttributeType as AttributeType
...
from thingsboard.endpoint.interface.attribute_listener import AttributeListener
from thingsboard.endpoint.interface.unique_identifiable import UniqueIdentifiable
from thingsboard.endpoint.topicuuid import TopicUuid


class Attribute(UniqueIdentifiable):
    """The leaf information in the endpoint data model
    """
    ...
    def set_value_from_cloud(self, value, timestamp):
        ...
        if self._timestamp is not None and self._timestamp >= timestamp:
            print('Warning: Ignoring new value from cloud. Not valid timestamp!')
            return False
    ...
    def set_name(self, name):
        ...
        if self._name is not None:
            raise ModificationException('The Attribute has already a name (Renaming attributes is forbidden)!')
    ...
    def set_type(self, the_type: [bool, int, float, str]):
        ...
        the attribute already has a type, the method fails with a runtime exception.

        :param the_type Python type like bool, int, float and str
        :type [bool, int, float, str]
        """
        if self._value:
            raise ModificationException('The Attribute has already a type (Changing the type is not allowed)!')
        ...
        if the_type in (bool, int, float, bytes, str):
            ...
            # Set attribute type accordingly
            if the_type in (bool,):
            ...
            raise InvalidAttributeException(the_type)
    ...
    def set_parent(self, parent):
        ...
        if self._parent:
            raise ModificationException('The parent of an Attribute can never be changed ' +
                                        '(Attributes can not be moved)!')
        # assert isinstance(parent, AttributeContainer), 'Wrong type for parent attribute!'
        self._parent = parent
    ...
    def set_constraint(self, constraint):
        ...
        :param constraint:
        :type constraint: AttributeConstraint
        :return:
        """
        ...
        if self._constraint:
            raise ModificationException('The Attribute has already a constraint ' +
    ...
    def to_json(self, encoder):
        ...
        # Get the type of the value and convert it to endpoint attribute type
