...
class RuntimeNode(Node):
    """The RuntimeNode class allows to create the structure of an endpoint node at runtime.

    It is in contrast with the Node class which uses a static model.
    """
    ...
    def add_object(self, name, cls_or_object):
        ...
        if self.is_node_registered_within_endpoint():
            raise RuntimeError('A RuntimeNode\'s structure can only be modified before it '
