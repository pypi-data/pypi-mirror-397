...
class EmbeddingServiceAsyncClient:
    ...
    def __init__(
        ...
        if config:
            ...
            server_port = config.get(
                "server.port", port or int(os.getenv("EMBEDDING_SERVICE_PORT", "8001"))
            )
            ...
            if config_base_url:
                ...
                host_value = (
                    server_host
                    or base_url
                    or os.getenv("EMBEDDING_SERVICE_BASE_URL", "http://localhost")
                )
            ...
            if "base_url" in server_config:
                ...
                port = server_config.get(
                    "port", port or int(os.getenv("EMBEDDING_SERVICE_PORT", "8001"))
                )
            ...
            try:
                self.base_url = base_url or os.getenv(
                    "EMBEDDING_SERVICE_BASE_URL", "http://localhost"
                )
        ...
        if self._adapter_transport:
            try:
                ...
                raise EmbeddingServiceError(
                    f"Failed to close adapter transport: {e}"
                ) from e
    ...
    async def cmd(
        ...
        try:
            ...
            raise EmbeddingServiceAPIError(
                error_dict.get("error", {"message": str(e)})
            ) from e
    ...
    async def wait_for_job(
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                logger.debug(
                    f"Job {job_id} status: {current_status}, full status: {status}"
                )
    ...
    async def list_queued_commands(
        ...
        """
        try:
            result = await self._adapter_transport.queue_list_jobs(
    ...
    @classmethod
    def with_auth(
        cls, base_url: str, port: int, auth_method: str, **kwargs
    ) -> "EmbeddingServiceAsyncClient":
        """
