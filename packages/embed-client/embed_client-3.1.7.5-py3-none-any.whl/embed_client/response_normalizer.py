...
class ResponseNormalizer:
    ...
    @staticmethod
    def _normalize_immediate_response(adapter_response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _normalize_queued_response(adapter_response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Add job metadata
        if job_id:
            normalized["result"]["data"]["job_id"] = job_id
            normalized["result"]["data"]["status"] = status
    ...
    def normalize_queue_status(queue_status: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not result:
            result = queue_status.get("data", {}).get("result") or queue_status.get("data", {})
