...
class VectorizationCLI:
    ...
    async def vectorize_texts(
        ...
        """Vectorize texts using high-level embed() helper.

        Notes:
            - Uses error_policy="continue" to align with Embedding Service contract:
              per-item validation errors are returned in data["results"][*]["error"].
            - Delegates all text validation to the server; no local fail-fast behaviour.
        """
        try:
            # Always use error_policy="continue" for batch safety and per-item errors
            data = await self.client.embed(texts, error_policy="continue")

            if show_full_data:
                if output_format == "json":
                    print(json.dumps(data, indent=2))
                else:
                    # For other formats, extract just embeddings from data
                ...
                # Extract just embeddings; fall back to response_parsers helper if needed
                embeddings = data.get("embeddings")
                if embeddings is None:
                    # Wrap into legacy result shape for response_parsers
                    wrapped_result: Dict[str, Any] = {
                        "result": {"success": True, "data": data}
                    }
                    embeddings = extract_embeddings(wrapped_result)
