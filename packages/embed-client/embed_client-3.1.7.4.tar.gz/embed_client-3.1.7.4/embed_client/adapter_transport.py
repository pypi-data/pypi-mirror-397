...
class AdapterTransport:
    ...
    async def execute_command(
        ...
        if use_cmd_endpoint:
            ...
            result = await client.execute_command(command, params, use_cmd_endpoint=False)
    ...
    async def queue_get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        For embedding service, tries embed_job_status first (for embed_queue jobs),
        then falls back to standard queue_get_job_status if needed.
        ...
        # For embedding service, try embed_job_status first (for embed_queue jobs)
        # This is a fallback for direct status checks when execute_command_unified
        # is not used (e.g., when calling job_status() directly)
        try:
            # Try embed_job_status first (for embedding service jobs)
            response = await client.execute_command("embed_job_status", {"job_id": job_id}, use_cmd_endpoint=False)
            ...
            # If embed_job_status fails, fall back to standard queue_get_job_status
            logger.debug(f"embed_job_status failed for job {job_id}, trying standard endpoint: {e}")

        # Fallback to standard queue_get_job_status
