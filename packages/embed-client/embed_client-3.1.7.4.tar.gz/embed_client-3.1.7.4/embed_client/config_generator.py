...
class ClientConfigGenerator:
    ...
    def generate_all_configs(
        ...
        configs["http"] = self.generate_http_config(
            host=host, port=port, output_path=output_dir / "http.json" if output_dir else None
        )

        # 2. HTTP + Token
        configs["http_token"] = self.generate_http_token_config(
            host=host, port=port, output_path=output_dir / "http_token.json" if output_dir else None
        )

        # 3. HTTP + Token + Roles
        configs["http_token_roles"] = self.generate_http_token_roles_config(
            host=host, port=port, output_path=output_dir / "http_token_roles.json" if output_dir else None
        )
...
def main():
    ...
    parser.add_argument("--output-dir", default="./configs", help="Output directory (for all modes)")
    ...
    if args.mode == "all":
        ...
        output_path = Path(args.output) if args.output else output_dir / f"{args.mode}.json"
        if output_path:
            output_path.parent.mkdir(parents=True, exist_ok=True)

        if args.mode == "http":
            generator.generate_http_config(host=args.host, port=args.port, output_path=output_path)
        elif args.mode == "http_token":
            generator.generate_http_token_config(
                host=args.host, port=args.port, api_key=args.api_key, output_path=output_path
            )
