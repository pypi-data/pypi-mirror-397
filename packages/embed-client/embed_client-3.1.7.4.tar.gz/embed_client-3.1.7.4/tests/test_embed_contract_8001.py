...
class EmbedContract8001Tester:
    ...
    async def test_mode(self, mode_name: str, config: Dict[str, Any]) -> bool:
        ...
        print(f"\n🔍 Testing embed contract for mode: {mode_name}")
        ...
        try:
            async with EmbeddingServiceAsyncClient(config_dict=config) as client:
                ...
                print(f"   ⏳ Embed contract test with {len(self.test_texts)} texts...")
                try:
                    data = await client.embed(self.test_texts, error_policy="continue")
                except EmbeddingServiceError as e:
                    print(f"   ❌ Embed failed with EmbeddingServiceError: {e}")
                    return False
                except Exception as e:  # noqa: BLE001
                    print(f"   ❌ Unexpected error during embed: {e}")
                    return False
                ...
                if not results:
                    print("   ❌ No 'results' in data – new contract not satisfied.")
                    return False

                if len(results) != len(self.test_texts):
                    print(f"   ❌ Wrong number of results: expected {len(self.test_texts)}, " f"got {len(results)}")
                    return False

                if embeddings is not None and len(embeddings) != len(self.test_texts):
                    print(
                        f"   ❌ Wrong number of embeddings: expected {len(self.test_texts)}, " f"got {len(embeddings)}"
                    )
                    return False
                ...
                for idx, (text, item) in enumerate(zip(self.test_texts, results)):
                    ...
                    if idx == 0:
                        ...
                        if err is not None or emb is None:
                            print(f"   ❌ Item 0 expected to be valid, got error={err}, embedding={emb}")
                            ok = False
                    else:
                        # Invalid texts should produce per-item error and null embedding
                        if emb is not None or err is None:
                            print(
                                f"   ❌ Item {idx} expected to have per-item error and null embedding, "
                                f"got error={err}, embedding={emb}"
                            )
                            ok = False
                        else:
                            if not isinstance(err, dict) or "code" not in err or "message" not in err:
                                print(f"   ❌ Item {idx} error does not contain code/message: {err}")
                                ok = False

                if ok:
                    print("   ✅ Embed contract satisfied for this mode.")
                else:
                    print("   ❌ Embed contract NOT satisfied for this mode.")
    ...
    async def run_all_tests(self) -> bool:
        ...
        print("=" * 80)
        print("🧪 Embed Contract Test Runner for All Security Modes on localhost:8001")
        print("=" * 80)
        ...
        print("\n" + "=" * 80)
        print("📊 Embed Contract Test Results")
        print("=" * 80)
        ...
        if failed == 0:
            print("\n🎉 Embed contract is satisfied for all modes!")
            return True

        print("\n⚠️  Embed contract FAILED for some modes.")
        return False
