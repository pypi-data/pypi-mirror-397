...
def is_bib_file_include(line: str):
    if '\\bibliography{' in line:
        return line.split('\\bibliography{')[1].split('}')[0]
...
def run(main_tex_file: Path, target_dir : Union[Path, None] = None, copy_files=None, verbose: bool = False, no_reduce_bib: bool = False, no_rename_figures: bool = False):
    if copy_files is None:
        copy_files = []
    base_path = main_tex_file.parent
    print(f'{base_path.absolute()=};')
    if target_dir is not None:
        out_dir = target_dir / 'bundle'
    else:
        out_dir = base_path / 'bundle'
    if out_dir.exists():
        rmtree(out_dir)
    ...
    with open(main_tex_file) as file:
        ...
        for aux_file in aux_files:
            ...
            if not no_reduce_bib and aux_file.endswith('.bib'):
                ...
                tex_cites = extract_citations(data)
                ...
                # logging.debug(f"Used references:{used}")
                # logging.debug(f"Unused references: {unused}")
                # logging.debug(f"Missing references: {missing}")
...
def main():
    ...
    run(Path(args.file), args.target_dir, args.copy_files, args.verbose, args.no_reduce_bib, args.no_rename_figures)
