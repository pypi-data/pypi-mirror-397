...
from .utils.constants import J2, EARTH_EQUATORIAL_RADIUS as R, GM_EARTH as MU
...
def raan_dot(a, e, i):
    ...
    raan_dot = (-1.5 * J2 * np.sqrt(MU) * R**2 * np.cos(i) /
                a**(7/2) * (1 - e**2)**2)
...
def sunlight_time_per_orbit(a: float, beta: float) -> float:
    ...
    if np.abs(np.sin(beta)) > R/a:
        phi = 0
    else:
        phi = np.acos(np.sqrt(1-(R/a)**2) / np.cos(beta))
