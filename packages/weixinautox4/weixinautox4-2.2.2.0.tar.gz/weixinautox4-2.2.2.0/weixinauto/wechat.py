...
class Wechat:
    ...
    def get_new_message(
            self,
            on_new: Callable[[List[ChatMessage], Callable[[ChatMessage, str], bool]], None],
            *,
        ...
        监听主窗口「未读消息」：
        参数说明：
        - on_new: 回调函数，签名为 on_new(msgs, reply)
            - msgs: 本轮扫描得到的 ChatMessage 列表
            - reply: 一个函数 reply(msg: ChatMessage, text: str) -> bool，用于快速回复
        - interval_sec: 轮询主窗口未读消息的间隔秒数（默认 1.0 秒）
        - need_nickname: 是否尝试解析每条消息的发送者昵称（填充 ChatMessage.sender / is_self）
        - auto_save_image: 是否自动接收并保存图片消息
            - 开启后，图片消息会在扫描阶段自动右键复制并保存到本地
            - 保存成功后，会写入 ChatMessage.file_path / file_name
        - auto_save_file: 是否自动接收并保存文件消息
            - 行为同上，只是针对文件类型

        返回值：
        - True: 监听线程已成功启动
        - False: 当前设备未激活或驱动未初始化，无法启动监听
        """
        if not self._ensure_driver():
            print("[Wechat.get_new_message] 当前设备未激活，无法监听窗口")
            return False

        # 根据参数创建 WeixinNewMessage 监听器
        ...
        def _handle_msgs(msgs: List[ChatMessage]):
            """
            真正注册给 WeixinNewMessage 的回调：
            - 只负责把 msgs 和一个 reply 方法转交给业务侧的 on_new
            """
            if not msgs:
                return

            def _reply(msg: ChatMessage, text: str) -> bool:
                # 这里直接调用 watcher.reply，内部已有 _ui_lock 保护 UI 操作
                return watcher.reply(msg, text)
        ...
        # 启动主窗口未读消息监听线程
        watcher.start_watch(_handle_msgs, interval_sec=interval_sec)

        # 挂到实例上，方便后续调用 stop_new_message_listen()
