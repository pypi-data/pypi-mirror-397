...
from aitrados_broker.utils.full_symbol_generator import FullSymbolGenerator
...
class CtpTdApi(TdApi):
    ...
    def __set_instrument_extra(self,contract: ContractData,data: dict):
        if contract.product==Product.FUTURES:
            expiration_date = datetime.strptime(data['ExpireDate'], "%Y%m%d").strftime("%Y-%m-%d")
            expiration_datetime=datetime.strptime(data['ExpireDate'], "%Y%m%d").strftime("%Y-%m-%d 15:00:00+08:00")
            listing_date=datetime.strptime(data['OpenDate'], "%Y%m%d").strftime("%Y-%m-%d")
            extra = {
                "delivery_year": data.get("DeliveryYear", None),
                "delivery_month": data.get('DeliveryMonth', None),
                "settlement_currency": "CNY",
                "listing_date": listing_date,
                "expiration_date": expiration_date,
                "expiration_datetime": expiration_datetime,
                "last_trading_date":expiration_date,
                "first_notice_date":expiration_date
            }
    ...
    def onRspQryInstrument(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        if product:
            ...
            self.__set_instrument_extra(contract, data)
            FullSymbolGenerator.ctp(contract)
            contract.broker_symbol = data["InstrumentID"]
