...
def pls(
    ...
    Returns:
        xr.Dataset: The dataset holding the results of the PLS analysis. Results will either be in variables with the same name as `xdata_array` or `ydata_array` or in variables `x` and `y` if the names on the respective array are not set.
    """
...
def pls_ds(
    ...
    Returns:
        xr.Dataset: The result of the call to `pls()`. Has the results of the PLS as variables in either the same names as `xname` and `yname` or in `x` and `y`.s
    """
    return pls(
