...
class JavaParser(BaseParser):
    ...
    def _visit_node(self, node: javalang.ast.Node, metrics: dict[str, int]) -> None:
        ...
        if isinstance(node, javalang.tree.MethodDeclaration):
            ...
            metrics["max_method_complexity"] = max(
                metrics["max_method_complexity"], self._calculate_complexity(node)
            )
    ...
    def _check_java_code(self, ast: javalang.tree.CompilationUnit) -> list[str]:
        ...
        def find_identifiers(
            node: javalang.ast.Node, name: str
        ) -> list[javalang.tree.Identifier]:
    ...
    def _visit_for_warnings(
        self, node: javalang.ast.Node, warnings: list[str], find_identifiers: Callable
    ) -> None:
        """
        ...
        if isinstance(node, javalang.tree.VariableDeclarator):
            if not find_identifiers(node, node.name):
                warnings.append(
                    f"Unused variable '{node.name}' at line {node.position.line}"
                )
        ...
        if isinstance(node, javalang.tree.ReturnStatement) and hasattr(node, "parent"):
            ...
            if any(
                ...
                warnings.append(
                    f"Unreachable code after return statement at line {node.position.line}"
                )
