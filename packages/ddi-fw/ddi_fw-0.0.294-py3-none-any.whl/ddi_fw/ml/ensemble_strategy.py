...
class StackingStrategy(EnsembleStrategy):
    ...
    def combine_predictions(self, predictions: List[np.ndarray]) -> np.ndarray:
        ...
        if self.processor and self.processing_config:
            self.meta_learner.train_data = self.processor().process2(self.meta_learner.train_data, self.processing_config)
            self.meta_learner.test_data = self.processor().process2(predictions, self.processing_config)
            ...
            if s == "stacking":
                ...
                for meta_learner in self.meta_learners:
                    self.selected_strategies.append(StackingStrategy(descriptor=meta_learner.descriptor,
                                                                     meta_learner=meta_learner, base_wrappers=self.base_wrappers))
            else:
