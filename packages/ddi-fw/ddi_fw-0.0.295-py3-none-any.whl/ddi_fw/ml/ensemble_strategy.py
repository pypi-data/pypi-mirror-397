...
class StackingStrategy(EnsembleStrategy):
    ...
    def combine_predictions(self, predictions: List[np.ndarray]) -> np.ndarray:
        ...
        if self.processor and self.processor_config:
            self.meta_learner.train_data = self.processor().process2(self.meta_learner.train_data, self.processor_config)
            self.meta_learner.test_data = self.processor().process2(predictions, self.processor_config)
