"""Changelog command for folder2md4llms CLI."""
...
@click.option(
    "--recent", "-r", type=int, metavar="N", help="Show last N versions (default: 5)"
)
@click.option(
    "--since", "-s", metavar="VERSION", help="Show all versions since VERSION"
)
@click.option(
    "--breaking-only",
    "-b",
    is_flag=True,
    help="Show only versions with breaking changes",
)
@click.option(
    "--full",
    "-f",
    is_flag=True,
    help="Show complete changelog entry (not just highlights)",
)
def changelog(
    version: str | None,
    recent: int | None,
    ...
    r"""View changelog for folder2md4llms versions.

    \b
    Examples:
      folder2md --changelog              # Show current version's changes
      folder2md --changelog 0.5.13       # Show specific version
      folder2md --changelog --recent 3   # Show last 3 versions
      folder2md --changelog --since 0.5.0  # Show all since v0.5.0
      folder2md --changelog --breaking-only  # Show only breaking changes
    """
    try:
...
def _show_recent(content: str, count: int, breaking_only: bool, full: bool) -> None:
    ...
    if not entries:
        if breaking_only:
            console.print(
                f"No breaking changes found in last {count} versions", style="yellow"
            )
        else:
...
def _show_since(
    content: str, since_version: str, breaking_only: bool, full: bool
) -> None:
    ...
    if not entries:
        if breaking_only:
            console.print(
                f"No breaking changes found since v{since_version}", style="yellow"
            )
        else:
