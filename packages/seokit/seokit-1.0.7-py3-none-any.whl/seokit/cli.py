...
def main(ctx):
    ...
    # Auto-install slash commands on any CLI invocation
    _install_commands()
...
def setup():
    ...
    click.echo("-" * 40)
    click.echo(f"Installed slash commands to: {commands_dir}/")
    for cmd in SLASH_COMMANDS:
        cmd_path = commands_dir / cmd
        if cmd_path.exists():
            click.echo(f"  /{cmd.replace('.md', '')}")
...
def update(force: bool):
    ...
    if force:
        click.echo(f"SEOKit v{__version__} - Reset to defaults!")
    else:
        click.echo(f"SEOKit v{__version__} - Updated successfully!")
