...
class TestDirectoryPaths:
    ...
    def test_get_commands_dir(self):
        ...
        from seokit.config import get_commands_dir, get_claude_dir
    ...
    def test_get_seokit_data_dir(self):
        ...
        from seokit.config import get_seokit_data_dir, get_claude_dir
        data_dir = get_seokit_data_dir()
        assert data_dir == get_claude_dir() / 'seokit'

    def test_get_package_commands_dir(self):
        """Test get_package_commands_dir returns valid path."""
        from seokit.config import get_package_commands_dir
        pkg_commands = get_package_commands_dir()
        assert isinstance(pkg_commands, Path)
        # Should end with 'commands'
        assert pkg_commands.name == 'commands'
...
class TestInstallerIntegration:
    """Test installer integration with config module."""

    def test_marker_file_location(self):
        """Test marker file is in commands directory."""
        from seokit.installer import get_marker_file
        from seokit.config import get_commands_dir
        marker = get_marker_file()
        assert marker.parent == get_commands_dir()
        assert marker.name == '.seokit-installed'
