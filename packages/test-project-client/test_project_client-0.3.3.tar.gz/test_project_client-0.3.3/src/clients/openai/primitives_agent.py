...
with the `store` parameter set to `true` will be returned.
        Args:
            model: The model used to generate the Chat Completions.            metadata: A list of metadata keys to filter the Chat Completions by. Example:

`metadata[key1]=value1&amp;metadata[key2]=value2`            after: Identifier for the last chat completion from the previous pagination request.            limit: Number of Chat Completions to retrieve.            order: Sort order for Chat Completions by timestamp. Use `asc` for ascending order or `desc` for descending order. Defaults to `asc`.            response_model: Optional Pydantic model to parse the response into
...
with the `store` parameter set to `true` will be returned.
        Args:
            completion_id: The ID of the chat completion to retrieve.            response_model: Optional Pydantic model to parse the response into
...
the only supported modification is to update the `metadata` field.
        Args:
            completion_id: The ID of the chat completion to update.            body: Request body
            response_model: Optional Pydantic model to parse the response into
...
created with the `store` parameter set to `true` can be deleted.
        Args:
            completion_id: The ID of the chat completion to delete.            response_model: Optional Pydantic model to parse the response into
...
returned.
        Args:
            completion_id: The ID of the chat completion to retrieve messages from.            after: Identifier for the last message from the previous pagination request.            limit: Number of messages to retrieve.            order: Sort order for messages by timestamp. Use `asc` for ascending order or `desc` for descending order. Defaults to `asc`.            response_model: Optional Pydantic model to parse the response into
