...
class FileConfigStorage(ConfigStorageBase):
    ...
    def find_config(self, config_dir: str = '.'):
        ...
        for possible_file in possible_settings_file_names:
            if self.exists(os.path.join(config_dir, possible_file)):
                logging.getLogger().info(f'Found config file with extension: {possible_file}')
                return os.path.join(config_dir, possible_file)
            elif self.exists(os.path.join('.', possible_file)):
                logging.getLogger().info(f'Found config file with extension: {possible_file}')
                return os.path.join(config_dir, possible_file)
