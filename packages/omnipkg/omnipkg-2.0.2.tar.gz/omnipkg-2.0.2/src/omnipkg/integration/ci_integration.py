...
from omnipkg.cli_executor import CLIExecutor, handle_run_command
...
@app.command()
def activate(
    shell: Optional[str] = typer.Option(None, help="Shell type (bash, zsh)")
):
    """
...
def status():
    ...
    if is_active:
        ...
        if env_dir.exists():
            ...
            print(f"   Run: 8pkg activate")
        else:
            print(f"   Run: 8pkg activate  # to create environment")
