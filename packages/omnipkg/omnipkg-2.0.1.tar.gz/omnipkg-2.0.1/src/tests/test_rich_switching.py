...
def setup_environment(omnipkg_core: OmnipkgCore):
    ...
    main_rich_version = active_install_info.get('Version') if active_install_info else None

    if main_rich_version:
        safe_print(f"   ⚠️  Found existing Rich v{main_rich_version} (unexpected after cleanup).")
    else:
        safe_print(f"   ℹ️  Rich not found. Installing baseline v{DEFAULT_RICH_VERSION}...")
...
def create_test_bubbles(omnipkg_core: OmnipkgCore):
    ...
    for version in BUBBLE_VERSIONS_TO_TEST:
        if version == active_version:
            safe_print(f"   ℹ️  Skipping bubble for v{version} (matches active version).")
            continue
            
        safe_print(f'   🫧 Creating bubble for rich=={version}')
        omnipkg_core.smart_install([f'rich=={version}'])

def test_version_via_daemon(expected_version: str, client: DaemonClient, is_bubble: bool):
    """
...
'''
        ...
        if result.get('success'):
            ...
            if actual_version != expected_version:
                raise AssertionError(f"Bubble mismatch! Expected {expected_version}, got {actual_version}")
        else:
        ...
        if actual_version != expected_version:
            raise AssertionError(f"Main env mismatch! Expected {expected_version}, got {actual_version}")
...
def run_comprehensive_test():
    ...
    try:
        ...
        try:
            test_version_via_daemon(main_version_to_preserve, daemon_client, is_bubble=False)
        ...
        if 'omnipkg_core' in locals():
            ...
            safe_print(f'   ✅ Main environment (v{main_version_to_preserve}) preserved.')
