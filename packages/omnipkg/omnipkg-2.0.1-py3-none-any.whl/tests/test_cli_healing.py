...
# --- Test Configuration ---
BROKEN_URLLIB3 = '1.25.11'  # The ONLY version that breaks httpie (no SKIP_HEADER)
...
def run_command(command_list, check=True, capture=False, stream=True):
    ...
    if capture:
        ...
        if check and result.returncode != 0:
            ...
            raise RuntimeError(_('Command failed with exit code {}').format(result.returncode))
        return result
...
def show_full_omnipkg_run():
    ...
    if result.returncode == 0:
        ...
        safe_print(_('   • Main environment UNCHANGED - bubbles used as first-class citizens!'))
        return True
    else:
        safe_print(_('\n❌ Exit code: {}').format(result.returncode))
        return False
...
def run_demo():
    ...
    try:
        ...
        safe_print(_(f'We\'ll break httpie with urllib3 {BROKEN_URLLIB3}, then watch 8pkg run heal it!'))
        ...
        if original_urllib3_version:
            ...
            safe_print(_(f'⚠️  urllib3 not detected, will install v{FALLBACK_URLLIB3} as baseline'))
        ...
        safe_print(_(f'💥 Installing urllib3=={BROKEN_URLLIB3} with latest-active strategy...'))
        ...
        direct_works = show_raw_http_failure()
        time.sleep(3)
        
        print_header('STEP 5: omnipkg run to the rescue! 🦸‍♂️')
        safe_print(_('🚀 Now let\'s see the FULL 8pkg run healing process...'))
        safe_print(_('   Watch it detect the error, heal it, and show performance stats!'))
        time.sleep(2)
        
        omnipkg_works = show_full_omnipkg_run()
        time.sleep(3)
        ...
        safe_print(_(f'🔄 Restoring your original urllib3 version: {original_urllib3_version}'))
        ...
        safe_print(_(f'   💀 Step 3: Broke httpie with urllib3 {BROKEN_URLLIB3} in main env'))
        safe_print(_('   ❌ Step 4: http command failed - RAW ImportError shown'))
        safe_print(_('   🦸‍♂️ Step 5: 8pkg run detected, healed, and executed successfully'))
        safe_print(_(f'   🫧 Magic: Broken {BROKEN_URLLIB3} stays in main, bubble used for execution'))
        safe_print(_(f'   ✅ Step 7: Restored original urllib3 {original_urllib3_version}'))
        safe_print(_('\n💡 THE KEY INSIGHT:'))
        ...
        if config_manager and original_urllib3_version and omnipkg_core:
            ...
            try:
                with temporary_install_strategy(omnipkg_core, 'latest-active'):
                    run_command(['8pkg', 'install', f'urllib3=={original_urllib3_version}'], check=False)
            except:
                pass
