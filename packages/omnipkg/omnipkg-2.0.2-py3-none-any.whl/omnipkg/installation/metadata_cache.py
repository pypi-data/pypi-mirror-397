...
class MetadataCache:
    ...
    def check_compatibility(self, package: str, python_version: str, platform: str) -> Optional[Dict]:
        """
        ...
        for result in reversed(info["compat"]["test_results"]):
            if (result.get("python_version") == python_version and 
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.cache_db) as conn:
            ...
            total_hits = conn.execute("SELECT SUM(cache_hit_count) FROM github_metadata_cache").fetchone()[0] or 0
