...
class Messager(SystemComponent):
    ...
    def __init__(self, **kwargs: object) -> None:
        ...
        logger.info("Messager actor created with config: %s", self.config.model_dump_json(indent=2))

    async def post_init(self, pod_manager: "BasePodManager") -> None:
        """
        Inject the pod manager handle after construction.

        Args:
            pod_manager (BasePodManager): Pod manager used to deliver inter-agent messages.
        """
        ...
        while not self.message_queue.empty():
            ...
            if message.kind in (MessageKind.FROM_AGENT_TO_AGENT, MessageKind.FROM_USER_TO_AGENT):
                ...
                if not self._pod_manager:
                    ...
                    delivery_tasks = [
                        self._pod_manager.deliver_message.remote(recipient, message) for recipient in recipients
                    ]
                    if delivery_tasks:
