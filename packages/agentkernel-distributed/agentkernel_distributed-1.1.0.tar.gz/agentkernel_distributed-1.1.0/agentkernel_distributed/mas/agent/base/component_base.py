"""Base component class for agent components backed by a single plugin and base plugin class for agent plugins."""
...
class AgentComponent(Generic[PluginType], ABC):
    ...
    async def post_init(self) -> None:
        """
        Run init method on the plugin after dependencies are injected.

        Raises:
            RuntimeError: When init method is called before a plugin is set.
        """
        if not self._plugin:
