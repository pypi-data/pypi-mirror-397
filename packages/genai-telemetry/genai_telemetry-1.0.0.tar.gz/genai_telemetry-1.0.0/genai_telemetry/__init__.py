#!/usr/bin/env python3
"""
GenAI Telemetry - Universal Observability SDK for LLM Applications
===================================================================
...
class ConsoleExporter(BaseExporter):
    ...
    def export(self, span_data: dict) -> bool:
        ...
        tokens = span_data.get('input_tokens', 0) + span_data.get('output_tokens', 0)
        ...
        if self.colored:
            ...
            print(f"{color}[{span_type:12}]{reset} {name:30} | {duration:>8.1f}ms | {status_color}{status:5}{reset} | {model} | {tokens} tokens")
        else:
            print(f"[{span_type:12}] {name:30} | {duration:>8.1f}ms | {status:5} | {model} | {tokens} tokens")
