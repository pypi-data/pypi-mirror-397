...
class PreviewEngine:
    ...
    def _compile_stages(self, force_evaluation: bool = False) -> None:
        ...
        if pre_bronze_expr_groups:
            ...
            # Append _metadata field to ALL preBronze stages to preserve it through the pipeline.
            # _metadata is generated by Databricks Autoloader and contains file metadata
            # (file_path, file_name, file_size, file_modification_time, etc.).
            # Without explicitly selecting it in each stage, subsequent stages would drop it.
            for table in tables:
                table["fields"].append({"name": "_metadata", "from": "_metadata"})
            for table in tables:
                self._pre_bronze = [
