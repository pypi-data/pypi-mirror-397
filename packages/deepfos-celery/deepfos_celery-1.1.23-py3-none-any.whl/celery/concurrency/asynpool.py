...
if hasattr(select, 'poll'):
    def _select_imp(readers=None, writers=None, err=None, timeout=0,
        ...
        if readers:
            fd_to_mask = {_ensure_fd(fd): POLLIN for fd in readers}
        else:
            fd_to_mask = {}
        if writers:
            for fd in map(_ensure_fd, writers):
                fd_to_mask[fd] = fd_to_mask.get(fd, 0) | POLLOUT
        if err:
            for fd in map(_ensure_fd, err):
...
class AsynPool(_pool.Pool):
    ...
    def _create_worker_process(self, i):
        ...
        self._worker_status[w.pid] = w._target.status  # noqa
        try:
