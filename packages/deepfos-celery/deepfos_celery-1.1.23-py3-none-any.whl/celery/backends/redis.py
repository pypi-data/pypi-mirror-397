...
class SentinelBackend(RedisBackend):
    ...
    def _get_sentinel_instance(self, **params):
        ...
        if connparams.get("ssl_cert_reqs") == ssl.CERT_NONE:
            connparams["ssl_cert_reqs"] = 'none'
...
class ClusterBackend(RedisBackend):
    ...
    def client(self) -> RedisCluster:
        connparams = self.connparams.copy()
        hosts = connparams.pop('hosts')
        startup_nodes = [
            cluster.ClusterNode(conf['host'], conf['port'])
            for conf in hosts
        ]
        return RedisCluster(**connparams, startup_nodes=startup_nodes)
