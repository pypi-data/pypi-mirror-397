...
@click.option('--channel-health-check-interval',
              type=int,
              cls=CeleryOption,
              callback=lambda ctx, _, value: value or ctx.obj.app.conf.worker_channel_health_check_interval,
              help_group="Worker Options",
              help="Interval in seconds between channel health check,"
                   "Zeor or negative will disable health check.")
@click.option('-c',
              '--concurrency',
              type=int,
              metavar="<concurrency>",
