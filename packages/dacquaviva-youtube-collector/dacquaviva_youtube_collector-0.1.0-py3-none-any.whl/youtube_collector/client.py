...
class YouTubeClient:
    ...
    def fetch_balanced_dataset(
        ...
        Fetches videos using multiple sort orders to get a mix of
        high and low performers, then assigns labels 1-5.
        ...
        Example:
            >>> client = YouTubeClient()
            >>> dataset = client.fetch_balanced_dataset(days_ago=30)
            >>> # Check label distribution
            >>> from collections import Counter
            >>> print(Counter(v['label'] for v in dataset))
        """
        logger.info("Fetching balanced dataset for ML training")

        # Fetch using different sort orders for variety
        all_videos = []
        per_order = videos_per_category // 3

        for order in ["viewCount", "date", "rating"]:
            videos = self.fetch_videos_by_publish_date(
                days_ago=days_ago,
                max_results_per_category=per_order,
        ...
        logger.info(
            f"After dedup and filtering: {len(filtered_videos)} videos "
            f"(removed {len(unique_videos) - len(filtered_videos)} "
            f"with <{min_subscribers} subscribers)"
        )
    ...
    def fetch_videos_by_publish_date(
        ...
        logger.info(
            ...
            f"across {len(categories)} categories (order={order})"
        )
        ...
        logger.info(f"Successfully fetched {len(all_results)} videos")
        return all_results
