...
class SiblingsByPerson(Members):
    ...
    def table_as_summary(self, ar):
        # For every child, we want to also display their human links.
        if ar.master_household is None:
            return tostring(E.p(ar.no_data_text))
    ...
    def find_links(self, ar, child, parent):
        ...
        for lnk in rt.models.humanlinks.Link.objects.filter(child=child):
            ...
            tt = str(lnk.type.as_child(lnk.child))
