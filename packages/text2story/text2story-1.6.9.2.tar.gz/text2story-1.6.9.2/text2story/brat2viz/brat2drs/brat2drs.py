...
def create_temporal_rules():
    """linguistic rules about time/aspect"""
    return [
        # Aspect -> temporal property
        "all x.(Tense(x,Past) & Aspect(x,Perfective) -> completed(x))",
        "all x.(Tense(x,Past) & Aspect(x,Imperfective) -> ongoing_past(x))",
        "all x.(Tense(x,Present) & Aspect(x,Progressive) -> ongoing_present(x))",
        "all x.(Tense(x,Future) -> not_yet(x))",

        # temporal order based on properties
        "all x y.(completed(x) & ongoing_present(y) -> occursBefore(x,y))",
        "all x y.(not_yet(x) & completed(y) -> occursBefore(y,x))",
        "all x y.(ongoing_past(x) & completed(y) & same_narrative(x,y) -> overlaps(x,y))",
    ]


def enrich_with_temporal_inference(dexpr_list):
    """add infered tlinks to the DRS"""
    prover = TableauProver()

    # 1. Extrair eventos do DRS
    events = extract_events(dexpr_list)

    # 2. linguistic rules
    rules = create_temporal_rules()

    # 3. for each pair of events, try to infer temporal relation
    inferred_tlinks = []
    for e1 in events:
        for e2 in events:
            if e1 == e2: continue

            # try to prove: e1 before e2?
            premises = [e1['drs'], e2['drs']] + [dexpr(r) for r in rules]
            conclusion = dexpr(f'occursBefore({e1["var"]},{e2["var"]})')

            if prover.prove(conclusion, premises):
                inferred_tlinks.append({
                    'event1': e1['var'],
                    'event2': e2['var'],
                    'relation': 'before',
                    'source': 'inferred_from_tense_aspect'
                })
