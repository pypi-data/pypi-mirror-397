"""
TACACS+ accounting filter endpoint module.

This module provides access to the log.tacacs+accounting/filter endpoint
for configuring which TACACS+ accounting events to log.

API Path: log.tacacs+accounting/filter
"""
...
class TacacsAccountingFilter:
    """
    Interface for configuring TACACS+ accounting filter settings.

    This class provides methods to manage TACACS+ accounting filter configuration,
    ...
    Example usage:
        ...
        fgt.api.cmdb.log.tacacs_accounting.filter.update(
            login_audit='enable',
    ...
    def __init__(self, client: "HTTPClient") -> None:
        """
        Initialize the TacacsAccountingFilter instance.

        Args:
            client: The HTTP client used to communicate with the FortiOS device
        """
        self._client = client
        self._endpoint = "log.tacacs+accounting/filter"

    def get(self) -> Dict[str, Any]:
        """
        Retrieve current TACACS+ accounting filter configuration.

        Returns:
            Dictionary containing TACACS+ accounting filter settings

        Example:
            >>> result = fgt.api.cmdb.log.tacacs_accounting.filter.get()
            >>> print(result['login-audit'])
            'enable'
        """
        path = "log.tacacs+accounting/filter"
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        """
        Update TACACS+ accounting filter configuration.
