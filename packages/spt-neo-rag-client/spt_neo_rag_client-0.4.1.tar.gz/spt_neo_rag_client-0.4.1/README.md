...
- 🔍 **Advanced Querying**: Multiple query strategies (hybrid search, knowledge graph, RAPTOR, etc.)
...
async def query_examples():
    ...
    try:
        ...
        # Execute a standard query using QueryRequest
        ...
        if response.result.sources:
            ...
            for source in response.result.sources:
                ...
                # Document metadata is now always included in sources
                if source.document_mime_type:
                    print(f"    MIME Type: {source.document_mime_type}")
                if source.document_page_count:
                    print(f"    Pages: {source.document_page_count}")
                if source.doc_metadata:
                    print(f"    Custom Metadata: {source.doc_metadata}")
                if source.structured_metadata:
                    print(f"    Structured Metadata: {source.structured_metadata}")

        # Stream a query response
        print(f"\nStreaming Query: {query_text}")
        stream_payload = QueryRequest(
            query=query_text,
            knowledge_base_ids=[kb_id],
            credentials=["team-alpha"]
        )
        full_streamed_answer = ""
        async for chunk_data in client.queries.stream_query(stream_payload):
        ...
        # Retrieve sources only (without generating an answer)
        sources_response = await client.queries.retrieve_sources(query_payload)
        print(f"\nRetrieved {len(sources_response.sources)} sources")
...
async def knowledge_graph_examples():
    ...
    try:
        ...
        print(f"Knowledge Graph ID: {kg.id}, Status: {kg.status}")
