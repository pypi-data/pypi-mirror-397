...
from mantarix_web.uploads import build_upload_query_string, get_upload_signature

logger = logging.getLogger(mantarix_fastapi.__name__)


class MantarixUpload:
    """
    Mantarix app uploads handler.
    ...
    * `secret_key` (str, optional) - secret key to sign and verify upload requests.
    ...
    def __init__(
        ...
        env_max_upload_size = os.getenv("MANTARIX_MAX_UPLOAD_SIZE")
        if env_max_upload_size:
            self.__max_upload_size = int(env_max_upload_size)

        self.__secret_key = secret_key

        env_upload_secret_key = os.getenv("MANTARIX_SECRET_KEY")
