...
from mantarix.core.page import Page
from mantarix.core.types import WebRenderer

from mantarix_web.fastapi.mantarix_app import (
    DEFAULT_MANTARIX_OAUTH_STATE_TIMEOUT,
    DEFAULT_MANTARIX_SESSION_TIMEOUT,
    MantarixApp,
)
from mantarix_web.fastapi.mantarix_fastapi import FastAPI
from mantarix_web.fastapi.mantarix_oauth import MantarixOAuth
...
def app(
    ...
    """
    Mount all Mantarix FastAPI handlers in one call.
    ...
    * `oauth_state_timeout_seconds` (int, optional) - OAuth state lifetime, in seconds, which is a maximum allowed time between starting OAuth flow and redirecting to OAuth callback URL.
    ...
    env_upload_dir = os.getenv("MANTARIX_UPLOAD_DIR")
    if env_upload_dir:
        upload_dir = env_upload_dir

    env_websocket_endpoint = os.getenv("MANTARIX_WEBSOCKET_HANDLER_ENDPOINT")
    websocket_endpoint = (
        "ws" if not env_websocket_endpoint else env_websocket_endpoint.strip("/")
    )

    env_upload_endpoint = os.getenv("MANTARIX_UPLOAD_HANDLER_ENDPOINT")
    upload_endpoint = (
        "upload" if not env_upload_endpoint else env_upload_endpoint.strip("/")
    )

    env_oauth_callback_endpoint = os.getenv("MANTARIX_OAUTH_CALLBACK_HANDLER_ENDPOINT")
    ...
    async def oauth_redirect_handler(request: Request):
        return await MantarixOAuth().handle(request)

    fastapi_app.mount(
        path="/",
        app=MantarixStaticFiles(
