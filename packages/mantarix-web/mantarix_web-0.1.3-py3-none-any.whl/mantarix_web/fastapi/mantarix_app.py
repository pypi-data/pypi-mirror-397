...
import mantarix_web.fastapi as mantarix_fastapi
from fastapi import WebSocket, WebSocketDisconnect
from mantarix.core.event import Event
from mantarix.core.local_connection import LocalConnection
from mantarix.core.page import Page, PageDisconnectedException
from mantarix.core.protocol import (
    ClientActions,
    ClientMessage,
    Command,
    CommandEncoder,
...
from mantarix.utils import random_string, sha1
from mantarix_web.fastapi.mantarix_app_manager import app_manager
from mantarix_web.fastapi.oauth_state import OAuthState
from mantarix_web.uploads import build_upload_url

logger = logging.getLogger(mantarix_fastapi.__name__)

DEFAULT_MANTARIX_SESSION_TIMEOUT = 3600
DEFAULT_MANTARIX_OAUTH_STATE_TIMEOUT = 600


class MantarixApp(LocalConnection):
    def __init__(
        self,
        loop: asyncio.AbstractEventLoop,
        session_handler,
        session_timeout_seconds: int = DEFAULT_MANTARIX_SESSION_TIMEOUT,
        oauth_state_timeout_seconds: int = DEFAULT_MANTARIX_OAUTH_STATE_TIMEOUT,
        upload_endpoint_path: Optional[str] = None,
        secret_key: Optional[str] = None,
    ):
        """
        Handle Mantarix app WebSocket connections.
        ...
        * `secret_key` (str, optional) - secret key to sign upload requests.
        ...
        logger.info(f"New MantarixApp: {self.__id}")
        ...
        env_session_timeout_seconds = os.getenv("MANTARIX_SESSION_TIMEOUT")
        if env_session_timeout_seconds:
            self.__session_timeout_seconds = int(env_session_timeout_seconds)

        env_oauth_state_timeout_seconds = os.getenv("MANTARIX_OAUTH_STATE_TIMEOUT")
    ...
    def dispose(self):
        logger.info(f"Disposing MantarixApp: {self.__id}")
        self.__page = None
