...
from mantarix.utils import Once, get_bool_env_var
...
class MantarixStaticFiles(StaticFiles):
    """
    Serve Mantarix app static files.

    Parameters:

    * `app_mount_path` (str) - absolute URL of Mantarix app. Default is `/`.
    ...
    * `websocket_endpoint_path` (str, optional) - absolute URL of Mantarix app WebSocket handler. Default is `{app_mount_path}/ws`.
    """
    ...
    def __init__(
        ...
        env_web_renderer = os.getenv("MANTARIX_WEB_RENDERER")
        if env_web_renderer:
            self.__web_renderer = WebRenderer(env_web_renderer)

        env_use_color_emoji = get_bool_env_var("MANTARIX_WEB_USE_COLOR_EMOJI")
        if env_use_color_emoji is not None:
            self.__use_color_emoji = env_use_color_emoji

        env_route_url_strategy = os.getenv("MANTARIX_WEB_ROUTE_URL_STRATEGY")
