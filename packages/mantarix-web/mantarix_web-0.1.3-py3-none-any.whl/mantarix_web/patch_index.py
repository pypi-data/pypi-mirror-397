...
def patch_index_html(
    ...
    index = index.replace("%MANTARIX_WEB_PYODIDE%", str(pyodide).lower())
    index = index.replace(
        "<!-- webRenderer -->",
    ...
    index = index.replace("%MANTARIX_ROUTE_URL_STRATEGY%", route_url_strategy)
    ...
    if websocket_endpoint_path:
        index = re.sub(
            r"\<meta name=\"mantarix-websocket-endpoint-path\" content=\"(.+)\">",
            r'<meta name="mantarix-websocket-endpoint-path" content="{}">'.format(
