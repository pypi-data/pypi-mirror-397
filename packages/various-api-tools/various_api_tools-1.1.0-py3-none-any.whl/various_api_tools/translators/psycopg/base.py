...
class BasePsycopgTranslator:
    ...
    def _translate_user_raise(self, code_msg: str, error_msg: str) -> str:
        """Handle custom exceptions raised via PostgreSQL's RAISE EXCEPTION.

        Searches the error message for known keys (e.g., field names) and maps them
        to user-friendly messages using `user_map`.
        Used for application-level validations.

        Args:
            code_msg: Base message from code_map for this error type.
            error_msg: Full PostgreSQL error text from RAISE.

        Returns:
            Formatted message with a friendly description if a matching key is found;
            otherwise returns the base code_msg.

        Example:
            For 'Invalid value for field: status' and user_map={"status": "Статус"},
            returns: "Невалидная запись в БД: Статус"

        """
        msg = code_msg

        pattern = re.compile(USER_RAISE_PATTERN)
        matched = pattern.search(error_msg)
        if matched is not None:
            raise_part = matched.group(1)
            description = UNKNOWN_USER_RAISE_DESCRIPTION

            for key in self.user_map:
                if key in raise_part:
                    description = self.user_map.get(key)
                    break

            if raise_part and description:
                msg = f"{code_msg}: {description}"
