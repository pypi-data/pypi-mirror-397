...
from .qdf import QDF, from_polars, Expr
...
class DataLoader:
    ...
    def __init__(self, name: str):
        ...
        self._df: pl.LazyFrame | pl.DataFrame = {}
        self._db: QDF = None
    ...
    def to_qdf(self):
        self._db = from_polars(self._df, self._index, align=True)
...
class Zoo:
    ...
    def label(self, ds_list: list[Dataset]) -> Zoo:
        if ds_list:
            ...
            if self._factor_dls._db is None:
                self._factor_dls.to_qdf()
            self._factor_dls._db.data = self._factor_dls._db.data.join(self._label_dls.data, on=["date", "time", "asset"], how="left")
        return self
    ...
    def filter(self, ds_list: list[Dataset] = None) -> Zoo:
        if ds_list:
            ...
            if self._factor_dls.data is not None:
                ...
                self._factor_dls._db = from_polars(self._factors, index, align=True)
            if self._mount_dls.data is not None:
                self._mounted = target_index.join(self._mount_dls.data, on=index, how="left")
        else:
            self._filter_dls = DataLoader("filter",)
            self._factor_dls._db = None
            self._factors = self._factor_dls.data
            self._mounted = self._mount_dls.data
        return self

    def to_alphalens(self,
                      expr: str,
                      bins: int = 10,
        ...
        if self._factor_dls._db is None:
            self._factor_dls.to_qdf()
        factor_data = self._factor_dls.sql(expr,).rename({alias: "factor"})
        dfList = [factor_data, self.factors.select(*index, *self.periods)]
        if by_group:
            grouper.append("group")
            dfList.append(self.mounted.select(*index, "group"))
            # factor_data = factor_data.join(self.mounted.select(*index, "group"), on=index, how="left")
        factor_data = pl.concat(dfList, how="align").sort(index)
        return (
            factor_data
            .with_columns(
                pl.col("factor")
            ...
            .select(*index, "factor", "group", *self.periods, "factor_quantile")
        )
