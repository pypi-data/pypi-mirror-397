...
class Zoo:
    ...
    def __init__(self):
        ...
        self._label_dls = DataLoader("label", align=False)
        ...
        self._labels: pl.DataFrame = None
    ...
    def filter(self, ds_list: list[Dataset] = None) -> Zoo:
        if ds_list:
            ...
            self._labels = target_index.join(self._label_dls.data, on=index, how="left")
    ...
    def to_alphalens(self,
        ...
        grouper = ["date", "time"]
        index = ("date", "time", "asset")
        periods = set(self.labels.columns).difference(set(index))
        periods = list(periods)
        factor_data = self.factors
        if by_group:
            grouper.append("group")
            factor_data = factor_data.join(self.mounted.select(*index, "group"), on=index, how="left")
        factor_data = factor_data.select(*index, pl.col(alpha_name).alias("factor"), *periods, "group")
        return (
            factor_data
            .with_columns(
                pl.col("factor")
            ...
            .select(*index, "factor", "group", *periods, "factor_quantile")
        )
