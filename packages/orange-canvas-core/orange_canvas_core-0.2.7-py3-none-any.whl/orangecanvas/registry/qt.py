...
def tooltip_helper(desc):
    ...
    if desc.inputs:
        ...
        tooltip.append(_tr.m[458, "Inputs:<ul>{0}</ul>"].format(inputs))
    else:
        tooltip.append(_tr.m[459, "No inputs"])

    if desc.outputs:
        outputs = "".join(inputs_fmt.format(name=out.name,
                                            class_name=type_str(out.types))
                          for out in desc.outputs)
        tooltip.append(_tr.m[460, "Outputs:<ul>{0}</ul>"].format(outputs))
    else:
        tooltip.append(_tr.m[461, "No outputs"])
