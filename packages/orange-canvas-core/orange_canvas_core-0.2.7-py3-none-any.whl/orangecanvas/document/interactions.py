...
class NewLinkAction(UserInteraction):
    ...
    def mousePressEvent(self, event):
        ...
        if anchor_item is not None and event.button() == Qt.LeftButton:
            ...
            event.accept()
            self.postQuickTip(
                self.tr((_tr.m[355, '<h3>Create new link</h3>'] + (_tr.m[356, '<p>Drag a link to an existing node or release on '] + (_tr.m[357, 'an empty spot to create a new node.</p>'] + (_tr.m[358, '<p>Hold Shift when releasing the mouse button to '] + _tr.m[359, 'edit connections.</p>']))))
...
def edit_links(
    ...
    dlg = EditLinksDialog(parent, windowTitle=_tr.m[360, "Edit Links"])
...
class EditNodeLinksAction(UserInteraction):
    ...
    def edit_links(self, initial_links=None):
        ...
        dlg = EditLinksDialog(self.document, windowTitle=_tr.m[361, "Edit Links"])
        ...
        if rval == EditLinksDialog.Accepted:
            ...
            stack = self.document.undoStack()
            stack.beginMacro(_tr.m[362, "Edit Links"])
...
class NewArrowAnnotation(UserInteraction):
    ...
    def start(self):
        ...
        self.postQuickTip(
            self.tr((_tr.m[363, '<h3>New arrow annotation</h3>'] + _tr.m[364, '<p>Click and drag to create a new arrow annotation</p>'])
...
class NewTextAnnotation(UserInteraction):
    ...
    def start(self):
        ...
        self.document.view().setCursor(Qt.CrossCursor)
        self.postQuickTip(
            self.tr((_tr.m[365, '<h3>New text annotation</h3>'] + (_tr.m[366, '<p>Click (and drag to resize) on the canvas to create '] + (_tr.m[367, 'a new text annotation item.</p>'] + (_tr.m[368, '<p>Right click on the annotation to change how it is '] + _tr.m[369, 'rendered (Markdown, HTML, ...).</p>']))))
...
class ResizeTextAnnotation(UserInteraction):
    ...
    def start(self) -> None:
        super().start()
        self.postQuickTip(
            self.tr((_tr.m[370, '<h3>Edit Text Annotation</h3>'] + (_tr.m[371, '<p>Drag control points to resize the annotation.</p>'] + (_tr.m[372, '<p>Right click on the annotation to change how it is '] + _tr.m[373, 'rendered (Markdown, HTML, ...).</p>']))))
        )
        ...
        if len(self.__accepts) == 1:
            ...
            class Title(QWidgetAction):
                def createWidget(self, parent):
                    return QLabel(_tr.m[374, "<b>Select a widget</b>"], parent, margin=2)
