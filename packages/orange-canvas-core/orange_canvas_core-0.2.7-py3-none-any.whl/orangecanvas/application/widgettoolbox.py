...
class WidgetToolBox(ToolBox):
    ...
    def __init__(self, parent=None):
        ...
        action = QAction(
            load_styled_svg_icon("Search.svg"), self.tr(_tr.m[300, "Search"]), self
        )
        self.__filterEdit = QLineEdit(
            objectName="filter-edit-line",
            placeholderText=self.tr(_tr.m[301, "Filter..."]),
            toolTip=self.tr(_tr.m[302, "Filter/search the list of available widgets."]),
        ...
        open_all = QAction(self.tr(_tr.m[303, "Open all"]), self)
        open_all.triggered.connect(self.openAllTabs)
        close_all = QAction(self.tr(_tr.m[304, "Close all"]), self)
