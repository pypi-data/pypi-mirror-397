...
def _session(cachedir=None):
    ...
    try:
        ...
        log.info(
            (f"Cache file creation/opening failed with: '{str(ex)}'. " + f"Using requests.Session instead of cached session.")
        )
...
class Command(Enum):
    Install = _tr.m[305, "Install"]
    Upgrade = _tr.m[306, "Upgrade"]
    Uninstall = _tr.m[307, "Uninstall"]
        ...
        try:
            if command == Install \
                ...
                self.setStatusMessage(
                    _tr.m[308, "Installing {}"].format(pkg.installable.name))
                if self.conda:
                    try:
                ...
                self.setStatusMessage(
                    _tr.m[309, "Upgrading {}"].format(pkg.installable.name))
                if self.conda:
                    try:
                ...
                self.setStatusMessage(
                    _tr.m[310, "Uninstalling {}"].format(pkg.local.name))
                if self.conda:
                    try:
            ...
            self.error.emit(
                _tr.m[311, "Command failed: python {}"].format(ex.cmd),
