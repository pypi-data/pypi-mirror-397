...
class SchemeInfoEdit(QWidget):
    ...
    def __setupUi(self):
        ...
        self.name_edit.setPlaceholderText(self.tr(_tr.m[222, "untitled"]))
        self.name_edit.setSizePolicy(QSizePolicy.Expanding,
                                     QSizePolicy.Fixed)
        self.desc_edit = QTextEdit(self)
        self.desc_edit.setTabChangesFocus(True)

        layout.addRow(self.tr(_tr.m[223, "Title"]), self.name_edit)
        layout.addRow(self.tr(_tr.m[224, "Description"]), self.desc_edit)
    ...
    def setScheme(self, scheme):
        ...
        self.scheme = scheme
        if not scheme.title:
            self.name_edit.setText(self.tr(_tr.m[225, "untitled"]))
    ...
    def commit(self):
        ...
        if self.__schemeIsUntitled and \
            self.name_edit.text() == self.tr(_tr.m[226, "untitled"]):
...
class SchemeInfoDialog(QDialog):
    ...
    def __setupUi(self):
        ...
        heading = self.tr(_tr.m[227, "Workflow Info"])
        ...
        self.__showAtNewSchemeCheck = \
            QCheckBox(self.tr(_tr.m[228, "Show when I make a New Workflow."]),
                      self,
        ...
        check_layout.addWidget(
               QLabel(self.tr((_tr.m[229, "You can also edit Workflow Info later "] + _tr.m[230, "(File -> Workflow Info)."])),
                      self,
