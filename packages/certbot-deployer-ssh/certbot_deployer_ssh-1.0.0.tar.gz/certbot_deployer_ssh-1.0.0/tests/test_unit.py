...
def test_argparse_post_required_args_are_actually_required() -> None:
    """
...
@pytest.mark.parametrize(
    "require_remote_dir,remote_dir,raises",
    [
        (True, "", argparse.ArgumentTypeError),
        (False, "", None),
        (True, "/any/path", None),
    ],
    ids=[
        "(raise if remote dir is required but not provided)",
        "(If no remote dir required, allow an empty value)",
        "(If remote dir is required and one is provided, succeed)",
    ],
)
def test_argparse_post_remote_dir_required(
    require_remote_dir: bool,
    remote_dir: str,
    raises: Optional[Exception],
) -> None:
    """
    Verify that remote dir is required when it should be
    """
    args: argparse.Namespace = argparse.Namespace(
        host="anyhost",
        require_remote_dir=require_remote_dir,
        remote_dir=remote_dir,
    )
    if raises is not None:
        with pytest.raises(raises):  # type:ignore
            SshDeployer.argparse_post(
                args=args,
            )
    else:
        SshDeployer.argparse_post(
