...
class SshDeployer(Deployer):
    ...
    def register_args(*, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "path if it does not exist. If `--require-remote-dir` is disabled, "
                ", it will be up to the server - that usually ends up being the "
        ...
        parser.add_argument(
            ...
            default=True,
            help=(
                "If set, a remote path will be required. This will avoid dumping "
                "the certificate bundle into wherever the user lands on the remote "
                "host."
            ),
            type=bool,
        )
    ...
    def argparse_post(*, args: argparse.Namespace) -> None:
        ...
        if (
            # Checking this way to make tests have to manage fewer details
            hasattr(args, "require_remote_dir")
            and args.require_remote_dir
            and not args.remote_dir
        ):
            raise argparse.ArgumentTypeError(
                "Since `require_remote_dir` is enabled, `remote_dir` must have a value"
            )
