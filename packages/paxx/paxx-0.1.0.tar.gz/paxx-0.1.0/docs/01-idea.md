...
**3. Feature discovery** (optional) - For auto-registering routers:
```python
# Generated: core/features.py
def discover_routers() -> list[APIRouter]:
    routers = []
    for feature_dir in Path("features").iterdir():
        if (feature_dir / "routes.py").exists():
            module = importlib.import_module(f"features.{feature_dir.name}.routes")
            if hasattr(module, "router"):
                routers.featureend((feature_dir.name, module.router))
    return routers
```
