...
These tests verify that the generated main.py, exceptions.py, and middleware.py templates:
...
def template_env():
    ...
    template_dir = (
        Path(__file__).parent.parent / "src" / "paxx" / "templates" / "project"
    )
...
class TestMainTemplateRendering:
    ...
    def test_main_includes_health_check(self, rendered_main):
        """Rendered main.py should include health check endpoint."""
        assert "@app.get(" in rendered_main
        assert '"/health"' in rendered_main
        assert "health_check" in rendered_main
