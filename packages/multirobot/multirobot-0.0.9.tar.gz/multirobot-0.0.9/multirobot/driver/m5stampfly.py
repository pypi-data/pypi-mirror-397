...
class M5StampFly(Drone):
    def __init__(self, name, uri='/dev/serial/by-name/m5stamp-forwarder', BAUD=115200, rate=50, odometry_source="mocap", verbose=False,**kwargs):
        ...
        while True:
            if self.orientation is None or self.position is None or self.velocity is None or self.target_position is None or self.target_velocity is None:
                ...
                if tick % 100 == 0:
                    ...
                    print(f"relative position: {clipped_relative_position[0]:.2f} {clipped_relative_position[1]:.2f} {clipped_relative_position[2]:.2f} velocity: {clipped_relative_velocity[0]:.2f} {clipped_relative_velocity[1]:.2f} {clipped_relative_velocity[2]:.2f}")
                    # print(f"orientation: {self.orientation[0]:.2f} {self.orientation[1]:.2f} {self.orientation[2]:.2f} {self.orientation[3]:.2f}")
                data_to_send = f"{clipped_relative_position[0]:0.3f},{clipped_relative_position[1]:0.3f},{clipped_relative_position[2]:0.3f},{angle_transmission:0.3f},{clipped_relative_velocity[0]:0.3f},{clipped_relative_velocity[1]:0.3f},{clipped_relative_velocity[2]:0.3f},{int(armed)}\n"
