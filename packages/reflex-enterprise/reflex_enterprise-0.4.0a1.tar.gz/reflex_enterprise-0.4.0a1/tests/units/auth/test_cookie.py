...
async def test_get_set_cookie_substates():
    ...
    async with app.state_manager.modify_state(
        ...
        assert state_instance._parent_cookie == "parent_value"
        state_instance._parent_cookie = "new_parent_value"
        assert state_instance._parent_cookie == "new_parent_value"
        ...
        assert (
            state_instance._parent_cookie == "new_parent_value_in_grandchild_instance"
        )
...
@pytest.mark.asyncio
async def test_sync_cookies_cross_origin():
    """Test that sync_cookies raises an error for cross-origin requests."""

    request = Mock(spec=Request)
    request.base_url = urlsplit("https://my-backend.com")
    request.headers = Headers(
        {
            "X-Reflex-Client-Token": str(uuid.uuid4()),
    ...
    assert response.body == b"Cross-origin cookie sync is not allowed"
