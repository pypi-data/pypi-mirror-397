...
class AdsReq(BaseModel):
    tokenId: str = Field(serialization_alias="coin_id")
    currencyId: str = Field(serialization_alias="cur_id")
    side: Literal["0", "1"] = Field(serialization_alias="is_sell")  # 0 покупка, # 1 продажа
    payment: list[str] = Field([], serialization_alias="pm_ids")  # int
    size: str = Field("20", serialization_alias="limit")  # int
    page: str = "1"  # int
    amount: str = ""  # float
    vaMaker: bool = Field(False, serialization_alias="vm_only")
    canTrade: bool = False
    userId: str = ""  # int
