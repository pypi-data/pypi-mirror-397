...
class BackupTarget(ExternalizableAbstractModel):
    ...
    :param location: (:attr:`location`) The location of the backup target. For example, a cluster or an object store endpoint such as AWS s3. 
    :type location: 
    
    
    :param last_sync_time: (:attr:`last_sync_time`) The time when the domain manager last synchronised or copied its configuration data to the backup target. This field refreshes every 30 minutes. 
    :type last_sync_time: 
    
    
    :param is_backup_paused: (:attr:`is_backup_paused`) Whether or not the backup is paused on the specified cluster. 
    :type is_backup_paused: 
    
    
    :param backup_pause_reason: (:attr:`backup_pause_reason`) Specifies a reason why the backup might have paused. This is empty if the isBackupPaused field is false. 
