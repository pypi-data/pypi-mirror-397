...
```json
{
  "mcpServers": {
    "sqlalchemy-mcp": {
      "command": "uvx",
      "args": [
        "--from", "sqlalchemy_fastmcp==1.0.9", "sqlalchemy-mcp-server", "stdio"],
      "env": {
        "DB_TYPE": "mysql",
        "DB_HOST": "1.1.1.1",
        "DB_PORT": "3306",
...
#### 方法一：指定版本号（推荐）
```bash
uvx --from sqlalchemy_fastmcp==1.0.9 sqlalchemy-mcp-server stdio
```
...
此命令会：
- 从 Nexus3 仓库安装 sqlalchemy_fastmcp 包（版本 1.0.9）
- 自动处理所有依赖关系
