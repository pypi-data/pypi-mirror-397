...
exports.PromptEditorViewProvider = void 0;
exports.registerPromptEditorCommands = registerPromptEditorCommands;
const vscode = __importStar(require("vscode"));
/**
 * Prompt Editor View Provider
 * Shows in the sidebar during debug sessions
 */
class PromptEditorViewProvider {
    constructor(_extensionUri) {
        this._extensionUri = _extensionUri;
        this._promptVersions = new Map();
        this._comparisonState = { enabled: false };
        this._onPromptChange = new vscode.EventEmitter();
        this.onPromptChange = this._onPromptChange.event;
        this._onRerunRequest = new vscode.EventEmitter();
        this.onRerunRequest = this._onRerunRequest.event;
    }
        ...
        webviewView.webview.onDidReceiveMessage(message => {
            switch (message.type) {
                case 'rerun':
                    ...
                    break;
                case 'saveVersion':
                    this._savePromptVersion(message.stageId, message.systemPrompt, message.userPrompt);
                    break;
                case 'loadVersion':
     ...
     */
    updateOutput(stageId, output, tokens) {
     ...
     */
    streamOutput(stageId, chunk, tokenIndex) {
        if (this._view && this._currentPrompt?.stageId === stageId) {
            this._view.webview.postMessage({
                type: 'streamChunk',
     ...
     */
    streamEnd(stageId, totalTokens, finalContent) {
        if (this._view && this._currentPrompt?.stageId === stageId) {
            this._view.webview.postMessage({
                type: 'streamEnd',
     ...
     */
    _savePromptVersion(stageId, systemPrompt, userPrompt, output, tokens, durationMs) {
     ...
     */
    _selectForComparison(stageId, versionId, side) {
        const versions = this._promptVersions.get(stageId) || [];
     ...
     */
    _loadPromptVersion(stageId, versionId) {
        const versions = this._promptVersions.get(stageId) || [];
        const version = versions.find(v => v.id === versionId);
...
exports.PromptEditorViewProvider = PromptEditorViewProvider;
PromptEditorViewProvider.viewType = 'flowmason.promptEditor';
/**
 * Register prompt editor commands
 */
function registerPromptEditorCommands(context, promptEditorProvider) {
    // Command to show prompt editor
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.showPromptEditor', () => {
        vscode.commands.executeCommand('flowmason.promptEditor.focus');
    }));
    // Command to edit current stage prompt
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.editStagePrompt', (stageId) => {
        // This would be called from the debug session when paused at an LLM node
        vscode.commands.executeCommand('flowmason.promptEditor.focus');
    }));
}
//# sourceMappingURL=promptEditorView.js.map