...
function registerMCPCommands(context, mcpService, outputChannel) {
    // Command: Create Pipeline with AI Assistance
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.mcp.createPipeline', async () => {
        await createPipelineWithAI(mcpService, outputChannel);
    }));
    // Command: Get Pipeline Suggestions
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.mcp.suggestPipeline', async () => {
        await suggestPipeline(mcpService, outputChannel);
    }));
    // Command: Generate Stage
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.mcp.generateStage', async () => {
        await generateStage(mcpService, outputChannel);
    }));
    // Command: Validate Current Pipeline
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.mcp.validatePipeline', async () => {
        await validateCurrentPipeline(mcpService, outputChannel);
    }));
    // Command: Add AI-Generated Stage to Current Pipeline
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.mcp.addGeneratedStage', async () => {
 ...
 */
async function createPipelineWithAI(mcpService, outputChannel) {
    ...
    // Create the pipeline
    const filePath = await mcpService.createPipeline(pipelineName, taskDescription, stages);
    if (filePath) {
 ...
 */
async function reviewSuggestion(suggestion, mcpService, outputChannel) {
    const stages = [];
    // Show stages for review
    const stageItems = suggestion.stages.map((s, i) => ({
    ...
    for (const item of selectedStages) {
        ...
        const generated = await vscode.window.withProgress({
            ...
            return mcpService.generateStage(item.stage.component, item.stage.purpose, inputSource);
        });
        if (generated) {
    ...
    if (reviewChoice.action === 'customize') {
        ...
        const proceed = await vscode.window.showInformationMessage('Edit the stages in the editor, then click "Continue" when done.', 'Continue', 'Cancel');
        if (proceed === 'Continue') {
 ...
 */
async function suggestPipeline(mcpService, outputChannel) {
    const taskDescription = await vscode.window.showInputBox({
    ...
    const action = await vscode.window.showInformationMessage(`Suggested ${suggestion.stages.length} stages for your pipeline.`, 'Create Pipeline', 'View Details', 'Dismiss');
    if (action === 'Create Pipeline') {
        ...
        for (const stage of suggestion.stages) {
            const inputSource = previousStageId
                ? `stages.${previousStageId}.output`
                : 'input';
            const generated = await mcpService.generateStage(stage.component, stage.purpose, inputSource);
            if (generated) {
                stages.push(generated);
                previousStageId = generated.id;
            }
 ...
 */
async function generateStage(mcpService, outputChannel) {
    ...
    vscode.window.showInformationMessage(`Generated ${componentType} stage: ${stage.id}`, 'Copy to Clipboard').then(action => {
 ...
 */
async function validateCurrentPipeline(mcpService, outputChannel) {
 ...
 */
async function addGeneratedStageToPipeline(mcpService, outputChannel) {
    const editor = vscode.window.activeTextEditor;
    if (!editor || !editor.document.fileName.endsWith('.pipeline.json')) {
