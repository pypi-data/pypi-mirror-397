...
exports.registerNewProjectCommand = registerNewProjectCommand;
 ...
 */
function getProjectManifest(options) {
 ...
 */
function getExampleNodeContent(projectName) {
 ...
 */
function getExampleOperatorContent(projectName) {
    const className = projectName
        .split('-')
        .map(part => part.charAt(0).toUpperCase() + part.slice(1))
        .join('') + 'Operator';
 ...
 */
function getExamplePipelineContent(projectName) {
    return JSON.stringify({
...
function registerNewProjectCommand(context) {
    const command = vscode.commands.registerCommand('flowmason.newProject', async () => {
        try {
            ...
            if (fs.existsSync(projectPath)) {
                const overwrite = await vscode.window.showWarningMessage(`Folder "${name}" already exists. Do you want to continue and potentially overwrite files?`, { modal: true }, 'Continue', 'Cancel');
            ...
            await vscode.window.withProgress({
                ...
                fs.writeFileSync(path.join(projectPath, 'flowmason.json'), getProjectManifest(options));
                progress.report({ increment: 20, message: 'Creating configuration files...' });
                // Create .gitignore
                fs.writeFileSync(path.join(projectPath, '.gitignore'), getGitignoreContent());
                // Create README.md
                fs.writeFileSync(path.join(projectPath, 'README.md'), getReadmeContent(options));
                // Create requirements.txt
                fs.writeFileSync(path.join(projectPath, 'requirements.txt'), getRequirementsContent());
                progress.report({ increment: 20, message: 'Creating example files...' });
                // Create __init__.py for components
                fs.writeFileSync(path.join(componentsPath, '__init__.py'), '"""FlowMason components for this project."""\n');
                // Create example files if requested
                if (options.includeExampleNode) {
                    fs.writeFileSync(path.join(componentsPath, 'example_node.py'), getExampleNodeContent(name));
                    fs.writeFileSync(path.join(componentsPath, 'example_operator.py'), getExampleOperatorContent(name));
                }
                if (options.includeExamplePipeline) {
                    fs.writeFileSync(path.join(pipelinesPath, 'main.pipeline.json'), getExamplePipelineContent(name));
                }
                progress.report({ increment: 40, message: 'Done!' });
            });
            // Ask if user wants to open the project
            const openProject = await vscode.window.showInformationMessage(`FlowMason project "${name}" created successfully!`, 'Open Project', 'Open in New Window', 'Close');
