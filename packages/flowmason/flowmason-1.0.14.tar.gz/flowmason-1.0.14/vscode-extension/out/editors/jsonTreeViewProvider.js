...
exports.JsonTreeViewProvider = void 0;
exports.registerJsonTreeViewProvider = registerJsonTreeViewProvider;
...
class JsonTreeViewProvider {
    ...
    getHtmlForWebview(webview, data, parseError, fileName) {
...
<body>
    ...
    renderTreeNode(data, key, depth, expanded) {
        ...
        if (typeof data === 'string') {
            ...
            return this.renderLeafNode(key, `<span class="tree-value-string">"${this.escapeHtml(displayValue)}"</span>`, depth, data);
        }
        if (typeof data === 'number') {
            return this.renderLeafNode(key, `<span class="tree-value-number">${data}</span>`, depth, String(data));
        }
        if (typeof data === 'boolean') {
            return this.renderLeafNode(key, `<span class="tree-value-boolean">${data}</span>`, depth, String(data));
        }
        ...
        if (typeof data === 'object') {
            ...
            let html = `<div class="tree-node">
                ...
                <div class="tree-children ${childrenClass}">`;
            keys.forEach(k => {
                html += this.renderTreeNode(data[k], k, depth + 1, depth < 1);
            });
            html += `</div>
                <div class="tree-row">
                    ${indent}<span class="tree-indent"></span>
    ...
    renderLeafNode(key, valueHtml, depth, rawValue) {
        const indent = '<span class="tree-indent"></span>'.repeat(depth);
...
JsonTreeViewProvider.viewType = 'flowmason.jsonTreeView';
/**
 * Helper function to register the JSON tree view provider
 */
function registerJsonTreeViewProvider() {
    const provider = new JsonTreeViewProvider();
    return vscode.window.registerCustomEditorProvider(JsonTreeViewProvider.viewType, provider, {
        webviewOptions: {
...
//# sourceMappingURL=jsonTreeViewProvider.js.map