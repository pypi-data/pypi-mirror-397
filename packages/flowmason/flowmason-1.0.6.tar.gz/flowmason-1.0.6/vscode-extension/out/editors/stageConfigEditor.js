...
exports.StageConfigEditor = void 0;
...
class StageConfigEditor {
    ...
    async openEditor(stage, document) {
        ...
        const panel = vscode.window.createWebviewPanel('flowmason.stageConfig', `Configure: ${stage.id}`, vscode.ViewColumn.Two, {
            enableScripts: true,
    ...
    getDataSources(pipeline, currentStageId, currentComponentDetail) {
        ...
        for (const stage of pipeline.stages) {
            ...
            if (!isDownstream) {
                ...
                const outputs = [{ name: 'result', type: 'any', description: 'Stage output' }];
                sources.push({
                    id: stage.id,
                    name: stage.id,
                    type: 'upstream',
    ...
    getHtml(stage, pipeline, dataSources, componentDetail, providers) {
        ...
        const outputSchema = componentDetail?.output_schema;
     ...
     */
    renderIncomingSection(dataSources) {
        const inputSource = dataSources.find(s => s.type === 'input');
     ...
     */
    renderTransformSection(inputSchema, config, dataSources, existingMappings, componentDetail, providers) {
     ...
     */
    renderAIConfigSection(componentDetail, providers, config) {
        ...
        const availableProviders = providers.filter(p => recommendedProviderNames.includes(p.name) || p.configured);
     ...
     */
    renderSchemaField(fieldName, schema, isRequired, currentValue, dataSources, defs) {
     ...
     */
    renderSchemaSelectField(name, label, description, options, required, currentValue, defaultValue) {
     ...
     */
    renderSchemaBooleanField(name, label, description, required, currentValue, defaultValue) {
     ...
     */
    renderSchemaNumberField(name, label, description, required, currentValue, schema) {
     ...
     * Render a text input
     */
    renderSchemaTextField(name, label, description, required, currentValue, schema) {
     ...
     */
    renderSchemaPromptField(name, label, description, required, currentValue, dataSources, schema) {
        // If value is an object, JSON.stringify it to prevent [object Object]
        const rawValue = currentValue ?? '';
        const value = typeof rawValue === 'object' && rawValue !== null
            ? JSON.stringify(rawValue, null, 2)
     ...
     */
    renderSchemaArrayField(name, label, description, required, currentValue, schema, dataSources) {
     ...
     */
    renderSchemaObjectField(name, label, description, required, currentValue) {
     ...
     */
    renderOutgoingSection(stage, pipeline, outputSchema, existingMappings) {
     ...
     */
    renderJsonTransformOutputs(stage, existingMappings) {
        return `
     ...
     */
    renderComponentOutputs(stage, outputFieldNames, outputProperties) {
     ...
     */
    renderOutputDestinations(pipelineDestinations, stageDestinations) {
     ...
     */
    getPlaceholderFromSchema(schema, label) {
        // Try examples first
...
exports.StageConfigEditor = StageConfigEditor;
StageConfigEditor.panels = new Map();
//# sourceMappingURL=stageConfigEditor.js.map