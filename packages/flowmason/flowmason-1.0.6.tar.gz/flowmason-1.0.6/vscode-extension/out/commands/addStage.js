...
exports.registerAddStageCommand = registerAddStageCommand;
 ...
 */
function getCurrentPipelineDocument() {
    ...
    const dagDocument = dagCanvasProvider_1.DagCanvasProvider.currentDocument;
    if (dagDocument && dagDocument.fileName.endsWith('.pipeline.json')) {
        return dagDocument;
    }
    return undefined;
}
function registerAddStageCommand(context, flowmasonService) {
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.addStage', async () => {
        await addStageFlow(flowmasonService);
    }));
    // Also register a command to add stage at a specific position
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.addStageAfter', async (afterStageId) => {
        await addStageFlow(flowmasonService, afterStageId);
    }));
}
async function addStageFlow(flowmasonService, afterStageId) {
    ...
    const componentItems = filteredComponents.map(c => createComponentQuickPickItem(c));
...
function createComponentQuickPickItem(component) {
...
async function completeAddStage(document, pipeline, component, afterStageId) {
    ...
    if (pipeline.stages && pipeline.stages.length > 0) {
        const dependencyItems = pipeline.stages.map(s => ({
            label: s.id,
 ...
 */
function generateDefaultConfig(component) {
    const config = {};
 ...
 */
async function insertStageIntoPipeline(document, pipeline, newStage, afterStageId) {
 ...
 */
function findStageEndPosition(document, stageId) {
    const text = document.getText();
 ...
 */
function findStagesArrayEndPosition(document) {
    const text = document.getText();
    // Find "stages": [
