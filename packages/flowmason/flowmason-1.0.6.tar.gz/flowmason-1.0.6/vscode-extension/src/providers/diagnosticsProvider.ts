...
interface DiagnosticRule {
    pattern: RegExp;
    check: (match: RegExpMatchArray, document: vscode.TextDocument, line: number) => vscode.Diagnostic | null;
}
...
export class FlowMasonDiagnosticsProvider {
    ...
    private isFlowMasonFile(text: string): boolean {
    ...
    private createRules(): DiagnosticRule[] {
        return [
            // Check for missing name in decorator
            {
                ...
                check: (match, document, line) => {
                    ...
                    return createDiagnostic(
                        range,
                        `@${match[1]} decorator requires at least a 'name' parameter`,
            ...
            // Check for non-kebab-case name
            {
                ...
                check: (match, document, line) => {
                    ...
                    if (!/^[a-z][a-z0-9-]*$/.test(name)) {
                        ...
                        return createDiagnostic(
                            range,
                            `Component name '${name}' should be kebab-case (lowercase letters, numbers, hyphens)`,
            ...
            // Check for missing Input class
            {
                ...
                check: (match, document, line) => {
                    ...
                    if (!textBefore.includes('class Input')) {
                        ...
                        return createDiagnostic(
                            range,
                            `Component '${className}' should have an Input class defined`,
                            vscode.DiagnosticSeverity.Warning
                        );
                    }
                    return null;
            ...
            // Check for missing Output class
            {
                ...
                check: (match, document, line) => {
                    ...
                    if (!textBefore.includes('class Output')) {
                        ...
                        return createDiagnostic(
                            range,
                            `Component '${className}' should have an Output class defined`,
                            vscode.DiagnosticSeverity.Warning
                        );
                    }
                    return null;
            ...
            // Check for async execute without await
            {
                ...
                check: (match, document, line) => {
                    ...
                    if (!hasAwait) {
                        ...
                        return createDiagnostic(
                            range,
                            'Async execute method should contain at least one await expression',
            ...
            // Check for missing description in decorator
            {
                ...
                check: (match, document, line) => {
                    ...
                    if (!decoratorText.includes('description')) {
                        ...
                        return createDiagnostic(
                            range,
                            'Consider adding a description to your component',
            ...
            // Check for Field without description
            {
                ...
                check: (match, document, line) => {
                    ...
                    if (!fieldText.includes('description')) {
                        ...
                        return createDiagnostic(
                            range,
                            'Consider adding a description to this field',
                            vscode.DiagnosticSeverity.Hint
                        );
                    }
                    return null;
    ...
    private runDocumentChecks(document: vscode.TextDocument, text: string): vscode.Diagnostic[] {
        const diagnostics: vscode.Diagnostic[] = [];
        ...
        for (const [className, line] of baseClasses) {
            if (!decoratedClasses.has(className)) {
                ...
                diagnostics.push(createDiagnostic(
                    range,
                    `Class '${className}' extends BaseNode/BaseOperator but is missing @node/@operator decorator`,
                    vscode.DiagnosticSeverity.Error
                ));
            }
        }
        ...
        if (executeMatch) {
            ...
            if (paramName !== 'input' && paramName !== 'inputs') {
                ...
                diagnostics.push(createDiagnostic(
                    range,
                    `Execute method parameter should be named 'input' or 'inputs', found '${paramName}'`,
                    vscode.DiagnosticSeverity.Information
                ));
            }
        }
        ...
        if (executeReturnMatch && !executeReturnMatch[1]) {
            ...
            diagnostics.push(createDiagnostic(
                range,
                'Consider adding return type annotation to execute method (-> Output)',
                vscode.DiagnosticSeverity.Hint
            ));
        }
