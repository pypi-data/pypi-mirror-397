...
exports.FlowMasonDebugAdapterDescriptorFactory = exports.FlowMasonDebugConfigurationProvider = void 0;
 ...
 */
class FlowMasonDebugConfigurationProvider {
    /**
    ...
    async resolveDebugConfiguration(folder, config, token) {
        ...
        if (config.inputFile) {
            ...
            if (!fs.existsSync(config.inputFile)) {
                await vscode.window.showWarningMessage(`Input file not found: ${config.inputFile}. Continuing without input.`);
        ...
        if (!config.studioUrl) {
            ...
            config.studioUrl = settings.get('studioUrl') || 'http://localhost:8999';
        }
        return config;
    }
    /**
     ...
     */
    async promptForInputs(pipelinePath) {
        try {
...
exports.FlowMasonDebugConfigurationProvider = FlowMasonDebugConfigurationProvider;
/**
 * Debug Adapter Descriptor Factory
 * Creates the debug adapter instance
 */
class FlowMasonDebugAdapterDescriptorFactory {
...
exports.FlowMasonDebugAdapterDescriptorFactory = FlowMasonDebugAdapterDescriptorFactory;
/**
 * Register debug commands
 */
function registerDebugCommands(context) {
    // Debug Pipeline command
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.debugPipeline', async () => {
        const editor = vscode.window.activeTextEditor;
        const { DagCanvasProvider } = await Promise.resolve().then(() => __importStar(require('../editors/dagCanvasProvider')));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.runPipeline', async () => {
        const editor = vscode.window.activeTextEditor;
        const { DagCanvasProvider } = await Promise.resolve().then(() => __importStar(require('../editors/dagCanvasProvider')));
        // Check text editor first, then DAG canvas
        let pipelineFile;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.debugPipelineWithInput', async () => {
        const editor = vscode.window.activeTextEditor;
        const { DagCanvasProvider } = await Promise.resolve().then(() => __importStar(require('../editors/dagCanvasProvider')));
        // Check text editor first, then DAG canvas
        let pipelineFile;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('flowmason.toggleBreakpoint', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor || !editor.document.fileName.endsWith('.pipeline.json')) {
 ...
 */
async function runPipelineDirectly(pipelineFile, context) {
    ...
    try {
        ...
        // Get studio URL
        const settings = vscode.workspace.getConfiguration('flowmason');
        const studioUrl = settings.get('studioUrl') || 'http://localhost:8999';
