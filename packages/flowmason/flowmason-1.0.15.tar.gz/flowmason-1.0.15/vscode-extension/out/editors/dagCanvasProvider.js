...
exports.DagCanvasProvider = void 0;
exports.registerDagCanvasProvider = registerDagCanvasProvider;
const vscode = __importStar(require("vscode"));
class DagCanvasProvider {
    static get currentDocument() {
        return DagCanvasProvider._currentDocument;
    }
    static isEditingDocument(uri) {
    ...
    async highlightStageInJson(document, stageId) {
    ...
    async addConnection(document, fromId, toId) {
        try {
            ...
            if (this.wouldCreateCycle(pipeline, fromId, toId)) {
                vscode.window.showWarningMessage(`Cannot connect "${fromId}" → "${toId}": this would create a circular dependency`);
                return;
            }
     ...
     */
    wouldCreateCycle(pipeline, fromId, toId) {
    ...
    async removeConnection(document, fromId, toId) {
        try {
            const pipeline = JSON.parse(document.getText());
            const toStage = pipeline.stages.find(s => s.id === toId);
            if (!toStage || !toStage.depends_on)
                return;
    ...
    async reconnectEdge(document, oldFromId, oldToId, newFromId, newToId) {
        try {
            const pipeline = JSON.parse(document.getText());
            ...
            if (newFromId !== oldFromId || newToId !== oldToId) {
                ...
                if (this.wouldCreateCycle(pipeline, newFromId, newToId)) {
                    vscode.window.showWarningMessage(`Cannot reconnect: this would create a circular dependency`);
                    return;
                }
                // Check if connection already exists
    ...
    async updatePositions(document, positions) {
        try {
            const pipeline = JSON.parse(document.getText());
            for (const stage of pipeline.stages) {
    ...
    async deleteStage(document, stageId) {
        const confirm = await vscode.window.showWarningMessage(`Delete stage "${stageId}"?`, { modal: true }, 'Delete');
        if (confirm !== 'Delete')
            return;
        try {
            const pipeline = JSON.parse(document.getText());
    ...
    async deleteStages(document, stageIds) {
        if (stageIds.length === 0)
            return;
        const confirm = await vscode.window.showWarningMessage(`Delete ${stageIds.length} stage(s)?`, { modal: true }, 'Delete');
        if (confirm !== 'Delete')
            return;
        try {
            const pipeline = JSON.parse(document.getText());
    ...
    async duplicateStage(document, stageId) {
        try {
            const pipeline = JSON.parse(document.getText());
    ...
    async addStageWithComponent(document, componentType, position) {
        try {
            const pipeline = JSON.parse(document.getText());
    ...
    async updateDocument(document, pipeline) {
...
exports.DagCanvasProvider = DagCanvasProvider;
DagCanvasProvider.viewType = 'flowmason.dagCanvas';
// Track which document URIs are being edited by DAG canvas (to allow bypass protection)
DagCanvasProvider._editingDocuments = new Set();
/**
 * Register the DAG Canvas custom editor
 */
function registerDagCanvasProvider(context) {
    const provider = new DagCanvasProvider(context);
    return vscode.window.registerCustomEditorProvider(DagCanvasProvider.viewType, provider, {
        webviewOptions: {
...
//# sourceMappingURL=dagCanvasProvider.js.map