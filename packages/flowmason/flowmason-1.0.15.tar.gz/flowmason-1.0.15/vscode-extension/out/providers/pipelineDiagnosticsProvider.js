...
exports.PipelineDiagnosticsProvider = void 0;
const vscode = __importStar(require("vscode"));
class PipelineDiagnosticsProvider {
    ...
    async validateDocument(document) {
        ...
        try {
            ...
            diagnostics.push(new vscode.Diagnostic(new vscode.Range(position, position.translate(0, 1)), `Invalid JSON: ${syntaxError.message}`, vscode.DiagnosticSeverity.Error));
    ...
    validatePipelineStructure(document, pipeline, diagnostics) {
        // Required: name
        if (!pipeline.name) {
            const range = this.findPropertyRange(document, 'name') ||
                new vscode.Range(0, 0, 0, 1);
            diagnostics.push(new vscode.Diagnostic(range, 'Pipeline must have a "name" property', vscode.DiagnosticSeverity.Error));
        }
        // Required: stages array
        if (!pipeline.stages) {
            diagnostics.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 1), 'Pipeline must have a "stages" array', vscode.DiagnosticSeverity.Error));
        }
        else if (!Array.isArray(pipeline.stages)) {
            const range = this.findPropertyRange(document, 'stages') ||
                new vscode.Range(0, 0, 0, 1);
            diagnostics.push(new vscode.Diagnostic(range, '"stages" must be an array', vscode.DiagnosticSeverity.Error));
        }
        else if (pipeline.stages.length === 0) {
            const range = this.findPropertyRange(document, 'stages') ||
                new vscode.Range(0, 0, 0, 1);
            diagnostics.push(new vscode.Diagnostic(range, 'Pipeline has no stages defined', vscode.DiagnosticSeverity.Warning));
        }
    }
    validateStage(document, stage, stageIds, allStages, diagnostics) {
        const stageRange = this.findStageRange(document, stage.id);
        const defaultRange = stageRange || new vscode.Range(0, 0, 0, 1);
        // Required: id
        if (!stage.id) {
            diagnostics.push(new vscode.Diagnostic(defaultRange, 'Stage must have an "id" property', vscode.DiagnosticSeverity.Error));
            return;
        }
        // Check for duplicate IDs
        if (stageIds.has(stage.id)) {
            const idRange = this.findStagePropertyRange(document, stage.id, 'id') || defaultRange;
            diagnostics.push(new vscode.Diagnostic(idRange, `Duplicate stage ID: "${stage.id}"`, vscode.DiagnosticSeverity.Error));
        }
        stageIds.add(stage.id);
        // Required: component_type
        if (!stage.component_type) {
            diagnostics.push(new vscode.Diagnostic(defaultRange, `Stage "${stage.id}" must have a "component_type" property`, vscode.DiagnosticSeverity.Error));
        }
            ...
            diagnostics.push(new vscode.Diagnostic(typeRange, `Unknown component type: "${stage.component_type}"`, vscode.DiagnosticSeverity.Warning));
        }
        ...
        if (stage.depends_on && Array.isArray(stage.depends_on)) {
            for (const dep of stage.depends_on) {
                ...
                if (!depExists) {
                    ...
                    diagnostics.push(new vscode.Diagnostic(depsRange, `Stage "${stage.id}" depends on unknown stage: "${dep}"`, vscode.DiagnosticSeverity.Error));
                }
                // Check for self-dependency
                if (dep === stage.id) {
                    const depsRange = this.findStagePropertyRange(document, stage.id, 'depends_on') || defaultRange;
                    diagnostics.push(new vscode.Diagnostic(depsRange, `Stage "${stage.id}" cannot depend on itself`, vscode.DiagnosticSeverity.Error));
                }
            }
        }
        ...
        if (stage.id && !/^[a-z][a-z0-9_-]*$/i.test(stage.id)) {
            const idRange = this.findStagePropertyRange(document, stage.id, 'id') || defaultRange;
            diagnostics.push(new vscode.Diagnostic(idRange, `Stage ID "${stage.id}" should use alphanumeric characters, hyphens, or underscores`, vscode.DiagnosticSeverity.Information));
        }
    }
    /**
     * Detect circular dependencies in the pipeline
     */
    detectCircularDependencies(stages, document, diagnostics) {
        ...
        const hasCycle = (stageId, path) => {
            if (recursionStack.has(stageId)) {
                ...
                diagnostics.push(new vscode.Diagnostic(range, `Circular dependency detected: ${cyclePath.join(' → ')}`, vscode.DiagnosticSeverity.Error));
                return true;
            }
    ...
    extractErrorPosition(message, document) {
...
exports.PipelineDiagnosticsProvider = PipelineDiagnosticsProvider;
//# sourceMappingURL=pipelineDiagnosticsProvider.js.map