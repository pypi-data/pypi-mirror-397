...
exports.FlowMasonCompletionProvider = void 0;
const vscode = __importStar(require("vscode"));
class FlowMasonCompletionProvider {
    ...
    getDecoratorCompletions() {
        ...
        nodeDecorator.insertText = new vscode.SnippetString('node(\n' +
            '    name="${1:my-node}",\n' +
            ...
            '    color="${5:#6366f1}",\n' +
            ')\n');
        nodeDecorator.documentation = new vscode.MarkdownString('Creates a FlowMason Node component.\n\n' +
        ...
        operatorDecorator.insertText = new vscode.SnippetString('operator(\n' +
            '    name="${1:my-operator}",\n' +
            '    description="${2:Description}",\n' +
            ...
            '    color="${5:#10b981}",\n' +
            ')\n');
        operatorDecorator.documentation = new vscode.MarkdownString('Creates a FlowMason Operator component.\n\n' +
    ...
    getFieldCompletions() {
        ...
        stringField.insertText = new vscode.SnippetString('${1:field_name}: str = Field(description="${2:Field description}")');
        ...
        intField.insertText = new vscode.SnippetString('${1:field_name}: int = Field(description="${2:Field description}", default=${3:0})');
        ...
        boolField.insertText = new vscode.SnippetString('${1:field_name}: bool = Field(description="${2:Field description}", default=${3:False})');
        ...
        listField.insertText = new vscode.SnippetString('${1:field_name}: List[${2:str}] = Field(description="${3:Field description}", default_factory=list)');
        ...
        dictField.insertText = new vscode.SnippetString('${1:field_name}: Dict[str, ${2:Any}] = Field(description="${3:Field description}", default_factory=dict)');
        ...
        optField.insertText = new vscode.SnippetString('${1:field_name}: Optional[${2:str}] = Field(description="${3:Field description}", default=None)');
        optField.detail = 'Optional field';
        completions.push(optField);
        return completions;
    }
    getUpstreamCompletions(document) {
        const completions = [];
    ...
    getDecoratorOptionCompletions(document, position) {
        const completions = [];
        ...
        if (isNodeDecorator || isOperatorDecorator) {
            ...
            if (isNodeDecorator) {
                options.push({ label: 'requires_llm', snippet: 'requires_llm=${1:True}', detail: 'Whether node requires LLM' }, { label: 'default_provider', snippet: 'default_provider="${1:anthropic}"', detail: 'Default LLM provider' }, { label: 'default_model', snippet: 'default_model="${1:claude-sonnet-4-20250514}"', detail: 'Default model' });
            }
    ...
    isInsideIOClass(document, position) {
    ...
    isInsideDecorator(document, position) {
...
exports.FlowMasonCompletionProvider = FlowMasonCompletionProvider;
