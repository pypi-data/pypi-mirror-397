...
export function registerBuildPackageCommand(
    context: vscode.ExtensionContext,
    outputChannel: vscode.OutputChannel
): void {
    const command = vscode.commands.registerCommand('flowmason.buildPackage', async (uri?: vscode.Uri) => {
        try {
            ...
            if (uri) {
                ...
                if (files.length === 0) {
                    ...
                    const create = await vscode.window.showQuickPick(
                        ['Create New Package', 'Cancel'],
                        { placeHolder: 'No flowmason-package.json found' }
                    );
...
async function createPackageManifest(workspaceDir: string): Promise<void> {
...
function validateManifest(manifest: Record<string, unknown>): string[] {
    ...
    if (manifest.id && !/^[a-z][a-z0-9-]*$/.test(manifest.id as string)) {
...
async function createPackageDirectory(
    ...
    // Copy manifest
    fs.copyFileSync(
        path.join(sourceDir, 'flowmason-package.json'),
        path.join(packageOutDir, 'flowmason-package.json')
    );
