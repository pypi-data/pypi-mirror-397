...
export class WebhookServer {
    ...
    private maxPayloads: number = 100;
        ...
        return new Promise((resolve, reject) => {
            ...
            this.server.on('error', (err: NodeJS.ErrnoException) => {
                if (err.code === 'EADDRINUSE') {
                    vscode.window.showErrorMessage(
                        `Port ${this.port} is already in use. Change flowmason.webhookPort in settings.`
                    );
                } else {
            ...
            this.server.listen(this.port, () => {
                ...
                vscode.window.showInformationMessage(
                    `Webhook server started on ${this.serverUrl}`
                );
                resolve();
            });
        });
    ...
    private handleRawView(req: http.IncomingMessage, res: http.ServerResponse): void {
        res.writeHead(200, {
    ...
    private handleClear(req: http.IncomingMessage, res: http.ServerResponse): void {
        this.payloads = [];
        this.outputChannel.appendLine('');
    ...
    private handleWebView(req: http.IncomingMessage, res: http.ServerResponse): void {
...
<body>
    ...
    private formatHeaders(headers: http.IncomingHttpHeaders): string {
    ...
    private logPayload(payload: WebhookPayload): void {
        this.outputChannel.appendLine('');
 ...
 */
export function registerWebhookServer(context: vscode.ExtensionContext): WebhookServer {
    const server = new WebhookServer(context);

    // Register commands
    context.subscriptions.push(
        vscode.commands.registerCommand('flowmason.startWebhookServer', () => server.start()),
        vscode.commands.registerCommand('flowmason.stopWebhookServer', () => server.stop()),
        vscode.commands.registerCommand('flowmason.toggleWebhookServer', () => server.toggle()),
        vscode.commands.registerCommand('flowmason.copyWebhookUrl', () => server.copyUrl()),
    );
