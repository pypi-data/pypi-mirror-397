...
class AIAgent(BaseAgent):
    ...
    def run(self, prompt: Prompt | str | List[BaseMessage], retries: int = 0):
        """
    ...
    async def arun(self, prompt: Prompt | str | List[BaseMessage], retries: int = 0):
        """
    ...
    async def stream(self, prompt: Prompt | str | List[BaseMessage], stream_mode: Optional[str] = "tokens", retries: int = 0) -> AsyncGenerator[Tuple[str, str | ToolCall | ToolResponse | BaseModel | bytes | AIResponse], None]:
        """
        ...
        for attempt in range(retries + 1):
            try:
                ## Call tools if any
                if hasattr(self, "tools"):
                    async for chunk in stream_react_agent_with_tooling(self.llm, self.tools, messages, stream_mode=stream_mode, structured_output=self.structured_output):
                        yield chunk
                else:
