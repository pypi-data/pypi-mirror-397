...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `pytractoviz`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in pytractoviz.__all__ or not hasattr(pytractoviz, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in pytractoviz.__all__:
...
def test_api_matches_inventory(inventory: Inventory, public_objects: list[griffe.Object | griffe.Alias]) -> None:
    ...
    ignore_paths = {
        ...
        "pytractoviz.viz.MIN_MEMORY_CHECK_MB",  # Internal constant for memory checks
    }
    not_in_inventory = [
