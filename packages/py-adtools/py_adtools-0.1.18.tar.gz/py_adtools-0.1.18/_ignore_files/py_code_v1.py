...
def _find_last_lineno_of_class_header(node: ast.ClassDef, source: str):
    """Consider following situation while parsing the class definition,
    where the class header takes multiple lines:
    -------------------------------------
    1 class PythonClass(
    2     BaseClass1,
    3     BaseClass2,
...
class _ProgramVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        if node.col_offset == 0:
            ...
            # Record the last line of parsed object
            # last_line = node.end_lineno  # Todo: Consider that the class header has multiple lines
            last_line = _find_last_lineno_of_class_header(node, self._sourcecode)
            ...
            for item in node.body if docstring is None else node.body[1:]:
                # We only parse methods in the class, other items are regraded as 'Class Vars and Code'
                if isinstance(item, ast.FunctionDef):
                    # The start line of entire function including the decorators
                    current_function_start_line = item.lineno
                    # Extract decorators
                    has_decorators = bool(item.decorator_list)
                    if has_decorators:
                        # Find the minimum line number and retain the code above
                        decorator_start_line = min(decorator.lineno for decorator in item.decorator_list)
                        current_function_start_line = decorator_start_line
                        # Dedent decorator code
                        decorator = self._get_code(decorator_start_line - 1, item.lineno - 1, dedent=True)
                    else:
                        decorator = None

                    # Check lines between the 'last_line' and the start line of current node
                    class_code_block = self._get_code(last_line, current_function_start_line - 1)
                    # # If the lines are not empty, we regard them as a code block
                    if class_code_block.strip():
                        code_block = PyCodeBlock(code=class_code_block.rstrip())
                        class_vars_and_code.append(code_block)
                        function_class_vars_and_code.append(code_block)
                    ...
                    # Update last line no
                    last_line = item.end_lineno

            # If the lines between last_line and the end of the class definition is not empty
            class_code_block = self._get_code(last_line, node.end_lineno)
            if class_code_block.strip():
                code_block = PyCodeBlock(code=class_code_block.rstrip())
                class_vars_and_code.append(code_block)
                function_class_vars_and_code.append(code_block)
...
if __name__ == '__main__':
    code = r'''import ast, numba                 # This part will be parsed into PyCodeBlock
import numpy as np
