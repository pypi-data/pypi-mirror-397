...
## 3.22.0
### Added
- `file-format` option was added to allow writing files in a `relaxed` mode (leaving completion dates in a task even if there is no creation date; see manual, [section "Conforming to"](https://vonshednob.cc/pter/documentation.html#conforming-to) for details)
- `add-completion-date` option was added to give the user control over whether a completion date should be added to a task when not strictly necessary
