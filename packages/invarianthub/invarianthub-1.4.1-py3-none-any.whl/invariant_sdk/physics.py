...
class HaloPhysics:
    ...
    def expand_query(self, words: List[str]) -> Dict[str, Dict]:
        ...
        # 3) CRYSTAL (Solid): 1-hop Halo expansion
        # V.1 Query Lensing: Only Solid words expand
        # Gas words (rank < √N) are hubs that would pull in noise
        visited: set[str] = set(query_hashes)
        
        for w, h8 in zip(query_words, query_hashes):
            # V.1 Query Lensing: Only Solid phase words expand
            if query_phases.get(h8) == "gas":
                continue  # Hub - don't expand (would create noise)
            
            # Get ALL neighbors above threshold (no artificial limit)
            degree = result[h8].get("degree", 100)
            try:
                page = self._client.get_halo_page(
                    h8,
                    cursor=0,
                    limit=degree,  # Request all, filter by threshold
            ...
            for neighbor in page.get("neighbors", []) or []:
                ...
                if n_h8 in visited:
                    ...
                    if n_h8 in result and not result[n_h8].get("is_direct"):
