...
def wifi_delete_connection(delete_args):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"{GREEN}✓ Successfully deleted connection: {connection_name}{RESET}")
            result_data = {
            ...
            if "not found" in error_msg.lower() or "no such" in error_msg.lower():
                ...
                print(f"{RED}✗ Failed to delete connection: {error_msg}{RESET}")
                result_data = {
                    "connection": connection_name,
                    "deleted": False,
                    "error": error_msg
        ...
        print(f"{RED}✗ Operation timeout{RESET}")
        print(json.dumps(error_result))
        sys.exit(1)
    except Exception as e:
