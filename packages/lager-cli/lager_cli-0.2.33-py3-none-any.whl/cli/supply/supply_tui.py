...
print("[DEBUG] supply_tui.py module loaded", file=sys.stderr)
...
class SupplyTUI(App):
    ...
    def __init__(self, ctx, netname, gateway, dut):
        print(f"[DEBUG] SupplyTUI.__init__ called with gateway={gateway}", file=sys.stderr)
        super().__init__()
        self.ctx = ctx
        ...
        print(f"[DEBUG] SupplyTUI.__init__ complete", file=sys.stderr)
