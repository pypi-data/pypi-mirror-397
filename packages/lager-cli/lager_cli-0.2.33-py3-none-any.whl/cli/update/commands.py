...
def update(ctx, box, dut, yes, skip_restart, version, branch, check_jlink, verbose):
    ...
    # Total steps: connectivity, repo check, fetch, checkout/pull, udev, docker stop, docker build, cleanup, /etc/lager, docker start, binaries, jlink, verify, version
    progress = None if verbose else ProgressBar(total_steps=14)
