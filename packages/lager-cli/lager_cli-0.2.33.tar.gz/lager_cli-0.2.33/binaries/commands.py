"""
    lager.binaries.commands

    Commands for managing custom binaries on gateway boxes
"""
import click
import requests
import base64
...
def binaries(ctx):
    """
        Manage custom binaries on gateway boxes

        Custom binaries are executables that can be called from lager Python
        scripts via subprocess. They are stored on the gateway and mounted
...
@click.option('--name', required=False, help='Name for the binary on the gateway (defaults to filename)')
@click.option('--yes', is_flag=True, help='Skip confirmation prompt')
@click.pass_context
def add(ctx, binary_path, box, name, yes):
    """
        Upload a binary to a gateway box

        The binary will be copied to the gateway and made executable.
        It can then be called from lager Python scripts using:

            from lager.binaries import run_custom_binary
            result = run_custom_binary('my_tool', '--version')

        Example:
            lager binaries add ./my_tool --box my-gateway
    """
    # Resolve box/DUT
    if not box:
        box = get_default_gateway(ctx)

    resolved_ip = resolve_and_validate_dut(ctx, box)
    ...
    try:
        ...
        click.secho(f"Error: Could not connect to gateway at {resolved_ip}", fg='red', err=True)
        click.secho("Ensure the gateway is online and reachable.", fg='yellow', err=True)
        ctx.exit(1)
...
def list_binaries(ctx, box):
    """
        List custom binaries on a gateway box
    """
    # Resolve box/DUT
    if not box:
        box = get_default_gateway(ctx)
...
def remove(ctx, binary_name, box, yes):
    """
        Remove a binary from a gateway box

        Example:
            lager binaries remove my_tool --box my-gateway
    """
    # Resolve box/DUT
    if not box:
        box = get_default_gateway(ctx)
