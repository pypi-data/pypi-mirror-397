#!/usr/bin/env python3
"""
BLE implementation for gateway execution - combines scan and connect functionality
...
def ble_info(info_args):
    ...
    try:
        ...
        if result.get('connected'):
            ...
            print(f"{RED}✗ Failed to get device info: {result.get('error', 'Unknown error')}{RESET}")
...
def ble_disconnect(disconnect_args):
    ...
    try:
        ...
        if result.get('disconnected'):
            print(f"{GREEN}✓ Disconnected from {address}{RESET}")
            if result.get('note'):
                print(f"{GREEN}  Note: {result['note']}{RESET}")
        else:
            print(f"{RED}✗ Disconnect failed{RESET}")
