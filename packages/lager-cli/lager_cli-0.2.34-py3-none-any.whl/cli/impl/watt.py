...
def main() -> int:
    try:
        ...
        sys.stderr.write(f"Use 'lager nets --dut <dut>' to list available nets\n")
        ...
        if "wrong type" in error_msg.lower() or "expected" in error_msg.lower():
            ...
            sys.stderr.write(f"This command requires a watt-meter net. Use 'lager nets --dut <dut>' to verify net types\n")
        else:
