...
This script runs on the gateway and communicates with the oscilloscope-daemon
...
def stream_start(params: dict):
    ...
    # Get gateway IP from params
    gateway_ip = params.get("gateway_ip", "localhost")
    ...
    if json_output:
        result = {
            ...
            "visualization_url": f"http://{gateway_ip}:8080/web_oscilloscope.html?host={gateway_ip}&port={DAEMON_COMMAND_PORT}",
            "command_port": DAEMON_COMMAND_PORT
        }
        print(json.dumps(result))
    elif not quiet:
        print("Streaming started")
        print(f"Visualization: http://{gateway_ip}:8080/web_oscilloscope.html?host={gateway_ip}&port={DAEMON_COMMAND_PORT}")
