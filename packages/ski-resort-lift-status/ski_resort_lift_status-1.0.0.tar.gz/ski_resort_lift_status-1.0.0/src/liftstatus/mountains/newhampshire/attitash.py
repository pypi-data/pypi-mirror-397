...
class Attitash(liftstatus.apis.epic.EpicMountain):
    """Implementation of :class:`liftstatus.Mountain` for Attitash, NH"""
    
    def __init__(self, session: requests.Session = requests.Session()):
        super().__init__(
            name="Attitash",
            server_url="https://www.attitash.com/the-mountain/mountain-conditions/lift-and-terrain-status.aspx",
    ...
    def _map_lift_type(self, lift):
        if lift['Type'] in ['quad']:
            if lift['Name'] in ['Abenaki Quad', 'Progression Quad']:
