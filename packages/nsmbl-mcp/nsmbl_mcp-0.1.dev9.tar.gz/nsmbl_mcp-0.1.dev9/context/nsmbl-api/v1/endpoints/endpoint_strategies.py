...
def build_strategy_response(strategy: Strategy) -> StrategyResponse:
    ...
    base_url = strategy.url.rsplit('/strategy/', 1)[0] if '/strategy/' in strategy.url else settings.FRONTEND_URL
    ...
    return StrategyResponse(
        ...
        url=strategy.url,  # Use stored URL
...
async def create_strategy(
    ...
    try:
        ...
        # Step 1: Run workflow (code generation + execution)
        # Name/symbol are now generated within the workflow
        ...
        initial_state = {
            ...
            "user_name": strategy_data.name,  # Pass user override
            "user_symbol": strategy_data.symbol,  # Pass user override
            "timing": {}
        }
        ...
        # Step 2: Extract name/symbol from workflow state
        ...
        if not strategy_name or not strategy_symbol:
            logger.error("Workflow completed but missing name/symbol metadata")
            raise HTTPException(
                status_code=500,
                detail={
                    "error": "metadata_missing",
                    "message": "Failed to extract strategy name/symbol from generated code"
                }
            )
        
        logger.info(f"Strategy metadata - Name: {strategy_name}, Symbol: {strategy_symbol}")
