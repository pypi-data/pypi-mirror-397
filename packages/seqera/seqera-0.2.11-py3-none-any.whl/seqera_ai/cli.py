...
class WebsocketReceiveInterrupt(Exception):
    """Raised when user interrupts query response generation."""
...
class Spinner:
    ...
    Does not use Rich Live blocks to avoid breaking the simple CLI interface.
    """

    FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
    INTERVAL = 0.1  # seconds between frames
    # Seqera purple (#7B89EE) as 24-bit ANSI color
    COLOR = "\033[38;2;123;137;238m"
    RESET = "\033[0m"
    ...
    def _spin(self):
        ...
        while not self._stop_event.is_set():
            ...
            # Use carriage return to overwrite the line, flush to ensure immediate display
            sys.stdout.write(f"\r\033[K{self.COLOR}{frame} {self.message}{self.RESET}")
            sys.stdout.flush()
            frame_idx += 1
    ...
    def restart(self, message: str = "Thinking... (Ctr+c to interrupt)"):
        """Stop, update message, and restart the spinner."""
        self.stop()
        self.message = message
        self.start()
...
class SeqeraClient:
    ...
    def prompt_for_approval(
        ...
        # Update prompt to indicate Enter defaults to option 1
        response = self._get_single_keypress("[bold]Select [1/2/3] (Enter=1): [/bold]")

        # Normalize response: handle Enter key (newline/carriage return) and empty input as option 1
        ...
        if response == "1":
            ...
            # Invalid key, default to option 1 (more forgiving UX)
            console.print(f"[dim]Invalid selection '{response}', defaulting to option 1.[/dim]")
            return True
    ...
    def _get_single_keypress(self, prompt_text: str) -> str:
        ...
        try:
            ...
            # Handle Enter key presses gracefully
            if char in ("\r", "\n"):
                console.print()  # Echo a newline for Enter
            else:
    ...
    def send_query_streaming(self, query: str):
        ...
        for attempt in range(self.MAX_QUERY_RETRIES):
            ...
            except WebsocketReceiveInterrupt:
                # User interrupted - don't retry, just return
                console.print("[dim]Response generation interrupted by user[/dim]")
                return
    ...
    def _send_query_streaming_impl(self, query: str):
        ...
        # Connect if needed (with spinner for reconnection)
        if not was_connected:
            connect_spinner = Spinner("Connecting...")
        ...
        try:
            ...
            # Start spinner for visual feedback while waiting for response
            spinner = Spinner("Thinking... (Ctr+c to interrupt)")
            spinner.start()

            while True:
                try:
                    # Update spinner with elapsed time for long waits
                    if not first_response_received:
                        elapsed = int(time.time() - query_start_time)
                        if elapsed >= 5:
                            spinner.update_message(f"Thinking... ({elapsed}s - Ctr+c to interrupt)")
                    ...
                    if msg_type == "session":
                        ...
                        spinner.restart("Thinking... (Ctr+c to interrupt)")
                        ...
                        # Stop spinner before local execution (may prompt for approval)
                        spinner.stop()
                    ...
                    # Close connection to stop backend generation and clear buffer
                    self._disconnect()
                    spinner.stop()
                    raise WebsocketReceiveInterrupt("User pressed Ctrl+C to interrupt query")
...
def print_banner(cwd: Optional[str] = None, version: Optional[str] = None):
    ...
    # Version info with beta badge and AI disclaimer
    banner_content.append("\n   ")
    banner_content.append("Seqera AI", style="bold #7B89EE")

    if version:
        banner_content.append(f" v{version}", style="bold #7B89EE")

    banner_content.append("  ", style="")
    banner_content.append("|", style="bold yellow")
    banner_content.append(" BETA ", style="bold yellow")
    banner_content.append("|", style="bold yellow")
    banner_content.append(" AI can make mistakes - always verify outputs", style="bold #FF6B3D")

    # Version update notification
    if version and (latest_version := get_latest_version()) and (version != latest_version):
        banner_content.append("\n\n   ", style="dim")
        banner_content.append("↑", style="bold green")
        banner_content.append(f" v{latest_version} available", style="green")
    ...
    banner_content.append("\n   ", style="")
    banner_content.append("━" * 75, style="dim #4A90E2")
    banner_content.append("\n\n")

    # Quick start section
    banner_content.append("   ", style="")
    banner_content.append("QUICK START\n", style="bold #FF6B3D")
    banner_content.append("\n   ", style="")
    banner_content.append("• ", style="dim")
    banner_content.append("Set up nf-core/rnaseq pipeline", style="italic #A0A0A0")
    banner_content.append("\n   ", style="")
    banner_content.append("• ", style="dim")
    banner_content.append("Help me create a custom process", style="italic #A0A0A0")
    banner_content.append("\n   ", style="")
    banner_content.append("• ", style="dim")
    banner_content.append("Explain this workflow configuration", style="italic #A0A0A0")
    banner_content.append("\n\n")
    
    # Commands section
    banner_content.append("   ", style="")
    banner_content.append("COMMANDS\n", style="bold #FF6B3D")
    banner_content.append("\n   ", style="")
    banner_content.append("/debug", style="bold cyan")
    banner_content.append("   Check pipeline syntax and errors", style="dim")
    banner_content.append("\n   ", style="")
    banner_content.append("/schema", style="bold cyan")
    banner_content.append("  Generate Nextflow schema", style="dim")
    banner_content.append("\n   ", style="")
    banner_content.append("/", style="bold cyan")
    banner_content.append("        Show all available commands", style="dim")
    banner_content.append("\n\n")
    
    # Help section
    banner_content.append("   ", style="")
    banner_content.append("FEEDBACK & HELP\n", style="bold #FF6B3D")
    banner_content.append("\n   ", style="")
    banner_content.append("Share feedback: ", style="dim")
    banner_content.append("https://feedback.seqera.io", style="bold #7B89EE underline")
    banner_content.append("\n")

    # Create gradient border (matching main CLI banner)
    panel_width = 84
    border_colors = ["#1DBEAE", "#22BDA1", "#2ABB95", 
                     "#FF6B3D", "#FF7B51", "#FF8B65",
                     "#FF8A7A", "#FF9E8E", "#FFB2A2",
                     "#4A90E2", "#609EE6", "#76ACEA"]
    
    chars_per_segment = 3
    num_segments = panel_width // chars_per_segment
    remainder = panel_width % chars_per_segment
    
    border_top = Text()
    for i in range(num_segments):
        color = border_colors[i % len(border_colors)]
        border_top.append("▀" * chars_per_segment, style=color)
    if remainder > 0:
        border_top.append("▀" * remainder, style=border_colors[num_segments % len(border_colors)])
    
    border_bottom = Text()
    for i in range(num_segments):
        color = border_colors[i % len(border_colors)]
        border_bottom.append("▄" * chars_per_segment, style=color)
    if remainder > 0:
        border_bottom.append("▄" * remainder, style=border_colors[num_segments % len(border_colors)])

    # Print the banner
    console.print("\n")
    console.print(Align.left(border_top, width=panel_width))
    console.print(
        Panel(
            banner_content,
            border_style="#7B89EE",
    ...
    console.print(Align.left(border_bottom, width=panel_width))
    console.print()
...
def print_landing_screen(version: str = __version__):
    """Print the Seqera CLI landing screen with enhanced styling and UX."""
    
    banner_content = Text()
    # Version info with beta badge
    banner_content.append("\n   ")
    banner_content.append("Seqera CLI", style="bold #7B89EE")
    banner_content.append(f" v{version}", style="bold #7B89EE")
    banner_content.append("  ", style="")
    banner_content.append("│ BETA", style="bold yellow")
    ...
    # Welcome message
    banner_content.append("   Welcome to ", style="dim white")
    banner_content.append("Seqera CLI ", style="bold highlight")
    banner_content.append("   ", style="")
    banner_content.append("━" * 75, style="dim #4A90E2")
    banner_content.append("\n\n")
    ...
    banner_content.append(" seqera ", style="dim")
    banner_content.append("ai", style="bold cyan")
    banner_content.append("\n   ", style="")
    banner_content.append("└─ Launch the AI assistant for workflow management\n", style="dim italic")
    
    banner_content.append("\n")
    
    # Help section
    banner_content.append("   ", style="")
    banner_content.append("NEED HELP?\n", style="bold #FF6B3D")
    banner_content.append("\n   ", style="")
    banner_content.append("• Get started: ", style="dim")
    banner_content.append("https://docs.seqera.io/platform-cloud/seqera-ai/use-cases", style="bold #7B89EE underline")
    banner_content.append("\n   ", style="")
    banner_content.append("• Type ", style="dim")
    banner_content.append("seqera --help", style="bold cyan")
    banner_content.append(" for all commands", style="dim")
    banner_content.append("\n")

    # Create smoother gradient border with better color transitions
    # Match the Panel width of 84 characters
    panel_width = 84
    border_colors = ["#1DBEAE", "#1FBEA8", "#22BDA1", "#26BC9B", "#2ABB95", 
                     "#FF6B3D", "#FF7347", "#FF7B51", "#FF835B", "#FF8B65",
                     "#FF8A7A", "#FF9484", "#FF9E8E", "#FFA898", "#FFB2A2",
                     "#4A90E2", "#5597E4", "#609EE6", "#6BA5E8", "#76ACEA"]
    
    # Calculate how many segments we need to fill the width
    chars_per_segment = 3
    num_segments = panel_width // chars_per_segment
    remainder = panel_width % chars_per_segment
    ...
    for i in range(num_segments):
        color = border_colors[i % len(border_colors)]
        border_top.append("▀▀▀", style=color)
    # Add remaining characters to match exact width
    if remainder > 0:
        color = border_colors[num_segments % len(border_colors)]
        border_top.append("▀" * remainder, style=color)
    
    border_bottom = Text()
    for i in range(num_segments):
        color = border_colors[i % len(border_colors)]
        border_bottom.append("▄▄▄", style=color)
    # Add remaining characters to match exact width
    if remainder > 0:
        color = border_colors[num_segments % len(border_colors)]
        border_bottom.append("▄" * remainder, style=color)
