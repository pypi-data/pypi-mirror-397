...
def define_run(profilefile: io.TextIOWrapper, bash_options: list = [''], works: list = None,
    ...
    None
    """
    profiling_call = f'likwid-perfctr -g PYPROFQUEUE -t 300s -O -f '
...
def create_custom_group():
    ...
    group_doc_lines = [
        "SHORT\tCustom group to extract MFLOP/s, Memory bandwidth in MBytes/s and Operational Intensity in FLOP/Byte\n",
        "\n",
        "EVENTSET\n",
    ...
    group_doc_lines += [v + "\n" for v in flop_metric_lines]
    metric_memory_sum = "+".join(list(memory_variables.keys()))
    group_doc_lines += [
        f'Memory Bandwidth [MBytes/s]\t1.0E-06*({metric_memory_sum})*64.0/time\n',
        "\n",
        "LONG\n",
        "Formulas:"
    ]
    ...
    if not os.path.isdir(f"~/.likwid/groups/{architecture}"):
        os.mkdir(f"~/.likwid/groups/{architecture}")
    subprocess.run(f"cp {os.getcwd()}/PYPROFQUEUE.txt ~/.likwid/groups/{architecture}", shell=True)
    return
