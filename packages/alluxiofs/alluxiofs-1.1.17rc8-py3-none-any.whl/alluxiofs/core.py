...
from alluxiofs.client.utils import set_log_level, get_prefetch_policy
...
class AlluxioFile(AbstractBufferedFile):
    def __init__(self, fs, path, mode="rb", **kwargs):
        ...
        if fs.alluxio and self.mcap_enabled:
            block_size = (
                int(self.fs.alluxio.config.local_cache_block_size_mb)
                * 1024 * 1024
            )
            self.prefetch_policy = get_prefetch_policy(
                self.fs.alluxio.config, block_size
            )
        else:
