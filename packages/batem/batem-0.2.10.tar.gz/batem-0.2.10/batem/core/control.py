...
class SignalBuilder:
    ...
    def filter(self, signal: list[float | None], filter_function: callable) -> list[float | None]:
        ...
        :return: Filtered signal (same list, modified in place).
...
class TemperatureSetpointPort(DiscreteModePort):
    ...
    def __init__(self, data_provider: DataProvider, zone_name: str, heating_levels: list[float], cooling_levels: list[float]) -> None:
        ...
        self.model_variable_name = 'TZ_OP:'+zone_name  # or TZ
...
class Simulation:
    ...
    def __init__(self, model_maker: ModelMaker, *, verbose: bool = False) -> None:  # , control_ports: list[Port]
        ...
        for zone_name in self.name_zones:
            ...
            for candidate in (
                ...
                if candidate in self.dp:
                    presence = np.asarray(self.dp.series(candidate), dtype=float)
                    break
            if presence is None:
                presence = np.ones_like(tz_c)
            gain = factor * (34.0 - tz_c) * presence
