...
class BioPypeLspServer(LanguageServer):
    ...
    def initialize_workspace(self, root_uri: str) -> None:
        ...
        try:
            ...
            logger.info(f"Searching for modules at: {root_path}/snippets/, {root_path}/profiles/, {root_path}/pipelines/")
            ...
            if (
                ...
                for subdir in sorted(root_path.glob("*")):
                    ...
                    try:
                        ...
                        if context.snippet_paths:
                            logger.debug(f"Found {len(context.snippet_paths)} snippets in {subdir}")
                            all_snippets.extend(context.snippet_paths)
                        if context.profile_paths:
                            logger.debug(f"Found {len(context.profile_paths)} profiles in {subdir}")
                            all_profiles.extend(context.profile_paths)
                        if context.pipeline_paths:
                            logger.debug(f"Found {len(context.pipeline_paths)} pipelines in {subdir}")
                ...
                if all_snippets or all_profiles or all_pipelines:
                    ...
                    logger.info(f"Found modules in subdirectories: {len(all_snippets)} snippets, {len(all_profiles)} profiles, {len(all_pipelines)} pipelines")
