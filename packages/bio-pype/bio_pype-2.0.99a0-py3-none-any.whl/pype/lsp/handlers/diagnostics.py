...
class DiagnosticsHandler:
    ...
    def _validate_with_content(self, validator, file_path: Path, content: str, validation_context):
        ...
        try:
            ...
            if validation_context:
                ...
                logger.info(f"  Available: {num_snippets} snippets, {num_profiles} profiles, {num_pipelines} pipelines")
            ...
            try:
                ...
                logger.debug(f"Created temp file with editor content: {temp_path} ({len(content)} chars)")
                ...
                logger.info(f"Validation complete: {len(result.diagnostics)} diagnostics found")
                if result.diagnostics:
                    for diag in result.diagnostics:
                        logger.info(f"  - {diag.severity.name}: {diag.message} (code: {diag.code})")
                return result
            ...
            return ValidationResult(file_path=file_path, file_type="unknown", diagnostics=[])
