...
class CompletionHandler:
    ...
    def get_completions(
        ...
        try:
            ...
            logger.info(f"Completion requested at {file_path.name}:{position.line}:{position.character}")
            ...
            logger.debug(f"Completion context: {len(validation_context.snippet_paths)} snippets, {len(validation_context.profile_paths)} profiles, {len(validation_context.pipeline_paths)} pipelines")

            lines = content.split("\n")

            # Get appropriate completions based on file type
            if file_path.suffix == ".md":
                return self._get_snippet_completions(lines, position, validation_context, file_path)
            elif file_path.suffix in [".yaml"]:
                return self._get_yaml_completions(lines, position, validation_context, file_path)
    ...
    def _get_snippet_completions(
        self, lines: List[str], position: types.Position, validation_context, file_path: Path
    ) -> Optional[types.CompletionList]:
        ...
        try:
            ...
            if "%" in line:
                ...
                if match:
                    ...
                    suggestions = self._get_variable_completions(
                        prefix, validation_context=validation_context, snippet_path=file_path
                    )
                    logger.info(f"Returning {len(suggestions)} variable completions for prefix '{prefix}'")
                ...
                if line[:char_pos].rstrip().endswith("%("):
                    suggestions = self._get_variable_completions(
                        "", validation_context=validation_context, snippet_path=file_path
                    )  # Show all variables
                    logger.info(f"Cursor after %(, returning {len(suggestions)} variable completions")
                    return types.CompletionList(is_incomplete=False, items=suggestions)
            ...
            if "_input_:" in line or "_output_:" in line:
                ...
                if input_match or output_match:
                    ...
                    logger.info(f"Returning {len(suggestions)} I/O variable completions for prefix '{prefix}'")
                    return types.CompletionList(is_incomplete=False, items=suggestions)
            ...
            if "namespace=" in line and line.lstrip().startswith("@"):
                ...
                if match:
                    ...
                    logger.info(f"Returning {len(suggestions)} namespace completions for prefix '{prefix}'")
                    return types.CompletionList(is_incomplete=False, items=suggestions)
                ...
                if line[:char_pos].rstrip().endswith("namespace="):
                    ...
                    logger.info(f"Cursor after namespace=, returning {len(suggestions)} namespace completions")
                    return types.CompletionList(is_incomplete=False, items=suggestions)
            ...
            if "@" in line:
                ...
                if match:
                    ...
                    prefix = match.group(1)
                    suggestions = self._get_profile_name_completions(line, char_pos, validation_context)
                    logger.info(f"Returning {len(suggestions)} profile completions for prefix '{prefix}'")
                    return types.CompletionList(is_incomplete=False, items=suggestions)

                # Also check if cursor is right after @
                if line[:char_pos].rstrip().endswith("@"):
                    suggestions = self._get_profile_name_completions(line, char_pos, validation_context)
                    logger.info(f"Cursor after @, returning {len(suggestions)} profile completions")
                    return types.CompletionList(is_incomplete=False, items=suggestions)
    ...
    def _get_yaml_completions(
        ...
        try:
            ...
            if context == "snippet_name":
                ...
                suggestions = self._get_snippet_name_completions(line, char_pos, validation_context)
                return types.CompletionList(is_incomplete=False, items=suggestions)
    ...
    def _get_namespace_completions(
        self, prefix: str, validation_context=None, snippet_path=None
    ) -> List[types.CompletionItem]:
        ...
        if snippet_path and snippet_path.exists():
            try:
                ...
                if result.profile_programs:
                    for program_name, namespace_string in result.profile_programs.items():
    ...
    def _get_variable_completions(self, prefix: str, validation_context=None, snippet_path=None) -> List[types.CompletionItem]:
        ...
        if snippet_path and snippet_path.exists():
            try:
                ...
                logger.debug(f"Extracted {len(variables)} variables from validator for {snippet_path.name}")
            except Exception as e:
