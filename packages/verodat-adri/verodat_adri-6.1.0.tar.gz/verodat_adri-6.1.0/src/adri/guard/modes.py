...
class FailFastMode(ProtectionMode):
    ...
    def handle_success(self, assessment_result: Any, success_message: str, verbose: bool = False) -> None:
...
class SelectiveMode(ProtectionMode):
    ...
    def handle_failure(self, assessment_result: Any, error_message: str, verbose: bool = False) -> None:
...
class DataProtectionEngine:
    ...
    def protect_function_call(
        ...
        try:
            ...
            if assessment_passed:
                ...
                effective_mode.handle_success(assessment_result, success_message, verbose=verbose)
            else:
    ...
    def _assess_data_quality(self, data: Any, standard_path: str, reasoning_mode: bool = False) -> Any:
        ...
        if reasoning_mode and isinstance(data, str):
            ...
            try:
                ...
                self.logger.info(f"✅ JSON parsed successfully (type: {type(parsed_data).__name__})")
    ...
    def _format_error_message(
        ...
        if hasattr(assessment_result, 'metadata') and 'schema_validation' in assessment_result.metadata:
            ...
            if critical_or_error_warnings:
                ...
                for warning in critical_or_error_warnings[:3]:
                    severity_marker = "🔴" if warning.get('severity') == 'CRITICAL' else "⚠️ "
                    message_lines.extend([
                    ...
                    if warning.get('auto_fix_available') and warning.get('type') == 'FIELD_CASE_MISMATCH':
                        case_matches = warning.get('case_insensitive_matches', {})
                        if case_matches:
                            # Show first 2 examples
                            examples = list(case_matches.items())[:2]
                            example_str = ', '.join([f"'{d}'→'{s}'" for d, s in examples])
