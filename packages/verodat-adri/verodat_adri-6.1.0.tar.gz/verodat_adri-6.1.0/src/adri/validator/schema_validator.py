...
def validate_schema_compatibility(
    ...
    case_matches = _find_case_insensitive_matches(
        data_fields - exact_matches,
        standard_fields - exact_matches
    )
    
    # Calculate statistics
    total_matches = len(exact_matches)
    case_insensitive_count = len(case_matches)
    match_percentage = (total_matches / len(standard_fields) * 100) if standard_fields else 100.0
...
def _generate_schema_warnings(
    ...
    if case_matches:
        ...
        mismatch_examples = [f"'{data}' should be '{std}'" for data, std in sample_mismatches]
        
        warnings.append(SchemaWarning(
            type=SchemaWarningType.FIELD_CASE_MISMATCH,
            severity=SchemaWarningSeverity.ERROR if not strict_mode else SchemaWarningSeverity.CRITICAL,
            message=(
            ...
            remediation=(
                f"Rename the following fields to match standard case:\n" +
                "\n".join(f"  • {data} → {std}" for data, std in sample_mismatches)
            ),
    ...
    if unmatched_standard:
        ...
        warnings.append(SchemaWarning(
            ...
            remediation=(
                f"Consider adding the following fields to your data:\n" +
                "\n".join(f"  • {f}" for f in sorted(list(unmatched_standard)[:10]))
            ),
            auto_fix_available=False
        ))
    ...
    if unmatched_data:
        warnings.append(SchemaWarning(
            ...
            remediation=(
                f"The following fields are not in the standard and will be ignored:\n" +
...
VALID_DERIVATION_STRATEGIES = {"ordered_precedence", "explicit_lookup", "direct_mapping", "calculated"}
...
def validate_field_spec_v2(field_name: str, field_spec: Dict[str, Any]) -> List[str]:
        ...
        if not isinstance(derivation, dict):
            ...
            if 'strategy' not in derivation:
                ...
                if strategy not in VALID_DERIVATION_STRATEGIES:
                    ...
                    strategy_errors = _validate_derivation_strategy(field_name, derivation, strategy)
    ...
    if 'reasoning_guidance' in field_spec:
        ...
        if not isinstance(guidance, str):
            errors.append(
                f"Field '{field_name}': reasoning_guidance must be a string"
            )
    
    return errors


def _validate_derivation_strategy(field_name: str, derivation: Dict, strategy: str) -> List[str]:
    ...
    if strategy == 'ordered_precedence':
        ...
        if 'inputs' not in derivation:
            ...
            errors.append(
                f"Field '{field_name}': 'inputs' must be an array"
            )
        ...
        if 'rules' not in derivation:
            ...
            errors.append(
                f"Field '{field_name}': 'rules' must be an array"
            )
        elif not derivation['rules']:
            errors.append(
                f"Field '{field_name}': 'rules' array cannot be empty"
            )
        ...
        if 'lookup_table' not in derivation:
            ...
            errors.append(
                f"Field '{field_name}': 'lookup_table' must be an array"
            )
        elif not derivation['lookup_table']:
            errors.append(
                f"Field '{field_name}': 'lookup_table' array cannot be empty"
            )
        ...
        if 'source_field' not in derivation:
            ...
            errors.append(
                f"Field '{field_name}': 'source_field' must be a string"
            )
        ...
        if 'mappings' not in derivation:
            ...
            errors.append(
                f"Field '{field_name}': 'mappings' object cannot be empty"
            )
        ...
        if 'formula' not in derivation:
            ...
            errors.append(
                f"Field '{field_name}': 'formula' must be a string"
            )
...
def validate_standard_schema_v2(field_requirements: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    for field_name, field_spec in field_requirements.items():
        if not isinstance(field_spec, dict):
            all_errors[field_name] = [f"Field specification must be an object/dictionary"]
            continue
...
def validate_conversation_structure(schema: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if 'can_modify' not in schema:
        if 'schema' in errors:
            errors['schema'].append("Conversation mode requires 'can_modify' field definition")
        else:
            errors['schema'] = ["Conversation mode requires 'can_modify' field definition"]
    ...
    if 'required_outputs' in schema:
        if not isinstance(schema['required_outputs'], dict):
            errors['schema.required_outputs'] = ["Must be an object/dictionary if present"]
...
class SchemaValidator:
    ...
    def format_warning_message(self, warning: SchemaWarning) -> str:
        ...
        if warning.auto_fix_available:
            lines.extend([
                "",
                "Note: Auto-fix suggestions available for this issue."
            ])
    ...
    def suggest_auto_fix(self, warning: SchemaWarning) -> Optional[str]:
        ...
        if warning.type == SchemaWarningType.FIELD_CASE_MISMATCH and warning.case_insensitive_matches:
            rename_dict = {
                data: std 
                for data, std in warning.case_insensitive_matches.items()
            }
            return (
