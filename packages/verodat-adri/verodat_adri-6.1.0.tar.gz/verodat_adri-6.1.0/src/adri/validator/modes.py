    ...
    def from_string(cls, mode_str: str) -> 'ADRIMode':
        ...
        for mode in cls:
            if mode.value == mode_str:
                return mode
        raise ValueError(f"Invalid mode: {mode_str}. Valid modes: {[m.value for m in cls]}")
...
def detect_mode(adri_spec: dict[str, Any]) -> ADRIMode:
    ...
    if 'requirements' in adri_spec:
        ...
        if isinstance(requirements, dict):
            ...
            if 'input_requirements' in requirements or 'output_requirements' in requirements:
                return ADRIMode.DETERMINISTIC
            # Check for reasoning mode in requirements section
            if 'context_requirements' in requirements or 'field_requirements' in requirements:
                return ADRIMode.REASONING
...
def get_mode_sections(mode: ADRIMode) -> dict[str, list[str]]:
    ...
    sections = {
        ...
        ADRIMode.REASONING: {
            'required': [],
            'optional': ['context_requirements', 'field_requirements', 'requirements.context_requirements', 'requirements.field_requirements']
        },
