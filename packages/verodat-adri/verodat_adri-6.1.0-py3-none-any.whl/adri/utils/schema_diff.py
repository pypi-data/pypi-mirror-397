...
class ImpactLevel(str, Enum):
    ...
    NON_BREAKING = "NON_BREAKING"  # Backward compatible (field additions, constraint relaxations)
...
class SchemaDiffService:
    ...
    STRUCTURAL_PROPERTIES = {
        "type", "items", "properties", "required", "enum"
    }
    
    # Properties that are constraints
    CONSTRAINT_PROPERTIES = {
        "min_value", "max_value", "min_length", "max_length",
        "min_items", "max_items", "pattern", "format"
    }
    
    # Properties that are validation rules
    VALIDATION_PROPERTIES = {
        "validation_rules", "custom_validators"
    }
    
    # Properties that are metadata/documentation
    METADATA_PROPERTIES = {
        "description", "examples", "default", "title"
    }
    ...
    def diff_files(
        ...
        return self.diff_dicts(source_schema, target_schema, source_version, target_version)
    ...
    def diff_dicts(
        ...
        breaking_count = sum(1 for c in changes if c.impact_level == ImpactLevel.BREAKING)
        non_breaking_count = sum(1 for c in changes if c.impact_level == ImpactLevel.NON_BREAKING)
        clarification_count = sum(1 for c in changes if c.impact_level == ImpactLevel.CLARIFICATION)
        
        # Generate summary
        summary = self._generate_summary(changes, breaking_count, non_breaking_count, clarification_count)
    ...
    def _detect_field_property_changes(
        ...
        changes.extend(self._detect_constraint_changes(field_name, source_field, target_field))
        
        # Check for validation rule changes
        changes.extend(self._detect_validation_rule_changes(field_name, source_field, target_field))
        ...
        if metadata_changed:
            changes.append(SchemaChange(
                ...
                field_name=field_name,
                impact_level=ImpactLevel.CLARIFICATION,
                before_value={k: source_field.get(k) for k in self.METADATA_PROPERTIES if k in source_field},
                after_value={k: target_field.get(k) for k in self.METADATA_PROPERTIES if k in target_field},
        ...
        for constraint in self.CONSTRAINT_PROPERTIES:
            ...
            if source_value != target_value:
                ...
                changes.append(SchemaChange(
                    ...
                    remediation=self._get_constraint_remediation(constraint, source_value, target_value, impact)
                ))
    ...
    def _classify_constraint_change_impact(
        ...
        if constraint.startswith("min"):
            return ImpactLevel.BREAKING if target_value > source_value else ImpactLevel.NON_BREAKING
    ...
    def _detect_validation_rule_changes(
        ...
        source_rule_names = {self._get_rule_identifier(r) for r in source_rules if isinstance(r, dict)}
        target_rule_names = {self._get_rule_identifier(r) for r in target_rules if isinstance(r, dict)}
...
def format_diff_report(diff_result: SchemaDiffResult) -> str:
    ...
    lines = []
    lines.append("=" * 70)
    lines.append(f"ADRI Schema Diff: {diff_result.source_version} → {diff_result.target_version}")
    lines.append("=" * 70)
    lines.append("")
...
def format_diff_for_autotune(diff_result: SchemaDiffResult) -> str:
    ...
    lines = []
    lines.append(f"Schema Changes ({diff_result.source_version} → {diff_result.target_version}):")
...
def compare_schemas(
    ...
    return service.diff_dicts(source_schema, target_schema, source_version, target_version)
