...
SQL_RESERVED_WORDS = {
    ...
    "DELETE", "JOIN", "INNER", "OUTER", "LEFT", "RIGHT", "FULL", "CROSS",
    "CASE", "WHEN", "THEN", "ELSE", "END", "NULL", "TRUE", "FALSE",
    "DATE", "TIME", "TIMESTAMP", "USER", "COUNT", "SUM", "AVG", "MIN", "MAX",
    "DISTINCT", "UNION", "INTERSECT", "EXCEPT", "HAVING", "AS", "ON",
...
POSTGRESQL_RESERVED_WORDS = SQL_RESERVED_WORDS | {
    ...
    "COLLATE", "CONCURRENTLY", "CURRENT_CATALOG", "CURRENT_DATE",
    "CURRENT_ROLE", "CURRENT_SCHEMA", "CURRENT_TIME", "CURRENT_TIMESTAMP",
    "CURRENT_USER", "DEFERRABLE", "DO", "FETCH", "FREEZE", "GRANT",
    "ILIKE", "INITIALLY", "ISNULL", "LATERAL", "LIMIT", "LOCALTIME",
    "LOCALTIMESTAMP", "NOTNULL", "OFFSET", "OVERLAPS", "PLACING",
    "RETURNING", "SESSION_USER", "SIMILAR", "SYMMETRIC", "TABLESAMPLE",
    "VARIADIC", "VERBOSE", "WINDOW"
}

# MySQL-specific reserved words
MYSQL_RESERVED_WORDS = SQL_RESERVED_WORDS | {
    "ACCESSIBLE", "ADD", "ALL", "ANALYZE", "ASENSITIVE", "BEFORE",
    "BIGINT", "BINARY", "BLOB", "BOTH", "CALL", "CASCADE", "CHANGE",
    "CHAR", "CHARACTER", "CONDITION", "CONSTRAINT", "CONTINUE", "CONVERT",
    "CURRENT_DATE", "CURRENT_TIME", "CURRENT_TIMESTAMP", "CURRENT_USER",
    "CURSOR", "DATABASE", "DATABASES", "DAY_HOUR", "DAY_MICROSECOND",
    "DAY_MINUTE", "DAY_SECOND", "DEC", "DECIMAL", "DECLARE", "DEFAULT",
    "DELAYED", "DETERMINISTIC", "DIV", "DOUBLE", "DUAL", "EACH", "ELSEIF",
    "ENCLOSED", "ESCAPED", "EXIT", "EXPLAIN", "FLOAT", "FLOAT4", "FLOAT8",
    "FORCE", "FULLTEXT", "GENERAL", "HIGH_PRIORITY", "HOUR_MICROSECOND",
    "HOUR_MINUTE", "HOUR_SECOND", "IF", "IGNORE", "INFILE", "INOUT",
    "INT", "INT1", "INT2", "INT3", "INT4", "INT8", "INTEGER", "INTERVAL",
    "ITERATE", "KEYS", "KILL", "LEADING", "LEAVE", "LIMIT", "LINEAR",
    ...
    "NUMERIC", "OPTIMIZE", "OPTION", "OPTIONALLY", "OUT", "OUTFILE",
    "PRECISION", "PROCEDURE", "PURGE", "RANGE", "READ", "READS", "REAL",
    "REGEXP", "RELEASE", "RENAME", "REPEAT", "REPLACE", "REQUIRE",
    "RESIGNAL", "RESTRICT", "RETURN", "REVOKE", "RLIKE", "SCHEMA",
    ...
    "TINYBLOB", "TINYINT", "TINYTEXT", "TRAILING", "UNDO", "UNLOCK",
    "UNSIGNED", "USAGE", "USE", "USING", "UTC_DATE", "UTC_TIME",
    "UTC_TIMESTAMP", "VARBINARY", "VARCHAR", "VARCHARACTER", "VARYING",
    "WHILE", "WITH", "WRITE", "X509", "XOR", "YEAR_MONTH", "ZEROFILL"
}

# Oracle-specific reserved words
ORACLE_RESERVED_WORDS = SQL_RESERVED_WORDS | {
    "ACCESS", "ADD", "ALL", "AUDIT", "CLUSTER", "COLUMN", "COMMENT",
    "COMPRESS", "CONNECT", "EXCLUSIVE", "FILE", "IDENTIFIED", "IMMEDIATE",
    "INCREMENT", "INITIAL", "LOCK", "LONG", "MAXEXTENTS", "MINUS", "MODE",
    "NOAUDIT", "NOCOMPRESS", "NOWAIT", "NUMBER", "OFFLINE", "ONLINE",
    "PCTFREE", "PRIOR", "RAW", "RENAME", "RESOURCE", "ROW", "ROWID",
    "ROWNUM", "ROWS", "SESSION", "SHARE", "SIZE", "SMALLINT", "START",
    "SUCCESSFUL", "SYNONYM", "SYSDATE", "UID", "VALIDATE", "VARCHAR",
    "VARCHAR2", "WHENEVER"
}
...
class SchemaConsistencyReport:
    ...
    def has_errors(self) -> bool:
        ...
        return any(
            i.severity in [ConsistencyIssueSeverity.CRITICAL, ConsistencyIssueSeverity.ERROR] 
            for i in self.issues
        )
    
    def get_issues_by_severity(self, severity: ConsistencyIssueSeverity) -> List[ConsistencyIssue]:
...
class SchemaConsistencyValidator:
    ...
    TYPE_INDICATORS = {
        "array": [
            r"\barray\b", r"\blist\b", r"\[\s*\]", r"JSON array",
            r"comma-separated", r"multiple", r"collection of"
        ],
        "integer": [
            r"\binteger\b", r"\bwhole number\b", r"\bcount\b", r"\bid\b"
        ],
        "float": [
            r"\bfloat\b", r"\bdecimal\b", r"\bpercent\b", r"\brate\b",
            r"\bscore\b", r"\bratio\b"
        ],
        "boolean": [
            r"\bboolean\b", r"\btrue/false\b", r"\byes/no\b", r"\bflag\b"
        ],
        "date": [
            r"\bdate\b", r"\btimestamp\b", r"\btime\b", r"ISO\s*8601"
        ]
    }
    ...
    def _load_type_patterns(self, adri_spec: Dict[str, Any]) -> None:
        ...
        for expected_type, patterns in type_patterns.items():
            if expected_type not in self.VALID_TYPES:
                logger.warning(f"Unknown type '{expected_type}' in type_patterns, skipping")
                continue
            
            if not isinstance(patterns, list):
                logger.warning(f"Patterns for type '{expected_type}' must be a list, skipping")
                continue
        ...
        if self.custom_type_patterns:
            logger.info(f"Loaded custom type patterns for {len(self.custom_type_patterns)} type(s)")
    ...
    def _load_sql_compatibility(self, adri_spec: Dict[str, Any]) -> None:
        ...
        if self.sql_dialects:
            logger.info(f"Loaded SQL compatibility for dialects: {', '.join(self.sql_dialects)}")
    ...
    def _check_sql_dialect_conflicts(self, field_name: str) -> List[ConsistencyIssue]:
        ...
        if field_name_upper in self.custom_sql_reserved:
            severity = ConsistencyIssueSeverity.ERROR if self.sql_strict_mode else ConsistencyIssueSeverity.WARNING
            issues.append(ConsistencyIssue(
    ...
    def _check_sql_reserved_words(self, field_name: str) -> List[ConsistencyIssue]:
        ...
        if field_name.upper() in SQL_RESERVED_WORDS:
            severity = ConsistencyIssueSeverity.ERROR if self.strict_mode else ConsistencyIssueSeverity.WARNING
            issues.append(ConsistencyIssue(
                type=ConsistencyIssueType.SQL_RESERVED_WORD,
                severity=severity,
                field_name=field_name,
    ...
    def _check_field_type(self, field_name: str, field_spec: Dict[str, Any]) -> List[ConsistencyIssue]:
    ...
    def _check_array_completeness(
        ...
        if "items" not in field_spec:
            ...
            issues.append(ConsistencyIssue(
                ...
                field_name=field_name,
                message=(
                    f"Array field '{field_name}' lacks 'items' specification"
                ),
                remediation=(
                    f"Add 'items' specification to define array element type:\n"
                    f"  items:\n"
                    f"    type: string  # or integer, float, etc.\n"
                    f"    min_length: 1  # optional constraints\n"
                    f"    max_length: 100"
                )
            ))
        else:
            ...
            if isinstance(items_spec, dict) and "type" not in items_spec:
                issues.append(ConsistencyIssue(
                    ...
                    remediation=(
                        f"Add 'type' to items specification:\n"
                        f"  items:\n"
                        f"    type: string  # or integer, float, etc."
                    )
                ))
        ...
        if "min_items" not in field_spec and "max_items" not in field_spec:
            issues.append(ConsistencyIssue(
                ...
                remediation=(
                    f"Consider adding array size constraints:\n"
                    f"  min_items: 1  # minimum number of elements\n"
                    f"  max_items: 50  # maximum number of elements"
                )
            ))
            ...
            if "min_value" in field_spec and "max_value" in field_spec:
                ...
                if min_val > max_val:
                    issues.append(ConsistencyIssue(
                        ...
                        conflicting_values={"min_value": min_val, "max_value": max_val}
                    ))
            ...
            if "min_length" in field_spec and "max_length" in field_spec:
                ...
                if min_len > max_len:
                    issues.append(ConsistencyIssue(
                        ...
                        conflicting_values={"min_length": min_len, "max_length": max_len}
                    ))
            ...
            if "min_items" in field_spec and "max_items" in field_spec:
                ...
                if min_items > max_items:
                    issues.append(ConsistencyIssue(
                        ...
                        conflicting_values={"min_items": min_items, "max_items": max_items}
                    ))
        
        # Check for invalid constraint values
        for constraint_name in ["min_value", "max_value", "min_length", "max_length", "min_items", "max_items"]:
...
def validate_schema_consistency(
    ...
    if report.issues_found > 0:
        ...
        for issue in report.issues:
            log_level = logging.ERROR if issue.severity in [
        ...
        logger.info(f"Schema consistency validation passed for {report.total_fields} field(s)")
