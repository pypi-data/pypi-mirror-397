    ...
    def assess(self, data, standard_path=None):
        ...
        if standard_path:
            ...
            try:
                ...
                strict_case_matching = self.config.get('schema_validation', {}).get('strict_case_matching', False)
                
                schema_result = validate_schema_compatibility(data, field_requirements, strict_mode=strict_case_matching)
                ...
                if not strict_case_matching and schema_result.case_insensitive_matches > 0:
                    # Find case mismatch warning
                    case_mismatch_warnings = [
                        w for w in schema_result.warnings 
                        if w.type.value == 'FIELD_CASE_MISMATCH' and w.case_insensitive_matches
                    ]
                    ...
                    if case_mismatch_warnings:
                        ...
                        if _should_enable_debug():
                            diagnostic_log.append(f"Auto-fixed {len(rename_dict)} field names to match standard case")
                        ...
                        schema_result.exact_matches += schema_result.case_insensitive_matches
                        schema_result.match_percentage = (
                            (schema_result.exact_matches / schema_result.total_standard_fields * 100)
                            if schema_result.total_standard_fields else 100.0
                        )
                ...
                if field_requirements:
                    ...
                    if extra_fields:
                        ...
                        data = data[[col for col in data.columns if col in schema_fields]]
                        
                        if _should_enable_debug():
                            diagnostic_log.append(f"Contract enforcement: Removed {len(extra_fields)} non-schema fields")
                        ...
                        logger.info(f"🛡️  Data contract enforced: Removed {len(extra_fields)} non-schema fields")
                ...
                if schema_result.match_percentage < 100.0:
                    ...
                    missing_fields = [
                        w.affected_fields for w in schema_result.warnings 
                    ...
                    if missing_list:
                        error_msg += f"Missing required fields: {', '.join(missing_list[:5])}"
                ...
                print(f"[SCHEMA ERROR] Schema validation failed: {str(e)}", file=sys.stderr)
                print(f"[SCHEMA ERROR] Traceback:\n{traceback.format_exc()}", file=sys.stderr)
                if _should_enable_debug():
                    diagnostic_log.append(f"Schema validation raised ValueError: {str(e)}")
                ...
                import sys
                import traceback
                print(f"[SCHEMA ERROR] Non-fatal schema validation error: {type(e).__name__}: {str(e)}", file=sys.stderr)
                print(f"[SCHEMA ERROR] Traceback:\n{traceback.format_exc()}", file=sys.stderr)
                if _should_enable_debug():
                    diagnostic_log.append(
        ...
        if 'schema_validation' in result.metadata:
            ...
            for warning in warnings:
                ...
                if warning.get('severity') in ['CRITICAL', 'ERROR']:
                    all_failures.append({
                        'dimension': 'schema',
                        'field': ', '.join(warning.get('affected_fields', [])[:5]),  # First 5 fields
                        ...
                        'auto_fix_available': warning.get('auto_fix_available', False),
                        'case_insensitive_matches': warning.get('case_insensitive_matches')
                    })
