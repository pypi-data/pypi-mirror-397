...
def _check_optional_sections(
    ...
    all_mode_indicators = {
        'conversation': ['schema.context', 'schema.required_outputs', 'schema.can_modify'],
        'reasoning': ['context_requirements', 'field_requirements', 
                     'requirements.context_requirements', 'requirements.field_requirements'],
...
def _validate_conversation_structure(
    ...
    if found:
        if not isinstance(req_outputs, dict):
            ...
            warnings.append("'schema.required_outputs' is empty - no output fields defined")
    ...
    if found:
        if not isinstance(can_modify, (bool, dict)):
            warnings.append("'schema.can_modify' should be a boolean or object with 'type: boolean'")
    ...
    if 'requirements' in adri_spec and isinstance(adri_spec['requirements'], dict):
        ...
        if 'context_requirements' in req:
            ...
            if not isinstance(req['context_requirements'], dict):
                errors.append("'requirements.context_requirements' must be an object/dictionary")
        
        if 'field_requirements' in req:
            has_fields = True
            if not isinstance(req['field_requirements'], dict):
                errors.append("'requirements.field_requirements' must be an object/dictionary")
...
def format_validation_report(result: StructureValidationResult) -> str:
    ...
    lines = []
    lines.append(f"ADRI Structure Validation Report")
