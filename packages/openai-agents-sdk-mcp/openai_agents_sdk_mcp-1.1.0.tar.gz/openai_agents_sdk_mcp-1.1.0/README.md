...
2. **Set up API key** (Required):
   ...
   > **Important**: The tool requires an OpenAI API key to perform intelligent documentation matching. Without it, you'll see "No matching documentation found" errors.
...
> **Important**: The tool requires an OpenAI API key to perform intelligent documentation matching using GPT-4o-mini. Without it, the `get_documentation` tool will fail with configuration errors.
...
- `OPENAI_API_KEY` - **Required** for AI-powered search functionality. Get your key from [OpenAI Platform](https://platform.openai.com/api-keys).

## Troubleshooting

### "No matching documentation found for" every topic

This usually means the OpenAI API key is not configured correctly:
...
2. **For MCP servers**, ensure the key is passed in the configuration:
   ```json
   {
     "mcpServers": {
       "openai-agents-sdk-docs": {
...
3. **For CLI usage**, set the environment variable:
   ```bash
...
The tool cannot find your OpenAI API key. Follow the steps above to configure it properly.

### Index not refreshing

If the documentation index seems stale:

```bash
# Force refresh the index
python -m openai_agents_sdk_mcp.documentation
```

Or use the MCP tool with `force_refresh: true`:
```json
{
  "name": "list_documentation_topics",
  "arguments": {
