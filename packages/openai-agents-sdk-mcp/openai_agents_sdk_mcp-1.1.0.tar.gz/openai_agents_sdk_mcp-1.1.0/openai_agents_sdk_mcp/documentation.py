...
def get_documentation_for_feature(feature_name: str, doc_map: Optional[Dict[str, str]] = None) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not doc_map:
        raise ValueError("Documentation map is empty. Unable to find any topics.")
...
Do not include any explanation or additional text."""
    ...
    try:
        ...
        print(f"[DEBUG] LLM matched '{feature_name}' to '{matched_topic}'")
        
        if matched_topic == "NO_MATCH":
            print(f"[DEBUG] LLM could not find a match for '{feature_name}'")
            return None, None
        
        # Verify the matched topic exists in the map
        if matched_topic in doc_map:
            print(f"[DEBUG] Found exact match in doc_map")
            ...
            for topic in doc_map:
                if topic.lower() == matched_topic.lower():
                    print(f"[DEBUG] Found case-insensitive match: '{topic}'")
                    return topic, doc_map[topic]
            
            # If still no match, try fuzzy matching
            print(f"[DEBUG] LLM returned '{matched_topic}' but it's not in the documentation map.")
            print(f"[DEBUG] Attempting fuzzy match...")
            
            # Try to find partial matches
            matched_topic_lower = matched_topic.lower()
            for topic in doc_map:
                if matched_topic_lower in topic.lower() or topic.lower() in matched_topic_lower:
                    print(f"[DEBUG] Found fuzzy match: '{topic}'")
                    return topic, doc_map[topic]
            
            print(f"[DEBUG] No fuzzy match found either")
            return None, None
