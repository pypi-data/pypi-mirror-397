    ...
    if name == "list_documentation_topics":
        ...
        try:
            ...
            if not topic or not url:
                ...
                if len(doc_map) > 10:
                    response_text += f"\n... and {len(doc_map) - 10} more topics available."
            ...
            # Configuration errors (missing API key, empty doc map, etc.)
            error_msg = f"Configuration Error: {str(e)}\n\n"
            error_msg += "Please ensure:\n"
            error_msg += "1. OPENAI_API_KEY is set in your environment or MCP configuration\n"
            error_msg += "2. The documentation index is not empty\n"
            return [TextContent(type="text", text=error_msg)]
        except RuntimeError as e:
            # API errors
            error_msg = f"API Error: {str(e)}\n\n"
            error_msg += "There was an issue calling the OpenAI API. Please check:\n"
            error_msg += "1. Your API key is valid\n"
            error_msg += "2. You have sufficient API credits\n"
            error_msg += "3. Your network connection is working\n"
            return [TextContent(type="text", text=error_msg)]
        except Exception as e:
