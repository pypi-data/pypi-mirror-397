...
class RsyncTransferStrategy:
    ...
    def _human_size_to_bytes(self, s: str) -> int:
        """Convert human-readable size string (e.g., '1.5M', '500K') to bytes."""
        s_clean = s.strip().replace(",", "")
        if s_clean.endswith("/s"):
            s_clean = s_clean[:-2]

        units = {"k": 1024, "m": 1024**2, "g": 1024**3, "t": 1024**4}
        m = re.match(r"^([0-9]+(?:\.[0-9]+)?)\s*([kKmMgGtT])?[bB]?$", s_clean)
        if not m:
            # Fallback for unrecognized format; ValueError expected for malformed rsync output
            try:
    ...
    def _parse_rsync_progress(self, line: str) -> TransferProgress | None:
        ...
        Returns a TransferProgress when a progress line is detected, None otherwise.
        """
        # Ignore rsync status/stats lines
        if any(
            marker in line
            for marker in (
        ...
        # Bare filename (no percent sign) - treat as current file indicator
        if "%" not in line and not line.startswith("(") and ("/" in line or "." in line):
        ...
        # Parse progress line: "  12,345  42%  1.23MB/s  0:00:10 (xfr#1, to-chk=3/10)"
        ...
        if not any([tochk_match, percent_match, speed_match, eta_match, bytes_match]):
            return None

        # Prefer to-chk (overall file count progress) over per-file percentage
        # for stable ETA estimation when file sizes vary.
        percent_val: float | None = None
        if tochk_match:
            remain = int(tochk_match.group("remain"))
            total = int(tochk_match.group("total"))
            if total > 0:
                percent_val = (1.0 - remain / total) * 100.0
        elif percent_match:
            percent_val = float(percent_match.group("percent"))

        return TransferProgress(
            bytes_transferred=self._human_size_to_bytes(bytes_match.group("size"))
            if bytes_match
            else 0,
            total_bytes=None,
