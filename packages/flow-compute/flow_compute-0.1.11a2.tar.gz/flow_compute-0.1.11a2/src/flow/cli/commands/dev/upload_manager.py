...
from flow.adapters.transport.code_transfer import CodeTransferError
from flow.adapters.transport.rsync import TransferError
...
class DevUploadManager:
    ...
    def _upload_to_default(self, source_dir: Path) -> str:
        ...
        # Validate and create remote target directory
        remote_ops = self.flow_client.get_remote_operations()
        plan = UploadTargetPlan(
            remote_parent=remote_parent_dir,
            remote_target=remote_upload_dir,
            container_workdir=None,
            mode=self.upload_mode,
        )
        ensure_remote_target(remote_ops, self.vm_task.task_id, plan)
        ...
        try:
            if (
                ...
                try:
                    ...
                    pass
        except (OSError, RuntimeError):  # UI update is best-effort
            pass
        ...
        except (OSError, RuntimeError):  # UI notice is best-effort
            pass
        return actual_dir
