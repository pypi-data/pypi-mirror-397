...
class RawString(str):
    ...
    def from_str(string: str, encoding: str) -> RawString:
        """
        Converts a std string to a RawString object.

        Args:
            string (str): Regular Python string to convert.
            encoding (str): String encoding.

        Returns:
            RawString: RawString object.
        """
    ...
    def decode(data: bytes) -> RawString:
        """
        Attempts to decode a string from bytes in order of the supported encodings.
        The first encoding that works will be assigned to the string.

        Args:
            data (bytes): Byte array.

        Returns:
            RawString: Decoded string.
        """
    ...
    def encode(string: RawString) -> bytes:  # type: ignore
        """
        Attempts to encode a string to bytes by trying out all supported encodings.

        Args:
            string (RawString): String to encode.
        ...
        for encoding in RawString.SUPPORTED_ENCODINGS:
            try:
                data: bytes = str(string).encode(encoding)
                string.encoding = encoding
                return data
...
class Hex:
    ...
    def parse(
        ...
        number: int = Integer.parse(data, type)
