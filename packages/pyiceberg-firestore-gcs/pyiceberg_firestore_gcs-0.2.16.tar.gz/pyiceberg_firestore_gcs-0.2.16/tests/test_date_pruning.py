...
# Get table metadata
table_ref = client.collection(catalog_name).document(namespace).collection("tables").document(table_name)
...
print(f"  Total files in snapshot: {snapshot_data.get('summary', {}).get('total-data-files', 'unknown')}")
...
for file_data in manifest_files[:4]:  # Show first 4
    ...
    print(f"  File {month:2d}: dates {min_date} to {max_date}, timestamps {min_ts.date()} to {max_ts.date()}")
...
test_cases = [
    ...
    ("Timestamp GreaterThan Oct 31", "timestamp", "GreaterThan", datetime(2024, 10, 31, 23, 59, 59), [11, 12]),
...
for description, data_type, filter_type, filter_value, expected_files in test_cases:
    if data_type == "date":
        pruned_files = [f['month'] for f in manifest_files if not would_prune_date(f, filter_type, filter_value)]
    elif data_type == "timestamp":
        pruned_files = [f['month'] for f in manifest_files if not would_prune_timestamp(f, filter_type, filter_value)]
    elif data_type == "string":
        pruned_files = [f['month'] for f in manifest_files if not would_prune_string(f, filter_type, filter_value, 'col_2')]
...
print(f"Date range: 2024-01-01 to 2024-12-31 (12 months)")
print()
print("Key findings:")
