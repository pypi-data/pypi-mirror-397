...
total += 1
if test_filter(
    "ID = 10000 (exact lower bound)", 
    1,
    EqualTo("id", 10000),
    "January IDs: 10000-19999"
):
    passed += 1
...
total += 1
if test_filter(
    "ID > 140999 (above all data)", 
    0,
    GreaterThan("id", 140999),
    "December IDs: 120000-140999"
):
    passed += 1
...
total += 1
if test_filter(
    "ID >= 20000 (Feb onwards)", 
    11,
    GreaterThanOrEqual("id", 20000),
    "Should include Feb-Dec"
):
    passed += 1
...
total += 1
if test_filter(
    "Amount = 100.0 (exact lower bound)", 
    1,
    EqualTo("amount", 100.0),
    "Jan amounts: 100-199"
):
    passed += 1
...
if test_filter(
    "event_type = 'login' (Q1)", 
    3,
    EqualTo("event_type", "login"),
    "Months 1-3 have 'login'"
):
    passed += 1
...
total += 1
if test_filter(
    "event_type = 'logout' (Q4)", 
    3,
    EqualTo("event_type", "logout"),
    "Months 10-12 have 'logout'"
):
    passed += 1
...
if test_filter(
    ...
    0,
    EqualTo("event_type", "nonexistent"),
    "Should prune all files"
):
    passed += 1
...
if actual <= 3:  # Allow some tolerance for string matching
    print(f"✓ PASS | {'user_id = user_012_500 (string prefix)':50s} | Expected: ~{expected:2d} | Got: {actual:2d}")
    passed += 1
else:
    print(f"✗ FAIL | {'user_id = user_012_500 (string prefix)':50s} | Expected: ~{expected:2d} | Got: {actual:2d}")
total += 1
