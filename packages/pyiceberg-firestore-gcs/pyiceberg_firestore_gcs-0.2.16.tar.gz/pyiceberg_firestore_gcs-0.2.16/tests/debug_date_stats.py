...
print(f"Parquet manifest path: gs://opteryx_data/prune_testing/default/events/metadata/manifest-{snapshot.snapshot_id}.parquet")
...
for i in range(min(3, len(parquet_table))):
    ...
    if lower and len(lower) > 4:
        ...
        if lower[4] == INT64_MIN:
            print(f"    ⚠️  Field 4 has INT64_MIN - likely NULL/missing!")
...
if manifests:
    ...
    for i, entry in enumerate(entries):
        ...
        if df.lower_bounds and 4 in df.lower_bounds:
            print(f"  Field 4 (event_date) in Avro:")
