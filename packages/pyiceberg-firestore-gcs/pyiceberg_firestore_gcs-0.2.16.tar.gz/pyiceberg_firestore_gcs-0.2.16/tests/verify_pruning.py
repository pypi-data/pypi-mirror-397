...
print("="*60)
scan = table.scan(
    row_filter=LessThan("event_date", date(2024, 2, 1))
)
tasks = list(scan.plan_files())
print(f"Filter: event_date < 2024-02-01")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 1 (January file only)")
if len(tasks) == 1:
...
tasks = list(scan.plan_files())
print(f"Filter: event_date >= 2024-03-01 AND event_date < 2024-07-01")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 4 (March, April, May, June)")
if len(tasks) == 4:
...
print("="*60)
scan = table.scan(
    row_filter=GreaterThan("event_timestamp", datetime(2024, 6, 30, 23, 59, 59))
)
tasks = list(scan.plan_files())
print(f"Filter: event_timestamp > 2024-06-30 23:59:59")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 6 (July through December)")
if len(tasks) == 6:
...
print("="*60)
scan = table.scan(
    row_filter=LessThan("id", 12000)
)
tasks = list(scan.plan_files())
print(f"Filter: id < 12000")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 1 (January: IDs 1000-11999)")
if len(tasks) == 1:
    print("✓ PASS - Correctly pruned to 1 file")
else:
    print(f"✗ FAIL - Expected 1 file, got {len(tasks)}")
...
print("="*60)
scan = table.scan(
    row_filter=GreaterThanOrEqual("amount", 600.0)
)
tasks = list(scan.plan_files())
print(f"Filter: amount >= 600.0")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 6 (July-Dec: amounts 700-1200)")
if len(tasks) == 6:
    print("✓ PASS - Correctly pruned to 6 files")
else:
    print(f"✗ FAIL - Expected 6 files, got {len(tasks)}")
...
print("="*60)
scan = table.scan(
    row_filter=EqualTo("user_id", "user_001_500")
)
tasks = list(scan.plan_files())
print(f"Filter: user_id = 'user_001_500'")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 1 (January only)")
if len(tasks) == 1:
    print("✓ PASS - Correctly pruned to 1 file")
else:
    print(f"✗ FAIL - Expected 1 file, got {len(tasks)}")
...
print("="*60)
scan = table.scan(
    row_filter=EqualTo("event_type", "purchase")
)
tasks = list(scan.plan_files())
print(f"Filter: event_type = 'purchase'")
print(f"Files selected: {len(tasks)}")
print(f"Expected: 3 (April, May, June have 'purchase')")
if len(tasks) == 3:
