...
print(f"✓ Created snapshot log entry")
...
for file_info in data_files_info:
    ...
    column_stats_firestore = {
        f"col_{col_id}": {
            "min": stats["min"] if not isinstance(stats["min"], bytes) else stats["min"].decode('utf-8'),
            "max": stats["max"] if not isinstance(stats["max"], bytes) else stats["max"].decode('utf-8'),
        }
...
from pyiceberg.types import (
    LongType, StringType, DateType, TimestampType, DoubleType, IntegerType
)
...
print(f"✓ Updated snapshot with parquet-manifest reference")
...
for file_info in data_files_info[:3]:  # Show first 3
    ...
    print(f"  Month {month:2d}: {min_date.date()} to {max_date.date()} ({file_info['record_count']:,} records)")
