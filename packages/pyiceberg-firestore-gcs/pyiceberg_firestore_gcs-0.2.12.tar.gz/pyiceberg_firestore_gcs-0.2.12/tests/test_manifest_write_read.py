...
def test_string_encoding():
    ...
    if negative_values:
        print(f"✗ Found negative int64 values (sign bit set):")
...
def test_pruning_logic(records):
    ...
    for i, df in enumerate(data_files):
        ...
        print(f"  {status} File {i+1}: min={min_val}, max={max_val}, prune={can_prune} (expected {expected_prune[i]})")
    ...
    expected_prune = [False, True, False]  # file1 has DEBUG-INFO, file3 has CRITICAL-ERROR (DEBUG is in between)
    for i, df in enumerate(data_files):
        min_val = df.lower_bounds.get(1)
        max_val = df.upper_bounds.get(1)
    ...
    print(f"  'INFO' as int64: {info_int}")
    print(f"  File2 has WARNING ({_value_to_int64(b'WARNING', string_type)}) > INFO, so overlaps range")
    ...
    print("  Note: OR predicates require conservative approach - only prune if ALL branches eliminate")
