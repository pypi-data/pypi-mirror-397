...
def Progress_Bar(simCount, totalParams, simStartTime, onCluster):
    """
    Functionality:
        Print a compact ETA/progress readout for long simulations when running locally.
        In terminals, uses a single-line update; in Jupyter, refreshes the output cell.
        Returns elapsed wall time regardless of environment.
    ...
    Returns:
        float: simElapsedTime (seconds) since simStartTime.
    """
    simPercentageComplete = 100.0 * (simCount + 1) / (totalParams if totalParams else 1)
    simElapsedTime = TIME.time() - simStartTime
    simTotalTime = simElapsedTime / (simPercentageComplete / 100.0) if simPercentageComplete > 0 else float("inf")
    ...
    if remainingTime < 60:
        ...
        msg = f"Remaining time (min): {remainingTime/60.0:.2f}, {int(simPercentageComplete)}% completed"
    elif remainingTime < 86400:
        msg = f"Remaining time (hrs): {remainingTime/3600.0:.2f}, {int(simPercentageComplete)}% completed"
    else:
        msg = f"Remaining time (days): {remainingTime/86400.0:.2f}, {int(simPercentageComplete)}% completed"

    # Notebook-safe rendering
    if _in_notebook():
        from IPython.display import clear_output
