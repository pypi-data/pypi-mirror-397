...
class TestFormats(unittest.TestCase):
    ...
    def test_colons_in_attributes(self):
        task = pytodotxt.Task("Strange task with:many:yes colons in attributes:here:yes,too")
        self.assertEqual(len(task.attributes), 2)
        self.assertIn("with", task.attributes)
        self.assertIn("attributes", task.attributes)
        self.assertEqual(task.attributes['with'], ['many:yes'])
        self.assertEqual(task.attributes['attributes'], ['here:yes,too'])
...
class TestSerializerDifferences(unittest.TestCase):
    def test_stored_serializer(self):
        text = "x 2016-10-03 Do something"
        task_pedantic = pytodotxt.Task(text)
        task_relaxed = pytodotxt.Task(text, serializer=pytodotxt.serialize_relaxed)

        self.assertEqual(task_pedantic.completion_date, datetime.date(2016, 10, 3))
        self.assertEqual(task_relaxed.completion_date, task_pedantic.completion_date)
        self.assertEqual(str(task_pedantic), "x Do something")
        self.assertEqual(str(task_relaxed), text)
        self.assertIs(task_relaxed.serializer, pytodotxt.serialize_relaxed)

    def test_ambiguous_single_date(self):
        text = 'x 2016-11-02 Decide how to parse this'
        task = pytodotxt.Task(text)

        self.assertEqual(task.completion_date, datetime.date(2016, 11, 2))

        # intentionally create an invalid task: completed, but only creation date
        task.creation_date = task.completion_date
        task.completion_date = None

        self.assertEqual(pytodotxt.serialize_pedantic(task),
                         "x Decide how to parse this")
        self.assertEqual(pytodotxt.serialize_relaxed(task),
                         "x Decide how to parse this")

    def test_completed_prioritized(self):
        task = pytodotxt.Task("x 2016-10-31 (A) 2014-08-13 Wait for October 2016")

        self.assertEqual(task.completion_date, datetime.date(2016, 10, 31))
        self.assertEqual(task.creation_date, datetime.date(2014, 8, 13))
        self.assertEqual(task.priority, 'A')

        self.assertEqual(pytodotxt.serialize_pedantic(task),
                         "x 2016-10-31 2014-08-13 Wait for October 2016")
        self.assertEqual(pytodotxt.serialize_relaxed(task),
                         "x 2016-10-31 (A) 2014-08-13 Wait for October 2016")
