...
def serialize_pedantic(task):
    """Serialize the Task ``task`` in a very strict manner

    This serialisation mechanism interpretes the todo.txt standard in a
    restricted manner, not allowing priority markers in completed tasks
    nor "completed on" dates if there is no "created on" date.

    The output of this format can still be parsed by pytodotxt."""
    result = []
    if task.is_completed:
        result.append('x')

        if task.completion_date is not None and \
           task.creation_date is not None:
            result.append(task.completion_date.strftime(type(task).DATE_FMT))

    if task.priority:
        if not task.is_completed:
            result.append(f'({task.priority})')

    if task.creation_date is not None and \
       (not task.is_completed or task.completion_date is not None):
        result.append(task.creation_date.strftime(type(task).DATE_FMT))
...
def serialize_relaxed(task):
    """Serialize the Task ``task`` in a relaxed manner

    This serialisation mechanism interpretes the todo.txt standard in a
    relaxed manner, allowing priority markers ("(A)") in completed tasks
    and "completed on" dates even if there is no "created on" date.
...
class Task:
    ...
    def __init__(self, line=None, linenr=None, todotxt=None, serializer=None):
        """Create a new task
        ...
        Note that attribute keys and values are split at the first ``:``. That
        means a task with ``a:b:c`` is understood to have the key ``a`` and the
        value ``b:c``.
        ...
        :param todotxt: the ``TodoTxt`` container to which this task belongs,
                        if any.
        :param serializer: What serializer funtion to use. Defaults to
                           ``serialize_pedantic``.
        ...
        self.serializer = serializer or serialize_pedantic
        """The serializer function to use."""
    ...
    def serialize(self):
        ...
        Calls the configured serializer, which by default will
        return a todo.txt-compatible string representation of this task."""
