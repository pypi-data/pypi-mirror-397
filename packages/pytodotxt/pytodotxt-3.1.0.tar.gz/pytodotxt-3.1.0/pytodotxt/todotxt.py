...
from .task import Task, serialize_pedantic
...
class TodoTxt:
    ...
    """
    def __init__(self, filename, encoding='utf-8', parser=None, serializer=None):
        ...
        :param serializer: The serializer that will be used when writing the
                           tasks back to disk. Defaults to ``serialize_pedantic``.
    ...
    def add(self, task):
        ...
        task.serializer = self.serializer
    ...
    def parse(self):
        ...
        Tasks will receive the same serializer as this TodoTxt instance.
    ...
    def save(self, target=None, safe=True, linesep=None):
        ...
        The ``self.serializer`` property will be used to serialize each
        individual task.
        ...
        if safe:
            tmpfile = tempfile.NamedTemporaryFile('wb',
                                                  buffering=0,
                                                  dir=pathlib.Path(target).parent,
                                                  delete=False,
                                                  prefix=".tmp",
                                                  suffix="~")
    ...
    def write_to_stream(self, stream, linesep=None):
        ...
        if linesep is None:
            linesep = self.linesep
        if len(self.lines) > 0:
            stream.write(bytes(linesep.join(self.lines) + linesep,
                               self.encoding))
    ...
    def build_lines(self):
        ...
        lines = [(task.linenr if task.linenr is not None else len(self.tasks),
                  self.serializer(task))
