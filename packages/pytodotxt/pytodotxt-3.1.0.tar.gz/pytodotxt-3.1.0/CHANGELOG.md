...
## 3.1.0
### Added
- `Task`'s method to serialize itself can be configured through the `serializer` parameter and property
- `TodoTxt` can be configured to use a different than the default serialization method (`serializer` parameter and property)
- Two serialization methods: `serialize_pedantic` and `serialize_relaxed`. The pedantic variant is the default and provides the same behaviour as previous versions.

The `serialize_relaxed` method uses a relaxed interpretation of the [`todo.txt`](https://github.com/todotxt/todo.txt)
