...
class BashSession:
    ...
    def pwd(self, state: 'ReplState'):
        ...
        with device_pods(state) as exec:
            return exec(command, action='bash', show_out=False)
...
def device_pods(state: ReplState):
    return DevicePodHandler(state)

class DevicePodHandler:
