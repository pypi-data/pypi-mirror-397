...
class CassandraClusters:
    def exec(sts: str, namespace: str, command: str, action: str = 'action',
        ...
        with Pods.parallel(pods, max_workers, samples, msg, action=action) as map:
            results: list[PodExecResult] = map(lambda pod: CassandraNodes.exec(pod, namespace, command, False, False, shell, background, log_file))
