...
from adam.commands.export.export_handlers import exporter
...
class ExportTables(Command):
    ...
    def run(self, cmd: str, state: ReplState):
        ...
        with self.validating(args, state, options=['--export-only']) as (args, state, export_only):
            with exporter(state, export_only=export_only) as exec:
                return exec(args)
