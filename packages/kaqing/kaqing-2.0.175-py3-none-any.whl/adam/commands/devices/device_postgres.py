...
class DevicePostgres(Command, Device):
    ...
    def show_pg_hosts(self, state: ReplState):
        if state.namespace:
            def line(p: str):
                pg = PostgresContext.apply(state.namespace, p)
                return f'{pg.path()},{pg.endpoint()}:{pg.port()},{pg.username()},{pg.password()}'

            tabulize(PostgresContext.hosts(state.namespace), line, header='NAME,ENDPOINT,USERNAME,PASSWORD', separator=',')
        else:
            def line(p: str):
                pg = PostgresContext.apply(state.namespace, p)
                return f'{pg.path()},{pg.namespace},{pg.endpoint()}:{pg.port()},{pg.username()},{pg.password()}'

            tabulize(PostgresContext.hosts(state.namespace), line, header='NAME,NAMESPACE,ENDPOINT,USERNAME,PASSWORD', separator=',')

    def show_pg_databases(self, pg: PostgresContext):
        tabulize(pg_database_names(pg.namespace, pg.path()), header='DATABASE', separator=',')

    def show_pg_tables(self, pg: PostgresContext):
        tabulize(pg_table_names(pg.namespace, pg.path()), header='NAME', separator=',')
