...
from adam.utils_k8s.k8s import cassandra
...
class DeviceCass(Command, Device):
    ...
    def show_table_preview(self, state: ReplState, table: str, rows: int):
        with cqlsh(state, show_out=True, use_single_quotes=True, on_any=True) as query:
    ...
    def exec_no_dir(self, command: str, state: ReplState):
        with cassandra(state) as exec:
            return exec(command, action='bash', show_out=True, shell='bash')

    def exec_with_dir(self, command: str, session_just_created: bool, state: ReplState):
        with cassandra(state) as exec:
            return exec(command, action='bash', show_out=not session_just_created, shell='bash')

    def bash_completion(self, cmd: str, state: ReplState, default: dict = {}):
        return {cmd: BashCompleter(lambda: [])} | \
               {f'@{p}': {cmd: BashCompleter(lambda: [])} for p in StatefulSets.pod_names(state.sts, state.namespace)}
