...
class ShowCassandraStatus(Command):
    ...
    def run(self, cmd: str, state: ReplState):
        ...
        with self.validating(args, state, options=['-s', '--show']) as (args, state, show_output):
            if state.namespace and state.pod:
                self.show_single_pod(state, show_output=show_output)
            elif state.namespace and state.sts:
                self.merge(state, Config().get('nodetool.samples', sys.maxsize), show_output=show_output)

            return state

    def show_single_pod(self, state: ReplState, show_output = False):
        try:
            with nodetool(state, show_out=False) as exec:
                result = exec('status')
                status = parse_nodetool_status(result.stdout)
                check_results = run_checks(cluster=state.sts, namespace=state.namespace, checks=[CompactionStats(), Gossip()], show_output=show_output)
    ...
    def merge(self, state: ReplState, samples: int, show_output=False):
        ...
        for pod_name in pod_names:
            ...
            try:
                with nodetool(state, pod=pod_name, show_out=False) as exec:
                    result = exec('status')
                    status = parse_nodetool_status(result.stdout)
                    if status:
