...
from adam.utils import tabulize, log, parallelize
...
class ExportSessions:
    ...
    def disply_export_session(sts: str, pod: str, namespace: str, session: str):
        ...
        log()
        tabulize([f'{table.keyspace}\t{table.table}\t{table.target_table}\t{"export_completed_pending_import" if table.status == "pending_import" else table.status}' for table in tables], header='KEYSPACE\tTABLE\tTARGET_TABLE\tSTATUS', separator='\t')