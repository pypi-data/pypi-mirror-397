...
def parallel(collection: list, max_workers: int = 0, samples = sys.maxsize, msg: str = None, collect = True):
    return ParallelMapHandler(collection, max_workers, samples = samples, msg = msg, collect = collect)

def offloaded(max_workers: int = 3, msg: str = None):
    return ParallelMapHandler(None, max_workers, msg = msg, collect=False)
