...
def setup_logging_middleware(app: FastAPI) -> None:
    ...
    async def logging_ctx(request: Request, call_next):
        ...
        try:
            ...
            request.state.logger.send(level="info", category="access", message=clean_path)
...
logger_client.send(level="info", category="service", message="애플리케이션 시작")
...
worker_logger.send(level="info", category="service", message="백그라운드 처리 중...")
```
