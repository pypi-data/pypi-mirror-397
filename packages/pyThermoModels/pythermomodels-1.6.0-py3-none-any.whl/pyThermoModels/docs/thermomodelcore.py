...
from ..activity import NRTL, UNIQUAC, UNIFAC
...
class ThermoModelCore(ThermoDB, ThermoLinkDB, ReferenceManager):
    ...
    def init_activities(
        ...
        **kwargs
        ...
        check_reference: bool
            check if reference is available for all components (default: True)

        Returns
        -------
        NRTL | UNIQUAC | UNIFAC: object
            NRTL, UNIQUAC, or UNIFAC object for the calculation of activity coefficient
        '''
        try:
            # SECTION: keywords
            # NOTE: kwargs
            ...
            # NOTE: mixture ids
            mixture_ids = kwargs.get('mixture_ids', None)
            ...
            # ! check reference (all components must have reference)
            if check_reference:
                reference = self._references.get(activity_model, None)
                ...
                if mixture_ids:
                    # check
                    if self.mixture_key not in mixture_ids.keys():
                        raise Exception(
                            f'Mixture key {self.mixture_key} not in mixture ids provided!')

                    # check
                    if 'Name' in mixture_ids.keys():
                        # append
                        components_.append(mixture_ids['Name'])

                    if 'Formula' in mixture_ids.keys():
                        # append
                        components_.append(mixture_ids['Formula'])

                # append
                components_.extend(components)
    ...
    def select_activities(
        ...
        **kwargs
        ...
        ----------
        components: list[Component]
            list of components as string. Each component is an instance of the `Component` class from `pythermodb-settings`.
        model_name: str
            activity coefficient model name as:
                - `NRTL`: Non-Random Two-Liquid,
                - `UNIQUAC`: Universal Quasi-Chemical
        ...
        check_reference: bool
            check if reference is available for all components (default: True)
        component_key: str
            component key to identify the component, as:
                - `Name-State`: component name with state, such as `carbon dioxide-g`
                - `Formula-State`: component formula with state, such as `CO2-g`
        ...
        try:
            ...
            # NOTE: component ids
            components_ids = kwargs.get('components_ids', None)
            ...
            if isinstance(components, list):
                ...
                # check all elements are Component instances
                if not all(isinstance(comp, Component) for comp in components):
                    raise Exception(
                        'All elements in components list must be instances of the Component class!')
            else:
                raise Exception('Components list not provided!')

            # NOTE: extract component ids
            components_name: list[str] = []
            components_formula: list[str] = []
            components_name_state: list[str] = []
            components_formula_state: list[str] = []
            ...
            if components_ids:
                ...
                if not isinstance(components_ids, dict):
                    raise Exception('components_ids must be a dictionary!')

                # check for Name
                if 'Name' in components_ids.keys():
                    components_name = components_ids['Name']

                # check for Formula
                if 'Formula' in components_ids.keys():
                    components_formula = components_ids['Formula']

                # check for Name-State
                if 'Name-State' in components_ids.keys():
                    components_name_state = components_ids['Name-State']

                # check for Formula-State
                if 'Formula-State' in components_ids.keys():
                    components_formula_state = components_ids['Formula-State']

                # set component ids
                component_ids: list[str] = components_name if component_key == 'Name-State' else components_formula
            ...
            if check_reference:
                ...
                # ! append
                # >> name-state
                components_.extend(components_name_state)
                # >> formula-state
                components_.extend(components_formula_state)
