...
if not isinstance(activity_nrtl, NRTL):
    raise TypeError(
        f"activity_nrtl is not NRTL, but {type(activity_nrtl)}")
...
# ! mat
dg_ij_mat = non_randomness_parameters.mat('dg', component_names=components)
print(type(dg_ij_mat))
print(dg_ij_mat)
...
# ! mat
alpha_ij_mat = non_randomness_parameters.mat(
    'alpha', component_names=components)
print(type(alpha_ij_mat))
print(alpha_ij_mat)
