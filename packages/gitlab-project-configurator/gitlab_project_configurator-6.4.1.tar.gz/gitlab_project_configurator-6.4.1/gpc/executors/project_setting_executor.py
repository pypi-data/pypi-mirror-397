...
class GitlabSettingExecutor(BaseSettingExecutor):
    ...
    def _apply(self):
        if not self.default_branch_updator:
            click.secho(
                "ERROR: DefaultBranchUpdator not found, cannot proceed with applying changes.",
                fg="red",
            )
            self.success = False
        elif not self.default_branch_updator.success:
            click.secho(
...
class DefaultBranchUpdator(LocalUpdator):
    ...
    def create_default_branch(self, branch_name):
        try:
            ...
            if exc.response_code == 500 or (
                isinstance(self.item, Project) and not self.item.commits.list(per_page=1)
            ):
