...
# =============================================================================
# DVT Compact Help Formatter (v0.58.8)
# =============================================================================
# Hides global options by default to make --help more readable.
# Global options are shown with --help-all or as a summary line.
# =============================================================================

# Global options that are hidden by default in help output
HIDDEN_GLOBAL_OPTIONS = {
    "--cache-selected-only", "--no-cache-selected-only",
    "--debug", "--no-debug", "-d",
    "--defer", "--no-defer",
    "--defer-state",
    "--fail-fast", "--no-fail-fast", "-x",
...
class CompactHelpFormatter(click.HelpFormatter):
    """Custom formatter that hides global options for cleaner help output."""

    def __init__(self, *args, show_all_options: bool = False, **kwargs):
        super().__init__(*args, **kwargs)
        self.show_all_options = show_all_options


class CompactCommand(click.Command):
    """Command with compact help that hides global options."""

    def format_options(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        """Writes all the options into the formatter, hiding globals."""
        opts = []
        hidden_count = 0
        ...
        for param in self.get_params(ctx):
            ...
            if rv is not None:
                # Check if this is a global option to hide
                param_opts = param.opts if hasattr(param, 'opts') else []
                is_hidden_global = any(opt in HIDDEN_GLOBAL_OPTIONS for opt in param_opts)

                if is_hidden_global and not getattr(formatter, 'show_all_options', False):
                    hidden_count += 1
                else:
                    opts.append(rv)
        ...
        # Add note about hidden options
        if hidden_count > 0:
            formatter.write_paragraph()
            formatter.write_text(
                f"  [{hidden_count} global options hidden. Use --help-all to see all options.]"
            )


class CompactGroup(click.Group):
    """Group with compact help that hides global options."""

    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        """Write the help into the formatter with commands shown."""
    ...
    def format_commands(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        """Format the commands in a compact way."""
        commands = []
        ...
        if commands:
            # Format commands in a compact way
...
@click.group(
    ...
    epilog="Run 'dvt COMMAND --help' for command-specific options.",
)
@click.pass_context
@global_flags
@p.show_resource_report
def cli(ctx, **kwargs):
    """DVT - Data Virtualization Tool for multi-source federation.

    DVT extends dbt with cross-database query federation, smart compute
    selection, and enhanced data profiling capabilities.

    \b
    Quick Start:
      dvt init my_project     Create new DVT project
      dvt run                 Execute models
      dvt target list         List database connections
      dvt compute list        List compute engines

    \b
    For dbt users: All dbt commands work as expected (run, test, build, etc.)
    """
...
# dbt docs
@cli.group(cls=CompactGroup)
@click.pass_context
@global_flags
def docs(ctx, **kwargs):
    """Generate or serve project documentation."""
...
def source(ctx, **kwargs):
    """Manage project sources (freshness checks)."""
...
@cli.group("metadata", cls=CompactGroup)
def metadata():
    """Manage DVT metadata store for type mapping.

    \b
    Commands:
      reset        Clear all metadata
      snapshot     Capture source/model metadata
      export       Display in CLI
...
@cli.group("profile", cls=CompactGroup)
@click.pass_context
@global_flags
def profile(ctx, **kwargs):
    """Profile data sources and models.

    \b
    Commands:
      run    Profile sources/models
      serve  Start profile viewer UI

    \b
    Examples:
      dvt profile run                 # Profile all
      dvt profile run --sample 10000  # Sample 10K rows
      dvt profile serve               # Web UI on :8580
    """
    pass
...
def compute(ctx, **kwargs):
    """Manage compute engines (Spark, DuckDB).

    \b
    Commands:
      list      List configured engines
      test      Test engine connectivity
      edit      Open computes.yml
      validate  Check configuration

    \b
    Examples:
      dvt compute list               # List engines
      dvt compute test spark-local   # Test Spark
    """
...
def target(ctx, **kwargs):
    """Manage database connections in profiles.yml.

    \b
    Commands:
      list    List available targets
      test    Test connection(s)
      add     Add new target
      remove  Remove target
      sync    Sync adapters/JDBC JARs

    \b
    Examples:
      dvt target list       # List targets
      dvt target test       # Test all
      dvt target test dev   # Test one
    """
...
@p.version
def java(ctx, **kwargs):
    """Manage Java for PySpark compatibility.

    \b
    Commands:
      check    Check Java/PySpark compatibility
      search   Find Java installations
      set      Configure JAVA_HOME
      install  Installation guide

    \b
    Compatibility:
      PySpark 4.0 -> Java 17/21
      PySpark 3.5 -> Java 8/11/17
    """
...
def spark(ctx, **kwargs):
    """Manage PySpark installations.

    \b
    Commands:
      check          Check PySpark/Java status
      set-version    Install PySpark version
      match-cluster  Match to cluster version
      versions       Show compatibility matrix

    \b
    Examples:
      dvt spark check         # Status
      dvt spark set-version   # Install
    """
