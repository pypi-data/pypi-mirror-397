//! Shared test utilities for ranex-atlas integration tests.
...
pub fn create_python_project() -> Result<TempDir, Box<dyn Error>> {
...
pub fn create_sample_project() -> Result<TempDir, Box<dyn Error>> {
...
pub fn create_project_with_error() -> Result<TempDir, Box<dyn Error>> {
    let project = create_python_project()?;

    create_python_file(&project, "app/good.py", "def valid(): pass")?;
    create_python_file(&project, "app/bad.py", "def broken(\n")?; // Syntax error
