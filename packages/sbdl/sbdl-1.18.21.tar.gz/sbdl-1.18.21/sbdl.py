...
_BK='sbdl_warn_new_config'
_BJ='sbdl_update_package'
_BI='sbdl_hash_check'
_BH='sbdl_hash_length'
_BG='sbdl_ast_pre_hooks'
_BF='sbdl_attribute_passthrough'
_BE='sbdl_file_extension'
_BD='sbdl_output_reference'
_BC='sbdl_element_color_map'
_BB='sbdl_grammar_write_statement_separator'
_BA='sbdl_grammar_write_attribute_separator'
_B9='sbdl_grammar_custom_comment'
_B8='sbdl_grammar_custom_noop'
_B7='sbdl_grammar_custom_end'
_B6='sbdl_grammar_custom_begin'
_B5='sbdl_grammar_custom_suffix'
_B4='sbdl_grammar_custom_prefix'
_B3='rocket_launch'
_B2='launch_protocol'
_B1='system_requirement_1'
_B0='rocket_steering'
_A_='rocket_system'
_Az='test/function_test.sbdl'
_Ay='from:yaml-tree'
_Ax='usecase-diagram'
_Aw='state-diagram'
_Av='function-process-diagram'
_Au='function-diagram'
_At='network-diagram'
_As='requirement-diagram'
_Ar='element-diagram'
_Aq='aspect-diagram'
_Ap='from:openfmea'
_Ao='openfmea-portfolio'
_An='yaml-tree'
_Am='compile'
_Al='format'
_Ak='<<{}>>'
...
def set_config_value(key,data,warn_on_new=_A):
	if not config_key_locked(key):
		if not key in _CONFIG_DATA and get_config_value(_BK,number=_A)and warn_on_new:f_print('WARNING: adding new key ({}) to config registry'.format(key),warning=_A)
	...
	def attribute_string(self,statement_object):
		...
		for attr in sorted(statement_object.data()):
			...
			if len(content)>0:
				...
				attributes+='{indent}{attrname}{assign}{delimiter}{attrcontent}{delimiter}{separator}{newline}{spacing}'.format(attrname=attr,assign=SBDL_Parser.Tokens.declaration_attribute_assign,delimiter=delim,attrcontent=content,separator=SBDL_Parser.Tokens.declaration_attribute_separator if get_config_value(_BA,number=_A)else'',indent=indent_str,newline=newline_str,spacing=spacing_str)
		return attributes
	@classmethod
...
class SBDL_Semantics:
	...
	def get_language_information_struct(self):
		...
		lang_info_struct[_v]=self.property_descriptions;lang_info_struct[_Ai]=dict.fromkeys([x for x in vars(SBDL_Parser.Macros).values()if isinstance(x,str)and x!=_j and x!=_Aj],'No Description')
	...
	def type_color(self,type_name):
		E='#FFFF00';D='#FF6347';C='#FF69B4';B='#9370DB';A='#1E90FF';color_list=['#FFC0CB','#FFA07A','#FFD700','#32CD32','#00FFFF',A,B,C,D,E,'#008000','#00FF7F','#00CED1','#00BFFF',A,B,C,D,'#FF4500',E,'#9ACD32','#ADFF2F','#7FFFD4','#AFEEEE','#ADD8E6','#87CEEB','#8A2BE2','#FF00FF','#FF1493'];color_index_map=get_config_value(_BC)
...
class SBDL_Element:
	...
	def hash(self):return hashlib.sha256(re.sub('\\s+',_I,self.definition().parser_element().content().strip()).encode()).hexdigest()[:get_config_value(_BH,number=_A)]
	@classmethod
...
def get_parser_elements_from_filepath(filepath,elements_list,print_l=print_null,file_opener=open_input_file,read_input_set=_B):
	...
	file_name=os.path.basename(filepath);file_ext=os.path.splitext(filepath)[1:][0];dir_name=filepath.replace(file_name,'');abs_path=os.path.abspath(filepath);native_sbdl_file=_A if get_config_value(_BE)==file_ext else _C;update_per_file_directives()
	...
	def import_directive(_,*inp):
		...
		try:
			print_l('Import: {}'.format(import_path));read_elements,import_errors=aggregate_all_parser_elements_from_files([import_path],do_recurse=get_config_value(_BL,number=_A),do_hidden=_C,print_l=print_l,file_opener=file_opener,read_input_set=read_input_set);elements_list.extend(read_elements);errors+=import_errors
	...
	def strip_comments(line):
		result=line;comment_markers=[SBDL_Parser.Tokens.comment,get_config_value(_B9)]
	...
	def detect_block_start_end(line):
		...
		if re.search(SBDL_Parser.Tokens.prefix_block_start,line)or re.search(get_config_value(_B6),line):native_sbdl_file=_A;result=_A
		elif re.search(SBDL_Parser.Tokens.prefix_block_end,line)or re.search(get_config_value(_B7),line):native_sbdl_file=_C;result=_A
		return result
	def extract_sbdl_line_non_native(line):
		nonlocal non_sbdl_line_prefix;result='';custom_prefix=get_config_value(_B4);custom_suffix=get_config_value(_B5);custom_noop=get_config_value(_B8)
	...
	def generate_sbdl_table_from_ast(self,sbdl_ast,id_in_body=_C,source_in_body=_C,print_l=print_null,multi_field_separator=_B):
		...
		def generate_table(element,row=_B,path=_B,add_single=_C):
			...
			if not element.identifier()in path:
				path[element.identifier()]=_A;custom_prop_prefix=SBDL_Parser.Attributes.custom_prefix+get_config_value(_BF)
...
def write_aggregated_output(sbdl_ast,output_file,file_opener,__,print_l):
	...
	with file_opener(output_file)as fmfile:
		...
		for element in sbdl_elements:
			if not get_config_value(_BD,number=_A):element.delete_property(SBDL_Parser.Attributes.reference)
			print_l('Writing element: {}'.format(element.identifier()),debug=_A);fmfile.write('{}{} {}'.format(element.definition().string(),SBDL_Parser.Tokens.statement_separator if get_config_value(_BB,number=_A)else'',new_line_str))
	return errors
...
def update_self(print_l,force=_C):
	print_l(f"Updating {__NAME}...");ret,stdout,stderr=exec_external([sys.executable,_F,'pip','install',*(['--trusted-host',f"{urllib.parse.urlparse(__URL).netloc}"]if force else[]),'--upgrade',urllib.parse.urljoin(url(),get_config_value(_BJ)),*(['--break-system-packages']if force else[])]);print_l(stdout)
...
def get_cache_file_path(input_file):
	cache_file_path=_B
	if input_file!=SBDL_Parser.Tokens.stdio_name:cache_file_path=f"{input_file}{get_config_value(_BN)}"
...
class LanguageLevelTests(unittest.TestCase):
	...
	def testFunction(self):main(handle_arguments([_Az,_E,self.TEST_OUTPUT_FILE]))
	...
	def testGeneralElementDiagram(self):main(handle_arguments([_J,_E,self.TEST_DIAGRAM_OUTPUT_FILE,_F,_Ar]))
	def testGeneralFunctionDiagram(self):main(handle_arguments([_Az,_E,self.TEST_DIAGRAM_OUTPUT_FILE,_F,_Au]))
	def testGeneralProcessDiagram(self):main(handle_arguments([_Az,_E,self.TEST_DIAGRAM_OUTPUT_FILE,_F,_Av]))
	def testGeneralStateDiagram(self):main(handle_arguments([_J,_E,self.TEST_DIAGRAM_OUTPUT_FILE,_F,_Aw]))
