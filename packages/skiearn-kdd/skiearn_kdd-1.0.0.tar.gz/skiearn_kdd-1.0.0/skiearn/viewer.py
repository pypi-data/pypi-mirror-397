...
def print_menu():
    ...
    print("=" * 80)
    print(" " * 20 + "SKIEARN DOCUMENTATION VIEWER")
    print(" " * 15 + "Knowledge Discovery & Data Mining Study Guide")
    print("=" * 80)
    print()
    ...
    for key in ['r', 's', 'v']:
        doc = DOCS[key]
        print(f"  [{key.upper()}] {doc['title']}")
        print(f"      └─ {doc['topics']}")
        print()
    
    print("=" * 80)
    print("  [A] View ALL files sequentially")
    print("  [Q] Quit")
    print("=" * 80)
...
def view_all_files():
    ...
    clear_screen()
    print("📚 Viewing ALL documentation files...\n")
    
    for key in sorted([k for k in DOCS.keys() if k.isdigit()], key=int):
        doc = DOCS[key]
        print("\n" + "=" * 80)
        print(f" [{key}] {doc['title']}")
        print("=" * 80)
        print(f"File: {doc['file']}")
        print("=" * 80)
        print()
    ...
    try:
        while True:
            ...
            if choice == 'q':
                ...
                print("Good luck with your KDD exam! 🎓\n")
                break
