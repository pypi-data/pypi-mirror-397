...
from tf_keras.layers import Conv2D, SeparableConv2D, Dense, ReLU, Layer
...
def quantize(model,
    ...
    Args:
        model (keras.Model): a standard Keras model
    ...
    Returns:
        keras.Model: a quantized Keras model
    """
...
def _convert_to_quantized_layer(layer, bitwidth, quantizer_function=None):
    ...
    Args:
        layer (keras.Layer): a standard Keras (Conv2D, SeparableConv2D or
...
def _quantize_activation_layer(layer, bitwidth):
    ...
    Args:
        layer (keras.Layer): an activation layer (ReLU or CNN2SNN quantized
            activation)
