...
class MaximOpenAIRealtimeConnection(RealtimeConnection):
    def __init__(
        ...
        class MaximInputAudioBuffer(RealtimeInputAudioBufferResource):
            ...
            def append(self, *, audio: str, **kwargs) -> None:  # type: ignore
                ...
                try:
                    ...
                    scribe().warning(
                        f"[MaximSDK][MaximOpenAIRealtimeConnection] Error capturing user audio: {str(e)}"
                    )
                # Call parent to actually send the audio
                super().append(audio=audio, **kwargs)

            def commit(self, **kwargs) -> None:  # type: ignore
    ...
    def _handle_event(self, event: RealtimeServerEvent) -> None:
        ...
        try:
            ...
            if event_type == "session.created":
                ...
                self._handle_response_output_audio_delta(event)
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling event: {str(e)}"
            )
    ...
    def _handle_session_updated(self, event: SessionUpdatedEvent) -> None:
        ...
        try:
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling session.updated: {str(e)}"
            )
    ...
    def _handle_conversation_item_input_audio_transcription_completed(
        ...
        try:
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling conversation.item.input_audio_transcription.completed: {str(e)}"
            )
                ...
                if call_id and output is not None:
                    ...
                    if call_id in self._tool_calls:
                        try:
                            ...
                            scribe().warning(
                                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error setting tool call result: {str(e)}"
                            )
                    else:
                        try:
                            ...
                            scribe().warning(
                                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error setting tool call result via logger: {str(e)}"
                            )

        except Exception as e:
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling conversation.item.added: {str(e)}"
            )
    ...
    def _handle_response_created(self, event: ResponseCreatedEvent) -> None:
        ...
        try:
            ...
            try:
                ...
                scribe().warning(
                    f"[MaximSDK][MaximOpenAIRealtimeConnection] Error creating generation: {str(e)}"
                )
        except Exception as e:
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling response.created: {str(e)}"
            )

    def _handle_response_output_audio_delta(
        ...
        try:
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling response.output_audio.delta: {str(e)}"
            )
    ...
    def _handle_function_call_arguments_delta(self, event: Any) -> None:
        ...
        try:
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling function call arguments delta: {str(e)}"
            )
        ...
        try:
            ...
            if self._current_trace is not None:
                try:
                    ...
                    scribe().warning(
                        f"[MaximSDK][MaximOpenAIRealtimeConnection] Error creating tool call: {str(e)}"
                    )
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling function call arguments done: {str(e)}"
            )
        ...
        try:
            ...
            if self._current_generation is not None:
                try:
                    ...
                    scribe().warning(
                        f"[MaximSDK][MaximOpenAIRealtimeConnection] Error logging generation result: {str(e)}"
                    )
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling response.done: {str(e)}"
            )

    def _handle_conversation_item_deleted(self, _event: Any) -> None:
        ...
        try:
            ...
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling conversation.item.deleted: {str(e)}"
            )
        ...
        try:
            ...
            if self._current_generation is not None:
                try:
                    ...
                    scribe().warning(
                        f"[MaximSDK][MaximOpenAIRealtimeConnection] Error logging generation error: {str(e)}"
                    )
        except Exception as e:
            scribe().warning(
                f"[MaximSDK][MaximOpenAIRealtimeConnection] Error handling error event: {str(e)}"
            )


class MaximOpenAIRealtimeConnectionManager(RealtimeConnectionManager):
    def __init__(self, client: OpenAI, logger: Logger, **kwargs):
        super().__init__(client=client, **kwargs)
        self._logger = logger
        self._extra_headers = kwargs.get("extra_headers", {})

    def __enter__(self) -> MaximOpenAIRealtimeConnection:
        """
        Override to return MaximOpenAIRealtimeConnection instead of base connection.
        """
        base_connection = super().__enter__()
...
class MaximOpenAIRealtime(Realtime):
    ...
    def connect(
        ...
        *,
        ...
        """
        Override to return MaximOpenAIRealtimeConnectionManager instead of base manager.
        """
        return MaximOpenAIRealtimeConnectionManager(
            client=self._client,
            logger=self._logger,
            call_id=call_id,
            model=model,
