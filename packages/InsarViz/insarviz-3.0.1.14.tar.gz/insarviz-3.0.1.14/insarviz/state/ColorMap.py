...
class ColorMap(ObservableStruct):
    name       = dynamic.variable()
    xzero      = dynamic.variable()
    xone       = dynamic.variable()
    image      = dynamic.variable()
    is_cyclic  = dynamic.variable()

    def __init__(self, name, image, xzero = 0.0, xone = 1.0, is_cyclic = False):
        super().__init__()
        self.name = name
        self.xzero = xzero
        self.xone = xone
    ...
    def from_dict(cls, dct, /, **kwargs):
        return cls(np.array(dct['image']), xzero = dct['xzero'], xone = dct["xone"], is_cyclic = dct['is_cyclic'])
    def init_from_dict(self, dct):
        self.name = dct['name']
        self.xzero = dct['xzero']
        self.xone = dct['xone']
        if "image" in dct:
            self.is_cyclic = dct['is_cyclic']
            self.image = np.array(dct['image'])
        else:
            if self.name.startswith("cyclic "):
                name = self.name.removeprefix("cyclic ")
                is_cyclic = True
            else:
                name = self.name
                is_cyclic = False
            self.is_cyclic = is_cyclic
            self.image = np.array(colormaps[name])*255.0

    def to_dict(self):
        return {
            'name': self.name,
            'xzero': float(self.xzero),
            'xone': float(self.xone),
            'is_cyclic': self.is_cyclic
        }

    @dynamic.method(SELF.image, SELF.is_cyclic)
    def createTexture(self):
        w, _ = self.image.shape
        ret = Qt.QOpenGLTexture(Qt.QOpenGLTexture.Target.Target1D)
        ret.setFormat(Qt.QOpenGLTexture.TextureFormat.RGB32F)
        ret.setWrapMode(Qt.QOpenGLTexture.WrapMode.Repeat if self.is_cyclic else Qt.QOpenGLTexture.WrapMode.ClampToEdge)
        ret.setSize(w)
        ret.allocateStorage(Qt.QOpenGLTexture.PixelFormat.RGB, Qt.QOpenGLTexture.PixelType.Float32)
        image_data = self.image.astype(np.float32) / 255.0
        ret.setData(Qt.QOpenGLTexture.PixelFormat.RGB, Qt.QOpenGLTexture.PixelType.Float32, image_data)
        ret.generateMipMaps()
        ret.create()
        return ret

    @dynamic.memo(SELF.xone, SELF.xzero)
    def value_to_color(self):
        scale = max(0.001, self.xone - self.xzero)
        return Matrix.product(
            Matrix.scale((1.0/scale, 1.0)),
            Matrix.translate((-self.xzero,))
        )

    def interpolated(self, width):
        data = np.array(self.image)
        l, _three = data.shape
        lin = np.linspace(0.0,l-1,num=width)
        rng = np.arange(l)
        data = np.stack([
           np.interp(lin, rng, data[:,0]),
           np.interp(lin, rng, data[:,1]),
           np.interp(lin, rng, data[:,2]),
        ], axis=-1)
        return data
...
colormaps['vik_inv'] = colormaps['vik'][::-1]
...
for name in gradients_names:
    colormap = pyqtgraph.colormap.ColorMap(*zip(*Gradients[name]["ticks"]))
    colormaps[name] = colormap.getLookupTable(nPts=256)
