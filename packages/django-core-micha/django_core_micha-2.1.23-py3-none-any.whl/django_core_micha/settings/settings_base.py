...
IS_EDGE = ORIGIN_SERVER_ID.upper().startswith("EDGE")
IS_MASTER = IS_PRODUCTION and ORIGIN_SERVER_ID.upper().startswith("MASTER")

SYNC_SHARED_SECRET = env("SYNC_SHARED_SECRET", default="change-me-in-prod")

MASTER_BASE_URL = env("MASTER_BASE_URL", default=None)

logger.info("Django Core Micha Base Settings loaded.")
logger.info(f"  ENV_TYPE={ENV_TYPE}, DEBUG={DEBUG}, ORIGIN_SERVER_ID={ORIGIN_SERVER_ID}")
logger.info(f"  IS_PRODUCTION={IS_PRODUCTION}, IS_STAGING={IS_STAGING}, IS_LOCAL={IS_LOCAL}")
logger.info(f"  IS_MASTER={IS_MASTER}, IS_EDGE={IS_EDGE}")
logger.info(f"  MASTER_BASE_URL={MASTER_BASE_URL}")
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        "invite_anon": "3/hour",
        "invite_admin": "50/hour",
        "access_code_validate": "60/hour",
    },
...
if "django.middleware.locale.LocaleMiddleware" not in MIDDLEWARE:
    try:
        # Suche Index der SessionMiddleware
        _session_idx = MIDDLEWARE.index("django.contrib.sessions.middleware.SessionMiddleware")
        # Füge LocaleMiddleware direkt danach ein
        MIDDLEWARE.insert(_session_idx + 1, "django.middleware.locale.LocaleMiddleware")
    except ValueError:
        # Fallback, falls SessionMiddleware fehlt (sollte nicht passieren)
        MIDDLEWARE.append("django.middleware.locale.LocaleMiddleware")

# Optional: Zur Kontrolle im Log ausgeben (nur bei Debug, um Spam zu vermeiden)
if DEBUG:
    import logging
    logging.getLogger("backend").debug(f"Middleware config updated: {MIDDLEWARE}")