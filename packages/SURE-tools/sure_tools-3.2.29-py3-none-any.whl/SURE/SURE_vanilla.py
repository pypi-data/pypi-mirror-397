...
class SUREVanilla(nn.Module):
    """SUccinct REpresentation of single-omics cells without codebook
    ...
    def __init__(self,
                 ...
                 flow_hidden_layers = [128,128,128],
    ...
    def setup_networks(self):
        ...
        self.encoder_zn = zuko.flows.NSF(features=self.latent_dim, context=self.input_size, 
    ...
    def model(self, xs, cs=None):
        ...
        with pyro.plate('data'):
            zn_loc = torch.zeros(batch_size, self.latent_dim, **self.options)
            zn_scale = torch.ones(batch_size, self.latent_dim, **self.options)
            if self.latent_dist == 'studentt':
                zn_dof = torch.ones(batch_size, self.latent_dim, **self.options)
            ...
            if (self.cov_size>0) and (cs is not None):
                zus = self.covariate_effect([zns,cs])
                zs = zns+zus
            else:
                zs = zns
...
def main():
    ...
    if args.save_model is not None:
        if args.save_model.endswith('gz'):
            SUREVanilla.save_model(sure, args.save_model, compression=True)
        else:
            SUREVanilla.save_model(sure, args.save_model)
