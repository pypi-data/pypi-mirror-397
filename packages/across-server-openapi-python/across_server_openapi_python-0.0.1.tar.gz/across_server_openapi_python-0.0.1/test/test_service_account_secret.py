# coding: utf-8

"""
    FastAPI

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 0.2.1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from across.sdk.v2.models.service_account_secret import ServiceAccountSecret

class TestServiceAccountSecret(unittest.TestCase):
    """ServiceAccountSecret unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ServiceAccountSecret:
        """Test ServiceAccountSecret
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ServiceAccountSecret`
        """
        model = ServiceAccountSecret()
        if include_optional:
            return ServiceAccountSecret(
                id = '',
                user_id = '',
                name = '',
                description = '',
                expiration = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                expiration_duration = 56,
                group_roles = [
                    across.sdk.v2.models.group_role.GroupRole(
                        id = '', 
                        name = '', 
                        permissions = [
                            across.sdk.v2.models.permission.Permission(
                                id = '', 
                                name = '', )
                            ], )
                    ],
                secret_key = ''
            )
        else:
            return ServiceAccountSecret(
                id = '',
                user_id = '',
                name = '',
                description = '',
                expiration = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                expiration_duration = 56,
                group_roles = [
                    across.sdk.v2.models.group_role.GroupRole(
                        id = '', 
                        name = '', 
                        permissions = [
                            across.sdk.v2.models.permission.Permission(
                                id = '', 
                                name = '', )
                            ], )
                    ],
        )
        """

    def testServiceAccountSecret(self):
        """Test ServiceAccountSecret"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
