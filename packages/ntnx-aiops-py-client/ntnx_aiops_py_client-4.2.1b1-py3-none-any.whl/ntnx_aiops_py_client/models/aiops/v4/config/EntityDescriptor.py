# coding: utf-8


"""
IGNORE:
    Nutanix AIOps APIs

    Manage infrastructure on-premises and in the cloud seamlessly through AIOps features such as Analysis, Reporting, Capacity Planning, What if Analysis, VM Rightsizing, Troubleshooting, App Discovery, Broad Observability, and Ops Automation through Playbooks.  # noqa: E501

    OpenAPI spec version: 4.2.1-beta-1
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
IGNORE
"""
import pprint
import json
import ast
import re  # noqa: F401

import six
from pathlib import Path
from ntnx_aiops_py_client.models.aiops.v4.config.MetricDescriptor import MetricDescriptor  # noqa: F401,E501
from ntnx_aiops_py_client.models.common.v1.response.ApiLink import ApiLink  # noqa: F401,E501
from ntnx_aiops_py_client.models.common.v1.response.ExternalizableAbstractModel import ExternalizableAbstractModel  # noqa: F401,E501

class EntityDescriptor(ExternalizableAbstractModel):

    """

    :param entity_type: (:attr:`entity_type`) Entity type of the data supported for a given source. For example VM, cluster etc.
    :type entity_type: 
    
    
    :param source: (:attr:`source`) Source name for the vendors. For example 'Nutanix'.
    :type source: 
    
    
    :param display_name: (:attr:`display_name`) Readable name for the entity.
    :type display_name: 
     max=50 chars
    
    :param parents: (:attr:`parents`) Parent entity types for the given entity.
    :type parents: 
    
    
    :param metrics: (:attr:`metrics`) Metrics data for the entity.
    :type metrics: 
    
    

    """
    """
    IGNORE:
    Attributes:
      swagger_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    IGNORE
    """  # noqa: E501
    swagger_types = {
        'entity_type': 'str',
        'source': 'str',
        'display_name': 'str',
        'parents': 'list[aiops.v4.config.EntityDescriptor]',
        'metrics': 'list[aiops.v4.config.MetricDescriptor]',
        'ext_id': 'str',
        'links': 'list[common.v1.response.ApiLink]',
        'tenant_id': 'str',
        '_reserved': 'dict(str, object)',
        '_object_type': 'str',
        '_unknown_fields': 'dict(str, object)',
    }

    attribute_map = {
        'entity_type': 'entityType',
        'source': 'source',
        'display_name': 'displayName',
        'parents': 'parents',
        'metrics': 'metrics',
        'ext_id': 'extId',
        'links': 'links',
        'tenant_id': 'tenantId',
        '_reserved': '$reserved',
        '_object_type': '$objectType',
        '_unknown_fields': '$unknownFields',
    }



    def __init__(self, entity_type=None, source=None, display_name=None, parents=None, metrics=None, ext_id=None, links=None, tenant_id=None, *args, **kwargs):  # noqa: E501
        ExternalizableAbstractModel.__init__(self, ext_id, links, tenant_id, *args, **kwargs)
        self.__entity_type = None
        self.__source = None
        self.__display_name = None
        self.__parents = None
        self.__metrics = None
        self.discriminator = None
        if entity_type is not None:
            self.__entity_type = entity_type
        if source is not None:
            self.__source = source
        if display_name is not None:
            self.__display_name = display_name
        if parents is not None:
            self.__parents = parents
        if metrics is not None:
            self.__metrics = metrics

    def _initialize_object_type(self):
        return 'aiops.v4.config.EntityDescriptor'

    def _initialize_object_version(self):
        return 'v4.r2.b1'


    @property
    def entity_type(self):
        """
        Entity type of the data supported for a given source. For example VM, cluster etc.

        :type:
                :class:`~str`
        """  # noqa: E501
        return self.__entity_type

    @entity_type.setter
    def entity_type(self, entity_type):

        self.__entity_type = entity_type

    @property
    def source(self):
        """
        Source name for the vendors. For example 'Nutanix'.

        :type:
                :class:`~str`
        """  # noqa: E501
        return self.__source

    @source.setter
    def source(self, source):

        self.__source = source

    @property
    def display_name(self):
        """
        Readable name for the entity.

        :type:
                :class:`~str`
        """  # noqa: E501
        return self.__display_name

    @display_name.setter
    def display_name(self, display_name):
        if display_name is not None and len(display_name) > 50:
            raise ValueError("Invalid value for `display_name`, length must be less than or equal to `50`")  # noqa: E501

        self.__display_name = display_name

    @property
    def parents(self):
        """
        Parent entity types for the given entity.

        :type:
             list[ :class:`~ntnx_aiops_py_client.models.aiops.v4.config.EntityDescriptor` ]
        """  # noqa: E501
        return self.__parents

    @parents.setter
    def parents(self, parents):

        self.__parents = parents

    @property
    def metrics(self):
        """
        Metrics data for the entity.

        :type:
             list[ :class:`~ntnx_aiops_py_client.models.aiops.v4.config.MetricDescriptor` ]
        """  # noqa: E501
        return self.__metrics

    @metrics.setter
    def metrics(self, metrics):

        self.__metrics = metrics

    def _to_dict(self, sanitize):
        """Returns the model properties as a dict. Omits None properties based on the provided "sanitize" parameter.

        :param sanitize: A flag to omit None properties if set to True
        :type sanitize: bool
        """

        result = {}
        for attr, attr_type in six.iteritems(self.swagger_types):

            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x._to_dict(sanitize) if hasattr(x, "_to_dict") else x,
                    value
                ))
            elif hasattr(value, "_to_dict"):
                result[attr] = value._to_dict(sanitize)
            elif isinstance(value, Path):
                result[attr] = str(value)
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1]._to_dict(sanitize)) if hasattr(item[1], "_to_dict")
                    else (
                        (item[0], str(item[1])) if item[0] == 'path' and '$objectType' in value and value['$objectType'] == 'pathlib.Path'
                        else item
                    ),
                    six.iteritems(value)
                ))
            else:
                if sanitize:
                    if value is not None:
                        result[attr] = value
                else:
                    result[attr] = value
        if issubclass(EntityDescriptor, dict):
            for key, value in six.iteritems(self):
                result[key] = value

        # handle unknown fields
        result['_unknown_fields'] = {}
        unknown = self.get_unknown_fields()
        if sanitize:
            unknown = self._sanitize_for_serialization(unknown)

        return {**result, **unknown}


    def to_dict(self):
        """Returns the model properties as a dictionary"""
        return self._to_dict(False)

    def to_str(self):
        """Returns the string representation of the model"""
        return pprint.pformat(self._to_dict(True))

    def __repr__(self):
        """For `print` and `pprint`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, EntityDescriptor):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other

