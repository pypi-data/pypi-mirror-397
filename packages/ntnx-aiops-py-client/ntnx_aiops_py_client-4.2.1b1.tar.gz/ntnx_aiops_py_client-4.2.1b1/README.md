# Python Client For Nutanix AIOps APIs

The Python client for Nutanix AIOps APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage infrastructure on-premises and in the cloud seamlessly through AIOps features such as Analysis, Reporting, Capacity Planning, What if Analysis, VM Rightsizing, Troubleshooting, App Discovery, Broad Observability, and Ops Automation through Playbooks.
## Features
...
api_client = ntnx_aiops_py_client.ApiClient(allow_version_negotiation=False)
```
...
## Configuration
The python client for Nutanix AIOps APIs can be configured with the following parameters
...
```python
# Initialize the API
capacity_reservation_api_instance = CapacityReservationApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Fetches the current global config of capacity reservation
try:
    api_response = capacity_reservation_api_instance.get_capacity_reservation_by_id(extId)
...
# Updates capacity reservation across all clusters.
try:
    # The body parameter in the following operation is received from the previous GET request's response which needs to be updated.
    api_response = capacity_reservation_api_instance.update_capacity_reservation_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=aiops&version=v4.2.b1&language=python). This documentation is auto-generated, and the location may change.
