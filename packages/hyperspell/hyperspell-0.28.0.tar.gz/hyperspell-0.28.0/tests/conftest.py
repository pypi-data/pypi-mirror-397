...
from hyperspell import Hyperspell, AsyncHyperspell, DefaultAioHttpClient
from hyperspell._utils import is_dict
...
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncHyperspell]:
    ...
    async with AsyncHyperspell(
        base_url=base_url, api_key=api_key, user_id=user_id, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
