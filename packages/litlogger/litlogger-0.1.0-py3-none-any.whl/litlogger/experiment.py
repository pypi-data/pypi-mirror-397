...
from litlogger.pty import rerun_in_pty_and_record
...
class Experiment:
    ...
    def __init__(
        ...
        if self.save_logs and os.environ.get("_IN_PTY_RECORDER") != "1":
            ...
            rerun_in_pty_and_record(self.terminal_logs_path)
            sys.exit(0)
    ...
    def log_files(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                ...
                for path, remote in zip(paths, remote_paths)
            }
    ...
    def get_file(self, path: str, remote_path: Optional[str] = None, verbose: bool = True) -> str:
        ...
        if verbose:
            print(f"✓ Retrieved file to {downloaded_path}")
        return downloaded_path

    def log_model_artifact(self, path: str, verbose: bool = False, version: Optional[str] = None) -> None:
        ...
        model_artifact.log()
        if verbose:
            print(f"✓ Logged model artifact {path}")
    ...
    def get_model_artifact(self, path: str, verbose: bool = False, version: Optional[str] = None) -> str:
        ...
        if verbose:
            print(f"✓ Retrieved model artifact to {path}")
        return result
    ...
    def get_model(self, staging_dir: Optional[str] = None, verbose: bool = False, version: Optional[str] = None) -> Any:
        ...
        if verbose:
            print("✓ Retrieved model object")
        return result

    def print_url(self) -> None:
        """Print the experiment URL for the user."""
        print(f"Your experiment is accessible at {self._accessible_url}")
