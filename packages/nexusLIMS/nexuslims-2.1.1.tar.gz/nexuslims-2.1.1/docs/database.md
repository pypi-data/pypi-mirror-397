...
the database is implemented using a single [SQLite](https://sqlite.org/index.html)
file stored in a location specified by the
{ref}`NX_DB_PATH <nexuslims-db-path>` environment variable. This database
is created using a custom [SQL Schema Definition](https://github.com/datasophos/NexusLIMS/blob/main/nexusLIMS/db/dev/NexusLIMS_db_creation_script.sql) (documented below) and can be
...
First, it serves as a location for entries related to when
...
Experiment (these entries are made by harvester modules, such as {py:mod}`~nexusLIMS.harvesters.nemo`).
...
As described above, the `session_log` table is where the harvesters (and the
...
event. The reservation and usage event harvester code (such as {py:mod}`~nexusLIMS.harvesters.nemo`) parses
...
For example, when migrating from a SharePoint-based reservation
...
| `calendar_url` | TEXT | The URL to this instrument's web-accessible calendar on the SharePoint resource (if using) |
...
| `harvester` | TEXT | The specific submodule within {py:mod}`nexusLIMS.harvesters` that should be used to harvest reservation information for this instrument. At the time of writing, the only possible values are `nemo` or `sharepoint_calendar`. |
