...
At the bottom is an [activity diagram](activity-diagram) that illustrates
...
are made to the {ref}`reference` as appropriate for further
...
context the code is run: [NX_CDCS_USER](nexusLIMS-user) and
[NX_CDCS_PASS](nexusLIMS-pass). The
...
[NX_DB_PATH](nexusLIMS-db-path) (`e.g.`
...
- `instrument` : {py:class}`~nexusLIMS.instruments.Instrument` :   An object representing the instrument associated with this session
...
1.  [(link)](starting-record-builder)
...
5.  [(link)](parse-metadata)
    ...
    [NX_DATA_PATH](nexuslims-data-path) environment variable.
6.  [(link)](separate-setup-parameters)
...
information are combined. As described [before](session-contents), a
...
specified in the [NX_INSTRUMENT_DATA_PATH](nexuslims-instrument-data-path) environment variable),
...
the Unix [`find` command](https://www.gnu.org/software/findutils/) by spawning a sub-process. This only works on Linux, and may
...
database, and the back-end proceeds with [step 1](starting-record-builder) for
...
a [Kernel Density Estimate](https://scikit-learn.org/stable/modules/density.html#kernel-density) (KDE) of the file creation times is generated. The
...
should be an [ISO format timestamp](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations) and is displayed in
...
means the [HyperSpy](https://hyperspy.org/) library is used, since it provides readers for
...
directory contained in the [NX_DATA_PATH](nexuslims-data-path) environment
variable that matches where the original raw
data file was found in the directory from the
[NX_INSTRUMENT_DATA_PATH](nexuslims-instrument-data-path) environment variable. A link to
...
generating a PNG format preview image (saved in a subdirectory within [NX_DATA_PATH](nexuslims-data-path)),
...
files contained within a given activity. [(go to top)](overview)
...
[earlier steps](build-activities), the record is completed.
...
[NX_DATA_PATH](nexuslims-data-path) (environment variable) or
[NX_RECORDS_PATH](nexuslims-records-path) if specified, for storage
before it is uploaded to the CDCS instance.

Regardless, the back-end then proceeds with [step 1](starting-record-builder)
...
[here](https://cdcs.nist.gov/cdcs-documentation/18-rest-api-examples.html)).
...
is looped periodically as described [at the top](general-approach) to
continually parse new sessions, as they occur. [(go to top)](overview)
...
![Activity diagram for record building process](_static/record_building.png)
