...
from tenacity import retry, stop_after_attempt, wait_random_exponential, retry_if_exception_type, before_sleep_log, retry_if_exception
import logging
...
class OpenAIEmbeddings:
    ...
    def __init__(self, base_url: str, model_name: str, api_key: str = None, batch_size: int = 32, max_retries: int = 3,
        retry_delay: float = 1.0, max_retry_delay: int = 10, *, client: Optional[OpenAI] = None,  max_input_length: int = 8191):
        """
        init
        :param base_url: 链接地址
        :param api_key: 密钥
        ...
        logger.info(f"✅ 初始化 OpenAIEmbeddings: model={model_name!r}, endpoint={self.base_url}")
        ...
        logger.info(f'重试策略: {self.retry_policy}')
    ...
    def batch_embed_documents(self, texts: List[str], *, batch_size: Optional[int] = None) -> List[List[float]]:
        ...
        logger.info(f"📦 开始嵌入 {len(texts)} 文本 (batch_size={batch_size})")
        ...
        for i in range(0, len(texts), batch_size):
            ...
            logger.info(f"📈 进度: {processed}/{len(texts)} ({processed / len(texts) * 100:.1f}%)")

        logger.info(f"✅ 批量嵌入完成: {len(all_embeddings)} 向量")
    ...
    def _prepare_texts(self, texts: List[str]) -> List[str]:
        ...
        for text in texts:
            ...
            if len(text) > self.max_input_length:
                logger.warning(f"文本长度 ({len(text)}) > max_input_length ({self.max_input_length})，已截断")
    ...
    def get_embedding_dimension(self) -> int:
        ...
        try:
            ...
            dim = len(test_emb)
            logger.info(f"🔍 探测到嵌入维度: {dim}")
            return dim
        except Exception as e:
            logger.error(f"维度探测失败: {e}")
