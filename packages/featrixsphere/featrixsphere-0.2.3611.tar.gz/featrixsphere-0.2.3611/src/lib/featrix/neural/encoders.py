...
def sample_marginal_masks(batch_mask, min_mask_ratio=0.40, max_mask_ratio=0.60, mean_nulls_per_row=None):
    """
...
class FeatrixTableEncoder(nn.Module):
    ...
    def count_model_parameters(self):
        ...
        if hasattr(self.joint_encoder, 'group_embeddings') and self.joint_encoder.group_embeddings is not None:
            relationship_group_params = sum(p.numel() for p in self.joint_encoder.group_embeddings.parameters())
            relationship_group_trainable_params = sum(
                p.numel() for p in self.joint_encoder.group_embeddings.parameters() if p.requires_grad
            )
            relationship_group_count = len(self.joint_encoder.group_embeddings)
    ...
    def compute_joint_infoNCE_loss(
        self, joint_encoding, unmasked_encoding, short=False, temp=None
    ):
    ...
    def compute_total_loss(
        ...
        # DEBUG: Log loss components on first few batches
        if not hasattr(self, '_loss_debug_counter'):
            self._loss_debug_counter = 0
        
        if self._loss_debug_counter < 5:
            logger.info(f"🔍 LOSS DEBUG [{self._loss_debug_counter}] RAW VALUES:")
            logger.info(f"   spread_loss: {spread_loss.item():.4f} (requires_grad={spread_loss.requires_grad})")
            logger.info(f"   joint_loss: {joint_loss.item():.4f} (requires_grad={joint_loss.requires_grad})")
            logger.info(f"   marginal_loss: {marginal_loss.item():.4f} (requires_grad={marginal_loss.requires_grad})")
            logger.info(f"   marginal_scaled: {marginal_loss_scaled.item():.4f} (coeff={getattr(loss_config, 'marginal_loss_scaling_coefficient', None)})")
            logger.info(f"🔍 LOSS DEBUG [{self._loss_debug_counter}] WEIGHTS:")
            logger.info(f"   spread_weight: {loss_config.spread_loss_weight:.4f}")
            logger.info(f"   joint_weight: {loss_config.joint_loss_weight:.4f}")
            logger.info(f"   marginal_weight: {loss_config.marginal_loss_weight:.4f}")
        ...
        # DEBUG: Log weighted contributions
        if self._loss_debug_counter <= 5:
            logger.info(f"🔍 LOSS DEBUG [{self._loss_debug_counter-1}] WEIGHTED CONTRIBUTIONS:")
            logger.info(f"   spread: {(loss_config.spread_loss_weight * spread_loss).item():.4f}")
            logger.info(f"   joint: {(loss_config.joint_loss_weight * joint_loss).item():.4f}")
            logger.info(f"   marginal: {(loss_config.marginal_loss_weight * marginal_loss_scaled).item():.4f}")
            logger.info(f"   TOTAL: {total.item():.4f} (requires_grad={total.requires_grad})")
    ...
    def forward(self, column_batches):
        ...
        mask_1, mask_2, rows_skipped = sample_marginal_masks(token_status_mask, self.min_mask_ratio, self.max_mask_ratio, self.mean_nulls_per_row)
