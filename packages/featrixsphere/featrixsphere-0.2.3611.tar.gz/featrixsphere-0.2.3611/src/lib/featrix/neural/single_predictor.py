    ...
    def __setstate__(self, state):
        ...
        for key, value in state.items():
            ...
            if isinstance(value, dict) and '__model_state_dict__' in value:
                ...
                # embedding_space is excluded from checkpoint - should be loaded separately
                # Don't set it here, it will be set by the caller
                logger.debug(f"Skipping embedding_space in __setstate__ (should be loaded separately)")
                continue
            else:
        ...
        if '_finetuned_encoder_state_dict' in state:
            # We have a fine-tuned encoder - store it for loading when embedding_space is available
            if not hasattr(self, '_pending_state_dicts'):
                self._pending_state_dicts = {}
            self._pending_state_dicts['_finetuned_encoder_state_dict'] = state['_finetuned_encoder_state_dict']
            logger.info(f"💾 Found fine-tuned encoder state_dict - will load when embedding_space is available")
                ...
                try:
                    ...
                    try:
                        ...
                        log_training_start_banner(
                            ...
                            batch_size=batch_size,
                            training_type="SP",
                            d_model=self.embedding_space.d_model if hasattr(self, 'embedding_space') else None,
