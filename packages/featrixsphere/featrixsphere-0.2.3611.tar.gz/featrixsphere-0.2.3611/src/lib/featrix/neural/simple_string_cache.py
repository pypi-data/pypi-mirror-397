...
class SimpleStringCache:
    ...
    - @lru_cache for in-memory caching (32,768 entries)
    - String server for encoding with batch support
    - No SQLite, no shared memory, no files
    """
    def __init__(self, initial_values=None, debugName="simple_cache", **kwargs):
        """
        Args:
        ...
        logger.info(f"SimpleStringCache ({debugName}): Initializing string server client...")
        ...
        # Pre-warm the cache if initial_values provided
        if initial_values and len(initial_values) > 0:
            logger.info(f"🔥 Pre-warming cache with {len(initial_values)} strings via batch encoding...")
            self._pre_warm_cache(initial_values)
        else:
            logger.info(f"✅ SimpleStringCache ({debugName}): String server ready (cache will warm on first access)")
    ...
    def _pre_warm_cache(self, string_list):
        """
        ...
        Uses individual encode calls which populate the @lru_cache properly.
        """
        import time
        ...
        try:
            ...
            for i, s in enumerate(unique_strings, 1):
                # This will cache the result in @lru_cache
                ...
                if num_strings > 10000 and i % log_interval == 0:
                    ...
                    logger.info(f"   Progress: {i}/{num_strings} ({100*i//num_strings}%) - {rate:.0f} strings/sec - ETA: {remaining:.0f}s")
            ...
            logger.info(f"   Cache populated: {new_misses} new entries added")
