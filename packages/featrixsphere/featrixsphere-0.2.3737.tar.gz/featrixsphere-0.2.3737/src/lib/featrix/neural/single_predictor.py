...
from torch.optim.lr_scheduler import CosineAnnealingLR, SequentialLR, ConstantLR, OneCycleLR
...
def _dump_cuda_memory_usage(context: str = ""):
    ...
    try:
        ...
        logger.error("="*80)
        logger.error(f"🔍 GPU MEMORY DUMP {f'({context})' if context else ''}")
        logger.error("="*80)
...
# TrainingFailureException is imported from featrix.neural.training_exceptions
...
class FeatrixSinglePredictor:
    def __init__(self, embedding_space: EmbeddingSpace, predictor, name: str = None, user_metadata: dict = None):
        ...
        self._features_applied_epochs = []  # Track when features were added
    ...
    def __getstate__(self):
        ...
        EXCLUDE_LARGE_DATA = {
            ...
            # NOTE: embedding_space is excluded EXCEPT when we fine-tuned the encoder
        }
        ...
        for key, value in self.__dict__.items():
            ...
            if key in MODEL_ATTRIBUTES and value is not None:
                ...
                if key == 'predictor_base' and hasattr(self, 'embedding_space') and self.embedding_space is not None:
                    # Full pickle file - save predictor_base as object for easier unpickling
                    # CRITICAL: We MUST save the full object, not state_dict, to avoid reconstruction issues
                    # If deepcopy fails, try moving to CPU first (might be on GPU causing issues)
                    try:
                        # Try to move to CPU if on GPU (deepcopy can fail with GPU tensors)
                        if hasattr(value, 'cpu'):
                            value_cpu = value.cpu()
                            state[key] = copy.deepcopy(value_cpu)
                            logger.debug(f"Saving {key} as object (moved to CPU first, full pickle with embedding_space)")
                        else:
                            state[key] = copy.deepcopy(value)
                            logger.debug(f"Saving {key} as object (full pickle with embedding_space)")
                        saved_keys.append(f"{key} (object)")
                        continue
                    except (TypeError, AttributeError, NotImplementedError) as e:
                        # If deepcopy still fails, try saving directly (pickle should handle it)
                        logger.warning(f"Deepcopy of {key} failed: {e}, trying direct assignment")
                        try:
                            state[key] = value  # Direct assignment - let pickle handle it
                            saved_keys.append(f"{key} (direct)")
                            continue
                        except Exception as e2:
                            logger.error(f"❌ CRITICAL: Cannot save {key} as object: {e2}, falling back to state_dict")
                            # Fall through to state_dict method below
    ...
    def _apply_dynamic_features(self, current_epoch: int):
        """
        Apply high-confidence feature suggestions during training.
        
        This is the SIMPLE version that adds features to DataFrames but doesn't
        rebuild the model mid-training. Features will be available for analysis
        and for the next training run.
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info(f"🚀 APPLYING DYNAMIC FEATURES AT EPOCH {current_epoch}")
        logger.info("=" * 80)
        
        for fname, finfo in features_to_apply:
            epochs_str = ', '.join(map(str, finfo['suggested_at_epochs']))
            logger.info(f"   • {fname}")
        ...
        try:
            ...
            self.train_df = engineer.fit_transform(self.train_df, verbose=True)
            
            # Apply to validation DataFrame if it exists
            if self.val_df is not None:
                self.val_df = engineer.transform(self.val_df, verbose=False)
            ...
            logger.info("")
            logger.info(f"✅ Features added to DataFrames")
            logger.info(f"   Training DF: {len(self.train_df.columns)} columns")
            if self.val_df is not None:
                logger.info(f"   Validation DF: {len(self.val_df.columns)} columns")
            else:
                logger.info(f"   Validation DF: None (no validation data provided)")
            logger.info(f"   Note: Features saved to DataFrames, will be used in next training run")
            logger.info("=" * 80)
            logger.info("")
            
        except Exception as e:
            logger.error(f"❌ Failed to apply features: {e}")
            traceback.print_exc()
    ...
    def _load_and_apply_previous_features(self, train_df: pd.DataFrame, val_df: Optional[pd.DataFrame] = None) -> tuple:
        ...
        # Look for feature_suggestion_history.json in qa.save subdirectory (new location)
        # and fallback to old locations for backwards compatibility
        search_paths = []
        
        if self._output_dir:
            # New location: qa.save subdirectory
            search_paths.append(Path(self._output_dir) / "qa.save" / "feature_suggestion_history.json")
            # Old location for backwards compatibility
            search_paths.append(Path(self._output_dir) / "feature_suggestion_history.json")
        
        # Also check parent directories in case we're in a nested run
        search_paths.append(Path("qa.save") / "feature_suggestion_history.json")
        search_paths.append(Path("feature_suggestion_history.json"))
        search_paths.append(Path("..") / "qa.save" / "feature_suggestion_history.json")
        search_paths.append(Path("../feature_suggestion_history.json"))
        search_paths.append(Path("../..") / "qa.save" / "feature_suggestion_history.json")
        search_paths.append(Path("../../feature_suggestion_history.json"))
        ...
        if not history_file:
            logger.debug("ℹ️  No previous feature_suggestion_history.json found - starting fresh")
            return train_df, val_df
        
        try:
            logger.info("")
            logger.info("=" * 80)
            logger.info(f"📂 LOADING PREVIOUS FEATURE SUGGESTIONS")
            ...
            # Extract features that were actually applied in the previous run
            applied_features = []
            for feature_name, info in suggestion_history.items():
                if info.get('applied', False):
                    applied_features.append({
                        ...
                        'applied_epoch': info.get('applied_epoch', -1)
                    })
            
            if not applied_features:
                logger.info("ℹ️  No features were applied in previous run - nothing to load")
                logger.info("=" * 80)
                logger.info("")
                return train_df, val_df
            
            # Sort by applied_epoch to apply in the same order
            applied_features.sort(key=lambda x: x['applied_epoch'])
            
            logger.info(f"✅ Found {len(applied_features)} previously applied features:")
            for feat in applied_features:
                logger.info(f"   • {feat['name']} ({feat['votes']} votes, applied at epoch {feat['applied_epoch']})")
            logger.info("=" * 80)
            logger.info("")
            
            # Apply features using FeatureEngineer
            suggestions = [feat['suggestion'] for feat in applied_features]
            engineer = FeatureEngineer(suggestions=suggestions)
            
            # Track existing columns before applying features
            original_train_cols = set(train_df.columns)
            
            logger.info("🔧 Applying features to training data...")
            train_df = engineer.fit_transform(train_df, verbose=True)
            
            # Identify new columns and prefix them with 'featrix_emphasis_'
            new_train_cols = set(train_df.columns) - original_train_cols
            if new_train_cols:
                rename_map = {col: f'featrix_emphasis_{col}' for col in new_train_cols}
                train_df = train_df.rename(columns=rename_map)
                logger.info(f"🏷️  Prefixed {len(new_train_cols)} new columns with 'featrix_emphasis_':")
                for old_name, new_name in rename_map.items():
                    logger.info(f"   • {old_name} → {new_name}")
            
            if val_df is not None:
                original_val_cols = set(val_df.columns)
                logger.info("🔧 Applying features to validation data...")
                val_df = engineer.transform(val_df, verbose=False)
                
                # Prefix new columns in validation set
                new_val_cols = set(val_df.columns) - original_val_cols
                if new_val_cols:
                    rename_map = {col: f'featrix_emphasis_{col}' for col in new_val_cols}
                    val_df = val_df.rename(columns=rename_map)
            
            logger.info("")
            logger.info("✅ Previous features successfully applied!")
            logger.info(f"   Training DF columns: {len(train_df.columns)}")
            if val_df is not None:
                logger.info(f"   Validation DF columns: {len(val_df.columns)}")
            logger.info("=" * 80)
            logger.info("")
            
            # Mark these features as already applied in the tracker to avoid re-applying
            for feat in applied_features:
                self._feature_tracker.applied_features.add(feat['name'])
                self._feature_tracker.suggestion_history[feat['name']] = {
                    'suggestion': feat['suggestion'],
                    'count': feat['votes'],
                    'first_seen_epoch': -1,  # From previous run
                    'last_seen_epoch': -1,
                    'suggested_at_epochs': [],
                    'applied': True,
                    'applied_epoch': feat['applied_epoch']
                }
            
            return train_df, val_df
            
        except Exception as e:
            logger.warning(f"⚠️  Failed to load/apply previous features: {e}")
            traceback.print_exc()
            return train_df, val_df
        ...
        if predictor_base_missing and hasattr(self, '_pending_state_dicts') and 'predictor' in self._pending_state_dicts:
            ...
            try:
                ...
                if final_layer_weight is None:
                    # Try to find any weight with target_dim as first dimension
                    for key, weight in predictor_state_dict.items():
                        if key.endswith('.weight') and len(weight.shape) == 2 and weight.shape[0] == target_dim:
                            final_layer_weight = weight
                            break
                
                if final_layer_weight is None:
                    raise ValueError("Cannot infer predictor architecture from state_dict - final layer weight not found")
                
                output_dim = final_layer_weight.shape[1]
                logger.debug(f"🔍 Inferred architecture: d_model={self.d_model} -> output_dim={output_dim} -> target_dim={target_dim}")
    ...
    async def train(
        ...
        # Track training start for hourly checkpoints
        training_start_time = time.time()
        last_checkpoint_time = [0.0]  # Use list for closure mutability
        ...
        if optimizer_params is None or "lr" not in optimizer_params:
            n_samples = len(self.train_df)
            
            # Conservative LR based ONLY on dataset size
            # Small datasets: risk of overfitting → lower LR
            # Large datasets: more stable estimates → can use slightly higher LR
            if n_samples < 1000:
                adaptive_lr = 5e-05  # Very small dataset: very conservative
            elif n_samples < 5000:
                adaptive_lr = 1e-04  # Small-medium dataset: conservative (YOUR CASE: 6k samples)
            elif n_samples < 20000:
                adaptive_lr = 2e-04  # Medium-large dataset: moderate
            else:
                adaptive_lr = 3e-04  # Large dataset: can handle higher LR
            
            logger.info(f"📊 ADAPTIVE LR CALCULATION:")
            logger.info(f"   Dataset size: {n_samples} samples → Base LR: {adaptive_lr:.6e}")
            logger.info(f"   OneCycleLR will explore up to {adaptive_lr * 3.0:.6e} (3x peak)")
            logger.info(f"   Starting from {adaptive_lr * 3.0 / 25.0:.6e} (safe warmup)")
            
            optimizer_params = {"lr": adaptive_lr, "weight_decay": 0.0}
        else:
            logger.info(f"📊 Using provided LR: {optimizer_params['lr']:.6e}")
        ...
        if use_8bit:
            try:
                ...
                optimizer = bnb.optim.AdamW8bit(params, **optimizer_params)
        ...
        if not optimizer_created:
            try:
                optimizer = torch.optim.AdamW(params, fused=True, **optimizer_params)
        ...
        # 
        # NOTE: OneCycleLR scheduler creation moved AFTER DataLoader creation
        # to use exact len(train_dataloader) instead of estimated math.ceil()
        # This prevents off-by-one errors where scheduler.step() is called
        # more times than total_steps allows.
        # 
        ...
        def get_lr():
            ...
            if hasattr(self, '_training_scheduler') and self._training_scheduler is not None:
                return self._training_scheduler.get_last_lr()
            else:
        ...
        # CRITICAL: Move predictor to GPU BEFORE training starts
        if is_gpu_available():
        ...
        # CRITICAL: Cap batch_size to prevent overfitting on small validation sets
        # For small datasets (<1000), use /4 instead of /8 to avoid tiny batches
        # For medium datasets (1000-10k), use /6
        # For large datasets (>10k), use /8
        ...
        if batch_size > max_batch_size:
            ...
            logger.warning(
                f"⚠️  batch_size ({original_batch_size}) capped to {batch_size} "
        ...
        # 
        # Set up OneCycleLR scheduler NOW (after DataLoader creation)
        ...
        if use_lr_scheduler:
            # Use EXACT number of batches from DataLoader (not estimated math.ceil)
            total_steps = n_epochs * n_batches_per_epoch
            base_lr = optimizer_params.get('lr', 1e-04)
            
            # OneCycleLR parameters:
            # - max_lr: Peak learning rate (3x base for safe exploration)
            # - pct_start: Fraction of training for warmup (30% = conservative)
            # - div_factor: Initial LR = max_lr / div_factor (25 = very gradual warmup)
            # - final_div_factor: Final LR = max_lr / final_div_factor (1e4 = aggressive cooldown)
            scheduler = OneCycleLR(
                optimizer,
                max_lr=base_lr * 3.0,  # Peak at 3x base LR (conservative multiplier)
                total_steps=total_steps,
                pct_start=0.3,  # 30% of training for warmup (safe for unstable models)
                div_factor=25.0,  # Start at max_lr/25 (very gentle warmup)
                final_div_factor=1e4,  # End at max_lr/10000 (strong regularization)
                anneal_strategy='cos',  # Cosine annealing (smooth transitions)
            )
            
            # Store scheduler for use in get_lr() function defined above
            self._training_scheduler = scheduler
            
            logger.info(f"📈 OneCycleLR Scheduler:")
            logger.info(f"   Base LR: {base_lr:.6e}")
            logger.info(f"   Max LR (peak): {base_lr * 3.0:.6e} (3x base)")
            logger.info(f"   Initial LR: {base_lr * 3.0 / 25.0:.6e} (warmup start)")
            logger.info(f"   Final LR: {base_lr * 3.0 / 1e4:.6e} (cooldown end)")
            logger.info(f"   Total steps: {total_steps} ({n_epochs} epochs × {n_batches_per_epoch} batches)")
            logger.info(f"   Warmup: {int(total_steps * 0.3)} steps (30%)")
            logger.info(f"   ✅ Using EXACT batch count from DataLoader (prevents off-by-one errors)")
        else:
            scheduler = None
            self._training_scheduler = None
        ...
        # Move validation batch to same device as encoder
        encoder_device = next(self.embedding_space.encoder.parameters()).device
        for key, tokenbatch in validation_batch.items():
            validation_batch[key] = tokenbatch.to(encoder_device)
        ...
        self._structured_logger = StructuredLogger(logger, target_col_name=self.target_col_name, output_dir=self._output_dir)
                ...
                try:
                    ...
                    for epoch_idx in range(n_epochs):
                        ...
                        for batch_idx, column_batch in enumerate(train_dataloader):
                            ...
                            # CRITICAL: Verify predictor is on GPU before training
                            predictor_device = next(self.predictor.parameters()).device if list(self.predictor.parameters()) else None
                            ...
                            try:
                                ...
                                # DIAGNOSTIC: Log encoder state on first batch
                                if epoch_idx == 0 and batch_idx == 0:
                                ...
                                # CRITICAL DIAGNOSTIC: Check if encoding produced valid, varied outputs
                                if epoch_idx == 0 and batch_idx == 0:
                            ...
                            # CRITICAL: Clamp logits to prevent extreme loss values that cause gradient explosions
                            # Cross-entropy loss is exp(logit), so logits > 20 cause numerical overflow
                            # Clamping to [-20, 20] prevents extreme losses while preserving model behavior
                            # (softmax is translation-invariant, so clamping doesn't change predictions)
                            out_clamped = torch.clamp(out, min=-20.0, max=20.0)
                            
                            # Check if we had to clamp (indicates potential instability)
                            if (out.abs() > 20.0).any():
                            ...
                            if use_pairwise_ranking_loss and not self.is_target_scalar:
                                # Check if this is binary classification - use centralized function
                                is_binary_batch = self.should_compute_binary_metrics()
                                
                                if is_binary_batch:
                                    # BF16 mixed precision: pairwise ranking loss also in autocast
                                    with torch.amp.autocast(device_type='cuda', dtype=autocast_dtype, enabled=use_autocast):
                            ...
                            # Set to 2.0 for aggressive protection against gradient explosions
                            # (previous value of 20.0 was far too permissive - allowed 400k+ gradients through!)
                            max_grad_norm = 2.0
                            ...
                            if batch_idx == 0:
                                ...
                                if scheduler is not None and isinstance(scheduler, OneCycleLR):
                                    # OneCycleLR: determine which phase we're in
                                    current_step = epoch_idx * n_batches_per_epoch + batch_idx
                                    total_steps = n_epochs * n_batches_per_epoch
                                    progress_pct = (current_step / total_steps) * 100
                                    
                                    # OneCycleLR phases: warmup (0-30%), peak (30-30.1%), cooldown (30.1-100%)
                                    if progress_pct < 30:
                                        phase = "warmup"
                                    elif progress_pct < 31:
                                        phase = "peak"
                                    else:
                                        phase = "cooldown"
                                    
                                    logger.info(f"📈 OneCycleLR : {phase} phase ({progress_pct:.1f}% complete, LR={lr_value:.6e})")
                            ...
                            # Step OneCycleLR scheduler after each batch (NOT at end of epoch)
                            # OneCycleLR requires per-batch stepping to properly execute its schedule
                            ...
                            if use_composite_score:
                                ...
                                if current_val_loss < best_val_loss:
                                    ...
                                    if epoch_idx >= 20:
                                        ...
                                        logger.debug(f"{log_prefix}   ⭐ New best val loss: {best_val_loss:.4f} (epoch {epoch_idx} < 20, not saving checkpoint)")
                        ...
                        try:
                            ...
                            # CRITICAL: Re-raise training failure exceptions - they should stop training!
                            raise
                        except Exception as struct_log_error:
                            logger.warning(f"⚠️  Structured logging error: {struct_log_error}")
                        
                        # Check /NODUMP file - skip best checkpoint saves unless last fold
                        nodump_exists = os.path.exists('/NODUMP')
                        is_last_fold = getattr(self, 'is_last_fold', True)  # Assume True if not set
                        
                        should_save_best = is_new_best and epoch_idx >= 20 and (not nodump_exists or is_last_fold)
                        
                        if is_new_best and epoch_idx >= 20:
                            if nodump_exists and not is_last_fold:
                                logger.info(f"   ⏭️  /NODUMP exists and not last fold - skipping best checkpoint save")
                            elif nodump_exists and is_last_fold:
                                logger.info(f"   ✅ /NODUMP exists BUT this is last fold - saving best checkpoint anyway")
                        ...
                        # OneCycleLR scheduler.step() is called after each batch (see line ~3420)
                        ...
                        # PERIODIC CHECKPOINT: Save every 2 epochs for inference
                        # This allows users to test the model during training
                        checkpoint_interval = 2  # Save every 2 epochs
                        should_save_periodic = (
                            not nodump_exists and
                            epoch_idx > 0 and  # Don't save at epoch 0
                            epoch_idx % checkpoint_interval == 0  # Every 2 epochs
                        )
                        
                        # Save hourly checkpoint (only after 59 min, then every 60 min)
                        elapsed_minutes = (time.time() - training_start_time) / 60.0
                        minutes_since_last = (time.time() - last_checkpoint_time[0]) / 60.0
                        
                        logger.debug(f"🕐 Checkpoint check (epoch {epoch_idx}): elapsed={elapsed_minutes:.1f}min, since_last={minutes_since_last:.1f}min, /NODUMP={'EXISTS' if nodump_exists else 'not found'}")
                        
                        should_save_hourly = (
                            not nodump_exists and  # Skip if /NODUMP exists
                            elapsed_minutes >= 59.0 and 
                            minutes_since_last >= 60.0
                        )
                        
                        # Save checkpoint if periodic OR hourly
                        should_save_checkpoint = should_save_periodic or should_save_hourly
                        ...
                        elif should_save_hourly:
                            logger.info(f"✅ HOURLY CHECKPOINT: elapsed={elapsed_minutes:.1f}min >= 59, since_last={minutes_since_last:.1f}min >= 60")
                        else:
                            logger.debug(f"⏭️  SKIPPING checkpoint: elapsed={elapsed_minutes:.1f}min (need 59+), since_last={minutes_since_last:.1f}min (need 60+)")
                        
                        if should_save_checkpoint:
                            checkpoint_type = "PERIODIC" if should_save_periodic else "HOURLY"
                            logger.info(f"")
                            if should_save_periodic:
                                logger.info(f"💾 PERIODIC CHECKPOINT (epoch {epoch_idx}, every {checkpoint_interval} epochs)")
                            else:
                                logger.info(f"💾 HOURLY CHECKPOINT (elapsed: {elapsed_minutes:.1f} min, since last: {minutes_since_last:.1f} min)")
                            ...
                            try:
                                ...
                                checkpoint_dir = self._output_dir if self._output_dir else "."
                                id_suffix = f"_{sp_identifier}" if sp_identifier else ""
                                # Use different naming for periodic vs hourly checkpoints
                                if should_save_periodic:
                                    epoch_checkpoint_path = os.path.join(checkpoint_dir, f"single_predictor{id_suffix}_epoch_{epoch_idx}.pickle")
                                else:
                                    epoch_checkpoint_path = os.path.join(checkpoint_dir, f"single_predictor{id_suffix}_epoch_{epoch_idx}_hourly.pickle")
                                ...
                                last_checkpoint_time[0] = time.time()
                                ...
                                try:
                                    ...
                                    with open(epoch_checkpoint_path, "wb") as f:
                                        ...
                                        cuda_was_available = is_gpu_available()
                            ...
                            # Skip checkpoint - not time yet
                            logger.debug(f"⏭️  Skipping checkpoint: {elapsed_minutes:.1f} min elapsed (need 59+ min and 60+ since last)")
                        ...
                        try:
                            ...
                            if self._features_applied_epochs:
                                ...
                                for feat_info in self._features_applied_epochs:
                                    ...
                                    epochs_str = ', '.join(map(str, feat_info['suggested_at_epochs']))
                    ...
                    if use_lr_scheduler:
                        logger.warning(f"   Recreating OneCycleLR scheduler (required on restart)...")
                        remaining_epochs = n_epochs  # Full epoch count since we're starting from 0
                        # Use EXACT batch count from DataLoader (same as initial scheduler creation)
                        remaining_batches_per_epoch = len(train_dataloader)
                        remaining_steps = remaining_epochs * remaining_batches_per_epoch
                        
                        # Recreate OneCycleLR with same config as initial training
                        scheduler = OneCycleLR(
                            optimizer,
                            max_lr=new_lr * 3.0,  # Use updated LR as base
                        ...
                        logger.warning(f"   ✅ OneCycleLR scheduler reset ({remaining_steps} steps = {remaining_epochs} epochs × {remaining_batches_per_epoch} batches, max_lr={new_lr * 3.0:.6e})")
        ...
        if self._validation_error_tracking is not None:
            try:
    ...
    def detect_training_failure_mode(self, raw_logits_array, pos_probs_array, pred_counts, 
        ...
        if epoch_idx < min_epoch_threshold:
            logger.debug(f"{log_prefix}Skipping failure diagnostics at epoch {epoch_idx} - waiting until epoch {min_epoch_threshold} ({min_epoch_threshold/n_epochs*100:.1f}% complete)")
        ...
        # FAILURE MODE 2: Constant Probability Output
            ...
            if epoch_idx < 5:
                ...
                action_msg = "   → WILL increase LR by 2x (one time, capped at 0.01 max)"
            else:
                action_msg = "   → WILL abort training (epoch 50+ with constant probs = failure)"
        ...
        with PredictorEvalModeContextManager(fsp=self, debugLabel="compute_classification_metrics"):
            ...
            if isinstance(self.target_codec, (SetEncoder, SetCodec)):
                ...
                if results and len(results) > 0:
                    # Get the actual keys from prediction dicts
                    prediction_keys = set(results[0].keys())
                                ...
                                if hasattr(self.target_codec, 'members'):
                                    ...
                                    if pos_label in codec_members:
                                        if pos_label in prediction_keys:
                                            ...
                                            logger.warning(f"⚠️  pos_label {pos_label!r} is in codec members but not in prediction keys: {prediction_keys}")
                            ...
                            if pos_label not in prediction_keys and str(pos_label) not in prediction_keys:
                                if hasattr(self.target_codec, 'tokens_to_members'):
                                    ...
                                    if matched_key and matched_key in prediction_keys:
                                        ...
                                        logger.warning(f"⚠️  Found matching codec value {matched_key!r} for pos_label {pos_label!r}, but it's not in prediction keys: {prediction_keys}")
                                    else:
            ...
            # Track per-row correct/wrong flags for this epoch
            if self._validation_error_tracking is not None and epoch_idx is not None:
                ...
                logger.debug(f"{log_prefix}📊 Validation error tracking: {sum(correct_flags)}/{len(correct_flags)} correct this epoch")
            ...
            if is_binary:
                ...
                try:
                    ...
                    if results:
                        ...
                        if pos_label in first_dict:
                            ...
                            for key in first_dict.keys():
                                ...
                                try:
                                    ...
                                    if type(key)(pos_label) == key:
                                        found_key = key
                                        extraction_method = f"fuzzy_match (pos_label '{pos_label}' converted to match key '{key}' type {type(key).__name__})"
                                        logger.warning(f"⚠️  FUZZY MATCH: Found pos_label by converting '{pos_label}' -> {type(key)(pos_label)} to match key '{key}'")
                                        logger.warning(f"   Dict keys: {sample_dict_keys}")
                                        break
                                except (ValueError, TypeError):
                                    pass
                                ...
                                if str(key).lower() == str(pos_label).lower():
                                    found_key = key
                                    extraction_method = f"fuzzy_match (case-insensitive string match: '{pos_label}' -> '{key}')"
                                    logger.warning(f"⚠️  FUZZY MATCH: Found pos_label by case-insensitive string match: '{pos_label}' -> '{key}'")
                                    logger.warning(f"   Dict keys: {sample_dict_keys}")
                                    break
                            ...
                            if found_key:
                                ...
                                # Last resort: try to infer from ground truth - find which prediction key corresponds to pos_label
                                # by checking which key has highest probability where ground_truth == pos_label
                                logger.warning(f"⚠️  Could not match pos_label={pos_label!r} to any prediction key using type conversion")
                                logger.warning(f"   Dict keys: {sample_dict_keys}")
                                logger.warning(f"   Attempting to infer correct key from ground truth alignment...")
                                
                                # Find indices where ground_truth == pos_label
                                pos_indices = [i for i, gt in enumerate(ground_truth) if gt == pos_label]
                                
                                if pos_indices:
                                    # For each prediction key, compute average probability at pos_label indices
                                    key_avg_probs = {}
                                    for key in first_dict.keys():
                                        if key == "<UNKNOWN>":
                                            continue
                                        # Get probabilities for this key at pos_label indices
                                        key_probs = [results[i].get(key, 0.0) for i in pos_indices]
                                        if key_probs:
                                            key_avg_probs[key] = sum(key_probs) / len(key_probs)
                                    
                                    if key_avg_probs:
                                        # Find the key with the highest average probability at pos_label indices
                                        best_key = max(key_avg_probs, key=key_avg_probs.get)
                                        best_avg_prob = key_avg_probs[best_key]
                                        logger.warning(f"   Best match: key '{best_key}' with avg prob {best_avg_prob:.4f} at {len(pos_indices)} pos_label indices")
                                        
                                        if best_avg_prob > 0.0:
                                            found_key = best_key
                                            extraction_method = f"inferred_from_ground_truth (key '{best_key}' has highest avg prob {best_avg_prob:.4f} where gt==pos_label)"
                                            logger.warning(f"⚠️  INFERRED KEY: Using '{best_key}' for pos_label {pos_label!r} based on ground truth probability alignment")
                                            pos_probs = [d.get(found_key, 0.0) for d in results]
                                        else:
                                            # All probabilities are 0, can't infer
                                            ...
                                            logger.error(f"   All keys have 0.0 average probability at pos_label indices")
                                            logger.error(f"   Tried: exact match, string conversion, type conversion, ground truth inference")
                                        ...
                                        logger.error(f"🚨 FAILED to extract probability for pos_label={pos_label} (type={type(pos_label).__name__})")
                                        logger.error(f"   Dict keys: {sample_dict_keys}")
                                        logger.error(f"   No valid keys found for probability inference")
                                        logger.error(f"   Tried: exact match, string conversion, type conversion, ground truth inference")
                                        logger.error(f"   Using 0.0 as fallback - METRICS WILL BE WRONG!")
                                        pos_probs = [0.0] * len(results)
                                else:
                                    # No ground truth values match pos_label
                                    logger.error(f"🚨 FAILED to extract probability for pos_label={pos_label} (type={type(pos_label).__name__})")
                                    logger.error(f"   Dict keys: {sample_dict_keys}")
                                    logger.error(f"   No ground truth values match pos_label (found {len(set(ground_truth))} unique GT values)")
                                    logger.error(f"   Tried: exact match, string conversion, type conversion, ground truth inference")
                                    logger.error(f"   Using 0.0 as fallback - METRICS WILL BE WRONG!")
                                    pos_probs = [0.0] * len(results)
                    ...
                    # Log always (INFO not DEBUG) - critical for diagnosing collapse
                    ...
                    logger.debug(f"{log_prefix}   Mean: {prob_mean:.4f}, Median: {np.median(pos_probs_array):.4f}")
                    logger.debug(f"{log_prefix}   Std: {prob_std:.4f}, Range: {prob_range:.4f}")
                    ...
                    if self._best_auc < 0:
                        ...
                        is_best_epoch = True
                        best_epoch_msg = f"⭐ New best epoch: AUC={auc:.3f}, F1={f1:.3f}, threshold={optimal_threshold:.4f}"
                        ...
                        self.optimal_threshold = optimal_threshold
                        is_best_epoch = True
                        best_epoch_msg = f"⭐ New best epoch: AUC={auc:.3f} (previous best: {previous_auc:.3f} @ epoch {previous_epoch}), threshold={optimal_threshold:.4f}"
                        ...
                        self.optimal_threshold = optimal_threshold
                        is_best_epoch = True
                        best_epoch_msg = f"⭐ New best epoch: AUC tied ({auc:.3f}) but F1 improved to {f1:.3f} (previous: {previous_f1:.3f} @ epoch {previous_epoch}), threshold={optimal_threshold:.4f}"
                    else:
                        # Current epoch is not the best - track best for reference
                        best_epoch_msg = f"📊 Current: AUC={auc:.3f}, F1={f1:.3f}, threshold={optimal_threshold:.4f} | Best: AUC={self._best_auc:.3f}, F1={self._best_f1_at_best_auc:.3f}, threshold={self._best_threshold_at_best_auc:.4f} @ epoch {self._best_auc_epoch}"
                    ...
                    logger.debug(f"{log_prefix}Binary metrics (optimal threshold {optimal_threshold:.4f}) - Precision: {precision:.3f}, Recall: {recall:.3f}, F1: {f1:.3f}, ROC-AUC: {auc:.3f}, PR-AUC: {pr_auc:.3f}")
                    ...
                    if failure_detected:
                        ...
                        if "DEAD_NETWORK" in failure_label:
                            ...
                            # Don't stop until epoch 50 - early epochs often have low variance
                            if epoch_idx >= 50:
                                should_stop = True
                                ...
                                logger.warning(f"⚠️  CONSTANT_PROBABILITY detected at epoch {epoch_idx} but continuing (need epoch 50+ to abort)")
                                
                                # AUTOMATIC LR INCREASE: If we haven't increased LR yet, do it now
                                if not hasattr(self, '_lr_increased_for_constant_prob'):
                                    self._lr_increased_for_constant_prob = False
                                
                                if not self._lr_increased_for_constant_prob and epoch_idx >= 5:
                                    # Increase LR by 2x (conservative increase to escape flat region)
                                    # Get current LR from optimizer (get_lr() is not in scope here)
                                    ...
                                    if optimizer and len(optimizer.param_groups) > 0:
                                        ...
                                        lr_value = 0.01
                                    new_lr = lr_value * 2.0  # Conservative 2x (was 5x - too aggressive)
                                    
                                    # CRITICAL: Cap at 0.01 to prevent gradient explosion
                                    # LR values above 0.01 often cause NaN gradients
                                    max_lr = 0.01
                                    if new_lr > max_lr:
                                        logger.warning(f"   ⚠️  Capping LR at {max_lr:.6e} (calculated {new_lr:.6e} would be too high)")
                                        new_lr = max_lr
                                    
                                    logger.error("="*80)
                                    logger.error("🚨 AUTOMATIC LEARNING RATE INCREASE")
                                    logger.error("="*80)
                                    logger.error(f"   Detected CONSTANT_PROBABILITY at epoch {epoch_idx}")
                                    logger.error(f"   Current LR: {lr_value:.6e}")
                                    logger.error(f"   Increasing LR by 2x to: {new_lr:.6e}")
                                    logger.error(f"   This should help the model escape the flat region")
                                    logger.error("="*80)
                                    ...
                                    # CRITICAL: Set flag immediately after optimizer update, BEFORE scheduler update
                                    # This prevents double-increase if scheduler update fails
                                    self._lr_increased_for_constant_prob = True
                                    
                                    # If scheduler exists, we need to update its base_lrs
                                    # Wrap in try/except to prevent flag from being unset if this fails
                                    try:
                                        scheduler = self._training_scheduler
                                        if scheduler is not None:
                                            # For SequentialLR, update both schedulers
                                            if hasattr(scheduler, 'schedulers'):
                                                for sub_scheduler in scheduler.schedulers:
                                                    if hasattr(sub_scheduler, 'base_lrs'):
                                                        sub_scheduler.base_lrs = [new_lr] * len(sub_scheduler.base_lrs)
                                                    if hasattr(sub_scheduler, 'eta_min'):
                                                        # Update eta_min proportionally for cosine annealing
                                                        sub_scheduler.eta_min = new_lr * 1e-4  # Keep same ratio
                                            elif hasattr(scheduler, 'base_lrs'):
                                                scheduler.base_lrs = [new_lr] * len(scheduler.base_lrs)
                                    except Exception as scheduler_err:
                                        logger.warning(f"   ⚠️  Could not update scheduler base_lrs: {scheduler_err}")
                                        logger.warning(f"   Optimizer LR still updated - scheduler will be out of sync")
                                    
                                    logger.info(f"✅ Learning rate increased to {new_lr:.6e} - will take effect on next batch")
    ...
    def _create_model_card_json(self, best_epoch_idx=None, metadata=None):
        ...
        model_card = {
            ...
            "model_architecture": {
                ...
                "predictor_total_layers": predictor_layer_count,
