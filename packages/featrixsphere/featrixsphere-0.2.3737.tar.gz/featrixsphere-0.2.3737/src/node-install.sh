...
import sys
success = True
packages = ['fastapi', 'uvicorn', 'pandas', 'pydantic_settings', 'jsontables', 'redis', 'celery', 'psutil', 'weightwatcher']
for pkg in packages:
    try:
        __import__(pkg)
        print(f'✅ {pkg} OK')
...
EOF
    ...
    MOTD_SCRIPT=""
    if [ -f "motd-featrix.sh" ]; then
        MOTD_SCRIPT="motd-featrix.sh"
    elif [ -f "$LOCAL_REPO_ROOT/motd-featrix.sh" ]; then
        MOTD_SCRIPT="$LOCAL_REPO_ROOT/motd-featrix.sh"
    elif [ -f "$REPO_ROOT/motd-featrix.sh" ]; then
        MOTD_SCRIPT="$REPO_ROOT/motd-featrix.sh"
    fi
    ...
    if [ -n "$MOTD_SCRIPT" ]; then
        ...
        if [ -d "/etc/update-motd.d" ]; then
            ...
            if safe_copy "$MOTD_SCRIPT" "/etc/update-motd.d/99-featrix"; then
                ...
                print_warning "Failed to install dynamic MOTD"
            fi
        else
        ...
        print_warning "motd-featrix.sh not found (optional)"
    fi
    ...
    if [ -n "$SBIT_SOURCE" ]; then
        ...
        if [ -f "$TARGET_DIR/sbit" ] && [ -u "$TARGET_DIR/sbit" ]; then
            ...
            if gcc -o "$TEMP_SBIT" "$TARGET_DIR/sbit.c"; then
                ...
                # Note: Old sbit might not have 'install' command, so fall back to manual install if it fails
                if "$TARGET_DIR/sbit" install "$TEMP_SBIT" 2>/dev/null; then
                    print_status "✅ New sbit installed using sbit install command"
                    rm -f "$TEMP_SBIT"
                else
                    # Old sbit doesn't have 'install' command - fall back to manual install
                    print_warning "Old sbit doesn't have 'install' command - using manual install"
...
fix_supervisor_if_broken() {
    ...
    print_status "Cleaning up supervisor sockets..."
    rm -f /var/run/supervisor.sock /var/run/supervisor/* /tmp/supervisor.sock /tmp/supervisord.sock 2>/dev/null || true
    # Also clean up any supervisor directories
    rm -rf /var/run/supervisor 2>/dev/null || true
...
detect_gpu_capacity() {
    ...
    if command -v nvidia-smi >/dev/null 2>&1; then
        ...
        if [ "$GPU_COUNT" -ge 2 ]; then
            CONCURRENCY=2
            print_status "   Detected $GPU_COUNT GPU(s) - setting concurrency to $CONCURRENCY"
        elif [ "$GPU_COUNT" -eq 1 ]; then
            # Check GPU memory - if >32GB, can handle 2 concurrent tasks
            GPU_MEM=$(nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits 2>/dev/null | head -1)
            if [ -n "$GPU_MEM" ] && [ "$GPU_MEM" -gt 32000 ]; then
                CONCURRENCY=2
                print_status "   Detected 1 GPU with ${GPU_MEM}MB memory - setting concurrency to $CONCURRENCY"
            else
                CONCURRENCY=1
                print_status "   Detected 1 GPU with ${GPU_MEM}MB memory - setting concurrency to $CONCURRENCY"
            fi
        else
            print_status "   No GPUs detected - setting concurrency to $CONCURRENCY"
        fi
    else
...
setup_supervisor() {
    ...
    if systemctl is-active --quiet supervisor && supervisorctl status >/dev/null 2>&1; then
        ...
        # Clean up supervisor sockets to ensure clean start
        rm -f /var/run/supervisor.sock /var/run/supervisor/* /tmp/supervisor.sock /tmp/supervisord.sock 2>/dev/null || true
        mkdir -p /var/run/supervisor 2>/dev/null || true
        chmod 755 /var/run/supervisor 2>/dev/null || true
...
start_services() {
    ...
    if [ -n "$SERVICES_TO_START" ]; then
        ...
        if ! supervisorctl start $SERVICES_TO_START; then
    ...
    for retry in $(seq 1 $MAX_RETRIES); do
        ...
        while IFS= read -r line; do
            if [[ -n "$line" ]]; then
                ...
                # Skip header lines
                if [[ "$SERVICE_NAME" == "supervisorctl" ]] || [[ "$SERVICE_NAME" == "unix://"* ]]; then
                    continue
                fi
...
upgrade_sbit_early_if_needed() {
    ...
    if [ -f "$TARGET_DIR/sbit" ] && [ -x "$TARGET_DIR/sbit" ]; then
        ...
        if ! "$TARGET_DIR/sbit" kill 2>&1 | grep -q "requires at least one argument"; then
            ...
            # Check if new sbit.c exists in package
            if [ -f "$TARGET_DIR/sbit.c" ]; then
                # Compile new sbit
                TEMP_SBIT="/tmp/sbit.new.$$"
                if gcc -o "$TEMP_SBIT" "$TARGET_DIR/sbit.c" 2>/dev/null; then
                    # Use existing sbit to install new one (if it has install command)
                    if "$TARGET_DIR/sbit" install "$TEMP_SBIT" 2>/dev/null; then
...
show_final_status() {
    ...
    STATUS_OUTPUT=$(supervisorctl status featrix-firmware:* 2>&1)
    EXIT_CODE=$?
    ...
    if [ ${#FAILED_SERVICES[@]} -gt 0 ]; then
        ...
        SERVICES_TO_START=$(get_services_except_string_server)
        if [ -n "$SERVICES_TO_START" ]; then
            supervisorctl start $SERVICES_TO_START || true
        else
            # Fallback - use group (never start string-server)
            supervisorctl start featrix-firmware:* || true
        fi
        sleep 2
        supervisorctl status
        return 1
    ...
    supervisorctl status | while read line; do add_prefix "${BLUE}   $line${NC}"; done
    add_prefix ""
