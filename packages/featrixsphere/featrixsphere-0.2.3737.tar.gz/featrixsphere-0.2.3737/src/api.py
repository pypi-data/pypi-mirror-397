...
def create_app() -> FastAPI:
                ...
                if job_type == "train_es":  # Check both running and done ES jobs
                    ...
                    if epoch_dir.exists():
                        ...
                        try:
                            ...
                            logger.error(f"❌ Permission denied listing epoch directory {epoch_dir}: {perm_err}")
                            post_slack_alert(f"session={id}; permission denied accessing epoch_projections directory")
                            raise HTTPException(
                                status_code=HTTPStatus.FORBIDDEN,
                                detail=f"Permission denied accessing epoch projections directory"
                            )
    ...
    async def get_model_card(id: str) -> JSONResponse:
        ...
        try:
            ...
            logger.warning(f"⚠️  Session {id} not found on this server")
            raise HTTPException(
                status_code=HTTPStatus.NOT_FOUND,
                detail=f"Session {id} not found on this server. It may exist on a different Sphere node, be unpublished, or have been deleted."
            ) from e
        
        try:
            # CRITICAL: If training is incomplete (embedding_space=None), return partial model card immediately
            # Don't waste time searching for files that don't exist yet!
            session_status = session.get("status", "unknown")
            embedding_space_path_str = session.get("embedding_space")
            
            if not embedding_space_path_str and session_status in ["pending", "running", "READY", "RUNNING"]:
                logger.info(f"📋 Session {id} training incomplete (status={session_status}, embedding_space=None)")
                logger.info(f"   Returning partial model card with current training progress")
                
                # Get current job progress from job_plan
                job_plan = session.get("job_plan", [])
                current_jobs = []
                for job in job_plan:
                    ...
                    if job_id:
                        try:
                            ...
                            if job_data:
                                ...
                                job_progress = job_data.get("progress", 0.0)
                                job_info = {
                            ...
                            logger.debug(f"Could not load job {job_id}: {e}")
                ...
                # Return partial model card with BIG WARNING
                partial_card = {
                    "⚠️ WARNING ⚠️": "PRELIMINARY RESULTS: STILL TRAINING",
                    "training_status": {
                        "complete": False,
                        "status": session_status,
                        "message": "🔴 Training in progress - this model card contains preliminary results only",
                        "warning": "DO NOT USE FOR PRODUCTION - MODEL NOT READY"
                    },
                    "model_info": {
                        "session_id": id,
                    ...
                    "data_info": {
                        "input_file": session.get("input_data") or session.get("input_filename"),
                        "num_rows": session.get("num_rows"),
                    },
                    "current_jobs": current_jobs,
                    "job_plan": job_plan,
                    "partial": True,  # Flag indicating this is a partial card
                    "refreshed_at": datetime.datetime.now().isoformat(),
                }
                # CRITICAL: Sanitize datetime objects to prevent JSON serialization crashes
                partial_card = sanitize_datetime_for_json(partial_card)
                return JSONResponse(partial_card)
            ...
            if embedding_space_path_str:
                ...
                if embedding_space_path_obj.exists():
                    ...
                    if model_card_path.exists():
                        logger.info(f"📋 Found existing model card at {model_card_path}")
                        
                        # Check if training just completed and model card is stale (partial/preliminary)
                        with open(model_card_path) as f:
                            model_card = json.load(f)
                        
                        is_partial = model_card.get("partial", False)
                        session_status = session.get("status", "unknown")
                        training_complete = session_status in ["done", "DONE", "completed", "COMPLETED"]
                        
                        if is_partial and training_complete:
                            logger.info(f"🔄 Training now complete but model card is partial - regenerating...")
                            # Delete stale partial card and regenerate below
                            try:
                                model_card_path.unlink()
                                logger.info(f"🗑️  Deleted stale partial model card")
                            except Exception as e:
                                logger.warning(f"⚠️  Could not delete stale model card: {e}")
                            # Continue to regeneration logic below
                        else:
                            # Model card is current - sanitize and return it
            ...
            # If not found locally, search backplane output directories
            if not job_dir:
                backplane_root = Path("/backplane/backplane1/sphere")
                if backplane_root.exists():
                    logger.debug(f"🔍 Job output not found locally, searching backplane for {id}...")
                    for host_dir in backplane_root.iterdir():
                        if host_dir.is_dir() and host_dir.name.startswith("host-"):
                            ...
                            if backplane_output_dir.exists() and backplane_output_dir.is_dir():
                                ...
                                logger.info(f"📦 Found job output in backplane: {host_dir.name}")
                                break
            
            # Process job_dir if found (either locally or in backplane)
            if job_dir:
                ...
                if model_card_path.exists():
                    ...
                    if is_partial and training_complete:
                        logger.info(f"🔄 Training now complete but model card is partial - will regenerate")
                        # Delete stale partial card and continue to regeneration logic
                        try:
                            model_card_path.unlink()
                            logger.info(f"🗑️  Deleted stale partial model card")
                        except Exception as e:
                            logger.warning(f"⚠️  Could not delete stale model card: {e}")
                        # Continue to search for embedding_space and regenerate below
                    else:
                        # Model card is current - sanitize and return it
                        model_card = sanitize_datetime_for_json(model_card)
                        return JSONResponse(model_card)
            ...
            if embedding_space_path:
                ...
                session_status = session.get("status", "unknown")
                training_just_completed = session_status in ["done", "DONE", "completed", "COMPLETED"]
                
                if training_just_completed:
                    logger.info(f"📋 Training complete but model card not found - generating now for session {id}")
                else:
                ...
                try:
                    ...
                    # Success - sanitize and return the model card
                    model_card = sanitize_datetime_for_json(model_card)
                    return JSONResponse(model_card)
    ...
    async def get_session_training_metrics(id: str) -> JSONResponse:
        ...
        if not metrics_path:
            ...
            try:
                ...
                # If not found in job outputs, search backplane
                if not fallback_paths:
                    logger.info("🔍 Job-based search failed, checking backplane...")
                    backplane_root = Path("/backplane/backplane1/sphere")
                    if backplane_root.exists():
                        for host_dir in backplane_root.iterdir():
                            if host_dir.is_dir() and host_dir.name.startswith("host-"):
    ...
    class PublishPartialFoundationRequest(BaseModel):
        ...
        session_name_prefix: Optional[str] = None  # Optional prefix for session ID
        publish: bool = True  # Move to /sphere/published/ directory
    ...
    async def train_single_predictor_on_session(id: str, request: TrainPredictorRequest) -> JSONResponse:
        ...
        try:
            ...
            # CRITICAL: Grid search results hardcoded here - fine_tune=True (+3.18pp), n_hidden_layers=7 (peak AUC 0.7409)
            training_spec = {
                ...
                "n_hidden_layers": 7,  # HARDCODED: Grid search showed peak performance (AUC 0.7409)
    ...
    async def train_on_partial_foundation(
        ...
        try:
            ...
            if checkpoint_epoch is not None:
                ...
                best_checkpoint = es_job_dir / "checkpoint_resume_training_BEST.pth"
            ...
            es_pickle_path = session_dir / "embedding_space.pickle"
            
            import pickle
            with open(es_pickle_path, 'wb') as f:
                pickle.dump(es, f)
...
"""
    ...
    async def publish_partial_foundation_endpoint(id: str, request: PublishPartialFoundationRequest) -> JSONResponse:
        ...
        try:
            ...
            # Optionally publish to /sphere/published/
            published_path = None
            if request.publish:
                try:
                    logger.info(f"📦 Publishing to /sphere/published/...")
                    publish_result = publish_session(new_session_id)
                    published_path = publish_result.get('published_path')
                    logger.info(f"✅ Published to: {published_path}")
                except Exception as e:
                    logger.warning(f"⚠️  Could not publish to /sphere/published/: {e}")
                        ...
                        try:
                            ...
                            if gpu_proc_match:
                                ...
                                try:
                                            ...
                                            try:
                                                with open(mapping_file, 'r') as f:
                                                    ...
                                                    if mapping.get('job_id') == job_id:
                                                        ...
                                                        logger.info(f"✅ Found training PID {job_pid} for job {job_id} from mapping file")
                                                        break
                                            except Exception:
                                                continue
                                    except Exception as e:
                                    ...
                                    # Fallback: search cmdline
                                    if not job_pid:
                                        for proc in psutil.process_iter(['pid', 'cmdline']):
                                            try:
                                                cmdline = proc.info.get('cmdline') or []
                                                cmdline_str = ' '.join(cmdline)
                                                if job_id in cmdline_str:
                                                    job_pid = proc.info['pid']
                                                    logger.info(f"✅ Found training PID {job_pid} for job {job_id} from cmdline")
                                                    break
                                            except (psutil.NoSuchProcess, psutil.AccessDenied):
                                                continue
    ...
    async def get_job_logs(
        ...
        try:
            ...
            if redis_progress.redis_available:
                redis_logs = redis_progress.get_log_messages(job_type, job_id, limit=limit)
            
            # Get job output directory for file-based logs
            from lib.job_manager import load_job
            job_data = load_job(job_id)
