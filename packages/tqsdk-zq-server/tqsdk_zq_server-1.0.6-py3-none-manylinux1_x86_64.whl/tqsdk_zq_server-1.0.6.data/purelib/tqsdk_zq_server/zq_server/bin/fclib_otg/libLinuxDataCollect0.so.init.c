...
#define CHECK(cond, fmt, ...) do { \
    if(!(cond)) { \
      fprintf(stderr, "implib-gen: libLinuxDataCollect0.so: " fmt "\n", ##__VA_ARGS__); \
...
static void *load_library() {
  ...
  extern void *libLinuxDataCollect0_load_callback(const char *lib_name);
  lib_handle = libLinuxDataCollect0_load_callback("libLinuxDataCollect0.so");
  CHECK(lib_handle, "callback 'libLinuxDataCollect0_load_callback' failed to load library");
#else
  lib_handle = dlopen("libLinuxDataCollect0.so", RTLD_LAZY | RTLD_GLOBAL);
...
extern void *_libLinuxDataCollect0_so_tramp_table[];

// Can be sped up by manually parsing library symtab...
void _libLinuxDataCollect0_so_tramp_resolve(int i) {
  ...
  _libLinuxDataCollect0_so_tramp_table[i] = dlsym(h, sym_names[i]);
  CHECK(_libLinuxDataCollect0_so_tramp_table[i], "failed to resolve symbol '%s'", sym_names[i]);
}

// Helper for user to resolve all symbols
void _libLinuxDataCollect0_so_tramp_resolve_all(void) {
  size_t i;
  for(i = 0; i + 1 < sizeof(sym_names) / sizeof(sym_names[0]); ++i)
    _libLinuxDataCollect0_so_tramp_resolve(i);
}
