  ...
  >
    <div class="train-dialog-content">
      <el-form :model="trainForm" label-width="100px">
        ...
        <el-form-item label="选择模型" required>
          ...
          <div class="form-tip">提示：选择模型后将使用该模型进行远程训练</div>
        </el-form-item>
        ...
        <!-- 训练说明 -->
        <el-form-item>
          <el-alert title="训练说明" type="info" :closable="false" show-icon>
            <template #default>
              <ul class="train-tips">
                <li v-if="selectedModel">选择了模型后，将使用该模型进行远程训练</li>
                <li v-else>请选择一个模型，系统将使用该模型进行远程训练</li>
                <li>训练将使用该模型关联的所有标签的标注数据</li>
                <li>训练任务将在远程服务器上执行</li>
              </ul>
            </template>
          </el-alert>
        </el-form-item>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" :loading="trainLoading" :disabled="!selectedModel" @click="handleTrain">
...
const handleTrain = () => {
  ...
  // 触发训练事件
  emit("train", selectedModel.value);
};
</script>
