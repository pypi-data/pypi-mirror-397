...
# FileName:     order_report_fill.py
# Description:  订单报表回填控制器
# Author:       ASUS
...
async def order_unlock(logger: Logger, page: OrderDetailPage, remark: str, order_id: int, timeout: float = 5.0) -> None:
    # 1. 获取订单操作锁的状态
...
async def fill_procurement_info(
        logger: Logger, page: OrderDetailPage, order_id: int, out_ticket_platform_type: str, out_ticket_platform: str,
        out_ticket_account: str, purchase_account_type: str, purchase_account: str, purchase_amount: float,
...
async def fill_itinerary(
        logger: Logger, page: OrderDetailPage, order_id: int, passengers_itinerary: Dict[str, Any], timeout: float = 5.0
) -> None:
    # 1. 获取订单操作锁的状态
