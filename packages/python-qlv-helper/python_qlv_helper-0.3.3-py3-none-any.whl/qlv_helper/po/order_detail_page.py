...
class OrderDetailPage(BasePo):
    ...
    async def get_order_lock_state_btn(self, timeout: float = 5.0) -> Tuple[str, Locator]:
        ...
        """
        selector_other_locked: str = '//div[@class="order_information "]//a[@class="view_detail_red" and contains(text(), "强制解锁")]'
        selector_unlock: str = '//div[@class="order_information "]//a[@class="view_detail_green" and contains(text(), "锁定")]'
        selector_self_locked: str = '//div[@class="order_information "]//a[@class="view_detail_green" and contains(text(), "解锁返回")]'
        try:
            locator: Locator = await self.get_locator(selector=selector_other_locked, timeout=timeout)
            # text: str = (await locator.inner_text()).strip()
            return "强制解锁", locator
        except (PlaywrightTimeoutError, PlaywrightError, RuntimeError, Exception):
            pass
        try:
            locator: Locator = await self.get_locator(selector=selector_unlock, timeout=timeout)
            # text: str = (await locator.inner_text()).strip()
            return "锁定", locator
        except (PlaywrightTimeoutError, PlaywrightError, RuntimeError, Exception):
            pass
        try:
            locator: Locator = await self.get_locator(selector=selector_self_locked, timeout=timeout)
            # text: str = (await locator.inner_text()).strip()
            return "解锁返回", locator
        except (PlaywrightTimeoutError, PlaywrightError, RuntimeError, Exception):
            pass
        raise RuntimeError("订单详情页面，获取订单锁单状态按钮失败")
    ...
    async def get_out_ticket_platform_type_dropdown(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//select[@id="OutTktPFTypeID"]'
    ...
    async def get_out_ticket_platform_type_select_option(self, select_option: str, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = f'//select[@id="OutTktPFTypeID"]/option[contains(text(), "{select_option}")]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_out_ticket_platform_dropdown(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//select[@id="OutTktPF"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_out_ticket_platform_select_option(self, select_option: str, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = f'//select[@id="OutTktPF"]/option[contains(text(), "{select_option}")]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_out_ticket_account_dropdown(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//select[@id="AccountNumber"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_out_ticket_account_select_option(self, select_option: str, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = f'//select[@id="AccountNumber"]/option[contains(text(), "{select_option}")]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_purchase_account_type_dropdown(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//select[@id="TypeName"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_purchase_account_type_select_option(self, select_option: str, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = f'//select[@id="TypeName"]/option[contains(text(), "{select_option}")]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_purchase_account_dropdown(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//select[@id="PurchaseAccount"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_purchase_account_select_option(self, select_option: str, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = f'//select[@id="PurchaseAccount"]/option[contains(text(), "{select_option}")]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_purchase_amount_input(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//input[@id="TransactionAmount"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_main_check_input(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//input[@id="MainCheckNumber"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_air_comp_order_id_input(self, timeout: float = 5.0) -> Locator:
        ...
        :return: (是否存在, 错误信息|元素对象)
        """
        selector: str = '//input[@id="AirCoOrderID"]'
        return await self.get_locator(selector=selector, timeout=timeout)
    ...
    async def get_passenger_itinerary_locators(self, timeout: float = 5.0) -> List[Dict[str, Any]]:
        ...
        for i in range(row_count):
            ...
            # 1️⃣ 用户名
            name_locator = await self.get_sub_locator(locator=row, selector='span[name="pname"]', timeout=timeout)
            username = (await name_locator.first.inner_text()).strip()

            # 2️⃣ 身份证号
            # a 标签里：javascript:showIDNo('身份证号', ...)
            id_link = await self.get_sub_locator(locator=row, selector='a[id^="IDNo_"]', timeout=timeout)
            ...
            ticket_input = await self.get_sub_locator(locator=row, selector='input[name="ticketNo"]', timeout=timeout)
        ...
        if results:
            ...
            raise RuntimeError("订单详情页面，乘客信息栏，获取票号输入框解析异常")
