...
class QueryGeneratorTeradata(QueryGeneratorBase):
    ...
    def generate_statement_table_chunks_md5(self, table_context: TableContext) -> str:
        """Generate SQL statement to create a table for storing MD5 checksums of data chunks.
        ...
        """
        return table_context.sql_generator.generate_statement_table_chunks_md5(
    ...
    def generate_extract_chunks_md5_query(self, table_context: TableContext) -> str:
        """Generate SQL query to extract MD5 checksums for all chunks of a table.
        ...
        """
        return table_context.sql_generator.generate_extract_chunks_md5_query(
            platform=self.platform.value,
            normalized_fully_qualified_name=table_context.normalized_fully_qualified_name,
            database_name=table_context.database_name,
            schema_name=table_context.schema_name,
    ...
    def generate_extract_md5_rows_chunk_query(
        ...
        """Generate SQL query to extract MD5 rows for a specific chunk of a table.
        ...
        """
        return table_context.sql_generator.generate_extract_md5_rows_chunk_query(
            platform=self.platform.value,
            chunk_id=chunk_id,
    ...
    def _generate_compute_md5_chunk_query(
        ...
        """Generate SQL query to compute MD5 hashes for a specific chunk of rows.

        Args:
            table_context: Configuration object containing table properties.
            chunk_id: The unique identifier for the chunk.
            fetch: Number of rows to fetch.
            offset: Starting row offset.

        Returns:
            Union[str, list[str]]: SQL query string or list of query strings.
        ...
        # First get the DDL statements - each in its own template
        template_path = "teradata_create_row_concatenated.sql.j2"
        create_row_concatenated_template = table_context.sql_generator.env.get_template(
            template_path
        )
        create_row_concatenated_sql = create_row_concatenated_template.render(
            chunk_id=chunk_id,
            index_column_collection=table_context.index_column_collection,
            table_id=table_context.id,
        )
        queries.append(create_row_concatenated_sql)

        template_path = "teradata_create_row_md5.sql.j2"
        create_row_md5_template = table_context.sql_generator.env.get_template(
            template_path
        )
        create_row_md5_sql = create_row_md5_template.render(
            chunk_id=chunk_id,
            index_column_collection=table_context.index_column_collection,
            table_id=table_context.id,
        )
        queries.append(create_row_md5_sql)

        # Then get the DML statements
        template_path = "teradata_compute_md5_sql.j2"
        compute_md5_template = table_context.sql_generator.env.get_template(
            template_path
        )
        compute_md5_sql = compute_md5_template.render(
            chunk_id=chunk_id,
        ...
        queries.append(compute_md5_sql)
