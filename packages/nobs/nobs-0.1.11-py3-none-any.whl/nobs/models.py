...
from typing import Awaitable, Callable, Generic, Literal, Protocol, Sequence, TypeVar
from pydantic import BaseModel, Field, SecretStr, ValidationError
from pydantic_core import PydanticUndefined
from pydantic_settings import BaseSettings

from nobs.secrets import ResourceTags, NatsConfig, ObjectStorageConfig, SqsConfig, SqsQueueSettings, UnleashConfig, Environments
        ...
        for name, field in settings_type.__pydantic_fields__.items():
            ...
            values.append(
                SecretValue(
                    ...
                    tags=[ tag for tag in field.metadata if isinstance(tag, str) ]
                )
            )
    return values


def settings_for_secrets(secrets: list[SecretValue], resources: dict[ResourceTags, str]) -> dict[str, str]:
    vals = {}
...
class FastAPIApp:
    ...
    def network_app(self, root_path: Path) -> NetworkApp:
        ...
        if self.app is None:
            command = ["/bin/bash", "-c", f"fastapi run --host 0.0.0.0 --port {self.port}"]
            ...
            command = ["/bin/bash", "-c", f"uvicorn {relative_file}:app --host 0.0.0.0 --port {self.port}"]
...
class CompiledJob(BaseModel):
    ...
    def command(self, project_name: str) -> str:
        ...
        return f"python -m nobs.cli run-job --project-name={project_name} --job-id={self.id()} --file-ref {function_ref} --args '{encoded_args}'"
...
class PubSub(Generic[T]):
    ...
    async def publish(self, content: T) -> None:
        from nobs.secrets import ResourceTags
...
class Worker:
    ...
    compute: Compute = field(default_factory=Compute)
    secrets: list[type[BaseSettings]] | type[BaseSettings] = field(default_factory=lambda: [SqsConfig])
    ...
    def read() -> "NobsServer":
        try:
            ...
            if not default_nobs_file.is_file():
                raise ValueError("Found no env vars or config file. Try to run `nobs auth`.")
