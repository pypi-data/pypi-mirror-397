...
async def convert_rt_struct_to_nii_labels(
    ...
    for task_idx, task in enumerate(tasks):
        for series_idx, series in enumerate(task.get("series", []) or []):
            ...
            mask, segment_map = convert_rt_struct_to_nii(
...
async def process_segmentation_upload(
    ...
    task_name = (
        task.get("name") or task.get("taskId") or (task.get("items") or ["test"])[0]
    )
    ...
    if (
        ...
        logger.debug(f"`{task_name}`: Converting segmentations to labelsMap")
        task["labelsMap"] = [
        ...
        logger.debug(f"`{task_name}`: Converting labelsPath to labelsMap")
    ...
    for volume_index, label_map in enumerate(labels_map):
        logger.debug(
            f"`{task_name}`: Processing label map: {label_map} for volume {volume_index}"
        )
        if not isinstance(label_map, dict) or not label_map.get("labelName"):
            logger.debug(
                f"`{task_name}`: Skipping label map: {label_map} for volume {volume_index}"
            )
            continue
        ...
        if isinstance(input_labels_path, str) and os.path.isdir(input_labels_path):
            logger.debug(f"`{task_name}`: Label path is a directory")
            input_labels_path = [
            ...
            if any(not os.path.isfile(input_path) for input_path in input_labels_path):
                logger.debug(f"`{task_name}`: Not all paths are files")
                input_labels_path = None
        elif input_labels_path:
            if isinstance(input_labels_path, str):
                logger.debug(f"`{task_name}`: Label path is string")
            ...
            if not input_labels_path or any(
                ...
                if external_paths:
                    logger.debug(
                        f"`{task_name}`: Downloading {len(external_paths)} segmentations"
                    )
                    ...
                    for ext_path, down_path in zip(external_paths, downloaded_paths):
                        if down_path:
                            ...
                            logger.warning(
                                f"`{task_name}`: Failed to download label: {ext_path}"
                            )
        ...
        logger.debug(f"`{task_name}`: Total {len(input_labels_path or [])} labels")
        if input_labels_path:
            ...
            async with semaphore:
                ...
                output_labels_path, instances, labels_error = (
                    await loop.run_in_executor(
                        ...
                        {
                            series_key: path_mapping.get(series_val, series_val)
                            or series_val
            ...
            if (
                ...
                logger.debug(f"`{task_name}`: Re-fetching output: {output_labels_path}")
            ...
            if output_labels_path and os.path.isfile(output_labels_path):
                ...
                if (
                    ...
                    logger.warning(f"`{task_name}`: Error uploading labels")
            elif output_labels_path:
                label_map["labelName"] = output_labels_path
            else:
                if labels_error:
                    raise ValueError(
                        f"`{task_name}`: Error processing labels: {labels_error}"
                    )

                logger.warning(f"`{task_name}`: Skipping empty labels")
                label_map["labelName"] = None
        else:
            raise ValueError(f"`{task_name}`: Invalid label files")

    labels_data_path: Optional[str] = None

    if len(task.get("labels") or []) > LABELS_ARRAY_LIMIT:
        logger.debug(f"`{task_name}`: Uploading labels data")
        ...
        if (
            ...
            raise ValueError(f"`{task_name}`: Error uploading labels")

    logger.debug(f"`{task_name}`: Processed segmentations for task")
