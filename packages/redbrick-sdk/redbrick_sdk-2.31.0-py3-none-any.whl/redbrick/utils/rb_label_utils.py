...
from redbrick.utils.common_utils import series_image_headers
...
def dicom_rb_format(
    ...
    if not task.get("seriesInfo"):
        task["seriesInfo"] = [{}]
    if "items" in task and any(
        not series.get("itemsIndices") for series in task["seriesInfo"]
    ):
    ...
    for volume_index, series_info in enumerate(task["seriesInfo"]):
        ...
        series_headers = series_image_headers(series_info)
        if series_headers:
            series["imageHeaders"] = series_headers

        if "itemsIndices" in series_info:
            series["items"] = []
    ...
    if no_consensus:
        if task.get("consensusTasks"):
            ...
            if consensus_task.get("tdId"):
                task["tdId"] = consensus_task["tdId"]
            elif task.get("tdId"):
                del task["tdId"]
        ...
        if (
            ...
            if task.get("tdId"):
                output["superTruth"]["labelsId"] = task["tdId"]
            dicom_rb_series(
        ...
        for consensus_idx, output_consensus_task in enumerate(output["consensusTasks"]):
            ...
            if consensus_task.get("tdId"):
                output_consensus_task["labelsId"] = consensus_task["tdId"]
            dicom_rb_series(
                item_index_map,
                consensus_task,
