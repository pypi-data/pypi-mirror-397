...
def convert_nii_to_rt_struct(
    ...
    try:
        ...
        for nifti_file in nifti_files:
            ...
            if not isinstance(img, Nifti1Image) and not isinstance(img, Nifti2Image):
                return None, {}
            ...
            for instance in unique_instances:
                ...
                category = cat.get("category") if isinstance(cat, dict) else cat
                roi_name = (
                    category if category and semantic_mask else f"Segment_{instance}"
                )
                if cat:
