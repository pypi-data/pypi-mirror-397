...
def merge_segmentations(
    ...
    try:
        nii = NiftiIO(input_file)
        assert nii.data is not None
        data = np.zeros(
            (nii.size,), dtype=np.uint8 if max(instances.values()) < 256 else np.uint16
        )
        merge_instances(nii.data, data, instances)
...
def process_download(
    ...
    try:
        ...
        if png_mask and label_map_data["masks"]:
            label_map_data["png_mask"], label_map_data["masks"] = convert_nii_to_png(
                (
                    [label_map_data["masks"]]
                    if isinstance(label_map_data["masks"], str)
                    else label_map_data["masks"]
...
def process_upload(
    ...
    for inst_id, mask in masks.items():
        if mask in reverse_masks:
            reverse_masks[mask] = reverse_masks[mask] + (int(inst_id),)
        else:
            reverse_masks[mask] = (int(inst_id),)
    ...
    try:
        ...
        if binary_mask:
            ...
            base_nz_data = base_data[base_nz]
            if base_nz_data.size > 0:
                file_instances.update(np.unique(base_nz_data).tolist())
        ...
        for file_ in files[1:]:
            ...
            if binary_mask:
                ...
                if data_nz_data.size > 0:
                    file_instances.update(np.unique(data_nz_data).tolist())
        ...
        if prune_segmentations:
            if file_excess := file_instances - map_instances:
                ...
                if match.any():
                    base_data[tuple(nz[match] for nz in base_nz)] = 0
        ...
        for nzidx, maskv in mask_data:
            ...
            for idx, unique_idxs in enumerate(unique_pairs):
                mask_v: int = maskv if is_int else int(unique_idxs[1])  # type: ignore
                if mask_v in file_excess:  # has been pruned
                    continue

                base_v: int = int(unique_idxs if is_int else unique_idxs[0])
                ...
                base_data[tuple(nz[midx] for nz in nzidx)] = mask_v
        ...
        if mask_data:
            final_nz = np.nonzero(base_data)
            if final_nz[0].size > 0:
                final_instances = set(np.unique(base_data[final_nz]).tolist())
            else:
                final_instances = set()
