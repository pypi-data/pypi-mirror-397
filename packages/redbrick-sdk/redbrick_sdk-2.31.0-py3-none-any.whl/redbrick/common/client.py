...
class RBClientImpl(RBClient):
    ...
    def _check_status_msg(self, response_status: int, start_time: float) -> None:
        ...
        if response_status == 404:
            raise ConnectionError(f"Endpoint not found: {self.url}")

        if response_status == 429:
            raise ConnectionError("Too Many Requests: You are being rate limited")
...
class RBClientLocal(RBClient):
    """Local client to communicate with RedBrick AI GraphQL Server."""

    def __init__(self, schema, context, url) -> None:
        """Construct RBClient."""
        self.api_key = ""
        self.url = url
        self.schema = schema
    ...
    def execute_query(
        ...
        """Execute a graphql query."""
        return asyncio.run(
            self.execute_query_async(None, query, variables, raise_for_error)  # type: ignore
        )
    ...
    async def execute_query_async(
        ...
        from ariadne import graphql  # type: ignore
        ...
        assert success, repr(success) + repr(result)
