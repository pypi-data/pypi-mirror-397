...
class ExportImpl(Export):
                ...
                if taxonomy.get("isNew") and (rt_struct or dicom_seg):
                    for idx, (series_dir, series) in enumerate(
                        ...
                        if dicom_seg:
                            ...
                            rtstruct, new_segment_map = convert_nii_to_rt_struct(
                                segmentations,
                                series_dir,
                                taxonomy.get("objectTypes", []) or [],
                                series.get("segmentMap", {}) or {},
    ...
    async def download_labels(
        ...
        presigned_urls = (
            presign_paths
            if is_presigned
            else self.context.export.presign_items(
        ...
        to_download: List[Tuple[Optional[str], Optional[str]]] = [
            (
                presign_path,
                (
                    uniquify_path(os.path.join(dirname, f"label-{idx}.json"))
                    if presign_path
                    else None
                ),
            )
        ...
        if len(downloaded) > 1 and isinstance(task.get("consensusTasks"), list):
            for idx in range(1, len(downloaded)):
                if (idx - 1) < len(task["consensusTasks"]) and (
                    fpath := downloaded[idx]
                ):
                    with open(fpath, "r", encoding="utf-8") as f_:
                        task["consensusTasks"][idx - 1]["labels"] = process_labels(
    ...
    async def download_and_process_segmentations(
        ...
        for label, path in zip(labels_map, paths):  # type: ignore
            if label and label.get("labelName"):
                label_map_data = process_download(
                    task.get("labels", []) or [],
                    path,
                    png_mask,
                    color_map,
        ...
        *,
        ...
        with tqdm.tqdm(my_iter, unit=" datapoints", leave=config.log_info) as progress:
            for task in progress:
                task = task_event_format(task, users)
    ...
    async def export_label_data(
        ...
        """Export label data."""
        if task.get("labelsDataPath"):
            await self.download_labels(task, taxonomy, task["labelsDataPath"], True)
        else:
            task["labels"] = process_labels(
                json.loads(task.get("labelsData") or "[]"), taxonomy
            )
    ...
    def export_task_labels(
        ...
        *,
        ...
        """Export labels for a task.

        Parameters
        -----------
        task_id: str
            Task ID for which to export the labels.

        labels_id: Optional[str] = None
            If set, exports the specified label version, else exports the latest labels of the task.
        ...
        """
        # pylint: disable=too-many-locals
        labels = self.context.export.task_data(
            self.project.org_id, self.project.project_id, task_id, labels_id
        )

        if not labels or not labels.get("tdId"):
            raise ValueError(f"No labels found for task ID: {task_id}")

        # Create output directory
        destination = destination or os.path.join(
            self.project.project_id, labels["tdId"]
        )
        ...
        task = {
            **labels,
            "taskId": task_id,
            "createdBy": user_format(labels.get("createdByEntity"), users),
        }
        output: Dict = asyncio.run(
            self.export_label_data(  # type: ignore
                task,
                self.project.taxonomy,
                segmentation_dir,
                semantic_mask,
