...
async def create_task(
    *,
    ...
    try:
        ...
        logger.debug(
            f"org={org_id}, workspace={workspace_id}, project={project_id}, task={point['name']}, "
            ...
            + f"prune={prune_segmentations}, update={update_items}, append={append}"
        )
        ...
        if update_items:
            logger.debug(f"`{point['name']}`: Updating task items")
            ...
            logger.debug(f"`{point['name']}`: Creating task")
            ...
            logger.debug(f"`{point['name']}`: Created task {response}")

        return response
    except Exception as error:  # pylint:disable=broad-except
        log_error(f"Error: `{point['name']}`")
        log_error(error)
        raise error
...
async def create_tasks(
    *,
    ...
    jobs = concurrency // 6 if concurrency >= 6 else 1
    ...
    for point, task in zip(points, tasks):  # type: ignore
        if isinstance(task, Exception):
            output.append({"name": point["name"], "error": str(task)})
            log_error(
                f"`{point['name']}`: Error {'updating' if update_items else 'uploading'} - {task}"
            )
        else:
            output.append({"name": point["name"], "response": task})
