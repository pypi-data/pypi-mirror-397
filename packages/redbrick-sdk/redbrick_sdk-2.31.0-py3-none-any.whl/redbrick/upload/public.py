...
class UploadImpl(Upload):
    ...
    def archive_tasks(self, task_ids: List[str], concurrency: int = 50) -> bool:
        ...
        """
        return self.send_tasks_to_stage(task_ids, "ARCHIVED", concurrency)
        ...
        try:
            async with get_session() as session:
                ...
                temp = await gather_with_concurrency(
                    min(concurrency, 10),
                    *coros,
                    progress_bar_name="Updating tasks",
                    keep_progress_bar=True,
                )
        finally:
    ...
    def send_tasks_to_stage(
        ...
        errors = asyncio.run(
            self._send_to_stage(
                ...
                (
                    "Archiving tasks"
                    if stage_name == "ARCHIVED"
                    else f"Sending tasks to {stage_name}"
                )
                + f" ({concurrency} per batch)",
            )
        )
        if errors:
            log_error(errors[0])
