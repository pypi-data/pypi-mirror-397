...
class RBOrganizationImpl(RBOrganization):
    ...
    def projects_raw(
        ...
        projects = self.context.project.get_projects(
            self._org_id, include_archived, with_stages, with_taxonomy
        )
        projects = list(
            filter(
    ...
    def projects(self, include_archived: bool = False) -> List[RBProject]:
        ...
        projects = self.projects_raw(include_archived, True, True)
        return [
            RBProjectImpl(
                self.context, self._org_id, proj["projectId"], include_archived, proj
            )
    ...
    def create_project_advanced(
        ...
        try:
            ...
            if not (project_data.get("ok") or False):
                raise Exception(
                    (project_data.get("errors") or ["Unknown error occurred"])[0]
                )
        except ValueError as error:
            raise Exception(
