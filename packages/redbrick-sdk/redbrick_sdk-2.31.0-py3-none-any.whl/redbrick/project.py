...
class RBProjectImpl(RBProject):
    ...
    def __init__(
        ...
        _ = self.stages
        stages: List[Dict] = self._stages or []
    ...
    def taxonomy(self) -> Taxonomy:
        ...
        if not (set((self._taxonomy or {}).keys()) - {"orgId", "taxId", "name"}):
            self._taxonomy = self.context.project.get_taxonomy(
                org_id=self.org_id, tax_id=None, name=self._taxonomy["name"]
            )
    ...
    def __wait_for_project_to_finish_creating(self, project: Optional[Dict]) -> Dict:
        if not project or project.get("status") == "CREATING":
            project = {}
            try:
    ...
    def _get_project(self, project: Optional[Dict]) -> None:
        """Get project to confirm it exists."""
        project = self.__wait_for_project_to_finish_creating(project)

        self._project_name = project["name"]
        self.td_type = project["tdType"]
        self._taxonomy = project["taxonomy"]
        self._workspace_id = (project.get("workspace", {}) or {}).get("workspaceId")
        self._stages = project.get("stages") or []
        self._project_url = project["projectUrl"]
        self._created_at = parser.parse(project["createdAt"])
        self._updated_at = parser.parse(project["updatedAt"])
