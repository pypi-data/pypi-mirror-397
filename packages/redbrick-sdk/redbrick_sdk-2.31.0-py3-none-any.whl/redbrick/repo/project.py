...
class ProjectRepoImpl(ProjectRepo):
    ...
    def get_project(self, org_id: str, project_id: str) -> Dict:
        ...
        """
        query = f"""
            query getProjectSDK($orgId: UUID!, $projectId: UUID!) {{
                project(orgId: $orgId, projectId: $projectId) {{
                    {project_shard(True, True)}
                }}
            }}
        """
    ...
    def get_stages(self, org_id: str, project_id: str) -> List[Dict]:
        """Get stages."""
        query = f"""
            query getStagesSDK($orgId: UUID!, $projectId: UUID!) {{
    ...
    def create_project(
        ...
        response: Dict[str, Dict] = (
            self.client.execute_query(query, variables).get("createProjectSimple") or {}
        )
        return {
            "orgId": org_id,
            "ok": response.get("ok") or False,
            "errors": response.get("errors") or [],
            "projectId": (response.get("project") or {}).get("projectId"),
        }
        ...
        query = f"""
            query getProjectsSDK($orgId: UUID!, $active: Boolean) {{
                projects(orgId: $orgId, active: $active) {{
                    {project_shard(with_stages, with_taxonomy)}
                }}
            }}
        """
