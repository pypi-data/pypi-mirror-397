...
from vibemem.models import ListFilters, Memory, MemoryUpdate, SearchHit, memory_from_properties, memory_to_weaviate_properties
...
class WeaviateStore:
    ...
    def _ensure_collection(self) -> None:
        ...
        for vectorizer in (Configure.Vectorizer.text2vec_transformers(), Configure.Vectorizer.none()):
            try:
    ...
    def add(self, memory: Memory) -> Memory:
        ...
        try:
            self._collection.data.insert(properties=props, uuid=memory.id)
        except Exception as e:
    ...
    def edit(self, memory_id: str, update: MemoryUpdate) -> Memory:
        ...
        merged.updated_at = __import__("datetime").datetime.now(__import__("datetime").timezone.utc).replace(
            microsecond=0
        ).isoformat()

        props = memory_to_weaviate_properties(merged)
        try:
            self._collection.data.replace(uuid=memory_id, properties=props)
        except Exception as e:
