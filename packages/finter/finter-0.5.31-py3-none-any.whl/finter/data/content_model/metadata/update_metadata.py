...
def generate_note(cm_name: str, category: str, universe: str, code_mappings: dict[str, str]) -> str:
    ...
    if "gics" in cm_name.lower():
        ...
        if universe == "us_stock":
            parts.append(f'cf.get_fc("pit-{cm_name}")')
        else:
...
def parse_datasets_md(file_path: Path) -> tuple[list[CMItem], dict[str, str]]:
    ...
    for line in content.split("\n"):
        ...
        if line.startswith("Example") or line.startswith("Metadata"):
            in_item_list = False
            continue

        if in_item_list and current_universe:
            item_match = item_pattern.match(line) or bullet_pattern.match(line)
            if item_match:
...
def dump_yaml(items: list[CMItem], output_dir: Path, code_mappings: dict[str, str]):
    """Dump items to YAML files per universe."""
    # Group by universe
        ...
        with open(yaml_path, "w", encoding="utf-8") as f:
            ...
            for category in sorted(by_category.keys()):
                ...
                for item in sorted(by_category[category], key=lambda x: x.cm_name):
                    ...
                    f.write(f"  note: '{generate_note(item.cm_name, item.category, item.universe, code_mappings)}'\n")
                    f.write("\n")
