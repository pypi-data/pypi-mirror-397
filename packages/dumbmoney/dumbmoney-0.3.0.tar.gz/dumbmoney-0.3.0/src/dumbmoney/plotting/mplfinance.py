...
class MPLFinancePlotter(Plotter):
  ...
  def create_series_plots(self, indicators: List[Indicator]) -> List[SeriesPlot]:
    ...
    for indicator in indicators:
      ...
      if isinstance(indicator, MACD):
        plts = self._macd_to_seriesplots(
          indicator,
          panel=panel
        )
    ...
    series_plots.extend(
      self._ma_indicators_to_seriesplots(
        ma_indicators,
        panel=0
      )
    )
  ...
  def _macd_to_seriesplots(
    ...
    fb_positive = dict(y1=hist.values, y2=0, where=hist > 0, color=Color.GREEN, alpha=0.6, interpolate=True, panel=panel)
    fb_negative = dict(y1=hist.values, y2=0, where=hist < 0, color=Color.RED, alpha=0.6, interpolate=True, panel=panel)
    histogram = SeriesPlot(
      ...
      params=dict(
        secondary_y=False,
        fill_between=[fb_positive, fb_negative]
      )
    )
