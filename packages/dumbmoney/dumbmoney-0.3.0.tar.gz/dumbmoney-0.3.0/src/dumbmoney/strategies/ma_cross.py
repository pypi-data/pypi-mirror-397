...
class MACrossParams:
  fast_window: int = 5
  slow_window: int = 20
  min_cross_strength: float = 0.0  # minimum difference between MAs to consider a valid cross
...
class MACrossStrategy(Strategy):
  ...
  def generate_signals(self, ohlcv: OHLCVData) -> SignalFrame:    
    ma_fast = MovingAverage(window=self.params.fast_window,)
    ma_fast.compute(ohlcv)
    ma_slow = MovingAverage(window=self.params.slow_window,)
    ...
    sig_frame.signals.loc[cross_up, "strength"] = 1.0 # could be refined based on cross magnitude
    
    # Generate FLAT signals
    sig_frame.signals.loc[cross_down, "signal_type"] = SignalType.FLAT
    sig_frame.signals.loc[cross_down, "strength"] = 1.0 # could be refined based on cross magnitude
