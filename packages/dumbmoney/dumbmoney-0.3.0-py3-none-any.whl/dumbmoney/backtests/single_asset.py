...
class SingleAssetBacktester:
  ...
  def _compute_metrics(
    ...
    ann_return = ((1 + total_return) ** (252 / len(equity_curve))) - 1.0 # assuming daily bars and 252 trading days
    ...
    if returns.std() > 1e-6:
      sharpe_ratio = (returns.mean() - self.risk_free_rate / 252) / returns.std() * (252 ** 0.5)
    else:
      sharpe_ratio = None
