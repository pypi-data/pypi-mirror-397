...
class Indicator(ABC):
  ...
  name: str
  inputs: List[str] = field(default_factory=list)           # names of columns used to compute the indicator values, e.g. ['close', 'volume']
  params: Dict[str, Any] = field(default_factory=dict)      # hyper-parameters, e.g. {'window': 14}
  output_names: Optional[List[str]] = field(default=None)   # names of output columns, e.g. ['ma20']
  ...
  def values(self) -> pd.DataFrame:
    ...
    if self.indicator_values is None:
      raise ValueError("Indicator values have not been computed yet. Call compute() first.")
  ...
  def compute(self, ohlcv: OHLCVData) -> pd.DataFrame:
    ...
    if isinstance(self.output_names, list) and len(self.output_names) == len(indicator_values.columns):
