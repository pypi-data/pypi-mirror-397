...
class CQL3D_GUI(tk.Tk):
    def __init__(self, yaml_file):
        ...
        if os.path.exists('./fpld_folder') and os.path.isdir('./fpld_folder'):
            self.fpld_folder = os.path.join(self.folder_dir,'fpld_folder')
...
#        # 2. Restore position on visibility
#        def _restore_sash_pos(event=None):
#            if hasattr(self, 'shared_sash_x') and self.shared_sash_x is not None:
#                # Use .after to ensure widget is drawn
#                def _apply():
#                    set_sash_position(content_split, self.shared_sash_x)
#                tab.after(10, _apply)
#            else:
#                # Optional: Initial 3:2 split if no saved state
#                def _initial_split():
#                    try:
...
#                    except: pass
#                tab.after(50, _initial_split)
#
#        tab.bind("<Visibility>", _restore_sash_pos)
...
#                widget.bind('<FocusOut>', on_focus_out)
    ...
    def run_cql3d(self):
        ...
        # 5. If fpld_dsk is required, create a symlink to 'fpld_dsk' INSIDE the new folder
        fpld = self.get_widget_value('fpld')
        if fpld and fpld != 0:
            try:
                # 1. Determine Source Path
                if os.path.isabs('fpld_dsk'):
                    src_path = 'fpld_dsk'
                else:
                    src_folder = getattr(self, 'fpld_folder', os.getcwd())
                    src_path = os.path.join(src_folder, 'fpld_dsk')

                # 2. Determine Destination Path
                # Always use basename to avoid creating subfolders
                dst_path = os.path.join(run_folder, 'fpld_dsk')

                if not os.path.exists(src_path):
                    messagebox.showerror("Error", f"fpld_dsk file not found at:\n{src_path}")
                    return
                ...
                messagebox.showerror("Error", f"Error linking fpld_dsk file '{file_name}':\n{e}")
                return
