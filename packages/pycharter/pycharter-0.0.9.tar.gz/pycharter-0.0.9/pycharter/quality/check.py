...
class QualityCheck:
        ...
        for i, (record_data, result) in enumerate(zip(data_list, validation_results)):
            if not result.is_valid:
                ...
                self.violation_tracker.record_violation(
                    ...
                    deduplicate=options.deduplicate_violations,
                )
                violation_count += 1
    ...
    def _persist_quality_metrics(
        ...
        try:
            ...
            # Check if metric already exists for this data (if skip_if_unchanged or deduplication)
            if skip_if_unchanged and data_fingerprint:
                ...
                if existing:
                    # Update timestamp but don't create duplicate
                    existing.check_timestamp = report.check_timestamp
                    self.db_session.commit()
                    import logging
                    logging.info(f"Updated existing quality metric (data unchanged)")
                    return

            # Try to find data_contract_id from schema_id
            data_contract_id = None
            if schema_id and self.store:
                try:
                    # Try to get contract info from store
                    # This is a simplified approach - you may need to adjust based on your store implementation
                    pass
                except Exception:
                    pass
