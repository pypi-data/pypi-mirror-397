...
class Markdown:
    def __init__(self, client: "rubxy.Client"):
        self.client = client

    def _format_text(self, text):
        if self.client.parse_mode is enums.ParseMode.HTML:
            text = self.to_markdown(text)
        return self.to_metadata(text)
    ...
    def to_metadata(self, text: str) -> Dict[str, Any]:
        ...
        return (
            current_text.strip(),
            {"meta_data_parts": meta_data_parts} if meta_data_parts else None
        )