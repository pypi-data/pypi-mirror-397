...
class BeaconClient:
    ...
    def _setup_opentelemetry(self) -> None:
        ...
        if hasattr(current_provider, '_atexit_handler'):
            # TracerProvider from SDK is already set up, don't override
            logger.debug('OpenTelemetry TracerProvider already configured, skipping auto-setup')
            return
    ...
    def create_span(
        ...
        *,
        ...
        if auto_parent:
            # First: Check SDK's custom ContextVar
            ...
            if current_span:
                ...
                # Fallback: Check OpenTelemetry's native context
                # This bridges SDK spans with OTel spans (e.g., from LangChain callbacks)
                try:
                    ...
                    if otel_span and otel_span.is_recording():
                        ...
                        if span_context.is_valid:
                            # Convert OTel trace_id (128-bit int) to hex string
                            trace_id = format(span_context.trace_id, '032x')
                            # Convert OTel span_id (64-bit int) to hex string for parent_id
                            parent_id = format(span_context.span_id, '016x')
                except ImportError:
                    # OpenTelemetry not installed, skip fallback
                    pass
