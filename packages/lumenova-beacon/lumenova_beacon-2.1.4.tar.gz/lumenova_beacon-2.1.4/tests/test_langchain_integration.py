...
class TestBeaconCallbackHandler:
    ...
    def test_init_with_defaults(self):
        ...
        with patch("lumenova_beacon.integrations.langchain.get_client") as mock_get:
            ...
            assert handler._default_session_id is None
    ...
    def test_init_with_custom_parameters(self):
        ...
        with patch("lumenova_beacon.integrations.langchain.get_client") as mock_get:
            ...
            assert handler._default_session_id == "test-session"
    ...
    def test_on_chain_start(self, handler, mock_client):
        ...
        assert "span.metadata.tags" in span.attributes
    ...
    def test_on_chain_end(self, handler, mock_client):
        ...
        # Check span was sent
        mock_client.send_span.assert_called_once()
        sent_span = mock_client.send_span.call_args[0][0]

        # Check span properties
        assert sent_span.end_time is not None
        assert sent_span.status_code == StatusCode.OK
        assert "span.output" in sent_span.attributes
    ...
    def test_on_llm_start(self, handler, mock_client):
        ...
        assert "span.input" in span.attributes
        assert "span.model.name" in span.attributes
        assert span.attributes["span.model.name"] == "gpt-4"
    ...
    def test_on_llm_end_with_token_usage(self, handler, mock_client):
        ...
        # Check token usage was captured as a single JSON object
        sent_span = mock_client.send_span.call_args[0][0]
        assert "span.usage_details" in sent_span.attributes

        # Verify it's properly formatted as JSON string
        import json
        usage_details = json.loads(sent_span.attributes["span.usage_details"])
        assert usage_details["prompt_tokens"] == 10
        assert usage_details["completion_tokens"] == 5
        assert usage_details["total_tokens"] == 15

        # Verify model name was captured
        assert "span.model.name" in sent_span.attributes
        assert sent_span.attributes["span.model.name"] == "gpt-4"
    ...
    def test_on_retriever_end(self, handler, mock_client):
        ...
        sent_span = mock_client.send_span.call_args[0][0]
        assert "span.output" in sent_span.attributes
        assert "span.metadata.document_count" in sent_span.attributes
    ...
    def test_on_agent_action(self, handler, mock_client):
        ...
        parent_span = handler._runs[str(parent_run_id)]["span"]
        action_key = f"span.metadata.agent_action_{str(action_run_id)[:8]}"
    ...
    def test_on_agent_finish(self, handler, mock_client):
        ...
        parent_span = handler._runs[str(parent_run_id)]["span"]
        assert "span.metadata.agent_finish" in parent_span.attributes
    ...
    def test_nested_tool_and_function_spans(self, handler, mock_client):
        ...
        assert mock_client.send_span.call_count >= 1

        # Get the function span (should be the first call)
        function_span = mock_client.send_span.call_args_list[0][0][0]
        ...
        assert mock_client.send_span.call_count >= 2
        sent_tool_span = mock_client.send_span.call_args_list[-1][0][0]
    ...
    def test_multiple_nested_function_calls_in_tool(self, handler, mock_client):
        ...
        function_calls = [call for call in mock_client.send_span.call_args_list]
    ...
    def test_context_cleared_after_root_langchain_span(self, handler, mock_client):
        ...
        function_span = mock_client.send_span.call_args_list[-1][0][0]
