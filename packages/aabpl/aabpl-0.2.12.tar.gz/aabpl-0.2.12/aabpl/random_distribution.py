...
def draw_random_points_in_sample_area(
    ...
    cells_fully_valid_max_lvl = grid.cells_fully_valid_max_lvl
    cells_partly_valid_max_lvl = grid.cells_partly_valid_max_lvl
    ...
    max_cells_fully_covered = max([
            sum([2**-(2*lvl) for lvl,(row, col) in cells_fully_valid_max_lvl if lvl==lvl_i])
            for lvl_i in set([lvl for lvl, (row, col) in cells_fully_valid_max_lvl])
        ])
    ...
    max_lvl_partly = max([lvl for lvl,(row,col) in (cells_partly_valid_max_lvl if len(cells_partly_valid_max_lvl)>0 else cells_fully_valid_max_lvl)])
    sample_cells_arr = _np_array([
        (lvl,row,col) for lvl,(row,col) in cells_fully_valid_max_lvl.union(
            [(lvl,(row,col)) for lvl,(row,col) in cells_partly_valid_max_lvl if lvl==max_lvl_partly]
        )
        ])
    ...
    if all_cells_eligible:
        ...
        sample_cells_arr = sample_cells_arr[sample_cells_arr[:,0]==min_lvl]
        def rand_int_transformer(rand_ints:_np_ndarray)->_np_ndarray:
            return rand_ints
        rand_int_stop = len(sample_cells_arr)
    else:
    ...
    while random_points_coordinates.shape[0] < n_random_points:
        ...
        if sample_area_contains_grid:
            ...
            rndm_cells_fully_valid = _np_array([
                ...
                )) in cells_fully_valid_max_lvl  for lvl,row,col in rndm_cells])
