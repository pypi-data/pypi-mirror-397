...
from zucaro.account import AccountManager
from zucaro.config import Config, ConfigManager
from zucaro.instance import InstanceManager
from zucaro.logging import logger
from zucaro.utils import Directory, cached_property
from zucaro.version import VersionManager
from zucaro.windows import get_appdata


def get_default_root():
    logger.debug("Resolving default application root")
    platforms = {
        "linux": lambda: Path("~/.local/share/zucaro").expanduser(),
        "win32": lambda: get_appdata() / ".zucaro",
        "darwin": lambda: Path("~/Library/Application Support/zucaro").expanduser(),
    }
...
class Launcher:
    ...
    def get_path(self, *pathsegments) -> Path:
        ...
        element of `pathsegments` is a `zucaro.utils.Directory`, it is resolved."""
