...
def _create_task_tool(
    *,
    ...
    def _validate_and_prepare_state(
        ...
        # For cloud subagents (cute-linear, cute-finish-itp, cute-screenshot), use minimal state with station_thread_id
        # For local subagents, use full state with messages
        if subagent_type in ["cute-linear", "cute-finish-itp", "cute-screenshot"]:
    ...
    def task(
        ...
        Args:
            description: Task description for the subagent
            subagent_type: Type of subagent to invoke
            inputs: Optional structured inputs as a dictionary
                   (e.g., {"borrower_name": "Smith", "loan_number": "12345"})
            ...
            if sync_station_id and variables_to_sync:
                try:
                    ...
                    sync_station = await asyncio.to_thread(
                        StationAgent,
                        station_thread_id=sync_station_id,  # For station sync, use the actual station_id
                        graph_thread_id=graph_thread_id,
                        token=station_token,
                        initial_state=None,  # Connect to existing station
                        langgraph_token=os.getenv("LANGGRAPH_TOKEN"),
