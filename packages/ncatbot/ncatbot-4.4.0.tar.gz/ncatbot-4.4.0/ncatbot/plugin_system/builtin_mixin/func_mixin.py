...
class FunctionMixin:
    ...
    @final
    def _create_func_handler(
        self, name: str, handler: Callable, filter_chain
    ) -> Callable:
        ...
        async def wrapped_handler(
            ...
            if not isinstance(data, BaseMessageEvent):
                LOG.error(
                    f"事件数据类型错误，预期 BaseMessageEvent，实际得到 {type(data)}"
                )
                return None
            try:
                ...
                LOG.error(
                    f"插件 {self.name} 功能 {current_name} 执行失败: {e}", exc_info=True
                )
                return None
