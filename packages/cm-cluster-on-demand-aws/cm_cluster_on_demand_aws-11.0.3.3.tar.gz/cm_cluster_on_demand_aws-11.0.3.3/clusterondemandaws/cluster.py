...
def construct_bright_tags(cluster_name: str, obj_name: str, bcm_type: str | None = None) -> list[TagTypeDef]:
    tags_dict: dict[str, str] = {
        BCM_CREATED_TAG_KEY: datetime.utcnow().isoformat() + "Z",
...
class Cluster:
    ...
    def _get_created_at(self) -> str | None:
        ...
        if self.vpc:
            created_at = self.get_tag_value(self.vpc.tags, BCM_CREATED_TAG_KEY, default="")
        if not created_at and self.primary_head_node:
            created_at = self.get_tag_value(self.primary_head_node.tags, BCM_CREATED_TAG_KEY, default="")
