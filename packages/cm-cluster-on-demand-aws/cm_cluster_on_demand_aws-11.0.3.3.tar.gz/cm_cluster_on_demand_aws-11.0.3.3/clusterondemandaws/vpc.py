...
def create_in_existing_vpc(cluster: Cluster) -> None:
    ...
    if config["configure_igw_in_existing_vpc"]:
        ...
        internet_gateway = _create_internet_gateway(cluster.vpc, ec2, cluster.name)
    ...
    if config["head_node_sg_id"] and config["node_sg_id"]:
        ...
        head_node_sg_id, node_sg_id = _create_security_groups(cluster.vpc, config["ingress_icmp"], cluster.name)
    ...
    cluster.primary_head_node = _create_head_node(
        ...
        placement_group_name, cluster.name)
    ...
    log.info(f"Done setting up VPC for {cluster.name}")
...
def create_vpc(cluster: Cluster) -> None:
    ...
    public_subnet, _ = _create_public_subnet(cluster.vpc, cluster.name)
    private_subnet, private_subnet_name = _create_private_subnet(cluster.vpc, cluster.name)

    if config["inbound_network_acl_rule"]:
        _create_network_acls(cluster.vpc, public_subnet, private_subnet, cluster.ec2c, cluster.name)
    ...
    if cluster.is_ha or config["configure_nat_gateway"]:
        ...
        _set_default_gateway_subnet(
            cluster.vpc, private_subnet, cluster.name, private_subnet_name, nat_gateway=nat_gateway
        )
    else:
        _set_default_gateway_subnet(
            cluster.vpc, private_subnet, cluster.name, private_subnet_name,
            head_node_instance=cluster.primary_head_node
        )

    log.info(f"Done setting up VPC for {vpc_name}")
...
def _create_internet_gateway(vpc: Vpc, ec2: EC2ServiceResource, cluster_name: str) -> InternetGateway:
    log.info("Adding internet connectivity...")

    internet_gateway_name = cluster_name
    tags = construct_bright_tags(cluster_name, internet_gateway_name)
...
def _create_public_subnet(vpc: Vpc, cluster_name: str) -> tuple[Subnet, str]:
    ...
    public_subnet_name = cluster_name + " public"
    tags = construct_bright_tags(cluster_name, public_subnet_name, BCM_TYPE_HEAD_NODE)
    kwargs["TagSpecifications"] = [{"ResourceType": "subnet", "Tags": tags}]

    public_subnet = vpc.create_subnet(**kwargs)
    time.sleep(0.5)

    return public_subnet, public_subnet_name


def _create_private_subnet(vpc: Vpc, cluster_name: str) -> tuple[Subnet, str]:
    ...
    private_subnet_name = cluster_name + " private"
    tags = construct_bright_tags(
        cluster_name, private_subnet_name, BCM_TYPE_HEAD_NODE
    )
    kwargs["TagSpecifications"] = [{"ResourceType": "subnet", "Tags": tags}]
    ...
    return private_subnet, private_subnet_name
...
def _set_default_gateway_subnet(
    ...
    route_table_name = subnet_name + " route table"
    tags = construct_bright_tags(cluster_name, route_table_name)
...
def _create_head_node(
    ...
    head_node_instance_and_ebs_name = (
        f"{cluster_name} (Bright COD-AWS {config['version']} head node)"
    )

    headnode_tags = construct_bright_tags(
        cluster_name, head_node_instance_and_ebs_name, BCM_TYPE_HEAD_NODE
    )
    ...
    headnode_ebs_tags = construct_bright_tags(
        cluster_name, head_node_instance_and_ebs_name
    )

    headnode_eni_tags = construct_bright_tags(
        cluster_name=cluster_name,
        obj_name=f"{cluster_name}-a network interface",
...
def _create_network_acls(
    vpc: Vpc, public_subnet: Subnet, private_subnet: Subnet, ec2_client: EC2Client, cluster_name: str
) -> None:
    ...
    log.info(f"Creating public and private Network ACLs for {cluster_name}")

    # Setup public subnet's ACL rule
    public_network_acl_name = cluster_name + " public network acl"
    public_tags = construct_bright_tags(cluster_name, public_network_acl_name)
    ...
    private_network_acl_name = cluster_name + " private network acl"
    private_tags = construct_bright_tags(cluster_name, private_network_acl_name)
...
def _create_security_groups(vpc: Vpc, ingress_icmp_cidr: list[str] | None, cluster_name: str) -> tuple[str, str]:
    head_sg_name = f"BCM {cluster_name}-headnode"
    head_sg_tags = construct_bright_tags(cluster_name, head_sg_name)
    head_sg = vpc.create_security_group(
        GroupName=head_sg_name,
        Description=(
            f"Security group for head node in BCM COD-AWS cluster {cluster_name}"
        ),
    ...
    node_sg_name = f"BCM {cluster_name}-node"
    node_sg_tags = construct_bright_tags(cluster_name, node_sg_name)
    node_sg = vpc.create_security_group(
        GroupName=node_sg_name,
        Description=(
            f"Security group for compute nodes in BCM COD-AWS cluster {cluster_name}"
        ),
