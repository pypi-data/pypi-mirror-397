            ...
            if query_columns:
                ...
                if export_mode == "pyarrow":
                    ...
                    with open(query_output_database_tmp, mode="a") as f, pgzip.open(
                        ...
                        # Chunk by fetch_arrow_batch seems to load all data in memory
                        # df = self.conn.execute(query).fetch_arrow_table(
                        #     rows_per_batch=chunk_size
                        # )  # .fetch_record_batch(chunk_size)

                        # Chunk by fetch_arrow_batch seems to load all data in memory
                        # df = self.conn.execute(query).fetch_arrow_reader(
                        #     rows_per_batch=chunk_size
                        # )  # .fetch_record_batch(chunk_size)
                        ...
                        # for d in df.to_batches():  # for fetch_arrow_table
                        for d in df:
                            ...
                            if export_options.get("format") in ["CSV"]:
                                ...
                                if (
                                    compression_mode_bgzip and False
                                ):  # Old code disabled because merge with csv export below

                                    # Code with polars write to bytes and then write to bgzip
                                    ...
                                    # # New code with pyarrow csv write to bytes and then write to bgzip
                                    # # Does not work because quoting_header not in this release of pyarrow
                                    ...
                                    # # If we need to write the CSV/TSV header based on columns
                                    # if header:
                                    #     # d.schema.names is available regardless of the Arrow batch type
                                    #     header_line = "\t".join(d.schema.names) + "\n"
                                    #     f.write(header_line.encode("utf-8"))

                                    # # Arrow to CSV/TSV to bytes
                                    ...
                                    # # Write the chunk to the bgzip
                                    # f.write(sink.getvalue().to_pybytes())

                                else:

                                    # Code with polars write directly to file
                                    # Due to structural types in Arrow, Polars is used to write CSV directly to file
                                    ...
                                    # # New code with pyarrow csv write to string and then write to file
                                    # # log.debug("Exporting CSV chunk with PyArrow...")

                                    # # Determine the quote option
                                    # quote_opt = export_options.get("quote", "")

                                    # if quote_opt:
                                    #     # With quote option
                                    #     quoting_style = "all_valid"  # PyArrow <15 : force quoting, always with "
                                    # else:
                                    #     # No quote
                                    #     quoting_style = "none"
                                    ...
                                    # # Write header manually if necessary
                                    # if header:
                                    #     header_line = (
                                    #         export_options.get("delimiter", "\t").join(
                                    #             d.schema.names
                                    #         )
                                    #         + "\n"
                                    #     )
                                    ...
                                    # # Write the Arrow batch via PyArrow CSV writer

                                    # # Prepare buffer sink
                                    # sink = pa.BufferOutputStream()
                                    # write_opts = pacsv.WriteOptions(
                                    #     delimiter=export_options.get("delimiter", "\t"),
                                    #     include_header=False,  # Header already written
                                    #     quoting_style=quoting_style,
                                    #     batch_size=chunk_size,
                                    # )
                                    # pacsv.write_csv(d, sink, write_options=write_opts)

                                    # # Write bytes to file (gzip or not)
                                    # if compression_mode_gzip:
                                    #     f.write(sink.getvalue().to_pybytes())
                                    # else:
                                    #     f.write(
                                    #         sink.getvalue().to_pybytes().decode("utf-8")
                                    #     )
