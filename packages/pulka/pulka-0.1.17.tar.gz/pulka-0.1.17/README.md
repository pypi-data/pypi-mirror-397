...
- Start in file browser mode by pointing Pulka at a directory (press `Enter` to
  open datasets from the listing):
...
- gu / r<space>: clear all selected rows
...
- viewport_top (zt), viewport_middle (zm), viewport_bottom (zb): jump within the visible viewport
...
- r_, resetmax: reset maximized column widths
- sort, filter <expr>, filter_value (+), filter_value_not (-), filter_eq <col> <value>
- reset (rr), reset_expr_filter (re), reset_sql_filter (rf), reset_sort (rs), goto <col>, render, quit
...
- transpose_row (t): transpose only the current row
...
- next_diff, prev_diff: repeat the last search or jump to next/previous selected row
- hide (d), unhide: hide current column or unhide all columns
...
- Use the `PULKA_TRANSPOSE_SAMPLE_ROWS` environment variable (legacy `PD_TRANSPOSE_SAMPLE_ROWS` is
  still recognised) or the `transpose [rows]` command to control how many rows are sampled for
  transpose mode; press `t` to transpose just the current row.
