...
from ...core.formatting import _format_float_two_decimals, _format_number_with_thousands_separator
...
class ColumnInsightPanel:
    ...
    # ------------------------------------------------------------------
    def _render_lines(self) -> list[PanelLine]:
        body = self._render_body_lines()
        lines: list[PanelLine] = [
            self._plain_line("", _BODY_STYLE),
            self._plain_line("Column Insight", _HEADER_STYLE),
        ]
        if body:
    ...
    def _render_stats_section(self, insight: ColumnInsight | None) -> list[PanelLine]:
        ...
        for label, value in ordered_keys:
            align_right = self._stat_label_align_right(dtype, label)
            if value is None:
    ...
    def _format_labeled_section(
        ...
        *,
        ...
        alignment = None
        if align_numeric and not placeholder and value_width > 0:
            samples: list[str] = []
    ...
    def _format_value_field(
        ...
        *,
        ...
        numeric = bool(entry.numeric is not None or self._looks_numeric(value))
    ...
    def _align_numeric_value(
        ...
        if available > 0:
            ...
            head = aligned if aligned is not None else self._fit_numeric_text(working, available)
        return f"{head}{suffix_text}"

    def _fit_numeric_text(self, text: str, width: int) -> str:
        if width <= 0:
            return ""
        visible = text
    ...
    def _format_decimal_plain(self, value: Decimal) -> str:
        ...
        sign = ""
        if left.startswith(("-", "+")):
            sign, left = left[0], left[1:]
        try:
            int_part = _format_number_with_thousands_separator(int(left or "0"))
        except Exception:
            int_part = left
        frac_part = f".{right}" if right else ""
    ...
    def _format_display_number(self, value: Any) -> tuple[str, Decimal | None]:
        ...
        if isinstance(value, int):
            ...
            return _format_number_with_thousands_separator(value), decimal_value
    ...
    def _format_compact_numeric(self, value: Decimal) -> str:
        ...
        for threshold, suffix in thresholds:
            if magnitude >= threshold:
                ...
                text = f"{scaled:.3g}".rstrip(".")
                sign = "-" if value < 0 else ""
                return f"{sign}{text}{suffix}"
        return self._format_decimal_plain(value)
    ...
    def _fmt_int(self, value: int | None) -> str:
        if value is None:
            return "?"
        try:
            return _format_number_with_thousands_separator(value)
        except Exception:
            return str(value)
