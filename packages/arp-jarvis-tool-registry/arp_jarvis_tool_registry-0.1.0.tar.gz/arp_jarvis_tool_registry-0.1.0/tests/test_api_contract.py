...
class TestToolRegistryApiContract(unittest.TestCase):
    ...
    def test_health(self) -> None:
        status, body, _ = self._app.handle(method="GET", path="/v1/health", body=None)
    ...
    def test_version(self) -> None:
        status, body, _ = self._app.handle(method="GET", path="/v1/version", body=None)
        self.assertEqual(int(status), 200)
        self.assertIsInstance(body, dict)
        ...
        self.assertIn("v1", body["supported_api_versions"])
    ...
    def test_get_tool_definition(self) -> None:
        status, body, _ = self._app.handle(method="GET", path="/v1/tools/tool_echo", body=None)
        self.assertEqual(int(status), 200)
        self.assertIsInstance(body, dict)
    ...
    def test_invoke_invalid_json(self) -> None:
        status, body, _ = self._app.handle(method="POST", path="/v1/tool-invocations", body=None)
    ...
    def test_route_not_found(self) -> None:
        status, body, _ = self._app.handle(method="GET", path="/v1/nope", body=None)
    ...
    def test_responses_validate_against_standard_schemas(self) -> None:
        ...
        status, body, _ = self._app.handle(method="GET", path="/v1/health", body=None)
        self.assertEqual(int(status), 200)
        self.assertEqual(_validate_against_schema(schema_rel="common/health.schema.json", instance=body), [])

        status, body, _ = self._app.handle(method="GET", path="/v1/version", body=None)
        self.assertEqual(int(status), 200)
        self.assertEqual(_validate_against_schema(schema_rel="common/version_info.schema.json", instance=body), [])
        ...
        for item in body:
            self.assertEqual(
                _validate_against_schema(schema_rel="tool_registry/tools/tool_definition.schema.json", instance=item),
                [],
            )

        status, body, _ = self._app.handle(method="GET", path="/v1/tools/tool_echo", body=None)
        self.assertEqual(int(status), 200)
        self.assertEqual(_validate_against_schema(schema_rel="tool_registry/tools/tool_definition.schema.json", instance=body), [])
        ...
        self.assertEqual(int(status), 200)
        self.assertEqual(
            _validate_against_schema(schema_rel="tool_registry/tools/tool_invocation_result.schema.json", instance=body),
            [],
        )

        status, body, _ = self._app.handle(method="GET", path="/v1/nope", body=None)
        self.assertEqual(int(status), 404)
        self.assertEqual(_validate_against_schema(schema_rel="common/error.schema.json", instance=body), [])
