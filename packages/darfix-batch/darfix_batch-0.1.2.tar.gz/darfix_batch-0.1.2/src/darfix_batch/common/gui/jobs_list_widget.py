...
from .workflow_executor import JobItem
...
class JobsListWidget(qt.QWidget):
    ...
    def __init__(
        ...
        self._listWidget.setTextElideMode(qt.Qt.TextElideMode.ElideLeft)
        self._listWidget.setHorizontalScrollBarPolicy(
    ...
    def _updateListItem(self, listItem: qt.QListWidgetItem, jobItem: JobItem) -> None:
        ...
        desc = self.jobItemDescription(jobItem)

        listItem.setText(desc)
        listItem.setToolTip(f"<b>{state.capitalize()}</b><br><b>Job:</b> {desc}")
        ...
        if state != "pending":
            # Remove reference to the jobItem to avoid to keep finished graph in memory.
            listItem.setData(qt.Qt.UserRole, None)
    ...
    def _getStateIcon(state: _FutureStateStr) -> qt.QIcon:
        ...
        color = (
            qt.QColor("red") if state in ("cancelled", "failed") else qt.QColor("black")
        )
        return qtawesome.icon(iconNames[state], color=color, scale_factor=0.75)

    @staticmethod
    def jobItemDescription(jobItem: JobItem) -> str:
        """Description of the job item that will be the text shown in the job list."""
        args = jobItem.getArguments()
    ...
    def _getFutureState(future: FutureInterface) -> _FutureStateStr:
        ...
        try:
            ...
            _logger.error("Ewoks Task Exception.", exc_info=True)
            return "failed"
        else:
            return "succeeded"
