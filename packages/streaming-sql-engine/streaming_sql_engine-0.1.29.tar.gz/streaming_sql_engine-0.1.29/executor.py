...
def execute_plan(
    ...
    if plan.where_expr:
        ...
        # We check by looking at the left side of the equality expression
        has_table_prefix = False
        try:
            from sqlglot import expressions as exp
            # Simple check: if WHERE expression is an equality and left side is a Column with table prefix
            if isinstance(plan.where_expr, exp.EQ):
                left = plan.where_expr.this
                if isinstance(left, exp.Column) and left.table:
                    has_table_prefix = True
                    if debug:
                        print(f"  [DEBUG] Table prefix detected: {left.table}.{left.name}")
            # Also check recursively for other expression types
            elif hasattr(plan.where_expr, 'this'):
                left = plan.where_expr.this
                if isinstance(left, exp.Column) and left.table:
                    has_table_prefix = True
                    if debug:
                        print(f"  [DEBUG] Table prefix detected in nested expression: {left.table}.{left.name}")
            
            if debug and not has_table_prefix:
                print(f"  [DEBUG] No table prefix detected in WHERE expression")
        except Exception as e:
