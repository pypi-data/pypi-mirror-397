...
class OWCanvasMainWindow(CanvasMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.show_report_action = QAction(
            _tr.m[103, "Show report"], self,
            objectName="action-show-report",
            toolTip=_tr.m[104, "Show a report window"],
    ...
    def open_report(self):
        ...
        dlg = QFileDialog(
            self,
            windowTitle=self.tr(_tr.m[108, "Open Report"]),
        ...
        dlg.setNameFilters([_tr.m[109, "Report (*.report)"]])
    ...
    def reset_widget_settings(self):
        ...
        mb = QMessageBox(
            self,
            windowTitle=_tr.m[110, "Clear settings"],
            text=_tr.e(_tr.c(111, f"{name} needs to be restarted for the changes to take effect.")),
            icon=QMessageBox.Information,
            informativeText=_tr.e(_tr.c(112, f"Press OK to restart {name} now.")),
        ...
        if res == QMessageBox.Ok:
            ...
            def restart():
                ...
                if any(w.isVisible() for w in windows):  # if a window close was cancelled
                    ...
                    QMessageBox(
                        text=_tr.m[113, "Restart Cancelled"],
                        informativeText=_tr.e(_tr.c(114, f"Settings will be reset on {name}'s next restart")),
                        icon=QMessageBox.Information
                    ).exec()
                else:
    ...
    def ask_save_report(self):
        ...
        mBox = QMessageBox(
            self,
            windowTitle=_tr.m[115, "Report window"],
            icon=QMessageBox.Question,
            text=_tr.m[116, "The report contains unsaved changes."],
            informativeText=_tr.m[117, "Would you like to save the report?"],
