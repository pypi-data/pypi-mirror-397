...
class ErrorReporting(QDialog):
    ...
    def __init__(self, data):
        ...
        super().__init__(None, Qt.Window, modal=True,
                         ...
                         windowTitle=_tr.m[93, 'Unexpected Error'],
                         finished=_finished)
        self._data = data
        ...
        labels_layout.addWidget(QLabel(
            (_tr.m[94, 'The program encountered an unexpected error. Please<br>'] + (_tr.m[95, 'report it anonymously to the developers.<br><br>'] + _tr.m[96, 'The following data will be reported:']))))
        ...
        settings = QSettings()
        cb = QCheckBox(
            _tr.m[97, 'Include workflow (data will NOT be transmitted)'], self,
        ...
        buttons_layout.addWidget(
            QPushButton(_tr.m[98, 'Send Report (Thanks!)'], default=True, clicked=self.accept))
        buttons_layout.addWidget(QPushButton(_tr.m[99, "Don't Send"], default=False, clicked=self.reject))
    ...
    def handle_exception(cls, exc):
        ...
        if (err_module, widget_module) in cls._cache or not internet_on():
            QMessageBox(QMessageBox.Warning, _tr.m[100, 'Error Encountered'],
                        _tr.m[101, 'Error encountered{}:<br><br><tt>{}</tt>'].format(
                            (_tr.m[102, ' in widget <b>{}</b>'].format(widget_class.name)
