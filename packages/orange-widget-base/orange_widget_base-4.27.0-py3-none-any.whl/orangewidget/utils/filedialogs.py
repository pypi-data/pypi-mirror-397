...
def fix_extension(ext, format, suggested_ext, suggested_format):
    dlg = QMessageBox(
        QMessageBox.Warning,
        _tr.m[70, "Mismatching extension"],
        (_tr.e(_tr.c(71, f"Extension '{ext}' does not match the chosen file format, {format}.")) + _tr.m[72, "\n\nWould you like to fix this?"]))
    role = QMessageBox.AcceptRole
    change_ext = \
        suggested_ext and \
        dlg.addButton(_tr.e(_tr.c(73, f"Change extension to {suggested_ext}")), role)
    change_format =\
        suggested_format and \
        dlg.addButton(_tr.m[74, "Save as "] + suggested_format, role)
    cancel = dlg.addButton(_tr.m[75, "Back"], role)
    ...
    while True:
        ...
        filename, format, filter = \
            open_filename_dialog(start_dir, start_filter, file_formats,
                                 add_all=False, title=_tr.m[76, "Save as..."], dialog=dialog)
        if not filename:
...
def open_filename_dialog(start_dir: str, start_filter: str, file_formats,
                         add_all=True, title=_tr.m[77, "Open..."], dialog=None):
    """
    ...
    if add_all == "*":
        ...
        filters.insert(0, _tr.m[78, "All files (*.*)"])
    elif add_all and len(file_formats) > 1:
        all_extensions = set()
        for f in file_formats:
            all_extensions.update(f.EXTENSIONS)
        file_formats.insert(0, None)
        filters.insert(0, _tr.m[79, "All readable files (*{})"].format(
...
class RecentPath:
    ...
    def __repr__(self):
        return ("{0.__class__.__name__}(abspath={0.abspath!r}, " + ("prefix={0.prefix!r}, relpath={0.relpath!r}, " + "title={0.title!r})")).format(self)
...
class RecentPathsWComboMixin(RecentPathsWidgetMixin):
    ...
    def set_file_list(self):
        ...
        if not self.recent_paths:
            self.file_combo.addItem(_tr.m[80, "(none)"])
