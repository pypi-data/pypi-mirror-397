...
class Message(
    ...
    def asHtml(self, includeShortText=True):
        ...
        if includeShortText:
            parts += [(('<div class="field-text">' + ('<img src="{iconurl}" width="{imgsize}" height="{imgsize}" />' + (' {text}' + '</div>')))
...
def summarize(messages):
    ...
    if errors:
        text_parts.append(_tr.e(_tr.c(82, f"{nerrors} {pl(nerrors, 'error')}")))
    if warnings:
        text_parts.append(_tr.e(_tr.c(83, f"{nwarnings} {pl(nwarnings, 'warning')}")))
    if info:
        if not (errors and warnings and lead):
            text_parts.append(_tr.e(_tr.c(84, f"{ninfo} {pl(ninfo, 'message')}")))
        else:
            text_parts.append(_tr.e(_tr.c(85, f"{ninfo} other {pl(ninfo, 'message')}")))
...
def main(argv=None):  # pragma: no cover
    ...
    messages = [
        ...
        Message(Severity.Warning,
                text="ACHTUNG!",
                detailedText=("<div style=\"color: red\">DAS KOMPUTERMASCHINE IST " + "NICHT FÜR DER GEFINGERPOKEN</div>"),
                textFormat=Qt.RichText),
