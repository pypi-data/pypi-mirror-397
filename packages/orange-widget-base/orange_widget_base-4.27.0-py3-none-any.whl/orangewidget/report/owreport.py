...
class ReportItemModel(QStandardItemModel):
    ...
    def add_item(self, item):
        ...
        self.setItem(row, Column.remove, self._icon_item(_tr.m[45, "Remove"]))
        self.setItem(row, Column.scheme, self._icon_item(_tr.m[46, "Open Scheme"]))
...
class OWReport(OWBaseWidget):
    ...
    def _setup_ui_(self):
        ...
        self.scheme_button = gui.button(
            self.controlArea, self, _tr.m[48, "Back to Last Scheme"],
            callback=self._show_last_scheme
        )
        box = gui.hBox(self.controlArea)
        box.setContentsMargins(-6, 0, -6, 0)
        self.save_button = gui.button(
            box, self, _tr.m[49, "Save"], callback=self.save_report, disabled=True
        )
        self.print_button = gui.button(
            box, self, _tr.m[50, "Print"], callback=self._print_report, disabled=True
        )
    ...
    def _build_html(self, selected_id=None):
        ...
        if selected_id is not None:
            onload = (f"(function (id) {{" + (f"     setSelectedId(id); scrollToId(id); " + (f"}}" + (f"(\"{selected_id}\"));" + f""))))
            html += f"<body onload='{onload}'>"
        else:
            html += "<body>"
        for i in range(self.table_model.rowCount()):
            item = self.table_model.item(i)
            html += ("<div id='{}' class='normal' " + ("onClick='pybridge._select_item(this.id)'>{}<div " + ("class='textwrapper'><textarea " + (_tr.m[51, "placeholder='Write a comment...'"] + ("onInput='this.innerHTML = this.value;" + ("pybridge._add_comment(this.parentNode.parentNode.id, this.value);'" + (">{}</textarea></div>" + "</div>"))))))).format(item.id, item.html, item.comment)
        html += "</body></html>"
    ...
    def save_report(self):
        ...
        filename, selected_format = QFileDialog.getSaveFileName(
            self, _tr.m[52, "Save Report"], self.save_dir, ';;'.join(formats.keys()))
        if not filename:
    ...
    def _print_report(self):
        ...
        print_dialog.setWindowTitle(_tr.m[53, "Print report"])
    ...
    def permission_error(self, filename):
        ...
        mb = QMessageBox(
            ...
            windowTitle=self.tr(_tr.m[54, "Error"]),
            text=self.tr(_tr.m[55, "Permission error when trying to write report."]),
            informativeText=self.tr((_tr.m[56, "Permission error occurred "] + _tr.m[57, "while saving '{}'."])).format(filename),
