...
class NodeLuksStatus(object):
    ...
    Allowed enum values:
        ...
        - _UNKNOWN
        - _REDACTED
        - NON_LUKS: Indicates that the node is not LUKS encrypted
        - LUKS: Indicates that the node is LUKS encrypted
        - PARTIAL_LUKS: Indicates that the node is partially LUKS encrypted
        - UNKNOWN_LUKS: Indicates that the LUKS status for this node is not known
    """
    _UNKNOWN = "$UNKNOWN"
    _REDACTED = "$REDACTED"
    NON_LUKS = "NON_LUKS"
    LUKS = "LUKS"
    PARTIAL_LUKS = "PARTIAL_LUKS"
    UNKNOWN_LUKS = "UNKNOWN_LUKS"
    ...
    def _initialize_object_type(self):
        return 'clustermgmt.v4.config.NodeLuksStatus'
        ...
        for attr in vars(self):
            ...
            if isinstance(value, list):
                ...
                if sanitize:
                    ...
                    result[attr] = value
        if issubclass(NodeLuksStatus, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, NodeLuksStatus):
            return False
