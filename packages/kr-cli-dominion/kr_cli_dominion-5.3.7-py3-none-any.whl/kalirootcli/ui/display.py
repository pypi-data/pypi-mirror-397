...
def print_banner() -> None:
    ...
    for i, line in enumerate(lines):
        ...
        # Gradient: red -> orange -> yellow (No blue/cyan)
        if progress < 0.33:
    ...
    console.print(Panel(
        ...
        subtitle=f"[italic {STYLE_YELLOW}]Advanced AI Security Operations[/italic {STYLE_YELLOW}]",
        padding=(1, 2)
    ))
    ...
    credits.append("Sebastian Lara", style=f"bold {STYLE_YELLOW}")
...
def print_menu_option(number: str, text: str, description: str = "") -> None:
    ...
    console.print(f" [bold {STYLE_ORANGE_RED}]{number}[/bold {STYLE_ORANGE_RED}] › [white bold]{text}[/white bold]")
    if description:
        console.print(f"    [dim]{description}[/dim]")


def print_panel(content: str, title: str = "", style: str = STYLE_YELLOW) -> None:
...
def print_ai_response(response: str, mode: str = "CONSULTATION", command: str = None) -> None:
    ...
    if command:
        console.print(f"{icon} [bold {STYLE_YELLOW}]{command}[/bold {STYLE_YELLOW}] [{mode_color}][{display_mode}][/{mode_color}]")
    else:
...
def get_input(prompt: str = "") -> str:
    ...
    return Prompt.ask(f"[bold {STYLE_ORANGE_RED}]?[/bold {STYLE_ORANGE_RED}] {prompt}")
...
def show_loading(message: str = "Processing..."):
    ...
    return console.status(f"[bold {STYLE_YELLOW}]{message}[/bold {STYLE_YELLOW}]", spinner="dots")
