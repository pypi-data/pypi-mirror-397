    ...
    try:
        ...
        if command in ANALYSIS_TARGETS or any(t in command for t in ANALYSIS_TARGETS):
            ...
            with show_loading("🧠 Analyzing results (RAG & AI)..."):
                # Use dedicated command analysis (NO chat history contamination)
                ai_handler = AIHandler(user_id)
                
                # Check if user can query
                can_query, reason = ai_handler.can_query()
                if not can_query:
                    console.print(f"\n[red]❌ {reason}[/red]")
                    return
                
                # Analyze ONLY the command output
                response = ai_handler.analyze_command_output(full_cmd, output_to_analyze)
