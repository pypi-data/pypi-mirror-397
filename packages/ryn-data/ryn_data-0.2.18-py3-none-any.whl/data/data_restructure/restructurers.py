...
class ImageClassificationRestructurer(BaseRestructurer):
    ...
    def _load_data(self, input_path: Path) -> DataFrame:
        ...
        if metadata_path.exists():
            ...
            meta_df = meta_df.withColumn("clean_rel_path", F.regexp_replace(F.col("image_path"), "^/", ""))
    ...
    def _save_images_as_parquet(self, df: DataFrame, output_path: Path) -> None:
        ...
        for split_name in splits:
            ...
            # Select final HF columns
            # Note: We keep 'file_path' temporarily if you need it for debugging, 
            # otherwise just select what Parquet needs.
            final_split_df = final_split_df.select("image_path", "label", "image")
    ...
    def restructure(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        ...
        total_rows = df.count()
        columns = ["file_path", "label", "split"]  # Final output columns
