...
This module provides the e2e test marker and skip behavior that can be shared
across multiple conftest.py files (e.g., djb's own tests and projects using djb).
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    if "--run-e2e" not in existing_options:
        group.addoption(
            "--run-e2e",
            action="store_true",
            help="Include tests marked as e2e (excluded by default).",
        )
    if "--only-e2e" not in existing_options:
        group.addoption(
            "--only-e2e",
            action="store_true",
            help="Run only tests marked as e2e (skips everything else).",
        )
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers", "e2e: end-to-end tests requiring external tools (GPG, age, SOPS)"
    )
