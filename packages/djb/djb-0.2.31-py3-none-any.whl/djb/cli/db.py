...
from djb.config import get_project_name_from_pyproject
...
def _get_db_settings_from_secrets(project_root: Path) -> DbSettings | None:
...
def get_db_settings(project_root: Path) -> DbSettings:
    ...
    """
    settings = _get_db_settings_from_secrets(project_root)
    if settings and settings.name and settings.user:
        return settings

    # Fall back to project-name defaults
    defaults = _get_default_db_settings(project_root)
...
def init_database(
    ...
    *,
    ...
    Returns:
        True on success, False on error
    """
    settings = get_db_settings(project_root)
...
def db_init(cli_ctx: CliContext, no_start: bool):
    ...
    * Grants necessary permissions
    ...
    if not init_database(project_root, start_service=not no_start):
