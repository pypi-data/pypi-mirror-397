...
from djb.config import PROJECT, config, load_config, migrate_legacy_config
...
def mock_identity():
    """Mock _configure_user_identity to avoid interactive prompts in tests."""
    with patch(
        "djb.cli.init._configure_user_identity", return_value=("Test User", "test@example.com")
    ):
        yield
...
def mock_project_name():
    """Mock _configure_project_name to avoid interactive prompts in tests."""
    with patch("djb.cli.init._configure_project_name", return_value="test-project"):
        yield


@pytest.fixture(autouse=True)
def mock_hostname():
    """Mock _configure_hostname to avoid interactive prompts in tests."""
    with patch("djb.cli.init._configure_hostname", return_value="test-project.com"):
        yield
...
@pytest.fixture(autouse=True)
def mock_legacy_migration():
    """Mock migrate_legacy_config to avoid migration side effects in tests."""
    with patch("djb.cli.init.migrate_legacy_config", return_value={"local": [], "project": []}):
        yield


class TestLegacyConfigMigration:
    """Tests for legacy config.yaml migration during init.

    These tests don't use the autouse fixtures - they test migration behavior directly.
    """

    @pytest.fixture(autouse=False)
    def mock_identity(self):
        """Override autouse fixture - don't mock for this class."""
        pass
    ...
    @pytest.fixture(autouse=False)
    def mock_gitignore_update(self):
        """Override autouse fixture - don't mock for this class."""
        pass

    @pytest.fixture(autouse=False)
    def mock_legacy_migration(self):
        """Override autouse fixture - don't mock for this class."""
        pass

    def test_hostname_from_legacy_config_not_prompted(self, tmp_path):
        """Test that hostname is not prompted if it exists in legacy config.yaml.

        Regression test: When hostname exists in .djb/config.yaml (legacy),
        it should be migrated to project.yaml and the config singleton should
        be reloaded so that _configure_hostname sees the migrated value.
        """
        # Create legacy config.yaml with hostname
        config_dir = tmp_path / ".djb"
        config_dir.mkdir()
        legacy_config = config_dir / "config.yaml"
        legacy_config.write_text(
            "name: Test User\nemail: test@example.com\nhostname: existing-hostname.com\n"
        )

        # Step 1: Initial config reload (simulates init() behavior)
        config.reload(tmp_path)

        # At this point, config.hostname should be None because legacy config.yaml
        # isn't included in load_merged_config (it only loads local.yaml + project.yaml)
        assert config.hostname is None, "Hostname should be None before migration"

        # Step 2: Migrate legacy config (simulates init() behavior)
        result = migrate_legacy_config(tmp_path)
        assert "hostname" in result["project"], "Hostname should be migrated to project config"

        # Verify migration actually happened
        project_config = load_config(PROJECT, tmp_path)
        assert project_config.get("hostname") == "existing-hostname.com"
        assert not legacy_config.exists()

        # Step 3: Reload config after migration (this is the fix)
        # init.py now does this after migration, so the config singleton sees migrated values
        config.reload(tmp_path)

        # After reload, config.hostname should have the migrated value
        assert (
            config.hostname == "existing-hostname.com"
        ), "Config singleton should have hostname after migration + reload"
...
class TestDjbInit:
    ...
    def test_init_with_project_root(self, runner, mock_subprocess_run, project_dir):
...
class TestTeamMembershipChanges:
    ...
    def test_bob_joins_no_warning_about_self(self, tmp_path):
        ...
        # Alice created encrypted secrets
    ...
    def test_warning_when_team_member_removed(self, tmp_path):
        ...
        # But encrypted files still have both Alice and Carol
        manager = SecretsManager(secrets_dir=secrets_dir, key_path=alice_key_path)
        for env in SECRETS_ENVIRONMENTS:
            manager.save_secrets(
    ...
    def test_reencrypts_after_member_removed(self, tmp_path):
        ...
        # Encrypted files have both Alice and Carol
        manager = SecretsManager(secrets_dir=secrets_dir, key_path=alice_key_path)
        for env in SECRETS_ENVIRONMENTS:
            manager.save_secrets(
                env, {"django_secret_key": "test"}, [alice_public_key, carol_public_key]
        ...
        # Verify Carol is currently a recipient
        dev_recipients = _get_encrypted_recipients(secrets_dir / "dev.yaml")
        assert carol_public_key in dev_recipients
        ...
        # After re-encryption, Carol should be removed
        dev_recipients = _get_encrypted_recipients(secrets_dir / "dev.yaml")
        assert alice_public_key in dev_recipients, "Alice should still be a recipient"
        assert carol_public_key not in dev_recipients, "Carol should no longer be a recipient"
