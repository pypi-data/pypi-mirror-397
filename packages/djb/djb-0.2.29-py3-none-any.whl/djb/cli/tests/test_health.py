...
from djb.config import DjbConfig, config
...
class TestHealthCommand:
    ...
    def test_health_subcommand_runs_tool(self, runner, mock_run_cmd, subcommand, expected_tool):
        ...
        assert result.exit_code == 0
        calls = [str(call) for call in mock_run_cmd.call_args_list]
        assert any(expected_tool in str(call) for call in calls)
...
class TestProjectContext:
    ...
    def test_context_inside_djb_dir(self, tmp_path):
        ...
        context = _get_project_context(tmp_path)
    ...
    def test_context_with_editable_djb(self, tmp_path, djb_project):
        ...
        with (
            ...
            context = _get_project_context(host_dir)
    ...
    def run_helper(self, tmp_path):
        ...
        def _run(project_ctx: ProjectContext) -> list[tuple[Path, str, bool]]:
            ...
            with (
                ...
                health_ctx.config = DjbConfig(project_dir=tmp_path)
...
class TestEditableAwareHealth:
    ...
    def test_all_subcommands_respect_project_context(
        self, runner, tmp_path, mock_health_context, subcmd
    ):
...
class TestGetHostDisplayName:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("myapp", "", "myapp"),  # Falls back when project name is empty
        ],
    )
    def test_get_host_display_name(self, tmp_path, subdir, project_name, expected):
...
class TestBackendStepBuilders:
    ...
    @pytest.mark.parametrize(
        "builder,scope,expected_label",
        [
            (_build_backend_lint_steps, "Python", "Python lint"),
            (_build_backend_lint_steps, "Backend", "Backend lint"),
            (_build_backend_typecheck_steps, "Python", "Python typecheck"),
            (_build_backend_typecheck_steps, "Backend", "Backend typecheck"),
            (_build_backend_test_steps, "Python", "Python tests"),
            (_build_backend_test_steps, "Backend", "Backend tests"),
        ],
    )
    def test_steps_use_scope_in_label(self, tmp_path, builder, scope, expected_label):
        """Test that step builders use the scope in labels."""
        # All builders accept prefix and scope; lint also takes fix
        if builder == _build_backend_lint_steps:
            steps = builder(tmp_path, fix=False, prefix="", scope=scope)
        else:
            steps = builder(tmp_path, prefix="", scope=scope)
        assert len(steps) == 1
        assert expected_label in steps[0].label
    ...
    def test_steps_with_prefix_and_scope(self, tmp_path, builder, prefix, scope, expected_label):
        """Test that step builders combine prefix and scope correctly."""
        if builder == _build_backend_lint_steps:
            steps = builder(tmp_path, fix=False, prefix=prefix, scope=scope)
        else:
            steps = builder(tmp_path, prefix=prefix, scope=scope)
