...
Quick start:
    from djb.secrets import load_secrets_for_mode, protected_age_key

    # Load decrypted secrets for a deployment mode
    secrets = load_secrets_for_mode("production")

    # Access protected age key (auto-decrypts with GPG)
    with protected_age_key() as key_path:
        # key_path is temporarily decrypted
