...
from djb.cli.context import CliHealthContext, pass_health_context
...
def _get_project_context(project_root: Path) -> ProjectContext:
...
def _get_host_display_name(host_path: Path) -> str:
    ...
    """
    config.reload(host_path)
    return config.project_name or host_path.name
...
def _run_for_projects(
    ...
    project_ctx = _get_project_context(health_ctx.config.project_dir)
