...
from typing import Dict, Any, Callable, Optional, Type, TypeVar, TYPE_CHECKING
...
class BaseLambdaHandler:
    ...
    def _get_response_case_preference(self, event: Dict[str, Any]) -> bool:
        """
        ...
        Returns:
            True to convert to camelCase, False to keep snake_case
        ...
        Examples:
            ...
            # Client requests camelCase in body
            POST /api/users {"name": "John", "responseCase": "camelCase"}
            # Returns: {"userId": "123"}
        """
        # Check query string parameters (both camelCase and snake_case)
        query_params = event.get('queryStringParameters') or {}
        response_case = query_params.get('responseCase') or query_params.get('response_case')or query_params.get('response-case')
        
        if response_case:
            normalized = response_case.lower().strip()
            if normalized in ('camelcase', 'camel', 'camel_case'):
                logger.debug(f"Client requested camelCase response via query param")
                return True
            elif normalized in ('snakecase', 'snake', 'snake_case'):
                logger.debug(f"Client requested snake_case response via query param")
                return False
        ...
        if isinstance(parsed_body, dict):
            ...
            if response_case:
                normalized = response_case.lower().strip()
                if normalized in ('camelcase', 'camel', 'camel_case'):
                    logger.debug(f"Client requested camelCase response via body")
                    return True
                elif normalized in ('snakecase', 'snake', 'snake_case'):
                    logger.debug(f"Client requested snake_case response via body")
                    return False
        
        # No client preference - check handler setting, then env var, then default to True
        if self.convert_response_case is not None:
            return self.convert_response_case
        
        # Check environment variable
        env_value = os.getenv('DEFAULT_RESPONSE_CASE', 'camel').lower()
        return env_value == 'camel'
