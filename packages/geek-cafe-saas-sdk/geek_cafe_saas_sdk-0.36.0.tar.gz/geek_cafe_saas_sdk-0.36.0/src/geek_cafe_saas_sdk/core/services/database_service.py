...
class DatabaseService(ABC, Generic[T]):
    ...
    def _save_model(
        ...
        *,
        ...
        try:
            ...
            #   old_model = self._get_model_by_id(id, ModelClass)
            #   # ... modify model ...
            ...
            # For updates, user must have EDIT permission (owner, admin, or edit share)
    ...
    def _get_by_id(
        ...
        *,
        ...
        # SECURITY CHECKS (optimized order - fast paths first)
        
        # 1. Owner check - user owns this resource
        # Check multiple possible owner fields (owner_id, created_by_id, user_id)
        resource_owner_id = (
            getattr(model, 'owner_id', None) or 
            getattr(model, 'created_by_id', None) or
            getattr(model, 'user_id', None)
        )
        authenticated_user_id = self._request_context.authenticated_user_id
        
        if resource_owner_id and resource_owner_id == authenticated_user_id:
            return model  # Owner - always allowed
        
        # 2. System/Admin check - system and admins can access anything in scope
        if self._is_system_context() or self._request_context.is_platform_admin():
            return model  # System or platform admin - full access
        
        resource_tenant_id = getattr(model, 'tenant_id', None)
        
        if self._request_context.is_tenant_admin():
            # Tenant admin - only within their tenant
            if resource_tenant_id == self._request_context.authenticated_tenant_id:
                return model
        
        # 3. Tenant-wide access check (only loads tenant settings if needed)
        if self._request_context.allow_tenant_wide_access:
            if resource_tenant_id == self._request_context.authenticated_tenant_id:
                return model
        
        # 4. Resource shares check (most expensive - DB lookup)
        access_result = self._access_checker.check_access(
            resource_id=resource_id,
            resource_owner_id=resource_owner_id,
            resource_tenant_id=resource_tenant_id,
            required_permission=required_permission,
            allow_tenant_access=False  # Already checked above
        )
    ...
    # ========================================
    # DEPRECATED METHODS - Use _get_by_id() instead
