...
from .utils.detect.measurements import MeasurementConfig, Counter, Interactions, Aggregator, MotionCounter, MotionMetrics, compute_counts_from_boxes
...
class VideoProcessor:
    ...
    def initialize(self):
        ...
        self.motion_counter = MotionCounter(
            paths=self.paths,
            config=self.config,
            start_time=self.start_time
        )
    ...
    def run(self):
        ...
        try:
            while not stop_event.is_set():
                ...
                self.motion_counter.process_frame(boxes_list, names, video_ts)
                self.motion_metrics.process_frame(boxes_list, names, video_ts)
            ...
            f5 = self.motion_counter.save_results()
            if f5:
                saved.append(f5)
            
            mean_counts = {
                iv["TIME"]: iv["Counts"]
                for iv in self.aggregator.intervals
            }
            f6 = self.motion_metrics.save_results(mean_counts)
            if f6:
                saved.extend(f6)
