...
class SetShellType(type):
    ...
    def __getattr__(cls, name):
        ...
        raise AttributeError("SetShell class attribute '{}' does not exist".format(name))
...
# If one of the read only properties we define then get it
        if name in SetShell._rprops:
            return Oasys.D3PLOT._connection.instanceGetter(self.__class__.__name__, self._handle, name)

        raise AttributeError("SetShell instance attribute '{}' does not exist".format(name))


    def __setattr__(self, name, value):
# If one of the read only properties we define then error
        if name in SetShell._rprops:
            raise AttributeError("Cannot set read-only SetShell instance attribute '{}'".format(name))
...
# Static methods
    ...
    def GetAll(model, property=Oasys.gRPC.defaultArg):
        """
        Returns a list of SetShell objects or properties for all of the shell sets in the model.
        If the optional property argument is not given then a list of SetShell objects is returned.
        If the property argument is given, that property value for each shell set is returned in the list
        instead of a SetShell object
        ...
        property : string
            Optional. Name for property to get for all shell sets in the model

        Returns
        -------
        list
            List of :py:class:`SetShell <SetShell>` objects or properties
        """
    ...
    def GetFlagged(model, flag, property=Oasys.gRPC.defaultArg):
        ...
        property : string
            Optional. Name for property to get for all flagged shell sets in the model
