...
def _gonzales_path() -> Path:
    return Path(__file__).resolve().parent.parent / "data" / "gonzales_subset.tsv"
...
class SentencePieceVocabDecodeTests(unittest.TestCase):
    def test_decode_preserves_space_count_on_gonzales_subset(self):
        data_path = _gonzales_path()
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = Config(
                data_dir=tmp,
                sentencepiece_model_prefix=str(tmp / "sp_gonzales"),
                vocab_size=256,
            )
