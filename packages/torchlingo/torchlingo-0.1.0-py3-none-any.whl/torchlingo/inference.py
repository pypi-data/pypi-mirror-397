...
def greedy_decode(
    ...
    with torch.no_grad():
        ...
        # LSTM-style seq2seq models expose embedding/encoder/decoder/output modules.
        if all(
            hasattr(model, a) for a in ("src_embed", "encoder", "decoder", "output")
        ):
            ...
            # Decoder hidden state initialized from encoder
            hidden = (h, c)

            # Prefer target embedding if available
            tgt_embed = getattr(model, "tgt_embed", None) or getattr(
                model, "src_embed", None
            )

            for _ in range(max_len):
                # Embed last token only and run one-step decoder to update hidden state
                last_token = ys[:, -1].unsqueeze(1)  # (batch, 1)
                emb = tgt_embed(last_token)  # (batch, 1, embed_dim)
                dec_out, hidden = model.decoder(emb, hidden)
                # dec_out shape (batch, 1, hidden_dim)
                logits = model.output(dec_out)  # (batch, 1, vocab)
