...
def FSharpList__get_Length(xs: FSharpList[Any]) -> int32:
...
def indexed[_A](xs: FSharpList[_A]) -> FSharpList[tuple[int32, _A]]:
    def _arrow51(i: int32, x: _A, xs: Any = xs) -> tuple[int32, _A]:
        return (i, x)
...
def map_fold[RESULT, STATE, T](
    ...
    def folder(
        ...
        def _arrow54(__unit: None = None, tupled_arg: Any = tupled_arg, x: Any = x) -> FSharpList[RESULT]:
            t: FSharpList[RESULT] = FSharpList(pattern_input[int32_1(0)], None)
            tupled_arg[int32_1(0)].tail_ = t
            return t

        return (_arrow54(), pattern_input[int32_1(1)])
...
def map_fold_back[RESULT, STATE, T](
    ...
    def _arrow55(acc: STATE, x: T, mapping: Any = mapping, xs: Any = xs, state: Any = state) -> tuple[RESULT, STATE]:
...
def try_find[_A](f: Callable[[_A], bool], xs: FSharpList[_A]) -> _A | None:
    def _arrow56(x: _A | None = None, f: Any = f, xs: Any = xs) -> _A | None:
...
def partition[T](f: Callable[[T], bool], xs: FSharpList[T]) -> tuple[FSharpList[T], FSharpList[T]]:
    ...
    def folder(
        ...
        if f(x):
            ...
            def _arrow57(__unit: None = None, tupled_arg: Any = tupled_arg, x: Any = x) -> FSharpList[T]:
                t: FSharpList[T] = FSharpList(x, None)
                lacc.tail_ = t
                return t
            ...
            def _arrow58(__unit: None = None, tupled_arg: Any = tupled_arg, x: Any = x) -> FSharpList[T]:
...
def contains[T](value: T, xs: FSharpList[T], eq: IEqualityComparer_1[Any]) -> bool:
    def _arrow59(v: T | None = None, value: Any = value, xs: Any = xs, eq: Any = eq) -> bool:
...
def initialize[T](n: int32, f: Callable[[int32], T]) -> FSharpList[T]:
    ...
    for i in range(int32.ZERO, n - int32.ONE, 1):
        ...
        def _arrow60(__unit: None = None, n: Any = n, f: Any = f) -> FSharpList[T]:
...
def replicate[_A](n: int32, x: _A) -> FSharpList[_A]:
    def _arrow61(_arg: int32, n: Any = n, x: Any = x) -> _A:
        return x
...
def for_all[_A](f: Callable[[_A], bool], xs: FSharpList[_A]) -> bool:
    def _arrow62(acc: bool, x: _A, f: Any = f, xs: Any = xs) -> bool:
...
def for_all2[_A, _B](f: Callable[[_A, _B], bool], xs: FSharpList[_A], ys: FSharpList[_B]) -> bool:
    def _arrow63(acc: bool, x: _A, y: _B, f: Any = f, xs: Any = xs, ys: Any = ys) -> bool:
...
def zip[_A, _B](xs: FSharpList[_A], ys: FSharpList[_B]) -> FSharpList[tuple[_A, _B]]:
    def _arrow66(x: _A, y: _B, xs: Any = xs, ys: Any = ys) -> tuple[_A, _B]:
        return (x, y)
...
def zip3[_A, _B, _C](xs: FSharpList[_A], ys: FSharpList[_B], zs: FSharpList[_C]) -> FSharpList[tuple[_A, _B, _C]]:
    def _arrow67(x: _A, y: _B, z: _C, xs: Any = xs, ys: Any = ys, zs: Any = zs) -> tuple[_A, _B, _C]:
        return (x, y, z)
...
def sort[T](xs: FSharpList[T], comparer: IComparer_1[T]) -> FSharpList[T]:
    def _arrow68(x: T, y: T, xs: Any = xs, comparer: Any = comparer) -> int32:
...
def sort_by[T, U](projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> FSharpList[T]:
    def _arrow69(x: T, y: T, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> int32:
...
def sort_descending[T](xs: FSharpList[T], comparer: IComparer_1[T]) -> FSharpList[T]:
    def _arrow70(x: T, y: T, xs: Any = xs, comparer: Any = comparer) -> int32:
...
def sort_by_descending[T, U](
    ...
    def _arrow71(x: T, y: T, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> int32:
...
def sum[T](xs: FSharpList[T], adder: IGenericAdder_1[T]) -> T:
    def _arrow72(acc: T, x: T, xs: Any = xs, adder: Any = adder) -> T:
...
def sum_by[T, U](f: Callable[[T], U], xs: FSharpList[T], adder: IGenericAdder_1[U]) -> U:
    def _arrow73(acc: U, x: T, f: Any = f, xs: Any = xs, adder: Any = adder) -> U:
...
def max_by[T, U](projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> T:
    def _arrow74(x: T, y: T, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> T:
...
def max[T](xs: FSharpList[T], comparer: IComparer_1[T]) -> T:
    def _arrow75(x: T, y: T, xs: Any = xs, comparer: Any = comparer) -> T:
...
def min_by[T, U](projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> T:
    def _arrow76(x: T, y: T, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> T:
...
def min[T](xs: FSharpList[T], comparer: IComparer_1[T]) -> T:
    def _arrow77(x: T, y: T, xs: Any = xs, comparer: Any = comparer) -> T:
...
def average_by[T, U](f: Callable[[T], U], xs: FSharpList[T], averager: IGenericAverager_1[U]) -> U:
    ...
    def _arrow78(acc: U, x: T, f: Any = f, xs: Any = xs, averager: Any = averager) -> U:
        nonlocal count
        count = count + int32.ONE
        return averager.Add(acc, f(x))

    total: U = fold(_arrow78, averager.GetZero(), xs)
    return averager.DivideByInt(total, count)
    ...
    def loop(
        ...
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def _arrow91(__unit: None = None, acc: Any = acc, xs_1: Any = xs_1) -> FSharpList[T]:
                    t: FSharpList[T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail_ = t
                    return t
    ...
    def loop(
        ...
        while True:
            ...
            if i <= int32.ZERO:
                ...
                def _arrow92(__unit: None = None, i: Any = i, acc: Any = acc, xs_1: Any = xs_1) -> FSharpList[T]:
                    t: FSharpList[T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail_ = t
                    return t
