...
class _DummyReader:
    ...
    def get_chromatograms_batch(
        self, target_mzs: list[float], ppm: float
...
def test_process_raw_file_direct_mz_collects_status_rows() -> None:
    ...
    reader = _DummyReader("sample.raw", [eic_hi, eic_lo])
...
def test_write_results_excel_writes_target_status_sheet(tmp_path: Path) -> None:
    ...
    assert "Target_Status" in xl.sheet_names
