...
class MultiRepoBase(Generic[T], ABC):
    ...
    def __init__(self, model_code: str, eo_id: str = "", instance_id: str = "", id_field: str = None, data_source_path: str = None):
        """
        ...
        self._data_source_path = data_source_path  # 指定数据源路径
    ...
    def find_by_id(self, pk: str) -> Optional[T]:
        ...
        """
        # 如果指定了data_source_path，优先使用它
        data_source = None
        if self._data_source_path and os.path.exists(self._data_source_path):
            data_source = self._data_source_path
        elif os.path.exists(self._data_file):
            data_source = self._data_file
                ...
                if isinstance(data, list):
                    ...
                    for item in data:
                        if self._matches_id(item, pk):
                            # Process relations before converting to domain
                            converted_record = self._convert_field_names(item)
                            record_with_relations = self._load_related_data(converted_record)
                            return self.to_domain(record_with_relations)
                else:
                    # Single item case
                    if self._matches_id(data, pk):
                        converted_record = self._convert_field_names(data)
                        record_with_relations = self._load_related_data(converted_record)
                        return self.to_domain(record_with_relations)
        except Exception:
            pass
    ...
    def list(self) -> List[T]:
        ...
        if self._data_source_path and os.path.exists(self._data_source_path):
            try:
                with open(self._data_source_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    if isinstance(data, list) and len(data) > 0:
                        ...
                        # Single item case, need to process relations
                        converted_record = self._convert_field_names(data)
                        record_with_relations = self._load_related_data(converted_record)
                        return [self.to_domain(record_with_relations)]
            except Exception:
    ...
    def _load_from_source(self):
        ...
        # 0. 最优先：如果指定了数据源路径，直接使用
        if self._data_source_path:
            if os.path.exists(self._data_source_path):
                possible_paths.append(self._data_source_path)
                return possible_paths

        # 1. 其次：尝试从SDK包内的数据目录加载
