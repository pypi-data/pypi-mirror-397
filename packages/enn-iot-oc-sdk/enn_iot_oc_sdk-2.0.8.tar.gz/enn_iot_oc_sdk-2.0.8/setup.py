...
def validate_data():
    ...
    data_file = 'data/data_v2/source/category_requirement.json'
    ...
    # 检查数据项
    valid_ids = [f"CAT_REQ_{i:03d}" for i in range(1, 7)]
    found_ids = [item.get('dimension_id') for item in data]

    invalid_ids = [id for id in found_ids if id not in valid_ids]
    if invalid_ids:
        raise ValueError(f"❌ 发现无效数据项: {invalid_ids}")

    print(f"✅ 数据验证通过: {len(data)}项")
