...
class CustomerCategoryRepoImpl(CustomerCategoryRepoBase[CustomerCategory]):
    ...
    def __init__(self, eo_id: str = "", instance_id: str = "") -> None:
        ...
        try:
            ...
            sdk_file = os.path.join(sdk_data_dir, "customer_category.json")
        ...
        # 使用找到的数据源
        super().__init__(model_code="customer_category", eo_id=eo_id, instance_id=instance_id, id_field="category_id", data_source_path=data_source_path)
    ...
    def _load_target_entity_data(self, target_table: str) -> List[Dict[str, Any]]:
        ...
        try:
            import ioc_data_sdk
            sdk_data_dir = os.path.join(os.path.dirname(ioc_data_sdk.__file__), 'data', 'source')
            sdk_file = os.path.join(sdk_data_dir, f'{target_table}.json')
            if os.path.exists(sdk_file):
