...
class RepositoryGenerator:
    ...
    def generate_complete_repository(self, entity_name: str, entity_data: List[Dict[str, Any]], data_dir: str = None) -> str:
        """
        生成完整的Repository实现
        ...
        # 4. Repository类定义
        code_parts.append(self._generate_repository_class(entity_name, entity_config, field_types, data_dir))
...
"""
    ...
    def _generate_repository_class(self, entity_name: str, entity_config: EntityConfig, field_types: Dict[str, str], data_dir: str = None) -> str:
        """生成Repository类定义"""
        lines = []
        ...
        # 构造函数
        lines.extend(self._generate_constructor(entity_config, data_dir))
    ...
    def _generate_constructor(self, entity_config: EntityConfig, data_dir: str = None) -> List[str]:
        """生成构造函数"""
        lines = []
        ...
        lines.append('        # 优先使用SDK包内数据，避免环境污染')
        lines.append('        data_source_path = None')
        lines.append('        try:')
        lines.append('            import ioc_data_sdk')
        lines.append(f'            sdk_data_dir = os.path.join(os.path.dirname(ioc_data_sdk.__file__), "data", "source")')
        lines.append(f'            sdk_file = os.path.join(sdk_data_dir, "{entity_config.table}.json")')
        lines.append('            if os.path.exists(sdk_file):')
        lines.append('                data_source_path = sdk_file')
        lines.append('        except ImportError:')
        lines.append('            pass')
        lines.append('')
        lines.append('        # 使用找到的数据源')
        if entity_config.row_type == "single":
            lines.append('        super().__init__(model_code="{}", eo_id=eo_id, instance_id=instance_id, data_source_path=data_source_path)'.format(entity_config.table))
        else:
            id_field = self.config.get_id_field_for_table(entity_config.name)
            lines.append('        super().__init__(model_code="{}", eo_id=eo_id, instance_id=instance_id, id_field="{}", data_source_path=data_source_path)'.format(entity_config.table, id_field))
        lines.append("")
        return lines
    ...
    def generate_all_repositories(self, data_dir: str, output_dir: str):
        ...
        for entity_name, entity_config in self.config.entities.items():
            ...
            # 生成Repository代码
            repository_code = self.generate_complete_repository(entity_name, entity_data, data_dir)
