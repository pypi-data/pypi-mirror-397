...
class MultiRepoBase(Generic[T], ABC):
    """
    base class for multi-row repositories
    ...
    def find_by_id(self, pk: str) -> Optional[T]:
        """
        implementation of find_by_id() method
        Returns entity with given primary key or None if not found
        """
        print(f"Finding entity by ID '{pk}' for model: {self.model_code}")
        ...
        print(f"No entity found with ID '{pk}'")
        return None
    ...
    def list(self) -> List[T]:
        ...
        """
        print(f"Listing all entities for model: {self.model_code}")
        ...
        # If no data, try to initialize from source data
        try:
        ...
        print(f"No data found for {self.model_code}, returning empty list")
        return []
    ...
    def save(self, entity: T) -> None:
        ...
        """
        print(f"Saving single entity for model: {self.model_code}")
        ...
        try:
            ...
            print(f"Successfully saved entity to {self._data_file}")
        except Exception as e:
    ...
    def save_all(self, entities: List[T]) -> None:
        """
        implementation of save_all() method
        Saves multiple entities
        """
        print(f"Saving {len(entities)} entities for model: {self.model_code}")
        ...
        try:
            with open(self._data_file, 'w', encoding='utf-8') as f:
                json.dump(current_data, f, ensure_ascii=False, indent=2)
            print(f"Successfully saved {len(entities)} entities to {self._data_file}")
        except Exception as e:
            print(f"Error saving data: {e}")
        ...
        for source_file in list(set(possible_paths)):
            try:
                with open(source_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                print(f"Loaded source data from {source_file}")
                return data
            except Exception as e:
                print(f"Error loading source file {source_file}: {e}")

        print(f"Could not find source data for model: {self.model_code}")
        return None
