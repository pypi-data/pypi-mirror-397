...
def _profit_factor(tlog):
    if _gross_profit(tlog) == 0:
        return 0
    if _gross_loss(tlog) == 0:
        return 1000
...
def _avg_profit_per_winning_trade(tlog):
    if _num_winning_trades(tlog) == 0:
        return 0
    return _gross_profit(tlog) / _num_winning_trades(tlog)


def _avg_loss_per_losing_trade(tlog):
    if _num_losing_trades(tlog) == 0:
        return 0
    return _gross_loss(tlog) / _num_losing_trades(tlog)


def _ratio_avg_profit_win_loss(tlog):
    if _avg_profit_per_winning_trade(tlog) == 0:
        return 0
    if _avg_loss_per_losing_trade(tlog) == 0:
        return 1000
    return _avg_profit_per_winning_trade(tlog) / _avg_loss_per_losing_trade(tlog) * -1
...
def stats(ts, tlog, dbal, capital):
    ...
    stats["drawdown_loss_period"], stats["drawdown_recovery_period"] = (
        _drawdown_loss_recovery_period(
...
def summary(stats, benchmark_stats=None, metrics=default_metrics, extras=None):
    ...
    for metric in metrics:
        ...
        if benchmark_stats is not None:
            data.append(
                (
                    _get_metric_value(stats, metric),
...
def optimizer_summary(strategies, metrics):
    ...
    for metric in metrics:
        index.append(metric)
        data.append(
            [_get_metric_value(strategy.stats, metric) for strategy in strategies]
        )
