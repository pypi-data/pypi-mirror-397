...
class GARClient:
    ...
    def clear_connection_state(self) -> None:
        ...
        with self._key_lock:
            with self._topic_lock:
                ...
                if hasattr(self, "first_heartbeat_received"):
    ...
    async def connect(self) -> None:
        ...
        while self.running and not self.connected:
            try:
                ...
                self.logger.info(
                    "Connecting to WebSocket server at %s; application: %s, user: %s",
                    self.ws_endpoint,
                    self.application,
                    self.user,
                )
                ...
                async with websockets.connect(
                    ...
                    **connect_kwargs,
                    ping_interval=min(
                        60, max(10, self.heartbeat_timeout_interval / 1000 / 2)
                    ),
                    ping_timeout=max(10, self.heartbeat_timeout_interval / 1000 + 1),
                ) as websocket:
                    self.websocket = websocket
                    self.connected = True
                    self.logger.info(
                        "Connected to WebSocket server at %s; application: %s, user: %s",
                        self.ws_endpoint,
                        self.application,
                        self.user,
                    )
    ...
    def _process_message(self, message: Dict[str, Any]) -> None:
        ...
        if msg_type == "TopicIntroduction":
            ...
            # Reset first heartbeat event for a new connection/intro
            self.first_heartbeat_received.clear()
    ...
    def route_query(
        ...
        if not self.connected:
            raise ConnectionError(
                f"GARClient.route_query called while not connected for application {self.application}"
            )
        ...
        cleanup()
        raise TimeoutError(f"{sub_name} timed out for application {self.application}")
