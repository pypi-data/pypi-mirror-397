...
def _get_form_base_class_by_model(model):
    ...
    if issubclass(model, models.MitreIdentifiableMixIn):
...
class MultipleStixIdentifierField(forms.Field):
    ...
    def to_python(self, value):
        objs = []
        for v in value:
            if isinstance(v, django_models.Model):
                objs.append(v)
                continue
...
@register_model_form()
class ReferenceForm(_get_form_base_class_by_model(models.Reference)):
...
@register_model_form()
class MatrixForm(_get_form_base_class_by_model(models.Matrix)):
...
@register_model_form()
class TechniqueForm(_get_form_base_class_by_model(models.Technique)):
    ...
    def _save_m2m(self):
        ...
        for chain in kill_chain_phases:
            tactic = models.Tactic.objects.filter(
    ...
    def save(self, *args, **kwargs):
        ...
        if obj.is_subtechnique:
            ...
            with suppress(models.Technique.DoesNotExist):
                obj.major_technique = models.Technique.objects.get(mitre_id=major_technique_id)
...
@register_model_form()
class TacticForm(_get_form_base_class_by_model(models.Tactic)):
...
@register_model_form()
class SoftwareForm(_get_form_base_class_by_model(models.Software)):
