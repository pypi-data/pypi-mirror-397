...
from .types import TResponseInputItem
from .types import FunctionTool, Tool
...
class Converter:
    ...
    def maybe_function_tool_call_output(
        ...
        if isinstance(item, ResponseFunctionToolCallOutputItem):
            # Special case for ResponseFunctionToolCallOutputItem.
            return FunctionCallOutput(**item.model_dump())
    ...
    def extract_all_content(
        ...
        for c in content:
            if isinstance(c, dict) and c.get("type") == "input_text":
                ...
                if (
                    "file_data" not in casted_file_param
                    or not casted_file_param["file_data"]
                ):
                    raise UserError(
                        f"Only file_data is supported for input_file {casted_file_param}"
                    )
                if (
                    "filename" not in casted_file_param
                    or not casted_file_param["filename"]
                ):
                    raise UserError(
