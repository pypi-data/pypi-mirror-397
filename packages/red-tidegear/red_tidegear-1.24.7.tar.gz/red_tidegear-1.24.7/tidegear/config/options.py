...
class ConfigScope(enum.StrEnum):
    """Configuration scopes for use when determining scope for a [`GlobalConfigOption`][tidegear.config.options.GlobalConfigOption] or a subclass.
...
class GlobalConfigOption(BaseModel, Generic[JsonableType]):
    ...
    Changes to this type of configuration option will impact the entire bot, so should usually only be modifiable by the bot owner.

    You shouldn't be creating instances of this class manually; as [`BaseConfigSchema.init`][tidegear.config.schema.BaseConfigSchema]
    ...
    Subclasses of `GlobalConfigOption` will have different scopes, allowing you to differentiate between them using the scope classvar.
    """
    ...
    It will be automatically generated from an attribute-level docstring. If this docstring is also not provided, this will be set to None.
    """
    ...
    def type(cls) -> type[JsonableType]:
        ...
        Raises:
            TypeError: If the configuration option has no generic type annotation.
                This will also be raised during schema registration, so you don't have to catch this exception during option usage.
    ...
    def _value(self) -> red_config.Value | red_config.Group:
        """Get the underlying [`Value`][redbot.core.config.Value] or [`Group`][redbot.core.config.Group] for this configuration option.
    ...
    def _serialize(self, obj: Any, /) -> pydantic.JsonValue:
        """Validate a Python object against this option's declared type constraints, then serialize it to a JSON-serializable object.
        ...
        Returns:
            A "serialized" Python object that can then be properly serialized to JSON through [`json.dumps`][] or the like.
                For instance, a Pydantic model would become a raw dictionary, containing only elements that can be serialized to JSON.
        """
    ...
    async def _get(self, c: red_config.Value | red_config.Group, *, acquire_lock: bool = True) -> JsonableType:
        ...
        try:
            ...
            msg = (
                ...
                "\nIf this error is originating from a cog you do not develop, and you have NOT modified the configuration file by hand, "
                "please report this to the cog author.\nIf you edited the configuration file by hand, it may be in an invalid state. "
                "Try backing up the configuration file and deleting the original. If that resolves the issue, you should restore from the backup and "
    ...
    async def _set(self, c: red_config.Value | red_config.Group, value: JsonableType, *, force: bool = False) -> None:  # noqa: PLR6301
        if self.read_only and not force:
            msg = (
                ...
                "This check can be overridden by passing 'force=True' to '.set()', but only do this if you really want to change this attribute."
            )
...
class GuildConfigOption(GlobalConfigOption, Generic[JsonableType]):
    ...
    async def __call__(self, guild: discord.Guild | int, *, acquire_lock: bool = True) -> JsonableType:  # pyright: ignore[reportIncompatibleMethodOverride]
        """Get a value for this configuration option. Erognomic alternative to [`get`][tidegear.config.options.GuildConfigOption.get].
...
class MemberConfigOption(GlobalConfigOption, Generic[JsonableType]):
    ...
    @override
    async def __call__(self, member: discord.Member | int, *, guild: discord.Guild | int | None = None, acquire_lock: bool = True) -> JsonableType:  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        Returns:
            The retrieved configuration value.
        """  # noqa: DOC502
        return await self.get(member, guild=guild, acquire_lock=acquire_lock)

    @override
    async def get(self, member: discord.Member | int, *, guild: discord.Guild | int | None = None, acquire_lock: bool = True) -> JsonableType:  # pyright: ignore[reportIncompatibleMethodOverride]
        """Get a value for this configuration option.
...
class CustomConfigGroup(BaseModel):
    ...
    Danger:
        ...
        Do note, however, that the order of keyword arguments *within [`CustomConfigOption`][tidegear.config.options.CustomConfigOption]'s
        methods* does not matter. So long as there is a keyword argument for each identifier in this list, changing the order of those
        keyword arguments will not result in accessing different data.

    These identifiers are used as keyword argument names in [`CustomConfigOption`][tidegear.config.options.CustomConfigOption] methods.
