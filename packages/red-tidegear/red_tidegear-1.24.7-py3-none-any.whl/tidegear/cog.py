...
class Cog(commands.Cog):  # noqa: PLR0904
    ...
    Attributes:
        ...
        session: A session that may be used for making HTTP requests. Opened during cog loading and closed automatically during cog unloading.
    ...
    async def tidegear_session_headers(self) -> dict[str, str]:
        ...
        Danger:
            ...
            This means that whatever this method returns will be sent as HTTP headers on any HTTP request made with the pre-configured ClientSession.
    ...
    @staticmethod
    async def _tidegear_session_on_request_start(session: ClientSession, trace_config_ctx: SimpleNamespace, params: TraceRequestStartParams) -> None:
    ...
    async def _tidegear_session_on_request_end(
        ...
        self._rest_logger.trace(
            "%s %s -> %s (%s)", params.method, url, params.response.status, self._tidegear_trace_get_duration(end_time, trace_config_ctx)
        )
    ...
    def format_help_for_context(self, ctx: commands.Context) -> str:
        ...
        tidegear_version = f"{cf.bold('Tidegear Version:')} {meta.markdown}"
        if meta.version < self._tidegear_pypi_version():
            tidegear_version += f"\n{constants.INFO} New version available: {cf.bold(str(self._tidegear_pypi_version()))}"
    ...
    async def cog_command_error(self, ctx: commands.Context, error: Exception) -> None:  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        if isinstance(error, TidegearError) and error.send_to_end_user is True:
            ...
            return None
        exc_id = random_string(length=10)
        await send_error(
            ctx, content=f"Encountered an internal error, please report this to the bot owner! Exception ID: {cf.inline(exc_id)}", ephemeral=True
        )
        self.logger.error("Uncaught exception in command '%s'! Exception ID: '%s'", ctx.command.qualified_name, exc_id, exc_info=error)
        return None
    ...
    async def cog_app_command_error(self, interaction: discord.Interaction, error: Exception) -> None:
        ...
        if isinstance(error, (app_commands.AppCommandError, commands.BadArgument, commands.CommandOnCooldown, commands.MaxConcurrencyReached)):
            await send_error(interaction, content=str(error), ephemeral=True)
            return None
        if isinstance(error, TidegearError) and error.send_to_end_user is True:
        ...
        await send_error(
            interaction,
            content=f"Encountered an internal error, please report this to the bot owner! Exception ID: {cf.inline(exc_id)}",
            ephemeral=True,
        )
        if interaction.command:
            self.logger.error("Uncaught exception in application command '%s'! Exception ID: '%s'", interaction.command.name, exc_id, exc_info=error)
        else:
            self.logger.error("Uncaught exception in the callback for interaction %d! Exception ID: '%s'", interaction.id, exc_id, exc_info=error)
        return None
