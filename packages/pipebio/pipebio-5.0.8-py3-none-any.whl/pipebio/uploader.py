...
class Uploader:
    ...
    def __init__(self,
        ...
        # Write header line for TSV format.
        self.file_writer.write(self.cols_to_header_line().encode('UTF-8'))
        # Header just written, so next row is not first.
        self.first_row_in_file = False
    ...
    def upload_if_ready(self, force=False):
        ...
        if self.index % self.chunk_size == 0 or force:
            ...
            # Only create a new file if we're continuing to write (not forcing final upload)
            if not force:
                self.file_path = f'/tmp/{self.entity_id}_{uuid.uuid4()}.tsv'
    ...
    def make_line(self, row_data: dict) -> str:
        ...
        """
        line = []
        # Skip prefix if we just wrote a header (prevents blank line after header)
        if self.first_row_in_file:
            prefix = ''
            self.first_row_in_file = False
        else:
    ...
    def _upload(self, file_path: str) -> bool:
        ...
        try:
            ...
            # Extract headers from response if present (for S3 encryption)
            headers = url_response.get('headers', None)

            print(f'entity_id={self.entity_id} Created signed url, uploading')
            self.sequences.upload(signed_url, file_path, headers)
    ...
    def upload(self, allow_empty=False) -> bool:
        ...
        if self.index > 0:
            ...
            raise ValueError('No sequences written to result. Was the filter too strict?')
        else:
            return False
    ...
    def add_natural_sort_columns(self, schema: List[Column]) -> List[Column]:
        ...
        """
        existing_column_names = [column.name for column in schema]
        result = []

        for column in schema:
            result.append(column)

            already_has_sort_column = f'{column.name}_sort' in existing_column_names
            is_sort_column = column.name[:-5] in existing_column_names
            if self.needs_natural_sort(column) and not already_has_sort_column and not is_sort_column:
