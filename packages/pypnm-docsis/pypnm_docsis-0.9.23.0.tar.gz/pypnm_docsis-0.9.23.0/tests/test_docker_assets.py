...
RELEASE_COMPOSE_PATH = REPO_ROOT / "deploy" / "compose" / "docker-compose.yml"
...
def test_compose_has_expected_service_structure() -> None:
    ...
    assert any(port.startswith("8000:8000") for port in ports), "API port mapping must expose 8000"
...
def test_release_compose_uses_ghcr_and_named_volumes() -> None:
    ...
    assert image.startswith(
        "ghcr.io/mgarcia01752/pypnm:"
    ), "Release compose must pull from ghcr.io/mgarcia01752/pypnm"
