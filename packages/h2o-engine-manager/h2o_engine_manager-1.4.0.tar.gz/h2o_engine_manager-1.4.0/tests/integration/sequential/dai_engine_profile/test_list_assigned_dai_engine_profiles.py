...
def test_list_assigned_dai_engine_profiles(
    ...
    profiles = dai_engine_profile_client_super_admin.list_all_assigned_dai_engine_profiles(parent=GLOBAL_WORKSPACE)
    ...
    profiles = dai_engine_profile_client_admin.list_all_assigned_dai_engine_profiles(parent=GLOBAL_WORKSPACE)
    print(profiles)
    ...
    profiles = dai_engine_profile_client.list_all_assigned_dai_engine_profiles(parent=GLOBAL_WORKSPACE)
    print(profiles)
    assert len(profiles) == 1
    assert profiles[0].name == "workspaces/global/daiEngineProfiles/p3"

    # test pagination
    page = dai_engine_profile_client_super_admin.list_assigned_dai_engine_profiles(parent=GLOBAL_WORKSPACE, page_size=1)
    assert len(page.dai_engine_profiles) == 1
    assert page.dai_engine_profiles[0].name == "workspaces/global/daiEngineProfiles/p3"
    assert page.next_page_token != ""

    page = dai_engine_profile_client_super_admin.list_assigned_dai_engine_profiles(
        parent=GLOBAL_WORKSPACE,
        page_size=1,
