...
class BatchDecoder():
    ...
    def decode(
        ...
        vc6il.vc6_il_batch_decode_legacy(len(self._decoders_py), self._decoders_c, num_images, input_io_array, output_io_array, num_echelons, echelon_array)
...
class BatchDecoder_exp():
    def __init__(
        ...
        status = vc6il.vc6_il_default_batch_decoder_configuration_create(self._codec_config)
        _raise_if_vc6_error(status, "Failed to create vc6 batch decoder config")
        ...
        os.environ['VC6_MINI_BATCH_SIZE'] = str(num_backends)
        status, self._decoder = vc6il.vc6_il_batch_decoder_create(self._codec_config)
        _raise_if_vc6_error(status, "Failed to create vc6 batch decoder")

        if output_memory_type == ImageMemoryType.CUDA_DEVICE:
            for i in range(num_buffers):
                self._output_buffers[i] = cupy_module.zeros(self.buffer_size, dtype=cupy_module.uint8, order="C")
        else:
            for i in range(num_buffers):
                self._output_buffers[i] = bytearray(self.buffer_size,)
    ...
    def decode(
        ...
        while num_images > len(self._output_buffers):
            if self._output_memory_type == ImageMemoryType.CUDA_DEVICE:
                self._output_buffers.append(cupy_module.zeros(self.buffer_size, dtype=cupy_module.uint8, order="C"))
            else:
                self._output_buffers.append(bytearray(self.buffer_size))
        ...
        tmp_io = vc6il.vc6_il_io_t()
        for i in range(num_images):
            vc6il.vc6_il_io_carray_set_item(input_io_array, i, input_buffers[i])
            if self._output_memory_type == ImageMemoryType.CUDA_DEVICE:
                tmp_interface = self._output_buffers[i].__cuda_array_interface__
                if(tmp_interface['version'] != 3):
                    raise Exception(
                ...
                vc6il.vc6_il_io_carray_set_item(output_io_array, i, self._output_buffers[i])
            if i < num_echelons :
                vc6il.Vc6FrameRegionArray_setitem(frame_region_array, i, FrameRegion(echelon=echelons[i])._region)
        vc6il.vc6_il_batch_decode(self._decoder, num_images, input_io_array, output_io_array, num_echelons, frame_region_array)
        
        for i in range(num_images):
            output_images[i] = ImageBuffer(self._output_memory_type, vc6il.Vc6IOArray_getitem(output_io_array, i), None)
        return output_images
