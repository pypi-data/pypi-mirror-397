...
def load_opscalesrv_config():
    ...
    """
    global ALLOWED_HOSTS, LOG_FILE, SERIAL_CONFIG, ENCODING, ANPR_CONFIG, ENABLE_LOGGING, CONFIG
...
class SerialServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if ANPR_MODULE_AVAILABLE and not force_test_mode:
            try:
                # Trigger ANPR fetch and log, get the plate and http status
        ...
        if TEST_MODE or force_test_mode:
            ...
            # Pass captured plate to serial response
            message_data = get_serial_response(plate=license_plate)
        ...
        if ON_REQUEST_CALLBACK:
            try:
                # Pass data to GUI through callback
                ON_REQUEST_CALLBACK(self.client_address[0], self.path, message_data)
            except Exception as e:
                logger.error(f"Callback error: {e}")
