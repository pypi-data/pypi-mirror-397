...
class AttributeBuilderObject(object): # pragma: no cover
    ...
    def __getattr__(self, attr_name:str) -> Optional[Union['AttributeObject',List['AttributeBuilderObject']]]:
        ...
        if attr_name in self._attr_names:
            ...
            #Check if the attribute name is an alias for a known attribute
            for name in self._attr_names:
                if self._shop_api.GetAttributeInfo(self._type, name, "dataFuncName") == attr_name:
                    return AttributeObject(self._shop_api, self._type, self._name, name, self.datatype_dict[name])
