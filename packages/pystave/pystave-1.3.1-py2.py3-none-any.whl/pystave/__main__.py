...
import sys
import argparse
from . import NoteTrainer, GrokDistribution, CLEFS
...
def RunNoteTrainer():
	...
	def midi(x):
		if x == 'auto': return x
		try: return int(x)
		except: raise argparse.ArgumentTypeError('must be an integer, or the string "auto"')
	parser.add_argument( '-m', '--midi-channel',   '--midichannel', metavar='N',         type=midi,       default='auto', help='attempt to open MIDI channel number `N` for input (the default is 0, meaning MIDI is disabled - in which case you can still use keys A-G on your computer keyboard)' )
	...
	parser.add_argument( '-k', '--key-signature',  '--key',         metavar='KEY',                        default='C',    help='the default is "C", which is short for "C major" but you could change this to "F# major" or "Bb minor" or whatever you like' )
	parser.add_argument( '-d', '--distribution',                    metavar='FILENAME',                   default='',     help='file or string dictating the distribution of challenges (use `-d HELP` to see details)' )
	...
	if OPTS.distribution.lower() == 'help':
		help = GrokDistribution.__doc__
		help = help[help.index('*')-1:]
		print(help)
		raise SystemExit(0)
	OPTS.clefs = ','.join( OPTS.clefs ).replace(',', ' ').strip().lower().split()
	unrecognized = [c for c in OPTS.clefs if c not in CLEFS]
