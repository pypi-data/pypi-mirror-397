# coding: utf-8

"""
    Pulp 3 API

    Fetch, Upload, Organize, and Distribute Software Packages

    The version of the OpenAPI document: v3
    Contact: pulp-list@redhat.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from pulpcore.client.pulp_container.models.patchedcontainer_container_repository import PatchedcontainerContainerRepository

class TestPatchedcontainerContainerRepository(unittest.TestCase):
    """PatchedcontainerContainerRepository unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PatchedcontainerContainerRepository:
        """Test PatchedcontainerContainerRepository
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PatchedcontainerContainerRepository`
        """
        model = PatchedcontainerContainerRepository()
        if include_optional:
            return PatchedcontainerContainerRepository(
                pulp_labels = {
                    'key' : ''
                    },
                name = '0',
                description = '0',
                retain_repo_versions = 1,
                remote = '',
                manifest_signing_service = ''
            )
        else:
            return PatchedcontainerContainerRepository(
        )
        """

    def testPatchedcontainerContainerRepository(self):
        """Test PatchedcontainerContainerRepository"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
