...
# 设置仅密钥登录
ssh-auto-upgrade --key-only-login yes
ssh-auto-upgrade -kl no

# 设置检测间隔时间（小时）
ssh-auto-upgrade --check-interval 6

# 设置SSH服务端口
ssh-auto-upgrade --ssh-port 2222
ssh-auto-upgrade -p 2222

# 启用调试模式
ssh-auto-upgrade --debug

# 清理并重新生成SSH私钥
ssh-auto-upgrade --clean-keys
ssh-auto-upgrade -sk

# 跳过依赖检查
ssh-auto-upgrade --skip-dependencies
ssh-auto-upgrade -sd

# 组合使用示例
ssh-auto-upgrade --force --upgrade-time 00:00:00-08:00:00 --root-login yes
ssh-auto-upgrade --service --mirror https://mirrors.aliyun.com/openssh/portable/ -rl no --ssh-port 2222
ssh-auto-upgrade --force --clean-keys --skip-dependencies --upgrade-time 22:00:00-06:00:00
```
...
| `--service` | `-s` | `false` | 注册为systemd服务 |
| `--upgrade-time` | `-t` | `00:00:00-08:00:00` | 升级时间段，格式为 HH:MM:SS-HH:MM:SS |
| `--root-login` | `-rl` | `auto` | root登录配置：auto(智能检测), yes(启用), no(禁用) |
| `--key-only-login` | `-kl` | `auto` | 仅密钥登录配置：auto(智能检测), yes(启用仅密钥登录), no(允许密码登录) |
| `--check-interval` | 无 | `1` | 检测间隔时间，单位小时，默认1，最低1，最大48 |
| `--ssh-port` | `-p` | `22` | SSH服务端口，默认22，建议使用非标准端口增强安全性 |
| `--debug` | `-debug` | `false` | 显示详细的依赖检测过程 |
| `--clean-keys` | `-sk` | `false` | 清理并重新生成SSH私钥（默认不清理旧私钥） |
| `--skip-dependencies` | `-sd` | `false` | 跳过依赖检查，直接执行升级流程（默认不跳过） |
