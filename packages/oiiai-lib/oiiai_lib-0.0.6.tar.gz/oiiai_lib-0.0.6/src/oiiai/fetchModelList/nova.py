"""
Nova (Amazon) model list fetching
"""
...
class FetchNova(FetchBase):
    ...
    def __init__(self, api_key: str = None):
        ...
        """
        self._api_key = api_key or os.getenv("NOVA_API_KEY", "")
    ...
    def fetch_models(self) -> List[str]:
        ...
        if not self._api_key:
            return self._handle_missing_env_var("NOVA_API_KEY")

        headers = {"Authorization": f"Bearer {self._api_key}"}

        try:
            response = self._http_get(NOVA_API_URL, headers=headers)
    ...
    def fetch_models_raw(self) -> Dict[str, Any]:
        ...
        if not self._api_key:
            self._log_warning(
                f"[{self.provider}] Missing environment variable: NOVA_API_KEY"
            )
            return {}
