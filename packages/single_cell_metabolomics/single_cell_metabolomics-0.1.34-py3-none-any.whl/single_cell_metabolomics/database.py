...
class DatabaseController:
    ...
    def create_data(self, serial_number, dataset_id, pipeline_stage, result_type, result_data):
        try:
            with self.conn.cursor() as cursor:
                insert_sql = SQL('''
                    INSERT INTO {} ("serialNumber", "datasetId", "pipelineStage", "resultName", "resultData")
                    VALUES (%s, %s, %s, %s, %s)
                ''').format(Identifier(self.table_name))
                cursor.execute(insert_sql, (serial_number, dataset_id, pipeline_stage, result_type, Jsonb(result_data)))
                self.conn.commit()
        except Exception as e:
            print(f"Error inserting data: {e}")
            raise e
    
    def read_data(self, serial_number, dataset_id, pipeline_stage, result_type):
        try:
            with self.conn.cursor() as cursor:
                select_sql = SQL('''
                    SELECT "resultData" FROM {}
                    WHERE "serialNumber" = %s AND "datasetId" = %s AND "pipelineStage" = %s AND "resultName" = %s
                ''').format(Identifier(self.table_name))
                cursor.execute(select_sql, (serial_number, dataset_id, pipeline_stage, result_type))
    ...
    def update_data(self, serial_number, dataset_id, pipeline_stage, result_type, new_result_data):
        try:
            with self.conn.cursor() as cursor:
                update_sql = SQL('''
                    UPDATE {}
                    ...
                    WHERE "serialNumber" = %s AND "datasetId" = %s AND "pipelineStage" = %s AND "resultName" = %s
                ''').format(Identifier(self.table_name))
                cursor.execute(update_sql, (Jsonb(new_result_data), serial_number, dataset_id, pipeline_stage, result_type))
                self.conn.commit()
        except Exception as e:
            print(f"Error updating data: {e}")
            raise e

    def delete_data(self, serial_number, dataset_id, pipeline_stage, result_type):
        try:
            with self.conn.cursor() as cursor:
                delete_sql = SQL('''
                    DELETE FROM {}
                    WHERE "serialNumber" = %s AND "datasetId" = %s AND "pipelineStage" = %s AND "resultName" = %s
                ''').format(Identifier(self.table_name))
                cursor.execute(delete_sql, (serial_number, dataset_id, pipeline_stage, result_type))
                self.conn.commit()
        except Exception as e:
            print(f"Error deleting data: {e}")
            raise e

    def save_large_dataframe(self, serial_number, dataset_id, pipeline_stage, result_type, result_dataframe, storage_folder):
        """Save large dataframe to Parquet file and store metadata in DB"""
        try:
            # Save dataframe to Parquet
            storage_path = storage_folder / f"{dataset_id}_{pipeline_stage}_{result_type}.parquet"
            ...
            self.create_data(serial_number, dataset_id, pipeline_stage, result_type, result_data)
        except Exception as e:
            print(f"Error saving large dataframe: {e}")
            raise e
        
    def load_large_dataframe(self, serial_number, dataset_id, pipeline_stage, result_type):
        """Load large dataframe from Parquet file using metadata from DB"""
        try:
            result_records = self.read_data(serial_number, dataset_id, pipeline_stage, result_type)
            if not result_records:
                print(f"No data found for dataset {dataset_id}, stage {pipeline_stage}, type {result_type}")
                return None
