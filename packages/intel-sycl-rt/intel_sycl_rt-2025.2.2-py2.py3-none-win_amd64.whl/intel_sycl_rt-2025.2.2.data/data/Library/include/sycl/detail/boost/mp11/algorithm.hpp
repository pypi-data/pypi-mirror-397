...
#ifndef SYCL_DETAIL_BOOST_MP11_ALGORITHM_HPP_INCLUDED
#define SYCL_DETAIL_BOOST_MP11_ALGORITHM_HPP_INCLUDED
...
#include <sycl/detail/boost/mp11/detail/mp_map_find.hpp>
#include <sycl/detail/boost/mp11/detail/mp_with_index.hpp>
#include <sycl/detail/boost/mp11/detail/mp_fold.hpp>
#include <sycl/detail/boost/mp11/detail/mp_min_element.hpp>
#include <sycl/detail/boost/mp11/detail/mp_copy_if.hpp>
#include <sycl/detail/boost/mp11/detail/mp_remove_if.hpp>
...
#if SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_MSVC, == 1900 ) || SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_GCC, < 40800 )
...
#if SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_CUDA, >= 9000000 && SYCL_DETAIL_BOOST_MP11_CUDA < 10000000 )
...
#if defined(SYCL_DETAIL_BOOST_MP11_HAS_TYPE_PACK_ELEMENT)
...
{
#if SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_MSVC, <= 1800 )
...
#if SYCL_DETAIL_BOOST_MP11_CLANG && defined( SYCL_DETAIL_BOOST_MP11_HAS_FOLD_EXPRESSIONS )
...
#elif !defined( SYCL_DETAIL_BOOST_MP11_NO_CONSTEXPR )
...
#if defined( SYCL_DETAIL_BOOST_MP11_HAS_CXX14_CONSTEXPR )
...
template<class... T, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR F mp_for_each_impl( mp_list<T...>, F && f )
{
    using A = int[sizeof...(T)];
    return (void)A{ ((void)f(T()), 0)... }, std::forward<F>(f);
}

template<class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR F mp_for_each_impl( mp_list<>, F && f )
{
...
template<class L, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR mp_if_c<mp_size<L>::value <= 1024, F> mp_for_each( F && f )
{
    return detail::mp_for_each_impl( mp_rename<L, mp_list>(), std::forward<F>(f) );
}

template<class L, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR mp_if_c<mp_size<L>::value >= 1025, F> mp_for_each( F && f )
{
...
template<class L, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR F mp_for_each( F && f )
{
    return detail::mp_for_each_impl( mp_rename<L, mp_list>(), std::forward<F>(f) );
}
...
#endif // #ifndef SYCL_DETAIL_BOOST_MP11_ALGORITHM_HPP_INCLUDED
