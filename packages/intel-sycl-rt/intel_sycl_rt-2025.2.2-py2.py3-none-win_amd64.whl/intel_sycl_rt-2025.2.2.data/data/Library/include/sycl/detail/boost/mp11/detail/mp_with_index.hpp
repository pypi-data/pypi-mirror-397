...
#ifndef SYCL_DETAIL_BOOST_MP11_DETAIL_MP_WITH_INDEX_HPP_INCLUDED
#define SYCL_DETAIL_BOOST_MP11_DETAIL_MP_WITH_INDEX_HPP_INCLUDED
...
# define SYCL_DETAIL_BOOST_MP11_CONSTEXPR14 constexpr
#else
# define SYCL_DETAIL_BOOST_MP11_CONSTEXPR14
#endif

#if defined( __GNUC__ ) || defined( __clang__ )
# define SYCL_DETAIL_BOOST_MP11_UNREACHABLE_DEFAULT default: __builtin_unreachable();
#elif defined( _MSC_VER )
# define SYCL_DETAIL_BOOST_MP11_UNREACHABLE_DEFAULT default: __assume(false);
#else
# define SYCL_DETAIL_BOOST_MP11_UNREACHABLE_DEFAULT
#endif
namespace sycl
{
...
{
    template<std::size_t K, class F> static SYCL_DETAIL_BOOST_MP11_CONSTEXPR14 decltype(std::declval<F>()(std::declval<mp_size_t<0>>())) call( std::size_t i, F && f )
    {
        if( i < N / 2 )
        {
...
template<> struct mp_with_index_impl_<2>
{
    template<std::size_t K, class F> static SYCL_DETAIL_BOOST_MP11_CONSTEXPR14 decltype(std::declval<F>()(std::declval<mp_size_t<0>>())) call( std::size_t i, F && f )
    {
        switch( i )
        {
        SYCL_DETAIL_BOOST_MP11_UNREACHABLE_DEFAULT
...
template<std::size_t N, class F> inline SYCL_DETAIL_BOOST_MP11_CONSTEXPR14 decltype(std::declval<F>()(std::declval<mp_size_t<0>>())) mp_with_index( std::size_t i, F && f )
{
    assert( i < N );
    return detail::mp_with_index_impl_<N>::template call<0>( i, std::forward<F>(f) );
}

template<class N, class F> inline SYCL_DETAIL_BOOST_MP11_CONSTEXPR14 decltype(std::declval<F>()(std::declval<mp_size_t<0>>())) mp_with_index( std::size_t i, F && f )
{
    return mp_with_index<std::size_t{N::value}>( i, std::forward<F>(f) );
}

#undef SYCL_DETAIL_BOOST_MP11_CONSTEXPR14
#undef SYCL_DETAIL_BOOST_MP11_UNREACHABLE_DEFAULT
...
#endif // #ifndef SYCL_DETAIL_BOOST_MP11_DETAIL_MP_WITH_INDEX_HPP_INCLUDED
