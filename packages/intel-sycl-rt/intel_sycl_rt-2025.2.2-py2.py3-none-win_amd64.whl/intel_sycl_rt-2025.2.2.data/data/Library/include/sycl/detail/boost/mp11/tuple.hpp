...
#ifndef SYCL_DETAIL_BOOST_MP11_TUPLE_HPP_INCLUDED
#define SYCL_DETAIL_BOOST_MP11_TUPLE_HPP_INCLUDED
...
#include <sycl/detail/boost/mp11/integer_sequence.hpp>
#include <sycl/detail/boost/mp11/list.hpp>
#include <sycl/detail/boost/mp11/function.hpp>
...
#if SYCL_DETAIL_BOOST_MP11_MSVC
# pragma warning( push )
...
template<class F, class Tp, std::size_t... J> SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_apply_impl( F && f, Tp && tp, integer_sequence<std::size_t, J...> )
...
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_apply( F && f, Tp && tp )
...
template<class T, class Tp, std::size_t... J> SYCL_DETAIL_BOOST_MP11_CONSTEXPR T construct_from_tuple_impl( Tp && tp, integer_sequence<std::size_t, J...> )
{
...
template<class T, class Tp,
    class Seq = make_index_sequence<std::tuple_size<typename std::remove_reference<Tp>::type>::value>>
SYCL_DETAIL_BOOST_MP11_CONSTEXPR T construct_from_tuple( Tp && tp )
{
...
template<class Tp, std::size_t... J, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR F tuple_for_each_impl( Tp && tp, integer_sequence<std::size_t, J...>, F && f )
{
    using A = int[sizeof...(J)];
    return (void)A{ ((void)f(get<J>(std::forward<Tp>(tp))), 0)... }, std::forward<F>(f);
}

template<class Tp, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR F tuple_for_each_impl( Tp && /*tp*/, integer_sequence<std::size_t>, F && f )
{
...
template<class Tp, class F> SYCL_DETAIL_BOOST_MP11_CONSTEXPR F tuple_for_each( Tp && tp, F && f )
{
...
template<class... T> SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tp_forward_r( T&&... t ) -> std::tuple<T&&...>
{
    return std::tuple<T&&...>( std::forward<T>( t )... );
}

template<class... T> SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tp_forward_v( T&&... t ) -> std::tuple<T...>
{
    return std::tuple<T...>( std::forward<T>( t )... );
}

template<std::size_t J, class... Tp>
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tp_extract( Tp&&... tp )
    -> decltype( tp_forward_r( get<J>( std::forward<Tp>( tp ) )... ) )
{
    return tp_forward_r( get<J>( std::forward<Tp>( tp ) )... );
}

#if !SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_MSVC, < 1900 )

template<class F, class... Tp, std::size_t... J>
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_transform_impl( integer_sequence<std::size_t, J...>, F const& f, Tp&&... tp )
...
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_transform_impl( integer_sequence<std::size_t, J...>, F const& f, Tp1&& tp1 )
...
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_transform_impl( integer_sequence<std::size_t, J...>, F const& f, Tp1&& tp1, Tp2&& tp2 )
...
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_transform_impl( integer_sequence<std::size_t, J...>, F const& f, Tp1&& tp1, Tp2&& tp2, Tp3&& tp3 )
...
#endif // !SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_MSVC, < 1900 )
...
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_transform( F const& f, Tp1&& tp1, Tp&&... tp )
...
SYCL_DETAIL_BOOST_MP11_CONSTEXPR auto tuple_transform( F const& f, Tp&&... tp )
    -> decltype( detail::tuple_transform_impl( Seq(), f, std::forward<Tp>(tp)... ) )
{
    return detail::tuple_transform_impl( Seq(), f, std::forward<Tp>(tp)... );
}

#endif // SYCL_DETAIL_BOOST_MP11_WORKAROUND( SYCL_DETAIL_BOOST_MP11_MSVC, < 1910 )
...
#endif // #ifndef SYCL_DETAIL_BOOST_TUPLE_HPP_INCLUDED
