...
from cnos_hub import CnosHub, AsyncCnosHub, DefaultAioHttpClient
from cnos_hub._utils import is_dict
...
logging.getLogger("cnos_hub").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[CnosHub]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with CnosHub(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncCnosHub]:
