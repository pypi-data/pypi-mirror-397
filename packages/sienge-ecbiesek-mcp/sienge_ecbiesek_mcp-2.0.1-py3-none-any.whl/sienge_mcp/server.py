...
async def ap_process_invoice_complete(
    ...
    """
    🏗️ PIPELINE COMPLETA: Processa uma nota fiscal completa automaticamente
    
    Fluxo:
    1. Valida centro de custo (se auto_validate=True)
    2. Cria nota fiscal
    3. Busca itens do pedido de compra
    4. Adiciona itens à NF
    5. Busca título criado automaticamente pelo Sienge
    6. Audita título (valores e parcelas)
    7. Anexa DANFE (se fornecido)
    8. Lista parcelas finais
    
    Args:
        invoice_data: Dados da nota fiscal (document_id, number, supplier_id, issue_date, movement_date, movement_type_id, notes, company_id opcional)
        purchase_order_id: ID do pedido de compra
        danfe_path: Caminho do arquivo DANFE/PDF (opcional)
        auto_validate: Validar centro de custo automaticamente (padrão: True)
    
    Returns:
        Dict com resultado completo do processamento e log detalhado
    ...
    try:
        # 1️⃣ Validar centro de custo
        if auto_validate:
            log_steps.append("🔍 Validando centro de custo do pedido...")
            logger.info("Validando centro de custo para pedido %s", purchase_order_id)
            
            validacao = await _validate_purchase_order_company_internal(
                purchase_order_id=purchase_order_id,
                company_id=invoice_data.get("company_id")
            )
            
            if not validacao.get("success"):
                log_steps.append(f"❌ Erro na validação: {validacao.get('message')}")
                return {
                    "success": False,
                    "error": "VALIDATION_FAILED",
                    "message": validacao.get("message"),
                    "log": log_steps,
                }
            
            if not validacao.get("valid"):
                # Usar empresa correta automaticamente
                correct_company_id = validacao["building"]["companyId"]
                log_steps.append(
                    f"⚠️ Empresa ajustada automaticamente: {invoice_data.get('company_id')} → {correct_company_id} "
                    f"({validacao['building']['companyName']})"
                )
                invoice_data["company_id"] = correct_company_id
            else:
                if "company_id" not in invoice_data:
                    invoice_data["company_id"] = validacao["building"]["companyId"]
                log_steps.append(
                    f"✅ Empresa validada: {validacao['building']['companyName']} (ID: {validacao['building']['companyId']})"
                )
        
        # 2️⃣ Criar nota fiscal
        log_steps.append(f"📄 Criando nota fiscal {invoice_data.get('number')}...")
        logger.info("Criando NF %s", invoice_data.get("number"))
        
        nf_result = await create_sienge_purchase_invoice(
        ...
        logger.info("Buscando itens do pedido %s", purchase_order_id)
        
        itens_result = await get_sienge_purchase_order_items(purchase_order_id=purchase_order_id)
        ...
        # 4️⃣ Adicionar itens à NF
        log_steps.append(f"📥 Adicionando {len(items)} itens à NF...")
        logger.info("Adicionando %d itens à NF %s", len(items), sequential_number)
        ...
        add_items_result = await add_items_to_purchase_invoice(
            sequential_number=sequential_number,
        ...
        # 5️⃣ Buscar título criado automaticamente
        log_steps.append("🔍 Buscando título criado automaticamente...")
        logger.info("Buscando título para NF %s", sequential_number)
        
        # Tentar múltiplos métodos para encontrar o título
        bill_id = await resolve_bill_id_for_invoice(sequential_number)
        
        if not bill_id:
            log_steps.append("❌ Título não foi criado automaticamente pelo Sienge")
            return {
                "success": False,
                "error": "BILL_NOT_FOUND",
                "message": "Título não foi criado automaticamente. Verifique a configuração do Sienge.",
                "sequential_number": sequential_number,
                "log": log_steps,
            }
        
        log_steps.append(f"✅ Título encontrado: ID {bill_id}")
        
        # 6️⃣ Auditar título
        log_steps.append("🔍 Auditando título...")
        logger.info("Auditando título %s", bill_id)
        
        auditoria = await ap_list_installments(bill_id=bill_id)
        
        if auditoria.get("success"):
            installments = auditoria.get("installments", [])
            total_installments = sum(Decimal(str(p.get("amount", 0))) for p in installments)
            log_steps.append(
                f"✅ Auditoria concluída: {len(installments)} parcela(s), "
                f"Total: R$ {float(total_installments):.2f}"
            )
        else:
            log_steps.append("⚠️ Não foi possível auditar as parcelas")
        
        # 7️⃣ Anexar DANFE (se fornecido)
        if danfe_path:
            log_steps.append(f"📎 Anexando DANFE: {danfe_path}...")
            logger.info("Anexando DANFE ao título %s", bill_id)
            ...
            if not os.path.exists(danfe_path):
                ...
                attach_result = await ap_attach_bill(
                    bill_id=bill_id,
                    description=f"DANFE {invoice_data.get('number')} - {invoice_data.get('notes', '')}",
                    file_path=danfe_path,
                )
                
                if attach_result.get("success"):
                    log_steps.append("✅ DANFE anexado com sucesso")
                else:
                    log_steps.append(f"⚠️ Erro ao anexar DANFE: {attach_result.get('message')}")
        
        # 8️⃣ Resultado final
        log_steps.append("✅ Processamento completo!")
        
        return {
            "success": True,
            "message": f"✅ NF {invoice_data.get('number')} processada com sucesso!",
            "sequential_number": sequential_number,
            "bill_id": bill_id,
            "items_count": len(items),
            "audit": auditoria if auditoria.get("success") else None,
            "log": log_steps,
        }
...
@mcp.tool
async def ap_attach_bill_file(bill_id: str, file_path: str, description: str) -> Dict:
    """
