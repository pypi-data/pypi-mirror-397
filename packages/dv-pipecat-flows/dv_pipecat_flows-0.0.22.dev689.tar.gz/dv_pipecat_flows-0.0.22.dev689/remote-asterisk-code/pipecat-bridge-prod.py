...
class Config:
    ...
    def reload(self):
        ...
        # Asia stage endpoint for PB_Fintech calls
        self.PIPECAT_WS_ASIA_STAGE_URL = os.getenv(
            "PIPECAT_WS_ASIA_STAGE_URL", "ws://127.0.0.1:8080/ws/asia-stage"
        )
...
PIPECAT_WS_ASIA_STAGE_URL = config.PIPECAT_WS_ASIA_STAGE_URL
...
async def config_reloader():
    ...
    global \
        PIPECAT_WS_URL, \
        PIPECAT_WS_URL_LOCAL, \
        PIPECAT_WS_ASIA_STAGE_URL, \
        BRIDGE_HOST, \
        DEFAULT_ENCODING, \
        DEFAULT_SR
...
def get_call_status(cause_code: int) -> str:
    ...
    elif cause_code in [17, 18, 19, 21]:
...
async def start_for_channel(
    ...
    if inbound_provider:
        ...
        if inbound_provider.lower() == "pb_fintech":
            ws_url = f"{PIPECAT_WS_ASIA_STAGE_URL}?{qs}"
            print(f"[bridge] Connecting to Asia Stage WebSocket for {inbound_provider}: {ws_url}")
        else:
                ...
                async for raw in ws:
                    ...
                    if t == "StasisStart":
                        ...
                        if ch_name.startswith("UnicastRTP/") and ch_id in pending_ext:
                            ...
                            try:
                                await ari.post(
                                    f"/ari/bridges/{bridge_id}/addChannel", channel=ch_id
                                )
                                print(
                            ...
                            originator = ch.get("linkedid")
                            print(
                                f"[bridge] Local channel started: {ch_name} ({ch_id}), linkedid: {originator}"
                            )
                        else:
                            if DEBUG_MODE:
                                print(f"[bridge-debug] Ignored channel type: {ch_type}")

                        args = ev.get("args", [])
                        app_data = ev.get("asterisk_id", "")
                        ...
                        # Skip UnicastRTP channels from being treated as new calls
                        if not ch_name.startswith("UnicastRTP/"):
                            if ch_id not in sessions and ch_id not in processing_channels:
                                ...
                                print(
                                    f"[bridge] Skipping {ch_id} - already in sessions or processing"
                                )
