...
def capture_function_locals(func, capture_locals=True, capture_self=True):
    ...
    def tracer(frame, event, arg):
        ...
        if frame.f_code is target_code and frame.f_globals.get("__name__") == target_module:
            try:
                print(f"[DEBUG] Tracer hit: {event} in {frame.f_code.co_name}") 

                if not entered:
                    entered = True
                    print(f"[DEBUG] Tracer ENTERED: {target_name}")
                ...
                if event == "return":
                    print(f"[DEBUG] Tracer RETURN: {target_name}")
                    f_locals = frame.f_locals
...
# (Legacy track_llm_call removed - use veriskgo.llm.track_llm_call)
