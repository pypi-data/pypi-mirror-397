...
def getPrompt(prompt : str) -> str:
    try:
        with importlib.resources.open_text("querycraft.templates", prompt) as fichier:
            return fichier.read()
    except FileNotFoundError:
        print(f"Le fichier '{prompt}' est introuvable dans querycraft.templates.")
        return ""
...
def loadCache(cacheName : str, duree = 2) -> dict():
    ...
    with importlib.resources.path("querycraft.cache", cacheName) as file:
        if existFile(file):
            ...
            cache = {k: v for k, v in cache.items() if diff_dates_iso(v[1], datetime.now().date().isoformat()) < duree}
    #print(f"Cache {cacheName} chargé")
    return cache
...
def compare_query_results_raw(
    ...
    """
    Compare deux tables de résultats SQL (sans pandas).
...
def format_table_1(headers, rows, padding=2, inter=False, table_size=MAX_TABLE_WIDTH, min_col_width=MIN_COLUMN_WIDTH):
    ...
    def format_row(row):
        return "│ " + " ┆ ".join(
            str(value if value is not None else "").replace("\n", " ").ljust(width) for value, width in zip(row, col_widths)
        ) + " │"
...
def format_table_2(headers, rows, padding=1, inter=False, table_size=MAX_TABLE_WIDTH, min_col_width=MIN_COLUMN_WIDTH):
    ...
    def format_row(row):
        cells = [
            fit_text(str(value if value is not None else "").replace("\n", " "), width) for value, width in zip(row, col_widths)
        ]
        return "│ " + " ┆ ".join(cells) + " │"

    def format_row_hd(row):
        cells = [
            "\033[1m" + fit_text(str(value).replace("\n", " "), width)+"\033[0m"  for value, width in zip(row, col_widths)
        ]
        return "│ " + " ┆ ".join(cells) + " │"
...
def diff_dates_iso(date_iso1: str, date_iso2: str, nb_jours: int = 60) -> bool:
    d1 = date.fromisoformat(date_iso1)
    d2 = date.fromisoformat(date_iso2)
    return abs(d2 - d1) > timedelta(days=nb_jours)
