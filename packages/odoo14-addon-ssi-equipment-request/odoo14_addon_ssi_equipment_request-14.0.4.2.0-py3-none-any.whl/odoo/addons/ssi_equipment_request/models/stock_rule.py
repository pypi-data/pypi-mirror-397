    ...
    def _get_stock_move_values(
        ...
        if move_dest_ids:
            for move_dest_tupple in move_dest_ids:
                ...
                if move.equipment_request_line_ids:
                    equipment_request_line_ids += move.equipment_request_line_ids.ids
                    result.update(
                        {
                            "equipment_request_line_ids": [
                                (6, 0, equipment_request_line_ids)
                            ],
                        }
                    )
        return result
