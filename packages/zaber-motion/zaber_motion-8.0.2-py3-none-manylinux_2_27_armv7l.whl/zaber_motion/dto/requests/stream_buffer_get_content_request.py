...
class StreamBufferGetContentRequest:
    ...
    def validate(self) -> None:
        ...
        if self.buffer_id is None:
            raise ValueError(f'Property "BufferId" of "StreamBufferGetContentRequest" is None.')

        if not isinstance(self.buffer_id, (int, float, decimal.Decimal)):
            raise ValueError(f'Property "BufferId" of "StreamBufferGetContentRequest" is not a number.')

        if int(self.buffer_id) != self.buffer_id:
            raise ValueError(f'Property "BufferId" of "StreamBufferGetContentRequest" is not integer value.')
