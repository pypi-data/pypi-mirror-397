...
class MyChrome(webdriver.Chrome, webdriver.Remote):
    ...
    def find_element_recursive(self, by, value, frames):
        for frame in frames:
            try:
                ...
                try:
                    ...
                    nested_frames = super(MyChrome, self).find_elements(
        ...
        raise NoSuchElementException(
            f"Element {value} not found in any frame or iframe"
        )
    ...
    def find_element(self, by=By.ID, value=None, iframe=False):
        ...
        if iframe:
            ...
            if not frames:
                raise NoSuchElementException(
                    f"No iframes found in the current page while searching for {value}"
                )
            self.iframe_env = True
    ...
    def find_elements_recursive(self, by, value, frames):
        ...
        raise NoSuchElementException(
            f"Elements with {value} not found in any frame or iframe"
        )
        ...
        for frame in frames:
            try:
                ...
                try:
                    ...
                    # Recurse into nested iframes
                    nested_frames = super(MyEdge, self).find_elements(
                        By.CSS_SELECTOR, "iframe"
                    )
                    if nested_frames:
                        element = self.find_element_recursive(by, value, nested_frames)
...
if sys.platform != "darwin":
    ES = True
    if ES:  # 用自己写的ES版本
        import mtmai.mtlibs.easyspider.undetected_chromedriver_ES as uc
    else:
