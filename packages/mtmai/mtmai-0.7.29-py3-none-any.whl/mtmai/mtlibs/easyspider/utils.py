...
def send_email(config):
    ...
    try:
        ...
        message["From"] = Header(
            f"{config['username'].split('@')[0]} <{config['username']}>"
        )
        to_name_list = []
...
def rename_downloaded_file(download_dir, stop_event):
    ...
    while not stop_event.is_set():
        ...
        for file in files:
            ...
            if (
                not full_path.endswith(".crdownload")
                and not full_path.endswith(".htm")
                and not full_path.endswith(".html")
                and not full_path.startswith("esfile_")
            ):
                new_name = (
                    "esfile_"
                    + file.split("/")[-1]
                ...
                if press_time["is_pressed"] == False:  # 没按下p键时，记录按下p键的时间
                    ...
                    if duration > 2:
                        if event._flag == False:
                            print(
                                "任务执行中，长按"
                                + press_time["pause_key"]
                                + "键暂停执行。"
                            )
                            print(
                                "Task is running, long press '"
                                + press_time["pause_key"]
                                + "' to pause."
                            )
...
def detect_optimizable(param, ignoreWaitElement=True, waitElement=""):
    ...
    if (
        param["beforeJS"] == ""
        and param["afterJS"] == ""
        and param["contentType"] <= 1
        and param["splitLine"] == 0
    ):
        if param["nodeType"] <= 2:
        ...
        try:
            ...
            if response.status_code == requests.codes.ok:
                ...
                JS = (
                    ...
                    + url
                    + "', true); xhr.responseType = 'blob'; xhr.onload = function() {var reader = new FileReader(); reader.readAsDataURL(xhr.response); reader.onloadend = function() { var base64data = reader.result;}}; xhr.send();"
                    ""
                )
                ...
                if base64data:
                    ...
                    browser.print_and_log(
                        "下载图片失败，只能使用元素截图功能下载图片。"
                    )
                    browser.print_and_log(
                        "Failed to download image, can only download image using element screenshot function."
                    )
                    # 使用元素截图功能下载图片
                    try:
                        ...
                        browser.print_and_log(
                            "下载图片失败|Error downloading image: ", e
                        )
        except Exception as e:
...
def replace_field_values(orginal_text, outputParameters, browser=None):
    ...
    try:
        replaced_text = re.sub(
            pattern,
            lambda match: outputParameters.get(match.group(1), ""),
            orginal_text,
        )
        ...
        print(
            "eval替换失败，请检查eval语句是否正确。| Failed to replace eval, please check if the eval statement is correct."
        )
        print(e)
...
def write_to_excel(file_name, data, types, record):
    ...
    if os.path.exists(file_name):
        ...
        if num_rows > 1000:
            print(
                "Excel文件中的数据行数已超过1000行，过多的行数将会导致追加模式写入数据速度变慢，建议增大任务保存对话框中的“每采集多少条数据保存一次”选项的值以提升采集速度，或者更换为CSV文件或MySQL数据库存储数据。正在读取数据，请稍等..."
            )
            print(
                "The number of rows in the Excel file already exceeds 1000, too many rows will cause the speed of writing data in append mode to slow down, it is recommended to increase the value of the 'Save every how many data' option in the task save dialog to improve the collection speed, or replace it with CSV file or MySQL database to store data. Reading data, please wait..."
            )
        ws = wb.active
        if num_rows > 1000:
            print("读取数据完成，正在追加数据...")
    ...
    for line in data:
        ...
        try:
            ...
            print(
                "Failed to write to Excel file, please check if the data type is correct."
            )
    # 保存工作簿
    try:
        wb.save(file_name)
        ...
        print(
            "Failed to save Excel file, please check if the file is opened by other programs."
        )
...
class myMySQL:
    def __init__(self, config_file="mysql_config.json"):
        ...
        try:
            ...
            print(
                "Failed to read configuration file, please check if the configuration file: "
                + config_file
                + " exists, or if the configuration information is incorrect."
            )
            print(e)
        self.connect()
    ...
    def remove_duplicate_data(self):
        ...
        sql = (
            f"CREATE TABLE {self.table_name}_temp AS "
            + "SELECT MIN(_id) AS _id, "
            + fields
            + f" FROM {self.table_name} GROUP BY "
            + fields
            + ";"
        )
        self.cursor.execute(sql)
