...
def _setup_telemetry(
    ...
    # longer experimental.
    if otel_to_cloud:
        _setup_gcp_telemetry_experimental(internal_exporters=internal_exporters)  # type: ignore
    elif _otel_env_vars_enabled():
        _setup_telemetry_from_env_experimental(
            internal_exporters=internal_exporters  # type: ignore
        )
    else:
...
def _setup_gcp_telemetry_experimental(
    internal_exporters: list[SpanProcessor] = None,  # type: ignore
):
    ...
    otel_hooks_to_add.append(
        get_gcp_exporters(
            ...
            google_auth=(credentials, project_id),  # type: ignore
        )
    )
...
class AdkWebServer:
    ...
    def get_fast_api_app(
        ...
        setup_observer: Callable[
            [Observer, "AdkWebServer"], None  # type: ignore
        ] = lambda o, s: None,
        ...
        async def list_apps(
            ...
            if detailed:
                apps_info = self.agent_loader.list_agents_detailed()  # type: ignore
