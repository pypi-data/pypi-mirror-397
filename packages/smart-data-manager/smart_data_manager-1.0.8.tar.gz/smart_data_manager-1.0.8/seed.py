...
# -----------------------------
# SCHEMA & CLEANUP (CRITICAL FIXES)
# -----------------------------
def drop_tables(engine):
    """Drops tables to clear 'bad' schemas created by Pandas automatically."""
    tables = [
    ...
    with engine.begin() as conn:
        # Disable foreign key checks for the drop if possible, or drop in order
        for table in tables:
            try:
...
def seed_database():
    ...
    # STEP 1: Wipe the 'broken' tables away
    drop_tables(engine)
    
    # STEP 2: Create the 'clean' tables using your SQL schema
    execute_schema(engine)
    
    # STEP 3: Seed data into the clean tables
