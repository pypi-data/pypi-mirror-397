...
def seed_orders_and_items(engine):
    ...
    # Seed Orders
    orders = [{"customer_id": random.choice(cust_ids), "total_amount": 0, "status": "Paid"} for _ in range(NUM_ORDERS)]
    ...
    for oid in order_ids:
        ...
        items.append({"order_id": oid, "product_id": p["product_id"], "quantity": 1, "price": p["price"]})
    
    pd.DataFrame(items).to_sql("OrderItems", engine, if_exists="append", index=False)
    logging.info(f"🧾 Inserted {NUM_ORDERS} orders and items")
