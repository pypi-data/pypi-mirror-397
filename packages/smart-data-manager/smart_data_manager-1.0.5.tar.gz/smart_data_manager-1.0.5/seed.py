...
def clear_tables(engine):
    tables_to_clear = [
        ...
        "DailySalesSummary", "CustomerSegments", "ProductPerformance",
        "DimDate", "DimCustomers", "DimProducts", "FactOrderItems", "FactOrders"
    ]
    
    with engine.begin() as conn:
        for table in tables_to_clear:
            try:
                # Using a cross-dialect safe way to check table existence
                conn.execute(text(f"DELETE FROM {table}"))
                print(f"  - Cleared {table}")
            except Exception as e:
                # If table doesn't exist (Error 208), just skip it
                if "208" in str(e):
