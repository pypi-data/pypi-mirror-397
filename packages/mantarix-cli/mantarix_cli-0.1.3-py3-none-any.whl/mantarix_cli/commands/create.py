...
from mantarix.utils import slugify
...
class Command(BaseCommand):
    """
    Create a new Mantarix app from a template.
    """
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--project-name",
            dest="project_name",
            help="project name for the new Mantarix app",
            required=False,
        )
        parser.add_argument(
            "--description",
            dest="description",
            help="the description to use for the new Mantarix project",
            required=False,
        )
        parser.add_argument(
            "--template",
            ...
            choices=["default"],
            default="default",
            help="template to use for new Mantarix project",
            required=False,
        )
        parser.add_argument(
            "--package-manager",
        ...
        parser.add_argument(
            ...
            dest="template_ref",
            type=str,
            help="the branch, tag or commit ID to checkout after cloning the repository with Mantarix app templates",
        )
    ...
    def handle(self, options: argparse.Namespace) -> None:
        ...
        template_data = {
            ...
            "mantarix_version": mantarix.version.version,
        }

        template_ref = options.template_ref
        if not template_ref and mantarix.version.version:
            template_ref = version.Version(mantarix.version.version).base_version
        ...
        try:
            cookiecutter(
                f"gh:mantarix-dev/mantarix-app-templates",
        ...
        console.print(f"mantarix run {app_dir}\n")
