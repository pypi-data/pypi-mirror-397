...
from mantarix.core.types import WebRenderer
from mantarix.utils import copy_tree, is_within_directory, random_string
...
class Command(BaseCommand):
    """
    Publish Mantarix app as a standalone web app.
    """
    ...
    def handle(self, options: argparse.Namespace) -> None:
        from mantarix.utils.pip import ensure_mantarix_web_package_installed

        ensure_mantarix_web_package_installed()
        from mantarix_web import get_package_web_dir, patch_index_html, patch_manifest_json
        ...
        if not os.path.exists(web_path):
            print(f"Mantarix module does not contain 'web' directory: {web_path}")
            sys.exit(1)
        ...
        # - <!-- pyodideCode -->
        # - <!-- flutterWebRenderer -->
        # - %MANTARIX_ROUTE_URL_STRATEGY%
        # - %MANTARIX_WEB_PYODIDE%

        base_url = (
            (options.base_url or get_pyproject("tool.mantarix.web.base_url") or "/")
            .strip("/")
            .strip()
        )
        ...
        app_name = (
            options.app_name or get_pyproject("tool.mantarix.product") or app_short_name
        )
        ...
        pwa_background_color = options.pwa_background_color or get_pyproject(
            "tool.mantarix.web.pwa_background_color"
        )

        pwa_theme_color = options.pwa_theme_color or get_pyproject(
            "tool.mantarix.web.pwa_theme_color"
        )
        ...
        patch_index_html(
            ...
            web_renderer=WebRenderer(
                (
                    ...
                    or get_pyproject("tool.mantarix.web.renderer")
                    or "canvaskit"
                )
            ),
            use_color_emoji=(
                ...
                if (
                    ...
                    or get_pyproject("tool.mantarix.web.use_color_emoji")
                )
                else False
            ),
            route_url_strategy=str(
                options.route_url_strategy
                or get_pyproject("tool.mantarix.web.route_url_strategy")
                or "path"
            ),
        )
