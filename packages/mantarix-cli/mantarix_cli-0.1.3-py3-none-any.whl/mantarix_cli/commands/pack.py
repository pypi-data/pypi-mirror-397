...
from mantarix.utils import is_macos, is_windows
from mantarix_cli.commands.base import BaseCommand


class Command(BaseCommand):
    """
    Package Mantarix app to a standalone bundle.
    """
    ...
    def handle(self, options: argparse.Namespace) -> None:
        from mantarix.utils.pip import ensure_mantarix_desktop_package_installed
        ...
        try:
            ...
            from mantarix_cli.__pyinstaller.utils import copy_mantarix_bin
            ...
            # copy "bin"
            hook_config.temp_bin_dir = copy_mantarix_bin()

            if hook_config.temp_bin_dir is not None:
                # delete mantarixd/mantarixd.exe
                mantarixd_path = os.path.join(
                    hook_config.temp_bin_dir, "mantarixd.exe" if is_windows() else "mantarixd"
                )
                if os.path.exists(mantarixd_path):
                    os.remove(mantarixd_path)

                if is_windows():
                    from mantarix_cli.__pyinstaller.win_utils import (
                        update_mantarix_view_icon,
                        update_mantarix_view_version_info,
                    )

                    exe_path = os.path.join(
                        hook_config.temp_bin_dir, "mantarix", "mantarix.exe"
                    )
                    if os.path.exists(exe_path):
                        ...
                        if options.icon:
                            ...
                            update_mantarix_view_icon(exe_path, icon_path)

                        # version info
                        version_info_path = update_mantarix_view_version_info(
                            exe_path=exe_path,
                    ...
                    from mantarix_cli.__pyinstaller.macos_utils import (
                        assemble_app_bundle,
                        unpack_app_bundle,
                        update_mantarix_view_icon,
                        update_mantarix_view_version_info,
                    ...
                    tar_path = os.path.join(
                        hook_config.temp_bin_dir, "mantarix-macos-amd64.tar.gz"
                    )
                    if os.path.exists(tar_path):
                        ...
                        if options.icon:
                            ...
                            if not Path(icon_path).is_absolute():
                                icon_path = str(Path(os.getcwd()).joinpath(icon_path))
                            update_mantarix_view_icon(app_path, icon_path)

                        # version info
                        app_path = update_mantarix_view_version_info(
                            app_path=app_path,
            ...
            print("Please install PyInstaller module to use mantarix pack command:", e)
            sys.exit(1)
