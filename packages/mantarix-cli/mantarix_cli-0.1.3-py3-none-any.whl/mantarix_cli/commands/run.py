...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            type=int,
            default=None,
            help="custom TCP port to run Mantarix app on",
        )
        parser.add_argument(
            "--host",
            dest="host",
            type=str,
            default=None,
            help='host to run Mantarix web app on. Use "*" to listen on all IPs.',
        )
        parser.add_argument(
            "--name",
            dest="app_name",
    ...
    def handle(self, options: argparse.Namespace) -> None:
        from mantarix.utils.pip import (
            ensure_mantarix_desktop_package_installed,
            ensure_mantarix_web_package_installed,
        )
        ...
        if options.web:
            ...
            ensure_mantarix_desktop_package_installed()
        from mantarix_desktop import close_mantarix_view
        ...
        if get_pyproject("tool.mantarix.app.path"):
            script_dir = script_dir.joinpath(get_pyproject("tool.mantarix.app.path"))
        ...
        mantarix_app_data_dir = project_dir / "storage" / "data"
        mantarix_app_data_dir.mkdir(parents=True, exist_ok=True)

        mantarix_app_temp_dir = project_dir / "storage" / "temp"
        if mantarix_app_temp_dir.exists():
            shutil.rmtree(str(mantarix_app_temp_dir), ignore_errors=True)
        mantarix_app_temp_dir.mkdir(parents=True, exist_ok=True)
        ...
        my_event_handler = Handler(
            ...
            mantarix_app_data_dir=str(mantarix_app_data_dir),
            mantarix_app_temp_dir=str(mantarix_app_temp_dir),
        )
        ...
        close_mantarix_view(my_event_handler.pid_file)
        my_observer.stop()
        my_observer.join()
...
class Handler(FileSystemEventHandler):
    def __init__(
        ...
        assets_dir,
        ignore_dirs,
        mantarix_app_data_dir,
        mantarix_app_temp_dir,
    ) -> None:
        super().__init__()
        self.args = args
        ...
        self.page_url = None
        self.mantarix_app_data_dir = mantarix_app_data_dir
        self.mantarix_app_temp_dir = mantarix_app_temp_dir
    ...
    def start_process(self):
        ...
        if self.web or self.ios or self.android:
            p_env["MANTARIX_FORCE_WEB_SERVER"] = "true"

            # force page name for ios
            if self.ios or self.android:
                p_env["MANTARIX_WEB_APP_PATH"] = "/".join(Path(self.script_path).parts[-2:])
        if self.port is not None:
            p_env["MANTARIX_SERVER_PORT"] = str(self.port)
        if self.host is not None:
            p_env["MANTARIX_SERVER_IP"] = str(self.host)
        if self.page_name:
            p_env["MANTARIX_WEB_APP_PATH"] = self.page_name
        if self.uds_path is not None:
            p_env["MANTARIX_SERVER_UDS_PATH"] = self.uds_path
        if self.assets_dir is not None:
            p_env["MANTARIX_ASSETS_DIR"] = self.assets_dir
        p_env["MANTARIX_DISPLAY_URL_PREFIX"] = self.page_url_prefix

        p_env["MANTARIX_APP_STORAGE_DATA"] = self.mantarix_app_data_dir
        p_env["MANTARIX_APP_STORAGE_TEMP"] = self.mantarix_app_temp_dir
            ...
            if line.startswith(self.page_url_prefix):
                if not self.page_url:
                    ...
                    if self.ios or self.android:
                        ...
                        th = threading.Thread(
                            target=self.open_mantarix_view_and_wait, args=(), daemon=True
                        )
                        th.start()
            else:
                print(line)

    def open_mantarix_view_and_wait(self):
        from mantarix_desktop import open_mantarix_view

        self.fvp, self.pid_file = open_mantarix_view(
    ...
    def print_qr_code(self, orig_url: str, android: bool):
        ...
        qr_url = (
            ...
            if android
                ...
                ("mantarix", "mantarix-host", quote(lan_url, safe=""), None, None, None)
            )
        )
        # print(qr_url)
        qr = qrcode.QRCode()
