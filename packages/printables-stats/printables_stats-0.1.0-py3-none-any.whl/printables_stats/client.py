...
class PrintablesClient:
    ...
    def parse_stats(self, html_content):
        ...
        try:
            ...
            # Look for stats-row class

            # Downloads & Likes usually in the first row or distinctive icons

            # Downloads: look for arrow-down-to-line icon
            ...
            # Followers / Following
            # Looking for buttons containing text
            ...
            if calendar_icon:
                ...
                if joined_div:
                    stats["joined_date"] = joined_div.get_text(strip=True).replace(
                        "Joined in ", ""
                    )

            # Models Count
            # In navigation tabs: 3D Models <small>4</small>
            # Find link ending in /models
            ...
            # Badges
            badges = {}
            # 1. Try JSON from scripts (most robust for unauthenticated scraping)
            json_badges_found = False
            scripts = soup.find_all("script")
            for s in scripts:
                if s.string and "badgesSelection" in s.string:
                    try:
                        # The script content is usually a JSON object
                            ...
                            if "data" in body_json and "user" in body_json["data"]:
                                ...
                                for b in badges_sel:
                                    if "badge" in b and "userLevel" in b:
                                        ...
                                        # Handle cases where userLevel might be None or missing badgeLevel
                                        if (
                                            b["userLevel"]
                                            ...
                                            # Fallback if no level info but badge exists
                                            badges[category] = 1
            ...
            # 2. Try detailed DOM list (authenticated view or different layout)
            if not badges:
            ...
            # 3. Fallback to simple badges (sidebar/mobile view)
            if not badges:
