    ...
    def to_toml(self, indent_level=0, table_path='', comments=None, source=None) -> str:
        ...
        for key, value, item_type in items_with_types:
            ...
            if item_type == 'simple':
                ...
                # Array of tables - use numbered sections instead of array-of-tables
                for idx, elem in enumerate(value.elements):
                    if isinstance(elem, TomlTable):
                        full_path = f'{table_path}.{key}' if table_path else key
                        section_name = full_path if idx == 0 else f'{full_path}_{idx}'
                        if lines:
                            lines.append('')
                        lines.append(f'[{section_name}]')
                        nested_content = elem._to_toml_content(0, section_name, {}, source)
                        if nested_content:
                            lines.append(nested_content)
...
def _handle_non_table_root(ast_node, leading_comments, inline_comments, source):
    ...
    if isinstance(ast_node, TomlArray) and ast_node.needs_array_of_tables_format():
        ...
        for idx, elem in enumerate(ast_node.elements):
            section_name = 'value' if idx == 0 else f'value_{idx}'
            lines.append(f'[{section_name}]')
...
def _handle_heterogeneous_array(ast_node, inline_comments, source):
    ...
    for idx, elem in enumerate(ast_node.elements):
        # Generate section name: [value] for first, [value_1] for second, [value_2] for third, etc.
        section_name = 'value' if idx == 0 else f'value_{idx}'
        lines.append(f'[{section_name}]')
        
        if isinstance(elem, TomlTable):
            lines.append(elem._to_toml_content(comments=inline_comments, source=source))
        elif isinstance(elem, TomlArray):
            has_inner_tables = any(isinstance(e, TomlTable) for e in elem.elements)
            if has_inner_tables:
                for table_idx, table_elem in enumerate(elem.elements):
                    if isinstance(table_elem, TomlTable):
                        table_section = f'{section_name}.{key_name}' if table_idx == 0 else f'{section_name}.{key_name}_{table_idx}'
                        lines.append(f'[{table_section}]')
                        lines.append(table_elem._to_toml_content(comments=inline_comments, source=source))
            else:
