...
from neurovc.util.IO_util import imagesc
...
class SingleRecording:
    ...
    def __init__(self, hdf_file, n_frames=None, shape=None):
        ...
        self.frames = h5py.File(self.hdf_file, 'r')[frame_key]
...
def __test_thermal2():
    ...
    thermal_file = "G:/covid-19/data/pilot3/elena/thermal/ThermalData_18_06_2020_13_24_58.h5"
...
def synchronize_timestamps(left_ts, right_ts, fps=30):
    ...
    idx_left = np.round(np.interp(left_time_resampled, left_ts,
                                  np.arange(0, left_ts.shape[0]))).astype(int)
    right_time_resampled = np.arange(start_time, end_time, 1000 / fps, float)
    idx_right = np.round(np.interp(right_time_resampled, right_ts,
                                   np.arange(0, right_ts.shape[0]))).astype(int)
