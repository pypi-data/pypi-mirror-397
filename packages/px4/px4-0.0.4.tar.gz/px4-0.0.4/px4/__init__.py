...
class PX4Commander:
    ...
    async def _execute_trajectory(
        ...
        target_position = start_pos + position_offset
        print(f"Moving to start position: {target_position} with yaw {target_yaw}")
        await self.client.goto(target_position, distance_threshold=DISTANCE_THRESHOLD, target_yaw=target_yaw, yaw_threshold=YAW_THRESHOLD)
        ...
        while iteration < n_iterations:
            ...
            self.client.command(target_position, target_velocity, yaw=target_yaw)
            await asyncio.sleep(0.01)
