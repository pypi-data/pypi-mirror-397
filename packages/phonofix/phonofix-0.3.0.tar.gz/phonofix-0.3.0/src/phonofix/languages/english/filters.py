...
def token_boundaries(*, tokenizer: Any, text: str) -> set[int]:
    """
    取得 token 邊界集合，供 exact 匹配避免子字串誤擊

    例：term "Go" 的 alias "go" 不應在 "gopher" 中誤命中。
    """
    boundaries: set[int] = set()
    for start, end in tokenizer.get_token_indices(text):
