...
def search_bible_names(gui=None, q='', **_):
    ...
    async def show_names():
        ...
        with names_container:
            spinner = ui.spinner(size='lg').classes('self-center mt-10')
            label = ui.label('Loading Bible Names...').classes('self-center text-gray-500')

        try:
            # 2. Preparation
            items = list(bible_names.items())
            chunk_size = 50  # Render 50 items at a time to keep UI responsive
            
            for i in range(0, len(items), chunk_size):
                chunk = items[i : i + chunk_size]
                
                with names_container:
                    for name, meaning in chunk:
                        # Row setup
                        with ui.row().classes('w-full shadow-sm rounded-lg items-center no-wrap border border-gray-200 !gap-0') as row:
                            ...
                            with ui.element('div').classes('flex-none pt-1'): 
                                with ui.chip(name, removable=True, icon='book').props('text-color=white').classes('cursor-pointer font-bold shadow-sm') as chip:
                                    with ui.menu():
                            ...
                            # --- Content ---
                            clean_meaning = re.sub(r"^<b>.*?</b> - ", "", meaning)
                            ui.html(clean_meaning, sanitize=False).classes('grow min-w-0 leading-relaxed pl-2 text-base break-words')
                
                # 3. Yield to the event loop so the spinner can spin and DOM can update
                await asyncio.sleep(0)

        finally:
            # 4. Remove Spinner
            names_container.remove(spinner)
            names_container.remove(label)
    ...
    if q:
        input_field.value = q
        ui.timer(0.5, lambda: filter_names(None, keep=False), once=True)