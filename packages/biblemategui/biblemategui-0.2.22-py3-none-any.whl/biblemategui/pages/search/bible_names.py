...
def search_bible_names(gui=None, q='', **_):
    ...
    #show_names(); wait a bit for the page to be ready
    #ui.timer(0.1, lambda: loading(show_names), once=True)
    async def check_q_at_startup():
        show_names()
        filter_names(None, keep=False)
    if q:
        input_field.value = q
        ui.timer(0.1, check_q_at_startup, once=True)
    else:
        ui.timer(0.1, lambda: loading(show_names), once=True)