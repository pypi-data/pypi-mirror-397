# Python Client For Nutanix Lifecycle Management APIs

The Python client for Nutanix Lifecycle Management APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Infrastructure, Software and Firmware Upgrades.
## Features
...
## Version Negotiation
By default, the client negotiates the API version with the server to ensure compatibility. Version negotiation is **enabled by default**. To disable version negotiation and use a fixed API version, set the `allow_version_negotiation` parameter to `False` when creating the ApiClient:

```python
import ntnx_lifecycle_py_client

# Disable version negotiation
api_client = ntnx_lifecycle_py_client.ApiClient(allow_version_negotiation=False)
```
...
```sh
<my-env>/bin/pip install ntnx-lifecycle-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-lifecycle-py-client
```
...
## Configuration
The python client for Nutanix Lifecycle Management APIs can be configured with the following parameters
...
```python
# Initialize the API
app_dr_api_instance = AppDrApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Retrieves an App Protection Policy by its external identifier.
try:
    api_response = app_dr_api_instance.get_app_protection_policy(extId)
...
# Updates an App Protection Policy.
try:
    # The body parameter in the following operation is received from the previous GET request's response which needs to be updated.
    api_response = app_dr_api_instance.update_app_protection_policy(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
```python
# Initialize the API
bom_api_instance = BomApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# List Bill of Materials Description
try:
    api_response = bom_api_instance.list_boms(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=lifecycle&version=v4.2&language=python). This documentation is auto-generated, and the location may change.
