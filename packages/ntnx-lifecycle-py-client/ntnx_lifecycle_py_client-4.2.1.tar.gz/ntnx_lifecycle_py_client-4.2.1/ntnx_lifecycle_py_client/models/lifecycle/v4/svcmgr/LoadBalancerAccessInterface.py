...
class LoadBalancerAccessInterface(object):

    """An access interface is a user's notion of a named load balancer network. It abstracts out the network details from developers and service deployers (dev-ops). The name should be something that defines the positioning of the load balancer in the data center. e.g. \"corp\", \"wan\", \"dc\", \"it\", \"it-hr\". Currently in a given subdomain - the interface should be unique - which greatly simplifies what the service deployer has to specify to expose the service. A single load balancer defines one or more interfaces at load balancer creation time. 

    :param cidr: (:attr:`cidr`) CIDR (Classless Inter-Domain Routing) address of the load balancer.
    :type cidr: 
    
    
    :param ips: (:attr:`ips`) Access Interface IPs of the load balancer.
    :type ips: 
    
    
    :param subnet: (:attr:`subnet`) Subnet configuration of the load balancer.
    :type subnet: required
    
    
    :param network: (:attr:`network`) Network identifier of the load balancer.
    ...
    swagger_types = {
        'cidr': 'str',
        'ips': 'list[common.v1.config.IPv4Address]',
        'subnet': 'str',
        'network': 'str',
    ...
    def __init__(self, cidr=None, ips=None, subnet=None, network=None, *args, **kwargs):  # noqa: E501
        self.__cidr = None
        self.__ips = None
        self.__subnet = None
        self.__network = None
    ...
    def _initialize_object_type(self):
        return 'lifecycle.v4.svcmgr.LoadBalancerAccessInterface'
    ...
    @cidr.setter
    def cidr(self, cidr):
        if cidr is not None and not re.search('^([0-9]{1,3}\\.){3}[0-9]{1,3}(\/([0-9]|[1-2][0-9]|3[0-2]))?$', cidr):  # noqa: E501
            raise ValueError(r"Invalid value for `cidr`, must be a follow pattern or equal to `/^([0-9]{1,3}\\.){3}[0-9]{1,3}(\/([0-9]|[1-2][0-9]|3[0-2]))?$/`")  # noqa: E501
        ...
        for attr, attr_type in six.iteritems(self.swagger_types):
            ...
            if isinstance(value, list):
                ...
                if sanitize:
                    ...
                    result[attr] = value
        if issubclass(LoadBalancerAccessInterface, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, LoadBalancerAccessInterface):
            return False
