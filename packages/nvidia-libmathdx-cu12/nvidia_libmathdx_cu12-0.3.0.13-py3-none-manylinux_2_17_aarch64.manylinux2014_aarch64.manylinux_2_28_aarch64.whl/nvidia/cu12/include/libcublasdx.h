...
typedef enum cublasdxDeviceFunctionType_t {
     ...
     * type value_type_c (passed by `void*` pointers), and tile_pipeline is
 ...
 *
 * Runtime shapes and strides are marked by passing `LIBMATHDX_RUNTIME` in \p shape and \p stride .
 ...
 * @param[in] shape An array of size `rank` indicating the tensor shape. LIBMATHDX_RUNTIME can be used to indicate a runtime shape.
 * @param[in] stride An array of size `rank` indicating the tensor stride. LIBMATHDX_RUNTIME can be used to indicate a runtime stride.
 ...
 * @param[in] pipeline_depth The depth of the pipeline. If `LIBMATHDX_MAX_PIPELINE_DEPTH` is passed, the pipeline depth will be set to the maximal depth based on available shared memory.
 ...
 *
 * @param[in] handle A cuBLASDx device function, output of \ref cublasdxCreateDeviceFunction.
 * @return `COMMONDX_SUCCESS` on success, or an error code.
 */
