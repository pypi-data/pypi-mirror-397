...
class AgentManager:
    ...
    async def init(self) -> None:
        ...
        logger.info(f"AgentManager: Initializing {len(self._agent_configs)} agents...")
        init_tasks = []
        ...
        logger.info(f"AgentManager: All agents initialized.")
    ...
    async def post_init(self, model_router: ModelRouter, controller: "BaseController") -> None:
        ...
        Args:
            ...
            controller (BaseController): Controller orchestrating this pod.
        """
        self._model_router = model_router
        self._controller = controller
        await asyncio.gather(*(agent.post_init(model_router, controller) for agent in self._agents.values()))
        logger.info(f"AgentManager: All agents post-initialized with dependencies.")
    ...
    async def deliver_message(self, to_id: str, message: Message) -> None:
        ...
        if agent is None:
            logger.error(f"Could not find agent '{to_id}' to deliver message.")
            return

        perception_component = agent.get_component("perceive")
        if perception_component is None:
            logger.warning(f"Agent '{to_id}' has no perception component.")
            return

        await perception_component.add_message(message)
        logger.debug(f"Delivered message to local agent '{to_id}'.")
    ...
    async def run_agent_method(
        ...
        if agent is None:
            raise ValueError(f"Agent ID '{agent_id}' not found in this pod.")

        component = agent.get_component(component_name)
        if component is None:
            raise ValueError(f"Component '{component_name}' not found in agent '{agent_id}'.")

        if not hasattr(component, method_name):
            raise AttributeError(f"Method or attribute '{method_name}' not found in component '{component_name}'.")
    ...
    async def save_to_db(self) -> None:
        ...
        """
        logger.info(f" Saving state for {len(self._agents)} agents...")
        save_tasks = [agent.save_to_db() for agent in self._agents.values()]
        if save_tasks:
            try:
                await asyncio.gather(*save_tasks)
                logger.info(f"All agent states saved successfully.")
            except Exception as e:
                logger.error(f"Error during concurrent agent save: {e}", exc_info=True)
        else:
            logger.info(f"No agents to save.")
    ...
    async def load_from_db(self) -> None:
        ...
        """
        logger.info(f"Loading state for {len(self._agents)} agents...")
        load_tasks = [agent.load_from_db() for agent in self._agents.values()]
        if load_tasks:
            try:
                await asyncio.gather(*load_tasks)
                logger.info(f"All agent states loaded successfully.")
            except Exception as e:
                logger.error(f"Error during concurrent agent load: {e}", exc_info=True)
        else:
            logger.info(f"No agents to load.")
    ...
    async def add_agent(self, agent_id: str, template_name: str, data: Dict[str, Any]) -> bool:
        ...
        if not agent_config:
            logger.error(f"Failed to generate config for new agent '{agent_id}'.")
            return False

        logger.info(f"Adding new agent '{agent_id}'...")
        try:
            ...
            self._agents[agent_id] = agent_instance
            logger.info(f"Successfully added agent '{agent_id}'. Total agents: {len(self._agents)}")
            return True
        except Exception as exc:
            logger.error(f"Failed to add agent '{agent_id}': {exc}", exc_info=True)
            return False
    ...
    async def remove_agent(self, agent_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error during cleanup for agent '{agent_id}': {exc}",
                exc_info=True,
            )

        del self._agents[agent_id]
        logger.info(f"Successfully deleted agent '{agent_id}'. Remaining agents: {len(self._agents)}")
        return True
