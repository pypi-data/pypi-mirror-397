...
class OPR(HttpTIProvider):
    ...
    def lookup_iocs(
        ...
        if not domain_list:
            return pd.DataFrame()
        results.extend(
            [pd.Series(elt) for elt in self._lookup_bulk_request(domain_list)],
        )

        all_results: list[pd.Series] = results + bad_requests
    ...
    def parse_results(self: Self, response: dict) -> tuple[bool, ResultSeverity, Any]:
        ...
        if "response" in response["RawResult"]:
            dom_records: list[dict[str, Any]] = response["RawResult"]["response"]
            dom_record: dict[str, Any] = dom_records[0]
            return self._parse_one_record(dom_record)
        return True, severity, {}

    def _parse_multi_results(self: Self, response: dict) -> Iterable[dict]:
        ...
        if not isinstance(response["RawResult"], dict):
            ...
            for dom_record in dom_records:
                ...
                domain_name: str = dom_record["domain"]
                yield {
                    "Ioc": domain_name,
                    "IocType": "dns",
    ...
    def _parse_one_record(
        dom_record: dict,
    ) -> tuple[bool, ResultSeverity, dict[str, Any]]:
        record_status: int = dom_record.get("status_code", 404)
        severity = ResultSeverity.information
        if record_status == httpx.codes.OK:
            return (
                True,
                severity,
                {
        ...
        if record_status == httpx.codes.NOT_FOUND:
            return (
                True,
    ...
    def _lookup_bulk_request(self: Self, ioc_list: Iterable[str]) -> Iterable[dict]:
    ...
    def _lookup_batch(self: Self, ioc_list: list) -> Iterable[dict]:
        ...
        path: str = self._QUERIES["dns"].path
        req_url: str = f"{self._BASE_URL}{path}?{qry_str}"
