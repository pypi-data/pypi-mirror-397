...
KOSMO_PROTOCOL = """
# KOSMO PROTOCOL (Embedded v1.0)

## Levels
- Level 0 (VOID): Raw/creative mode. Use `void setup` to initialize workspace.
- Level 1 (KOSMO): Strict determinism, zero-entropy execution.
- Level 2 (MASKS): Expert personas (Athena, Chiron).

## Commands
- `void setup`: Initialize KOSMO workspace with protocol files.
- `kosmo help`: Display this help.
- `kosmo plan <goal>`: Generate implementation plan.
- `kosmo build <file>`: Build from plan.
- `kosmo be athena`: Activate Theoretical Verifier.
...
- `CHAOS SET CHAIN <name>`: Create chain.
- `CHAOS PUSH CHAIN <name>`: Save chain.
- `CHAOS CHAINLIST`: List chains.
- `CHAOS DERIVE`: Generate codebase snapshot.

## Scope Enforcement
Commands ONLY execute within `%hermios::{...}` braces.
Outside braces = NO-OP.
"""
