...
from planar.evals.exceptions import EvalNotFoundError
...
def _coerce_agent_input(agent: Agent, payload: dict[str, Any] | str) -> Any:
    input_type = getattr(agent, "input_type", None)
    if input_type and isinstance(payload, dict):
        try:
            return input_type.model_validate(payload)  # type: ignore[attr-defined]
...
def _get_agent(agent_name: str) -> Agent:
    ...
    if not agent:
        raise EvalNotFoundError("agent not found")
    return agent
...
async def run_eval_case_step(
    ...
    try:
        ...
        agent_input = _coerce_agent_input(agent, case.input_payload)
...
async def execute_eval_run(run_id: UUID) -> None:
    ...
    try:
        ...
        # Process cases in batches according to concurrency limit
        ...
        # Determine overall run status based on case results
