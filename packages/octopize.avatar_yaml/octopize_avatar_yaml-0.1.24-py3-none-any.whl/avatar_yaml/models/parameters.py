...
class ReportParametersSpec:
    ...
    output_format: str = "pdf"  # Supported formats: 'pdf', 'docx'

    def serialize(self) -> dict[str, Any]:
        """Serialize to dictionary, excluding default output_format value and None values.

        This keeps YAML flexible by omitting output_format when it's the default "pdf".
        Users can include it explicitly or omit it.

        Returns:
            Dictionary representation with output_format excluded if it equals "pdf"
        ...
        result = asdict(
            self, dict_factory=lambda d: {key: value for (key, value) in d if value is not None}
        )
        # Remove output_format if it's the default "pdf" to keep YAML clean
        if result.get("output_format") == "pdf":
            del result["output_format"]
        return result
