...
from ..drivers.type_mapping import _extract_column_metadata_from_description, _infer_sqlite_column_types_from_query, _create_fallback_columns
from typing import List, Dict, Any, Tuple, Optional, Union, Literal, Self
import logging
import pandas as pd
...
def read_sql(
    ...
    **kwargs
    ...
    assert output_format in ['pandas', 'polars', 'dask', 'pyarrow'], \
    ...
    try:
        ...
        if native_fallback:
            ...
            # Dask fallback: Requires index column for partitioning
                ...
                assert isinstance(dask_index_column, str), \
                    "dask_index_column must be a string when using Dask"
                try:
