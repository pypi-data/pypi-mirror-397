    ...
    **kwargs
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self._connection_params = {k: v for k, v in {
        ...
        if pull_from_env:
            ...
            self._connection_params = {**env_config, **{k: (env_config.get(k) if v is None else v) for k, v in self._connection_params.items()}}
    ...
    def _retrieve_credentials_from_secrets(**kwargs) -> Optional[Dict[str, Any]]:
        ...
        if secret_sources_count > 1:
            logger.warning(
                "Multiple secret sources specified. Using first one found in priority order.",
                emoji="⚠️"
            )
        ...
        try:
            ...
            if vault_dynamic_role:
                ...
                logger.info(
                    f"Generated dynamic credentials (valid {creds.get('lease_duration', 0)}s)",
                    emoji="✅"
                )
                return {
    ...
    def execute_query_with_metadata(self, query: str, parameters: Optional[Union[Tuple, Dict[str, Any]]] = None, **kwargs) -> Any:
    ...
    def bulk_insert(self, sql: str, data: List[Union[Tuple, List, Dict[str, Any]]], 
                   chunk_size: Optional[int] = None, on_conflict: Optional[str] = None, **kwargs) -> int:
    ...
    def execute_many(self, query: str, parameters_list: List[Union[Tuple, Dict[str, Any]]], 
                    chunk_size: Optional[int] = None, **kwargs) -> int:
    ...
    def to_sqlalchemy(self, **engine_kwargs) -> Any:
        ...
        try:
            ...
            connection_result = DatabaseConnectionFactory.create_connection(
                ...
                connection_params=enhanced_params,
                connection_options=engine_kwargs
            )
    ...
    def read_sql_connector_x(self, 
        ...
        if partition_on is not None:
            if partition_range is None or partition_num is None:
                raise ValueError("When partition_on is specified, both partition_range and partition_num must be provided")
        ...
        try:
            ...
            logger.debug(f"Executing ConnectorX query for {self._dialect.description} with return_type={return_type}", emoji="🚀")
            ...
            if hasattr(result, 'shape'):
                ...
                logger.debug(f"ConnectorX query completed: {rows:,} rows × {cols} cols in {execution_time:.3f}s", emoji="⚡")
    ...
    def __repr__(self) -> str:
        ...
        return (f"DatabaseConnection(dialect={self._dialect.name_value}, "
