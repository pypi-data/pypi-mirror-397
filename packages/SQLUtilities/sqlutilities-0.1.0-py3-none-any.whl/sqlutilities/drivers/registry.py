...
class DatabaseDriver(Enum):
    ...
    CONNECTORX_SQLSERVER = DriverConfig(
        ...
        parameter_mappings={
            ...
            'trust_server_certificate_ca': ParameterMapping('trust_server_certificate_ca', 'trust_server_certificate_ca')
        },
    ...
    ORACLEDB = DriverConfig(
        ...
        supported_kwargs=[
            'dsn', 'mode', 'encoding', 'nencoding', 'threaded', 'events',
            'purity', 'newpassword', 'wallet_location', 'wallet_password',
            'disable_oob', 'stmtcachesize', 'edition', 'tag', 'matchanytag',
    ...
    CX_ORACLE = DriverConfig(
        ...
        requires_dsn=True,
        supported_kwargs=[
            'mode', 'encoding', 'nencoding', 'threaded', 'events',
            'purity', 'newpassword', 'tag', 'matchanytag', 'edition',
    ...
    PSYCOPG2 = DriverConfig(
        ...
        supported_kwargs=[
            'sslmode', 'connect_timeout', 'application_name', 'client_encoding',
            'options', 'sslcert', 'sslkey', 'sslrootcert', 'sslcrl',
            'requiressl', 'krbsrvname', 'gsslib', 'service'
        ],
        priority=9,  # Demoted from 10
    ...
    PSYCOPG3 = DriverConfig(
        ...
        supported_kwargs=[
            'sslmode', 'connect_timeout', 'application_name', 'client_encoding',
            'options', 'sslcert', 'sslkey', 'sslrootcert', 'sslcrl',
            'requiressl', 'krbsrvname', 'gsslib', 'service', 'target_session_attrs'
        ],
    ...
    PYODBC_SQLSERVER = DriverConfig(
        ...
        supports_windows_auth=True,
        supported_kwargs=[
            'driver', 'timeout', 'connection_timeout', 'command_timeout',
            'trustservercertificate', 'encrypt', 'multisubnetfailover',
            'applicationintent', 'failoverpartner', 'attachdbfilename',
            'authentication', 'integrated_security', 'trusted_connection',
            'workstation_id', 'app_name', 'language', 'mars_connection',
            'packet_size', 'autocommit', 'ansi', 'quoted_id'
        ],
        priority=9,  # Demoted from 10
    ...
    PYMSSQL = DriverConfig(
        ...
        supported_kwargs=[
            'timeout', 'login_timeout', 'charset', 'as_dict', 'appname',
            'port', 'tds_version', 'autocommit'
        ],
        priority=4,  # Demoted from 5
    ...
    MYSQLCLIENT = DriverConfig(
        ...
        supported_kwargs=[
            'charset', 'connect_timeout', 'read_timeout', 'write_timeout',
            'autocommit', 'local_infile', 'ssl', 'ssl_ca', 'ssl_cert',
            'ssl_key', 'ssl_verify_cert', 'ssl_cipher', 'ssl_capath',
            'init_command', 'sql_mode', 'compress', 'named_pipe',
            'use_unicode', 'client_flag', 'cursorclass'
        ],
    ...
    MYSQL_CONNECTOR = DriverConfig(
        ...
        supported_kwargs=[
            'ssl_disabled', 'ssl_ca', 'ssl_cert', 'ssl_key', 'ssl_verify_cert',
            'ssl_verify_identity', 'ssl_cipher_suites', 'connect_timeout',
            'autocommit', 'charset', 'collation', 'use_pure', 'auth_plugin',
            'sql_mode', 'time_zone', 'init_command', 'client_flags',
            'compress', 'converter_class', 'failover', 'option_files',
            'option_groups', 'allow_local_infile', 'use_unicode'
        ],
        priority=9,  # Demoted from 10
        optimal_chunk_size=1000  # Good for multi-row INSERT
    )
    ...
    PYMYSQL = DriverConfig(
        ...
        supported_kwargs=[
            'charset', 'connect_timeout', 'read_timeout', 'write_timeout',
            'autocommit', 'local_infile', 'ssl', 'ssl_ca', 'ssl_cert',
            'ssl_key', 'ssl_verify_cert', 'ssl_verify_identity', 'ssl_check_hostname',
            'init_command', 'cursor', 'defer_connect', 'sql_mode',
            'client_flag', 'program_name', 'server_public_key'
        ],
        priority=7,  # Demoted from 8
        optimal_chunk_size=1000  # Good for multi-row INSERT
    )
    ...
    BIGQUERY_CLIENT = DriverConfig(
        ...
        parameter_mappings={
            'project': ParameterMapping('project', 'project', required=True),  # Map BIGQUERY_PROJECT or project to project
        ...
        supported_kwargs=[
            'credentials', 'credentials_path', 'location', 'default_query_job_config',
            'client_info', 'client_options', 'dataset_id',
            'api_endpoint', 'emulator_host', 'emulator', 'auth'  # Add emulator params
        ],
        priority=8,  # Demoted from 9 (was demoted from 10)
    ...
    PANDAS_GBQ = DriverConfig(
        ...
        parameter_mappings={
            'project_id': ParameterMapping('project_id', 'project_id', required=True)
        },
        supported_kwargs=[
            'auth_method', 'service_account_path', 'location',
    ...
    REDSHIFT_CONNECTOR = DriverConfig(
        ...
        supported_kwargs=[
            'ssl', 'sslmode', 'timeout', 'max_prepared_statements',
            'tcp_keepalive', 'application_name', 'preferred_role',
            'principal_arn', 'credentials_provider', 'region',
            'cluster_identifier', 'auto_create', 'db_groups'
        ],
    ...
    SQLALCHEMY = DriverConfig(
        ...
        supported_kwargs=[
            ...
            'pool_pre_ping', 'echo', 'echo_pool', 'logging_name',
            'connect_args', 'poolclass', 'pool_events'
        ],
