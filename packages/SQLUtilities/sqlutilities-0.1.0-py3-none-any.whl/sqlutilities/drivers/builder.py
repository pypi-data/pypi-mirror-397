...
class DriverConnectionBuilder:
    ...
    def _convert_parameter_value(self, param_name: str, value: Any) -> Any:
        ...
        boolean_params = {'ssl', 'ssl_insecure', 'iam', 'auto_create', 'force_lowercase',
                         'allow_db_user_override', 'database_metadata_current_db_only',
                         'iam_disable_cache', 'tcp_keepalive'}
        ...
        integer_params = {'port', 'timeout', 'max_prepared_statements', 'idp_response_timeout',
                         'listen_port', 'client_protocol_version'}
    ...
    def build_connection_params(self,
        ...
        for generic_name, mapping in self.config.parameter_mappings.items():
            ...
            if value is not None:
                ...
                for other_name, other_mapping in self.config.parameter_mappings.items():
                    if (other_name != generic_name and
                        other_mapping.driver_param == mapping.driver_param and
        ...
        for key, value in connection_params.items():
            if (key.lower() in [k.lower() for k in self.config.supported_kwargs] and
                key not in self.config.parameter_mappings and
                value is not None):
    ...
    def should_use_windows_auth(self, connection_params: Dict[str, Any]) -> bool:
        ...
        if (integrated_sec in ['yes', 'true', '1'] or
            trusted_conn in ['yes', 'true', '1'] or
            authentication in ['windows', 'activedirectoryintegrated']):
            return True
        ...
        if (not connection_params.get('username') and
            not connection_params.get('password') and
            self.config.supports_windows_auth):
            return True

        return False

    def build_odbc_connection_string(self,
                                   connection_params: Dict[str, Any],
                                   detected_driver: Optional[str] = None) -> str:
        """
        Build ODBC connection string for drivers that require it.
        ...
        for key, value in connection_params.items():
            ...
            elif (key.lower() in [k.lower() for k in self.config.supported_kwargs] and
                  key.lower() not in [m.param_name.lower() for m in self.config.parameter_mappings.values()]):
    ...
    def validate_parameters(self, connection_params: Dict[str, Any]) -> List[str]:
        ...
        for generic_name, mapping in self.config.parameter_mappings.items():
            if mapping.required and connection_params.get(generic_name) is None:
                ...
                if mapping.default_value is None:
                    ...
                    has_alias = any(
                        connection_params.get(other_name) is not None and
                        other_mapping.driver_param == mapping.driver_param
                        for other_name, other_mapping in self.config.parameter_mappings.items()
        ...
        all_supported = (
            list(self.config.parameter_mappings.keys()) +
            [k.lower() for k in self.config.supported_kwargs]
        )
    ...
    def build(self, connection_params: Dict[str, Any], connection_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
