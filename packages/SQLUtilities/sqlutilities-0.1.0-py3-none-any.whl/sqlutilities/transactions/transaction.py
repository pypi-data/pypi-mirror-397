...
try:
    ...
    from errors.registry import SQLErrorRegistry, ErrorCategory, RetryPolicy
    from transactions.config import (
        TransactionState, TransactionConfig, TransactionMetrics,
...
class RobustTransaction:
    ...
    def _should_retry_error(self, error: Exception, attempt: int) -> bool:
        ...
        if (retry_policy == RetryPolicy.RETRYABLE_WITH_DELAY and 
            error_category == ErrorCategory.CONCURRENCY and
            self.config.enable_deadlock_detection):
    ...
    def _begin_transaction(self) -> None:
        ...
        try:
            ...
            if (self.dialect == SQLDialect.SQLSERVER and 
                self.connection.current_driver and 
                self.connection.current_driver['name'] == 'pyodbc' and
                raw_conn is not None and
            ...
            self.metrics.driver_used = self.connection.current_driver['name'] if self.connection.current_driver else None
    ...
    def _commit_transaction(self) -> None:
        ...
        try:
            ...
            if (self.dialect == SQLDialect.SQLSERVER and 
                self.connection.current_driver and 
                self.connection.current_driver['name'] == 'pyodbc'):
    ...
    def _execute_raw(self, query: str, parameters: Optional[Union[Tuple, Dict[str, Any]]] = None, include_metadata: bool = False) -> Any:
        ...
        if isinstance(raw_conn, dict):
            ...
            if conn_type == 'pandas_gbq':
                ...
                try:
                    ...
                    return pandas_gbq.read_gbq(
                        query, 
                        project_id=raw_conn['project_id'],
                        **raw_conn.get('options', {})
                    )
                except ImportError:
            ...
            if (self.connection.current_driver and
                self.connection.current_driver['name'] in ['google-cloud-bigquery'] and
                raw_conn is not None):
                ...
                try:
                    ...
                    if include_metadata:
                        ...
                        description = [(field.name, field.field_type, None, None, None, None, None)
                                       for field in schema]
                        return (rows, description)
                    else:
                        return rows
            ...
            try:
                ...
                if query_upper.startswith('SELECT') or query_upper.startswith('PRAGMA') or query_upper.startswith('DESCRIBE') or query_upper.startswith('SHOW'):
    ...
    def execute(self, query: str, parameters: Optional[Union[Tuple, Dict[str, Any]]] = None, 
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if self._should_retry_error(e, attempt):
                    ...
                    logger.warning(f"Query failed (attempt {attempt + 1}), retrying in {retry_delay:.2f}s: {e}", emoji="🔄")
    ...
    def bulk_insert(self, sql: str, data: List[Union[Tuple, List, Dict[str, Any]]], 
                   chunk_size: Optional[int] = None, on_conflict: Optional[str] = None) -> int:
    ...
    def execute_many(self, query: str, parameters_list: List[Union[Tuple, Dict[str, Any]]], 
    ...
    def _get_optimal_chunk_size(self) -> int:
        ...
        if self.connection.current_driver:
            ...
            from .sql_fundamentals import DatabaseDriver
    ...
    def _parse_insert_columns_and_table(cls, sql: str) -> Tuple[List[str], str]:
        ...
        # For an INSERT, the columns are in the 'this' attribute of the expression
        if isinstance(parsed, sqlglot.exp.Insert): #type: ignore
            return [col.this for col in parsed.this.this.expressions], (lambda p: (p.this.this if isinstance(p.this, sqlglot.exp.Schema) else p.this).sql())(sqlglot.parse_one(sql)) # type: ignore
        else:
            raise ValueError("SQL must be an INSERT statement to extract columns")


    def _normalize_bulk_data_optimized(self, data: List[Union[Tuple, List, Dict[str, Any]]], 
            ...
            try:
                if exc_type is None:
                    ...
                    if self.state == TransactionState.ACTIVE:
                        ...
                        logger.warning(f"Transaction not ACTIVE (state: {self.state.value}), skipping commit", emoji="⚠️")
                else:
    ...
    def __repr__(self) -> str:
        ...
        return (f"RobustTransaction(dialect={self.dialect.name_value}, "
