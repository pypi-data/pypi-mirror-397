...
try:
    from tables.definitions import (
        ...
        TableDefinition, TableComponentType
    )
...
class SQL_TABLE:
    ...
    def __init__(self, 
        ...
        self.database = SQL_DIALECT_REGISTRY.validate_identifier(dialect=self.dialect, identifier=database,
                                                                 context=DatabaseObjectType.DATABASE,
                                                                 correction_method=correction_method)['final'] if database else None
        self.table_name = SQL_DIALECT_REGISTRY.validate_identifier(dialect=self.dialect, identifier=table_name,
                                                                 context=DatabaseObjectType.TABLE,
                                                                 correction_method=correction_method)['final']
        self.original_schema = schema
        self.schema = SQL_DIALECT_REGISTRY.validate_identifier(dialect=self.dialect, identifier=schema,
                                                                 context=DatabaseObjectType.SCHEMA,
                                                                 correction_method=correction_method)['final'] if schema else None
    ...
    def full_table_name(self) -> str:
        ...
        if self.dialect == SQLDialect.BIGQUERY:
            ...
            project_id = self.connection._connection_params.get('project_id') or self.connection._connection_params.get('project')
            dataset_id = self.connection._connection_params.get('dataset_id') or self.connection._connection_params.get('dataset')
            if self.schema:
    ...
    def _generate_existence_query(self) -> str:
        ...
        if self.dialect == SQLDialect.ORACLE:
            ...
            if self.table_name.startswith(self.dialect.quote_character) and self.table_name.endswith(self.dialect.quote_character):
            ...
            if self.schema:
                if self.schema.startswith(self.dialect.quote_character) and self.schema.endswith(self.dialect.quote_character):
    ...
    def populate_definition_from_existing(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Populated table definition with {len(existing_columns)} existing columns", 
                emoji="✅")
            return True
    ...
    def create_table(self, 
                     ...
                     **kwargs) -> Union[bool, str, Tuple[bool, List[str]]]:
        """
    ...
    def recreate_table(self, dry_run: bool = False, **kwargs) -> Union[bool, str]:
        ...
        if dry_run:
            ...
            create_statements, deferred_statements = self._generate_all_creation_sql('fail')  # Don't use IF NOT EXISTS for recreate
    ...
    def _validate_identifier(self, identifier: str, context: DatabaseObjectType, force_encapsulate: bool = False) -> str:
        """
        ...
        return SQL_DIALECT_REGISTRY.validate_identifier(dialect=self.dialect,
                                                         identifier=identifier,
                                                         context=context,
                                                         correction_method='encapsulate' if force_encapsulate else self.correction_method)['final'] #type: ignore
    ...
    def _quote_identifier(self, identifier: str, context: DatabaseObjectType) -> str:
        ...
        """
        return self._validate_identifier(identifier=identifier, 
                                           context=context, 
                                           force_encapsulate=True)
    ...
    def add_primary_key_constraint(self, name: str, columns: List[str]) -> None:
        ...
        constraint = ConstraintDefinition(
            name=name,
            constraint_type=TableComponentType.PRIMARY_KEY,
            columns=columns
        )
    ...
    def add_foreign_key_constraint(self, 
                                   ...
                                   on_update: Optional[str] = None) -> None:
        ...
        self.definition.constraints.append(constraint)
        logger.debug(f"Added foreign key constraint: {name} ({columns} -> {reference_table}.{reference_columns})", emoji="🔗")
    
    def add_unique_constraint(self, name: str, columns: List[str]) -> None:
        """Add a unique constraint definition."""
        constraint = ConstraintDefinition(
            name=name,
            constraint_type=TableComponentType.UNIQUE_CONSTRAINT,
            columns=columns
        )
        self.definition.constraints.append(constraint)
    ...
    def drop_constraint(self, constraint_name: str, **kwargs) -> bool:
        ...
        try:
            constraint_name_quoted = self._validate_identifier(identifier=constraint_name, context=DatabaseObjectType.CONSTRAINT)
    ...
    def _generate_all_creation_sql(self, if_exists: Literal['fail', 'replace', 'append'], defer_constraints: bool = False, defer_indexes: bool = False) -> Tuple[List[str], List[str]]:
        ...
        for constraint in self.definition.constraints:
            ...
            if (constraint.constraint_type == TableComponentType.PRIMARY_KEY and 
            ...
            if constraint.constraint_type == TableComponentType.PRIMARY_KEY:
                ...
                if len(constraint.columns) == 1:
                    ...
                    for i, col_def in enumerate(column_definitions):
                        if col_def.strip().startswith(self._quote_identifier(column_name, context=DatabaseObjectType.COLUMN)):
    ...
    def _map_column_type_to_sql(self, 
                                col_type: COLUMNDTYPE, 
                                size_spec: Union[int, Tuple[int, ...], str, None] = None) -> str:
        """
    ...
    def _generate_constraint_sql(self, constraint: ConstraintDefinition) -> str:
        ...
        if constraint.constraint_type == TableComponentType.PRIMARY_KEY:
            columns_str = ", ".join([self._quote_identifier(col, context=DatabaseObjectType.COLUMN) for col in constraint.columns])
            return f"{base_sql} PRIMARY KEY ({columns_str})"
        
        elif constraint.constraint_type == TableComponentType.FOREIGN_KEY:
            columns_str = ", ".join([self._quote_identifier(col, context=DatabaseObjectType.COLUMN) for col in constraint.columns])
            ref_columns_str = ", ".join([self._quote_identifier(col, context=DatabaseObjectType.COLUMN) for col in constraint.reference_columns or []])
    ...
    def _generate_index_sql(self, index: IndexDefinition) -> str:
        ...
        columns_str = ", ".join([self._quote_identifier(col, context=DatabaseObjectType.COLUMN) for col in index.columns])
        ...
        if index.include_columns and self.dialect == SQLDialect.SQLSERVER:
            include_str = ", ".join([self._quote_identifier(col, context=DatabaseObjectType.COLUMN) for col in index.include_columns])
    ...
    def prepare_insert_statement(self, 
        ...
        metadata = {
            ...
            'has_defaults': any(col.default_value or col.default_expression for col in self.definition.columns if col.name in target_columns)
        }
    ...
    def _get_parameter_style(self) -> Dict[str, str]:
        ...
        return {
            'style': style, 
            'description': f'{description} - {self.dialect.description}'
        }
    ...
    def _generate_upsert_sql(self, 
                           base_sql: str, 
                           values_clause: str, 
                           batch_placeholders: Optional[str],
        ...
        if self.dialect == SQLDialect.POSTGRESQL:
            conflict_cols = ", ".join([self._quote_identifier(col, context=DatabaseObjectType.COLUMN) for col in conflict_columns])
            update_sets = []
...
"""
    ...
    def create_table_from_dataframe(self, 
                                    ...
                                    if_exists: Literal['fail', 'replace', 'append'] = 'append',
                                    dry_run: bool = False) -> Union[bool, str, Tuple[bool, List[str]]]:
        """
        ...
        if 'TableName' in df.columns:
            ...
            if len(table_names) > 1:
                ...
                logger.debug(f"DataFrame contains {len(table_names)} different tables. Processing each separately.", emoji="🔄")
                results = []
                sql_outputs = []
                ...
                logger.debug(f"Completed processing {len(table_names)} tables. {success_count}/{len(table_names)} successful.", emoji="📊")
                return all_success
        ...
        for index, row in df.iterrows():
            ...
            identity_increment = int(row.get('identityIncrement', 1)) if pd.notna(row.get('identityIncrement')) else None
            ...
            if is_foreign_key:
                if not fk_table_name or not fk_field_name:
                    logger.warning(f"Column {field_name} marked as foreign key but missing fkTableName or fkFieldName", emoji="⚠️")
        ...
        logger.debug(f"Creating table {self.full_table_name} with {len(self.definition.columns)} columns (mode: {if_exists})", emoji="🏗️")
        return self.create_table(if_exists=if_exists, dry_run=dry_run)
    
    def _parse_datatype_specifications(self, datatype: str) -> Tuple[Optional[int | str], Optional[int], Optional[int], Optional[int]]:
        """
        ...
        # Pattern for VARCHAR(n), CHAR(n), etc.
        length_pattern = r'(?:VARCHAR|CHAR|NVARCHAR|NCHAR|TEXT)\s*\((\d+|MAX)\)'
        length_match = re.search(length_pattern, datatype.upper())
        if length_match:
            length = f'({length_match.group(1)})' if length_match.group(1).upper() == 'MAX' else int(length_match.group(1))
    ...
    def _map_string_to_columndtype(self, datatype: str) -> Optional[COLUMNDTYPE]:
        ...
        try:
            ...
            if compatible_type != mapped_type:
                logger.trace(f"Dialect compatibility: {mapped_type} -> {compatible_type} for {self.dialect.value}", emoji="🔀")
