...
class TestDatabaseConnection:
    ...
    def test_database_connection_has_connect_method(self):
        ...
        assert hasattr(DatabaseConnection, 'connect') or \
               hasattr(DatabaseConnection, '__init__'), \
    ...
    def test_database_connection_context_manager_support(self):
        ...
        assert has_enter and has_exit, \
            "DatabaseConnection should support context manager protocol"

    @pytest.mark.unit
    def test_database_connection_has_execute_method(self):
        """Test that DatabaseConnection has execute method."""
        connection_methods = dir(DatabaseConnection)
        has_execute = any(method in connection_methods for method in
                         ['execute', 'execute_query', 'query', 'run'])
    ...
    def test_database_connection_has_cursor_method(self):
        ...
        assert 'get_raw_connection' in connection_methods, \
...
class TestConnectionLifecycle:
    ...
    def test_connection_initialization_parameters(self, dialect, config_name, request):
        ...
        assert conn.state.value in ['connected', 'error', 'disconnected'], \
            "Connection should have valid state"
    ...
    def test_connection_state_tracking(self):
        ...
        assert hasattr(DatabaseConnection, 'state'), \
            "DatabaseConnection should have state property"
        ...
        assert conn.state == ConnectionState.DISCONNECTED, \
            f"Initial state should be DISCONNECTED, got {conn.state}"

        # After connecting, state should be CONNECTED
        conn.connect()
        assert conn.state == ConnectionState.CONNECTED, \
            f"After connect, state should be CONNECTED, got {conn.state}"
    ...
    def test_connection_error_handling(self, dialect, config_name, request):
        ...
        assert conn.state.value in ['error', 'disconnected', 'closed'], \
...
class TestConnectionContextManager:
    ...
    def test_context_manager_enter_returns_connection(self, dialect, config_name, request):
        ...
        with DatabaseConnection(
            ...
            **config
            ...
            assert hasattr(conn, 'execute_query'), \
                "Returned object should be usable for executing queries"
    ...
    def test_context_manager_exit_closes_connection(self, dialect, config_name, request):
        ...
        assert conn.state.value in ['disconnected', 'closed', 'idle'] or \
...
class TestConnectionMethods:
    ...
    @pytest.mark.integration
    @pytest.mark.parametrize("dialect,config_name,param_style", [
...
class TestConnectionProperties:
    ...
    def test_connection_has_database_property(self, dialect, config_name, request):
        ...
        assert 'database' in conn_info or 'dbname' in conn_info, \
            "Connection should track database name"
    ...
    def test_connection_string_sanitization(self, dialect, config_name, request):
        ...
        if 'password' in conn_info:
            assert conn_info['password'] == '***', \
                "Password should be masked in connection info"
...
class TestConnectionCompatibility:
    ...
    def test_connection_autocommit_support(self, dialect, config_name, request):
        ...
        has_autocommit = hasattr(raw_conn, 'autocommit') or \
                        hasattr(raw_conn, 'set_autocommit') or \
                        hasattr(raw_conn, 'get_autocommit')
...
class TestConnectionMetadata:
    ...
    @pytest.mark.integration
    @pytest.mark.parametrize("dialect,config_name,query", [
        ...
        (SQLDialect.SQLSERVER, "sqlserver_config", "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='BASE TABLE'"),
        ...
        with DatabaseConnection(dialect=dialect, **config) as conn:
            ...
            try:
                if dialect == SQLDialect.SQLITE:
                    ...
                    conn.execute_query(
                        f"CREATE TABLE `{project_id}.{dataset_id}.test_metadata` "
                ...
                assert len(columns) >= 2, f"Should have at least 2 columns (id, name) for {dialect.name}, got {len(columns)}"
