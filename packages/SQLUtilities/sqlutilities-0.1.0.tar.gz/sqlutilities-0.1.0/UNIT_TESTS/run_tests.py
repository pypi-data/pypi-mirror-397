...
class TestRunner:
    ...
    def run_dialect_tests(
        self, dialect: str, verbose: bool = False, coverage: bool = False
    ) -> int:
        """
    ...
    def run_specific_module(
        self, module: str, verbose: bool = False, coverage: bool = False
    ) -> int:
        """
...
Examples:
  ...
  python run_tests.py --check-containers
    ...
    parser.add_argument(
        "--module",
        choices=["core_enums", "core_types", "drivers", "validation", "connections", "transactions", "errors", "tables"],
    ...
    parser.add_argument(
        "--check-containers",
        action="store_true",
        help="Check status of database containers before running tests"
    )
    ...
    try:
        if args.module:
            # Run tests for specific module
            exit_code = runner.run_specific_module(
                args.module,
            ...
            # Run unit tests only
            exit_code = runner.run_unit_tests_only(
                verbose=args.verbose,
                coverage=args.coverage
            )
            ...
            exit_code = runner.run_integration_tests_only(
                verbose=args.verbose,
                coverage=args.coverage
            )
            ...
            exit_code = runner.run_dialect_tests(
                args.dialect,
                verbose=args.verbose,
                coverage=args.coverage
            )
