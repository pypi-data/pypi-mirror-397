...
from sqlutilities.download import read_sql, execute_query_with_metadata
    ...
    def test_basic_pandas_query(self, dialect, config_name, request):
        ...
        try:
            ...
            if dialect == SQLDialect.SQLITE:
                ...
                """
                insert_sql = f"INSERT INTO {table_name} (id, name, value) VALUES (%s, %s, %s)" if dialect != SQLDialect.SQLITE else f"INSERT INTO {table_name} (id, name, value) VALUES (?, ?, ?)"
                select_query = f"SELECT * FROM {table_name}"
    ...
    def test_basic_polars_query(self, dialect, config_name, request):
        ...
        try:
            ...
            if dialect == SQLDialect.SQLITE:
                ...
                conn.execute_query(f"BEGIN EXECUTE IMMEDIATE 'DROP TABLE \"{table_name}\"'; EXCEPTION WHEN OTHERS THEN NULL; END;")
                ...
                insert_sql = f"INSERT INTO {table_name} (id, name) VALUES (%s, %s)" if dialect != SQLDialect.SQLITE else f"INSERT INTO {table_name} (id, name) VALUES (?, ?)"
                select_query = f"SELECT * FROM {table_name} WHERE id <= 2"
