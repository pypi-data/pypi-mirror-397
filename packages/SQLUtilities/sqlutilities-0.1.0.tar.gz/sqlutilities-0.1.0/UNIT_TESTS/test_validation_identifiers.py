...
from sqlutilities.core import SQLDialect, DatabaseObjectType
    ...
    def test_get_reserved_words(self, all_dialects):
        ...
        for dialect in all_dialects:
            ...
            assert isinstance(reserved_words, set), \
                f"{dialect.name} reserved_words should be a set"
            assert len(reserved_words) > 0, \
                f"{dialect.name} should have reserved words"
    ...
    def test_common_reserved_words(self, all_dialects):
        ...
        for dialect in all_dialects:
            ...
            for keyword in common_keywords:
                assert keyword in reserved_words, \
                    f"{keyword} should be reserved in {dialect.name}"
    ...
    def test_get_dialect_objects(self, all_dialects):
        ...
        for dialect in all_dialects:
            ...
            assert isinstance(objects, set), \
                f"{dialect.name} objects should be a set"

            # Common objects should be supported
            assert DatabaseObjectType.TABLE in objects, \
                f"{dialect.name} should support TABLE"
            assert DatabaseObjectType.COLUMN in objects, \
                f"{dialect.name} should support COLUMN"
    ...
    def test_get_identifier_rules(self, all_dialects):
        ...
        for dialect in all_dialects:
            ...
            assert isinstance(rules, dict), \
                f"{dialect.name} identifier_rules should be a dict"

            # Check required keys
            required_keys = ['max_length', 'case_sensitive', 'allowed_chars']
            for key in required_keys:
                assert key in rules, \
                    f"{dialect.name} identifier_rules missing '{key}'"
...
class TestReservedWordChecking:
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("dialect,keyword", [
    ...
    def test_is_reserved_word_common_keywords(self, dialect, keyword):
        ...
        is_reserved = SQL_DIALECT_REGISTRY.is_reserved_word(
            dialect, DatabaseObjectType.TABLE, keyword
        )
        assert is_reserved, f"{keyword} should be reserved in {dialect.name}"

    @pytest.mark.unit
    @pytest.mark.parametrize("dialect,identifier", [
    ...
    def test_is_not_reserved_word_valid_identifiers(self, dialect, identifier):
        ...
        is_reserved = SQL_DIALECT_REGISTRY.is_reserved_word(
            dialect, DatabaseObjectType.TABLE, identifier
        )
        assert not is_reserved, \
            f"{identifier} should not be reserved in {dialect.name}"
    ...
    def test_case_insensitive_reserved_check(self):
        ...
        for variant in variants:
            is_reserved = SQL_DIALECT_REGISTRY.is_reserved_word(
                SQLDialect.MYSQL, DatabaseObjectType.TABLE, variant
            )
    ...
    def test_context_specific_exemptions(self):
        ...
        exemptions = SQL_DIALECT_REGISTRY.get(
            SQLDialect.SQLITE, 'context_exemptions'
        )

        assert isinstance(exemptions, dict), \
            "Context exemptions should be a dict"
...
class TestIdentifierValidation:
    ...
    def test_validate_valid_identifiers(self, dialect, identifier):
        ...
        result = SQL_DIALECT_REGISTRY.validate_identifier(
            dialect, identifier, DatabaseObjectType.TABLE
        )

        assert result['valid'], \
            f"{identifier} should be valid for {dialect.name}"
        assert result['original'] == identifier, \
            "Original identifier should be preserved"
    ...
    def test_validate_problematic_identifiers(self, dialect, identifier):
        ...
        assert 'correction_applied' in result, \
            "Result should have 'correction_applied' key"
    ...
    def test_encapsulation_correction(self, dialect):
        ...
        result = SQL_DIALECT_REGISTRY.validate_identifier(
            dialect, "SELECT", DatabaseObjectType.TABLE,
            correction_method='encapsulate'
        )

        # Should encapsulate the identifier
        assert result['correction_applied'] == 'encapsulated', \
            f"Should encapsulate reserved word for {dialect.name}"

        # Final identifier should be quoted
        quote_char = dialect.quote_character
        assert result['final'].startswith(quote_char) and \
               result['final'].endswith(quote_char), \
    ...
    def test_normalize_correction(self):
        ...
        result = SQL_DIALECT_REGISTRY.validate_identifier(
            SQLDialect.MYSQL, "123invalid", DatabaseObjectType.TABLE,
            correction_method='normalize',
            default_prefix_correction='tbl_'
        )

        # Should add prefix to fix invalid start
        assert result['final'].startswith('tbl_'), \
            "Should add prefix to fix invalid identifier"

    @pytest.mark.unit
    def test_validation_result_structure(self):
        """Test that validation result has expected structure."""
        result = SQL_DIALECT_REGISTRY.validate_identifier(
            SQLDialect.MYSQL, "test_table", DatabaseObjectType.TABLE
        )
    ...
    def test_max_length_enforcement(self, dialect):
        ...
        if max_length:
            ...
            result = SQL_DIALECT_REGISTRY.validate_identifier(
                dialect, long_identifier, DatabaseObjectType.TABLE
            )

            # Should truncate to max length
            assert len(result['final']) <= max_length, \
    ...
    def test_empty_identifier_handling(self):
        ...
        with pytest.raises(AssertionError):
            SQL_DIALECT_REGISTRY.validate_identifier(
                SQLDialect.MYSQL, "", DatabaseObjectType.TABLE
            )
    ...
    def test_whitespace_stripping(self):
        ...
        assert not result['final'].startswith(' '), \
            "Leading whitespace should be stripped"
        assert not result['final'].endswith(' '), \
...
class TestDialectSpecificBehavior:
    ...
    def test_mysql_backtick_encapsulation(self):
        ...
        result = SQL_DIALECT_REGISTRY.validate_identifier(
            SQLDialect.MYSQL, "SELECT", DatabaseObjectType.TABLE,
            correction_method='encapsulate'
        )

        assert result['final'].startswith('`') and result['final'].endswith('`'), \
    ...
    def test_postgres_double_quote_encapsulation(self):
        ...
        result = SQL_DIALECT_REGISTRY.validate_identifier(
            SQLDialect.POSTGRES, "SELECT", DatabaseObjectType.TABLE,
            correction_method='encapsulate'
        )
    ...
    def test_oracle_case_conversion(self):
        ...
        if rules.get('case_conversion') == 'upper':
            result = SQL_DIALECT_REGISTRY.validate_identifier(
                SQLDialect.ORACLE, "table_name", DatabaseObjectType.TABLE,
                correction_method='normalize'
            )

            assert result['final'].isupper(), \
                "Oracle should convert identifiers to uppercase"
...
class TestDataTypeRetrieval:
    ...
    def test_retrieve_datatypes_all_dialects(self, all_dialects):
        ...
        for dialect in all_dialects:
            ...
            assert datatypes is not None, \
                f"Should retrieve datatypes for {dialect.name}"
            assert isinstance(datatypes, dict), \
                f"Datatypes should be a dict for {dialect.name}"
    ...
    def test_retrieve_datatypes_structure(self):
        ...
        for category in expected_categories:
            ...
            assert isinstance(datatypes[category], dict), \
                f"Category {category} should be a dict"
    ...
    def test_retrieve_datatypes_numeric_subcategories(self):
        ...
        for subcat in expected_subcategories:
            assert subcat in numeric_types, \
                f"Numeric types missing subcategory: {subcat}"
    ...
    def test_retrieve_datatypes_character_subcategories(self):
        ...
        for subcat in expected_subcategories:
            assert subcat in character_types, \
                f"Character types missing subcategory: {subcat}"
