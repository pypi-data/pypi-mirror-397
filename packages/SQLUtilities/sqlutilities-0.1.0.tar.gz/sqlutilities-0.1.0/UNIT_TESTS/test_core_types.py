...
import pytest
from sqlutilities.core import COLUMNDTYPE, Column_Type, TemporalPrecision, SQLDialect
...
class TestCOLUMNDTYPE:
    ...
    def test_common_data_types_exist(self):
        ...
        expected_types = [
            "INTEGER", "BIGINT", "SMALLINT", "VARCHAR", "CHAR",
            "TEXT", "DECIMAL", "NUMERIC", "FLOAT", "DOUBLE",
    ...
    def test_data_type_has_metadata(self):
        ...
        required_keys = [
            "category", "description", "min_bytes", "max_bytes",
            "supported_dialects"
        ]
    ...
    def test_data_type_categories(self):
        ...
        valid_categories = [
            "numeric_integer", "numeric_decimal", "numeric_float", "numeric_money",
            "text_fixed", "text_variable", "text_large",
            "date", "time", "datetime", "interval",
            "binary", "binary_large", "boolean",
            "json", "xml", "uuid", "geometric", "network",
            "range", "array", "structured", "semi_structured", "system"
        ]

        for dtype in COLUMNDTYPE:
            category = dtype.value[1].get("category")
            assert category in valid_categories, \
                f"{dtype.name} has invalid category: {category}"
    ...
    def test_numeric_types_have_range(self):
        ...
        numeric_int_types = [
            COLUMNDTYPE.TINYINT, COLUMNDTYPE.SMALLINT,
            COLUMNDTYPE.INTEGER, COLUMNDTYPE.BIGINT
        ]
    ...
    def test_data_type_supported_dialects(self):
        ...
        for dtype in COLUMNDTYPE:
            ...
            assert isinstance(supported_dialects, list), \
                f"{dtype.name} supported_dialects should be a list"

            # Check that each supported dialect is valid
            for dialect in supported_dialects:
                assert isinstance(dialect, SQLDialect), \
                    f"{dtype.name} has non-SQLDialect in supported_dialects"
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("dtype,expected_optimal", [
    ...
    def test_dialect_overrides_structure(self):
        ...
        for dtype in COLUMNDTYPE:
            ...
            assert isinstance(overrides, dict), \
                f"{dtype.name} dialect_overrides should be a dict"

            for dialect, override_value in overrides.items():
                assert isinstance(dialect, SQLDialect), \
                    f"{dtype.name} has invalid dialect key in overrides"

                # Override value should be a tuple (type_name, size_spec)
                assert isinstance(override_value, tuple), \
                    f"{dtype.name} override for {dialect.name} should be a tuple"
    ...
    def test_string_types_have_length_support(self):
        ...
        for dtype in string_types:
            ...
            assert metadata.get("max_precision") is not None or \
                   metadata.get("max_bytes") is not None, \
    ...
    def test_decimal_types_have_precision_scale(self):
        ...
        for dtype in decimal_types:
            metadata = dtype.value[1]
            assert "max_precision" in metadata, \
                f"{dtype.name} should have max_precision"

    @pytest.mark.unit
    def test_temporal_types_timezone_support(self):
        """Test that temporal types specify timezone support."""
        temporal_types = [
            COLUMNDTYPE.TIMESTAMP, COLUMNDTYPE.TIMESTAMPTZ,
            COLUMNDTYPE.TIME, COLUMNDTYPE.TIMETZ
        ]

        for dtype in temporal_types:
            if hasattr(COLUMNDTYPE, dtype.name):
                metadata = dtype.value[1]
                assert "time_zone_support" in metadata, \
                    f"{dtype.name} should specify time_zone_support"
...
class TestColumnType:
    ...
    def test_column_type_has_required_fields(self):
        ...
        for field in expected_fields:
            assert field in Column_Type._fields, \
                f"Column_Type missing field '{field}'"
...
class TestDataTypeConversion:
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("from_dialect,to_dialect", [
    ...
    def test_data_type_dialect_conversion(self, from_dialect, to_dialect):
        ...
        for dtype in test_types:
            ...
            if from_dialect in metadata.get("supported_dialects", []):
                ...
                assert is_supported or has_override or True, \
    ...
    def test_sqlite_compatibility(self):
        ...
        common_types = [
            COLUMNDTYPE.INTEGER, COLUMNDTYPE.VARCHAR,
            COLUMNDTYPE.DECIMAL, COLUMNDTYPE.TIMESTAMP
        ]
