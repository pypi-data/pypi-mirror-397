...
- **Automatic Retries**: All underlying Google API calls are automatically retried with exponential backoff, making your authentication flows resilient to transient network errors and rate limits.
...
Here are the different ways you can use `simple-gcp-auth` to get credentials. All authentication flows include automatic retries for transient network issues, so you don't need to build custom retry logic.
...
### 7. Using the Retry Decorator

For advanced use cases, the underlying `tenacity` retry decorator is exposed as `google_api_retry`. You can use it to add the same resilience to your own functions that make calls to Google APIs.

```python
from simple_gcp_auth import google_api_retry, from_interactive_user
from google.cloud import storage
from googleapiclient.errors import HttpError

# Get credentials using one of the helper functions
credentials = from_interactive_user(
...
@google_api_retry
def upload_file_with_retry(bucket_name, file_path, blob_name):
    """
    This function will automatically retry on transient errors
    like 429s or 503s from the Google Cloud Storage API.
    """
    try:
        bucket = storage_client.get_bucket(bucket_name)
        blob = bucket.blob(blob_name)
        blob.upload_from_filename(file_path)
        print(f"Successfully uploaded {file_path} to {bucket_name}/{blob_name}")
    except HttpError as e:
        print(f"An HTTP error occurred: {e}")
        raise # Re-raise the exception to allow the decorator to handle it

# Example usage
# upload_file_with_retry('my-bucket', 'path/to/my/file.txt', 'remote-file-name.txt')
