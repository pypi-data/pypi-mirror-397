...
class TestChatCommands(unittest.TestCase):
    ...
    @patch('prompt_toolkit.PromptSession.prompt')
    def test_chat_with_agent_success(self, mock_prompt, mock_agent_session, mock_agent_client):
        mock_prompt.side_effect = ["Hello", "exit"]
    ...
    @patch('json.load')
    def test_chat_with_agent_restore_session_success(self, mock_json_load, mock_open, mock_prompt, mock_agent_session, mock_agent_client):
        mock_prompt.side_effect = ["Hello", "exit"]
        mock_session_instance = mock_agent_session.return_value
        mock_session_instance.get_answer.return_value = "Hello, I'm your agent!"
        mock_session_instance.stream_answer.return_value = iter(["Hi there!"])
    ...
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.dump')
    def test_chat_with_agent_save_session_success(self, mock_json_dump, mock_open, mock_prompt, mock_agent_session, mock_agent_client):
        mock_prompt.side_effect = ["Hello", "exit"]
        mock_session_instance = mock_agent_session.return_value
        mock_session_instance.get_answer.return_value = "Hello, I'm your agent!"
        mock_session_instance.stream_answer.return_value = iter(["Hi there!"])
