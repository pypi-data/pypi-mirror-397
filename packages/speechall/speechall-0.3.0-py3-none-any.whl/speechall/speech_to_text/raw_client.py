...
class RawSpeechToTextClient:
    ...
    def transcribe_remote(
        ...
        *,
        ...
        _response = self._client_wrapper.httpx_client.request(
            ...
            json={
                "file_url": file_url,
                "replacement_ruleset": convert_and_respect_annotation_metadata(
                    object_=replacement_ruleset, annotation=typing.Sequence[ReplacementRule], direction="write"
                ),
                "model": model,
                "language": language,
                "output_format": output_format,
    ...
    ) -> HttpResponse[typing.List[SpeechToTextModel]]:
        """
        ...
        try:
            if 200 <= _response.status_code < 300:
                _data = typing.cast(
                    ...
                    parse_obj_as(
                        type_=typing.List[SpeechToTextModel],  # type: ignore
                        object_=_response.json(),
                    ),
                )
                return HttpResponse(response=_response, data=_data)
...
class AsyncRawSpeechToTextClient:
    ...
    async def list_speech_to_text_models(
        self, *, request_options: typing.Optional[RequestOptions] = None
    ) -> AsyncHttpResponse[typing.List[SpeechToTextModel]]:
        """
        Returns a detailed list of all STT models accessible through the Speechall API.
        Each model entry includes its identifier (`provider.model`), display name, description,
        supported features (languages, formats, punctuation, diarization), and performance characteristics.
