...
from rsxml.util import pretty_duration, sizeof_fmt
    ...
    def output(self):
        ...
        if NO_UI:
            if first_time or elapsed_time > self.timer:
                ...
                if self.byte_format:
                    writestr = (
                        f"        PROGRESS: {sizeof_fmt(self.progress)} / {sizeof_fmt(self.total)}    "
                        f"{self.text}     (elapsed: {duration})\n"
                    )
                else:
                    pct_done = int(100 * (self.progress / self.total))
                    writestr = (
                        f"        PROGRESS: {self.progress:,} / {self.total:,}  ({pct_done}%)  " f"{self.text}     (elapsed: {duration})\n"
                    )
        ...
        if first_time or elapsed_time > self.timer:
            ...
            if self.byte_format:
                writestr = (
                    f"\r[{linebar}{spacepad}]  {sizeof_fmt(self.progress)} / {sizeof_fmt(self.total)}  "
                    f"{self.text} (elapsed: {duration})     \n"
                )
            else:
                writestr = f"\r[{linebar}{spacepad}]  {self.progress:,} / {self.total:,}  " f"{self.text} (elapsed: {duration})     \n"
