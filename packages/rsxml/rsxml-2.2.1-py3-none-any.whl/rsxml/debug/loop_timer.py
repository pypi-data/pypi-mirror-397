...
class LoopTimer:
    ...
    def __init__(self, name, logger=None, use_ms=False, interval=500):
        self.use_ms = use_ms
        self.logger = logger or Logger("TIMER")
    ...
    def progprint(self, mid_str=None):
        ...
        if self.ticks > 0 and since_last > self.interval:
            ...
            self.lastupdate = time.time()
            writestr = (
                f"\rAVG_TIMER::{self.name}{middle_str}:: Count: {self.ticks:,}, "
                f"Total Time: {self.total:f}s, Average: {avg:f}{unit}     \n"
            )
