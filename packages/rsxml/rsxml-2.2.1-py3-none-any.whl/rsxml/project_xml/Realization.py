...
class Realization(RSObj):
    ...
    def __ds_eq__(self, first_list: List[Dataset | RefDataset], other_list: List[Dataset | RefDataset]) -> bool:
        ...
        if first_list is None or other_list is None or len(first_list) != len(other_list):
            return False
        return all(a == b for a, b in zip(first_list, other_list, strict=False))
    ...
    def __eq__(self, other: RSObj) -> bool:
        ...
        return (
            ...
            and self.__ds_eq__(self.datasets, other.datasets)
            and self.__ds_eq__(self.logs, other.logs)
            and self.__ds_eq__(self.inputs, other.inputs)
            and self.__ds_eq__(self.intermediates, other.intermediates)
            and self.__ds_eq__(self.outputs, other.outputs)
            and self.__ds_eq__(self.analyses, other.analyses)
        )
    ...
    def from_xml(xml_node: ET.Element, common_datasets: List[Dataset] | None = None) -> Realization:
        ...
        try:
            ...
            log.error(f'dateCreated is not a valid ISO 8601 date like "{datetime.now().isoformat()}". ' f'Got: "{date_created_node}"')
            raise err
