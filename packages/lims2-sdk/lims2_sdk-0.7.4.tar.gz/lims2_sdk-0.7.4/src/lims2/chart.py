...
class ChartService(OSSMixin):
    ...
    def _set_file_info(self, request_data: dict, file_name: Optional[str]) -> None:
        ...
        如果用户未传入 file_name，则根据图表属性生成唯一文件名：
        - 基础名称：chart_name
        - 如果存在 contrast/sample_id/chart_type，追加8位哈希后缀
        """
        if file_name:
            request_data["file_name"] = file_name
        elif "file_name" not in request_data:
            request_data["file_name"] = self._generate_unique_filename(request_data)
        request_data["file_format"] = "json"

    def _generate_unique_filename(self, request_data: dict) -> str:
        """根据图表属性生成唯一文件名

        将 contrast、sample_id、chart_type 组合计算哈希，追加到 chart_name 后

        Args:
            request_data: 图表请求数据

        Returns:
            str: 唯一文件名，格式为 "{chart_name}_{hash8}" 或 "{chart_name}"
        """
        chart_name = request_data["chart_name"]
        ...
        return f"{chart_name}_{hash_suffix}"
