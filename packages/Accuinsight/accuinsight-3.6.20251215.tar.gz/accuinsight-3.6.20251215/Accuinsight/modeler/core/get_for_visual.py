...
def y_pred_reshape(X_test, model=None):
...
def y_score_proba(X_test, y_test, model=None):
    y_score = None
    if model is not None:
...
def roc_pr_curve(X_test, y_test, f1_average, recall_average, model=None, *args):
    ...
    if model is not None:
        ...
        y_score = y_score_proba(X_test, y_test, model)

        # y_pred (dim=2/dim=1, 두 가지 경우 존재)
        y_pred = y_pred_reshape(X_test, model)
    ...
    else: # y, y_hat 만 입력할 때
        y_score = y_score_proba(X_test, y_test)
        y_pred = y_pred_reshape(X_test)
...
def get_visual_info_regressor(X_test, model=None):
    if model is not None:
