...
int main(int argc, char **argv)
{
	...
	printf("num models %d\n", argc -2);
	double ***gamma = (double***) malloc((argc - 2)*sizeof(double**));
	for (i = 0; i < (argc - 2); i++){
		gamma[i] = (double**) malloc(num_burst*sizeof(double*));
		for (j = 0; j < num_burst; j++){
			gamma[i][j] = (double*) malloc(models[i].nstate*len_bursts[j]*sizeof(double));
		}
	}
	...
	/*for (i = 0; i < (argc - 2); i++){
		...
		for (j = 0; j < num_burst; j++){
			printf("[");
			for (k = 0; k < len_bursts[j]*models[i].nstate; k++){
				printf("%f, ", gamma[i][j][k]);
				gamma[i][j][k] = 0.0;
			}
		}
	}*/
	printf("Done calc_multi_gamma\n");
	for (i =0; i < (argc -2); i++)
	{
		for (j = 0; j < num_burst; j++)
		{
