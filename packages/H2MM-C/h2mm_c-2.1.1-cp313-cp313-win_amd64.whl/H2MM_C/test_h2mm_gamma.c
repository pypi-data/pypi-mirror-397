...
int main(int argc, char **argv)
{
	...
	h2mm_minmax *minmaxlimit = (h2mm_minmax*) calloc(1,sizeof(h2mm_minmax));
	for (k = 0; k < argc - 2; k++){
		out_mod = allocate_models(1, models[k].nstate, models[k].ndet, models[k].nphot);
		gamma = (double**) malloc(num_burst*sizeof(double*));
		for (i = 0; i < num_burst; i++){
			gamma[i] = (double*) malloc(models[k].nstate*len_bursts[i]*sizeof(double));
		}
		printf("Begin h2mm_optimize %d\n", argc -2); 
		h2mm_optimize_gamma(num_burst, len_bursts, times, detectors, &models[k], out_mod, &gamma, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		printf("Done h2mm_optimize\n");
		print_model(out_mod);
		for (i = 0; i < num_burst; i++){
			printf("[");
			for (j = 0; j < len_bursts[i]*out_mod->nstate; j++){
				printf("%f, ", gamma[i][j]);
				gamma[i][j] = 0.0;
			}
			printf("]\n");
		}
		for(i=0; i < out_mod->nstate; i++){
			out_mod->prior[i] = 0.0;
			for(j=0; j < out_mod->nstate; j++){
				out_mod->trans[i*out_mod->nstate+j] = 0.0;
			}
			for(j=0; j < out_mod->ndet; j++){
				out_mod->obs[j*out_mod->nstate+i] = 0.0;
			}
		}
		for (i=0; i < num_burst; i++)
		{
			for (j = 0; j < len_bursts[i]; j++){
				gamma[i][j] = 0.0;
			}
			free(gamma[i]);
		}
		free(gamma);
		free_models(1, out_mod);
