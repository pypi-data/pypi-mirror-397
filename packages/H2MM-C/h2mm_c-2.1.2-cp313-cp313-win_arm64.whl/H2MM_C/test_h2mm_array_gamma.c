// File: h2mm_array_gamma_interface.c
...
int main(int argc, char **argv)
{
	...
	for (k = 0; k < argc - 2; k++)
	{
		...
		h2mm_optimize_gamma_array(num_burst, len_bursts, times, detectors, &models[k], &out_mod, &gamma, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		printf("Done h2mm_optimize\n");
		for (i = 0; i < limits->max_iter+2; i++){
			if (out_mod[i].conv & CONVCODE_OUTPUT){
				print_model(&out_mod[i]);
			}
		}
		for (i = 0; i < num_burst; i++){
			printf("[");
			for (j = 0; j < len_bursts[i]*models[k].nstate; j ++){
				printf("%f, ", gamma[i][j]);
				gamma[i][j] = 0.0;
			}
			printf("]\n");
