...
int main(int argc, char **argv)
{
	...
	ph_path* path_arr = NULL;
	for (k = 0; k < argc - 2; k++)
	{
		path_arr = allocate_paths(num_burst, len_bursts, models[k].nstate);
		printf("Begin viterbi %d\n", argc -2); 
		viterbi(num_burst, len_bursts, times, detectors, &models[k], path_arr, limits->num_cores);
		printf("Done viterbi\n");
		for (i = 0; i < num_burst; i++)
		{
			for (j=0; j < path_arr[i].nphot; j++)
			{
				path_arr[i].path[j] = 0;
				path_arr[i].scale[j] = 0;
			}
		}
		free_paths(num_burst, path_arr);
	}
	for(i=0; i < num_burst; i++)
	{
		free(times[i]);
