...
@pytest.mark.parametrize(
    "in_features, kernel_size", [((1, 1, 2), 1), ((4, 2, 6), (2, 1, 2))]
)
@pytest.mark.parametrize("cov", [None, bnn.params.FactorizedCovariance()])
def test_generalizes_pytorch_conv3d_layer(
    batch_shape,
    in_channels,
    out_channels,
    in_features,
    ...
    """Test whether the BNN Conv3d layer generalizes the PyTorch Conv3d layer."""
...
@pytest.mark.parametrize("in_features", [(2, 4, 4), (1, 5, 1)])
@pytest.mark.parametrize(
    "conv3d",
    [
        bnn.Conv3d(in_channels=3, out_channels=1, kernel_size=(1, 2, 1)),
        bnn.Conv3d(in_channels=1, out_channels=2, bias=False, kernel_size=1),
    ],
)
def test_shape(sample_shape, batch_shape, in_features, conv3d):
    """Test whether the output shape is correct."""
    generator = torch.Generator().manual_seed(0)
    input = torch.randn(
        *batch_shape, conv3d.in_channels, *in_features, generator=generator
    )
    output = conv3d(input, sample_shape=sample_shape, generator=generator)
...
def test_forward_is_deterministic_given_generator(seed):
    ...
    conv_layer = bnn.Conv3d(5, 3, kernel_size=2)

    input = torch.randn(
        5, 9, 2, 4, generator=torch.Generator().manual_seed(seed + 2452345)
    )
...
@pytest.mark.parametrize(
    "conv_layer_to_load",
    [
        bnn.Conv3d(3, 2, kernel_size=(1, 2, 2)),
        bnn.Conv3d(3, 2, kernel_size=(1, 2, 2), cov=params.FactorizedCovariance()),
        nn.Conv3d(3, 2, kernel_size=2),
    ],
)
...
def test_register_forward_hook():
    ...
    conv_layer = bnn.Conv3d(3, 2, kernel_size=1)
    ...
    conv_layer(torch.randn(5, 3, 9, 4, 4, generator=generator))
...
def test_if_bias_is_none_cov_bias_is_none(cov):
    ...
    linear_layer = bnn.Conv3d(5, 3, kernel_size=5, bias=False, cov=cov)
...
@pytest.mark.parametrize(
    "layer_width,cov",
    [
        (10, params.FactorizedCovariance()),
