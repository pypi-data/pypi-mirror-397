...
def get_ELMER_MPI_THREADS (settings):
    ...
    if ELMER_MPI_THREADS is not None:
        ...
        ELMER_MPI_THREADS = os.environ.get("ELMER_MPI_THREADS")
        if ELMER_MPI_THREADS is not None:
            print(f'MPI multithreading specified using environment variable ELMER_MPI_THREADS={ELMER_MPI_THREADS}')
        else:    
            print(f'MPI multithreading disabled: environment variable ELMER_MPI_THREADS not set, settings["ELMER_MPI_THREADS"] not set in model code')
            ELMER_MPI_THREADS = 1
