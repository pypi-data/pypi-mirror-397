"""
Template tags to create smart query strings.
...
def query_string(parser, token):
    ...
    .. code-block:: html
       ...
       {% query_string  '?tag=a&m=1&m=3&tag=b' tag+'c' m=2 tag-'b' as myqs %}
...
class QueryStringNode(template.Node):
    ...
    def __init__(self, qdict, mods, asvar):
        """Initialise QueryStringNode."""
        self.qdict = qdict
        self.mods = mods
        self.asvar = asvar

    def render(self, context):
        """Render QueryStringNode."""
    ...
    @staticmethod
    def _get_initial_query_dict(qdict):  # noqa: CCR001
        """Get the initial query dict."""
        if not qdict:
