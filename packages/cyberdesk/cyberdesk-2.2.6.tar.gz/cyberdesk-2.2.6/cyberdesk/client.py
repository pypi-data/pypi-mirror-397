...
def _calculate_retry_delay(attempt: int, retry_after_ms: Optional[int] = None) -> float:
    """
    ...
    if retry_after_ms and retry_after_ms > 0:
        ...
        return min(retry_after_ms, MAX_RETRY_DELAY_MS) / 1000.0
    
    # Full jitter exponential backoff
    exponential_delay = INITIAL_RETRY_DELAY_MS * (2 ** attempt)
    max_delay = min(exponential_delay, MAX_RETRY_DELAY_MS)
    return random.random() * max_delay / 1000.0
...
class RetryTransport(httpx.BaseTransport):
    """
    Custom httpx transport with automatic retry logic.
    ...
    def handle_request(self, request: httpx.Request) -> httpx.Response:
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                delay = _calculate_retry_delay(attempt, retry_after_ms)
        ...
        raise RuntimeError("Unexpected state in retry transport")
...
class AsyncRetryTransport(httpx.AsyncBaseTransport):
    """
    Async version of RetryTransport for httpx.AsyncClient.
    """
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        if last_exception is not None:
            raise last_exception
        raise RuntimeError("Unexpected state in async retry transport")
...
class CyberdeskClient:
    """Main Cyberdesk SDK client with automatic retry logic."""
    ...
    def __init__(
        ...
        Example:
            ...
            client = CyberdeskClient('your-api-key', max_retries=3)
            
            # Disable retries
            client = CyberdeskClient('your-api-key', max_retries=0)
        ...
        # Create retry-enabled httpx transports
        if max_retries > 0:
        ...
        # Create httpx clients with retry transports
        ...
        # Override the internal httpx clients with our retry-enabled versions
        self._client.set_httpx_client(sync_client)
        self._client.set_async_httpx_client(async_client)
