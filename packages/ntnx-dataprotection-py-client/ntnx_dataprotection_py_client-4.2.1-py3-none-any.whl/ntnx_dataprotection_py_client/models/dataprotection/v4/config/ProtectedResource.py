...
class ProtectedResource(ExternalizableAbstractModel):
    ...
    :param site_protection_info: (:attr:`site_protection_info`) The data protection details of the protected resource that are relevant to any of the sites in the local Prism Central, like the time ranges available for recovery.
    ...
    :param category_fq_names: (:attr:`category_fq_names`) Category key-value pairs associated with the protected resource at the time of protection. The category key and value are separated by '/'. For example, a category with key 'dept' and value 'hr' is represented as 'dept/hr'.
