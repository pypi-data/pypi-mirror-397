# Python Client For Nutanix Data Protection APIs

The Python client for Nutanix Data Protection APIs is designed for Python client application developers offering them simple and flexible access to APIs that business Continuity with full spectrum of Disaster Recovery and Backup solution. Spanning across Single PC, Cross AZ, MultiSite. Configuration of Recovery points, Protection policies, Recovery Plans. Execution and monitoring of back up and recovery orchestrations on OnPrem as well as Cloud.
## Features
...
api_client = ntnx_dataprotection_py_client.ApiClient(allow_version_negotiation=False)
```
...
```sh
<my-env>/bin/pip install ntnx-dataprotection-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-dataprotection-py-client
```
...
## Configuration
The python client for Nutanix Data Protection APIs can be configured with the following parameters
...
# Get details of a consistency group
try:
...
```python
# Initialize the API
data_protection_cluster_capabilities_api_instance = DataProtectionClusterCapabilitiesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get data protection cluster capabilities
try:
    api_response = data_protection_cluster_capabilities_api_instance.list_data_protection_cluster_capabilities(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=dataprotection&version=v4.2&language=python). This documentation is auto-generated, and the location may change.
