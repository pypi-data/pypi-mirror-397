...
class SecsHandler:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def send_and_waitfor_response(self, *args, **kwargs):
        ...
        if display_log_func := getattr(self, "display_equipment_log", None):
            display_log_func("发送", str(args[0]))
    ...
    def _handle_stream_function(self, message):
        ...
        try:
            ...
            if display_log_func := getattr(self, "display_equipment_log", None):
                display_log_func("回复", str(result))
            if result is not None:
                self.send_response(result, message.header.system)
            if sf_callback_func := getattr(self, f"{sf_callback_index}_callback", None):
                sf_callback_func(message, result)
