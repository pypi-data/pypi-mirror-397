...
from subquadratic_ops_torch.utils import get_module, next_pow2, get_cuda_arch
...
def _(x: Tensor, weight: Tensor) -> Tensor:
    if x.dim() != 3 or weight.dim() != 2 or x.shape[-1] % weight.shape[-1] != 0:
        raise ValueError(
            f"Input and weights must be 3D and 2D tensors, {x.shape}, {weight.shape}, and x % weight should be 0, but got {x.shape[-1] % weight.shape[-1]}"
        )
...
def _(grad_out: Tensor, x: Tensor, weight: Tensor) -> List[Tensor]:
    ...
    module(grad_x.detach(), grad_weight.detach(), grad_out_y.detach(),
           x.detach(), weight.detach(), arch, stream_id)
...
def fft_causal_conv1d_bwd(ctx, grad_out):
    ...
    dx, dw = torch.ops.subquadratic_ops_torch.fft_causal_conv1d_bwd_primitive(
        grad_out, x, weight)
    return dx, dw
...
def _(x: Tensor, weight: Tensor) -> List[Tensor]:
    ...
    inter_x1 = torch.empty(
        (batch_size, feat_dim, intermediate_size), device=x.device, dtype=inter_dtype)
    inter_filter = torch.empty(
        (feat_dim, intermediate_size), device=x.device, dtype=inter_dtype)

    inter_x0 = torch.empty(
        (batch_size, feat_dim, intermediate_size), device=x.device, dtype=inter_dtype)

    module = get_module("long_fft_causal_conv1d_fwd", [x.dtype])
    arch = get_cuda_arch(x.device.index)
    stream_id = torch.cuda.current_stream(x.device).stream_id
    y = torch.empty((batch_size, feat_dim, seq_length),
                    device=x.device, dtype=x.dtype)

    module(y.detach(), x.detach(), weight.detach(), inter_x0.detach(),
           inter_x1.detach(), inter_filter.detach(), arch, stream_id)
...
def _(x: Tensor, weight: Tensor) -> List[Tensor]:
    ...
    intermediate_size = 2 * (m//2 + 1) * n
    return [torch.empty_like(x), torch.empty((x.shape[0], x.shape[1], intermediate_size), device=x.device, dtype=dtype), torch.empty((x.shape[0], x.shape[1], intermediate_size), device=x.device, dtype=dtype), torch.empty((x.shape[1], intermediate_size), device=x.device, dtype=dtype)]
...
def _(grad_out: Tensor, x: Tensor, weight: Tensor, inter_x: Tensor, inter_filter: Tensor) -> List[Tensor]:
    ...
    grad_x = torch.empty((batch_size, feat_dim, seq_length),
                         device=x.device, dtype=x.dtype)
    grad_weight = torch.empty((feat_dim, seq_length),
                              device=x.device, dtype=x.dtype)
    ...
    inter_dx = torch.empty(
        (batch_size, feat_dim, intermediate_size), device=x.device, dtype=dtype)
    inter_dw = torch.empty((feat_dim, intermediate_size),
    ...
    module(grad_x.detach(), grad_weight.detach(), grad_out.detach(), inter_x.detach(
    ), inter_filter.detach(), inter_dx.detach(), inter_dw.detach(), arch, stream_id)
...
def long_fft_causal_conv1d_bwd(ctx, grad_out):
    ...
    dx, dw = torch.ops.subquadratic_ops_torch.long_fft_causal_conv1d_bwd_primitive(
        grad_out_y, x, weight, inter_x, inter_filter)
    return dx, dw
...
def fft_causal_conv1d(x: Tensor, weight: Tensor) -> Tensor:
    ...
    if short_fft_is_available(weight_length, x.dtype):
        ...
        if seq_length < filter_length or seq_length % filter_length != 0:
            ...
            seq_length_final = ((seq_length + filter_length - 1) //
                                filter_length) * filter_length
            x = F.pad(x, (0, seq_length_final - seq_length))
            diff = seq_length_final - seq_length
            seq_length = x.shape[-1]

        y = torch.ops.subquadratic_ops_torch.fft_causal_conv1d_fwd_primitive(
            x, weight)
        return y if not is_padding_needed else y[..., :-diff]
    else:

        filter_length = max(compatible_filter_length(
            weight_length), compatible_filter_length(x.shape[-1]))
        seq_length = x.shape[-1]
        ...
        y = torch.ops.subquadratic_ops_torch.long_fft_causal_conv1d_fwd_primitive(x, weight)[
            0]
        return y if not is_padding_needed else y[..., :-diff]
