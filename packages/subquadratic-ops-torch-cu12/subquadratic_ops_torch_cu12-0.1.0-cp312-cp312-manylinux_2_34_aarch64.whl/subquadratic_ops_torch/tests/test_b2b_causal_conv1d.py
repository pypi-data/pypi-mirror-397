...
def test_b2b_causal_conv1d(in_dim, seq_dim, proj_conv_width, mixer_conv_width, dtype):
    ...
    model_mixer = Conv1DModel(in_dim, mixer_conv_width,
                              dtype, skip_bias=True).cuda()
    batch_size = 2
    ...
    model_proj.conv.weight.data = weights_proj.reshape(
        in_dim*3, 1, proj_conv_width).to(dtype)

    model_mixer.weight.data = weights_mixer.to(dtype)
    model_mixer.conv.weight.data = weights_mixer.reshape(
        in_dim, 1, mixer_conv_width).to(dtype)
    model_mixer.skip_bias.data = skip_bias.to(dtype).reshape(1, -1, 1)
    y_predicted = b2b_causal_conv1d(x.to(dtype), weights_proj.to(
        dtype), weights_mixer.to(dtype), skip_bias.to(dtype))
...
def test_b2b_causal_conv1d_bwd(in_dim, seq_dim, proj_conv_width, mixer_conv_width, dtype):
    ...
    torch.autograd.gradcheck(b2b_causal_conv1d, (x.to(dtype), weights_proj.to(
        dtype), weights_mixer.to(dtype), skip_bias.to(dtype)))
