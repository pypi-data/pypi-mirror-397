...
def generate_implicit_filter_kernels(order: int, dtype: str, fast_math: bool = True):
        ...
        for id_l2 in range(4):
            ...
            if id_output < L:
                ...
                for id_o in range(order):
                    tmp += wp.exp(tile_glogp[id_o] *
                                  wpdtype(id_output)) * tile_r[id_o]
    ...
    def implicit_filter_bwd_kernel(
        ...
        for i in range(IMPLICIT_FILTER_L_SEG_SIZE):
            ...
            dh_reg = wp.where(id_l >= L, wpdtype(0), wp.exp(
                glogp_reg * wpdtype(id_l)) * dh[id_d, id_l])
            dR_reg += dh_reg
    ...
    fwd_kernel = wp.Kernel(implicit_filter_fwd_kernel, key=fwd_key,
                           module=fwd_module
                           )

    bwd_kernel = wp.Kernel(implicit_filter_bwd_kernel, key=bwd_key,
                           module=bwd_module
                           )
