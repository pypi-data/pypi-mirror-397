...
def test_fft_causal_conv1d(in_dim, seq_dim, conv_width, dtype):
    ...
    if seq_dim % conv_width != 0:
        ...
        x = F.pad(x, (0, padding))
    y_actual = fft_causal_conv1d_ref(
        x.to(dtype), weights.to(dtype))[..., :seq_dim]

    torch.testing.assert_close(y_predicted.to(
        dtype), y_actual.to(dtype), atol=1e-3, rtol=1e-3)
...
def test_fft_causal_conv1d_grad(in_dim, seq_dim, conv_width, dtype):
    ...
    dx_predicted, dw_predicted = torch.autograd.grad(
        y_predicted, (x, weights), dy)
    x_padded = x
    if seq_dim < conv_width:
...
def test_fft_causal_conv1d_bwd(in_dim, seq_dim, conv_width, dtype):
    ...
    torch.autograd.gradcheck(
        fft_causal_conv1d, (x, weights), eps=1e-1, atol=1e-3, rtol=1e-3)
