...
from subquadratic_ops_torch.utils import get_module, next_pow2
from subquadratic_ops_torch.utils import get_cuda_arch
...
def _(x: Tensor, weight: Tensor) -> List[Tensor]:
    ...
    if x.shape[-1] % 2 == 1:
        raise ValueError(
            f"Input sequence length must be even, got {x.shape[-1]}"
        )
    ...
    inter_w = torch.empty(weight.shape[0], scratch_space_size[0],
                          scratch_space_size[1], device=x.device, dtype=x.dtype)

    module(y.detach(), x.detach(), weight.detach(),
           inter_w.detach(), arch, stream_id)
...
def _(x: Tensor, weight: Tensor) -> List[Tensor]:
    ...
    return [torch.empty_like(x),
            torch.empty(weight.shape[0], scratch_space_size[0],
                        scratch_space_size[1], device=x.device, dtype=x.dtype)]
...
def _(grad_out: Tensor, x: Tensor, weight: Tensor, inter_w: Tensor) -> List[Tensor]:
    ...
    if weight.shape[-1] % 2 == 1:
        raise ValueError(
            f"Filter length must be even, got {weight.shape[-1]}"
        )
    ...
    module(grad_x.detach(), grad_weight.detach(), grad_out_y.detach(),
           x.detach(), inter_w.detach(), inter_dw.detach(), arch, stream_id)
...
def fft_conv2d_bwd(ctx, grad_out):
    ...
    grad_x, grad_weight = torch.ops.subquadratic_ops_torch.fft_conv2d_bwd_primitive(
        grad_out_y, x, weight, inter_w)
    return grad_x, grad_weight
