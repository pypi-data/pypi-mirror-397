...
def _(x: Tensor, weight_proj: Tensor, weight_mixer: Tensor, skip_bias: Tensor) -> List[Tensor]:
    if x.dim() != 3 or weight_proj.dim() != 2 or weight_mixer.dim() != 2:
        raise ValueError(
            f"Input and weights must be 3D and 2D tensors, {x.shape}, {weight_proj.shape}, {weight_mixer.shape}"
        )
    if x.shape[1] % 3 != 0 or x.shape[1] != weight_proj.shape[0] or x.shape[1] != 3 * weight_mixer.shape[0]:
        raise ValueError(
            f"Input width must be divisible by 3, and equal to the number of rows in weight_proj and 3 times the number of rows in weight_mixer, got {x.shape[1]}, {weight_proj.shape[0]}, {weight_mixer.shape[0]}")

    if skip_bias.dim() != 1 or skip_bias.shape[0] != x.shape[1] // 3:
        raise ValueError(
            f"Skip bias must be a 1D tensor with the same number of rows as the input, got {skip_bias.shape}")

    x = x.contiguous()
    weight_proj = weight_proj.contiguous()
    weight_mixer = weight_mixer.contiguous()
    skip_bias = skip_bias.contiguous()
    y = torch.empty((x.shape[0], x.shape[1]//3, x.shape[2]),
                    device=x.device, dtype=x.dtype)
    y_gated = torch.empty(
        (x.shape[0], x.shape[1]//3, x.shape[2]), device=x.device, dtype=x.dtype)

    module = get_module("b2b_causal_conv1d_fwd", [x.dtype])
    stream_id = torch.cuda.current_stream(x.device).stream_id
    module(y.detach(), y_gated.detach(), x.detach(), weight_proj.detach(),
           weight_mixer.detach(), skip_bias.detach(), stream_id)
...
def _(x: Tensor, weight_proj: Tensor, weight_mixer: Tensor, skip_bias: Tensor) -> List[torch.Tensor]:
    return [torch.empty((x.shape[0], x.shape[1]//3, x.shape[2]), device=x.device, dtype=x.dtype), torch.empty((x.shape[0], x.shape[1]//3, x.shape[2]), device=x.device, dtype=x.dtype)]
...
def _(grad_out: List[torch.Tensor], x: Tensor, weight_proj: Tensor, weight_mixer: Tensor, skip_bias: Tensor, y: Tensor) -> List[torch.Tensor]:
    if x.dim() != 3 or weight_proj.dim() != 2 or weight_mixer.dim() != 2:
        raise ValueError(
            f"Input and weights must be 3D and 2D tensors, {x.shape}, {weight_proj.shape}, {weight_mixer.shape}"
        )
    ...
    grad_weight_proj = torch.zeros(
        weight_proj.shape, device=x.device, dtype=torch.float32 if x.dtype.itemsize == 2 else x.dtype)
    grad_weight_mixer = torch.zeros(
        weight_mixer.shape, device=x.device, dtype=torch.float32 if x.dtype.itemsize == 2 else x.dtype)
    grad_skip_bias = torch.zeros(skip_bias.shape, device=x.device,
    ...
    module(grad_x.detach(), grad_weight_proj.detach(), grad_weight_mixer.detach(), grad_skip_bias.detach(), grad_out_y_gated.detach(
    ), y.detach(), x.detach(), weight_proj.detach(), weight_mixer.detach(), skip_bias.detach(), stream_id)

    return [grad_x, grad_weight_proj.to(x.dtype), grad_weight_mixer.to(x.dtype), grad_skip_bias.to(x.dtype)]


@torch.library.register_fake("subquadratic_ops_torch::b2b_causal_conv1d_bwd_primitive")
def _(grad_out: List[torch.Tensor], x: Tensor, weight_proj: Tensor, weight_mixer: Tensor, skip_bias: Tensor, y: Tensor) -> List[torch.Tensor]:
    return [torch.empty_like(x), torch.empty_like(weight_proj), torch.empty_like(weight_mixer), torch.empty_like(skip_bias)]
...
def b2b_causal_conv1d_bwd(ctx, grad_out):
    ...
    dx, dw_proj, dw_mixer, db = torch.ops.subquadratic_ops_torch.b2b_causal_conv1d_bwd_primitive(
        grad_out, x, weight_proj, weight_mixer, skip_bias, y)
...
def b2b_causal_conv1d(x: Tensor, weight_proj: Tensor, weight_mixer: Tensor, skip_bias: Tensor) -> Tensor:
    _, y_gated = torch.ops.subquadratic_ops_torch.b2b_causal_conv1d_fwd_primitive(
        x, weight_proj, weight_mixer, skip_bias)
    return y_gated
