...
// 超过这个阈值就走分块（避免小消息也走分块带来额外 overhead）
const LARGE_THRESHOLD_BYTES: usize = 64 * 1024 * 1024; // 64 MiB
// 单块大小（务必小于 codec max_frame_length，且留出 bincode/msg 开销余量）
const CHUNK_LEN_BYTES: usize = 8 * 1024 * 1024; // 8 MiB
...
type FramedStream =
futures_util::stream::SplitStream<Framed<TcpStream, LengthDelimitedCodec>>;
        ...
        let assigned_idx = match recv_msg_stream(&mut stream).await? {
            Msg::Hello { idx } => {
                if let Some(want) = idx {
                    ...
                    let mut i = 0usize;
                    while peers.contains_key(&i) {
                        i += 1;
                    }
                    drop(peers);
                    if i >= hub_accept.n {
                        bail!("all {} party slots filled", hub_accept.n);
                    }
                    i
                }
            }
        ...
        tokio::spawn(async move {
            loop {
                match recv_msg_stream(&mut stream_reader).await {
                    // small message party->(party or coordinator)
                    Ok(Msg::DataTo { to, tag, payload }) => match to {
                        Some(dest) => {
                    ...
                    // large message party->party (forward)
                    Ok(Msg::LargeBeginTo {
                           to,
                           tag,
                           id,
                        ...
                        if let Some(dest) = to {
                            ...
                            // party->coordinator large inbox：这版先不做
                        }
                    }
                    Ok(Msg::LargeChunkTo { to, id, seq, data }) => {
                        if let Some(dest) = to {
                            let peers = hub_reader.peers.read().await;
                            if let Some(tx) = peers.get(&dest) {
                                let _ = tx
                                    .send(Msg::LargeChunkFrom {
                                        from: assigned_idx,
                                        id,
                                        seq,
                                        data,
                    ...
                    Ok(Msg::LargeEndTo { to, id, n_chunks }) => {
                        if let Some(dest) = to {
                            let peers = hub_reader.peers.read().await;
                            if let Some(tx) = peers.get(&dest) {
                                let _ = tx
                                    .send(Msg::LargeEndFrom {
                                        from: assigned_idx,
                                        id,
                                        n_chunks,
                                    })
...
impl Coordinator {
    ...
    fn recv<'p>(&self, py: Python<'p>) -> PyResult<&'p PyAny> {
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            let inbox = guard
                .recv()
                .await
                .ok_or_else(|| PyRuntimeError::new_err("coordinator closed"))?;
            Python::with_gil(|py| {
    ...
    fn send<'p>(
        ...
        // pickle now (注意：这里仍会一次性生成大 bytes)
        let payload: Vec<u8> = {
            let pickle = py.import("pickle")?;
        ...
        // ✅ 关键修复：在进入 async 前生成 id（避免 ThreadRng 跨 await 导致 future !Send）
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            let tx = peers_guard
                .get(&to)
                .ok_or_else(|| PyValueError::new_err(format!("destination {} not connected", to)))?
                .clone();

            if payload.len() <= LARGE_THRESHOLD_BYTES {
                let _ = tx
                    .send(Msg::DataFrom {
                        from: usize::MAX,
                        tag,
            ...
            let chunk_len = CHUNK_LEN_BYTES as u32;
            ...
            let mut seq: u32 = 0;
            for chunk in payload.chunks(CHUNK_LEN_BYTES) {
                let _ = tx
                    .send(Msg::LargeChunkFrom {
                        from: usize::MAX,
                        id,
    ...
    fn broadcast<'p>(
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            if payload.len() <= LARGE_THRESHOLD_BYTES {
                for (idx, tx) in peers_guard.iter() {
                    let _ = tx
                        ...
                        .await;
                    if verbose {
                        println!("[hub] broadcast small -> {}", idx);
                    }
                }
                return Ok(());
            }
            ...
            for (idx, tx) in peers_guard.iter() {
                ...
                if verbose {
                    println!(
                        "[hub] broadcast large -> {} ({} bytes, {} chunks)",
                        idx, total_len, seq
                    );
                }
            }
...
struct Party {
    ...
    rx: Arc<Mutex<mpsc::Receiver<(usize, Option<String>, Vec<u8>)>>>, // from hub (already reassembled)
    idx: usize,
    verbose: bool,
}
...
impl Party {
    ...
    fn send<'p>(
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            // 仍保持原 small message 行为（不影响你现有用法）
            let _ = tx.send(Msg::DataTo { to, tag, payload }).await;
            if verbose {
                match to {
                    Some(t) => println!("[party] sent -> {}", t),
                    None => println!("[party] sent -> coordinator"),
                }
            }
            Ok(())
        })
    ...
    fn recv<'p>(&self, py: Python<'p>) -> PyResult<&'p PyAny> {
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            let mut guard = rx.lock().await;
            let (from, tag, payload) = guard
                .recv()
                .await
                .ok_or_else(|| PyRuntimeError::new_err("party connection closed"))?;
            Python::with_gil(|py| {
                let pickle = py.import("pickle")?;
...
fn start_coordinator(py: Python<'_>, bind: String, n: usize, verbose: bool) -> PyResult<&PyAny> {
    pyo3_asyncio::tokio::future_into_py(py, async move {
        ...
        let peers_shared: Arc<RwLock<HashMap<usize, PeerTx>>> =
        ...
        tokio::spawn(async move {
            if let Err(e) =
                serve_coordinator(bind_clone, n, verbose, ready_tx, in_tx, peers_shared).await
            {
...
fn connect_party(
    ...
    pyo3_asyncio::tokio::future_into_py(py, async move {
        ...
        // reader: now with reassembly for Large*
        tokio::spawn(async move {
            struct Asm {
                tag: Option<String>,
                total_len: u64,
            ...
            let mut map: HashMap<(usize, u64), Asm> = HashMap::new();
            ...
            loop {
                match recv_msg_stream(&mut reader).await {
                    ...
                    Ok(Msg::LargeChunkFrom { from, id, seq, data }) => {
                        if let Some(a) = map.get_mut(&(from, id)) {
                            // 最小实现：要求按序到达，否则丢弃该条消息
                            if seq != a.next_seq {
                                map.remove(&(from, id));
                                continue;
                            }
                    ...
                    Ok(Msg::LargeEndFrom { from, id, n_chunks }) => {
                        if let Some(a) = map.remove(&(from, id)) {
                            if a.received == n_chunks && a.buf.len() as u64 == a.total_len {
                                let _ = in_tx.send((from, a.tag, a.buf)).await;
                            } else {
                                // 校验失败：丢弃
                            }
                        }
