...
class JsonResultsManager:
    ...
    whose values are :any:`Validator` that handle the checking, loading and
    saving of specific types.
    """
    ...
    def save(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        Check the data with the validators, convert them into a dictionary
        serializable in Json, and save it to the Json file.
    ...
    def get(self) -> dict[str, dict[str, Any]] | None:
        ...
        Check the data, convert them into ``quanti_fret`` module types and,
        and returns it.

        Returns:
            dict[str, dict[str, Any]]: data loaded and converted.
        """
        self._check_json_file()
    ...
    def _check_json_file(self) -> None:
        """ Check is the json file has a good type, if it exists and if it is
        a proper file.
