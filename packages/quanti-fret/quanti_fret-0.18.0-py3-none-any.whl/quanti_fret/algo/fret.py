...
class FretCalculator:
    ...
    To run on Single mode, just call the method :meth:`single_run`.

    To run on Series mode, you need to:

    * First call :meth:`series_reset`.
    * Then set the parameters that will be used for the series by calling
      :meth:`series_params`.
    * Then for each triplet of the series, you nee to call :meth:`series_run`
    ...
    * Finally, you can perform analysis on the series by calling
    ...
    def series_params(
        ...
        Args:
            ...
            analysis_details (bool, optional): Weather or not to compute
                detailed data for analysis purposes. Default to ``False``.
            analysis_sampling (int, optional): Keep one value every
                ``analysis_sampling`` values on data used to generate analysis
                details. Default to ``100``.
    ...
    def series_run(
        ...
        * **Ew**: Fret Filtered.
        * **S**: Stochiometry.
        * **extras**: Optional extras analysis data. Dictionary containing:
        ...
        and sampled data will not be kept. In consequences, the extra
    ...
    def series_analysis(
        ...
        Analysis data in a dictionary whose keys are:

        * ``e_boxplot``: Boxplot of E each triplet.
        * ``s_boxplot``: Boxplot of S each triplet.
        * ``hist_2d``: 2D histogram of S vs E.
        * ``median_sampled``: An array containing the median values for
          DD/DA/AA/E/Ew/S for each triplets.
    ...
    def single_run(
        ...
        * Extras data: Optional extras analysis data. Dictionary containing:
        ...
        Args:
            triplet (Triplet): Triplet to compute the FRET efficiency on.
            alpha_bt (float): :math:`\\alpha^{BT}` value from calibration.
            delta_de (float): :math:`\\delta^{DE}` value from calibration.
            background (BackgroundEngine): background engine to use.
            beta_x (float)::math:`\\beta^{X}` value from calibration.
            gamma_m (float): :math:`\\gamma^{M}` value from calibration.
            target_S (float): Mean value of the gaussian distribution of the
                stochiometry.
            sigma_S (float): standard deviation of the gaussian distribution of
                the stochiometry.
