...
class PlotGenerator:
    """ Provide a set of methods to plot figures for the **QuanTI-FRET**
    ...
    def boxplot_seq_overview(
        ...
        Args:
            ...
            subtitle (str, optional): Subtitle to print on the figure. Default
                is ``''`` and no subtitle will be plot.
    ...
    def scatterplot(
        ...
        Args:
            ...
            legends (list[str], optional): Legend to print for each data on the
                Y axis. If empty, no legends will be printed. Default is
                ``[]``.
            title (str, optional): title of the figure. Default is ``''``.
            subtitle (str, optional): subtitle of the figure. Default is
                ``''``.
            integer_on_x (bool, optional): True to force integer values on X
                axis. Default is ``False``.
            integer_on_y (bool, optional): True to force integer values on Y
                axis. Default is ``False``.
    ...
    def scatterplot_signal_intensity(
        ...
        """ Function to ease the call of :meth:`scatterplot` to plot a 1d
        signal vs intensity scatter.
    ...
    def scatterplot_3d(
        ...
        Args:
            ...
            plane_x_y_factors (tuple[float, float] | None, optional):
                X and Y factors to use to plot the plane representing
                :math:`Z = aX + bY`.
            title (str, optional): title of the figure. Default is ``''``.
            subtitle (str, optional): subtitle of the figure. Default is
                ``''``.
    ...
    def hist2d_signal_intensity(
        ...
        * The 1D histogram for the Y data.
        ...
        Args:
            ...
            range (str | tuple[DataRange, DataRange], optional):
                ...
                * ``((xmin, xmax), (ymin, ymax))``: custom range.
                ...
                Default is ``((0, 100), (0, 100))``.
            percentile_range ((x_min, x_max), (y_min, y_max), optional):
                percentile in between the range will be set if
                `range` == 'percentile'. Default is ``((1., 99.), (1., 99.))``.
            normed_hist (bool, optional): Normalize or not the histogram.
                Default is ``False``
            title (str, optional): Title of the graph. Default is ``''``.
            subtitle (str, optional): Subtitle of the graph. Default is ``''``.
    ...
    def image_with_colorbar(
        ...
        Args:
            ...
            subtitle (str, optional): Subtitle of the plot. Default is ``''``.
            range (str | tuple[float, float], optional):
                Range of the colomap. It can be:

                * ``percentile``: Range is computed in between the two
                  percentile set in percentile_range.
                * ``minimaxi``: range is minimum to maximul values of the
                  image.
                * ``(vmin, vmax)``: use custom values.

                Default is ``'percentile'``.
            percentile_range (tuple[float, float], optional): Percentile in
                between the range will be set if ``range`` == ``'percentile'``.
                Default is ``(1, 99)``.
            mask (np.ndarray | None, optional): if not ``None``, and the range
                is a ``str``, it is the area used to compute the range. Default
                is ``None``.
            nticks (int, optional): Number of ticks to display. Default is
                ``10``.
    ...
    def _get_range_single(
        ...
        * ``(min, max)``: custom range.
        ...
        Args:
            ...
            range (DataRange): Range asked. Can be ``'minimaxi'``,
                ``'percentile'`` or a custom range.
            percentile_range (tuple[float, float], optional):
                Percentile in between the range will be set if
                ``range`` == ``'percentile'``. Default is ``(1., 99.)``.

        Raises:
            QtfException: The range asked is incorrect.

        Returns:
            tuple[float, float]: The range calculated ``(min, max)``.
        """
