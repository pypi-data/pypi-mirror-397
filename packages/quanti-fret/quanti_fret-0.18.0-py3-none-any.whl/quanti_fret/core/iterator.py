...
class SeriesIterator:
    """ Iterator utility that helps iterating over triplets in a series.

    It has two modes:

    * **Default**: Will iterate over all triplets of all the sequences.
    * **sample_sequences**: Will return one chosen triplet per sequence. For
      now it chooses the first triplet.
    ...
    .. code:: python
        ...
        sit = series.iterator(sample_sequence)
        for triplet in sit:
            print(f'Processing {sit.current} / {sit.size}')
            print(f'Current sequence ID: {sit.id[1]}')
            print(f'Triplet ID in sequence: {sit.id[2]}')
            # Do something with triplet

    Current id, sequence id ans triplet id start at 1 not 0.
    """
    ...
    def __iter__(self) -> Iterator[Triplet]:
        ...
        If ``sample_sequences`` is set to ``False``, will iterate on all the
        triplets of all the sequences. Otherwise will select the fist triplet
        of every sequences.
    ...
    @property
    def size(self) -> int:
        """ Number of triplets that the iterator will returns.
        """
    ...
    def current(self) -> int:
        """ The current index iterated.
        """
        return self._current_index + 1

    @property
    def id(self) -> tuple[int, int, int]:
        """ The id of the current triplet.

        The id returned is a tuple containing in order:

        * The current index in all the iteration process. Same index than
          :meth:`current`.
        * The index of the sequence.
        * The index of the triplet in the sequence.
        """
        return (
    ...
    def _get_sampled_index(self) -> int:
        """ Get the index of the triplet to extract from a sequence in sampled
        mode..
