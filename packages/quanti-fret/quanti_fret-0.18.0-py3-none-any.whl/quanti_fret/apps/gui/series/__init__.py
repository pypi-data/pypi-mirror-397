...
class SeriesWidget(QWidget):
    """ Handle the selection of the series for a given phase.

    This supports the selection of one or multiple series in the same widget.

    The widget is split in 3 parts:

    * A Layout containing a title widget for all the series (showing their
      name, number of sequences associated, and a button to open their
      settings).
    * A collapsible layout allowing the user to modify the regexes used to find
      the sequences inside a folder.
    * A separator line.
    * A layout containing the setting of the selected series (see
      :any:`TripletScannerWidget`).

    If the phase has only one series, the detail button will be hidden, and the
    series settings will be always shown.

    This detects, for a given phase, the series available in the
    :any:`QtfSeriesManager`, and creates one :any:`SingleSeriesManager` per
    series found.
    """
