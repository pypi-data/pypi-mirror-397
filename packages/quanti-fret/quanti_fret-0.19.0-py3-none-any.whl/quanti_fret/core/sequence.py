...
class TripletSequence:
    """ Sequence of triplets representing either an acquisition of a single
    triplet, or multiple frame that could be considered as a video.

    A Sequence is linked to a folder where all the triplets it contains should
    be present.

    During the **Calibration** phase, the ``Sequence`` can be enabled or
    disabled by the presence of the file ``.qtf_ignore.txt`` in its folder.
    """
    ...
    def as_numpy(self) -> np.ndarray:
        """ Loads an returns the sequence's triplets stacked in a numpy array
        under the shape ``(nb_triplets, 3, img_height, img_width)``.
        """
        return np.stack([t.as_numpy for t in self._triplets], axis=0)

    @property
    def mask_cells(self) -> np.ndarray:
        """ Loads an returns the sequence's triplets' cell mask stacked in a
        numpy array under the shape ``(nb_triplets, img_height, img_width)``.
        """
        return np.stack([t.mask_cell for t in self._triplets], axis=0)

    @property
    def mask_bckgs(self) -> np.ndarray:
        """ Loads an returns the equence's triplets' background mask stacked in
        a numpy array under the shape ``(nb_triplets, img_height, img_width)``.
        """
    ...
    def is_enabled(self) -> bool:
        """ Returns ``True`` if the sequence is enabled for calibration.
        ``False`` otherwise.

        A sequence enabled is a sequence without a ``.qtf_ignore.txt`` file in
        its folder.
        """
    ...
    def set_enabled(self, val: bool) -> None:
        ...
        I the sequence goes from disabled to enabled, it deletes the
        ``.qtf_ignore.txt`` file in the sequence's folder. In the other way
        arround, it creates it.
    ...
    @property
    def size(self) -> int:
        """ Number of triplets associated with the sequence.
        """
        return len(self._triplets)

    def __getitem__(self, index: int) -> Triplet:
        """ Get the triplet at the index ``index`` in the sequence.
        ...
        Returns:
            Triplet: Triplet at the index ``index``.
        """
    ...
    def __iter__(self) -> Iterator[Triplet]:
        ...
        Yields:
            Iterator[Triplet]: Current triplet iterated.
        """
    ...
    def folder(self) -> Path:
        """ Folder where the sequence is stored.
        """
        return self._folder
    ...
    def subfolder(self) -> Path:
        ...
        Otherwise returns the folder to the sequence.
        """
