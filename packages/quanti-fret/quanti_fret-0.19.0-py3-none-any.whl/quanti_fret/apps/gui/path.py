...
class PathWidget(QGroupBox):
    ...
    * A Label showing the current path selected.
    * A button to select a path.

    It connects automatically to the config to save the newly selected path,
    and to load the path that is in the config.

    If the selected path leads to issues, a message box with the error is
    displayed.
    ...
    def __init__(
        ...
        Args:
            ...
            configKey (tuple[str, str]): Config's section and key associated
                pointing to the option managed by this widget.
            type (str): Type of path to look for. Either ``file`` or
                ``folder``.
    ...
    def _updatePath(self, path: Path | None) -> None:
        """ Update the path with the one chosen by the user.

        This displays a message box if the path selected leads to issues.
    ...
    def _updateDisplay(self) -> None:
        """ Update the path value from the config.
        """
        current_path = self._iopm.config.get(*self._configKey)
...
class FretCaliConfigFileWidget(PathWidget):
    ...
    In addition to the path selection, this provides a button that select the
    active calibration config file.
    """
    ...
    def _setToCurrentCaliConfig(self):
        """ Set the calibration config path to the active one.
        """
    ...
    def _updateButton(self) -> None:
        """ Update the button to select the ctive config.

        If the config is already the active one, will inform the user and
        disable the button.
        """
