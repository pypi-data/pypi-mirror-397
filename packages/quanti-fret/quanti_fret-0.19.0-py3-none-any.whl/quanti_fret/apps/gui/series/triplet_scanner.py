...
class TripletScannerWidget(QWidget):
    """ Handle the the selection of a series folder, the loading of the
    sequences whithin, and the display and settings of the found sequences.

    The user will be ask to select a folder, and will then have the possibility
    to view them and to set their ``enabled`` state.

    Everytime the series folder, or a ``enabled`` state changed, this creates
    a new ``QtfSeries`` that is passed to the ``QtfSeriesManager``.

    As multiple :class:`TripletScannerWidget` can share the same sequences,
    there is a system to inform the others that an ``enbaled`` state was
    changed, in order for them to update themselves.

    The Sequences are displayed in a table with one row per sequence, and whose
    columns are:

    * *View*: Button to open the sequence.
    * *Enabled* - optional: Button to enable or disable a sequence. For
      calibration only.
    * *Triplets*: Number of triplets in the sequence.
    * *Path*: Path to the folder of the sequence.

    It comes with the following signals.

    * *sequenceEnabledStateChanged*: Signal that a sequence enable state was
      updated, giving the path to the sequence folder.
    """
    ...
    def externalSeriesSequenceEnabledStateChanged(self, path: Path) -> None:
        """ Inform this object that a sequence's ``enabled`` state was updated
        by another :class:`TripletScannerWidget`.

        If this scaner handle the same sequence, will reload the series
        associated.
    ...
    def _setSeriesFolder(self, folder: Path | None, force_reset: bool) -> None:
        ...
        If the folder is different from the previous one, and is not ``None``,
        it:

        * Scans for triplet sequences inside the folder.
        * Creates the triplet sequences table widget.
        * Updates the triplet sequences count labels.
        * Displays the result box widget.

        Args
            folder (Path | None): Folder to set for the series.
            force_reset (bool): If true, force the reload of the series even
                if folder didn't change. This handle the case where the regex
                changed.
        """
