...
class SingleSeriesManager(QObject):
    """ Manage sequences settings for a specific series.

    It provides two widgets:

    * A title widget that can be highlighted and that contains the following
      informations:

      * The Series name.
      * The number of sequences selected for this series.
      * A button to open the series settings.

    * A settings widget allowing you to select the sequences folder for this
      series, and to view or disable the sequences one by one.

    This class does not handle itself its display (highlight title, show or
    hide settings, ...) as the place is shared with others
    :class:`SingleSeriesManager`. It lets the :any:`SeriesWidget` be the
    referee.

    As multiple :any:`SingleSeriesManager` can share the same sequences, this
    class provides a way to inform them if a sequence's ``enabled`` state was
    modified. This way they can trigger the reloading of the series associated.

    This class provides the following signals:

    * *seriesManagerSelected*: emitted when the user pressed the manager's
      title `settings` button. The manager's object is passed as parameter.
      This signals the :any:`SeriesWidget` to update the display.
    * *sequenceEnabledStateChanged*: Signal that a sequence enable state
      was updated, giving the path to the sequence folder. This helps telling
      the others :any:`SingleSeriesManager` that could potentially share the
      same sequence to update themselves.
    ...
    def highlight(self, value: bool) -> None:
        ...
        A highlighted title will surround itself with a frame box, and the
        settings button will go from ``+`` to ``-``.
    ...
    def externalSeriesSequenceEnabledStateChanged(self, path: Path) -> None:
        """ Inform the settings widget that a sequence's ``enabled`` state
        was updated by another manager.

        The settings needs to check if it has actions to perform accordingly.
