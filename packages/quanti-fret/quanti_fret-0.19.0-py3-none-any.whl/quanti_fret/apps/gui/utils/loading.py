...
class LoadingProgressLayout(QHBoxLayout):
    """ Layout containing a :class:`LoadingAnimationWidget` and a ``QLabel``
    that can display a loading animation along side a text describing a
    progress.

    You can put this layout inside another widget or layout. By default the
    widget is hidden. You can call :meth:`start()` to show the widget and start
    the animation. And you can call :meth:`stop()` to hide the widget and stop
    the animation. Finally, you can call :meth:`setProgress()` to set the
    progress message.
    """
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        Args:
            parent (QWidget | None, optional): Parent widget to set to the
                widgets inside the layout. Default is ``None``.
        """
        super().__init__()
...
class LoadingAnimationWidget(QWidget):
    ...
    By default the widget is hidden. You can call :meth:`start()` to show the
    widget and start the animation. And you can call :meth:`stop()` to hide the
    widget and stop the animation.
    """
    def __init__(
        ...
        Args:
            interval (int, optional): interval in ms between two frames.
                Default to ``50``.
            parent (QWidget | None, optional): Parent widget to set. Default is
                ``None``.
        """
...
class LoadingAnimationLayout(QVBoxLayout):
    """ Layout containing a :class:`LoadingAnimationWidget` that can display a
    loading animation.

    This class is just a layout containing the widget. It exists to be prevent
    the user to be forced to create a layout.
