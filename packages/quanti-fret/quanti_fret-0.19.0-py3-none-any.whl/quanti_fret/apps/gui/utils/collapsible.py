...
class CollapsibleBox(QGroupBox):
    """ Box that can be expended or collapsed because Qt doesn't come with such
    widget...

    The box consist of

    * A title allongside an arrow. Both can be clicked to expand or collapse
      the content.
    * A content which is a layout that can be filled with other layouts or with
      widgets.

    By default, the box is collapsed. You can set the visibility in the code
    by calling :meth:`setContentVisible`.

    You can customize it by inheriting from it or just by giving the correct
    title, and adding content to the box by calling :meth:`addContentWidget` or
    :meth:`addContentLayout`.

    The layout of the content is a ``QVBoxLayout``.
    """
