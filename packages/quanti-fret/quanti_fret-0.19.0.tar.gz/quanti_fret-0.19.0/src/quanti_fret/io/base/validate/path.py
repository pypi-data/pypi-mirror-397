...
class PathValidator(Validator):
    ...
    * *convert_from*: returns a ``Path`` or ``None`` if the path is empty.
    * *convert_to*: accepts a ``Path`` or ``None`` if the path is empty.
    """
    ...
    def __init__(
        ...
        Args:
            type (str): Wether the path represents a file or a folder. Must
                be in ``['file', 'folder']``.
            allow_non_existing (bool, optional): Allow or not the path to point
                to a non existing location if not empty. Default to ``False``.
            allow_empty (bool, optional): Allow or not the value to be empty.
                Default to ``True``.
        """
    ...
    def _validate_path(self, path: Path | None):
        ...
        Raises:
            QtfException: The path is not valid.
        """
        if path is None:
