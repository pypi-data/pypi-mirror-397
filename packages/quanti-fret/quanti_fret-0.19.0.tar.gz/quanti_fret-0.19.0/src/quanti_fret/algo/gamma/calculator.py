...
class GammaCalculator(abc.ABC):
    """ Abstract class that computes the ``gamma`` value from a given series.

    The two ``gamma`` values of **QuanTI-FRET** are :math:`\\alpha^{BT}` and
    :math:`\\delta^{DE}`.

    To compute a ``gamma`` value, we first computes the ``gamma`` matrix of
    one triplet of all the sequences in the series. It is the matrix obtained
    by dividing the DA channel image by either the DD or the AA channel image.
    The gamma value is then extracted by selecting only relevant values within
    the cell and taking the median value of all the matrices.
    ...
    * To get the final results call :meth:`compute_results`.
    ...
    this class constructor (can be for example ``alpha_BT`` or ``delta_DE`` and
    you must define the following:

    * :meth:`_get_gamma_channels_index`: returns the index of the gamma
      channel.
    * :meth:`_get_gamma_plot_params`: returns some params for plots.
    """
    ...
    def params(
        ...
        Args:
            ...
            discard_low_percentile (float, optional): percentile of pixels to
                discard under this value, after applying the mask. Default is
                ``O.``.
            plot_sequence_details (float, optional): True to plot the hist2d
                and gamma figures for each dequences. Default is ``False``.
    ...
    def add_triplet(
        ...
        Returns:
            dict[str, Figure]:
                Figures associated with the triplet if plot_sequence_details is
                set to True. Otherwise the dictionary is empty. The keys are:
    ...
    def compute_results(self) -> tuple[float, float, int, dict[str, Figure]]:
        ...
        Returns:
            tuple[float, float, int, dict[str, Any]]:
                ...
                * Figures detailing the data. keys are ``boxplot`` and
                  ``scatter``.
        """
        if self._params is None:
    ...
    def _compute_and_extract_gammas(
        ...
        """ Compute the gamma matrix for the given triplet. The gamma returned
    ...
    def _summary_plots(
        ...
        * The scatter plot of median gamma per sequence with regards to medium
          intensity per sequence.
    ...
    def _detail_seq_plot(
        ...
        Args:
            ...
            masked_gamma (np.ndarray): The gamma values extracted from the
                mask.
        ...
        Returns:
            dict[str, str | Figure]:
                The plot in the form of a dict with:
