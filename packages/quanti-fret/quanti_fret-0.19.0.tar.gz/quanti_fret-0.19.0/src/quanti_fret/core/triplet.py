...
class Triplet:
    ...
    * (**MaskBckg**: Background mask) -> Optional
    ...
    The images are expected to be in ``TIFF`` format, and the paths to the
    images are provided at initialization.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            mask_bckg_path (os.PathLike | str, optional): Path to the
                background mask image. If set to ``''``, consider the Triplet
                do not have background Mask. Default to ``''``.
            img_type (np.dtype | None, optional): Force the type of the
                DD/DA/AA images. Default to ``np.float32``.
        """
    ...
    def dd(self) -> np.ndarray:
        """ The DD channel as a numpy array.
        """
    ...
    def da(self) -> np.ndarray:
        """ The DA channel as a numpy array.
        """
    ...
    def aa(self) -> np.ndarray:
        """ The AA channel as a numpy array.
        """
    ...
    def as_numpy(self) -> np.ndarray:
        """ The DD/DA/AA channels stacked in a numpy array under the shape
    ...
    def mask_cell(self) -> np.ndarray:
        """ The cell mask as a numpy array.
    ...
    def mask_bckg(self) -> np.ndarray:
        """ The background mask as a numpy array.
