...
class TripletResultsTable(QTableWidget):
    ...
    The table contains one line per result, and can show the following columns:

    * *ID*: Id of the result.
    * *Res* - Optional: Opens the results of the triplet (from
      :any:`StageResults.get_triplet_results`).
    * *Plot* - Optional: Opens the extra results of the triplet (from
      :any:`StageResults.get_triplet_extras`)
    * *Sequence (Triplet)*: Path to the sequence used. In parenthesis, there is
      the index of the triplet used relative to the sequence.

    To use this widget, please connect to the following signal if relevant:

    * *resultsClicked*: Signal that the results button of the triplet was
      clicked. Pass the id of the triplet clicked.
    * *plotClicked*: Signal that the plot button of the triplet was clicked.
      Pass the id of the triplet clicked.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            hasResults (bool, optional): Enable the results column. Default to
                ``False``.
            hasPlot (bool, optional): Enable the plot column. Default to
                ``False``.
            detailIndex (int, optional): Index of the element in the settings
                returned by the :any:`StageResults.get_stage_settings` that
                describe if the results were computing asking for detail plots
                or not. If ``-1``, plot details always be activated. If Default
                to ``-1``.
        """
    ...
    def _addTableRow(
        ...
        Args:
            ...
            enablePlots (bool): If set to ``False``, will set the plot
                to disable mode.
        """
        # Id Column
