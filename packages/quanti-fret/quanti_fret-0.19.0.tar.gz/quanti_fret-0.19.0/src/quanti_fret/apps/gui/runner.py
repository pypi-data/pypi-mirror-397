...
class SingletonQObject(type(QObject)):  # type: ignore
    """ Metaclass that allows to create a singleton compatible with a QObject.

    Because apparently Qt doesn't allow that...
    """
...
class CalculatorRunner(QObject, metaclass=SingletonQObject):
    ...
    It delegates the actual run to the :class:`CalculatorWorker` that is
    working in a thread.
    ...
    * ``finished``: Signaling that the run is finished. Also gives the stage
      that was run so that the widget can know if they were concerned or not.
    * ``progress``: Signaling the progress of the run (not used in all stages).
...
class CalculatorWorker(QObject):
    ...
    It delegates the run to the :any:`QtfRunner` class by selecting the proper
    ...
    * Trigger the worker connecting the :meth:`run()` function to a signal.
