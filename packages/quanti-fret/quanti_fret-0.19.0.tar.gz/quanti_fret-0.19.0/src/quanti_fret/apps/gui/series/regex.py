...
class RegexBox(CollapsibleBox):
    ...
    once all the regexes have been set. This avoid to parse the file system
    after changing only one regex out of 3 or 5.
...
class RegexWidget(QWidget):
    """ Handle the setting of a regex of a single image.

    Will display a title alongside with a line edit that will update the config
    on change.

    In order to have all the :class:`RegexWidget` titled aligned, we provide
    function to get and set the title width only.
    """
    ...
    def getTitleWidth(self) -> int:
        ...
        Returns:
            int: The minimum width of the title.
        """
