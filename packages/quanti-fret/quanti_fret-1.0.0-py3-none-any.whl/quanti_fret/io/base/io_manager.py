...
class IOPhaseManager(abc.ABC):
    """ Class that provides an access, from a single object, to a
    :any:`Config`, a :any:`QtfSeriesManager`, a :any:`ResultsManager` and a
    :any:`StageParams` for a given phase.
    ...
    properties to get the config (:meth:`config`), the series manager
    (:meth:`series`), the result manager (:meth:`results`), or the stage params
    (:meth:`params`) in case they changed since last call. (Do not store them
    locally).

    Config, SeriesManager, ResultsManager and StageParams are proxied to catch
    their ``save`` or ``set`` method to inform the :class:`IOPhaseManager` of
    any changes.

    :meth:`_updated_config`, :meth:`_updated_series` and
    :meth:`_updated_results` are called every time ``set`` is called on the
    config or series manager, and everytime ``save`` is called from results
    manager.

    You must call :meth:`load_config` before being able to call the different
    getters.
    """
    def __init__(
        ...
        Args:
            phase (str): Name of the phase associated with the manager.
            load_series (bool, optional): If ``True``, the IOPhaseManager will
                load the series itself using the values found in the config.
                Otherwise, it is let to the user to handle the series loading.
                Defaults to ``True``.
            exclude_disabled_seq (bool, optional): If ``True``, will exclude
                disabled sequences when loadng a series. This option is
                relevant only when ``load_series`` is set to ``True``. Default
                to ``True``.
        """
    ...
    def config(self) -> Config:
        """ :any:`Config` class associated with the IOPM.

        Can't be called without calling :meth:`load_config` first.

        Raises:
            QtfException: No config exists.
        """
    ...
    def series(self) -> QtfSeriesManager:
        """ :any:`QtfSeriesManager` class associated with the IOPM.

        Can't be called without calling :meth:`load_config` first.

        Raises:
            QtfException: No series manager exists.
        """
    ...
    def results(self) -> ResultsManager:
        """ :any:`ResultsManager` class associated with the IOPM.

        Can't be called without calling :meth:`load_config` first.

        Raises:
            QtfException: No results manager exists.
        """
    ...
    def params(self) -> StageParams:
        """ :any:`StageParams` class associated with the IOPM.

        Can't be called without calling :meth:`load_config` first.

        Raises:
            QtfException: No stage params exists.
        """
    ...
    def load_config(self, config_path: os.PathLike | str) -> None:
        """ Load a new config from the given ini file.
    ...
    def reset_config(self) -> None:
        ...
        The iopm can't be used without a :meth:`load_config` after that.
        """
        self._allow_updates = False
        self._config = None
    ...
    def link_iopm(self, other: 'IOPhaseManager | None') -> None:
        ...
        The purpose of this method is to allow notification between IOPM from
        different phases. For example, the FRET IOPM must be notify from a
        change in the Calibration IOPM.

        This method is linked to :meth:`external_config_update`.
    ...
    def external_config_update(self, section: str, key: str, val: Any) -> None:
        """ Signal this object that the config of the IOPM linked has been
        updated and that some objects might need to be regenerated.

        This method is linked to :meth:`link_iopm`.
    ...
    def _updated_config(self, section: str, key: str) -> None:
        """ Signal that the config was updated, and check if any component
    ...
    def _updated_series(self, name: str) -> None:
        """ Signal that the series manager was updated, and check if any
        component needs to be reloaded.
    ...
    def _updated_results(self, stage: str):
        """ Signal that the results manager was updated, and check if any
        component needs to be reloaded.
    ...
    def _updated_params(self) -> None:
        """ Signal that the stage params was updated, and check if any
        component needs to be reloaded.
    ...
    def _new_series_manager(self) -> None:
        """ Create a new :any:`QtfSeriesManager` and wrap its
        :any:`QtfSeriesManager.set` method to inform the IOPhaseManager that a
        series was updated.
        """
    ...
    def _generate_sequence_loader(self) -> TripletSequenceLoader:
        """ Generate a :any:`TripletSequenceLoader` using the configuration.
        """
        regex = {
            'dd_path': '',
            'da_path': '',
    ...
    def _new_results_manager(self) -> None:
        """ Create a new :any:`ResultsManager` and wrap its
        :any:`ResultsManager.save_stage` method to inform the IOPhaseManager
        that the ResultsManager was updated.
        """
    ...
    def _create_series_manager(self, load_series: bool) -> None:
        ...
        Must be called after :meth:`_create_config`.
    ...
    def _create_stage_params(self) -> None:
        ...
        Must be called after :meth:`_create_config`,
        :meth:`_create_series_manager` and :meth:`_create_results_manager`
