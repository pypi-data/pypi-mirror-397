...
class IOGuiManager(metaclass=Singleton):
    ...
    instance of a :any:`IOPhaseGuiManager` for the calibration and for the
    fret phases.
    """
    ...
    def __init__(self):
        ...
        Creates the IOPhaseGuiManagers.
        """
    ...
    def get_iopm(self, phase: str) -> 'IOPhaseGuiManager':
        ...
        Raises:
            QtfException: The phase doesn't exists.

        Returns:
            IOPhaseGuiManager: GUI manager associated.
        """
    ...
    def cali(self) -> 'IOPhaseGuiManager':
        """ The Calibration IOPhaseGuiManager.

        Returns:
            IOPhaseGuiManager: The calibration iop manager.
        """
        return self._iopm_cali

    @property
    def fret(self) -> 'IOPhaseGuiManager':
        """ The Fret IOPhaseGuiManager.

        Returns:
            IOPhaseGuiManager: The fret iop manager
        """
        return self._iopm_fret
...
class IOPhaseGuiManager(IOPhaseManager):
    ...
    application's :any:`Config`, :any:`QtfSeriesManager`, :any:`ResultsManager`
    and :any:`StageParams`.

    It manage the selection, saving and restoration of config and already
    computed results in between two runs of the application. It also provide a
    default config if none exists.

    When the :any:`Config`, :any:`QtfSeriesManager`, :any:`ResultsManager` are
    modified, in addition to the :any:`IOPhaseManager` default behavior, the
    signal ``stateChanged`` is emitted. For the :any:`Config`, it is also saved
    automatically.
    ...
    * ``[user_data_dir]/output``: Default output dir location.
    ...
    See platformdirs to know where are located the folder starting with
    ``"user_"``.

    You must call :meth:`load_config` before calling the different getters.

    This class emit the following signals:

    * ``stateChanged``: The state of the IO changed (either config, series or
      results).
    * ``noConfig``: There is no more config attached to the manager.
    * ``errorConfig``: An error occured while loading the config. Send the path
      to the config that was being loaded and the associated error and trace
    """
    ...
    class SignalEmmiter(QObject):
        """ Placeholder for the signals emitted by :class:`IOPhaseGuiManager`.

        This class is needed because Multiple inheritance with QObject doesn't
        work with PyQt.
        """
        stateChanged = Signal()
        noConfig = Signal()
    ...
    def __init__(self, phase: str) -> None:
        ...
        Args:
            phase (str): Phase associated with the IOPM. Must be in ``[
                'calibration', 'fret']``.

        Raises:
            QtfException: Unknown phase.
        """
    ...
    def reset_config(self) -> None:
        ...
        The  iom can't be used without a :meth:`load_config` after that.
        """
    ...
    def change_config(self, path: Path | None, mode: ConfigChangeMode) -> bool:
        ...
        Args:
            path (Path | None): New config  path, if ``None`` is set, the iom
                will be reset.
        ...
        Returns:
            bool:
                ``True`` if the config changed, ``False`` if an error occured.
        """
    ...
    def load_base_config(self) -> bool:
        ...
        exists. It's location is described in
        :any:`AppDefaultPath.base_config_file`.

        Returns:
            bool:
                ``True`` if the config was loaded, ``False`` if an error
                occured.
        """
        try:
    ...
    def get_active_config_path(self) -> Path | None:
        ...
        Returns:
            Path | None:
                Path to the active config if it exists. ``None`` otherwise.
