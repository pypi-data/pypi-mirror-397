...
class XMCalculator:
    """ Compute the :math:`\\beta^{X}` and :math:`\\gamma^{M}` calibration
    values.

    To use this class on a series, you need to follow the following workflow:

    * Call :meth:`reset`.
    * Set the params using :meth:`params`.
    * For each triplet of the series, call :meth:`add_triplet`.
    * To get the final results call :meth:`compute_results`.
    ...
    def params(
        ...
        Args:
            alpha_bt (float): :math:`\\alpha^{BT}` value from calibration.
            delta_de (float): :math:`\\delta^{DE}` value from calibration.
            ...
            percentile_range (tuple[float, float]): The percentile range
                ...
                the mask.
            analysis_details (bool, optional): Wether or not to compute
                detailed data for analysis purposes. Default is ``False``.
            analysis_sampling (int): Keep one value every ``analysis_sampling``
                values on data used to generate analysis details. Default is
                ``100``.
    ...
    def compute_results(
        ...
        """ Compute the :math:`\\beta^{X}` and :math:`\\gamma^{M}` values
        using the data computed on all the triplet added to the series
        computation.

        Also returns :math:`\\chi^2_\\nu`, :math:`R^2`, :math:`Q` and
        extra analysis data if asked.

        Analysis data are computed on sampled data, extracted within the cell
        mask, and returned as a dictionary organized as follow:

        * ``'hist2d_s_vs_e'``: 2D histogram of S vs E.
        * ``'hist2d_e_vs_iaa'``: 2D histogram of E vs AA.
        * ``'hist2d_s_vs_iaa'``: 2D histogram of S vs AA.
        * ``'median_sampled'``: Median value of shape ``(nb_triplets, 5)`` with
          one line per triplet, and for the columns: ``(DD, DA, AA, E, S)``.
        * ``'sampled_list'``: List of sample data per triplet used.
        * ``'e_boxplot'``: Boxplot of E per sequence.
        * ``'s_boxplot'``: Boxplot of S per sequence.
        * ``'inspection'``: dictionary cotaining:

          * ``'scatter_3d'``: 3D plots of the channels DD, DA, and AA.
          * ``'triplets_per_seq'``: Median value of DD, DA and AA per sequence.
          * ``'s_per_seq'``: S median value per sequence.
          * ``'s_vs_e'``: S median value per E median value.

        Returns:
            tuple[float, float, float, float, float]:
                :math:`\\beta^{X}`, :math:`\\gamma^{M}`, :math:`\\chi^2_\\nu`,
                :math:`R^2`, :math:`Q`, Extras
        """
        if self._params is None:
    ...
    def _extract_sampled_data(
        ...
        * median_sampled: An Array of shape ``(nb_seq, 5)`` containing for
          each sequence the median value of ``(DD, DA, AA, E, S)``.
    ...
    def _plot_boxplot(
        ...
        Args:
            sampled_list (list[np.ndarray]): Sampled data for each sequence
                with E, and S respectively on channel 3, and 4
    ...
    def _generate_3d_plot(
        ...
        Args:
            ...
            beta_x (float): :math:`\\beta^{X}` computed (used for plane).
            gamma_m (float): :math:`\\gamma^{M}` computed (used for plane).
    ...
    def _3d_plot_mask(
        ...
        * We divide the range covered by the data into ``n_bins`` bins. For
          each bins, we keep only (at max) ``max_samples_per_bins`` values.
        * To select the values for each bins : we extract the all the values
          inside the bin into a subvector - we sort it - we take
          ``max_samples_per_bins`` indices evenly spread out from 0 to the size
          of this vector - We take the values associated with this indices.
