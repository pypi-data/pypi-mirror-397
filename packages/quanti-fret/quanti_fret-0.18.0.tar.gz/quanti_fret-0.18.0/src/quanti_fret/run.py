...
def _notify_disabled(msg: str) -> None:
    """ Default notify function to use. Does nothing.
...
class QtfRunner:
    ...
    It gets the stages params and save their outputs using the
    :any:`IOPhaseManager` passed to the constructor.

    Most the stages are run like this:

    #. Get the params of the stage and set them to the stage class.
    #. Get a series iterator.
    #. For each triplet.

       #. Run the stage on the triplet.
       #. Save the result of the triplet.

    #. Run the stage computation.
    #. Save the results of the stage.

    During ths process, it will call regularly the notify callback to inform
    about the progress.
    """
    ...
    def __init__(self, iom: IOManager):
        ...
        Args:
            iom (IOManager): IOManager to get the params and save the results
                for the run of each stage.
        """
        self._iom = iom
    ...
    def run_all(self) -> None:
        """ Run the all the **QuanTI-FRET** stages.
        """
    ...
    def run_bt(self, notify: Callable[[str], None] = _notify_disabled) -> None:
        ...
        Args:
            notify (Callable[[str], None], optional): Callback that will be
                called between each steps to inform the user of the progress.
                Please note that the run will be paused while the callback is
                being called. Default is :func:`_notify_disabled`.
