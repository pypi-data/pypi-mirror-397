...
class Experiment:
    ...
    * From all these generated matrices, and the physical values, we
    ...
    All matrices generated are squares of width
    :any:`ExperimentsParameters.img_width`.
    """
    ...
    def _generate_donors(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray:
                The matrix of size containing the number of donors per pixels.
        """
    ...
    def _generate_acceptors(
        ...
        Args:
            nb_donors (np.ndarray): Number of donors per pixel. S (np.ndarray):
            Stochiometry per pixels.

        Returns:
            np.ndarray:
                The matrix of size containing the number of acceptors per
                pixels.
        """
    ...
    def _compute_intensities(
        ...
        Returns:
            tuple[np.ndarray, np.ndarray, np.ndarray]:
                I_DD, I_DA, and I_AA intensities per pixels.
        """
        # Get parameters
        La = self.EP.La
        Ld = self.EP.Ld
...
class Analyser:
    ...
    * computes the GammaM and BetaX values that would return the QuanTI-FRET
      algorithm.
    * compute the error between the computed values and the theoretical ones
      for GammaM, BetaX ans E.
    ...
    def _generate_interval(
        ...
        Args:
            ...
            reverse (bool, optional): If in geomspace and if set to True, will
                generate more high values than low values. Defaults to
                ``False``.

        Returns:
            np.ndarray: Interval generated.
        """
        min = range[0]
        max = range[1]
        if reverse:
