...
class FretResults(StageResults):
    ...
    * **settings**:
      ...
      * beta_x (``float``).
      * gamma_m (``float``).
      * background engine (:any:`BackgroundEngine`).
      * target_s (``float``).
      * sigma_s (``float``).
      * sigma_gauss (``float``).
    ...
    * **triplets extra** (dict):
    ...
    def _write_tiff(self, img: np.ndarray, path: Path) -> None:
        """ Utility function to write a tiff file
