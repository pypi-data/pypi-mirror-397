...
class NumericalSort:
    ...
    It is designed to be used with the ``sort()`` or ``sorted()`` Python's
    function by passing the :meth:`NumericalSort.key` method to their ``key``
    argument. It splits the string into parts, where each part is either a
    string or an integer. The integers are converted to ``int`` type, so that
    they are sorted numerically rather than lexicographically. This is useful
    when sorting file names or other strings that contain numbers, such as
    ...
    .. code:: Python
        ...
        l = ['aa_5', 'aa_0_a', 'zz_156', 'aa_0_b', 'bb_2', 'aa_22', 'zz_8']
        results = sorted(l, key=NumericalSort().key)
        # results:
        # ['aa_0_a', 'aa_0_b', 'aa_5', 'aa_22', 'bb_2', 'zz_8', 'zz_156']
    """
...
class TripletSequenceLoader:
    ...
    so, loads it into a :any:`TripletSequence` class.
    ...
    * MaskBkg (optional)
    ...
    Note that the sequence can contain only one frame, this means just one set
    of the above images.

    If one triplet of the sequence has one optional image, we expect all of
    them to have the same optional image.

    By default, we expect all images

    * To be in a same folder.
    * To respect the regex patterns described in :any:`DEFAULT_REGEX_PATTERNS`.
    * To be differenciated by a number in the file name, such as
      ``"001_DD.tif"``, if the sequence contains more than one triplet.

    The user can overwrite the regex patterns.
    """
    ...
    """ Default regex pattern to find the different images. """
    ...
    def __init__(self, regex_patterns: dict[str, str] | None = None):
        ...
        Initializes the :class:`TripletSequenceLoader` either with the default
        regex patterns or with the provided ones.

        If you decide to overwrite the default regex patterns, please provide a
        dictionary containing the same keys as :any:`DEFAULT_REGEX_PATTERNS`
        (not all the keys have to be presents). If the value of the overwrite
        dictionarry is an empty string, this key will be ignored.

        Args:
            regex_patterns (dict[str, str] | None, optional):
                If not ``None``, overwrite default regex pattern to search for
                each image channel. Defaults to ``None``.
        """
    ...
    def check_and_load(
        ...
        loads it into a :any:`TripletSequence` class.
        ...
        Args:
            ...
            verbose (bool, optional):
                To print or not the reason why the sequence is not valid.
                Defaults to ``False``.

        Returns:
            TripletSequence | None:
                The :any:`TripletSequence` object if the folder is valid,
                otherwise ``None``.
        """
    ...
    def _load_images_matching_patterns(
        ...
        Args:
            folder (os.PathLike | str): Path to the folder to search for
                images.
    ...
    def _validate_images(
        ...
        If one triplet have one optional image, all triplets must have the same
        optional image.

        If verbose is ``True``, it will print the reason why the sequence is
        not valid.
...
class TripletScanner:
    """
    Class that scans for all the triplet sequences that can be found in a
    folder and its subfolders.

    The folders are checked recursively, but if one contains an sequence, its
    subfolders are not checked.

    Triplet sequences are loaded using the default
    :class:`TripletSequenceLoader` class or the one passed as a parameter to
    the constructor.
    """
    ...
    def __init__(self, loader: TripletSequenceLoader | None = None):
        ...
        Args:
            loader (TripletSequenceLoader | None, optional):
                ...
                :class:`TripletSequenceLoader` is instanciated with the default
                regex patterns. Default to ``None``.
        """
        if loader is None:
    ...
    def scan(self, folder: os.PathLike | str) -> QtfSeries:
        ...
        Args:
            folder (os.PathLike | str): Path to the root folder to search in.
        Returns:
            QtfSeries:
                Series containing all the triplet sequences found in the folder
                and its subfolders.
        """
