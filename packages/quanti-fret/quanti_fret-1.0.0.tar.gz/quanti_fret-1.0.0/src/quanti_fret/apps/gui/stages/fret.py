...
class StageFretWidget(StageCalculatorWidget):
    """ Handle the stage that computes the FRET stages.

    You can have a look at :meth:`_updateSettings` and :meth:`_loadResults`
    yo have an idea of important widgets created.
    """
    ...
    def _buildResultsSetting(self, parent: QWidget) -> QLayout | None:
        ...
        self._rTargetS = ResultCells(parent, 'TargetS', layout, 7)
        self._rSigmaS = ResultCells(parent, 'SigmaS', layout, 8)
    ...
    def _loadResults(self) -> bool:
        ...
        if settings is None:
            return False
        series, nb_seq, alphaBt, deltaDe, betaX, gammaM, background, targetS, \
            sigmaS, sigmaGauss, weightsThreshold,  analysisDetails, \
