...
class SchemaVisualizer:
    ...
    def _format_as_text(self, result: Dict[str, Any], format: str) -> str:
        """Format analysis result as plain text for agent consumption."""
        lines = []
        
        # Header
        schema_name = result.get("schema_name", "current_database")
        lines.append("=" * 60)
        lines.append(f"SCHEMA VISUALIZATION: {schema_name}")
        lines.append("=" * 60)
        lines.append("")
        ...
        if stats.get("status") == "success":
            ...
            lines.append(f"Total Constraints: {stats.get('total_constraints', 0)}")
            lines.append(f"Tables with Relationships: {stats.get('tables_with_relationships', 0)}")
            
            orphaned_tables = stats.get("orphaned_tables", [])
            if orphaned_tables:
                lines.append(f"Orphaned Tables ({len(orphaned_tables)}): {', '.join(orphaned_tables)}")
            
            largest_tables = stats.get("largest_tables", [])
            if largest_tables:
                lines.append("\nLargest Tables (by row count):")
                for table_name, row_count in largest_tables[:5]:
                    lines.append(f"  - {table_name}: {row_count:,} rows")
            lines.append("")
        
        # ER Diagram section (main content)
        ...
        if relationships:
            ...
            for rel in relationships:
                lines.append(
                    f"{rel['parent_table']}.{rel['parent_column']} -> "
                    f"{rel['child_table']}.{rel['child_column']} ({rel.get('type', 'unknown')})"
                )
            lines.append("")
        
        # Circular references
        circular_refs = result.get("circular_references", [])
        if circular_refs:
        ...
        duration = round(result.get("analysis_duration_seconds", 0), 2)
        timestamp = result.get("timestamp", "")
        if timestamp:
            lines.append(f"Generated at: {timestamp} (took {duration}s)")
        
        return "\n".join(lines)

    def _format_timeout_result(self, timeout: int) -> str:
        """Format timeout result."""
        return f"ERROR: Analysis timed out after {timeout} seconds. Try reducing schema complexity or increasing timeout limit."
