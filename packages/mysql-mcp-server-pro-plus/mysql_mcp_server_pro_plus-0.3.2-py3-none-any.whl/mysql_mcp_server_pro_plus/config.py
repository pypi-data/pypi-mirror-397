...
class DatabaseConfig(BaseModel):
    ...
    pool_name: Optional[str] = Field(
        default=None, description="Custom pool name (auto-generated if not provided)"
    )

    def get_pool_name(self) -> str:
        """Generate a short pool name (max 64 chars) from connection parameters.
        
        Uses a deterministic hash to ensure the same connection params produce
        the same pool name, while keeping it under the 64-character limit.
        """
        if self.pool_name:
            return self.pool_name[:64]  # Ensure it's within limit
        
        # Create a deterministic hash from connection parameters
        pool_key = f"{self.host}:{self.port}:{self.database}:{self.user}"
        pool_hash = hashlib.sha256(pool_key.encode()).hexdigest()[:16]
        return f"mysql_pool_{pool_hash}"
    ...
    def from_url(cls, url: str) -> "DatabaseConfig":
        ...
        return cls(
            ...
            pool_name=get_param("pool_name", None) or None,
        )
    ...
    def from_env(cls) -> "DatabaseConfig":
        ...
        return cls(
            ...
            == "true",
            pool_name=os.getenv("MYSQL_POOL_NAME") or None,
        )
