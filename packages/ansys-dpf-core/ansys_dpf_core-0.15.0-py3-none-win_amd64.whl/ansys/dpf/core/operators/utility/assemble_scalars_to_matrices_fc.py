...
class assemble_scalars_to_matrices_fc(Operator):
    ...
    def __init__(
        ...
        super().__init__(
            name="assemble_scalars_to_matrices_fc",
            config=config,
            server=server,
            inputs_type=InputsAssembleScalarsToMatricesFc,
            outputs_type=OutputsAssembleScalarsToMatricesFc,
        )
        if xx is not None:
    ...
    def _spec() -> Specification:
        description = r"""Take nine scalar fields container and assemble them as a 3x3 matrix
fields. If the ‘symmetrical’ input is set to true, only six field
containers are required (xx, yy, zz, xy, xz and yz).
"""
        spec = Specification(
    ...
    @property
    def inputs(self) -> InputsAssembleScalarsToMatricesFc:
