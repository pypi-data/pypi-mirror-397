...
class InputsApplySvd(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._field_contaner_to_compress: Input[FieldsContainer] = Input(
            apply_svd._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._field_contaner_to_compress)
        self._scalar_int: Input[int] = Input(apply_svd._spec().input_pin(1), 1, op, -1)
        self._inputs.append(self._scalar_int)
        self._scalar_double: Input[float] = Input(
            apply_svd._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._scalar_double)
        self._boolean: Input[bool] = Input(apply_svd._spec().input_pin(3), 3, op, -1)
        self._inputs.append(self._boolean)

    @property
    def field_contaner_to_compress(self) -> Input[FieldsContainer]:
    ...
    @property
    def scalar_double(self) -> Input[float]:
...
class OutputsApplySvd(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._us_svd: Output[FieldsContainer] = Output(
            apply_svd._spec().output_pin(0), 0, op
        )
        self._outputs.append(self._us_svd)
        self._vt_svd: Output[FieldsContainer] = Output(
            apply_svd._spec().output_pin(1), 1, op
        )
    ...
    @property
    def us_svd(self) -> Output[FieldsContainer]:
