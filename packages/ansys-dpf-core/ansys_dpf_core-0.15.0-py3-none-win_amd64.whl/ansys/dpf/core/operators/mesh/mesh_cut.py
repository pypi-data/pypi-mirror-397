...
class InputsMeshCut(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._field: Input[Field] = Input(mesh_cut._spec().input_pin(0), 0, op, -1)
        self._inputs.append(self._field)
        self._iso_value: Input[float] = Input(mesh_cut._spec().input_pin(1), 1, op, -1)
        self._inputs.append(self._iso_value)
        self._closed_surface: Input[int] = Input(
            mesh_cut._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._closed_surface)
        self._mesh: Input[MeshedRegion] = Input(
            mesh_cut._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._mesh)
        self._slice_surfaces: Input[bool] = Input(
            mesh_cut._spec().input_pin(4), 4, op, -1
        )
...
class OutputsMeshCut(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._mesh: Output[MeshedRegion] = Output(mesh_cut._spec().output_pin(2), 2, op)
