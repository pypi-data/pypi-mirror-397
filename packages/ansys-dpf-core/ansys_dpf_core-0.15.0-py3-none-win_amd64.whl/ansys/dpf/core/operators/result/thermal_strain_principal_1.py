...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="ETH1", server=server)

    @property
    def inputs(self) -> InputsThermalStrainPrincipal1:
...
class InputsThermalStrainPrincipal1(_Inputs):
    ...
    thermal_strain_principal_1 operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(thermal_strain_principal_1._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            thermal_strain_principal_1._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            thermal_strain_principal_1._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            thermal_strain_principal_1._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            thermal_strain_principal_1._spec().input_pin(3), 3, op, -1
        )
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            thermal_strain_principal_1._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            thermal_strain_principal_1._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._read_cyclic: Input[int] = Input(
            thermal_strain_principal_1._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            thermal_strain_principal_1._spec().input_pin(22), 22, op, -1
        )
...
class OutputsThermalStrainPrincipal1(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(thermal_strain_principal_1._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            thermal_strain_principal_1._spec().output_pin(0), 0, op
        )
