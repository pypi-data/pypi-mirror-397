...
class OutputsReadCmsRbdFile(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._v_trsf: Output = Output(read_cms_rbd_file._spec().output_pin(5), 5, op)
        self._outputs.append(self._v_trsf)
        self._k_mat: Output[Field] = Output(
            read_cms_rbd_file._spec().output_pin(6), 6, op
        )
        self._outputs.append(self._k_mat)
        self._mass_mat: Output[Field] = Output(
            read_cms_rbd_file._spec().output_pin(7), 7, op
        )
        self._outputs.append(self._mass_mat)
        self._c_mat: Output[Field] = Output(
            read_cms_rbd_file._spec().output_pin(8), 8, op
        )
        self._outputs.append(self._c_mat)
        self._rhs: Output[Field] = Output(
            read_cms_rbd_file._spec().output_pin(9), 9, op
        )
        self._outputs.append(self._rhs)
        self._dn: Output = Output(read_cms_rbd_file._spec().output_pin(10), 10, op)
        self._outputs.append(self._dn)
        self._dr_cross_n: Output = Output(
            read_cms_rbd_file._spec().output_pin(11), 11, op
        )
        self._outputs.append(self._dr_cross_n)
        self._drn: Output = Output(read_cms_rbd_file._spec().output_pin(12), 12, op)
        self._outputs.append(self._drn)
        self._dn_cross_n: Output = Output(
            read_cms_rbd_file._spec().output_pin(13), 13, op
        )
        self._outputs.append(self._dn_cross_n)
        self._dnx_y: Output = Output(read_cms_rbd_file._spec().output_pin(14), 14, op)
        self._outputs.append(self._dnx_y)
        self._dny_y: Output = Output(read_cms_rbd_file._spec().output_pin(15), 15, op)
        self._outputs.append(self._dny_y)
        self._dnz_y: Output = Output(read_cms_rbd_file._spec().output_pin(16), 16, op)
        self._outputs.append(self._dnz_y)
        self._dyx_n: Output = Output(read_cms_rbd_file._spec().output_pin(17), 17, op)
        self._outputs.append(self._dyx_n)
        self._dyy_n: Output = Output(read_cms_rbd_file._spec().output_pin(18), 18, op)
        self._outputs.append(self._dyy_n)
        self._dyz_n: Output = Output(read_cms_rbd_file._spec().output_pin(19), 19, op)
        self._outputs.append(self._dyz_n)
        self._dnxn: Output = Output(read_cms_rbd_file._spec().output_pin(20), 20, op)
        self._outputs.append(self._dnxn)
        self._dnyn: Output = Output(read_cms_rbd_file._spec().output_pin(21), 21, op)
        self._outputs.append(self._dnyn)
        self._dnzn: Output = Output(read_cms_rbd_file._spec().output_pin(22), 22, op)
