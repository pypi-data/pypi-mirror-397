...
class part_hourglass_energy(Operator):
    r"""Read Part Hourglass Energy (LSDyna) by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.part_hourglass_energy()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsPartHourglassEnergy,
            outputs_type=OutputsPartHourglassEnergy,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read Part Hourglass Energy (LSDyna) by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="M_ENG_AHO", server=server)

    @property
    def inputs(self) -> InputsPartHourglassEnergy:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsPartHourglassEnergy.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsPartHourglassEnergy:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsPartHourglassEnergy.
        """
        return self._outputs


class InputsPartHourglassEnergy(_Inputs):
    """Intermediate class used to connect user inputs to
    part_hourglass_energy operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(part_hourglass_energy._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            part_hourglass_energy._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            part_hourglass_energy._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._entity_scoping: Input[Scoping] = Input(
            part_hourglass_energy._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._entity_scoping)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            part_hourglass_energy._spec().input_pin(50), 50, op, -1
        )
...
class OutputsPartHourglassEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(part_hourglass_energy._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            part_hourglass_energy._spec().output_pin(0), 0, op
        )
