...
class interface_contact_area(Operator):
    r"""Read Interface Contact Area (LSDyna) by calling the readers defined by
    the datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Interface Contact Area (LSDyna) by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="R_CA", server=server)

    @property
    def inputs(self) -> InputsInterfaceContactArea:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsInterfaceContactArea.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsInterfaceContactArea:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsInterfaceContactArea.
        """
        return self._outputs


class InputsInterfaceContactArea(_Inputs):
    """Intermediate class used to connect user inputs to
    interface_contact_area operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(interface_contact_area._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            interface_contact_area._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            interface_contact_area._spec().input_pin(4), 4, op, -1
        )
...
class OutputsInterfaceContactArea(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(interface_contact_area._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            interface_contact_area._spec().output_pin(0), 0, op
        )
