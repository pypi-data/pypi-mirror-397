...
class compute_total_strain_XZ(Operator):
    ...
    def __init__(
        ...
        super().__init__(
            name="compute_total_strain_XZ",
            config=config,
            server=server,
            inputs_type=InputsComputeTotalStrainXz,
            outputs_type=OutputsComputeTotalStrainXz,
        )
...
class InputsComputeTotalStrainXz(_Inputs):
    """Intermediate class used to connect user inputs to
    compute_total_strain_XZ operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(compute_total_strain_XZ._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            compute_total_strain_XZ._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._scoping: Input[Scoping] = Input(
            compute_total_strain_XZ._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._scoping)
        self._streams_container: Input[StreamsContainer] = Input(
            compute_total_strain_XZ._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            compute_total_strain_XZ._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._extrapolate: Input[int] = Input(
            compute_total_strain_XZ._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._extrapolate)
        self._nonlinear: Input[int] = Input(
            compute_total_strain_XZ._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._nonlinear)
        self._abstract_meshed_region: Input[MeshedRegion] = Input(
            compute_total_strain_XZ._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._abstract_meshed_region)
        self._requested_location: Input[str] = Input(
            compute_total_strain_XZ._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._displacement: Input[FieldsContainer | Field] = Input(
            compute_total_strain_XZ._spec().input_pin(10), 10, op, -1
        )
    ...
    @property
    def extrapolate(self) -> Input[int]:
    ...
    @property
    def displacement(self) -> Input[FieldsContainer | Field]:
...
class OutputsComputeTotalStrainXz(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(compute_total_strain_XZ._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            compute_total_strain_XZ._spec().output_pin(0), 0, op
        )
