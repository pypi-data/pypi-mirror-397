...
class total_pressure(Operator):
    r"""Read Total Pressure by calling the readers defined by the datasources.
    ...
    >>> op = dpf.operators.result.total_pressure()
    ...
    def _spec() -> Specification:
        description = r"""Read Total Pressure by calling the readers defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="P_TOT", server=server)

    @property
    def inputs(self) -> InputsTotalPressure:
...
class OutputsTotalPressure(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(total_pressure._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            total_pressure._spec().output_pin(0), 0, op
        )
