...
class surface_heat_rate(Operator):
    r"""Read Surface Heat Rate by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.surface_heat_rate()
    ...
    def _spec() -> Specification:
        description = r"""Read Surface Heat Rate by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="Q", server=server)

    @property
    def inputs(self) -> InputsSurfaceHeatRate:
...
class OutputsSurfaceHeatRate(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(surface_heat_rate._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            surface_heat_rate._spec().output_pin(0), 0, op
        )
