...
class joint_relative_angular_velocity(Operator):
    r"""Read/compute joint relative angular velocity by calling the readers
    ...
    def _spec() -> Specification:
        description = r"""Read/compute joint relative angular velocity by calling the readers
defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="JOMG", server=server)

    @property
    def inputs(self) -> InputsJointRelativeAngularVelocity:
...
class InputsJointRelativeAngularVelocity(_Inputs):
    ...
    joint_relative_angular_velocity operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(joint_relative_angular_velocity._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            joint_relative_angular_velocity._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            joint_relative_angular_velocity._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            joint_relative_angular_velocity._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            joint_relative_angular_velocity._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            joint_relative_angular_velocity._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global: Input[bool] = Input(
            joint_relative_angular_velocity._spec().input_pin(5), 5, op, -1
        )
...
class OutputsJointRelativeAngularVelocity(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(joint_relative_angular_velocity._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            joint_relative_angular_velocity._spec().output_pin(0), 0, op
        )
