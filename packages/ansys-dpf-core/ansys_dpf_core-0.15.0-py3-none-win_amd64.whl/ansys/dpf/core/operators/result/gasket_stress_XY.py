...
class gasket_stress_XY(Operator):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsGasketStressXy,
            outputs_type=OutputsGasketStressXy,
        )
...
class InputsGasketStressXy(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            gasket_stress_XY._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            gasket_stress_XY._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            gasket_stress_XY._spec().input_pin(2), 2, op, -1
        )
        ...
        self._data_sources: Input[DataSources] = Input(
            gasket_stress_XY._spec().input_pin(4), 4, op, -1
        )
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            gasket_stress_XY._spec().input_pin(7), 7, op, -1
        )
        ...
        self._read_cyclic: Input[int] = Input(
            gasket_stress_XY._spec().input_pin(14), 14, op, -1
        )
...
class OutputsGasketStressXy(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            gasket_stress_XY._spec().output_pin(0), 0, op
        )
