...
class part_internal_energy(Operator):
    r"""Read Part Internal Energy (LSDyna) by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.part_internal_energy()
    ...
    def _spec() -> Specification:
        description = r"""Read Part Internal Energy (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="M_IE", server=server)

    @property
    def inputs(self) -> InputsPartInternalEnergy:
...
class OutputsPartInternalEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(part_internal_energy._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            part_internal_energy._spec().output_pin(0), 0, op
        )
