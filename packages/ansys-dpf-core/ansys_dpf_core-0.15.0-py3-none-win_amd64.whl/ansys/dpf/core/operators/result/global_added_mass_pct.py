...
class global_added_mass_pct(Operator):
    r"""Read Global Added Mass (percentage) (LSDyna) by calling the readers
    ...
    def _spec() -> Specification:
        description = r"""Read Global Added Mass (percentage) (LSDyna) by calling the readers
defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_AMP", server=server)

    @property
    def inputs(self) -> InputsGlobalAddedMassPct:
...
class InputsGlobalAddedMassPct(_Inputs):
    ...
    global_added_mass_pct operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(global_added_mass_pct._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_added_mass_pct._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_added_mass_pct._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_added_mass_pct._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalAddedMassPct(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_added_mass_pct._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_added_mass_pct._spec().output_pin(0), 0, op
        )
