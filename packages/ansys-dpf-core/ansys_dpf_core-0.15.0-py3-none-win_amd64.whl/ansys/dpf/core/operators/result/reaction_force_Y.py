...
class reaction_force_Y(Operator):
    r"""Read/compute nodal reaction forces Y component of the vector (2nd
    ...
    def _spec() -> Specification:
        description = r"""Read/compute nodal reaction forces Y component of the vector (2nd
component) by calling the readers defined by the datasources.
"""
        spec = Specification(
    ...
    @property
    def inputs(self) -> InputsReactionForceY:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsReactionForceY.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsReactionForceY:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsReactionForceY.
        """
...
class InputsReactionForceY(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            reaction_force_Y._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            reaction_force_Y._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            reaction_force_Y._spec().input_pin(2), 2, op, -1
        )
        ...
        self._data_sources: Input[DataSources] = Input(
            reaction_force_Y._spec().input_pin(4), 4, op, -1
        )
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            reaction_force_Y._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._read_cyclic: Input[int] = Input(
            reaction_force_Y._spec().input_pin(14), 14, op, -1
        )
...
class OutputsReactionForceY(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            reaction_force_Y._spec().output_pin(0), 0, op
        )
