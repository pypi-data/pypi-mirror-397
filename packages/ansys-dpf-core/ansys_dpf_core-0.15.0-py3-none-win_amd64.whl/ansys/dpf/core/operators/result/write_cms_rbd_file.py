...
class write_cms_rbd_file(Operator):
    ...
    def _spec() -> Specification:
        description = r"""Write the invariant terms and the model data in a cms_rbd file
"""
        spec = Specification(
            ...
            map_input_pin_spec={
                ...
                23: PinSpecification(
                    ...
                    type_names=["string"],
                    optional=False,
                    document=r"""path with cms_rbd extension where the export occurs""",
                ),
            },
            map_output_pin_spec={
                0: PinSpecification(
                    ...
                    optional=False,
                    document=r"""Data Source containing the cms_rbd file generated.""",
                ),
            },
        )
        return spec
    ...
    @property
    def inputs(self) -> InputsWriteCmsRbdFile:
...
class InputsWriteCmsRbdFile(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._v_trsf: Input = Input(write_cms_rbd_file._spec().input_pin(5), 5, op, -1)
        self._inputs.append(self._v_trsf)
        self._k_mat: Input[Field] = Input(
            write_cms_rbd_file._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._k_mat)
        self._mass_mat: Input[Field] = Input(
            write_cms_rbd_file._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mass_mat)
        self._c_mat: Input[Field] = Input(
            write_cms_rbd_file._spec().input_pin(8), 8, op, -1
        )
        self._inputs.append(self._c_mat)
        self._rhs: Input[Field] = Input(
            write_cms_rbd_file._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._rhs)
        self._dn: Input = Input(write_cms_rbd_file._spec().input_pin(10), 10, op, -1)
        self._inputs.append(self._dn)
        self._dr_cross_n: Input = Input(
            write_cms_rbd_file._spec().input_pin(11), 11, op, -1
        )
        self._inputs.append(self._dr_cross_n)
        self._drn: Input = Input(write_cms_rbd_file._spec().input_pin(12), 12, op, -1)
        self._inputs.append(self._drn)
        self._dn_cross_n: Input = Input(
            write_cms_rbd_file._spec().input_pin(13), 13, op, -1
        )
        self._inputs.append(self._dn_cross_n)
        self._dnx_y: Input = Input(write_cms_rbd_file._spec().input_pin(14), 14, op, -1)
        self._inputs.append(self._dnx_y)
        self._dny_y: Input = Input(write_cms_rbd_file._spec().input_pin(15), 15, op, -1)
        self._inputs.append(self._dny_y)
        self._dnz_y: Input = Input(write_cms_rbd_file._spec().input_pin(16), 16, op, -1)
        self._inputs.append(self._dnz_y)
        self._dyx_n: Input = Input(write_cms_rbd_file._spec().input_pin(17), 17, op, -1)
        self._inputs.append(self._dyx_n)
        self._dyy_n: Input = Input(write_cms_rbd_file._spec().input_pin(18), 18, op, -1)
        self._inputs.append(self._dyy_n)
        self._dyz_n: Input = Input(write_cms_rbd_file._spec().input_pin(19), 19, op, -1)
        self._inputs.append(self._dyz_n)
        self._dnxn: Input = Input(write_cms_rbd_file._spec().input_pin(20), 20, op, -1)
        self._inputs.append(self._dnxn)
        self._dnyn: Input = Input(write_cms_rbd_file._spec().input_pin(21), 21, op, -1)
        self._inputs.append(self._dnyn)
        self._dnzn: Input = Input(write_cms_rbd_file._spec().input_pin(22), 22, op, -1)
        self._inputs.append(self._dnzn)
        self._file_path: Input[str] = Input(
            write_cms_rbd_file._spec().input_pin(23), 23, op, -1
        )
...
class OutputsWriteCmsRbdFile(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._data_sources: Output[DataSources] = Output(
            write_cms_rbd_file._spec().output_pin(0), 0, op
        )
