...
class creep_strain_Y(Operator):
    ...
    >>> op = dpf.operators.result.creep_strain_Y()
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EPCRY", server=server)

    @property
    def inputs(self) -> InputsCreepStrainY:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsCreepStrainY.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsCreepStrainY:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsCreepStrainY.
        """
        return self._outputs


class InputsCreepStrainY(_Inputs):
    """Intermediate class used to connect user inputs to
    creep_strain_Y operator.
...
class OutputsCreepStrainY(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(creep_strain_Y._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            creep_strain_Y._spec().output_pin(0), 0, op
        )
