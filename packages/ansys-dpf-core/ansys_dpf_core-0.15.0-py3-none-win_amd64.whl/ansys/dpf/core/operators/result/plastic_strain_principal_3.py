...
class plastic_strain_principal_3(Operator):
    r"""Read/compute element nodal component plastic strains 3rd principal
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsPlasticStrainPrincipal3,
            outputs_type=OutputsPlasticStrainPrincipal3,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute element nodal component plastic strains 3rd principal
component by calling the readers defined by the datasources and
computing its eigen values. This operation is independent of the
coordinate system unless averaging across elements is requested, in
which case a rotation to the global coordinate system is performed. The
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EPPL3", server=server)

    @property
    def inputs(self) -> InputsPlasticStrainPrincipal3:
...
class InputsPlasticStrainPrincipal3(_Inputs):
    ...
    plastic_strain_principal_3 operator.
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            plastic_strain_principal_3._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            plastic_strain_principal_3._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            plastic_strain_principal_3._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            plastic_strain_principal_3._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            plastic_strain_principal_3._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global: Input[bool] = Input(
            plastic_strain_principal_3._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            plastic_strain_principal_3._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            plastic_strain_principal_3._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._read_cyclic: Input[int] = Input(
            plastic_strain_principal_3._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            plastic_strain_principal_3._spec().input_pin(22), 22, op, -1
        )
...
class OutputsPlasticStrainPrincipal3(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            plastic_strain_principal_3._spec().output_pin(0), 0, op
        )
