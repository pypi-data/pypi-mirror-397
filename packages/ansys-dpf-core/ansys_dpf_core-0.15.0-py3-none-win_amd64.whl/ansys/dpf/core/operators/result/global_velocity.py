...
class global_velocity(Operator):
    r"""Read Global Velocity (LSDyna) by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Global Velocity (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_V", server=server)

    @property
    def inputs(self) -> InputsGlobalVelocity:
...
class InputsGlobalVelocity(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container: Input[StreamsContainer] = Input(
            global_velocity._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_velocity._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_velocity._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalVelocity(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_velocity._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_velocity._spec().output_pin(0), 0, op
        )
