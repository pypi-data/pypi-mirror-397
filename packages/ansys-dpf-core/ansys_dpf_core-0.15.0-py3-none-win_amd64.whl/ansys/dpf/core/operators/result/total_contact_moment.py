...
class total_contact_moment(Operator):
    r"""Read/compute total contact moment by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read/compute total contact moment by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="CFTTOT", server=server)

    @property
    def inputs(self) -> InputsTotalContactMoment:
...
class OutputsTotalContactMoment(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(total_contact_moment._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            total_contact_moment._spec().output_pin(0), 0, op
        )
