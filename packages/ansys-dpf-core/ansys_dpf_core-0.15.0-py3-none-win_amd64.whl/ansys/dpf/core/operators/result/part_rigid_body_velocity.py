...
class part_rigid_body_velocity(Operator):
    r"""Read Part Rigid Body Velocity (LSDyna) by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.part_rigid_body_velocity()
    ...
    def _spec() -> Specification:
        description = r"""Read Part Rigid Body Velocity (LSDyna) by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="M_RBV", server=server)

    @property
    def inputs(self) -> InputsPartRigidBodyVelocity:
...
class InputsPartRigidBodyVelocity(_Inputs):
    ...
    part_rigid_body_velocity operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(part_rigid_body_velocity._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            part_rigid_body_velocity._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            part_rigid_body_velocity._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._entity_scoping: Input[Scoping] = Input(
            part_rigid_body_velocity._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._entity_scoping)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            part_rigid_body_velocity._spec().input_pin(50), 50, op, -1
        )
...
class OutputsPartRigidBodyVelocity(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(part_rigid_body_velocity._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            part_rigid_body_velocity._spec().output_pin(0), 0, op
        )
