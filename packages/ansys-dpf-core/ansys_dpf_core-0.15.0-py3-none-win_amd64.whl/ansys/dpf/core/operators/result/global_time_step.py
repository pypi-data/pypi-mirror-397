...
class global_time_step(Operator):
    r"""Read Global Time Step (LSDyna) by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.global_time_step()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsGlobalTimeStep,
            outputs_type=OutputsGlobalTimeStep,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read Global Time Step (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_DT", server=server)

    @property
    def inputs(self) -> InputsGlobalTimeStep:
...
class InputsGlobalTimeStep(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_time_step._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_time_step._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_time_step._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_time_step._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalTimeStep(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_time_step._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_time_step._spec().output_pin(0), 0, op
        )
