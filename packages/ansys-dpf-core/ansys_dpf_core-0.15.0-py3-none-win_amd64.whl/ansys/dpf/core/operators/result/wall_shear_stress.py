...
class wall_shear_stress(Operator):
    r"""Read Wall Shear Stress by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Wall Shear Stress by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="TAU", server=server)

    @property
    def inputs(self) -> InputsWallShearStress:
...
class OutputsWallShearStress(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(wall_shear_stress._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            wall_shear_stress._spec().output_pin(0), 0, op
        )
