...
class modal_coordinate(Operator):
    r"""Read/compute modal coordinate by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.modal_coordinate()
    ...
    def _spec() -> Specification:
        description = r"""Read/compute modal coordinate by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="MCF", server=server)

    @property
    def inputs(self) -> InputsModalCoordinate:
...
class InputsModalCoordinate(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(modal_coordinate._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            modal_coordinate._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._streams_container: Input[StreamsContainer] = Input(
            modal_coordinate._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            modal_coordinate._spec().input_pin(4), 4, op, -1
        )
...
class OutputsModalCoordinate(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(modal_coordinate._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            modal_coordinate._spec().output_pin(0), 0, op
        )
