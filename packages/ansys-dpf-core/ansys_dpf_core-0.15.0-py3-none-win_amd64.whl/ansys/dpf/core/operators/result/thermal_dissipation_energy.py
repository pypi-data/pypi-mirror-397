...
class thermal_dissipation_energy(Operator):
    r"""Read/compute thermal dissipation energy by calling the readers defined
    by the datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read/compute thermal dissipation energy by calling the readers defined
by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="ENG_TH", server=server)

    @property
    def inputs(self) -> InputsThermalDissipationEnergy:
...
class InputsThermalDissipationEnergy(_Inputs):
    ...
    thermal_dissipation_energy operator.
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            thermal_dissipation_energy._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            thermal_dissipation_energy._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            thermal_dissipation_energy._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            thermal_dissipation_energy._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            thermal_dissipation_energy._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            thermal_dissipation_energy._spec().input_pin(7), 7, op, -1
        )
...
class OutputsThermalDissipationEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            thermal_dissipation_energy._spec().output_pin(0), 0, op
        )
