...
class InputsForceSummationPsd(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._nodal_scoping: Input[Scoping | ScopingsContainer] = Input(
            force_summation_psd._spec().input_pin(1), 1, op, -1
        )
        ...
        self._streams: Input[StreamsContainer] = Input(
            force_summation_psd._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams)
        self._data_sources: Input[DataSources] = Input(
            force_summation_psd._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._spoint: Input[Field | FieldsContainer] = Input(
            force_summation_psd._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._spoint)
        self._abs_rel_key: Input[int] = Input(
            force_summation_psd._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._abs_rel_key)
        self._signif: Input[float] = Input(
            force_summation_psd._spec().input_pin(8), 8, op, -1
        )
    ...
    @property
    def abs_rel_key(self) -> Input[int]:
