...
class window_hanning(Operator):
    ...
    >>> op = dpf.operators.math.window_hanning()
...
class InputsWindowHanning(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(window_hanning._spec().inputs, op)
        self._field: Input[Field | FieldsContainer] = Input(
            window_hanning._spec().input_pin(0), 0, op, -1
        )
...
class OutputsWindowHanning(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(window_hanning._spec().outputs, op)
        self.field_as_field = Output(
            _modify_output_spec_with_one_type(
                window_hanning._spec().output_pin(0), "field"
            ),
            0,
            op,
        )
        ...
        self.field_as_fields_container = Output(
            _modify_output_spec_with_one_type(
                window_hanning._spec().output_pin(0), "fields_container"
            ),
            0,
            op,
        )
