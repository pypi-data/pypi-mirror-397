...
class window_bartlett(Operator):
    ...
    >>> op = dpf.operators.math.window_bartlett()
...
class InputsWindowBartlett(_Inputs):
    """Intermediate class used to connect user inputs to
    window_bartlett operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(window_bartlett._spec().inputs, op)
        self._field: Input[Field | FieldsContainer] = Input(
            window_bartlett._spec().input_pin(0), 0, op, -1
        )
...
class OutputsWindowBartlett(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(window_bartlett._spec().outputs, op)
        self.field_as_field = Output(
            _modify_output_spec_with_one_type(
                window_bartlett._spec().output_pin(0), "field"
            ),
            0,
            op,
        )
        ...
        self.field_as_fields_container = Output(
            _modify_output_spec_with_one_type(
                window_bartlett._spec().output_pin(0), "fields_container"
            ),
            0,
            op,
        )
