...
class field_signed_high_pass(Operator):
    ...
    def __init__(self, field=None, threshold=None, both=None, config=None, server=None):
        super().__init__(
            name="core::field::signed_high_pass",
            config=config,
            server=server,
            inputs_type=InputsFieldSignedHighPass,
            outputs_type=OutputsFieldSignedHighPass,
        )
...
class InputsFieldSignedHighPass(_Inputs):
    """Intermediate class used to connect user inputs to
    field_signed_high_pass operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(field_signed_high_pass._spec().inputs, op)
        self._field: Input[Field | FieldsContainer] = Input(
            field_signed_high_pass._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._field)
        self._threshold: Input[float | Field] = Input(
            field_signed_high_pass._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._threshold)
        self._both: Input[bool] = Input(
            field_signed_high_pass._spec().input_pin(2), 2, op, -1
        )
...
class OutputsFieldSignedHighPass(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(field_signed_high_pass._spec().outputs, op)
        self._field: Output[Field] = Output(
            field_signed_high_pass._spec().output_pin(0), 0, op
        )
