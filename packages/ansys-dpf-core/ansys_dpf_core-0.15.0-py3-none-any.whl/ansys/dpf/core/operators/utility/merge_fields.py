...
class InputsMergeFields(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._sum_merge: Input[bool] = Input(
            merge_fields._spec().input_pin(-2), -2, op, -1
        )
        self._inputs.append(self._sum_merge)
        self._merged_support: Input = Input(
            merge_fields._spec().input_pin(-1), -1, op, -1
        )
        self._inputs.append(self._merged_support)
        self._fields1: Input[Field | FieldsContainer] = Input(
            merge_fields._spec().input_pin(0), 0, op, 0
        )
...
class OutputsMergeFields(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._merged_field: Output[Field] = Output(
            merge_fields._spec().output_pin(0), 0, op
        )
        self._outputs.append(self._merged_field)

    @property
    def merged_field(self) -> Output[Field]:
