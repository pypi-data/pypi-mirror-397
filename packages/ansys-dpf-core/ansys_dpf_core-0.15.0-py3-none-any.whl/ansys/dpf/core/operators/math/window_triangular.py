...
class window_triangular(Operator):
    ...
    >>> op = dpf.operators.math.window_triangular()
...
class InputsWindowTriangular(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(window_triangular._spec().inputs, op)
        self._field: Input[Field | FieldsContainer] = Input(
            window_triangular._spec().input_pin(0), 0, op, -1
        )
...
class OutputsWindowTriangular(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(window_triangular._spec().outputs, op)
        self.field_as_field = Output(
            _modify_output_spec_with_one_type(
                window_triangular._spec().output_pin(0), "field"
            ),
            0,
            op,
        )
        ...
        self.field_as_fields_container = Output(
            _modify_output_spec_with_one_type(
                window_triangular._spec().output_pin(0), "fields_container"
            ),
            0,
            op,
        )
