...
class global_rigid_body_stopper_energy(Operator):
    r"""Read Global Rigid Body Stopper Energy (LSDyna) by calling the readers
    ...
    >>> op = dpf.operators.result.global_rigid_body_stopper_energy()
    ...
    def _spec() -> Specification:
        description = r"""Read Global Rigid Body Stopper Energy (LSDyna) by calling the readers
defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_RBE", server=server)

    @property
    def inputs(self) -> InputsGlobalRigidBodyStopperEnergy:
...
class InputsGlobalRigidBodyStopperEnergy(_Inputs):
    ...
    global_rigid_body_stopper_energy operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(global_rigid_body_stopper_energy._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_rigid_body_stopper_energy._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_rigid_body_stopper_energy._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_rigid_body_stopper_energy._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalRigidBodyStopperEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_rigid_body_stopper_energy._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_rigid_body_stopper_energy._spec().output_pin(0), 0, op
        )
