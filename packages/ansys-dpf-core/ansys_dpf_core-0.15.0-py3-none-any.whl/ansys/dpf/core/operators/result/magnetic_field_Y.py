...
class magnetic_field_Y(Operator):
    r"""Read/compute Magnetic Field Y component of the vector (2nd component) by
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsMagneticFieldY,
            outputs_type=OutputsMagneticFieldY,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute Magnetic Field Y component of the vector (2nd component) by
calling the readers defined by the datasources. Regarding the requested
location and the input mesh scoping, the result location can be
Nodal/ElementalNodal/Elemental.
"""
    ...
    @property
    def inputs(self) -> InputsMagneticFieldY:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsMagneticFieldY.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsMagneticFieldY:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsMagneticFieldY.
        """
...
class InputsMagneticFieldY(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            magnetic_field_Y._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            magnetic_field_Y._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            magnetic_field_Y._spec().input_pin(2), 2, op, -1
        )
        ...
        self._data_sources: Input[DataSources] = Input(
            magnetic_field_Y._spec().input_pin(4), 4, op, -1
        )
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            magnetic_field_Y._spec().input_pin(7), 7, op, -1
        )
        ...
        self._read_cyclic: Input[int] = Input(
            magnetic_field_Y._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            magnetic_field_Y._spec().input_pin(22), 22, op, -1
        )
...
class OutputsMagneticFieldY(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            magnetic_field_Y._spec().output_pin(0), 0, op
        )
