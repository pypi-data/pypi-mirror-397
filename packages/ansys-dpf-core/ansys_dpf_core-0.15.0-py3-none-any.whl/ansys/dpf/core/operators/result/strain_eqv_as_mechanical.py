...
class strain_eqv_as_mechanical(Operator):
    r"""Computes the equivalent (Von Mises) elastic strains and averages it to
    the nodes (by default). For multibody simulations, averaging across
    ...
    def _spec() -> Specification:
        description = r"""Computes the equivalent (Von Mises) elastic strains and averages it to
the nodes (by default). For multibody simulations, averaging across
bodies can either be activated or deactivated.
"""
        spec = Specification(
    ...
    @property
    def inputs(self) -> InputsStrainEqvAsMechanical:
...
class InputsStrainEqvAsMechanical(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            strain_eqv_as_mechanical._spec().input_pin(7), 7, op, -1
        )
        ...
        self._poisson_ratio: Input[int | float] = Input(
    ...
    @property
    def poisson_ratio(self) -> Input[int | float]:
