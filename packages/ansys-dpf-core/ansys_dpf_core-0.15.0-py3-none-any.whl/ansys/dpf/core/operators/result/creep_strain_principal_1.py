...
class creep_strain_principal_1(Operator):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsCreepStrainPrincipal1,
            outputs_type=OutputsCreepStrainPrincipal1,
        )
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EPCR1", server=server)

    @property
    def inputs(self) -> InputsCreepStrainPrincipal1:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsCreepStrainPrincipal1.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsCreepStrainPrincipal1:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsCreepStrainPrincipal1.
        """
        return self._outputs


class InputsCreepStrainPrincipal1(_Inputs):
    """Intermediate class used to connect user inputs to
    creep_strain_principal_1 operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(creep_strain_principal_1._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            creep_strain_principal_1._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            creep_strain_principal_1._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            creep_strain_principal_1._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            creep_strain_principal_1._spec().input_pin(3), 3, op, -1
        )
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            creep_strain_principal_1._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            creep_strain_principal_1._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._read_cyclic: Input[int] = Input(
            creep_strain_principal_1._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            creep_strain_principal_1._spec().input_pin(22), 22, op, -1
        )
...
class OutputsCreepStrainPrincipal1(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(creep_strain_principal_1._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            creep_strain_principal_1._spec().output_pin(0), 0, op
        )
