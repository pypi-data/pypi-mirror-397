...
class magnetic_vector_potential(Operator):
    r"""Read/compute Magnetic Vector Potential by calling the readers defined by
    the datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read/compute Magnetic Vector Potential by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="MVP", server=server)

    @property
    def inputs(self) -> InputsMagneticVectorPotential:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsMagneticVectorPotential.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsMagneticVectorPotential:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsMagneticVectorPotential.
        """
        return self._outputs


class InputsMagneticVectorPotential(_Inputs):
    """Intermediate class used to connect user inputs to
    magnetic_vector_potential operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(magnetic_vector_potential._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            magnetic_vector_potential._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            magnetic_vector_potential._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            magnetic_vector_potential._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            magnetic_vector_potential._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            magnetic_vector_potential._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            magnetic_vector_potential._spec().input_pin(7), 7, op, -1
        )
...
class OutputsMagneticVectorPotential(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(magnetic_vector_potential._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            magnetic_vector_potential._spec().output_pin(0), 0, op
        )
