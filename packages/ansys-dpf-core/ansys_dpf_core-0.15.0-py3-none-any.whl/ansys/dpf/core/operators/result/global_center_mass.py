...
class global_center_mass(Operator):
    r"""Read Global Center of Mass (LSDyna) by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.global_center_mass()
    ...
    def _spec() -> Specification:
        description = r"""Read Global Center of Mass (LSDyna) by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_XCM", server=server)

    @property
    def inputs(self) -> InputsGlobalCenterMass:
...
class InputsGlobalCenterMass(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_center_mass._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_center_mass._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_center_mass._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_center_mass._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalCenterMass(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_center_mass._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_center_mass._spec().output_pin(0), 0, op
        )
