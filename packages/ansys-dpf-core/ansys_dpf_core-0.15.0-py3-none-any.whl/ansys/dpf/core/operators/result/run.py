...
class InputsRun(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._mapdl_exe_path: Input[str] = Input(run._spec().input_pin(0), 0, op, -1)
        self._inputs.append(self._mapdl_exe_path)
        self._working_dir: Input[str] = Input(run._spec().input_pin(1), 1, op, -1)
        self._inputs.append(self._working_dir)
        self._number_of_processes: Input[int] = Input(
            run._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._number_of_processes)
        self._number_of_threads: Input[int] = Input(run._spec().input_pin(3), 3, op, -1)
        self._inputs.append(self._number_of_threads)
        self._data_sources: Input[DataSources] = Input(
            run._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._server_mode: Input[bool] = Input(run._spec().input_pin(5), 5, op, -1)
        self._inputs.append(self._server_mode)

    @property
    def mapdl_exe_path(self) -> Input[str]:
    ...
    @property
    def working_dir(self) -> Input[str]:
    ...
    @property
    def number_of_processes(self) -> Input[int]:
    ...
    @property
    def number_of_threads(self) -> Input[int]:
    ...
    @property
    def server_mode(self) -> Input[bool]:
...
class OutputsRun(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._data_sources: Output[DataSources] = Output(
            run._spec().output_pin(0), 0, op
        )
        self._outputs.append(self._data_sources)
        self._ip: Output[str] = Output(run._spec().output_pin(1), 1, op)
        self._outputs.append(self._ip)
        self._port: Output[str] = Output(run._spec().output_pin(2), 2, op)
