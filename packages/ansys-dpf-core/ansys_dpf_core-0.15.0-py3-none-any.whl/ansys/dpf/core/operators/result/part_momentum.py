...
class part_momentum(Operator):
    r"""Read Part Momentum (LSDyna) by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Part Momentum (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="M_MV", server=server)

    @property
    def inputs(self) -> InputsPartMomentum:
...
class OutputsPartMomentum(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(part_momentum._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            part_momentum._spec().output_pin(0), 0, op
        )
