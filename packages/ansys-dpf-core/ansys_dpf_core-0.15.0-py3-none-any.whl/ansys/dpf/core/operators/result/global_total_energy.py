...
class global_total_energy(Operator):
    r"""Read Global Total Energy (LSDyna) by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Global Total Energy (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_ENG_TE", server=server)

    @property
    def inputs(self) -> InputsGlobalTotalEnergy:
...
class InputsGlobalTotalEnergy(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container: Input[StreamsContainer] = Input(
            global_total_energy._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_total_energy._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_total_energy._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalTotalEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            global_total_energy._spec().output_pin(0), 0, op
        )
