...
class thermal_strain_principal_3(Operator):
    r"""Read/compute element nodal component thermal strains 3rd principal
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsThermalStrainPrincipal3,
            outputs_type=OutputsThermalStrainPrincipal3,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute element nodal component thermal strains 3rd principal
component by calling the readers defined by the datasources and
computing its eigen values. This operation is independent of the
coordinate system unless averaging across elements is requested, in
which case a rotation to the global coordinate system is performed. The
...
"""
    ...
    @property
    def inputs(self) -> InputsThermalStrainPrincipal3:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsThermalStrainPrincipal3.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsThermalStrainPrincipal3:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsThermalStrainPrincipal3.
        """
        return self._outputs


class InputsThermalStrainPrincipal3(_Inputs):
    """Intermediate class used to connect user inputs to
    thermal_strain_principal_3 operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(thermal_strain_principal_3._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            thermal_strain_principal_3._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            thermal_strain_principal_3._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            thermal_strain_principal_3._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            thermal_strain_principal_3._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            thermal_strain_principal_3._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global: Input[bool] = Input(
            thermal_strain_principal_3._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            thermal_strain_principal_3._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            thermal_strain_principal_3._spec().input_pin(9), 9, op, -1
        )
        ...
        self._read_beams: Input[bool] = Input(
            thermal_strain_principal_3._spec().input_pin(22), 22, op, -1
        )
...
class OutputsThermalStrainPrincipal3(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(thermal_strain_principal_3._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            thermal_strain_principal_3._spec().output_pin(0), 0, op
        )
