...
class compute_stress_Y(Operator):
    ...
    def __init__(
        ...
        super().__init__(
            name="compute_stress_Y",
            config=config,
            server=server,
            inputs_type=InputsComputeStressY,
            outputs_type=OutputsComputeStressY,
        )
...
class InputsComputeStressY(_Inputs):
    """Intermediate class used to connect user inputs to
    compute_stress_Y operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(compute_stress_Y._spec().inputs, op)
        self._scoping: Input[Scoping] = Input(
            compute_stress_Y._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._scoping)
        self._streams_container: Input[StreamsContainer] = Input(
            compute_stress_Y._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            compute_stress_Y._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._requested_location: Input[str] = Input(
            compute_stress_Y._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._strain: Input[FieldsContainer | Field] = Input(
            compute_stress_Y._spec().input_pin(10), 10, op, -1
        )
...
class OutputsComputeStressY(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(compute_stress_Y._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            compute_stress_Y._spec().output_pin(0), 0, op
        )
