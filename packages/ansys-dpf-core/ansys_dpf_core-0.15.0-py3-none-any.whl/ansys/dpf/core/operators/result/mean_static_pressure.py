...
class mean_static_pressure(Operator):
    r"""Read Mean Static Pressure by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.mean_static_pressure()
    ...
    def _spec() -> Specification:
        description = r"""Read Mean Static Pressure by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="P_SA", server=server)

    @property
    def inputs(self) -> InputsMeanStaticPressure:
...
class OutputsMeanStaticPressure(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(mean_static_pressure._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            mean_static_pressure._spec().output_pin(0), 0, op
        )
