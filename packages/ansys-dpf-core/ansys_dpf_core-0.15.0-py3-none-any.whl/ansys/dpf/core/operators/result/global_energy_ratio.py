...
class global_energy_ratio(Operator):
    r"""Read Global Energy Ratio (LSDyna) by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.global_energy_ratio()
    ...
    def _spec() -> Specification:
        description = r"""Read Global Energy Ratio (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_ER", server=server)

    @property
    def inputs(self) -> InputsGlobalEnergyRatio:
...
class InputsGlobalEnergyRatio(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_energy_ratio._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_energy_ratio._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_energy_ratio._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_energy_ratio._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalEnergyRatio(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_energy_ratio._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_energy_ratio._spec().output_pin(0), 0, op
        )
