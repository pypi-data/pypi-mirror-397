...
class turbulent_viscosity(Operator):
    r"""Read Turbulent Viscosity by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Turbulent Viscosity by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="MUT", server=server)

    @property
    def inputs(self) -> InputsTurbulentViscosity:
...
class InputsTurbulentViscosity(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._qualifiers2: Input[dict] = Input(
            turbulent_viscosity._spec().input_pin(1001), 1001, op, 1
        )
...
class OutputsTurbulentViscosity(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(turbulent_viscosity._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            turbulent_viscosity._spec().output_pin(0), 0, op
        )
