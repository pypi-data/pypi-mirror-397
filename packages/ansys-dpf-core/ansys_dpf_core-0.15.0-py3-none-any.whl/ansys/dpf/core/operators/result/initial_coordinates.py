...
class initial_coordinates(Operator):
    r"""Read/compute Initial Coordinates (LSDyna) by calling the readers defined
    by the datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read/compute Initial Coordinates (LSDyna) by calling the readers defined
by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="XI", server=server)

    @property
    def inputs(self) -> InputsInitialCoordinates:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsInitialCoordinates.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsInitialCoordinates:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsInitialCoordinates.
        """
        return self._outputs


class InputsInitialCoordinates(_Inputs):
    """Intermediate class used to connect user inputs to
    initial_coordinates operator.
...
class OutputsInitialCoordinates(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(initial_coordinates._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            initial_coordinates._spec().output_pin(0), 0, op
        )
