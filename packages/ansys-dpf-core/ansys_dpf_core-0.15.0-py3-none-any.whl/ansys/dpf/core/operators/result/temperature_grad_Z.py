...
class temperature_grad_Z(Operator):
    r"""Read/compute Temperature Gradient Z component of the vector (3rd
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsTemperatureGradZ,
            outputs_type=OutputsTemperatureGradZ,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute Temperature Gradient Z component of the vector (3rd
component) by calling the readers defined by the datasources. Regarding
the requested location and the input mesh scoping, the result location
can be Nodal/ElementalNodal/Elemental.
"""
    ...
    @property
    def inputs(self) -> InputsTemperatureGradZ:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsTemperatureGradZ.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsTemperatureGradZ:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsTemperatureGradZ.
        """
...
class InputsTemperatureGradZ(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            temperature_grad_Z._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            temperature_grad_Z._spec().input_pin(1), 1, op, -1
        )
        ...
        self._data_sources: Input[DataSources] = Input(
            temperature_grad_Z._spec().input_pin(4), 4, op, -1
        )
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            temperature_grad_Z._spec().input_pin(7), 7, op, -1
        )
        ...
        self._read_cyclic: Input[int] = Input(
            temperature_grad_Z._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            temperature_grad_Z._spec().input_pin(22), 22, op, -1
        )
...
class OutputsTemperatureGradZ(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            temperature_grad_Z._spec().output_pin(0), 0, op
        )
