...
class part_eroded_internal_energy(Operator):
    r"""Read Part Eroded Internal Energy (LSDyna) by calling the readers defined
    by the datasources.
    ...
    >>> op = dpf.operators.result.part_eroded_internal_energy()
    ...
    def _spec() -> Specification:
        description = r"""Read Part Eroded Internal Energy (LSDyna) by calling the readers defined
by the datasources.
"""
        spec = Specification(
            ...
            map_input_pin_spec={
                ...
                6: PinSpecification(
                    ...
                    type_names=["scoping"],
                    optional=True,
                    document=r"""entity (part for matsum, interface for rcforc) where the result will be scoped""",
                ),
                50: PinSpecification(
                    name="unit_system",
                    type_names=[
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="M_ERIE", server=server)

    @property
    def inputs(self) -> InputsPartErodedInternalEnergy:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsPartErodedInternalEnergy.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsPartErodedInternalEnergy:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsPartErodedInternalEnergy.
        """
        return self._outputs


class InputsPartErodedInternalEnergy(_Inputs):
    """Intermediate class used to connect user inputs to
    part_eroded_internal_energy operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(part_eroded_internal_energy._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            part_eroded_internal_energy._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            part_eroded_internal_energy._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._entity_scoping: Input[Scoping] = Input(
            part_eroded_internal_energy._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._entity_scoping)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            part_eroded_internal_energy._spec().input_pin(50), 50, op, -1
        )
    ...
    @property
    def entity_scoping(self) -> Input[Scoping]:
        r"""Allows to connect entity_scoping input to the operator.
...
class OutputsPartErodedInternalEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(part_eroded_internal_energy._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            part_eroded_internal_energy._spec().output_pin(0), 0, op
        )
