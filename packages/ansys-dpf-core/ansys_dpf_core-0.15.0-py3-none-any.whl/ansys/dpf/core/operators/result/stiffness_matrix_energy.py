...
class stiffness_matrix_energy(Operator):
    r"""Read/compute element energy associated with the stiffness matrix by
    ...
    def _spec() -> Specification:
        description = r"""Read/compute element energy associated with the stiffness matrix by
calling the readers defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="ENG_SE", server=server)

    @property
    def inputs(self) -> InputsStiffnessMatrixEnergy:
...
class InputsStiffnessMatrixEnergy(_Inputs):
    ...
    stiffness_matrix_energy operator.
    ...
    def __init__(self, op: Operator):
        ...
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            stiffness_matrix_energy._spec().input_pin(7), 7, op, -1
        )
        ...
        self._phi: Input[float] = Input(
            stiffness_matrix_energy._spec().input_pin(19), 19, op, -1
        )
...
class OutputsStiffnessMatrixEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            stiffness_matrix_energy._spec().output_pin(0), 0, op
        )
