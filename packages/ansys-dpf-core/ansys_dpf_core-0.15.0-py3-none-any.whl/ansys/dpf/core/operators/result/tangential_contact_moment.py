...
class tangential_contact_moment(Operator):
    r"""Read/compute tangential contact moment by calling the readers defined by
    the datasources.
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsTangentialContactMoment,
            outputs_type=OutputsTangentialContactMoment,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute tangential contact moment by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="CFTTAN", server=server)

    @property
    def inputs(self) -> InputsTangentialContactMoment:
...
class InputsTangentialContactMoment(_Inputs):
    ...
    tangential_contact_moment operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(tangential_contact_moment._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            tangential_contact_moment._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            tangential_contact_moment._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            tangential_contact_moment._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            tangential_contact_moment._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            tangential_contact_moment._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global: Input[bool] = Input(
            tangential_contact_moment._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            tangential_contact_moment._spec().input_pin(7), 7, op, -1
        )
...
class OutputsTangentialContactMoment(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(tangential_contact_moment._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            tangential_contact_moment._spec().output_pin(0), 0, op
        )
