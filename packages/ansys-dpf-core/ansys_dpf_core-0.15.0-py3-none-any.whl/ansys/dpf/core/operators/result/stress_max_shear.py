...
class stress_max_shear(Operator):
    ...
    >>> op = dpf.operators.result.stress_max_shear()
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="S_max_shear", server=server)

    @property
    def inputs(self) -> InputsStressMaxShear:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsStressMaxShear.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsStressMaxShear:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsStressMaxShear.
        """
        return self._outputs


class InputsStressMaxShear(_Inputs):
    """Intermediate class used to connect user inputs to
    stress_max_shear operator.
...
class OutputsStressMaxShear(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(stress_max_shear._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            stress_max_shear._spec().output_pin(0), 0, op
        )
