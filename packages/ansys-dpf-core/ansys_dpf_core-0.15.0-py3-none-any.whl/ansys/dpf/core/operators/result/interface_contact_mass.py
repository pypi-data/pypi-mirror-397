...
class interface_contact_mass(Operator):
    r"""Read Interface Contact Mass (LSDyna) by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.interface_contact_mass()
    ...
    def _spec() -> Specification:
        description = r"""Read Interface Contact Mass (LSDyna) by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="R_CMass", server=server)

    @property
    def inputs(self) -> InputsInterfaceContactMass:
...
class InputsInterfaceContactMass(_Inputs):
    ...
    interface_contact_mass operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(interface_contact_mass._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            interface_contact_mass._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            interface_contact_mass._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._entity_scoping: Input[Scoping] = Input(
            interface_contact_mass._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._entity_scoping)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            interface_contact_mass._spec().input_pin(50), 50, op, -1
        )
...
class OutputsInterfaceContactMass(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(interface_contact_mass._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            interface_contact_mass._spec().output_pin(0), 0, op
        )
