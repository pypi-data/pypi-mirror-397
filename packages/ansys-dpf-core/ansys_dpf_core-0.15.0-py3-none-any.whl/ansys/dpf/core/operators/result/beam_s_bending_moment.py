...
class InputsBeamSBendingMoment(_Inputs):
    ...
    beam_s_bending_moment operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(beam_s_bending_moment._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            beam_s_bending_moment._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            beam_s_bending_moment._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._streams_container: Input[StreamsContainer] = Input(
            beam_s_bending_moment._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            beam_s_bending_moment._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            beam_s_bending_moment._spec().input_pin(50), 50, op, -1
        )
...
class OutputsBeamSBendingMoment(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(beam_s_bending_moment._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            beam_s_bending_moment._spec().output_pin(0), 0, op
        )
