...
class normal_contact_moment(Operator):
    r"""Read/compute normal contact moment by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read/compute normal contact moment by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="CFTNOR", server=server)

    @property
    def inputs(self) -> InputsNormalContactMoment:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsNormalContactMoment.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsNormalContactMoment:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsNormalContactMoment.
        """
        return self._outputs


class InputsNormalContactMoment(_Inputs):
    """Intermediate class used to connect user inputs to
    normal_contact_moment operator.
...
class OutputsNormalContactMoment(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(normal_contact_moment._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            normal_contact_moment._spec().output_pin(0), 0, op
        )
