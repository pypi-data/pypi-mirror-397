...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EL", server=server)

    @property
    def inputs(self) -> InputsTotalStrain:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsTotalStrain.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsTotalStrain:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsTotalStrain.
        """
        return self._outputs


class InputsTotalStrain(_Inputs):
    """Intermediate class used to connect user inputs to
    total_strain operator.
...
class OutputsTotalStrain(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(total_strain._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            total_strain._spec().output_pin(0), 0, op
        )
