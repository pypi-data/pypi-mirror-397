...
class global_external_work(Operator):
    r"""Read Global External Work (LSDyna) by calling the readers defined by the
    datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read Global External Work (LSDyna) by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_EW", server=server)

    @property
    def inputs(self) -> InputsGlobalExternalWork:
...
class InputsGlobalExternalWork(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_external_work._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_external_work._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_external_work._spec().input_pin(4), 4, op, -1
        )
...
class OutputsGlobalExternalWork(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_external_work._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_external_work._spec().output_pin(0), 0, op
        )
