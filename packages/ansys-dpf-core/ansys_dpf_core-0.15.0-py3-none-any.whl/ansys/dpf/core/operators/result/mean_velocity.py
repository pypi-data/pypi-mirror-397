...
class mean_velocity(Operator):
    r"""Read Mean Velocity by calling the readers defined by the datasources.
    ...
    >>> op = dpf.operators.result.mean_velocity()
    ...
    def _spec() -> Specification:
        description = r"""Read Mean Velocity by calling the readers defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="V_A", server=server)

    @property
    def inputs(self) -> InputsMeanVelocity:
...
class OutputsMeanVelocity(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(mean_velocity._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            mean_velocity._spec().output_pin(0), 0, op
        )
