...
class pressure(Operator):
    r"""Read/compute Pressure by calling the readers defined by the datasources.
    ...
    def _spec() -> Specification:
        description = r"""Read/compute Pressure by calling the readers defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="Pressure", server=server)

    @property
    def inputs(self) -> InputsPressure:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsPressure.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsPressure:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsPressure.
        """
        return self._outputs


class InputsPressure(_Inputs):
...
class OutputsPressure(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(pressure._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            pressure._spec().output_pin(0), 0, op
        )
