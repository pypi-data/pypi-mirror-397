...
class global_eroded_hourglass_energy(Operator):
    r"""Read Global Eroded Hourglass Energy (LSDyna) by calling the readers
    ...
    >>> op = dpf.operators.result.global_eroded_hourglass_energy()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsGlobalErodedHourglassEnergy,
            outputs_type=OutputsGlobalErodedHourglassEnergy,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read Global Eroded Hourglass Energy (LSDyna) by calling the readers
defined by the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GLOB_ENG_ERAHO", server=server)

    @property
    def inputs(self) -> InputsGlobalErodedHourglassEnergy:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsGlobalErodedHourglassEnergy.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsGlobalErodedHourglassEnergy:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsGlobalErodedHourglassEnergy.
        """
        return self._outputs


class InputsGlobalErodedHourglassEnergy(_Inputs):
    """Intermediate class used to connect user inputs to
    global_eroded_hourglass_energy operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(global_eroded_hourglass_energy._spec().inputs, op)
        self._streams_container: Input[StreamsContainer] = Input(
            global_eroded_hourglass_energy._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            global_eroded_hourglass_energy._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._unit_system: Input[int | str | UnitSystem] = Input(
            global_eroded_hourglass_energy._spec().input_pin(50), 50, op, -1
        )
...
class OutputsGlobalErodedHourglassEnergy(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(global_eroded_hourglass_energy._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            global_eroded_hourglass_energy._spec().output_pin(0), 0, op
        )
