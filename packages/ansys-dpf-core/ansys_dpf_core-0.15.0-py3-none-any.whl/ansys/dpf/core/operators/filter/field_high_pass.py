...
class InputsFieldHighPass(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._field: Input[Field | FieldsContainer] = Input(
            field_high_pass._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._field)
        self._threshold: Input[float | Field] = Input(
            field_high_pass._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._threshold)
        self._both: Input[bool] = Input(field_high_pass._spec().input_pin(2), 2, op, -1)
...
class OutputsFieldHighPass(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._field: Output[Field] = Output(
            field_high_pass._spec().output_pin(0), 0, op
        )
