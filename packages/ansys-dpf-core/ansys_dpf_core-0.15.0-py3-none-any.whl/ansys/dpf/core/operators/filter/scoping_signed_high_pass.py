...
class InputsScopingSignedHighPass(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(scoping_signed_high_pass._spec().inputs, op)
        self._field: Input[Field | FieldsContainer] = Input(
            scoping_signed_high_pass._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._field)
        self._threshold: Input[float | Field] = Input(
            scoping_signed_high_pass._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._threshold)
        self._both: Input[bool] = Input(
            scoping_signed_high_pass._spec().input_pin(2), 2, op, -1
        )
...
class OutputsScopingSignedHighPass(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(scoping_signed_high_pass._spec().outputs, op)
        self._scoping: Output[Scoping] = Output(
            scoping_signed_high_pass._spec().output_pin(0), 0, op
        )
