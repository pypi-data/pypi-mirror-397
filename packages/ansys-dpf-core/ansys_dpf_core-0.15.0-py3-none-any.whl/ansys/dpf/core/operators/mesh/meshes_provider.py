...
class InputsMeshesProvider(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int] = Input(
            meshes_provider._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._streams_container: Input[StreamsContainer] = Input(
            meshes_provider._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            meshes_provider._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._read_cyclic: Input[int] = Input(
            meshes_provider._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._region_scoping: Input[Scoping | int] = Input(
            meshes_provider._spec().input_pin(25), 25, op, -1
        )
...
class OutputsMeshesProvider(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._meshes: Output[MeshesContainer] = Output(
            meshes_provider._spec().output_pin(0), 0, op
        )
