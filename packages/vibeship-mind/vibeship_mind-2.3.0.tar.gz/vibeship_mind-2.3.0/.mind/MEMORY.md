...
learned: Found: Phases 6-9 are fully IMPLEMENTED in code but NOT WIRED into mind_recall(). Functions exist in self_improve.py, but feature flags missing and some integrations incomplete.
fixed: Wired Phases 6-9 of self-improvement: decay (filter_by_confidence in context), reinforcement (type=reinforce in mind_log), contradiction (add_pattern_with_contradiction_check), learning_style (promote during session gap). All 101 tests pass.
learned: Python scoping gotcha: conditional imports inside functions make the name LOCAL to the entire function. If the condition is False, the local is never assigned but any later use fails with "cannot access local variable". Fix: don't re-import inside functions if already imported at top level.
problem: found bug: mind init uses "." which resolves to vibeship-mind due to uv --directory flag, not the user's actual cwd
fixed: fixed multi-project init: CLI now requires explicit path when using uv --directory, updated README with correct examples
fixed: published mind-memory v2.1.0 to PyPI - now installable via pip install mind-memory
fixed: published vibeship-mind to PyPI - install with pip install vibeship-mind, init with mind init
decided: design settled: keyword overlap check on rejected logs, ~20 lines, returns warning in response
fixed: loop detection for rejected logs - keyword overlap check with 50% threshold, returns warning in response
decided: implemented semantic similarity for loop detection: sentence-transformers with 0.5 threshold catches rephrased attempts like 'increase the delay' -> 'increasing timeout' at 62% while avoiding false positives like 'redis caching' at 10%
decided: adjusted semantic similarity threshold from 0.5 to 0.6 - better balance between catching loops and avoiding false positives
learned: researched Graphiti, Titans/MIRAS, Cipher for semantic architecture ideas - key insights: 1) Graphiti uses bi-temporal model (event time + ingestion time) with hybrid search (semantic + keyword + graph), temporal edge invalidation for conflicts 2) Titans uses surprise metric for selective memory - only store what's unexpected, adaptive forgetting 3) Cipher has dual memory (System 1: facts, System 2: reasoning traces), cross-IDE persistence, team sharing
decided: design decision: promotion handling for similar memories - default to Link (C) to preserve thought evolution, but Supersede (D) when confidence is high enough through repeated confirmation. Superseded entries stay but marked as outdated until proven wrong again.
decided: design decisions: 1) Bug memory: surprising OR reusable bugs (would future-me benefit?), use semantic matching on promotion to filter. 2) Overwhelm: methodology first (zoom out checklist), then spawn agent. 3) Spawned agent: can implement on current branch, not isolated - gets problem + SESSION.md tried approaches, returns with fix or recommendation
decided: design decision: agent spawn trigger is semi-auto - Mind suggests spawning in loop warning response, user confirms before spawn happens
decided: design decision: use Obsidian wikilinks [[MEMORY#section]] for memory cross-references - enables graph visualization of thought evolution, clickable navigation, zero extra cost
fixed: shipped: semantic memory architecture design doc - 7 phases covering bidirectional session/memory flow, bug memory, overwhelm response, agent spawning, Obsidian compatibility
fixed: completed Phase 1: semantic search for mind_search() - 25 tests passing, pushed to GitHub
fixed: completed Phase 2: Memory -> Session retrieval - retrieve_relevant_memories() works (tested directly), MCP may need restart to pick up changes
fixed: completed Phase 3: smart promotion with novelty checking and link/supersede - tests passing, pushed to GitHub
fixed: completed Phase 4: bug memory filter with reusability checking - 152 tests passing, pushed to GitHub
fixed: completed Phase 5: enhanced loop warning with severity levels and methodology - 157 tests passing, pushed to GitHub
fixed: completed Phase 6: mind_spawn_helper tool for packaging problems for fresh agent investigation - pushed to GitHub
fixed: completed all 7 phases of semantic memory architecture: 1) semantic search 2) memory->session retrieval 3) smart promotion with novelty/link/supersede 4) bug reusability filter 5) enhanced loop warning with methodology 6) agent spawning tool 7) Obsidian compatibility - 157 tests passing
fixed: fixed mind_spawn_helper bug - was calling undefined parse_session_content, replaced with inline dict using parse_session_section for each section
fixed: tested all features from last session: test suite (157 pass), mind_spawn_helper (fixed bug - parse_session_content undefined), semantic search, smart promotion, loop warning system - all working
fixed: published vibeship-mind 2.2.0 to PyPI - semantic similarity, loop detection, smart promotion, mind_spawn_helper
fixed: Added Amplitude event tracking to mind.vibeship.co landing page: copy button clicks (pip install, mind init, MCP config), scroll depth tracking (25/50/75/100%), section view tracking with IntersectionObserver

<!-- Promoted from SESSION.md on 2025-12-15 -->
decided against: tried Redis for caching/storage - infrastructure overhead too high
decided against: tried using Redis for session storage - too complex for our needs (see also: [[MEMORY#L171]])
learned: analyzing: current bug/fix capture in MEMORY.md - we log fixes but not in a structured way that links problem -> solution -> root cause
learned: design direction: bidirectional Session <-> Memory connection - continuous loop where each informs the other (see also: [[MEMORY#L105]])
learned: researched mem0, Cipher, Zep/Graphiti, Deep Agents architecture - key findings: 1) mem0 uses LLM-driven deduplication with embeddings + 0.7 similarity threshold 2) Graphiti uses temporal edge invalidation for conflicts 3) Deep Agents prevent loops via explicit planning + plan updates on failure 4) Anthropic recommends progress documentation + feature-list guardrails

decided: removed all Unicode characters from mascot.py - now ASCII-only for Windows cp1252 compatibility. can_use_unicode() always returns False, EXPRESSIONS uses ASCII, box-drawing removed
fixed: cleaned up CLAUDE.md - removed 4 duplicate sections and fixed corrupted Unicode arrows (-> instead of various broken encodings)
fixed: Completed comprehensive Unicode codebase scan - found 85+ Unicode arrow characters in markdown files, mascot emoji in MEMORY.md, and dead code references to can_use_unicode() function in Python files
decided: Updated Mind landing page header to match vibeship.co (Scanner) exactly - same class names, CSS values, layout structure. Logo 24x24px, font 1.44rem Instrument Serif, max-width 1200px with 1.625rem padding, theme toggle and GitHub button 36x36px
decided: Improved landing page readability: increased base html font from 14px to 16px, bumped all font size variables up (text-xs: 0.75rem, text-sm: 0.875rem, text-base: 1rem), added line-height: 1.7 to body text, updated feature cards and descriptions to use larger fonts
fixed: implemented SESSION.md auto-categorization - mind_log now detects rejected/blocker/assumption patterns automatically, and update_session_section auto-repairs malformed files

<!-- Promoted from SESSION.md on 2025-12-18 -->
learned: cleaning up Unicode characters that cause issues on Windows (cp1252 encoding) (see also: [[MEMORY#L26]])

fixed: Phase 1 complete: added 82 new unit tests (test_session.py, test_config.py, test_mind_log_integration.py). All 239 tests pass.
fixed: Phase 6-7 complete: integrated logging_levels.py into MCP handle_log (filters based on efficient/balanced/detailed), created retention.py with usage-based decay, integrated decay on mind_recall gap and search result tracking