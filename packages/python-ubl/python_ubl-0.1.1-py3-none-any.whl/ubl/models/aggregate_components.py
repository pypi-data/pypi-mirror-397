...
from .common import BaseElement, CacMixin
...
# =============================================================================
# Group 2: Address Component
# =============================================================================


@dataclass
class PostalAddress(CacMixin, BaseElement):
    """
    UBL cac:PostalAddress - Full postal address.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.street_name:
            raise ValidationError('PostalAddress (building): street_name is required')
        if not self.city_name:
            raise ValidationError('PostalAddress (building): city_name is required')
        if not self.postal_zone:
            raise ValidationError('PostalAddress (building): postal_zone is required')
        if not self.country:
            raise ValidationError('PostalAddress (building): country is required')
...
@dataclass
class TaxCategory(CacMixin, BaseElement):
    """
...
# Alias for item-level tax categories (same as TaxCategory)
...
class Item(CacMixin, BaseElement):
    ...
    classified_tax_category: TaxCategory | None = field(default=None, kw_only=True)
...
class PartyIdentification(CacMixin, BaseElement):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if self.schemeID == other.schemeID:
    ...
    def __hash__(self) -> int:
        ...
        return hash((self.value.lower(), self.schemeID))
...
class Party(CacMixin, BaseElement):
    ...
    def party_identification_list(self) -> list[PartyIdentification]:
        """
        All identifiers as PartyIdentification list.

        Used for cac:PartyIdentification elements (multiple allowed).

        Returns:
            List of PartyIdentification objects (empty if no identifiers)
        """
    ...
    def party_tax_scheme(self) -> PartyTaxScheme | None:
        ...
        Returns:
            PartyTaxScheme or None if no VAT or country_code
        """
        if self.vat and self.country_code:
            return PartyTaxScheme(
                company_id=CompanyID(value=self.vat, schemeID=f'{self.country_code}:VAT'),
            )
        return None
    ...
    def party_legal_entity(self) -> PartyLegalEntity | None:
        ...
        if self.registration and self.country_code:
            # Use first registration scheme for company_id
            ...
            if reg_schemes:
                company_id = CompanyID(value=self.registration, schemeID=reg_schemes[0].scheme_id)
