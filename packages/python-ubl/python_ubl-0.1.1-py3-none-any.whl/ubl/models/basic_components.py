...
class Amount(CbcMixin, WithAsMixin, BaseElement):
    ...
    # Note: Amounts CAN be negative in UBL (credit notes, returns, adjustments)
    # No additional validation needed beyond type casting from parent mixins
...
class BaseIdentifier(CbcMixin, BaseElement, ABC):
    ...
    def _handle_financial_id(self) -> None:
        """Handle financial IDs (IBAN, BIC) - no validation per user requirement."""
        pass  # Accept as-is

    def _handle_peppol_id(self) -> None:
        """
        Handle PEPPOL participant ID - apply normalization.

        Logic:
        1. Look up scheme in PEPPOL registry
        2. Extract value using scheme regex if defined
        3. Lowercase value if schemeID is ISO6523 numeric code
        4. Auto-populate schemeName from scheme.name
        """
        ...
        if scheme.regex:
            ...
            # If extraction fails, keep original value

        # Step 2: Lowercase if ISO6523 code
        if self.schemeID.isdigit():
...
class Date(CbcMixin, WithAsMixin, BaseElement):
    ...
    def __post_init__(self):
        ...
        # Finally: trigger type casting through mixin chain
        self._super_post_init(super())
