...
from .common import BaseElement
...
class BaseDocument(BaseElement):
    ...
    # Mapping from XML element names to property/field names
    ...
    def tax_totals(self) -> list[TaxTotal]:
        ...
        from .basic_components import TaxableAmount, TaxAmount

        # Group taxes by category and rate
        tax_groups = defaultdict(lambda: {'taxable': Decimal('0'), 'tax': Decimal('0'), 'category': None})
        ...
        for (cat_id, percent_value), data in tax_groups.items():
            subtotals.append(
                TaxSubtotal(
                    taxable_amount=TaxableAmount(value=data['taxable']),
                    tax_amount=TaxAmount(value=data['tax']),
    ...
    def legal_monetary_total(self) -> LegalMonetaryTotal:
        ...
        from .basic_components import LineExtensionAmount, PayableAmount, TaxExclusiveAmount, TaxInclusiveAmount

        # Sum line totals
        line_total = sum(line.line_extension_amount.value for line in self.lines)

        # Sum taxes
        tax_total = sum(tt.tax_amount.value for tt in self.tax_totals)
...
class Invoice(BaseDocument):
    ...
    class_name_to_attr: ClassVar[dict[str, str]] = {
        k: v
        for k, v in BaseDocument.class_name_to_attr.items()
        if k != 'CreditNoteLine'
    }
