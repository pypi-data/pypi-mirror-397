...
def demonstrate_commit_behavior() -> None:
    """Show how the new commit() function works for batching metrics at the same step."""

    with expt_logger.init(name="commit-demo") as run:
        print("\nDemonstrating batching with commit() (new default behavior):")

        # These all go to the same step (step 1) - commit=False is now the default
        expt_logger.log({"metric_a": 1.0})
        print("  Logged metric_a=1.0 (pending at step 1)")

        expt_logger.log({"metric_b": 2.0})
        print("  Logged metric_b=2.0 (pending at step 1)")

        expt_logger.log({"metric_c": 3.0})
        print("  Logged metric_c=3.0 (pending at step 1)")

        # Commit all three at once
        expt_logger.commit()
        print("  Called commit() - all 3 metrics committed at step 1, now at step 2")

        # Log more at next step
        expt_logger.log({"metric_a": 1.1})
        expt_logger.log({"metric_b": 2.1})
        expt_logger.commit()
        print("  Logged 2 more metrics and committed (step 2 -> step 3)")
