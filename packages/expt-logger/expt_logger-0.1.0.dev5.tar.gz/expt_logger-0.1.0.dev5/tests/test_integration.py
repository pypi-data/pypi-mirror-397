...
class TestBasicLogging:
    ...
    def test_log_scalars(self) -> None:
        ...
        # Log some metrics
        expt_logger.log({"train/loss": 1.5, "train/accuracy": 0.6}, commit=True)
        expt_logger.log({"train/loss": 1.2, "train/accuracy": 0.7}, commit=True)
        expt_logger.log({"eval/loss": 1.3}, step=2, commit=True)
    ...
    def test_commit_behavior(self) -> None:
        ...
        expt_logger.log({"metric_c": 3.0}, commit=True)  # Commits all

        assert run.step == 2  # Should be at step 2 now (incremented after commit)

        expt_logger.end()

    def test_commit_function(self) -> None:
        """Test the standalone commit() function."""
        run = expt_logger.init(name="test-commit-func")

        # Log multiple metrics and rollouts without committing
        ...
        # All should be at step 1 still
        assert run.step == 1

        # Commit all at once
        expt_logger.commit()

        # Now step should have incremented
        assert run.step == 2

        expt_logger.end()

    def test_log_rollout_with_commit_true(self) -> None:
        """Test log_rollout with commit=True increments step."""
        run = expt_logger.init(name="test-rollout-commit")

        # Log rollout with commit=True
        expt_logger.log_rollout(
            prompt="What is 2+2?",
        ...
        # Step should have incremented again
        assert run.step == 3

        expt_logger.end()

    def test_log_rollout_with_commit_false(self) -> None:
        """Test log_rollout with commit=False (default) keeps step."""
        run = expt_logger.init(name="test-rollout-no-commit")

        # Log rollouts without committing (default behavior)
        expt_logger.log_rollout(
            prompt="test1",
        ...
        # Step should have incremented once
        assert run.step == 2

        expt_logger.end()

    def test_mixed_log_and_log_rollout_with_commit(self) -> None:
        """Test mixing log() and log_rollout() calls with manual commit."""
        run = expt_logger.init(name="test-mixed-commit")

        # Log metrics and rollouts at the same step
        expt_logger.log({"loss": 0.5})
        expt_logger.log({"accuracy": 0.9})
        expt_logger.log_rollout(
            prompt="test",
        ...
        # All should be at step 1
        assert run.step == 1

        # Commit all together
        expt_logger.commit()

        # Step should increment once
        assert run.step == 2

        # Log more for next step
        expt_logger.log({"loss": 0.4}, commit=False)
        expt_logger.log_rollout(
            prompt="test3",
...
class TestGRPOStyleLogging:
    ...
    def test_grpo_experiment(self) -> None:
        ...
        for step in range(num_steps):
            ...
            expt_logger.log(train_metrics, step=step, commit=True)

            # Log eval scalars
            eval_metrics = generate_scalars(step, num_steps, eval_mean_reward, "eval")
            expt_logger.log(eval_metrics, step=step, commit=True)
