...
from .utils import parse_conversation, parse_metric_key, sanitize_config
...
class Run:
    ...
    def __init__(
        ...
        if config:
            try:
                sanitized_config, errors = sanitize_config(config)
                self.config.update(sanitized_config)

                # Store config errors in a special key
                if errors:
                    self.config._data["_config_errors"] = errors
            except TypeError as e:
                # Config is not a dict - raise immediately
        ...
        # Track pending rollouts for commit=False behavior
        self._pending_rollouts: list[Rollout] = []
    ...
    def _increment_step(self) -> None:
        """Increment the step counter and clear pending step."""
        if self._pending_step is not None:
            self._current_step = self._pending_step + 1
            self._pending_step = None

    def _commit_pending(self) -> None:
        """Commit all pending metrics to the buffer (without incrementing step)."""
        ...
        # Clear pending metrics and mode (but not step - that's cleared in _increment_step)
    ...
    def log_rollout(
        ...
        if commit:
            ...
            # Set pending step for increment
            self._pending_step = step
            self._increment_step()
            ...
            self._pending_rollouts.append(rollout)
            self._pending_step = step
    ...
    def commit(self) -> None:
        ...
        Example:
            ...
            run.log_rollout(..., commit=False)
            run.commit()  # Both are committed at the same step, then step increments
        """
        if self._finished:
            return

        # Check if there's anything to commit
        has_pending_metrics = bool(self._pending_metrics and self._pending_step is not None)
        has_pending_rollouts = bool(self._pending_rollouts and self._pending_step is not None)

        if not has_pending_metrics and not has_pending_rollouts:
            return

        # Commit pending metrics to buffer
        if has_pending_metrics:
            self._commit_pending()

        # Commit pending rollouts to buffer
        if has_pending_rollouts:
            with self._lock:
                self._rollout_buffer.extend(self._pending_rollouts)
            self._pending_rollouts = []

        # Increment step counter once
        self._increment_step()
