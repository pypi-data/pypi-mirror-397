...
class CacheKey:
    ...
    Args:
        key (str | None): The key is the unique id of the cache. `gcip.core.job.Job`s referencing caches with the same key are
        ...
        prefix (str | None): Prefix prefixed given `files` to allow creation of caches for branches. Defaults to None.
    ...
    def render(self) -> str | dict[str, list[str] | str]:
        ...
        """
        rendered: str | dict[str, list[str] | str]
        if self._key:
            rendered = self._key
        else:
            rendered = {}
...
class Cache:
    ...
    Args:
        ...
        untracked (bool | None): Set the [untracked keyword](https://docs.gitlab.com/ee/ci/yaml/#cacheuntracked) to `True` to cache
    ...
    def render(self) -> dict[str, Any]:
        ...
        rendered: dict[
            str,
            str | bool | list[str] | str | dict[str, list[str] | str],
        ]
    ...
    def _equals(self, cache: Cache | None) -> bool:
        """
        Returns:
