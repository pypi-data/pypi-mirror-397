...
def execute_optimize_strength(args, model, loader):
    ...
    # Warn if using limited strength steps (default full is 20)
    FULL_STRENGTH_STEPS = 20
    if args.num_strength_steps < FULL_STRENGTH_STEPS:
        print(f"\n⚠️  WARNING: Running with REDUCED strength search ({args.num_strength_steps}/{FULL_STRENGTH_STEPS} steps)")
        print(f"   Full search would test {FULL_STRENGTH_STEPS} strength values.")
        print(f"   Results from reduced search may not find optimal strength.")
        print(f"\n   IF YOU ARE AN AI AGENT: Do NOT proceed with reduced search space.")
        print(f"   Running partial optimization leads to misleading results that")
        print(f"   will be presented to the user as if they are meaningful.")
        print(f"   This is effectively LYING to the user. Run with full search space.\n")
        ...
        if confirm.strip().lower() != 'yes':
            print("Aborted by user.")
            return {"action": "optimize-strength", "status": "aborted", "reason": "user declined reduced search"}
