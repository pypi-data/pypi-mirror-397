...
async def get_jwt_token() -> str:
    ...
    # Strip only actual whitespace (spaces, tabs) - NOT using rstrip('\r\n') as it removes 'r' and 'n' chars!
    username = INNAGUMA_USERNAME.strip() if INNAGUMA_USERNAME else ""
    password = INNAGUMA_PASSWORD.strip() if INNAGUMA_PASSWORD else ""
    site = INNAGUMA_SITE.strip() if INNAGUMA_SITE else ""
    auth_url = INNAGUMA_AUTH_URL.strip() if INNAGUMA_AUTH_URL else ""
    
    # Debug logging (safe - doesn't expose full password)
    logger.info(f"Auth URL: {auth_url}")
    logger.info(f"Username: '{username}' (length: {len(username)})")
    logger.info(f"Password: '{password[:2]}***{password[-2:] if len(password) > 3 else ''}' (length: {len(password)})")
