                ...
                if response.status == 200:
                    try:
                        ...
                        if not JWT_TOKEN:
                            ...
                            raise ToolError("Token not found in authentication response")
                        logger.info("JWT token obtained successfully")
                        return JWT_TOKEN
                    except aiohttp.ContentTypeError:
                        text = await response.text()
                        logger.error(f"Invalid JSON response from auth: {text[:200]}")
                        raise ToolError(f"Invalid JSON response from auth: {text[:200]}")
                else:
                    text = await response.text()
                    logger.error(f"Authentication failed ({response.status}): {text[:200]}")
                    raise ToolError(f"Authentication failed ({response.status}): {text[:200]}")
