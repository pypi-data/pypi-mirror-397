...
async def get_jwt_token() -> str:
    ...
    # Log raw password details for debugging (check for hidden characters)
    logger.info(f"Raw password length: {len(raw_password)}")
    logger.info(f"Raw password last 3 chars (repr): {repr(raw_password[-3:]) if len(raw_password) >= 3 else repr(raw_password)}")
    
    # Strip whitespace and carriage returns (common issue with .env files on Windows)
    username = INNAGUMA_USERNAME.strip().rstrip('\r\n') if INNAGUMA_USERNAME else ""
    password = INNAGUMA_PASSWORD.strip().rstrip('\r\n') if INNAGUMA_PASSWORD else ""
    site = INNAGUMA_SITE.strip().rstrip('\r\n') if INNAGUMA_SITE else ""
    auth_url = INNAGUMA_AUTH_URL.strip().rstrip('\r\n') if INNAGUMA_AUTH_URL else ""
    
    # Debug logging (safe - doesn't expose full password)
    logger.info(f"Auth URL: {auth_url}")
    logger.info(f"Username: '{username}' (length: {len(username)})")
    logger.info(f"Password after strip: '{password[:2]}***{password[-2:] if len(password) > 3 else ''}' (length: {len(password)})")
    logger.info(f"Password last 2 chars (repr): {repr(password[-2:]) if len(password) >= 2 else 'N/A'}")
    logger.info(f"Site: '{site}'")
    
    try:
        # Crear FormData para multipart/form-data (requerido por la API)
        form_data = aiohttp.FormData()
        form_data.add_field("username", username, content_type="text/plain; charset=utf-8")
        form_data.add_field("passwd", password, content_type="text/plain; charset=utf-8")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(INNAGUMA_AUTH_URL, data=form_data) as response:
                ...
                try:
                    ...
                    logger.error(f"Failed to parse authentication response: {text[:200]}")
                    raise ToolError(f"Invalid response format from auth server: {text[:200]}")
                
                # Check if authentication was successful
                if result.get("result") is True and result.get("token"):
                    ...
                    logger.error(f"Authentication failed ({error_status}): {error_msg}")
                    
                    # Provide helpful error messages
                    if "password" in error_msg.lower():
                        raise ToolError(f"Invalid credentials - check INNAGUMA_PASSWORD in .env file: {error_msg}")
                    elif "user" in error_msg.lower():
                        raise ToolError(f"Invalid credentials - check INNAGUMA_USERNAME in .env file: {error_msg}")
                    else:
                        raise ToolError(f"Authentication failed: {error_msg}")
