...
class HubspotAdapter(BaseOAuthAdapter):
    """Hubspot OAuth 2.0 adapter - placeholder for implementation"""
    
    def __init__(self, client_id: str = "{{HUBSPOT_CLIENT_ID}}", client_secret: str = "{{HUBSPOT_CLIENT_SECRET}}"):
        super().__init__(client_id, client_secret)
        self.platform_name = "hubspot"
    
    async def get_authorization_url(self, state: str, redirect_uri: str, scopes: Optional[list[str]] = None) -> str:
        raise NotImplementedError("HubspotAdapter requires implementation")
