...
def compute_reference_stats(dataset, vit_model, device, transform, max_samples=200):
    """
    Computes reference attention statistics (mu_ref, sigma_ref) from authentic samples if available.
    Falls back to using tampered images or default constants if necessary.
    """
    ...
    if len(authentic_indices) > 0:
        print(f"Found {len(authentic_indices)} authentic samples for reference statistics.")
        selected_indices = random.sample(authentic_indices, min(len(authentic_indices), max_samples))

    # Case 2: No authentic samples (e.g., DF2023)
    elif len(authentic_indices) == 0:
        print("No authentic samples found — falling back to tampered samples for reference stats.")
        selected_indices = random.sample(range(len(dataset)), min(len(dataset), max_samples))
    ...
    for idx in tqdm(selected_indices, total=len(selected_indices)):
        ...
        try:
            outputs = vit_model(img_t, output_attentions=True, interpolate_pos_encoding=True)
            attn_maps = outputs.attentions
            attn_mean_layers = torch.stack([a.mean(dim=1)[:, 0, 1:] for a in attn_maps]).mean(dim=0)
    ...
    if len(attn_means) == 0:
        print("Could not compute valid reference stats, using defaults (mu_ref=0.5, sigma_ref=0.1)")
