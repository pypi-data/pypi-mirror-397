...
def compute_scores(
    ...
    mu_ref_f = float(mu_ref) if not torch.is_tensor(mu_ref) else float(mu_ref.detach().cpu().item())
    sigma_ref_f = float(sigma_ref) if not torch.is_tensor(sigma_ref) else float(sigma_ref.detach().cpu().item())
