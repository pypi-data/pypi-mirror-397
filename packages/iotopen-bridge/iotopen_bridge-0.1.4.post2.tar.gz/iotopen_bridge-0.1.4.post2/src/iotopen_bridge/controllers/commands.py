...
def _switch_publish_value(fx: FunctionX, on: bool) -> Any:
    # Preserve your existing behavior to avoid breaking devices:
    # - If state_on/off exists, use it (covers 255/0 etc).
    # - Otherwise fallback to "ON"/"OFF" strings.
    if on:
        return fx.state_on if fx.state_on is not None else "ON"
    return fx.state_off if fx.state_off is not None else "OFF"


def _pick_set_topic(fx: FunctionX) -> str | None:
    """Pick the best writable topic for this function.

    Priority:
      1) topic_set
      2) topic_write

    This makes the controller work for inventories where writable topics are stored as topic_write.
    """
    ts = (getattr(fx, "topic_set", None) or "").strip()
    if ts:
        return ts
    tw = (getattr(fx, "topic_write", None) or "").strip()
    if tw:
        return tw
    return None
        ...
        *,
        ...
        if not topic:
            self._emit(
                ...
                value=None,
                ok=False,
                error="no topic_set/topic_write",
            )
            return
