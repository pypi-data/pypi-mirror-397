...
class RunPipe:
    ...
    def fit_automl(self, pipe, train_ratio=1.0, hyperparams=None):
        ...
        Example:
            automl = CustomAutoML(method=pipe[0], data_folder=self.args.path, ...)
        """
    ...
    def fit_hybrid(self, pipe, train_ratio=1.0, hyperparams=None):
        ...
        Example:
            imputer = CustomImputer(method=pipe[0], data_folder=self.args.path, ...)
            hybrid = CustomHybrid(method=pipe[1], data_folder=self.args.path, ...)
        """
        # Store hyperparams in args for components to access
        if hyperparams:
            self.args.hyperparams = hyperparams
    ...
    def fit(self, pipe, train_ratio=1.0, hyperparams=None):
        ...
        Example:
            imputer = CustomImputer(method=pipe[0], data_folder=self.args.path, ...)
            resampler = CustomResamplar(method=pipe[1], data_folder=self.args.path, ...)
            classifier = CustomClassifier(method=pipe[2], data_folder=self.args.path, ...)
        """
        # Store hyperparams in args for components to access
        if hyperparams:
            self.args.hyperparams = hyperparams
