...
class PydanticAgentExecutor(AgentExecutor):
    ...
    def __init__(self, agent_parameters: AgentParameters, history_storage: PydanticAiAgentHistoryStorage | None = None):
        ...
        self.history_storage = history_storage if history_storage else InMemoryHistoryStorage()
    ...
    async def execute(  # pylint: disable=too-many-locals
        ...
        history_message = await self.history_storage.get(task.id)
        ...
        await self.history_storage.store(task.id, result.new_messages())
    ...
    def _build_agent(self, session_tuple: SessionIdTuple, auth_token: str | None) -> AbstractAgent:
