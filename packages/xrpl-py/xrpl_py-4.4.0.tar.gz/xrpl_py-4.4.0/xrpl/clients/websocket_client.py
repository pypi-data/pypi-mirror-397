...
class WebsocketClient(SyncClient, WebsocketBase):
    ...
    def __iter__(self: Self) -> Iterator[Dict[str, Any]]:
        ...
        while self.is_open():
            ...
            try:
                print(
                    "INFO: waiting for the future to complete at timestamp: ",
                    time.time(),
                )
                print("INFO: recently popped message: ", future)
                print(
                    "INFO: number of messages in the message queue: ",
                    cast(asyncio.Queue[Dict[str, Any]], self._messages).qsize(),
                )
                print("INFO: size of the open requests: ", len(self._open_requests))
                print("INFO: contents of the open requests: ", self._open_requests)
