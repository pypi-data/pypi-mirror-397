...
class WebsocketBase(Client):
    ...
    async def _handler(self: Self) -> None:
        ...
        async for response in cast(websocket_client.ClientConnection, self._websocket):
            ...
            # print("handler processing response_dict: ", response_dict)
            # print(
            #     "Does this response correspond to an open request? ",
            #     "id" in response_dict and response_dict["id"] in self._open_requests,
            # )
            # print("# of the open requests: ", len(self._open_requests))
            # print("contents of the open requests: ", self._open_requests)
            # print(
            #     "# of the messages in the message queue: ",
            #     cast(_MESSAGES_TYPE, self._messages).qsize(),
            # )
    ...
    async def _do_pop_message(self: Self) -> Dict[str, Any]:
        ...
        """
        print(
            "INFO: popping the next messsage from the message queue at timestamp: ",
            time.time(),
        )
        ...
        cast(_MESSAGES_TYPE, self._messages).task_done()
        print(
            "INFO: next message in the message queue: ",
            msg,
        )
        return msg
