...
from mircat_v2.configs import read_stats_models_config, read_dbase_config, radiomics_config, logger_setup
...
class RadiomicsExtractor:
    ...
    def extract_radiomics(self, niftis: list[Path]):
        ...
        if self.n_processes > 1:
            logger.info(f"Starting radiomics extraction on {self.total_niftis} nifti files with {self.n_processes} processes.")
        else:
            logger.info(f"Starting radiomics extraction on {self.total_niftis} nifti files.")

        if self.n_processes > 1:
            with Pool(processes=self.n_processes) as pool:
                for i, result in enumerate(
                    pool.imap_unordered(self._process_single_nifti, niftis), start=1
                ):
                    if result is None:
                        logger.error(
                            f"Radiomics [{i}/{self.total_niftis}] ({i / self.total_niftis:.2%}) ✗"
                        )
                    else:
                        nifti_name, radiomics_df, structures_extracted = result
                        if self.save_to_database and radiomics_df is not None:
                            self._insert_radiomics_to_database(radiomics_df, structures_extracted)
                        logger.success(
                            f"Radiomics [{i}/{self.total_niftis}] ({i / self.total_niftis:.2%}) ✓ {nifti_name}"
                        )
        else:
            for i, nifti in enumerate(niftis, start=1):
                result = self._process_single_nifti(nifti)
                if result is None:
                    logger.error(
                        f"Radiomics [{i}/{self.total_niftis}] ({i / self.total_niftis:.2%}) ✗"
                    )
    ...
    def _process_single_nifti(self, nifti_path: Path) -> tuple | None:
        """Process a single NIfTI file for radiomics extraction. Worker method for multiprocessing.
        ...
        """
        # Re-initialize logger and limit threads in child processes
