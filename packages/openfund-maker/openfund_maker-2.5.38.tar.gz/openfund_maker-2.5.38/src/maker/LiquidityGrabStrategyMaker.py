...
class LiquidityGrabStrategyMaker(StrategyMaker):
    ...
    def process_pair(self, symbol, pair_config):
        ...
        try:
            ...
            atf_result = self._analyze_atf_liquidity_zones(
                symbol, atf, htf_result, open_body_break, liquidity_strategy, precision, market_price
            )
            if not atf_result:
                return
            ...
            if not self._perform_ai_validation(
                symbol, htf, htf_result, atf_result, ltf_result,
                entry_price, liquidity_strategy, precision
            ):
                return
    ...
    def _analyze_htf_trend_and_levels(
        ...
        if htf_cache_key not in self.htf_cache:
            ...
            # 使用AI分析HTF趋势和后续交易方向
            htf_trend, htf_side = self._analyze_htf_trend_with_ai(symbol, htf, htf_df, htf_latest_struct)

            self.logger.info(f"{symbol} : {step}.4. HTF {htf} AI分析 - 趋势: {htf_trend}, 交易方向: {htf_side}")
    ...
    def _analyze_htf_trend_with_ai(self, symbol, htf, htf_df, htf_latest_struct, support_price=None, resistance_price=None):
        """使用AI分析HTF趋势方向和交易方向
        ...
        """
        # 如果AI验证未启用，使用传统方法
        if not (self.ai_validation_coordinator and self.ai_validation_coordinator.is_enabled()):
            trend = htf_latest_struct[self.STRUCT_DIRECTION_COL]
            side = self.BUY_SIDE if trend.upper() == self.BULLISH_TREND.upper() else self.SELL_SIDE
            return trend, side
        
        try:
            # 获取最近16条K线数据
            recent_candles = htf_df.tail(16).to_dict('records') if htf_df is not None else []
            ...
            prompt = self._build_htf_trend_analysis_prompt(
                symbol, htf, recent_candles, htf_latest_struct, support_price, resistance_price
            )
            ...
            if ai_response:
                ...
                if ai_trend in ['BULLISH', 'BEARISH'] and ai_side in ['buy', 'sell']:
                    ...
                    self.logger.warning(f"{symbol}: AI返回无效结果 - 趋势: {ai_trend}, 方向: {ai_side}")
    ...
    def _build_htf_trend_analysis_prompt(self, symbol, htf, recent_candles, htf_latest_struct, support_price=None, resistance_price=None):
        """构建HTF趋势分析的AI提示词"""
        ...
        if '_' in str(struct_type_raw):
            struct_type = str(struct_type_raw).split('_')[-1]  # 取最后部分，如BOS、CHoCH、SMS
        else:
        ...
        prompt = f"""
你是一位专业的金融分析师，专门分析加密货币市场趋势。请基于以下{htf}时间框架的最近16条K线数据，分析{symbol}的趋势方向。
...
**K线数据（最近16条，{htf}时间框架）：**
{self._format_candle_data_for_prompt(candle_data)}
...
"""
    ...
    def _call_ai_for_trend_analysis(self, prompt):
        ...
        try:
            # 使用现有的AI客户端
            if hasattr(self.ai_validation_coordinator, 'ai_client') and self.ai_validation_coordinator.ai_client:
                # 记录发送的提示词
                self.logger.info("=" * 80)
                ...
                # 发送请求
                response = self.ai_validation_coordinator.ai_client._make_request_raw(request_data)
    ...
    def _analyze_atf_liquidity_zones(
        self, symbol, atf, htf_result, open_body_break, strategy, precision, market_price
    ):
        """Step 2: ATF识别等高等低流动性聚集区"""
        step = "2"
        ...
        if atf_cache_key not in self.atf_cache:
            ...
            atf_support_price = atf_support_resistance.get(self.SUPPORT_PRICE_KEY) if atf_support_resistance else None
            atf_resistance_price = atf_support_resistance.get(self.RESISTANCE_PRICE_KEY) if atf_support_resistance else None

            # 2.3 识别等高等低流动性（基于HTF方向，筛选满足利润空间的）
            trader_side = self.BULLISH_TREND if htf_result.side == self.BUY_SIDE else self.BEARISH_TREND
            # 根据交易方向确定目标价格：多头用支撑位，空头用阻力位
            target_price = atf_support_price if trader_side == self.BULLISH_TREND else atf_resistance_price
            ...
            if not liquidity_result:
                ...
                # 回退到HTF寻找流动性
                htf_df = self.get_historical_klines_df_by_cache(symbol=symbol, tf=htf_result.tf_value)
            ...
            confluence_result = self._validate_support_resistance_confluence(
                symbol, atf_support_price, atf_resistance_price, liquidity_result, trader_side, precision, strategy
            )
    ...
    def _get_equal_liquidity(self, df_equal_liquidity, trend, target_price=None, min_profit_percent=None):
        """获取满足利润空间要求且时间最新的流动性
    ...
    def _validate_support_resistance_confluence(
        self, symbol, atf_support_price, atf_resistance_price, liquidity_result, trader_side, precision, strategy
    ):
        ...
        if trader_side == self.BUY_SIDE:
            # 多头：需要支撑位+等高流动性配合
            if atf_support_price is None:
                self.logger.info(f"{symbol} : {step}.1. ATF未找到支撑位，无法验证多头机会")
                return None
            ...
            # 空头：需要阻力位+等低流动性配合
            if atf_resistance_price is None:
                self.logger.info(f"{symbol} : {step}.1. ATF未找到阻力位，无法验证空头机会")
                return None
    ...
    def _perform_ai_validation(
        self, symbol, htf, htf_result, atf_result, ltf_result, entry_price, strategy, precision
    ):
        """Step 5: AI验证等高等低形态
        ...
        if liquidity_key in self.ai_validation_cache[symbol]:
            ...
            if cached_validation['decision'] == TradingDecision.SKIP:
                ...
                confluence_result["ai_validation"] = cached_validation.get("ai_validation")
                return True
            else:  # FALLBACK
                return self.ai_validation_coordinator.validation_config.fallback_mode != "skip"
        ...
        htf_context = self.build_timeframe_context(
            timeframe=htf_result.tf_value, trend=htf_result.trend, struct=htf_result.struct,
            support_price=htf_result.support_price, resistance_price=htf_result.resistance_price,
        ...
        atf_context = self.build_timeframe_context(
            timeframe=atf_result.tf_value, trend=atf_result.trend, struct=atf_result.struct,
        )
        
        # 获取K线数据
        htf_df = self.get_historical_klines_df_by_cache(symbol=symbol, tf=htf_result.tf_value)
        atf_df = self.get_historical_klines_df_by_cache(symbol=symbol, tf=atf_result.tf_value)
        ...
        validation_result = self.ai_validation_coordinator.validate_pattern(
            symbol, strategy_signal, existing_equal_points_df=atf_result.df_equal_liquidity
        )
        
        # 缓存验证结果
        cache_entry = {"decision": validation_result.decision, "timestamp": datetime.now()}
        ...
        if validation_result.decision == TradingDecision.SKIP:
            ...
            self.logger.info(f"{symbol} : {step}. AI验证通过 - 置信度: {validation_result.confidence:.3f}")
            ai_validation_info = {
                "confidence": validation_result.confidence,
                "reasoning": validation_result.ai_response.reasoning if validation_result.ai_response else None,
                "model_version": validation_result.ai_response.model_version if validation_result.ai_response else None,
            }
