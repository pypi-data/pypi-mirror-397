"""sklearn-compatible NimbusGMM classifier.

Bayesian Gaussian Mixture Model with class-specific covariances (QDA-style).
"""
...
from .inference import nimbus_gmm_predict_proba


class NimbusGMM(NimbusClassifierMixin):
    """Bayesian Gaussian Mixture Model classifier.

    A Bayesian GMM/QDA-style classifier with class-specific covariances,
    ...
    >>> pipe = make_pipeline(StandardScaler(), NimbusGMM())
    ...
    Notes
    -----
    GMM/QDA allows each class to have its own covariance structure. This is
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "NimbusGMM":
        """Fit the Bayesian GMM model.
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        return nimbus_gmm_predict_proba(self.model_, X)
    ...
    def partial_fit(
        ...
        y_encoded = self._encode_labels(y)
        self.model_ = nimbus_gmm_update(self.model_, X, y_encoded)
