...
def validate_data(data: BCIData, require_labels: bool = False) -> bool:
    ...
    # Validate label range if labels exist
    if data.labels is not None:
        ...
        # Nimbus supports both 0-indexed ([0..K-1]) and 1-indexed ([1..K]) label conventions.
        if not (
            (min_label == 0 and max_label < n_classes)
            or (min_label == 1 and max_label <= n_classes)
        ):
            raise ValueError(
                "Labels must be in range [0, n_classes-1] or [1, n_classes]. "
                f"Got min={min_label}, max={max_label}, n_classes={n_classes}."
            )
