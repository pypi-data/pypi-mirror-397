...
Example:
    # =====================================================
    # 1. OpenAI 兼容客户端（vLLM、通义千问、DeepSeek 等）
    # =====================================================
    ...
    client = OpenAIClient(
        ...
        max_qps=50,            # QPS 限制
        retry_times=3,         # 自动重试
        cache=ResponseCacheConfig.default(),  # 响应缓存（24小时TTL）
    )
    ...
    # =====================================================
    # 2. Gemini 客户端（支持 Developer API 和 Vertex AI）
    # =====================================================
    ...
    cache = ResponseCacheConfig.default()      # 默认（24小时 TTL）
    cache = ResponseCacheConfig.persistent()   # 永不过期
    cache = ResponseCacheConfig.disabled()     # 禁用缓存
