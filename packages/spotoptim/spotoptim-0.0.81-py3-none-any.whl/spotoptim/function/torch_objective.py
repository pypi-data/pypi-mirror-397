...
class TorchObjective:
    ...
    def _prepare_data(self) -> tuple[DataLoader, DataLoader]:
        ...
        if isinstance(data, SpotDataFromArray):
            ...
            x_train = torch.tensor(x_train, dtype=torch.float32) if not torch.is_tensor(x_train) else x_train.float()
            y_train = torch.tensor(y_train, dtype=torch.float32) if not torch.is_tensor(y_train) else y_train.float()
            ...
            if val_data:
                ...
                x_val = torch.tensor(x_val, dtype=torch.float32) if not torch.is_tensor(x_val) else x_val.float()
                y_val = torch.tensor(y_val, dtype=torch.float32) if not torch.is_tensor(y_val) else y_val.float()
    ...
    def train_model(self, model: nn.Module, train_loader: DataLoader, val_loader: Optional[DataLoader], params: Dict[str, Any]) -> float:
        """
    ...
    def __call__(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_samples):
            ...
            if has_var_keyword:
                ...
                filtered_kwargs = {k: v for k, v in model_kwargs.items() if k in sig.parameters}
            ...
            try:
                ...
                raise TypeError(f"Failed to instantiate model {self.experiment.model_class.__name__}: {e}")
