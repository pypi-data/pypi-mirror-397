...
class SpotDataSet(ABC):
    ...
    """
    def __init__(self, input_dim: int, output_dim: int, target_column: Optional[str] = None):
...
class SpotDataFromArray(SpotDataSet):
    ...
    def __init__(self, 
        ...
        output_dim = y_train.shape[1] if hasattr(y_train, 'shape') and len(y_train.shape) > 1 else 1
    ...
    def get_train_data(self) -> Tuple[Union[np.ndarray, torch.Tensor], Union[np.ndarray, torch.Tensor]]:
        return self.x_train, self.y_train

    def get_validation_data(self) -> Optional[Tuple[Union[np.ndarray, torch.Tensor], Union[np.ndarray, torch.Tensor]]]:
        if self.x_val is not None:
            return self.x_val, self.y_val
        return None

    def get_test_data(self) -> Optional[Tuple[Union[np.ndarray, torch.Tensor], Union[np.ndarray, torch.Tensor]]]:
