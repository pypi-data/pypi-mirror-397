...
class Adapter:
    ...
    async def open(self) -> bool:
        try:
            ...
            client_id = f"pyolive_{id(self)}"
            self.logger.info(f"Adapter: Attempting to connect to Athena Broker at {self.broker_url}")
            self.client = AthenaClient(self.broker_url, client_id=client_id)
            await self.client.connect()

            self.logger.info(f"Adapter: Connected to Athena Broker at {self.broker_url}")
            return True
        except AthenaException as e:
            # AthenaException은 이미 상세한 메시지를 포함하고 있음
            self.logger.error(f"Adapter: {e}")
            return False
        except Exception as e:
            # pynng 예외 등 다른 예외 처리
            error_type = type(e).__name__
            self.logger.error(
                f"Adapter: Connection failed to Athena Broker at {self.broker_url}.\n"
    ...
    async def publish(self, exchange: str, routing_key: str, message: str):
        try:
            ...
            self.logger.error(f"Adapter: Publish failed to {exchange}:{routing_key} - {e}")
    ...
    async def start_consume(self, namespace: str, callback: Callable[[AthenaMessage], Awaitable[None]],
        ...
        try:
            ...
            self.logger.info(f"Adapter: Consumer registered for named queue '{queue_name}'")

            # 폴링 루프 시작 (process.c:77-100)
            from .athena_client import MSM_TIMEOUT
            self.logger.info(f"Adapter: Started consuming on queue {queue_name} (timeout={MSM_TIMEOUT}s)")

            while running_flag is None or running_flag.is_set():
                try:
                    # 연결 상태 확인 및 재연결 (process.c:95-99 re_connect 레이블)
                    if not self.client or not self.client.is_connected():
                        self.logger.warning("Adapter: Connection lost, attempting to reconnect...")
                    ...
                    if "connect" in error_msg.lower() or "closed" in error_msg.lower():
                        self.logger.warning(f"Adapter: Connection error, will reconnect: {e}")
                        if not await self._reopen():
                            await asyncio.sleep(5)
                    else:
                        self.logger.error(f"Adapter: Consume error - {e}")
            ...
            self.logger.info("Adapter: Consume cancelled")
        except Exception as e:
        ...
        try:
            ...
            if self.client:
                try:
                    ...
                    self.logger.debug(f"Adapter: Error closing old client: {e}")

            # 새로 연결
            success = await self.open()
            if success:
                self.logger.info(f"Adapter: Reconnected successfully, queue: {self._current_queue}")
            return success

        except Exception as e:
            self.logger.error(f"Adapter: Reconnect failed - {e}")
            return False
    ...
    async def _ensure_client_open(self):
        ...
        """
        if not self.client or not self.client.is_connected():
            self.logger.info("Adapter: Client disconnected, reconnecting...")
            success = await self.open()
            if not success:
    ...
    def get_queue_binding(self, queue_name: str) -> tuple[str, str]:
        ...
        Args:
            queue_name: 큐 이름 (예: "ovq.dps-worker")
        ...
        # 큐 이름에서 subsystem 추출 (예: "ovq.dps-worker" -> "dps")
            ...
            if '-' in queue_part:
                ...
                for entry in entries:
                    ...
                    if isinstance(entry, dict):
                        # dict 형식: {"queue": "ovq.dps-worker", "routing_key": "...", "exchange": "..."}
