...
def get_renamed_table_name(source_table):
    ...
    """
    if NO_RENAMING_ENFORCED:
        return source_table
    if source_table not in RENAMED_TABLE_NAMES:
        RENAMED_TABLE_NAMES[source_table] = safe_drop_underscores(source_table)
        if source_table != RENAMED_TABLE_NAMES[source_table]:
            print_library_log(f"Renamed table {source_table} to {RENAMED_TABLE_NAMES[source_table]}",
                              dev_log=True)
...
def get_renamed_column_name(source_column):
    ...
    if source_column not in RENAMED_COL_NAMES:
        ...
        if source_column != RENAMED_COL_NAMES[source_column]:
            print_library_log(f"Renamed column {source_column} to {RENAMED_COL_NAMES[source_column]}",
                              dev_log=True)
