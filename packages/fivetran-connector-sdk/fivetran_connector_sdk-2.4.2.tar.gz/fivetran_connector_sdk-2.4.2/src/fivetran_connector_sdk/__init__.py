...
class Connector(connector_sdk_pb2_grpc.SourceConnectorServicer):
    ...
    def deploy(self, project_path: str, deploy_key: str, group: str, connection: str, hd_agent_id: str,
        ...
        if connection_id:
            if service != 'connector_sdk':
                ...
                if force:
                    ...
                    confirm = input(
                        ...
                        f"Tip: You may want to download a copy of the existing connector code from the Fivetran dashboard "
                        f"before proceeding.\n"
                ...
                if confirm.lower() == "y" and (not connection_config["secrets_list"] or (confirm_config.lower() == "y")):
                    ...
                    upload_project(
                        project_path, deploy_key, group_id, group_name, connection, False)
            ...
            upload_project(project_path, deploy_key,
                                  group_id, group_name, connection, True)
    ...
    def debug(self,
        ...
        try:
            ...
            for log_msg in run_tester(java_exe, tester_root_dir, project_path, available_port, json.dumps(self.state), json.dumps(self.configuration), profile):
                print(log_msg, end="")
...
def main():
    ...
    if args.command.lower() == "deploy":
        ...
        try:
            ...
            connector_object.debug(args.project_path, configuration, state, args.profile)
        except subprocess.CalledProcessError as e:
