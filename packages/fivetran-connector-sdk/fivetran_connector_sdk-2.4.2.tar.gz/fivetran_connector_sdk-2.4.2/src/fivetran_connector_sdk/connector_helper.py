...
def log_unused_deps_error(package_name: str, version: str, is_deploy: bool):
    if is_deploy:
        ...
        print_library_log(f"Remove `{package_name}` from requirements.txt."
                          f" The latest version of `{package_name}` is always available when executing your code."
                          f" Current version: {version}", Logging.Level.INFO)
...
def validate_requirements_file(project_path: str, is_deploy: bool, version: str):
    ...
    if version_mismatch_deps:
        if is_deploy:
            ...
            print_library_log("We recommend using the current stable version for the following libraries:", Logging.Level.INFO)
    ...
    if unused_deps:
        handle_unused_deps(unused_deps, version, is_deploy)
        if is_deploy:
...
def handle_unused_deps(unused_deps, version, is_deploy):
    if 'fivetran_connector_sdk' in unused_deps:
        log_unused_deps_error("fivetran_connector_sdk", version, is_deploy)
    if 'requests' in unused_deps:
        log_unused_deps_error("requests", "2.32.4", is_deploy)
    if is_deploy:
...
def upload_project(project_path: str, deploy_key: str, group_id: str, group_name: str, connection: str, is_updated_connector: bool = False):
    print_library_log(
        f"Deploying '{project_path}' to connection '{connection}' in destination '{group_name}'.")
    upload_file_path = create_upload_file(project_path)
    upload_result = upload(
        upload_file_path, deploy_key, group_id, connection, is_updated_connector)
    delete_file_if_exists(upload_file_path)
...
def load_ftignore(directory_path: str) -> list[str]:
    ...
    if os.path.exists(ftignore_path):
        try:
            ...
            Logging.warning(f"Could not read .ftignore file at {ftignore_path}: {e}. Proceeding with empty ignore patterns.")
            return []
    return []
...
def _initialize_walker_patterns(top, project_root, parent_patterns):
    ...
    try:
        ...
        print_library_log(f"Unable to parse .ftignore patterns: {e}", Logging.Level.SEVERE)
        sys.exit(1)
...
def _classify_directory_entries(top, project_root, combined_spec, has_negation):
...
def dir_walker(top, project_root=None, parent_patterns=None):
    ...
    dirs, files = _classify_directory_entries(top, project_root, combined_spec, has_negation)

    yield top, files

    for name in dirs:
        new_path = os.path.join(top, name)
        yield from dir_walker(new_path, project_root, combined_patterns)
...
def upload(local_path: str, deploy_key: str, group_id: str, connection: str, is_updated_connector: bool = False) -> bool:
    ...
    Args:
        ...
        is_updated_connector (str): Indicates if the connector is being updated.
    ...
    response = rq.post(f"{constants.PRODUCTION_BASE_URL}/v1/deploy/{group_id}/{connection}",
                       ...
                       headers={
                           ...
                           "X-OS-ARCH": get_os_arch_suffix(),
                           "User-Agent": "fivetran-connector-sdk",
                           "X-DEPLOYMENT-MODE": "new" if not is_updated_connector else "update"})
    if response.ok:
        print("✓")
        return True
...
def run_tester(java_exe_str: str, root_dir: str, project_path: str, port: int, state_json: str,
               configuration_json: str, profile: bool = False):
    ...
    Args:
        ...
        profile (bool): Defaults to false, required to pass --no-output flag for tester run command
