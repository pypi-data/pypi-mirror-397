...
def _get_compose_files(
    prod: bool = False, network: Network = Network.HOST, cache: bool = False
) -> List[ValidPath]:
...
def up(
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(prod=config.prod, network=config.network),
...
def build(
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(prod=False, cache=cache),
        compose_project_directory=get_project_root(),
    )
    services_list = list(services) if services else None
...
@config_dir_option
def configure(default: bool, include_defaults: bool, config_dir: str):
    """Configure Lookout"""
    set_initial_env(config_dir)
