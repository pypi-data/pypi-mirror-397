...
class TgtNode:
    ...
    def __init__(self, spec, remark: str = ""):
        """If remark is used, it will fill an edge label.
        """
        if spec is None:
...
class Graph:
    def __init__(self, node_name_prefix: str = ""):
        ...
        self._nd_end = GraphNode("__end__", None)   
            # artificial node used during recording, and resolved to when announcing
            # the edge "... -> TgtNode.end()" as required with enforce_structure==True
    ...
    def _resolve_node_spec(self, node_spec):
        ...
        if node_spec == TgtNode.start():    # <-- review
    ...
    def set_entry_point(self, entry_point):
        """:entry_point: either a GraphNode instance or a node function
        """
    ...
    def exists_added_edge(self, nd1: GraphNode, nd2: GraphNode) -> bool:
        """Returns True if an added edge exists from nd1 to nd2.
        """
        if not nd1 in self.edges_added: return False
        return self.edges_added[nd1] == nd2
        
    def exists_announced_edge(self, nd1: GraphNode, nd2: GraphNode) -> bool:
        """Returns True if an announced edge exists from nd1 to nd2.
        """
    ...
    def add_node(self, node_spec, label: str = ""):
        """Add a node to the graph. The underlying function must have signature
            (state, ctx: CfCtx) -> TgtNode.

        :param node_spec: A reference to the underlying function.
        :type str: 
        :type function: 

        :raises ValueError: in case of non-resolvable node function reference
    ...
    def announce_edge(self, node_spec1, node_spec2, label: str = ""):
        """Used to convey to the graph that the node 1 may
            pass control flow to node 2 via return value."""
        nd1 = self._get_node_for_spec(node_spec1)
        nd2 = self._get_node_for_spec(node_spec2)
    ...
    def edges_from_string(self, edges_spec: str) -> None:
        """Input format yet unstable!
        
            Creates edges in the current graph, as specified by the spec 
            provided as string. Format expected is what comes out from
            edges_to_string(..., include_counts=False).
        """
    ...
    def edges_to_string(self, indent: str = "", include_counts=False):
        """Output format yet unstable!
        
            Returns a (graphviz) dot-format-like description of all edges.
            (If include_counts is True, also includes traversal count
            information; this output is instable; use for debug only.)
        """
        res = ""
    ...
    #def register_fallback_code(self, xx):
    #    pass
    ...
    def add_as_pipeline(self, node_funcs: list) -> None:
        """     
            :param node_funcs: a list of node functions
        ...
        for node_func in node_funcs:
            if isinstance(node_func, tuple):
                node_func, label = node_func
            else:
                # expect python function
                label = ""
            node = self._get_node_for_spec(node_func)
            if not node:
                self.add_node(node_func, label=label)
                node = self._get_node_for_spec(node_func)
            # Here have node and label
...
class GraphRunner:
    ...
    def __init__(self, *, 
                    ...
                    infer_structure: bool = False,              # <-- deprec
                    tracking_graph: Graph | None = None,
                    default_ttl: int | None = None, 
                    step_delay_secs: float | None = None,
                    propagate_node_exceptions: bool = False):
        """
        :param default_ttl: TTL value to attach to the state prior running the graph; None for "no limit".
        :type int | None:
        """
        self.enforce_structure = enforce_structure
        self.infer_structure = infer_structure
        ...
        while next_spec:
            ...
            try:
                ...
                if tgt_node == TgtNode.returned():
                    ...
                    next_spec = prev_nd
                    prev_nd = None  # TODO: review, or document decision
                    continue
                prev_nd = nd
                if tgt_node == TgtNode.start(): 
                    ##old: orig_start_nd = graph._get_node_for_spec(orig_start_spec)
                    if self.enforce_structure:
                ...
                if tgt_node == TgtNode.next():
                    ...
                    if next_spec == graph._nd_end: # ##003
                        return RunResult(RunResult.RC.END)
                    continue
                if True:
                    ...
                    if self.tracking_graph: self.tracking_graph._record_traversal(nd, next_spec, tgt_node)
                # graph structure commands and monitoring and checkpointing here
                continue
            except RunnerException as ex:
                raise ex    # re-raise
            except Exception as ex:
                print("E: Running node {}".format(nd.name))
                print("E:", str(ex))
                # If defined fallback mechanism for node, trigger it here
...
class GraphVisualization:
    ...
    def as_graphviz_obj(graph: Graph, comment=None):
        ...
        if graph.entry_point: 
            ...
            ##old: nd_entry_point = graph._get_node_for_spec(graph.entry_point)
...
if __name__ == "__main__":
    ...
    class MyNodes:
        ...
        def node1(state: ChatHistory, ctx: CfCtx) -> TgtNode: 
            # state is passed by reference, and results should be saved in it.
            # ctx should not be changed, but may be read from.
