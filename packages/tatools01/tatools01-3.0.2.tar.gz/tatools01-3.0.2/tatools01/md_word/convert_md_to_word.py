...
def md2w():
    ...
    parser = argparse.ArgumentParser(
        description='Chuyển đổi file Markdown (.md) thành file Word (.docx)',
...
Ví dụ:
  ...
  python convert_md_to_word.py D:/Documents/README.md
  python convert_md_to_word.py D:/Documents/README.md --autofit content
  python convert_md_to_word.py . -a window
'''
    )
    parser.add_argument('path', help='Đường dẫn file .md hoặc thư mục chứa file .md')
    parser.add_argument(
        '-a', '--autofit',
        choices=['content', 'window'],
        default='window',
        help='Chế độ autofit cho table: content (theo nội dung) hoặc window (100%% chiều rộng). Mặc định: window'
    )

    args = parser.parse_args()
    input_path = args.path
    autofit_mode = args.autofit
    path = Path(input_path)

    # Kiểm tra xem là file .md hay thư mục
    if path.is_file() and path.suffix.lower() == '.md':
        # Chuyển đổi một file cụ thể
        convert_single_file(input_path, autofit_mode)
    else:
        # Chuyển đổi tất cả file trong thư mục
        convert_all_md_in_directory(input_path, autofit_mode)
