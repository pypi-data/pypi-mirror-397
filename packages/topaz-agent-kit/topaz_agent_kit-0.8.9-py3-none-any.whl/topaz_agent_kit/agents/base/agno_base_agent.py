...
class AgnoBaseAgent(BaseAgent):
    ...
    async def _filter_mcp_tools(self) -> None:
        """Filter MCP tools based on agent's pipeline.yml configuration per server"""
        if not self.tools:
        ...
        for server in servers_config:
            ...
            server_tools_map[normalized_url] = {
                "patterns": server.get("tools", []),
                "toolkits": server.get("toolkits", []),
            }
        
        # Filter each MCPTools instance based on its server
        for tool in self.tools:
            if hasattr(tool, 'functions') and hasattr(tool, 'server_params'):
                # Get the server URL from the tool
                tool_url = tool.server_params.url.replace("/mcp", "")
                
                # Find matching server config
                server_entry = None
                for config_url, tools in server_tools_map.items():
                    if config_url in tool_url or tool_url in config_url:
                        server_entry = tools
                        break
                
                if server_entry:
                    patterns = server_entry["patterns"]
                    toolkits = server_entry["toolkits"]
                    ...
                    # Filter functions to only allowed ones for this server using wildcard matcher
                    filtered_functions = {
                        name: fn for name, fn in tool.functions.items()
                        if matches_tool_patterns(name, patterns, toolkits)
                    }
                    tool.functions = filtered_functions
                else:
                    self.logger.warning(f"No server config found for MCP tool at {tool_url}")
