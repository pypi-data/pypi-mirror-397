...
class FileUploadHandler:
    ...
    def _show_progress_bar(self, filename: str, progress: int = 0, file_size: int = 0, uploaded_size: int = 0):
        ...
        if file_size > 0:
            ...
            print(f"\r📁 Uploading {filename}... [{bar}] {progress}% ({uploaded_mb:.1f}MB/{size_mb:.1f}MB)", end="", flush=True)
        else:
            # Agent execution progress
            print(f"\r🔄 Processing {filename}... [{bar}] {progress}%", end="", flush=True)
    ...
    def _stop_progress_bar(self, filename: str, success: bool = True):
        ...
        if success:
            print(f"\r🔄 Processing {filename}... [████████████████████] 100% Complete")
        else:
            print(f"\r🔄 Processing {filename}... Failed")
        print()  # New line
