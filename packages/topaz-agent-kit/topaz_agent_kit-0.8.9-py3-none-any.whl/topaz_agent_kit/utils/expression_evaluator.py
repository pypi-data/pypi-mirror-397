...
class ExpressionEvaluator:
    ...
    def _resolve_variable(self, var_name: str) -> Any:
        ...
        if "." in var_name:
                    ...
                    if len(field_path) >= 2 and field_path[0] == "intermediate":
                        ...
                        if output_id in intermediate_dict:
                            ...
                            for field in remaining_path:
                                if isinstance(value, dict) and field in value:
                                    ...
                                    raise ValueError(f"Field not found: {var_name} (missing {field} in intermediate output)")
                            return value
                        else:
                            raise ValueError(f"Intermediate output '{output_id}' not found in pipeline '{first_part}'")
                        ...
                        if node_id in nodes_dict:
                            ...
                            for field in remaining_path:
                                if isinstance(value, dict) and field in value:
                                    ...
                                    raise ValueError(f"Field not found: {var_name} (missing {field} in node output)")
                            return value
                        else:
                            raise ValueError(f"Node '{node_id}' not found in pipeline '{first_part}'")
                    else:
                        ...
                        for field in field_path:
                            if isinstance(value, dict) and field in value:
                                ...
                                raise ValueError(f"Field not found: {var_name} (missing {field} in pipeline final output)")
                        return value
                ...
                if isinstance(node_data, dict) and "parsed" in node_data:
                    ...
                    for field in field_path:
                        if isinstance(value, dict) and field in value:
                            ...
                            raise ValueError(f"Field not found: {var_name} (missing {field} in path)")
                    return value
                else:
            ...
            if first_part in hitl:
                gate_result = hitl[first_part]
                if isinstance(gate_result, dict):
                    ...
                    for field in field_path:
                        if isinstance(value, dict) and field in value:
                            ...
                            raise ValueError(f"Field not found: {var_name} (missing {field} in gate result)")
                    return value
            ...
            if first_part in self.context:
                ...
                for field in field_path:
                    if isinstance(value, dict) and field in value:
                        ...
                        raise ValueError(f"Field not found: {var_name}")
                return value
