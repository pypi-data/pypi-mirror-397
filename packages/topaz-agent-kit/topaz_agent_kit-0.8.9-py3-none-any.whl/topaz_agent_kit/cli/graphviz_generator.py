...
class GraphvizGenerator:
            ...
            if is_enhanced_repeat_edge:
                ...
                if count < 5:
                    ...
                    if label and not is_dotted:
                        # ALL CAPS for all labels, including expressions
                        display_label = str(label).upper()
                        lines.append(
                            f'  "{from_node}" -> "{to_node}" ['
                            f'    label="{display_label}",'
                            f'    color="{arrow_color}",'
                            f'    fontcolor="{arrow_color}",'
                            "    fontsize=9,"
                            "    penwidth=2"
                            "  ];"
                        )
    ...
    def _process_step(
        ...
        if pattern_type in ("sequential", "parallel", "loop") and step.get("condition"):
            ...
            # FALSE path bypasses the entire pattern to next_node (or END)
            ...
            if first_node and first_node not in ["START", "END"]:
                edges.append((cond_node_label, first_node, "TRUE"))
        ...
        if "gate" in step:
            ...
            if gate_condition:
                ...
                # Store what comes after the gate for FALSE path
                next_real = next_node or "END"

                # Connect condition node to what comes after gate (FALSE path)
            ...
            if node_condition:
                ...
                # Store what comes after the node for FALSE path
                next_real = next_node or "END"

                # Connect condition node to what comes after node (FALSE path)
                # This bypasses the node
                edges.append((cond_node_label, next_real, "FALSE"))
            ...
            entry_label_parts = []
            if iterate_over:
                # Show what we're iterating over
            ...
            if termination_condition:
                ...
                if iterate_over:
                    ...
                    if termination_condition:
                        # Format termination condition for display (truncate if too long)
                        term_cond_display = termination_condition.upper()
                        if len(term_cond_display) > 50:
                            term_cond_display = term_cond_display[:47] + "..."
                    ...
                    if termination_condition:
                        # Format termination condition for display (truncate if too long)
                        term_cond_display = termination_condition.upper()
                        if len(term_cond_display) > 60:
                            term_cond_display = term_cond_display[:57] + "..."
                        loop_condition_label_parts.append(term_cond_display)
                ...
                if prev_node.startswith("COND: P"):
                    ...
                    # Include iterate_over/max_iterations info if available
                    if entry_label:
                        edges.append((prev_node, loop_cond_node, f"TRUE ({entry_label})"))
                    else:
                        edges.append((prev_node, loop_cond_node, "TRUE"))
                else:
                ...
                # FALSE path: exit loop to next_node (user requirement: "loop false edge should go to next node")
                next_real = next_node or "END"
                edges.append((loop_cond_node, next_real, "FALSE"))
                
                # TRUE path: enter loop body (when condition is TRUE - continue loop)
                ...
                if loop_entry_target and loop_entry_target not in ["START", "END"]:
                    edges.append((loop_cond_node, loop_entry_target, "TRUE"))
            ...
            if isinstance(body, dict):
                ...
                if last_node and last_node not in ["START", "END"]:
                    edges[:] = [e for e in edges if not (e[0] == last_node)]
                
                # Add loop-back edge from last node to loop entry point (control returns to beginning)
                # If we have a loop condition node, loop back to it; otherwise loop back to convergence point or first_node
                if termination_condition:
            ...
            for case_key, case_steps in cases.items():
                ...
                for i, case_step in enumerate(case_steps):
                    ...
                    if "node" in modified_step:
                        original_node = modified_step["node"]
                        node_id = original_node.split(":")[0]
                        new_node_id = f"{node_id}{case_suffix}"
                        protocol = (
                            original_node.split(":")[1]
                            if ":" in original_node
                            else "a2a"
                        )
                        modified_step["node"] = f"{new_node_id}:{protocol}"
            ...
            for i in range(num_instances):
                ...
                for j, nested_step in enumerate(nested_steps):
                    ...
                    if j < len(nested_steps) - 1:
                        ...
                        # Last step in sequence - no next, will connect to next_node later
                        nested_next = None
