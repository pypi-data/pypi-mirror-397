...
def generate_mock_buyers(cursor, count: int = 25) -> List[Dict[str, Any]]:
    ...
    countries = ["USA", "China", "Germany", "UK", "Japan", "Singapore", "Netherlands", "South Korea"]
    country_tlds = {
...
def generate_policy_pdf(
    ...
    policy_data = [
        ...
        ["", ""],
        ["Coverage Details", ""],
        ["Coverage Limit:", f"${coverage_limit:,.2f}"],
...
def generate_invoice_pdf(claim: Dict, output_path: Path) -> str:
    ...
    for i in range(num_items):
        ...
        subtotal += amount
        line_items.append([product, str(quantity), f"${unit_price:,.2f}", f"${amount:,.2f}"])
    
    # Adjust to match invoice amount
    adjustment = claim['invoice_amount'] - subtotal
    if abs(adjustment) > 0.01:
        line_items.append(["Adjustment", "", "", f"${adjustment:,.2f}"])
    
    line_items.append(["", "", "Total:", f"${claim['invoice_amount']:,.2f}"])
...
def generate_past_claim_history(
    ...
    for i in range(num_claims):
        ...
        # Random decision and status
        decision = random.choice(decisions)
        status = random.choice(statuses)
        
        # Risk score based on decision
        if decision == "approve":
