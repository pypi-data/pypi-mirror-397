...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="RFP topic (if not provided, will prompt interactively)"
    )
    ...
    if not topic:
        print("\n" + "=" * 70)
        print("RFP Topic Selection")
        print("=" * 70)
        print("\nPlease enter the RFP topic (e.g., 'Cloud Infrastructure', 'ERP System', 'Network Equipment'):")
        print("Examples:")
        print("  - Cloud Infrastructure")
        print("  - CRM Software")
        print("  - Data Center Equipment")
        print("  - Construction Services")
        print("  - Marketing Agency Services")
        print()
        topic = input("Topic: ").strip()
        if not topic:
            print("⚠ No topic provided. Using default: 'Cloud Infrastructure'")
    ...
    if sys.stdin.isatty():  # Only prompt if running interactively
        ...
        print("Press Enter to use this directory, or type a different path:")
        try:
        ...
        print(f"\nOutput directory: {output_dir} (non-interactive mode)")
    
    # Confirm supplier count
    supplier_count = args.supplier_count
    if sys.stdin.isatty():  # Only prompt if running interactively
        print(f"\nNumber of supplier responses to generate: {supplier_count}")
        print("Press Enter to use this count, or type a different number:")
        try:
            user_count = input("> ").strip()
            if user_count:
                try:
                    supplier_count = int(user_count)
                    if supplier_count < 1:
            ...
            print("\n⚠ Using default supplier count")
    else:
        print(f"\nSupplier responses: {supplier_count} (non-interactive mode)")
