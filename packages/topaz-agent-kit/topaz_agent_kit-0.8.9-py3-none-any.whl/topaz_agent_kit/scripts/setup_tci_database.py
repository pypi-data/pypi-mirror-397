...
def main():
    ...
    # Allow interactive override of insurance company name
    insurance_company_name = args.insurance_company_name
    if not args.insurance_company_name or args.insurance_company_name == "Export Credit Guarantee Corporation of India":
        try:
            user_input = input(f"\nInsurance Company Name [{insurance_company_name}]: ").strip()
            if user_input:
                insurance_company_name = user_input
        except (EOFError, KeyboardInterrupt):
            # Use default if input is not available (non-interactive mode)
            pass
