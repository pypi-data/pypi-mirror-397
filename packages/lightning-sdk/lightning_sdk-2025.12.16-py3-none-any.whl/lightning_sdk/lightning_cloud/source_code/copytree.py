...
from lightning_sdk.lightning_cloud.env import DOT_IGNORE_FILENAME

logger = logging.getLogger(__name__)


def copytree(src: Union[Path, str],
             dst: Union[Path, str],
             ignore_functions: List[Callable] = None,
             dirs_exist_ok=False,
             dry_run=False) -> List[str]:
    """
    Vendor in from `shutil.copytree` to support ignoring files recursively
    based on `.lightningignore`, like `git` does with `.gitignore`. Also
    removed a few checks from the original copytree related to symlink
    checks. Differences between original and this function are
...
def get_ignore_function(src: Path) -> Callable:
    ...
    def filter_ignored(current_dir: Path, entries: List[Path]) -> List[Path]:
        ...
        return [
            ...
            if str(relative_dir / entry.name) not in ignored_names_set
        ]
...
def _ignore_filename_spell_check(src: Path):
    ...
    possible_spelling_mistakes.extend(
        [p.lstrip(".") for p in possible_spelling_mistakes])
