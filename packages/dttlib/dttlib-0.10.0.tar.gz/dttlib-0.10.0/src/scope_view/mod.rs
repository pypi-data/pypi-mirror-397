...
impl ScopeView {
    ...
    async fn results_loop(
        ...
        if let Err(e) = rc.output_handle.send(rc.clone(), ScopeViewDone { id }) {
            log::debug!("error sending ScopeViewDone({}): {}", id, e);
        }
    }
...
impl ScopeViewHandle {
    ...
    pub(crate) fn new_online(
        ...
        let command_tx = ScopeView::create(rc, data_source, id, set, span, true, false);
        Self { id, command_tx }
    }
    ...
    pub(crate) fn new_snapshot(
        ...
        let span = ViewSpan::new_fixed(start_pip, end_pip);
        let id = NEXT_SCOPE_VIEW_ID.fetch_add(1, Ordering::Relaxed);
        let command_tx = ScopeView::create(rc, data_source, id, set, span, false, true);
        Self { id, command_tx }
    }
