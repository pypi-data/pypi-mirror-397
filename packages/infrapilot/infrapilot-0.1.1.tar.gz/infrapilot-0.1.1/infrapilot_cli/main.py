...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if not logged_in:
            console.print("[red]Authentication is required. Exiting.[/]")
            return 1
        ...
        if backend_reachable and logged_in:
            _bootstrap_user_context(console, config_store, backend_client, logger)

        if backend_reachable and logged_in and not bool(config_store.get("onboarding_complete")):
