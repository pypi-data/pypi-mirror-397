...
def test_slugify_and_titleize_strip_number_prefix():
    # Number prefixes with underscore
    assert utils.slugify("01_introduction") == "introduction"
    assert utils.slugify("2_getting-started") == "getting-started"
    assert utils.titleize("01_introduction.md") == "Introduction"
    assert utils.titleize("2_getting-started.md") == "Getting Started"
    # Number prefixes with hash
    assert utils.slugify("01#chapter-one") == "chapter-one"
    assert utils.titleize("01#chapter-one.md") == "Chapter One"
    # Number prefixes with hyphen
    assert utils.slugify("03-section-three") == "section-three"
    assert utils.titleize("03-section-three.md") == "Section Three"
    # Without number prefix (should remain unchanged)
    assert utils.slugify("no-prefix") == "no-prefix"
    assert utils.titleize("no-prefix.md") == "No Prefix"
