...
class MySQLDatabase(object):
	...
	def insert(self, table, records):
		...
		if records_insert: 	
			...
			if 'ID' in field_names:
				records_with_ID		= [r for r in records_insert if 'ID' in r and r['ID']]
				records_without_ID	= [r for r in records_insert if 'ID' not in r or not r['ID']]
				records_insert		= records_with_ID+records_without_ID
				insert_max_ID		= max([r['ID'] for r in records_with_ID])
			...
			# 先查询最大ID
			max_ID_before	= self.table_max_ID(table)
			...
			# 更新redis
			self.update_redis(table, max_ID=max_ID_before)

			# 再查询最大ID
			max_ID_after	= self.table_max_ID(table)

			# 计算插入的ID
			inserted_ID_list	= list(range(max(max_ID_before,insert_max_ID), max_ID_after+1))
	...
	## 查询表的最大ID
	def table_max_ID(self, table):
		result	= self.query_single(f"SELECT MAX(ID) FROM `{table}`")
		try:
			return int(result['MAX(ID)'])
		except Exception:
