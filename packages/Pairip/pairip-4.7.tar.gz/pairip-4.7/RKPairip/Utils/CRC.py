...
def CRC_Fix(M_Skip, apk_path, build_dir, file_types):
    ...
    if M_Skip:
        ...
        LD = f"classes{int(
            sorted(
                [
                    classesX.filename
                    for classesX in M.zipfile.ZipFile(build_dir, 'r').infolist()
                    if classesX.filename.endswith('.dex')
                ]
            )[-1]
            .split('classes')[1]
            .split('.dex')[0]
        ...
        input(
            f"\n{C.FYI} {C.C} Now Script is Listen Mode, Cuz The value of your Last Dex {C.Y}{LD} {C.C}Field/method is greater than 65536., So you can do Max Value Dex Redivision {C.PN}( 65536 ) {C.G}using MT/ApkTool_M then correct the name of the APK again and then press enter in the script, which will bypass CRC )\n\n"
            f"\n{C.S} CRC Fix {C.E} {C.C} Press Enter to After Dex Redivision & Should Apk Name is {C.Y}{M.os.path.basename(build_dir)} ...\n"
        )
    ...
    for entry in Logs:
        print(f"\n{C.G}{entry[0]:<22}{entry[1]}{'':<4}{entry[2]}{'':<4}{entry[4]}\n")
