...
class SQLAlchemyGraphQLAsyncRepository(SQLAlchemyGraphQLRepository[DeclarativeT, AnyAsyncSession]):
    ...
    async def _update_to_many_relations(self, data: Input[DeclarativeT], mutated_ids: Sequence[RowLike]) -> None:
        ...
        for level in data.filter_by_level(RelationType.TO_MANY, ["set"]):
            ...
            for model_type_or_table, set_values in params.insert_m2m.items():
                if current_ids := params.delete_m2m[model_type_or_table]:
                    ...
                    await self.session.execute(remove_previous_stmt)
    ...
    async def _create_to_many_relations(self, data: Input[DeclarativeT], mutated_ids: Sequence[RowLike]) -> None:
