...
class MetaboliticsAnalysis:
    ...
    def update_bound(self, active_node, reactions, toy_graph):
        ...
        dosya_adi = "output_breast_equal_val.txt"
        count = 0
                    ...
                    for gpr_list in toy_graph.get_vertex(react).get_vert_info()['gpr']:
                        ...
                        for gene in gpr_list:
                            if gene not in act_gens_uniprot:
                                ...
                                break
                        if valid_gene:
                            hold.append(min((active_node[key + '_protein'] / len(set(i.get_end_vertex().get_id() for i in toy_graph.get_vertex(key + '_protein').get_outbound_edges() if toy_graph.get_vertex(i.get_end_vertex().get_id()).get_omic_type() == 'R')))
                                            for key in gpr_list if key + '_protein_x' in active_node or key + '_protein' in active_node))
                    value = max(hold)
                    if value == 0:
                        count += 1
