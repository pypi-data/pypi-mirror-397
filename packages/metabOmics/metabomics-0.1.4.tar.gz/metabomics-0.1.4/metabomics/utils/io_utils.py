...
def load_metabolite_mapping(naming_file='synonym', dataset = [], dataset_name=''):
    ...
    if (naming_file == 'synonym' or naming_file == 'new-synonym') and (len(dataset) > 0):
        ...
        try:
            with open('%s/naming/refmet_recon3d.json' % DATASET_PATH) as f:
            ...
            print(e)
        with open('%s/naming/%s-mapping.json' % (DATASET_PATH, naming_file), 'w') as f: 
        ...
        if dataset_name != '':
            ...
            test = pd.read_csv('%s/naming/test.csv' % DATASET_PATH)
