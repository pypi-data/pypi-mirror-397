...
class MetaboliticsAnalysis:
    ...
    def update_bound(self, active_node, reactions, toy_graph):
        ...
        dosya_adi = "output_breast_first_trig_allSteps.txt"
        count = 0
    ...
    def variability_analysis(self, measured_metabolites, x_tr):
        ...
        self.set_objective(measured_metabolites)
        #dosya_adi = "X_diff_pdac_first_trig.txt"
        count = 0
        # Dosyayı ekleme kipiyle aç
        #with open(dosya_adi, 'a') as dosya:
        #    dosya.write("Prosessing sample : {} - {}\n".format(str(self.sample_count), str(datetime.now())))
                        ...
                        for edge in graph1.get_vertex(node).get_edges():
                            if edge.get_start_vertex().get_id() in activated_neighbors:
                                if edge.get_int_info()["interaction"]["0"] == 'Repression':
                                    val_rep.append(edge.get_start_vertex().get_vert_info()['value'])
                                else :
                                    val_act.append(edge.get_start_vertex().get_vert_info()['value'])
                        if val_act and val_rep:
                            val = np.mean(val_act) - np.mean(val_rep)
                        elif val_act:  # val_rep boşsa
                            val = np.mean(val_act)  # veya başka bir mantıklı varsayılan değer
                        elif val_rep:  # val_act boşsa
                            val = np.mean(val_rep)
                        else :
                            val = 0
                        activated[node] = val
                        graph1.get_vertex(node).set_vert_info({'value': val})
                        new_nodes_activated = True
