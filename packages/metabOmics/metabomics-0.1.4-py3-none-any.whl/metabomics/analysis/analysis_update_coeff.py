...
class MetaboliticsAnalysis:
    ...
    def set_objective(self, measured_metabolites, active_node, reactions, toy_graph):
        '''
        ...
        dosya_adi = "output_pdac_first_trig_logReg_update_coeff_noMLpipeline.txt"
        count = 0
        ...
        with open(dosya_adi, 'w') as dosya:
            for react in reactions:
                try:
                    ...
                    value = max(hold)
                    r = self.model.reactions.get_by_id(react)
                    r.objective_coefficient += value
    ...
    def update_bound(self, active_node, reactions, toy_graph):
        ...
        dosya_adi = "output_pdac_first_trig_up_coeff.txt"
        count = 0
        # Dosyayı ekleme kipiyle aç
        with open(dosya_adi, 'w') as dosya:
            # Print ile yazdırma
    ...
    def variability_analysis(self, measured_metabolites, x_tr):
        ...
        self.set_objective(measured_metabolites, active_node, reactions, G)
                    ...
                    if len(activated_neighbors) > 0:
                        ...
                        if influence < 0.25:
                            ...
                            if val == 0:
                                ...
                                activated[node] = pred
                                graph1.get_vertex(node).set_vert_info({'value': pred})
                                new_nodes_activated = True
