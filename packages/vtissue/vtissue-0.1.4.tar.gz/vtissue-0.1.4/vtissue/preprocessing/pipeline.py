...
def preprocess_dataset(
    ...
    for i, adata in enumerate(adatas):
        ...
        global_expr, mask = mapper.create_global_matrices(adata)
...
def preprocess_and_save(
    ...
    for idx, item in enumerate(inputs, start=1):
        ...
        if skip_mapping:
            ...
            global_expr, mask = mapper.create_global_matrices(adata, layer=map_layer)
            new_adata = ad.AnnData(
                X=global_expr,
                obs=adata.obs.copy()
            )
            ...
            for lname in list(adata.layers.keys()):
                ...
                expr_l, _mask_l = mapper.create_global_matrices(adata, layer=lname)
...
def assign_patches(adata: ad.AnnData, config: dict) -> None:
    ...
    if 'patch_id' not in adata.obs.columns:
        ...
        try:
            ...
            pass
    unique_images = adata.obs[image_col].astype(str).unique()
    for img_idx, img_id in enumerate(unique_images):
        ...
        patch_names = np.array([f"img{img_idx}_p{int(a)}_{int(b)}" for a, b in zip(ix, iy)], dtype=object)
        row_index = adata.obs.index[mask_img]
        adata.obs.loc[row_index, 'patch_id'] = patch_names
