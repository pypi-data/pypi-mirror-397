...
class GlobalGeneMapper:
    ...
    def map_genes(self, panel_genes: List[str]) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        return np.array(panel_idxs, dtype=int), np.array(global_idxs, dtype=int)
    
    def create_global_matrices(self, adata, layer: Optional[str] = None) -> Tuple[sparse.csr_matrix, sparse.csr_matrix]:
        """
        ...
        panel_idxs, global_idxs = self.map_genes(panel_genes)
