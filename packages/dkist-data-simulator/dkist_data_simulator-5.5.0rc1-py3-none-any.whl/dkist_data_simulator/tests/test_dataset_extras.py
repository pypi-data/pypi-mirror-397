...
@pytest.mark.parametrize(
    "dataset_extra_name",
    [
        pytest.param(DatasetExtraName.dark, id="dark"),
...
@pytest.mark.parametrize(
    "keep_schemas",
    [
        pytest.param([DatasetExtraTables.ip_task], id="iptask_schema"),
        pytest.param(None, id="no_schema"),
    ],
)
def test_dataset_extra_base(dataset_extra_name, keep_schemas):
    """
    Given: instrument schema with a schema table passed to keep
    When: generating a default size dataset extra Dataset with that schema
    Then: the correct keys are present and the incorrect keys are not present
    """

    class TestDatasetExtraSchema(DatasetExtraSchema):
        def __init__(self, *args, **kwargs):
            super().__init__(
                instrument=InstrumentTables.visp, keep_schemas=keep_schemas, **kwargs
            )

    ds = DatasetExtraBase(
        file_schema=TestDatasetExtraSchema(), dataset_extra_name=dataset_extra_name
    )
...
def test_instrument_dataset_extras(dataset_extra_cls):
    """
    Given: an instrument
    ...
    for d in ds:
        header = d.header()
        if ds.file_schema.instrument:
            assert header["INSTRUME"] == ds.file_schema.instrument
        else:
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(all_visp_dataset_extras, 51, id="visp"),
        pytest.param(all_cryo_dataset_extras, 32, id="cryonirsp"),
        pytest.param(all_dlnirsp_dataset_extras, 24, id="dlnirsp"),
    ],
)
def test_instrument_dataset_extras_combined(all_extras_function, total_files, tmpdir):
    """
    Given: an instrument
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(CryoDarkExtra, "CNBEAM", id="cryo_multi_beam"),
        pytest.param(VispDarkExtra, "VSPBEAM", id="visp_multi_beam"),
    ],
)
def test_dataset_extra_index_beam(dataset_extra_cls, key):
    """
    Given: an instrument with multiple beams
    When: making a dataset extra that has a separate file for each beam
    Then: the beam id keywords are set correctly
    """
    ds = dataset_extra_cls()
    assert ds.n_files == 2
