...
def child_process_cleanup(wait_time_s: int = 3) -> dict:
    """
    ...
    """
    # Get the list of all known children.
    ...
    for p in alive:
        click.echo(
            "Process {proc} could not be terminated normally. Forcibly ending it...".format(
                proc=p
            )
        )
        p.kill()
...
atexit.register(child_process_cleanup)
