...
def run_optimization_job(
    ...
    def progress_callback(step: str, progress: int, details: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if thinking_content and isinstance(thinking_content, str) and thinking_content.strip():
                thinking_history = {step: {"content": thinking_content, "timestamp": datetime.utcnow().isoformat()}}
            else:
                # Even without thinking content, add the step
                thinking_history = {step: {"content": f"**{step.replace('_', ' ').title()}**\n\nStep completed.", "timestamp": datetime.utcnow().isoformat()}}
    ...
    try:
        ...
        # Signal metrics completion (all metrics have been published)
        try:
        ...
        # Wait for metrics aggregation to complete (with timeout)
            ...
            try:
                ...
                if 'function' in error_msg.lower() or 'callable' in error_msg.lower():
                    ...
                    try:
                        ...
                        essential_fields = [
                            'workflow_stage', 'current_step', 'completed_steps',
        ...
        if serialization_failed:
            ...
            logger.warning(f"Returning simplified result for job {job_id} due to serialization issues")
